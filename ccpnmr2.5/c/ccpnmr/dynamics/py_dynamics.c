...
static struct PyModuleDef DyDynamics_module_def =
{
    PyModuleDef_HEAD_INIT,
    "DyDynamics",
    "CCPNMR DyDynamics module (Python 3 compatible)",
    -1,
    Dynamics_type_methods
};

PyMODINIT_FUNC PyInit_DyDynamics(void)
{
    if (PyType_Ready(&Dynamics_type) < 0)
        return NULL;

    PyObject *m = PyModule_Create(&DyDynamics_module_def);
    if (!m)
        return NULL;
