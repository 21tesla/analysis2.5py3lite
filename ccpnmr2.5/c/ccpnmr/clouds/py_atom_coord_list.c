...
static struct PyModuleDef AtomCoordList_module_def =
{
    PyModuleDef_HEAD_INIT,
    "AtomCoordList",
    "CCPNMR AtomCoordList module (Python 3 compatible)",
    -1,
    Atom_coord_list_type_methods
};

PyMODINIT_FUNC PyInit_AtomCoordList(void)
{
    if (PyType_Ready(&Atom_coord_list_type) < 0)
        return NULL;

    PyObject *m = PyModule_Create(&AtomCoordList_module_def);
    if (!m)
        return NULL;
