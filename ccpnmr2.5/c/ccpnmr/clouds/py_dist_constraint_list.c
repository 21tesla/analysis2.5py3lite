...
static struct PyModuleDef DistConstraintList_module_def =
{
    PyModuleDef_HEAD_INIT,
    "DistConstraintList",
    "CCPNMR DistConstraintList module (Python 3 compatible)",
    -1,
    Dist_constraint_list_type_methods
};

PyMODINIT_FUNC PyInit_DistConstraintList(void)
{
    if (PyType_Ready(&Dist_constraint_list_type) < 0)
        return NULL;

    PyObject *m = PyModule_Create(&DistConstraintList_module_def);
    if (!m)
        return NULL;
