...
static PyObject *repr_py_atom_coord(PyObject *self)
{
    ...
    char buf[64];
    snprintf(buf, sizeof buf, "<mass=%.2e, x=%.2e, y=%.2e, z=%.2e>",
		atom_coord->mass, atom_coord->x, atom_coord->y, atom_coord->z);
...
static PyTypeObject Atom_coord_type =
{
    ...
    "AtomCoord -- atom coordinate",           /* tp_doc */
    0,                                        /* tp_traverse */
    0,                                        /* tp_clear */
    0,                                        /* tp_richcompare */
    0,                                        /* tp_weaklistoffset */
...
static struct PyModuleDef atom_coord_module_def =
{
    PyModuleDef_HEAD_INIT,
    "AtomCoord",
    "CCPNMR AtomCoord module (Python 3 compatible)",
    -1,
    Atom_coord_type_methods
};

PyMODINIT_FUNC PyInit_AtomCoord(void)
{
    if (PyType_Ready(&Atom_coord_type) < 0)
        return NULL;

    PyObject *m = PyModule_Create(&atom_coord_module_def);
    if (!m)
        return NULL;
