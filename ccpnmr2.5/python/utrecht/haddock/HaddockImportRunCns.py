...
class runCnsImporter:
    ...
    def createNewRun(self):
        ...
        for param in self.formattedparams:
            if param not in self.energyStores:
    ...
    def formatSingleParams(self):
        ...
        for param in self.parameters:
            if param in self.rawparams: self.formattedparams[self.parameters[param]] = self.rawparams[param]
    ...
    def formatEnergyStores(self):
        ...
        for energyStore in self.energyStores:
            if energyStore == 'rdcProtocolStore':
                formatted = {}; count = 1
                while not count == 6:
                    if 'rdc%i_choice' % count in self.rawparams:
                ...
                for param in DEFAULT_SCORE:
                    ...
                    for stage in [0,1,2]:
                        ...
                        if key in self.rawparams: stagelist.append(float(self.rawparams[key]))
                        else:
                ...
                for param in formatted:
                    if param in self.rawparams: formatted[param] = float(self.rawparams[param])
