...
def writeBlockData(file, data, npoints, blockSizes=None):
    ...
    if npts != len(data):
        raise Exception("npoints = %s so should have len(data) = %d but it is %d" % (npoints, npts, len(data)))

    if not blockSizes:
        blockSizes = determineBlockSizes(npoints)

    if len(blockSizes) != ndim:
        raise Exception("len(npoints) = %s but len(blockSizes) = %d" % (ndim, len(blockSizes)))
