...
def nonlinearFit(
    x, y, a, func, w=None, noise=None, findDev=False, maxIteration=MAX_ITERATION, maxCondition=MAX_CONDITION
):
    ...
    while iter < maxIteration and cond < maxCondition:
        old_chisq = chisq
        (a, lmbda, chisq, alpha, beta, yFit) = _nonlinearModel(
            x, y, a, func, w=w, lmbda=lmbda, chisq=chisq, alpha=alpha, beta=beta, yFit=yFit
        )
        if chisq > old_chisq:
            cond = 0
