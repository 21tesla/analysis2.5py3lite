...
class LinkChart(Frame):
    def __init__(self, parent, scrollX=True, scrollY=True, *args, **kw):
        ...
        alignItems = [
            {"kind": "command", "label": "Y axis", "command": self._alignHorizontal},
            {"kind": "command", "label": "X axis", "command": self._alignVertical},
        ]

        distributeItems = [
            {"kind": "command", "label": "Horizontal", "command": self._distributeHorizontal},
            {"kind": "command", "label": "Verical", "command": self._distributeVertical},
        ]

        menuItems = [
            {"kind": "cascade", "label": "Align", "submenu": alignItems},  # 0
            {"kind": "cascade", "label": "Distribute", "submenu": distributeItems},  # 1
            {"kind": "command", "label": "Link Selected", "command": self.linkSelected},  # 2
            {"kind": "command", "label": "Reset Links", "command": self._resetLinks},  # 3
            {"kind": "separator"},
            {
                "kind": "command",
            ...
            {"kind": "separator"},
            {"kind": "command", "label": "Save PostScript File", "command": self.printCanvas},
        ]
    ...
    def _mouseDrag(self, event):
        ...
        if objs:
            ...
            if not inMotion:
                for item in items:
                    if self.inMotion2 or item in self.cornerDict:
                        obj = self.node
            ...
            if self.dragRegion:
                ...
                item = canvas.create_rectangle(x0, y0, x, y, outline="#008000", fill="#00E000", stipple="gray25")
...
class Node:
    ...
    def _getOutlineCoords(self, thickness, width, cx, cy):
        ...
        if self.shape == CIRCLE:
            ...
            outlineCoords = (x3, y1, x4, y2, x5, y3, x4, y4, x3, y5, x2, y4, x1, y3, x2, y2)
    ...
    def draw(self, coords=None):
        ...
        outlineCoords, inlineCoords = self._getOutlineCoords(thickness, self.width, cx, cy)
        ...
        if self.canvasItems:
            ...
            outline = create(width=thickness, outline=colorA, fill=colorA, *outlineCoords)

            inline = create(width=thickness, outline=colorB, fill=color, *inlineCoords)

            box = canvas.create_rectangle(x1, y1, x2, y2, width=thickness, outline=colorC, fill=fillColor)

            corner = canvas.create_polygon(x2, y2 - c, x2, y2, x2 - c, y2, outline=colorC, fill=colorC)

            title = canvas.create_text(0, 0, text=self.title, font=self.titleFont, fill=textColor, anchor="nw")

            text = canvas.create_text(0, 0, text=self.text, font=self.mainFont, fill=textColor, anchor="nw")
...
class Link:
    ...
    def draw(self):
        ...
        if self.canvasItems:
            ...
            canvas.itemconfigure(line, fill=lineColor, width=thickness, smooth=smooth)
            ...
            line = canvas.create_line(
                x1, y1, x2, y2, x3, y3, smooth=smooth, fill=lineColor, width=thickness, splines=25
            )
            text = canvas.create_text(x0, y0, text=self.text, fill=lineColor, anchor="center")

            xA, yA, xB, yB = canvas.bbox(text)
            box = canvas.create_rectangle(xA, yA, xB, yB, outline=lineColor, fill=fillColor)
...
if __name__ == "__main__":
    ...
    editWidget = EditWidget(tkWidget, linkChartB, setupCallback=setupFunc, commitCallback=commitFunc)
    ...
    for x in range(len(SHAPES)):
        ...
        if nodes:
            prev = nodes[-1]
            link = Link(linkChartB, node, prev, text="%s-%s" % (prev.title[-1], node.title[-1]))
    ...
    link = Link(linkChartB, nodes[0], nodes[2], text="%s-%s" % (1, 3), coords=(320, 50))
