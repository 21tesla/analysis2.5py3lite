...
class ScrolledGraph(Frame):
    ...
    def __init__(
        ...
        **kw,
        ...
        self.scrolledCanvas = ScrolledCanvas(self, borderwidth=2, relief=relief, width=width + 100, height=height + 100)
    ...
    def configMenu(self):
        ...
        zoom_items = [
            {"kind": "command", "label": "In", "command": lambda event: self.setZoom(self.zoom * 1.2)},
            {"kind": "command", "label": "Out", "command": lambda event: self.setZoom(self.zoom / 1.2)},
            {"kind": "command", "label": "Reset", "command": lambda event: self.setZoom(1.0)},
        ]

        type_items = [
            {"kind": "command", "label": "Line", "command": lambda event: self.setGraphType("line")},
            {"kind": "command", "label": "Scatter", "command": lambda event: self.setGraphType("scatter")},
            {"kind": "command", "label": "Bar chart", "command": lambda event: self.setGraphType("bar")},
            {"kind": "command", "label": "Histogram", "command": lambda event: self.setGraphType("histogram")},
        ]

        symboltype_items = [
            {"kind": "command", "label": "Circle", "command": lambda event: self.setSymbols("circle")},
            {"kind": "command", "label": "Square", "command": lambda event: self.setSymbols("square")},
            {"kind": "command", "label": "Triangle", "command": lambda event: self.setSymbols("triangle")},
            {"kind": "command", "label": "Star", "command": lambda event: self.setSymbols("star")},
        ]

        symbolsize_items = [
            {"kind": "command", "label": "+", "command": lambda event: self.setSymbolSize(self.symbolSize + 1)},
            {"kind": "command", "label": "-", "command": lambda event: self.setSymbolSize(max(self.symbolSize - 1, 1))},
            {"kind": "command", "label": "Small", "command": lambda event: self.setSymbolSize(3)},
            {"kind": "command", "label": "Normal", "command": lambda event: self.setSymbolSize(5)},
            {"kind": "command", "label": "Big", "command": lambda event: self.setSymbolSize(7)},
        ]
        ...
        items = [
            ...
            {"kind": "cascade", "label": "Graph type", "submenu": type_items},
            {"kind": "cascade", "label": "Symbol type", "submenu": symboltype_items},
            {"kind": "cascade", "label": "Symbol size", "submenu": symbolsize_items},
            {"kind": "command", "label": "Set Title", "command": self.setGraphTitle},
            {"kind": "command", "label": "Print to file", "command": self.scrolledCanvas.printCanvas},
        ]
    ...
    def mouseDrag(self, event):
        ...
        if inMotion or obj in itemDict:
            plotRegion = self.getPlotRegion()
    ...
    def mouseEnter(self, event):
        ...
        if (x < x0) or (x > x1) or (y < y1) or (y > y0):
            ...
            if crosshairText:
                ...
                self.crosshairText = canvas.create_text(
                    x0 + (0.9 * (x1 - x0)), y0 + y1, text=coordStr, fill=self.fgColor
                )
    ...
    def drawBoxes(
        self, plotRegion, coords, size, outline="#000000", fill="#808080", setSel=0, numSets=1, graphType="bar"
    ):
    ...
    def drawSymbols(self, coords, size, lineWidth, shape="circle", outline="#000000", fill="#808080"):
        ...
        if shape == "triangle":
            ...
            for x, y, yU, yL in coords:
                ...
                item = create_polygon(
                    x0, y0, x2, y2, x4, y4, x1, y1, x3, y3, outline=outline, fill=fill, width=lineWidth
                )
                itemsAppend(item)
    ...
    def draw(self):
        ...
        for dataSet in dataSets:
            ...
            if self.graphType == "line":
                ...
                items = self.drawSymbols(coords, symbolSize, lineWidth, shape=symbol, outline=color, fill=fcolor)
                self.items.extend(items)
            elif self.graphType == "scatter":
                items = self.drawSymbols(coords, symbolSize, lineWidth, shape=symbol, outline=color, fill=fcolor)
                self.items.extend(items)
            else:
                items = self.drawBoxes(
                    plotRegion,
                    coords,
                    size,
    ...
    def drawAxes(self, plotRegion, dataRegion, lineWidth):
        ...
        background = self.canvas.create_rectangle(x0, y0, x1, y1, outline="#FFFFFF", fill="#FFFFFF", width=lineWidth)
        ...
        items.append(self.canvas.create_rectangle(x0, y0, x1, y1, outline=self.mgColor, width=lineWidth))
        items.append(self.canvas.create_line(x0, y0, x1, y0, fill=self.fgColor, width=lineWidth))
        items.append(self.canvas.create_line(x0, y0, x0, y1, fill=self.fgColor, width=lineWidth))
    ...
    def drawLegend(self, plotRegion):
        ...
        for dataName in self.dataNames:
            ...
            if dataName is not None:
                ...
                if graphType == "line":
                    symbol = symbols[i]
                    items0 = drawSymbols(coordsA, symbolSize, lineWidth, shape=symbol, outline=color, fill=fcolor)
