...
class TabbedFrame(Frame):
    def __init__(
        ...
        **kw,
        ...
        self.canvas = Tkinter.Canvas(self, highlightbackground=self.bg, background=self.bg, width=1, height=1)
        self.canvas.grid()
    ...
    def setTabs(self, options, selected=None, tipTexts=None, docKeys=None):
        ...
        for i in range(self.numTabs):
            ...
            frame = Frame(self, background=self.bg, docKey=docKeys[i], tipText=tipTexts[i], createToolTip=False)
    ...
    def draw(self, w, h):
        ...
        for i in range(self.numTabs):
            ...
            if i == self.selected:
                ...
                yf = h0 + bw + th + 2
                frame.place(
                    x=xf, y=yf, anchor="nw", width=w - (xf + xf), height=max(0, h - (yf + xf)), bordermode="ignore"
                )
                ci(text, fill="black")
        ...
        x += bw
        self.sideFrame.place(x=x, y=0, anchor="nw", bordermode="ignore", width=w - x, height=t1)
