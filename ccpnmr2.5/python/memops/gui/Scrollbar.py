...
class Scrollbar(Frame):
    def __init__(
        ...
        **kw,
        ...
        self.arrow1Tl = c.create_polygon(0, 0, 0, 0, 0, 0, 0, 0, fill=color3, outline="")
        self.arrow1Br = c.create_polygon(0, 0, 0, 0, 0, 0, 0, 0, fill=color4, outline="")
        self.arrow1Fill = c.create_polygon(0, 0, 0, 0, 0, 0, fill=bg, outline="")

        self.arrow2Tl = c.create_polygon(0, 0, 0, 0, 0, 0, 0, 0, fill=color3, outline="")
        self.arrow2Br = c.create_polygon(0, 0, 0, 0, 0, 0, 0, 0, fill=color4, outline="")
        self.arrow2Fill = c.create_polygon(0, 0, 0, 0, 0, 0, fill=bg, outline="")
    ...
    def set(self, lo, hi, do_callback=False):
        ...
        if (abs(lo - self.lo) < min_delta) and (abs(hi - self.hi) < min_delta):
            return
        ...
        if self.min_thickness and (hi - lo) < 0.1 * self.min_thickness:
            ...
            if ((self.hi - self.lo) > (self.min_thickness + min_delta)) or (
                (hi - lo) > (self.hi - self.lo + min_delta)
            ):
                a = 0.5 * (lo + hi)
