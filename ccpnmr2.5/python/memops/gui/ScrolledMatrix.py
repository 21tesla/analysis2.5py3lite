...
from memops.gui.Color import invertColor, invertColorRgb, scaleColor
...
from memops.universal.Util import buttonMotion, buttonPress, useWheelMouse

float_re = re.compile(r"\s*(-?\d+\.*\d*|\d*\.*\d+)")
numStr_re = re.compile(r"(\D*)(-?\d+\.*\d*|\d*\.\d+)(.*)")
...
stringType = str
unicodeType = str
months = (None, "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")  # Months start at 1
...
class ScrolledMatrix(Frame):
    def __init__(
        ...
        **kw,
        ...
        self.horizScrollbar = Scrollbar(
            self,
            orient=Tkinter.HORIZONTAL,
            background=cellBg,
            troughcolor=headingBg,
            width=self.scrollbarWidth,
            borderwidth=1,
            callback=self.moveHScrollbar,
        )
    ...
    def getGraphMenuOptions(self):
        ...
        for opt in opts:
            ...
            for optY in opts:
                ...
                yItems.append(
                    {"kind": "command", "label": nameY, "command": lambda x=opt, y=optY: self.selectGraph(x, y)}
                )
            ...
            if self.multiSelect and (event.state & 4 or event.state & 1):
                ...
                if row < len(self.objectList):
                    ...
                    if typ not in HEADING_CELL_ITEMS:
                        ...
                        if (
                            (self.editWidgets and self.editSetCallbacks) or self.editGetCallbacks
                        ) and self.currentObject is not None:
        ...
        if headingLine:
            ...
            for i in range(N):
                ...
                if (i >= len(textHeading)) or not textHeading[i]:
                    ...
                    filterMk = createText(
                        x2 - 7, y - 7, text="?", font=smallFont, fill=headingOutline, justify="center"
                    )
        ...
        for i in range(visibleRows):
            ...
            for j in range(N):
                ...
                if (j >= len(cellTextMatrix[i])) or not cellTextMatrix[i][j]:
                    textItem = createText(
                        x + pad, y + pad, text=text, font=font, fill=cellFg, justify=justify, anchor=anchor
                    )
                    bbox = cbbox(textItem)
                ...
                if text is None:
                    itconf(textItem, fill=headingFg, font=font, text=blank, justify=justify, anchor=anchor)
                else:
                    itconf(textItem, fill=headingFg, font=font, text=text, justify=justify, anchor=anchor)
    ...
    def refreshScrollbars(self, reset=False):
        ...
        if useHoriz:
            ...
            if useVert:
                self.vertScrollbar.place(
                    x=width - size, y=self.headHeight + 1, width=size, height=height - self.headHeight - 2
                )
...
class GraphMatrixPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        texts = ["Zoom In", "Zoom Out", "Reset Zoom", "Line", "Scatter", "Bar Chart", "Histogram"]
        commands = [
        ...
        bottomButtons = UtilityButtonList(
            guiFrame, texts=texts, doHelp=False, doClone=False, commands=commands, expands=True
        )
...
class FilterMatrixPopup(BasePopup):
    def __init__(self, parent, scrolledMatrix):
        ...
        BasePopup.__init__(self, parent=parent, title="Filter Options", transient=False, borderwidth=6)
    ...
    def body(self, guiFrame):
        ...
        self.pulldownMenu = PulldownList(guiFrame, texts=columnNames, callback=self.changeColumn, grid=(0, 1))
        ...
        commands = [self.close, self.unfilterMatrix, self.filterInclude, self.filterExclude]
        buttons = ButtonList(guiFrame, texts=texts, commands=commands, grid=(3, 0), gridSpan=(1, 3))

        objFrame = Frame(guiFrame, grid=(0, 2))
        objLabel = Label(objFrame, text="Objects to filter:", grid=(0, 0))
        objButtons = RadioButtons(objFrame, objEntries, self.filteredObjs, borderwidth=0, relief="flat", grid=(0, 1))
    ...
    def unfilterMatrix(self):
        ...
        self.scrolledMatrix.update(textMatrix=self.textMatrix, colorMatrix=self.colorMatrix, objectList=self.objectList)
    ...
    def filterMatrix(self, includeMatches=True):
        ...
        for row, textRow in enumerate(textMatrix):
            ...
            if col == "[All]":
                ...
                if ((not includeMatches) and (nMatch == len(textRow))) or (includeMatches and nMatch):
                    appendText(textRow)
...
class FilterMatrix:
    def __init__(
        self, parent, scrolledMatrix, searchString, column=None, mode=SEARCH_MODES[0], selectedObjectsOnly=False
    ):
