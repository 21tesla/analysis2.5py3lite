...
class LabeledEntries(Frame):
    ...
    def addLabelEntry(self, n, label="", entry="", show="", frozen=False, doGrid=True):
        ...
        e = Entry(
            self, text=entry, show=show, width=self.entry_width, returnCallback=lambda n=n: self.returnCallback(n)
        )
        if frozen:
