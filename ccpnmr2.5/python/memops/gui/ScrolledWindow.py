...
class TickCanvas(Canvas, Ticks):
    def __init__(self, parent, orient, size, tickScale=None, expandYAxisDecimals=True, *args, **kw):
...
class ScrolledWindow(Frame):
    ...
    def __init__(
        ...
        width=None,
        height=None,  # initial width/height in pixels, can be tuples
        show_xcrosshair=True,
        show_ycrosshair=True,
        show_xticks=True,
        show_yticks=True,
        xtick_size=30,
        ytick_size=40,
        ...
        xslice_size=100,
        yslice_size=100,
        show_xscrollbar=True,
        show_yscrollbar=True,
        view_set_func=None,
        key_func=None,
        configure_func=None,
        expose_func=None,
        ...
        enter_func=None,
        leave_func=None,
        slice_configure_func=None,
        slice_expose_func=None,
        slice_motion_func=None,
        slice_enter_func=None,
        slice_leave_func=None,
        slice_key_func=None,
        select_single_func=None,
        select_multi_func=None,
        drag_func=None,
        units_scroll=0.1,
        pages_scroll=1.0,  # fraction for scrolling
        scroll_borderwidth=1,
        separator_button=1,
        separator_thickness=4,
        get_aspect_ratio=None,
        get_geometry=None,
        set_geometry=None,
        has_value_axis=False,
        min_region_event_delta=20,
        expand_y_axis_tick_decimals=True,
        *args,
        **kw,
    ):
        ...
        for i in range(ncols - 1):
            self.xseparators.append(Separator(self, orient=Tkinter.VERTICAL, width=self.separator_thickness))

        self.yseparators = []
        for j in range(nrows - 1):
            self.yseparators.append(Separator(self, orient=Tkinter.HORIZONTAL, height=self.separator_thickness))
    ...
    def selectSingleBind(self, button, state=no_key_state):
        ...
        self.setupBind(button, self.markLocation, releaseFunc=self.selectLocation, state=state)

    def selectMultiBind(self, button, state=no_key_state):

        self.setupBind(button, self.markLocation, self.dragBox, self.selectRegion, state=state)
    ...
    def setupBind(self, button, pressFunc="", motionFunc="", releaseFunc="", state=no_key_state):
  ...
  """
    ...
    def gridAll(self):
        ...
        if (rowspan > 0) and (colspan > 0):
            self.corner_canvas.grid(
                row=2 * nrows - 1, column=2 * ncols - 1, rowspan=rowspan, columnspan=colspan, sticky=Tkinter.NSEW
            )
        else:
    ...
    def xResize(self, canvas, s, resize_type):
        ...
        if hasattr(canvas, "ymax_extent"):
            ...
            ymax_extent = None
        canvas.xview_region.resizeFraction(s, resize_type, r, other_region, self.world_region.y_region, ymax_extent)
    ...
    def yResize(self, canvas, t, resizeType):
        ...
        if hasattr(canvas, "xmax_extent"):
            ...
            xmax_extent = None
        canvas.yview_region.resizeFraction(t, resizeType, r, other_region, self.world_region.x_region, xmax_extent)
        self.updateView(canvas)
    ...
    def splitWindowVertical(self, event):
        ...
        for j in range(self.nrows):
            ...
            c = WindowCanvas(
                self, xview_region1, cc.yview_region, width=w1, height=cc.winfo_height(), background=self.background
            )
            # background overwritten by GL (if GL used)
        ...
        s = WindowScrollbar(
            self,
            orient=Tkinter.HORIZONTAL,
            units_scroll=self.units_scroll,
    ...
    """
        ...
        for i in range(self.ncols):
            ...
            w = c.winfo_width()
            c = WindowCanvas(self, xview_region, yview_region, width=w, height=h, background=self.background)
            # background overwritten by GL (if GL used)
            cs.append(c)
        ...
        c = TickCanvas(self, Tkinter.VERTICAL, self.ytick_size, expandYAxisDecimals=self.expand_y_axis_tick_decimals)
        self.yticks.append(c)
