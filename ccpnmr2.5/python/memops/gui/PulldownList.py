...
class PulldownList(Frame):
    ...
    def __init__(
        ...
        **kw,
        ...
        self.menu = Menu(
            self.canvas,
            tearoff=False,
            relief="solid",  # bg=menuBg,  # NOTE:ED removed because locks MacOS colour
            borderwidth=1,
            activeborderwidth=1,
        )
        ...
        self.setup(self.texts, self.objects, index, self.colors, self.categories, self._fonts)
    ...
    def insert(self, index, text, object=None, color=None, category=None, select=False, font=None):
    ...
    def append(self, text, object=None, color=None, category=None, select=False, font=None):
        ...
        self.insert(
            len(self.texts), text, object=None, color=None, category=None, select=False, font=None
        )  # NOTE:ED - not sure these should be None.. but not changing
...
"""
    ...
    def _resizeCallback(self, *event):
        ...
        h = c.winfo_height() - 1
        c.delete("all")
        c.create_rectangle(0, 0, 10, 2, fill=fill, outline=outline)
        c.create_polygon(0, 4, 0, 6, 5, 11, 10, 6, 10, 4, fill=fill, outline=outline)
            ...
            if cat:
                ...
                for index2, text2, color2, _fnt2 in categoryDict.get(cat, []):
                    ...
                    if color2:
                        ...
                        item2 = {"kind": "command", "label": string2, "command": command, "columnbreak": columnbreakB}
                        if _fnt2:
                            item2.update({"font": _fnt2})
                ...
                item = {"kind": "cascade", "label": string, "submenu": items, "columnbreak": columnbreak}
                if _fnt:
                ...
                if color:
                    ...
                    item = {"kind": "command", "label": string, "command": command, "columnbreak": columnbreak}
                    if _fnt:
                        item.update({"font": _fnt})
