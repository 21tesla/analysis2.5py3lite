...
class Tree(Frame):
    def __init__(
        self, parent, iconSize="medium", multiSelect=False, font="Helvetica 8", doubleCallback=None, *args, **kw
    ):
        ...
        iconItems = [
            {"kind": "command", "label": "Small", "command": self.smallIcons},
        ...
        menu_items = [
            {"kind": "cascade", "label": "Icon size", "submenu": iconItems},
        ]
        ...
        self.canvas = Canvas(
            self, relief="flat", borderwidth=0, background=BG_COLOR, xscrollcommand=self._setHScrollbar
        )
    ...
    def update(self, parents, objects, labels=None, icons=None, callbacks=None, editWidgets=None):
        ...
        for i, parent in enumerate(parents):
            ...
            node = Node(self, parent, object, labels[i], icons[i], callbacks[i], False, False, editWidgets[i])
    ...
    def add(
        ...
        node = Node(self, parent, object, label, icon, callback, isOpen, isSelected, editWidgets)
        ...
        if path.isdir(file):
            ...
            #  icon = 'text-html'
                ...
                if mimeType:
                    ...
                    icon = icon_dict.get(mimeType) or icon_dict.get(mimeType.split("/")[0]) or icon_dict["none"]
                else:
                    icon = icon_dict["none"]
