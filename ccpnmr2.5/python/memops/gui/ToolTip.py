...
class ToolTip:
    ...
    def _appear(self):
        ...
        if self._text and not self._panel:
            ...
            if self._isAqua:
                panel.tk.call("::tk::unsupported::MacWindowStyle", "style", panel._w, "help", "none")

            label = Tkinter.Label(
                panel, text=self._text, bd=1, padx=2, pady=2, relief="solid", bg="#FFFFB0", justify="left", font=FONT
            )
...
if __name__ == "__main__":
    ...
    root = Tkinter.Tk()
    button = Button(root, text="Close (hover for tip)", command=root.destroy, toolTip="This will close the window")
    button.pack()
