        ...
        if self.widgetType != "CheckButton":
                ...
                if not self.addButton:
                    if self.plusImage:
                        self.addButton = Button(
                            ...
                            docKey="+",
                            tipText="Add extra row",
                            command=self.addRow,
                            borderwidth=1,
                        )
                    else:
                        self.addButton = Button(
                            self, text="+", command=self.addRow, tipText="Add extra row", borderwidth=1, bg=blueColor
                        )
            ...
            if not self.allButton:
                self.allButton = Button(
                    self, text="All", command=self._selectAll, borderwidth=1, bg=blueColor, tipText="Select all options"
                )
            ...
            if not self.noneButton:
                self.noneButton = Button(
                    self,
                    text="None",
                    command=self._selectNone,
                    borderwidth=1,
                    bg=blueColor,
                    tipText="Select no options",
                )
        ...
        if self.callback:
            if not self.commitButton:
                if self.tickImage:
                    ...
                    self.commitButton = Button(
                        self, text="OK", command=self.commit, tipText="Commit selection", borderwidth=1, bg=blueColor
                    )
            ...
            if not self.cancelButton:
                if self.crossImage:
                    self.cancelButton = Button(
                        ...
                        borderwidth=1,
                        docKey="Cancel",
                        tipText="Abort selection",
                    )
                else:
                    self.cancelButton = Button(
                        self, text="Cancel", command=self.cancel, borderwidth=1, bg=blueColor, tipText="Abort selection"
                    )
...
if __name__ == "__main__":
    ...
    mw2 = MultiWidget(
        root, Entry, callback=fn, minRows=0, values=["Easy", "As", "One", "Two", "Three"], useImages=False
    )
    mw2.grid()
    ...
    mw3 = MultiWidget(
        ...
        options=["Bjorn", "Agnetha", "Benny", "Anni-Frid"],
    )
    mw3.grid()
