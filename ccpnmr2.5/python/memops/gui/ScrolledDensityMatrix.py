...
class ScrolledDensityMatrix(ScrolledCanvas):
    ...
    def mouseEnter(self, event):
        ...
        if self.canvasDict.get(item) and (self.canvasDict[item][0] == "box"):
            ...
            if self.xText:
                ...
                (x1, y1, x2, y2) = c.bbox(self.xText)
                self.xBg = c.create_rectangle(
                    x1 - 4, y1 - 4, x2 + 4, y2 + 4, fill="#a0a0ff", outline="#5050b0", width=2
                )
                (x1, y1, x2, y2) = c.bbox(self.yText)
                self.yBg = c.create_rectangle(
                    x1 - 4, y1 - 4, x2 + 4, y2 + 4, fill="#a0a0ff", outline="#5050b0", width=2
                )
    ...
    def draw(self):
        ...
        if self.title:
            ...
            item = createText(
                midWay, self.origin[1] + self.indent - h, font=self.boldFont, text=self.title, anchor="s", fill="black"
            )
        ...
        if self.xAxisLabel:
            ...
            item = createText(
                midWay,
                self.origin[1] + (M * h) + self.indent + h + h + h,
                font=self.boldFont,
        ...
        if labelAxes:
            for j in range(M):
                yText = createText(x - w2, y + h2, font=self.normFont, text=yLabels[j], anchor="e", fill="black")
        ...
        for i in range(N):
            ...
            for j in range(M):
                ...
                if barPlot:
                    ...
                    item = boxes[i][j]
                    if item is None:
                        boxes[i][j] = makeBox(c, x, yA, i, j, width=w, height=yB - yA, fillColor=color)
                    else:
                    ...
                    item = boxes[i][j]
                    if item is None:
                        boxes[i][j] = makeBox(c, x, y, i, j, width=w, height=h, fillColor=color)
                    else:
                        # setBoxCoords(c,i,j,x,y,w,h) # Substituted for speed
            ...
            if labelAxes:
                xText = createText(x + w2, y + h2, font=self.normFont, text=xLabels[i], anchor="n", fill="black")
        ...
        for i in range(B):
            ...
            if barPlot:
                ...
                if len(legend) < B:
                    ...
                    itemB = createText(x + sz + sz / 2, y, font=self.normFont, text=text, anchor="w", fill="black")
                ...
                if len(legend) < B:
                    itemA = createRect(x, y, x + sz, y + sz, fill=color, outline=borderColor, width=1)
                    itemB = createText(x + sz + sz / 2, y, font=self.normFont, text=text, anchor="w", fill="black")
    ...
    def makeBox(self, canvas, x, y, i, j, fillColor="white", width=8, height=8):
        ...
        item = canvas.create_rectangle(x, y, x + width, y + height, fill=fillColor, width=0, outline=fillColor)
