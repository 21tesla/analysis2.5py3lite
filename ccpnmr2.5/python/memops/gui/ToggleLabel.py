...
class ToggleLabel(Frame):
    def __init__(
        ...
        **kw,
        ...
        self.arrow = ToggleArrow(
            self, arrowSize=arrowSize, outline=outline, fill=fill, isArrowClosed=isArrowClosed, callback=callback
        )
        self.arrow.grid(row=0, column=0, sticky=Tkinter.W)
        if font:
            self.label = Button(
                self, relief="flat", borderwidth=0, padx=1, pady=1, text=text, font=font, command=self.arrow.flipState
            )
        else:
            self.label = Button(
                self, relief="flat", borderwidth=0, padx=1, pady=1, text=text, command=self.arrow.flipState
            )
...
if __name__ == "__main__":
    ...
    toggle_label = ToggleLabel(root, callback=callback, text="Test frame:", arrowSize=20)
