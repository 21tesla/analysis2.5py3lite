...
class PrintFrame(Frame):
    def __init__(self, parent, getOption=None, setOption=None, haveTicks=False, doOutlineBox=True, *args, **kw):
        ...
        row = 0
        button = Button(self, text="File:", command=self.findFile, tipText="Select location to save print file")
        button.grid(row=row, column=0, sticky="e")
        self.file_entry = Entry(self, width=40, text=self.file_name, tipText="Location where file is saved on disk")
        self.file_entry.grid(row=row, column=1, sticky="ew")

        row += 1
        label = Label(self, text="Title:")
        label.grid(row=row, column=0, sticky="e")
        self.title_entry = Entry(self, width=40, text=self.title, tipText="Title of the printout, displayed at top")
        self.title_entry.grid(row=row, column=1, sticky="ew")

        row += 1
        label = Label(self, text="X axis label:")
        label.grid(row=row, column=0, sticky="e")
        self.x_axis_entry = Entry(self, width=40, text=self.x_axis_label, tipText="X axis label for the printout")
        self.x_axis_entry.grid(row=row, column=1, sticky="ew")

        row += 1
        label = Label(self, text="Y axis label:")
        label.grid(row=row, column=0, sticky="e")
        self.y_axis_entry = Entry(self, width=40, text=self.y_axis_label, tipText="Y axis label for the printout")
        ...
        self.size_menu = PulldownList(
            frame, callback=self.changedSize, texts=entries, index=size_index, tipText="The paper size for the printout"
        )
        ...
        self.other_unit_menu = PulldownList(
            self.other_frame, texts=paper_units, index=other_unit_index, tipText="The unit for the Other paper size"
        )
        ...
        label.grid(row=0, column=0, sticky="e")
        self.orientation_menu = PulldownList(
            frame,
        ...
        if haveTicks:
            ...
            self.tick_buttons = CheckButtons(
                frame,
            ...
            self.spacing_menu = PulldownList(
                frame,
                texts=spacing_choices,
                index=0,
            ...
            self.x_major_entry = FloatEntry(
                ff, tipText="The spacing in display units of the major tick marks in the X dimension"
            )
            self.x_major_entry.grid(row=1, column=1, sticky="ew")
            self.x_minor_entry = FloatEntry(
                ff,
                tipText="The spacing in display units of the minor tick marks in the X dimension (not printed if left blank)",
            )
            self.x_minor_entry.grid(row=1, column=2, sticky="ew")
            self.x_decimal_entry = IntEntry(
                ff, tipText="The number of decimal places for the tick numbers in the X dimension"
            )
            self.x_decimal_entry.grid(row=1, column=3, sticky="ew")
            label = Label(ff, text="Y:")
            label.grid(row=2, column=0, sticky="w")
            self.y_major_entry = FloatEntry(
                ff, tipText="The spacing in display units of the major tick marks in the Y dimension"
            )
            self.y_major_entry.grid(row=2, column=1, sticky="ew")
            self.y_minor_entry = FloatEntry(
                ff,
                tipText="The spacing in display units of the minor tick marks in the Y dimension (not printed if left blank)",
            )
            self.y_minor_entry.grid(row=2, column=2, sticky="ew")
            self.y_decimal_entry = IntEntry(
                ff, tipText="The number of decimal places for the tick numbers in the Y dimension"
            )
            ...
            label.grid(row=0, column=0, sticky="e")
            # TBD: put preferred choice in data model
            self.tick_length_menu = PulldownList(
                frame,
                texts=tick_length_choices,
                index=0,
        ...
        label.grid(row=0, column=0, sticky="e")
        # TBD: put preferred choice in data model
        self.scaling_menu = PulldownList(
            frame,
            texts=scaling_choices,
            index=0,
        ...
        self.scaling_scale = Scale(
            frame,
            orient=Tkinter.HORIZONTAL,
            value=self.scaling,
            tipText="The percentage of the maximum size that would fit on the paper that the plot is scaled by",
        )
        self.scaling_scale.grid(row=0, column=2, columnspan=3, sticky="ew")

        self.x_scaling_label = Label(frame, text="X:")
        self.x_scaling_entry = FloatEntry(
            frame, tipText="The scaling that should be used in the X dimension as cms or inches per unit"
        )
        self.y_scaling_label = Label(frame, text="Y:")
        self.y_scaling_entry = FloatEntry(
            frame, tipText="The scaling that should be used in the Y dimension as cms or inches per unit"
        )
        ...
        label.grid(row=0, column=0, sticky="e")
        tipTexts = (
            "Whether the time and date should be included in the printout",
        ...
        self.border_buttons = CheckButtons(frame, entries=border_decorations, selected=selected, tipTexts=tipTexts)
        self.border_buttons.grid(row=0, column=1, sticky="w")

        label = Label(frame, text="    Using Font:")
        label.grid(row=0, column=2, sticky="e")
        self.border_font_list = FontList(
            frame, mode="Print", selected=self.border_font, tipText="The font used for the border texts"
        )
        ...
        self.linewidth_entry = FloatEntry(self, width=10, text=self.linewidth, tipText="Line width for drawing")
