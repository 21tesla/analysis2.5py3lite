...
class ProgressBar(BasePopup):
    def __init__(
        self, parent, text="", progress=0, total=100, title=None, width=200, height=15, transient=True, *args, **kw
    ):
        ...
        BasePopup.__init__(self, parent=parent, title=title or "Progress Bar", transient=transient, *args, **kw)
    ...
    def body(self, parent):
        ...
        self.bar = self.canvas.create_rectangle(
            self.bw, self.bw, self.bw, self.cHeight, outline="#B05848", fill="#B05848", width=self.bw
        )
        self.update()
