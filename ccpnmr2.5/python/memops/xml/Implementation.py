...
Autogenerated by PyXmlMapWrite revision 1.29 on Mon Mar  2 17:24:08 2015
...
def makeMapping(globalMap):
    ...
    currentMap = {}
    contentMap["createdBy"] = currentMap
    mapsByGuid["www.ccpn.ac.uk_Fogh_2006-12-31-09:00:59_00002__www.ccpn.ac.uk_Fogh_2006-08-16-14:22:53_00047"] = (
        currentMap
    )
    ...
    currentMap = {}
    contentMap["isModifiable"] = currentMap
    mapsByGuid["www.ccpn.ac.uk_Fogh_2006-08-17-14:16:26_00010__www.ccpn.ac.uk_Fogh_2006-08-16-14:22:53_00047"] = (
        currentMap
    )
    ...
    currentMap = {}
    contentMap["lastUnlockedBy"] = currentMap
    mapsByGuid["www.ccpn.ac.uk_Fogh_2006-12-31-09:00:59_00003__www.ccpn.ac.uk_Fogh_2006-08-16-14:22:53_00047"] = (
        currentMap
    )
...
def saveToStream(stream, topObject, mapping=None, comment=None, simplified=True, compact=True, expanded=False):
    ...
    if isinstance(topObject, memops.api.Implementation.MemopsRoot):
        ...
        raise ApiError("""XML save: top obj is neither MemopsRoot nor TopObject""" + ": %s" % (topObject,))
    ...
    strapp(
        '%s<%s release="%s" originator="%s" packageGuid="%s" time="%s">\n'
        ...
        if val is startObj:
            ...
            if typ == "simple":
                ...
                for ii in reversed(range(len(keys))):
                    key = keys[ii]
                    stack.append(key)
                    if isinstance(key, memops.api.Implementation.MemopsObject) or isinstance(
                        key, memops.api.Implementation.MemopsDataTypeObject
                    ):
...
def loadFromStream(stream, topObjId=None, topObject=None, partialLoad=False):
    ...
    if topObject is not None:
        # check for topObject
        if not (
            isinstance(topObject, memops.api.Implementation.MemopsRoot)
            or isinstance(topObject, memops.api.Implementation.TopObject)
        ):
            raise ApiError("""XML load: top obj is neither MemopsRoot nor TopObject""" + ": %s" % (topObject,))
    ...
    try:
            ...
            if event == "start":
                ...
                if loadMaps is not None:
                    ...
                    try:
                        ...
                        raise ApiError("""no map found for element""" + ": %s" % (tag,))
                    except Exception:
                        raise ApiError("""Load maps not set up correctly - should not get here""" + ": %s" % (tag,))
                    ...
                    if "cplx" == curMap.get("eType"):
                        ...
                        if "cplx" == typ or "class" == typ:
                                ...
                                if topObjectKey is None:
                                    ...
                                    if topObject is None:
                                        ...
                                        if mapping[".prefix"] == implPrefix:
                                            ...
                                            raise ApiError(
                                                """non-Implementation package called without TopObject"""
                                                + ": %s" % (implPrefix,)
                                            )
                                        ...
                                        if clazz is not topObject.__class__:
                                            raise ApiError(
                                                """TopObject class does not fit first element class"""
                                        ...
                                        if topObject.__dict__.get("isModified", False):
                                            raise ApiError(
                                                """trying to load already modified TopObject""" + ": %s" % (topObject,)
                                            )
                                        ...
                                        if objId != elemId:
                                            raise ApiError(
                                                """TopObject identifier does not fit stored identifier"""
                                                + ": %s:%s" % (objId, elemId)
                                            )
                                    ...
                                    if elemId != topObjId:
                                        raise ApiError(
                                            """TopObject identifier does not fit passed=in identifier"""
                                            + ": %s:%s" % (elemId, topObjId)
                                        )
                            ...
                            for tag2, value in elem.items():
                                ...
                                if "_ID" == tag2:
                                    ...
                                    try:
                                        ...
                                        raise ApiError("""no map found for XML attribute""" + ": %s" % (tag2,))
                            ...
                            raise ApiError("""partial load with in incorrect context for """ + ": %s" % (xx,))
                    ...
                    if "simple" == typ:
                        ...
                        if curMap.get("isTop", False):
                            ...
                            if yy is None:
                                ...
                                raise ApiError(
                                    """Read topObj with guid conflicts with existing topObj""" + ": %s" % (guid,)
                                )
                        ...
                        if "delay" == proc:
                            ...
                            if "exolink" == typ:
                                ...
                                for ii, vv in enumerate(val):
                                    ...
                                    if value is None:
                                        raise ApiError(
                                            "No object found with package,class,key: %s"
        ...
        if not (earlyExit):
            ...
            if objStack or skipElement is not None:
                raise ApiError(
                    """Illegal state after parsing: objStack length, skipElement"""
...
def getLoadingMaps(packageGuid, fileVersion):
    ...
    if mapping is None:
        raise ApiError("""No package mapping with guid found""" + ": %s" % (packageGuid,))
...
def linkChildData(delayDataDict, topObj, mapping, linkTopToParent=False, doOutOfPackage=False):
    ...
    if doOutOfPackage:
        ...
        if isinstance(topObj, memops.api.Implementation.MemopsRoot):
            ...
            raise ApiError("""linkChildData: called with doOutOfPackage for non-root""" + ": %s" % (topObj,))
            ...
            if kids:
                ...
                if curMap.get("singleKid") is not None:
                    ...
                    if sibling is None:
                        ...
                        raise ApiError("""Attempt to override single child""" + ": %s" % (sibling,))
        ...
        while linkData:
            ...
            for vv in val:
                try:
                    ...
                    raise ApiError("""Linked-to object with ID not found""" + ": %s:%s:%s" % (obj, name, vv))
