...
def sortByDataObjTypeDependency(objList):
    ...
    return topologicalSortSubgraph(objList, "subtypes", func=dataObjTypeDependencies)
...
def checkLinkCircularity(myself, tag):
    ...
    while next is not None:
        ...
        if next is myself:
            raise MemopsError("%s is (indirectly) its own %s" % (myself, tag))
...
def topologicalSortSubgraph(objList, tag, func=getattr):
    ...
    # check for cycles
    if inLinkCount:
        raise MemopsError("Cycles in '%s' graph. Objects involved were %s" % (tag, inLinkCount.keys()))
    #
    return result
...
def dataObjTypeDependencies(obj, tag):
    ...
    if not isinstance(obj, MetaDataObjType):
        raise MemopsError("dataObjTypeDependencies called with non-MetaDataObjType object %s" % obj)
...
def parseCardinality(cardString):
    ...
    for tag in cardString.strip().split(".."):
        if tag == "*":
            ...
            try:
                ...
                raise MemopsError("inValid content in cardinality string %s" % str(cardString))
    ...
    if result == [genConstants.infinity]:
        ...
        raise MemopsError("inValid cardinality string %s" % str(cardString))
...
class SimpleGuidGenerator:
    ...
    def __init__(self, operator, organisation, initialSerial=1):
        ...
        for char in ImpConstants.xmlDisallowedChars:
            if char in self.operator:
                raise MemopsError("operator %s contains disallowed character %s" % (str(self.operator), str(char)))
            if char in self.organisation:
                raise MemopsError(
                    "organisation %s contains disallowed character %s" % (str(self.organisation), str(char))
                )
    ...
    def newGuid(self):
        ...
        return "%s_%s_%s_%05d" % (self.organisation, self.operator, timeStamp, serial)
...
# Utilities moved here from ModelTraverse
...
def getOperation(target, opType, inClass=None, opSubType=None):
    ...
    for op in inClass.getAllOperations():
        if (op.target) is target and op.opType == opType and op.opSubType == opSubType:
            return op
    #
    return None
...
def getFuncname(op, inClass=None):
    ...
    #
    if op is None:
        raise MemopsError("No operation with opSubType None corresponds to %s" % op.qualifiedName())
    else:
        return op.name
