...
class ModelTraverse_py_2_1:
    ...
    baseDataTypeObjName = "%s.%s" % (implPackageName, ImpConstants.baseDataTypeObjName)
    ...
    implObjClassName = "%s.%s" % (implPackageName, ImpConstants.implObjClassName)
    ...
    def addModelFlavour(self, key, val):
        ...
        if flav == val:
            ...
            raise MemopsError("attempt to reset modelFlavour %s:%s to %s)" % (key, flav, val))
                        ...
                        for jj in range(len(spz)):
                            ...
                            for cc3 in classes[ic][ii:]:
                                if valueType in cc3.getAllSupertypes():
                                    raise MemopsError("Partitioning role %s connects ancestor and descendant" % (role,))
                            continue
                        break
            else:
                continue
