...
class ModelPortal:
    ...
    def __init__(self, topPackage, dataModelVersion=None):
        ...
        # check topPackage
        if isinstance(topPackage, MetaModel.MetaPackage) and topPackage is topPackage.topPackage():
            self.topPackage = topPackage
        else:
            raise MetaModel.MemopsError(
                "ModelPortal initialised with %s, must be top package" % topPackage.qualifiedName()
            )
        ...
        self._branchPackages = metaUtil.sortByMethodCall(branchPackages, "qualifiedName")
        del branchPackages

        # sort leafPackages by import (imported before importing)
        self._leafPackages = metaUtil.topologicalSortSubgraph(leafPackages, "accessedPackages")
        del leafPackages
    ...
    def resetObjectDicts(self):
        ...
        for pp in self._leafPackages:
            ...
            _dataObjTypesDict[pp] = metaUtil.sortByDataObjTypeDependency(pp.dataObjTypes)
    ...
    def requiredInputElements(self, complexDataType):
        ...
        for attr in complexDataType.getAllAttributes():
            if (
                not attr.isDerived
                and not attr.isImplementation
                and not attr.isAutomatic
                and attr.locard > 0
                and not attr.defaultValue
            ):
                elems.append(attr)
        ...
        if isinstance(complexDataType, MetaModel.MetaClass):
            for role in complexDataType.getAllRoles():
                if (
                    not role.isDerived
                    and not role.isImplementation
                    and not role.isAutomatic
                    and role.locard > 0
                    and (role.hierarchy == ImpConstants.no_hierarchy)
                ):
                    elems.append(role)
    ...
    def setModelFlavour(self, key, val):
        ...
        if flav is not None and flav != val:
            raise MemopsError("attempt to change model flavour from %s to %s" % (str(flav), str(val)))
        else:
            flavours[key] = val
    ...
    def checkModelFlavour(self, key, val):
        ...
        if flav != val:
            raise MemopsError(
                "found wrong model flavour %s:%s, should be :%s"
                % (
                    key,
                    str(flav),
                    str(val),
