        ...
        for guids in elementPairings:
            ...
            for ii in 0, 1:
                ...
                if guid not in dicts[ii]:
                    raise MemopsError("%s from elementPairings not found in model %s " % (guid, ii))
                if guid in dicts[jj]:
                    raise MemopsError("%s (%s) from elementPairings found in model %s " % (guid, dicts[jj][guid], jj))

            # check diffs and append
            diffs = compareElements(dict1[guids[0]], dict2[guids[1]], ignoreImplicit=ignoreImplicit)
...
def compareElements(ee, ee2, ignoreImplicit=True, language="python"):
    ...
    ignoreTagVals = set(
        ["repositoryTag", "repositoryId", "docDiagramNames", "packageGroup", "isDraft", "isReferenceData"]
    )

    # dictionaries with language
    languageCoded = set(
        ["typeCodes", "codeStubs", "constructorCodeStubs", "destructorCodeStubs", "postDestructorCodeStubs"]
    )
    ...
    for pName, pData in clazz.parameterData.items():
        ...
        for tag, val in pData.items():
            try:
                ...
                raise MemopsError("%s: illegal tag %s for parameter %s" % (clazz.__name__, str(tag), str(pName)))
        ...
        # Booleans
        if "isFixed" in pData and pData["isFixed"] not in (True, False):
            raise MemopsError(
                "%s: tag %s for parameter %s has illegal value %s"
        ...
        if pData.get("nameList") and (myType != "content" or tag == "constraints"):
            raise MemopsError(
                "%s: parameter %s is not standard content but has 'namelist'" % (clazz.__name__, str(pName))
            )

        if myType is None:
            if (clazz, pName) not in unTypedPars:
                raise MemopsError("%s: parameter %s has no explicit type " % (clazz.__name__, str(pName)))
            ...
            raise MemopsError("%s: parameter %s has unsupported type %s" % (clazz.__name__, str(pName), myType))
...
class MetaModelElement:
    ...
    def __getattr__(self, tag):
        ...
        try:
            ...
            raise AttributeError("%s object has no attribute %s" % (self.__class__, str(tag)))
    ...
    def __setattr__(self, tag, value):
        ...
        if pData is None:
            ...
            if setterFunc == "unsettable":
                raise MemopsError(
                    "Tried to set implementation attribute %s in class %s" % (tag, self.__class__.__name__)
                )

            elif pType == "content":
                raise MemopsError("Tried to set content attribute %s in class %s" % (tag, self.__class__.__name__))
    ...
    def __init__(self, **params):
        ...
        if params.get("name") is None:
            raise MemopsError("Tried to create %s without a name" % (self.__class__.__name__))

        if params.get("container") is None:
            if not (isinstance(self, MetaPackage) and params["name"] == ImpConstants.rootPackageName):
                raise MemopsError(
                    "Tried to create %s %s without a container" % (self.__class__.__name__, params["name"])
                )
        ...
        if nn:
            ...
            raise MemopsError(
                """%s: Input parameters contained %s undefined names:
%s"""
                % (self, nn, ll2)
            )
        ...
        if container is not None:
            ...
            if self.name in containerDict:
                raise MemopsError("%s already has %s named %s" % (container, self.__class__.__name__, self.name))
            else:
    ...
    def addTaggedValue(self, tag, value):
        ...
        if type(value) != str:
            raise MemopsError("%s tagged value %s value %s is not a string" % (self, tag, str(value)))
        ...
        if tag in allowedTags:
            ...
            if allowedVals is not None and value not in allowedVals:
                raise MemopsError("%s tag %s has illegal value %s" % (self, tag, value))
        else:
    ...
    def removeTaggedValue(self, tag):
        ...
        if tag in self.__dataDict["taggedValues"]:
    ...
    def _getCloningDict(self):
        ...
        for pName, pData in self.__class__.parameterData.items():
            ...
            elif pData.get("type") == "content" or pData.get("setterFunc") == "unsettable":
                pass
    ...
    def checkValid(self, complete=False):
        ...
        if container:
            ...
            if dd.get(name) is not self:
                raise MemopsError("Two-way container link %s-%s is broken. Bug (1)?" % (container, self))
        ...
        for tag, pData in self.parameterData.items():
            ...
            # default values
            if "default" in pData and value == pData["default"]:
                # Always OK.
                ...
                raise MemopsError("%s - wrong value %s for fixed attribute %s" % (self, str(value), tag))

            # string dictionaries
            if pType == "StringDict":
                for key, val in value.items():
                    if not key or not isinstance(key, str):
                        raise MemopsError("%s: non-string or empty key %s in StringDict %s" % (self, str(key), tag))
                    if not val or not isinstance(val, str):
                        raise MemopsError("%s: non-string or empty value %s in StringDict %s" % (self, str(val), tag))
                continue
            ...
            if hicard == 1:
                ...
                # hicard check
                raise MemopsError("%s.%s - more than %s elements in value: %s" % (self, tag, hicard, str(value)))
            ...
            if enumeration is not None:
                for item in value:
                    if item not in enumeration:
                        raise MemopsError("%s.%s - %s not among allowed values" % (self, tag, str(value)))
                continue
            ...
            if pType == str:
                for item in value:
                    if not item or not isinstance(item, str):
                        raise MemopsError("%s.%s - %s is empty or not a string" % (self, tag, str(value)))

            elif pType == "Token":
                for item in value:
                    if not item or not isinstance(item, str):
                        raise MemopsError("%s.%s - %s is empty or not a string" % (self, tag, str(item)))
                    for char in item:
                        if char not in ImpConstants.validNameChars:
                            raise MemopsError(
                                "%s %s %s contains illegal character %s" % (self, tag, str(item), str(char))
                            )

                    if item[0] in string.digits:
                        raise MemopsError("%s %s %s starts with a digit" % (self, tag, str(item)))
            ...
            # other cases:
                ...
                for item in value:
                    if not isinstance(item, pType):
                        raise MemopsError("%s - %s should be type %s for attribute %s" % (self, str(value), pType, tag))

        # specific checks for MetaModelElement attributes

        # name
        if not self.isImplicit and name not in ("_ID", "_lastId"):
            # autogenerated elements can be assumed to be OK
            if name[0] == ImpConstants.underscore:
                raise MemopsError("%s: name %s of non-implicit element starts with '_'" % (self, str(name)))

            if (
                ImpConstants.underscore in name[1:]
        ...
        for char in """&'"!=""":
            if char in guid:
                raise MemopsError("%s: guid %s contains illegal character %s" % (self, guid, repr(char)))
        # check guid uniqueness
        guidDict = MetaModelElement.guidDict
        if guidDict is not None:
            competitor = guidDict.get(guid)
            if competitor is not None:
                raise MemopsError("%s: guid %s duplicates guid of %s" % (self, guid, competitor))
            ...
            if self._tempData:
                raise MemopsError(
                    "%s - _tempData dictionary is not emptied, keys are %s" % (self, self._tempData.keys())
                )
        ...
        for obj in self.__elementDict.values():
            ...
            if obj.container is not self:
                raise MemopsError("Two-way container link %s-%s is broken. Bug (2)?" % (self, obj))
...
class ConstrainedElement(MetaModelElement):
    ...
    def __init__(self, **params):
        ...
        if self.__class__ is ConstrainedElement:
            raise MemopsError("Attempt to create istance of abstract class ConstrainedElement")
    ...
    def checkValid(self, complete=False):
        ...
        for constraint in self.__constraints.values():
            ...
            if constraint.container is not self:
                raise MemopsError("Two-way container link %s-%s is broken. Bug (3)?" % (self, constraint))
...
class HasParameters(MetaModelElement):
    ...
    def __init__(self, **params):
        ...
        if self.__class__ is HasParameters:
            raise MemopsError("Attempt to create istance of abstract class HasParameters")
...
class HasSupertype:
    ...
    def __init__(self, **params):
        ...
        raise MemopsError("Attempt to create istance of abstract class HasSupertype")
    ...
    def removeSupertype(self, supertype):
        ...
        if supertype in ll:
            ...
            raise MemopsError("%s: Attempt to remove non-existing supertype %s" % (self, supertype))
    ...
    def addSupertype(self, supertype):
        ...
        if supertype not in ll:
            ...
            raise MemopsError("%s: Attempt to add pre-existing supertype %s" % (self, supertype))
...
class AbstractDataType(ConstrainedElement, HasSupertype):
    ...
    def __init__(self, **params):
        ...
        if self.__class__ is AbstractDataType:
            raise MemopsError("Attempt to create istance of abstract class AbstractDataType")
    ...
    def checkValid(self, complete=False):
        ...
        for supertype in self.supertypes:
            ...
            if self.isAbstract and not supertype.isAbstract:
                raise MemopsError("Abstract %s has non-abstract supertype %s" % (self, supertype))

            # package access check
            if not self.canAccess(supertype):
                raise MemopsError("%s - cannot access supertype %s" % (self, supertype))

            # check two-way link
            if complete:
                # NB check is too slow to be done routinely
                if supertype.subtypes.count(self) != 1:
                    raise MemopsError("Two-way supertype link %s-%s is broken. Bug (1)?" % (supertype, self))
        ...
        for obj in subtypes:
            if self not in obj.supertypes:
                raise MemopsError("Two-way supertype link %s-%s is broken. Bug (2)?" % (self, obj))
...
class AbstractValue(ConstrainedElement):
    ...
    def __init__(self, **params):
        ...
        if self.__class__ is AbstractValue:
            raise MemopsError("Attempt to create istance of abstract class AbstractValue")
    ...
    def checkValid(self, complete=False):
        ...
        if self.hicard != infinity:
            ...
            if self.hicard < self.locard:
                raise MemopsError("%s: hicard %s < locard %s" % (self, self.hicard, self.locard))
        ...
        if self.hicard == 1:
            for tag in ("isOrdered", "isUnique"):
                ...
                if getattr(self, tag) != default:
                    raise MemopsError("%s: %s must be %s for hicard==1" % (self, tag, default))
...
class ClassElement(AbstractValue):
    ...
    def __init__(self, **params):
        ...
        if self.__class__ is ClassElement:
            raise MemopsError("Attempt to create istance of abstract class ClassElement")
    ...
    def checkValid(self, complete=False):
        ...
        if len(name) > ImpConstants.maxTagLength:
            raise MemopsError("%s: name %s longer than %s characters" % (self, name, ImpConstants.maxTagLength))

        if len(baseName) > ImpConstants.maxTagLength:
            raise MemopsError("%s: baseName %s longer than %s characters" % (self, baseName, ImpConstants.maxTagLength))

        AbstractValue.checkValid(self, complete=complete)

        # check changeability
        if self.changeability == ImpConstants.add_only:
            raise MemopsError("%s: changeability %s not implemented yet" % (self, self.changeability))

        # check baseName usage.

        if baseName != name and self.hicard == 1:
            raise MemopsError(
                "%s hicard is 1 but baseName %s differs from name %s" % (self, str(self.baseName), str(self.name))
            )

        # give warning for dissimilar name and basename:
        nn = len(name) / 2
        if name[:nn] != baseName[:nn]:
            print("WARNING, %s baseName %s dissimilar to name %s" % (self, str(self.baseName), str(self.name)))
        ...
        if self.isDerived:
            ...
            if self.changeability == ImpConstants.frozen:
                ...
                if not setters:
                    ...
                    raise MemopsError("derived changeable %s lacks explicit setFunction" % self)
        ...
        # check implementation elements:
            ...
            if container is Base and self.name != "override":
                raise MemopsError("%s: ClassElement of %s must be Implementation or Derived" % (self, Base))
            ...
            if self.isAutomatic or self.changeability == ImpConstants.frozen:
                for tag in ("add", "remove"):
                    ...
                    if ops:
                        raise MemopsError("%s is automatic or frozen and has %sFunction" % (self, tag))
        ...
        if name == ImpConstants.serial_attribute:
            ...
            if not isinstance(self.container, MetaClass):
                raise MemopsError("%s: name 'serial' only allowed inside MetaClass" % self)
            ...
            serialValues = {
                "isAutomatic": True,
                "changeability": ImpConstants.frozen,
                "hicard": 1,
                "locard": 1,
            }
        ...
        mayNotOverride = Base not in allSupertypes
        for supertype in allSupertypes[1:]:
            ...
            if superElem is not None:
                if superElem.__class__ is not self.__class__:
                    raise MemopsError("%s overrides %s but types are different" % (superElem, self))

                # classes without supertype cannot override or be overridden
                if mayNotOverride and not superElem.isAbstract:
                    raise MemopsError(
                        "Name clash between %s and %s - classes do not descend from %s" % (superElem, self, Base)
                    )
                ...
                for tag in parameterData.keys():
                    ...
                    if val == superval:
                        ...
                        if (superval == 1) != (val == 1):
                            raise MemopsError(
                                "%s overriding %s: hicard must be 1 in both or neither" % (self, superElem)
                            )
                        ...
                        raise MemopsError("%s overrides %s but differs for %s" % (self, superElem, tag))

                for name in self._ConstrainedElement__constraints.keys():
                    if superElem.getConstraint(name) is not None:
                        raise MemopsError("%s constraint %s overrides inherited constraint" % (self, name))
...
class ComplexDataType(AbstractDataType):
    ...
    parameterData["attributes"] = {"type": "content", "namelist": "_ComplexDataType__attributeNames"}
    parameterData["operations"] = {"type": "content", "namelist": "_ComplexDataType__operationNames"}
    ...
    def addConstructorCodeStub(self, tag, value):
        ...
        if type(value) != str:
            raise MemopsError("%s codeStub %s value %s is not a string" % (self, tag, str(value)))
    ...
    def removeConstructorCodeStub(self, tag):
        ...
        if tag in self._MetaModelElement__dataDict["constructorCodeStubs"]:
        ...
        if hasattr(self, "roles"):
            for obj in self.roles:
                if obj.isImplementation and not obj.otherRole:
                    ...
                    raise MemopsError("%s: attempt to inherit down non-implementation or two-way role" % (obj))
    ...
    def checkValid(self, complete=False):
        ...
        if constructorCode:
            if self.isAbstract:
                raise MemopsError("Abstract ComplexDataType %s has special constructor" % (self))

            codeTags = ImpConstants.codeStubTags
            for codeTag in constructorCode.keys():
                # check code tags
                if codeTag not in codeTags:
                    raise MemopsError("%s: Ilegal dcontructorCodeStubs tag %s" % (self, codeTag))

        # check uniqueness of Operations signatures:
        opCheckDict = {}
        for op in self.getAllOperations():
            tt = (op.opType, op.opSubType, op.target)
            if tt in opCheckDict:
                raise MemopsError("%s: Class has another operation with opType,opSubType,target %s" % (self, tt))
            else:
                opCheckDict[tt] = None

        # Now check presence of an opSubType=None variant for all operations.
        for opType, opSubType, target in opCheckDict.keys():
            if opSubType is not None and (opType, None, target not in opCheckDict):
                raise MemopsError(
                    """%s: opType,opSubType,target is %s. 
MetaOperation with opSubType:None not found"""
                    % (self, tt)
                )
        ...
        if not self.isAbstract:
            for attr in self.getAllAttributes():
                if attr.isAbstract:
                    raise MemopsError("%s is not abstract but has abstract attr %s" % (self, attr.name))
            for op in self.getAllOperations():
                if op.isAbstract:
                    raise MemopsError("%s is not abstract but has abstract operation %s" % (self, attr.name))
...
class MetaPackage(MetaModelElement):
    ...
    def addImportedPackage(self, importedPackage):
        ...
        if not isinstance(importedPackage, MetaPackage):
            raise MemopsError(
                "%s: addImportedPackage parameter should be MetaPackage, was %s" % (self, importedPackage)
            )
        ...
        if importedPackage in importedPackages:
            raise MemopsError("%s: addImportedPackage, %s already imported" % (self, importedPackage))

        if self in accessedPackages:
            raise MemopsError("%s: addImportedPackage, %s already accesssed" % (self, importedPackage))
    ...
    def removeImportedPackage(self, importedPackage):
        ...
        if importedPackage in importedPackages:
            ...
            raise MemopsError("%s: removeImportedPackage, %s was not imported" % (self, importedPackage))
    ...
    def checkValid(self, complete=False):
        ...
        for pp in importedPackages:
            if pp._MetaModelElement__dataDict["accessedPackages"].count(self) != 1:
                raise MemopsError("Two-way import/access link %s-%s broken. Bug (1)?" % (self, pp))

        for pp in accessedPackages:
            if pp._MetaModelElement__dataDict["importedPackages"].count(self) != 1:
                raise MemopsError("Two-way import/access link %s-%s broken. Bug (2)?" % (self, pp))

        # check for 'None'; container
        if self.container is None:
            # root package. Check name
            if self.name != ImpConstants.rootPackageName:
                raise MemopsError("Root package named %s, must be %s" % (str(self.name), ImpConstants.rootPackageName))
        ...
        if not isinstance(ModellingPackage, MetaPackage):
            raise MemopsError("No package %s found" % ImpConstants.modellingPackageName)

        Impl = ModellingPackage.getElement(ImpConstants.implementationPackageName)
        AccessControl = ModellingPackage.getElement(ImpConstants.accessControlPackageName)
        ...
        if self is Impl:
            if self._MetaModelElement__dataDict["importedPackages"]:
                raise MemopsError("Implementation package %s may not import other packages" % self.qualifiedName())
            ...
            for tag in ImpConstants.implementationClassNames:
                ...
                for tag in (ImpConstants.repositoryRole, ImpConstants.packageLocatorRole):
                    if not isinstance(cc.getElement(tag), MetaRole):
                        raise MemopsError("MemopsRoot class '%s' does not contain role %s" % (cc, tag))

        # check container packages:
        if self.__containedPackageNames:
            if accessedPackages or importedPackages:
                raise MemopsError("%s: Packages containing other packages can not import or access" % (self,))

            if (
                self.__classNames
                or self.__dataObjTypeNames
                or self.__dataTypeNames
                or self.__exceptionNames
                or self.__constantNames
            ):
                raise MemopsError("%s: Packages containing other packages can contain nothing else" % (self,))
            ...
            # name style check:
            if self.name[0] not in ImpConstants.lowercase and self.container is not None:
                print("WARNING, name of %s does not start with lower case" % self)
            ...
            if self is not Impl and hasNonAbstract and not topObjects:
                raise MemopsError("%s: has non-abstract classes but lacks TopObject" % (self,))

            if len(topObjects) > 1:
                raise MemopsError("%s: has several TopObjects: %s" % (self, topObjects))

            # check package importing
            if self is not Impl:
                ll = self.importedPackages
                if Impl not in ll:
                    raise MemopsError("package %s does not import the Implementation package" % self)
        ...
        if self is RootPackage:
            ...
            for ss in (ImpConstants.baseClassName, ImpConstants.baseDataTypeObjName):
                ...
                for obj in ll:
                    ...
                    tmp = [xx for xx in subtypes if xx._MetaModelElement__dataDict["supertypes"][0] is not obj]
                    if tmp:
                        raise MemopsError("%s: classes should have %s as first supertype" % (tmp, obj))
...
class MetaClass(ComplexDataType):
    ...
    parameterData.update(
        {
            ...
            "parentRole": {"getterFunc": "getParentRole", "setterFunc": "unsettable", "default": None},
        }
    )
    ...
    def __init__(self, **params):
        ...
        if (
            params.get("name") == ImpConstants.dataRootName
    ...
    def removeDestructorCodeStub(self, tag):
        ...
        if tag in self._MetaModelElement__dataDict["destructorCodeStubs"]:
    ...
    def removePostDestructorCodeStub(self, tag):
        ...
        if tag in self._MetaModelElement__dataDict["postDestructorCodeStubs"]:
    ...
    def delete(self):
        ...
        for role in self.getAllRoles():
            if role.isImplementation and role.otherRole is None:
                ...
                raise MemopsError("Attempt to delete %s, class has non-implementation roles" % self)
    ...
    def getChildRoles(self):
        """get child roles"""
        return [x for x in self.getAllRoles() if x.hierarchy == ImpConstants.child_hierarchy]
    ...
    def checkValid(self, complete=False):
        ...
        if self.isDerived:
            ...
            for x in self.attributes:
                if not x.isDerived or x.isImplementation or x.name in keyNames:
                    raise MemopsError(
                        "Element %s of derived class %s neither derived, implementation nor a key" % (x.name, self)
                    )
            for role in self.roles:
                if not (role.isDerived or role.isImplementation or role.name in keyNames or role is self.parentRole):
                    raise MemopsError(
                        "Element %s of derived class %s neither derived, implementation nor a key" % (role.name, self)
                    )
                ...
                if otherRole:
                    ...
                    for op in otherRole.container.getAllOperations():
                        if op.target is otherRole and op.opType == "get" and not op.isImplicit:
                            break
                    else:
                        raise MemopsError(
                            "Link %s to derived class %s must have manually defined 'get' function" % (otherRole, self)
                        )
            ...
            if not self.isAbstract:
                ...
                if not factoryFunction or factoryFunction.isImplicit:
                    raise MemopsError(
                        "Derived class %s/%s must have manually defined factory function" % (otherRole, self)
                    )
        ...
        for tag in self.getKeyNames():
            ...
            if keyElement is None:
                raise MemopsError("%s keyName %s is not an element in the class" % (self, tag))
            ...
            if keyElement.hicard != keyElement.locard:
                raise MemopsError("%s key %s does not have n..n cardinality" % (self, tag))
            if isinstance(keyElement, MetaRole):
                if not self.canAccess(keyElement.valueType):
                    raise MemopsError("%s: key %s is link into nonacessible package" % (self, tag))

                if TopObject in self.getAllSupertypes() and TopObject not in keyElement.valueType.getAllSupertypes():
                    raise MemopsError("%s: key %s is link to non-topObject" % (self, tag))

        # check name length
        if len(self.name) > ImpConstants.maxClassNameLength:
            raise MemopsError(
                "%s: name %s longer than %s characters" % (self, self.name, ImpConstants.maxClassNameLength)
            )
        ...
        for role in self.getAllRoles():
            if role.hierarchy == ImpConstants.child_hierarchy:
                for cc in role.valueType.getNonAbstractSubtypes():
                    ...
                    if cname in dd:
                        raise MemopsError(
                            "%s has two child classes named %s;\n links are %s and %s" % (self, cname, role, dd[cname])
                        )
        ...
        if parentRole is not None:
            ...
            if parentRole.otherRole is None:
                if not parentRole.isAbstract:
                    raise MemopsError("%s: non-Abstract class with one-way parentRole" % self)
        ...
        allSupertypes = [x for x in self.getAllSupertypes() if x.name != "ComplexDataType"]

        if Base not in allSupertypes and not self.isAbstract:
            raise MemopsError("%s: non-abstract class is not subtype of %s" % (self, Base))

        if self.container is Impl:
            # Implementation package

            if ImplObject not in allSupertypes and not self.isAbstract:
                raise MemopsError("%s: non-abstract Implementation class is not subtype of %s" % (self, ImplObject))

            if parentRole is not None and parentClass is not DataRoot:
                raise MemopsError("%s: has incorrect parentClass %s" % (self, parentClass))

            if self is DataRoot:
                # DataRoot

                if not self.partitionsChildren:
                    raise MemopsError("%s: %s does not have partitionsChildren True" % (self, DataRoot))
                if parentRole is not None:
                    raise MemopsError("%s: subType of %s has parentRole %s" % (self, DataRoot, parentRole))
                if self.container.topObjectClass is not self:
                    raise MemopsError("%s has topObjectClass %s, should be %s" % (Impl, DataRoot, self))

            if self is TopObject:
                if not self.partitionsChildren:
                    raise MemopsError("%s: %s does not have partitionsChildren True" % (self, TopObject))

        else:
            if DataObject not in allSupertypes:
                if not self.isAbstract:
                    raise MemopsError("%s: non-abstract class is not subtype of %s" % (self, DataObject))

                if [x for x in self.roles if x.otherRole and not x.implementation]:
                    raise MemopsError(
                        "%s: class with non-implementation or two-way role is not subtype of %s" % (self, DataObject)
                    )
            ...
            if TopObject in allSupertypes:
                ...
                if parentClass is not DataRoot:
                    raise MemopsError(
                        "%s: TopObject has incorrect parentClass %s, should be %s" % (self, parentClass, DataRoot)
                    )
                ...
                for role in DataRoot.getAllRoles():
                    ...
                    raise MemopsError("%s: TopObject is not pointed to by a 'current' link" % (self,))
                ...
                if parentRole is not None:
                    if parentClass.container is not self.container:
                        raise MemopsError("%s: has parentClass %s from different package" % (self, parentClass))

        # check that links are not inherited across package boundaries
        # except for certain one-way links
        if self is not DataObject:
            if [
                x for x in self.roles if x.otherRole or not x.isAbstract and not x.isDerived and not x.isImplementation
            ]:
                pp = self.container
                ll = [x for x in self.getAllSubtypes() if x.container is not pp]
                if ll:
                    raise MemopsError("%s: has both roles and out-of-package subclasses:\n%s" % (self, ll))
        ...
        if not self.isAbstract and DataRoot not in allSupertypes:
            ...
            if not self.keyNames and parentRole.otherRole.hicard != 1:
                raise MemopsError("%s: non-abstract class lacks keyNames and is not only child" % (self,))
        ...
        if not self.partitionsChildren:
            ...
            if ll:
                raise MemopsError("%s does not partition children, but its superclass(es) do:\n%s" % (self, ll))
        ...
        while sup is not None:
            ...
            if sup is self:
                raise MemopsError("%s (%s) is (indirectly) its own parentClass" % (self, tag))
        ...
        if not self.isAbstract:
            for role in self.getAllRoles():
                if role.isAbstract:
                    raise MemopsError("%s is not abstract but has abstract role %s" % (self, role.name))
        ...
        for tag in ("destructorCodeStubs", "postDestructorCodeStubs"):
            ...
            if destructorCode:
                if self.isAbstract:
                    raise MemopsError("Abstract class %s has special destructor" % (self))
        ...
        if serial is not None:
            if not isinstance(serial, MetaAttribute):
                raise MemopsError("%s - non-MetaAttribute named %s" % (serial, serial.name))

            if (
                not serial.isAutomatic
                or serial.hicard != 1
                or serial.locard != 1
...
class MetaDataObjType(ComplexDataType):
    ...
    def checkValid(self, complete=False):
        ...
        BaseDataType = self.metaObjFromQualName(
            ".".join(
                [
                    ImpConstants.modellingPackageName,
                    ImpConstants.implementationPackageName,
        ...
        if self.isChangeable:
            raise MemopsError(
                """%s has changeable attribute - 
       Changeable MetaDataObjTypes not implemented"""
                % self
            )
        ...
        if BaseDataType not in self.getAllSupertypes():
            if not self.isAbstract:
                raise MemopsError("%s: non-abstract DataObjType is not subtype of %s" % (self, BaseDataType))
...
class MetaDataType(AbstractDataType):
    ...
    def addTypeCode(self, tag, value):
        ...
        if type(value) != str:
            raise MemopsError("%s typeCode %s value %s is not a string" % (self, tag, str(value)))
    ...
    def removeTypeCode(self, tag):
        ...
        if tag in self._MetaModelElement__dataDict["typeCodes"]:
    ...
    def checkValid(self, complete=False):
        ...
        if not self.isOpen and not enumeration:
            raise MemopsError("%s isOpen set to %s for non-enumeration" % (self, self.isOpen))

        for ee in enumeration:
            if not self.isValid(ee):
                raise MemopsError("%s : Illegal enumeration value %s" % (self.qualifiedName(), str(ee)))
    ...
    def checkValid(self, complete=False):
        ...
        if supertypes:
            ...
            if supertype._MetaModelElement__dataDict["subtypes"].count(self) != 1:
                raise MemopsError("Two-way supertype link %s-%s is broken. Bug (3)?" % (supertype, self))
        ...
        for obj in self._MetaModelElement__dataDict["subtypes"]:
            if obj.supertype is not self:
                raise MemopsError("Two-way supertype link %s-%s is broken. Bug (4)?" % (self, obj))
        ...
        if not self.isImplicit:
            ...
            if len(pars) > 1:
                raise MemopsError("%s: more than one return parameter : %s" % (self, [x.name for x in pars]))
...
class MetaOperation(HasParameters):
    ...
    def removeConstructorCodeStub(self, tag):
        ...
        if tag in self._MetaModelElement__dataDict["codeStubs"]:
    ...
    def checkValid(self, complete=False):
        ...
        implPackage = self.metaObjFromQualName(".".join([IC.modellingPackageName, IC.implementationPackageName]))
        ...
        if opTypeInfo["group"] != "other":
            ...
            if (opTypeInfo["group"] not in ("query", "otherQuery")) != (not (self.isQuery)):
                # Queries *must* be isQuery. Other opTypes may not, 'other' may choose
                raise MemopsError("%s has group == %s but isQuery == %s" % (self, opTypeInfo["group"], self.isQuery))
        ...
        if targetTag != "masterOp":
            ...
            for par in parameters:
                if not par.isImplicit:
                    raise MemopsError("%s with opType %s has explicit parameter %s" % (self, self.opType, par))
        ...
        if not isinstance(target, targetTypes[targetTag]):
            raise MemopsError("%s: target %s does not fit target type %s" % (self, target, targetTag))

        # type specific checks
        if targetTag == "container":
            # container target
            if target is not self.container:
                raise MemopsError("%s: target %s differs from op container %s" % (self, target, self.container))

        elif targetTag == "masterOp":
            # masterOp target
            if self.opSubType is None:
                if target is not self:
                    raise MemopsError("%s: target should be operation itself, is %s" % (self, target))

            else:
                if target.opType != self.opType or target.target != target or target.opSubType is not None:
                    raise MemopsError("%s: target should be opType=None version of operation, is %s" % (self, target))

        elif targetTag == "ChildClass":
            # ChildClass target
            if target.getParentClass() is not self.container:
                raise MemopsError("%s: target should be childclass of %s, is %s" % (self, self.container, target))

        else:
            # ClassElement target
            if target.container not in self.container.getAllSupertypes():
                raise MemopsError("%s: target %s must be element of same class as op or a superclass" % (self, target))

        # special case - findFirst and findAll
        if opType in ("findFirst", "findAll") and isinstance(target.valueType, MetaDataType):
            raise MemopsError("%s: %s or operation has Datatype attribute target %s" % (self, opType, target))
        ...
        for supertype in allSupertypes[1:]:
            ...
            if superElem is not None:
                ...
                parameterData = self.parameterData
                for tag in parameterData.keys():
                    if tag in ("codeStubs", "isAbstract", "container", "documentation", "guid"):
                        continue
                    ...
                    if val == superval:
                        ...
                        for tt, vv in superval.items():
                            if val.get(tt) != vv:
                                raise MemopsError("%s overrides %s but tagged value %s differs" % (self, superElem, tt))
                                raise MemopsError
                ...
                if self.opType != "init":
                    ...
                    if len(parameters) != len(superElem.parameters):
                        raise MemopsError("%s overrides %s but number of parameters is different" % (self, superElem))
        ...
        if not self.isImplicit:
            ...
            if len(pars) == 1 and not pars[0].isImplicit and pars[0].name != "value":
                if self.container.container is not implPackage:
                    ...
                    print(("WARNING, %s: single input parameter is not named 'value' but %s") % (self, pars[0].name))
            ...
            for par in pars:
                if par.taggedValues.get("isSubdivided"):
                    if len(pars) >= 2 and par is pars[-2]:
                        if pars[-1].taggedValues.get("isSubdivided"):
                            ...
                            raise MemopsError(
                                "%s: isSubdivided parameter %s is not the last input parameter" % (self, par.name)
                            )
                ...
                elif (par.locard == 0 and par.hicard == 1) or par.defaultValue is not None:
...
class MetaRole(ClassElement):
    ...
    def setOtherRole(self, value):
        ...
        if value is None:
            if current:
                raise MemopsError("%s - attempt to unset (partially) set otherRole" % (self,))
            else:
            ...
            if current is value and reverse is self:
                ...
                if self.hierarchy in (ImpConstants.parent_hierarchy, ImpConstants.child_hierarchy):
                ...
                raise MemopsError("%s - attempt to overwrite (partially) set otherRole with value %s" % (self, value))
    ...
    def removeOtherRole(self):
        ...
        if current is None:
            raise MemopsError("%s - attempt to remove non-existing otherRole" % (self,))
        else:
    ...
    def checkValid(self, complete=False):
        ...
        Impl = self.metaObjFromQualName(
            ".".join([ImpConstants.modellingPackageName, ImpConstants.implementationPackageName])
        )
        DataRoot = Impl.getElement(ImpConstants.dataRootName)
        Base = Impl.getElement(ImpConstants.baseClassName)
        DataObject = Impl.getElement(ImpConstants.dataObjClassName)
        ...
        if self.canAccess(valueType):
            ...
            if otherRole is None:
                ...
                if not (
                    DataObject is self.container
                    or (DataRoot is self.container and TopObject in valueType.getAllSupertypes())
                ):
                    # MemopsRoot.currentTopObject has special handling.
                    # any other case is illegal.
                    raise MemopsError("%s - one-way link to %s in non-imported package" % (self, valueType))

            elif self.isDerived:
                pass

            elif self.locard != 0:
                raise MemopsError("%s - mandatory link to %s in non-imported package" % (self, valueType))
            elif self.changeability == ImpConstants.frozen:
                raise MemopsError("%s - frozen link to %s in non-imported package" % (self, valueType))
        else:
            raise MemopsError("%s - link to %s in non-imported package" % (self, valueType))
            ...
            if constraints:
                if container.container is not otherRole.container.container:
                    if not self.canAccess(otherRole):
                        raise MemopsError("%s: constraints on wrong side of interpackage link" % (self,))

                elif hicard != 1 and otherRole.hicard == 1:
                    raise MemopsError("%s: constraints on wrong side of one-to-many link" % (self,))
        ...
        if self.hierarchy == ImpConstants.parent_hierarchy:
            if (
                ...
                or self.isDerived
                or self.isAutomatic
                or self.isImplementation
                or (otherRole is None and not self.isAbstract)
            ):
                raise MemopsError(
                    """%s - invalid parentRole :
...
isDerived:%s, isAutomatic:%s, isImplementation:%s, otherRole:%s"""
            ...
            if pr is not self:
                raise MemopsError(
                    "% has hierarchy %s, but %s is class parent" % (self, ImpConstants.parent_hierarchy, pr)
                )
            ...
            raise MemopsError("%s - non-parentRole has aggregation %s" % (self, self.aggregation))
        ...
        if self.partitionsChildren:
            if self.hierarchy != ImpConstants.no_hierarchy:
                raise MemopsError("%s: only crosslinks can have partitionsChildren True" % (self,))

        if otherRole is None:
            if not self.isAbstract and not self.isDerived and not self.isImplementation:
            ...
            if self.isAbstract != otherRole.isAbstract:
                raise MemopsError("%s: association has abstract and non-abstract role" % self.qualifiedName())

            # check coherence with otherRole
            if otherRole.otherRole is not self:
                raise MemopsError("%s otherRole incoherent across Association" % self.qualifiedName())
            elif otherRole.container is not valueType:
                raise MemopsError("%s valueType differs from otherRole.container" % self.qualifiedName())
            elif container is not otherRole.valueType:
                raise MemopsError("%s otherRole.valueType differs from container" % self.qualifiedName())

            # check limits on cardinalities (NB may later be relaxed)

            # limit on bi-ordering:
            if self.isOrdered and otherRole.isOrdered and not self.isDerived and not self.isImplementation:
                raise MemopsError(
        ...
        if Base not in allSupertypes and not self.isAbstract and not self.isImplementation:
            raise MemopsError(
                "%s: Non-abstract, non-implementation role in class that does not inherit from %s" % (self, Base)
            )
            ...
            if superElem is not None:
                ...
                if otherRole is None:
                    if superval is not None:
                        raise MemopsError("one-way link %s overrides two-way link %s:" % (self, superval))
...
class MetaAttribute(ClassElement):
    ...
    def checkValid(self, complete=False):
        ...
        if not isinstance(self.valueType, MetaDataType) and defaultValue:
            raise MemopsError("%s - default value only when valueType is a MetaDataType" % (self,))
        ...
        if defaultValue:
            ...
            if len(defaultValue) > self.hicard and self.hicard != infinity:
                raise MemopsError(
                    "%s - default value %s longer than hicard %s" % (self, self.defaultValue, self.hicard)
                )

            if len(defaultValue) < self.locard:
                raise MemopsError(
                    "%s - default value %s shorter than locard %s" % (self, self.defaultValue, self.locard)
                )
            ...
            for dv in defaultValue:
                if not self.valueType.isValid(dv):
                    raise MemopsError("%s - default value %s from  %s is invalid" % (self, dv, str(self.defaultValue)))

        # package access check
        if not self.canAccess(self.valueType):
            raise MemopsError("%s - cannot access valueType %s" % (self, self.valueType))

        # special check:
        if isinstance(self.valueType, MetaClass):
            raise MemopsError("%s -attribute can not have class valueType: %s" % (self, self.valueType))
...
class MetaParameter(AbstractValue):
    ...
    def checkValid(self, complete=False):
        ...
        if Base is not None and Base not in superValueTypes:
            # raise MemopsError("%s valueType %s does not descend from %s"
            #                  % (self, valueType, Base))
            # NBNB TBD tenporary HACK - test should be put back soonest
            print("WARNING - %s valueType %s does not descend from %s" % (self, valueType, Base))
        ...
        if self.defaultValue:
            if self.direction != ImpConstants.in_direction:
                raise MemopsError("%s - non-input parameter has explicit default %s" % (self, self.defaultValue))

            if self.hicard != 1:
                raise MemopsError("%s - has explicit default %s and hicard %s" % (self, self.defaultValue, self.hicard))

            if self.locard != 0:
                raise MemopsError("%s - is mandatory but has explicit default %s" % (self, self.defaultValue))
            ...
            if not self.valueType.isValid(self.defaultValue):
                raise MemopsError("%s - default value %s is invalid" % (self, str(self.defaultValue)))

        # optional parameters
        if self.locard == 0 and self.hicard == 1 and self.direction != ImpConstants.in_direction:
        ...
        if self.taggedValues.get("isSubdivided"):
            if self.direction != ImpConstants.in_direction:
                raise MemopsError("%s - isSubdivided and direction is not %s" % (self, self.direction))
            ...
            if self.hicard == 1:
                ...
                if skDict not in self.valueType.getAllSupertypes():
                    raise MemopsError("%s - isSubdivided, hicard == 1 and type is not %s" % (self, skDict))
        ...
        if isinstance(container, MetaOperation):
            for supertype in container.container.getAllSupertypes()[1:]:
                superContainer = supertype._MetaModelElement__elementDict.get(container.name)
        ...
        if superContainer is not None and superContainer.opType != "init":
            ...
            if superElem is None:
                raise MemopsError(
                    "%s: new parameter %s in overriding %s " % (container, self.name, superContainer.__class__.__name__)
                )
                    ...
                    for tag in parameterData.keys():
                        ...
                        if val == superval:
                            ...
                            for tt, vv in superval.items():
                                if val.get(tt) != vv:
                                    raise MemopsError(
                                        "%s overrides %s but tagged value %s differs" % (self, superElem, tt)
                                    )
...
class MetaConstraint(MetaModelElement):
    ...
    def checkValid(self, complete=False):
        ...
        if isinstance(self.container, ClassElement) and self.container.isDerived:
            raise MemopsError("MetaConstraint %s is attached to derived element" % self.qualifiedName())

        # check if container is Implementation
        if isinstance(self.container, ClassElement) and self.container.isImplementation:
            raise MemopsError("MetaConstraint %s is attached to Implementation element" % self.qualifiedName())
        ...
        for codeTag in codeStubs.keys():
            ...
            if codeString is not None and codeString.find("isValid") == -1 and len(codeString.split("\n")) != 1:
                raise MemopsError(
                    """MetaConstraint:
%s
