...
from memops.metamodel import ImpConstants, ModelTraverse_py_2_1, TaggedValues
from memops.metamodel.ModelPortal import ModelPortal
from memops.universal.ElementTree import ElementInclude, ElementTree
...
def writeModel(
    topPackage=None, modelPortal=None, rootFileName=None, rootDirName=None, releaseVersion=None, skipImplicit=None, **kw
):
    """write XML file version of model"""

    if modelPortal is None:
        if topPackage is None:
            raise MemopsError("writeModel must have either modelPortal or topPackage parameter")
        else:
...
def readModel(
    rootFileName=None, rootDirName=None, excludePackageNames=None, includePackageNames=None, checkValidity=True, **kw
):
    """XML model reader
...
class XmlModelRead(TextWriter_py_2_1.TextWriter_py_2_1):
    ...
    def readModel(self):
        ...
        if hasattr(self, "rootFile") and self.rootFile:
            ...
            for elem in modelElem:
                ...
                if classNameMapping.get(tag) is None:
                    ...
                    if tag == XINCLUDE_FALLBACK:
                        ...
                        if packageName:
                            if self.includePackageNames and packageName not in self.includePackageNames:
                                continue
                            if self.excludePackageNames and packageName in self.excludePackageNames:
                                continue
                        ...
                        try:
                            ...
                            if fallback is None:
                                raise MemopsError("Error in file %s - include file %s not found" % (absFilename, href))
                            else:
    ...
    def loadDelayedData(self):
        ...
        while ll:
            ...
            if not modified:
                ...
                raise MemopsError("Delayed load data do not load (no container). %s objects remain" % len(ll))
            ...
            if _tempData:
                        ...
                        if pType == "Boolean":
                            ...
                            for ii in range(len(val)):
                                ...
                                if vv == xmlTrue:
                                    ...
                                    raise MemopsError("illegal Boolean %s in %s parameter %s" % (vv, obj, tag))
                            ...
                            for ii in range(len(val)):
                                try:
                                    ...
                                    raise MemopsError(
                                        "%s parameter %s - link to nonexisting object %s" % (obj, tag, val[ii])
                                    )
                            ...
                            for ii in range(len(val)):
                                try:
                                    ...
                                    raise MemopsError("%s parameter %s - error converting %s" % (obj, tag, val[ii]))
                ...
                for tag, hicard, val in valueTypeData:
                    ...
                    if typeCode == "Boolean":
                        ...
                        fromString = getattr(genConstants.baseDataTypeModule, typeCode).fromString
...
class XmlModelGen(TextWriter_py_2_1.TextWriter_py_2_1, ModelTraverse_py_2_1.ModelTraverse_py_2_1):
    codeDirName = genConstants.xmlCodeDir
    ...
    def __init__(self, **kw):
        ...
        ic = ImpConstants
        Impl = self.modelPortal.metaObjFromQualName(".".join([ic.modellingPackageName, ic.implementationPackageName]))
        ...
        if not isinstance(self.modelPortal, ModelPortal):
            raise MemopsError("ModelTraverse input %s is not a ModelPortal" % self.modelPortal)
        ...
        try:
            ...
            while oldPackages:
                ...
                if ll:
                    ...
                    for tag in ("dataTypes", "constants", "exceptions", "dataObjTypes", "classes"):
    ...
    def processBranchPackage(self, package):
        ...
        if package.container:
            ...
            for pp in package.containedPackages:
                self.writeXmlInclude(os.path.join(pp.name, packageFile), objName=pp.qualifiedName(), isMandatory=False)
        else:
            # root package
            for pp in package.containedPackages:
    ...
    def processLeafPackage(self, package):
        ...
        for tag in ("dataTypes", "constants", "exceptions", "dataObjTypes", "classes"):
            self.writeNewline()
            self.writeComment("Start %s contained in %s " % (tag, package.qualifiedName()))
            for obj in getattr(package, tag):
                self.writeXmlInclude("%s.%s" % (obj.name, self.fileSuffix))
            self.writeComment("End %s contained in %s " % (tag, package.qualifiedName()))
    ...
    def writeElementRecursive(self, metaObj):
        ...
        if isinstance(metaObj, MetaModel.MetaClass):
            ...
            if self.TopObject in supertypes and self.DataRoot not in supertypes:
                # handle special case - currentTopObject link and its operations
                #  stored on wrong side

                ll = [
                    x
                    for x in self.DataRoot._MetaModelElement__elementDict.values()
                    if isinstance(x, MetaModel.MetaRole) and x.valueType is metaObj and x.otherRole is None
                ]
                for obj in ll:
            ...
            for junk, obj in ll:
                if isinstance(obj, MetaModel.MetaRole):
                    ...
                    if otherRole is None:
                        if (
                            self.DataRoot in obj.container.getAllSupertypes()
    ...
    def startXmlElement(self, metaObj):
        ...
        self.fp.write('%s<%s name="%s"' % (ind, metaObj.__class__.__name__, metaObj.name))
    ...
    def writeList(self, metaObj, tag):
        ...
        if value:
            ...
            for val in value:
                # write element. NB strings can *not* be indented
                self.writeOne("<item>%s</item>" % (self.toXmlString(metaObj, tag, val)))
    ...
    def writeXmlInclude(self, filePath, objName=None, isMandatory=True):
        ...
        if isMandatory and not objName:
            self.writeOne('<xi:include href="%s" xmlns:xi="http://www.w3.org/2001/XInclude"/>' % filePath)
        else:
            self.writeOne('<xi:include href="%s" xmlns:xi="http://www.w3.org/2001/XInclude">' % filePath)
            self.indent += self.INDENT
            if objName:
