...
operationData["get"]["subOps"] = {
    None: {
        ...
        "parameters": [
            {
                "name": "result",
                "direction": ImpConstants.return_direction,
                "useCollection": True,
...
operationData["findFirst"]["subOps"] = {
    None: {
        "parameters": [
            {
                "name": "conditions",
                "direction": ImpConstants.in_direction,
...
operationData["checkDelete"]["subOps"] = {
    None: {
        "opDocTemplate": """checkDelete for %s:
   ...
   *Implementation function* - should be called only by API delete function.""",
        "parameters": [
            ...
            {
                "name": "objsToBeChecked",
                "direction": ImpConstants.in_direction,
                "parDocumentation": "List of objects to be checked for deletion (input/ouput)",
                "target": "memops.Implementation.MemopsObject",
                "hicard": genConstants.infinity,
                "locard": 0,
                "isOrdered": True,
            ...
            },
            {
                "name": "linkCounter",
                "direction": ImpConstants.in_direction,
                "parDocumentation": "Dictionary to track links with locard checks (input/output)",
...
def getTarget(metaOp, opData=None):
    ...
    if infoDict is None:
        raise MemopsError("%s: operation has unrecognised opType %s" % (metaOp, str(opType)))
    ...
    if targetTag == "container":
        ...
        if metaOp.opSubType is None:
            ...
            if not masterOp:
                raise MemopsError("%s: no master operation (subType:None) found" % (metaOp,))
        ...
        if targetTag == "masterOp":
                ...
                for childClass in childRole.valueType.getAllSubtypes():
                    if childClass.name == targetName:
                        if result is None:
                            ...
                            raise MemopsError(
                                "%s: Ambiguous child class target for operation %s" % (metaOp, masterOp.name)
                            )
            if result is None:
                raise MemopsError("%s: No child class target for operation %s" % (metaOp, masterOp.name))
            ...
            if infoDict.get("useCollection"):
                ...
                ll = [x for x in metaOp.container.getAllClassElements() if x.baseName == targetName]
                if ll:
                    result = ll[0]
                else:
                    result = None
            ...
            if not result:
                raise MemopsError(
                    "%s, opType %s, targetTag %s: no target named %s found" % (metaOp, opType, targetTag, targetName)
                )
    #
    return result
