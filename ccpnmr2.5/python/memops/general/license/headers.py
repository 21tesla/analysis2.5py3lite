...
def setLicenses(directory=None, licenseDir="license", warnSkippedFiles=(not 0), mode="act"):
...
def doSetLicenses(curDir, licenseDir, infoModule, level, warnSkippedFiles, mode):
    ...
    if "%s.py" % infoFileName in contents:
        if infoFileName in sys.modules:
            del sys.modules[infoFileName]
        infoModule = __import__(infoFileName)
        setattr(infoModule, "moduleLocation", os.path.join(curDir, "%s.py" % infoFileName))
    ...
    if infoModule is None:
        ...
        if infoModule.includeDirs:
            ...
            if infoModule.excludeDirs:
                raise ("Error in %s/%s.py: \nYou cannot have both includeDirs and excludeDirs" % (curDir, infoFileName))
        else:
            includeDirs = []
    ...
    for ii in infoRange:
        ...
        if infi == "all":
            ...
            includeFileMatches = [re.compile(x) for x in dict["includeFileMatches"]]
            for ss in oldFiles:
                if ss in infi:
                    addHeader(curDir, ss, ii, infoModule, mode)
                else:
...
def addHeader(directory, fName, paramIndex, infoModule, mode):
    ...
    if headerStart is None and headerEnd is None:
        ...
        for ii in range(len(lines)):
            line = lines[ii]
            if line.find(licenseStartLine) == 0 and line.rfind(referenceEndLine) == (
                len(line) - len(referenceEndLine) - 1
            ):
                headerStart = ii
                headerEnd = ii + 1

    # add headers:
    if headerStart is not None and headerEnd is not None and headerStart < headerEnd:
        # there is a previous header - replace it

        if lines.count(headerStartLine) > 1 or lines.count(headerEndLine) > 1:
            print("WARNING, file %s has (partly) multiple license headers" % fileName)
        ...
        if not (headerStart is None and headerEnd is None):
            ...
            print("WARNING, file %s has partial license header" % fileName)
            ...
            for expr in formatDict["ignoreStartLines"]:
                ...
                if startIndex == -1:
                    ...
                    while ind == -1:
                        ii = ii + 1
                        if ii >= nlines:
                            raise (" ERROR, file %s contains unclosed header comment" % fileName)
                        line = lines[ii]
...
if __name__ == "__main__":
    ...
    setLicenses(directory=directory, mode="test")
