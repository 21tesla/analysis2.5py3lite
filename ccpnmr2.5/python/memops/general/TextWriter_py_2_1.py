...
class TextWriter_py_2_1:
    ...
    def clearOutDir(self, topDirName, forceClearOut=False):
        ...
        if os.path.exists(topDirName):
            if not os.path.isdir(topDirName):
                raise MemopsError("clearOutDir input %s is not a directory" % topDirName)
        else:
            return
        ...
        while base:
            if base in self.clearOutGuards:
                raise MemopsError("%s may not be cleared out - contains %s" % (topDirName, base))
            stem, base = os.path.split(stem)
        ...
        for dirName in dirs:
            ...
            if not forceClearOut:
                for ss in self.clearOutGuards:
                    if ss in ll:
                        raise MemopsError("cannot clear out %s, contains file or directory %s" % (dirName, ss))
    ...
    def openFile(self, fileName):
        ...
        if self.fp:
            raise MemopsError(
                "trying to open file '%s' while another file '%s' is still open" % (fileName, self.fileName)
            )
    ...
    def getObjFileName(self, metaObj, absoluteName=True, addSuffix=False):
        ...
        if metaObj.container is None:
            ...
            if isinstance(metaObj, MetaModel.MetaPackage) and (metaObj.containedPackages or not self.classesInPackage):
                pathList.append(self.packageFile)

            elif not isinstance(metaObj.container, MetaModel.MetaPackage):
                raise MemopsError(" file names not implemented for objects of type %s" % (metaObj.__class__.__name__,))
    ...
    def getVersionString(self, metaobj=None, scriptName=None, scriptRevision=None, objRevision=None, date=None):
        """
        ...
        if metaobj:
            ...
            pkgLine = "  from data model element %s revision %s" % (metaobj.qualifiedName(), objRevision)
        else:
            pkgLine = ""
    ...
    def getCreditsString(self, metaobj, programType):
        ...
        if packageGroup is None:
            raise MemopsError("object %s package lacks packageGroup tagged value" % metaobj)
