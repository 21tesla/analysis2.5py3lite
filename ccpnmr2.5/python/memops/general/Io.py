...
from memops.universal.Io import doConvertStringToFileName, getTopDirectory, joinPath, normalisePath, removePath
    ...
    try:
        ...
        if newPath != oldPath:
            ...
            if os.path.isdir(oldPath):
                ...
                print(
                    "Copying directory %s to %s (this might take some time if there are big files)" % (oldPath, newPath)
                )
                shutil.copytree(oldPath, newPath)

                # but need toz remove all implementation files
                implPath = joinPath(newPath, ImpConstants.modellingPackageName, ImpConstants.implementationPackageName)
        ...
        if badTopObjects:
            if showWarning:
                showWarning(
                    "Incomplete save", "It looks like one or more files did not save completely, see console for list"
                )
...
def saveRepository(repository, newPath=None, createFallback=False, removeExisting=False, showYesNo=None):
    ...
    if repository.name == "userData":
        saveProject(
            project, newPath=newPath, createFallback=createFallback, removeExisting=removeExisting, showYesNo=showYesNo
        )
        return False
    ...
    topObjects = set()
    for topObject in project.topObjects:
        if topObject.isModified and not topObject.isDeleted and repository == topObject.findFirstActiveRepository():
...
def newProject(projectName, path=None, removeExisting=False, showYesNo=None):
    ...
    for name in repositoryNameMap.keys():
        fullPath = normalisePath(joinPath(path, projectName) + repositoryNameMap[name], makeAbsolute=True)
...
def loadProject(path, projectName=None, showWarning=None, askFile=None, askDir=None, suppressGeneralDataDir=False):
    """
    ...
    if project is not None and (not xmlUtil.areAllTopObjectsPresent(project) or not isGeneralDataOk(project)):
                    ...
                    if dataLocation == oldDirectory or (
                        ...
                        if os.path.exists(newPath):
                            warningMessages.append(
                                ...
                                % (dataStore.dataLocationStore.name, dataStore.serial, oldDirectory, newDataUrlPath)
                            )
        ...
        if topObject.isModified:
            ...
            if repository:
                ...
                if os.path.exists(origFile):
                    ...
                    if not os.path.exists(backupFile) or (modificationTime(backupFile) < modificationTime(origFile)):
                    ...
                    # one is stuffed
                    print(
                        'Warning: could not backup %s since could not find original file "%s"' % (topObject, origFile)
                    )
            else:
                # one is stuffed
...
def modifyPackageLocators(
    project, repositoryName, repositoryPath, packageNames, resetPackageLocator=True, resetRepository=False
):
    """
    ...
    if not repository:
        repository = project.newRepository(name=repositoryName, url=Implementation.Url(path=ss))
    ...
    for packageName in packageNames:
        ...
        if not packageLocator:
            raise ApiError("Cannot modify repository 'any' for package %s" % packageName)

        if resetPackageLocator:
            packageLocator.repositories = (repository,)
        elif repository not in packageLocator.repositories:
...
def getCcpFileString(fileString):
    ...
    """
    from memops.general.Constants import defaultFileNameChar, validFileNamePartChars

    return doConvertStringToFileName(fileString, validFileNamePartChars, defaultFileNameChar)
