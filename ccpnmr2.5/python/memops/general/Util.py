...
def getRepositoryInfo(repositoryId):
    ...
    if len(fields) >= 7:
        ...
        source = fields[5]
        state = fields[6]
        info = {"date": date, "file": file, "revision": revision, "source": source, "state": state}
    else:
        info = {}
...
def copySubTree(sourceObj, newParent, maySkipCrosslinks=False, topObjectParameters=None, objectMap=None):
    ...
    result = transferData(
        ...
        ignoreMissing=maySkipCrosslinks,
        useOptLinks=True,
    )
    #
    return result
...
def transferData(
    newParent, sourceObj, oldToNew=None, oldVersionStr=None, targetObjParams=None, ignoreMissing=True, useOptLinks=False
):
    ...
    try:
            ...
            if ll:
                ...
                if parent is newParent:
                    ...
                    if parent.root is newParent:
                        ...
                        obj = targetObj = curMap["class"](parent, isReading=True, **targetObjParams)
                    else:
        ...
        if oldVersionStr is None:
            # copy subtree
            delayedLoadLinksCopy(localOldToNew, crossLinkData, ignoreMissing=ignoreMissing)
            ...
            if guid not in newTopObjByGuid:
                ...
                raise Exception("CCPN API error: %s: guid %s already in use" % (targetObj, targetObj.__dict__["guid"]))
        ...
        if not targetObj.isDeleted:
            ...
            xmlImplementation.linkChildData(delayDataDict, targetObj, mapping, linkTopToParent=True)
...
def delayedLoadLinksCopy(objectDict, linkData, ignoreMissing=False):
    ...
    while linkData:
        ...
        if hicard == 1:
            ...
            if newVal:
                ...
                try:
                    ...
                    if ignoreMissing:
                        ...
                        raise Exception("%s: Out-of-subtree -to-one link %s cannot be copied" % (obj, name))
            ...
            if not done:
                # link could not be handled
                raise Exception("%s: Out-of-subtree link %s cannot be copied" % (obj, name))
...
def downlinkTagsByImport(root):
    ...
    leafPackages = metaUtil.topologicalSortSubgraph(leafPackages, "accessedPackages")
    tags = []
