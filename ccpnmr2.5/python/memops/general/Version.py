...
class Version:
    def __init__(self, major=0, minor=0, level="", release=0, timestamp=None, name="Test"):
    ...
    def __cmp__(self, other):
        ...
        result = ((self.major, self.minor) > (other.major, other.minor)) - ((self.major, self.minor) < (other.major, other.minor))

        if not result:
            result = (self.level > other.level) - (self.level < other.level)
...
def parseVersionString(s):
    ...
    if len(ll) == 3:
        ...
        raise ValueError("Invalid argument to getVersion: %s" % str(s))
...
def cmpVersionStrings(str1, str2):
    ...
    result = (tpl1[:2] > tpl2[:2]) - (tpl1[:2] < tpl2[:2])

    if not result:
        ll = [tpl1[2], tpl2[2]]
        result = (ll[0] > ll[1]) - (ll[0] < ll[1])
        ...
        if result:
            ...
            result = (tpl1[3] > tpl2[3]) - (tpl1[3] < tpl2[3])
