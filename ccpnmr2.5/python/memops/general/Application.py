...
def registerNotify(notify, classname, funcname, application=None, keyword=None):
    ...
    if application is None or keyword is None:
        assert application is None and keyword is None, "application = %s, keyword = %s, both must be None" % (
            application,
            keyword,
        )
...
class Application:
    ...
    def getValue(self, object, keyword, defaultValue=None, deleteAppData=False):
        ...
        appData = object.findFirstApplicationData(application=self.name, keyword=keyword)
