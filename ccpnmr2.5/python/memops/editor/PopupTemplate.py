...
class PopupTemplate(BasePopup):
    ...
    **Popup Template**
    ...
    This popup still needs its documentation.
    """
    ...
    def body(self, mainFrame):
        ...
        entry = Entry(frame, text="Entry", returnCallback=self.showWarning, tipText="Hint to user")
        entry.grid(row=0, column=1, sticky=Tkinter.W)

        self.check = CheckButton(
            frame, text="Checkbutton", selected=True, callback=self.updateObjects, tipText="Hint to user"
        )
        self.check.grid(row=0, column=2, sticky=Tkinter.W)

        # stick a button to the East wall
        button = Button(frame, text="Button", command=self.pressButton, tipText="Hint to user")
        ...
        self.checkButtons = CheckButtons(
            mainFrame, entries, selected=selected, select_callback=self.changedCheckButtons
        )
        ...
        selected = [str]
        self.partitionedSelector = PartitionedSelector(
            mainFrame,
            labels=labels,
            objects=objects,
            colors=["red", "yellow", "green", "#000080"],
            callback=self.toggleSelector,
            selected=selected,
        )
        ...
        self.radioButtons = RadioButtons(
            frame.frame, entries=entries, select_callback=self.checkRadioButtons, selected_index=1, relief="groove"
        )
        ...
        self.multiWidget = MultiWidget(
            self, Entry, options=None, values=None, callback=self.setKeywords, minRows=3, maxRows=5
        )
        ...
        tipTexts = ["Name of the whatsit", "Colour of the whatsit", "The whatsit's number", "The whatsit's keywords"]
        ...
        self.scrolledGraph.update(
            dataSets=[dataSet1, dataSet2], xLabel="X axis label", yLabel="Y axis label", title="Main Title"
        )
        ...
        label.grid(row=row, column=0, sticky=Tkinter.E)
        self.floatEntry = FloatEntry(
            mainFrame, text=3.14159265, tipText="Hint to user", returnCallback=self.floatEntryReturn
        )
    ...
    def selectFile(self):
        ...
        fileSelectPopup = FileSelectPopup(
            self, title="Choose file", dismiss_text="Cancel", selected_file_must_exist=True
        )
