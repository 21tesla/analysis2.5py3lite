...
class UrlViewerFrame(Frame):
    ...
    def __init__(
        ...
        show_storages=True,
        storage_closed=True,
        show_data_locations=True,
        data_location_closed=True,
        *args,
        **kw,
    ):
        ...
        row = 0
        label = ToggleLabel(self, text="Urls ", isArrowClosed=url_closed, callback=self.urlToggle)
        label.grid(row=row, column=0, sticky=Tkinter.W)
        self.url_radio = RadioButtons(
            self, entries=self.all_used_entries, select_callback=self.updateUrlList, selected_index=1
        )
        row = row + 1
        ...
        self.url_list = ScrolledMatrix(
            self.url_frame,
            headingList=headings,
            justifyList=justifies,
            initialRows=5,
            callback=self.selectUrl,
        ...
        if show_storages:
            label = ToggleLabel(self, text="Storages ", isArrowClosed=storage_closed, callback=self.storageToggle)
            label.grid(row=row, column=0, sticky=Tkinter.W)
            self.storage_radio = RadioButtons(
                self, entries=self.all_used_entries, select_callback=self.updateStorageList, selected_index=1
            )
            row = row + 1
            ...
            editWidgets = [
                ...
                None,
                None,
                self.storageUrlSerialWidget,
                None,
                self.storagePathWidget,
                None,
                None,
                None,
                None,
            ...
            editGetCallbacks = [
                ...
                None,
                None,
                self.getStorageUrlSerial,
                None,
                self.getStoragePath,
                self.toggleIsModifiable,
                None,
                None,
                None,
            ]
            ...
            self.storage_list = ScrolledMatrix(
                self.storage_frame,
                headingList=headings,
                justifyList=justifies,
                initialRows=8,
        ...
        if show_data_locations:
            label = ToggleLabel(
                self, text="Data locations ", isArrowClosed=data_location_closed, callback=self.dataLocationToggle
            )
            label.grid(row=row, column=0, sticky=Tkinter.W)
            row = row + 1
            ...
            self.data_location_list = ScrolledMatrix(
                self.data_location_frame,
                headingList=headings,
                justifyList=justifies,
                initialRows=3,
                editWidgets=editWidgets,
  ...
  """
    ...
    def getUrlPath(self, url):
        ...
        if self.dir_popup:
            ...
            self.dir_popup = FileSelectPopup(self, title="Select directory", file_types=file_types, show_file=False)
    ...
    def deleteUrl(self):
        ...
        if url:
            if showYesNo("Delete URL", 'Are you sure you want to delete URL "%s"?' % url.name):
                url.delete()
