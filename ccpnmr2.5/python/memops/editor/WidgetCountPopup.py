...
class WidgetCountPopup(BasePopup):
    ...
    def body(self, master):
        ...
        self.on_off_buttons = RadioButtons(
            master, entries=self.on_off_entries, select_callback=self.applyAuto, selected_index=ind
        )
        ...
        label.grid(row=row, column=0, sticky=Tkinter.W)
        self.freq_entry = IntEntry(master, text=60, returnCallback=self.applyAuto)
        ...
        buttons = createDismissHelpButtonList(
            master, texts=texts, commands=commands, help_msg=self.help_msg, help_url=self.help_url
        )
