...
class ApplicationPopup(BasePopup):
    ...
    **Application Template**
    ...
    def __init__(self, root, programName="ApplicationTemplate"):
        ...
        BasePopup.__init__(self, parent=root, title=programName, location="+100+100", class_=self.application.name)
    ...
    def body(self, guiParent):
        ...
        buttons = createDismissHelpButtonList(
            frame, dismiss_text="Quit", dismiss_cmd=self.quit, help_url=self.help_url, expands=1
        )
    ...
    def makeProjectMenu(self):
        ...
        menu.add_command(
            label="Close", shortcut="C", command=self.closeProject, tipText="Close the current CCPN project"
        )
        menu.add_command(
            label="Save",
            shortcut="S",
        ...
        menu.add_command(label="Quit", shortcut="Q", command=self.quit, tipText="Exit from the DANGLE program")
    ...
    def makeOtherMenu(self):
        ...
        fontsMenu = FontMenu(self.mainMenu, self.setFont, sizes=(8, 10, 12), doItalic=0, doBoldItalic=0, tearoff=0)
    ...
    def modalAskSaveFile(self):
        ...
        popup = SaveProjectPopup(self, project=self.project, dismiss_text="Cancel Quit", modal=True)
    ...
    def destroyPopup(self, popup_name):

        if popup_name in self.popups.keys():
            popup = self.popups.pop(popup_name)
            popup.destroy()
...
def setAreDialogsTransient(project, areTransient):
    ...
    if project:
        ...
        application.setValue(project, keyword="areDialogsTransient", value=areTransient, isBoolean=True)


def getAreDialogsTransient(project):

    if project:
        """ Below does not work in v2
    application = project.application
...
def launchApplication(filename=None):
    ...
    if filename:
        ...
        askdir = lambda title, prompt, initial_value, default_dir: askDir(
            title, prompt, initial_value, parent=top, extra_dismiss_text="Skip", default_dir=default_dir
        )
        askfile = lambda title, prompt, initial_value: askFile(
            title, prompt, initial_value, parent=top, extra_dismiss_text="Skip"
        )
        try:
            project = loadProject(path=file, showWarning=showWarning, askDir=askdir, askFile=askfile)
