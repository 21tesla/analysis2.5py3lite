...
implPackageName = "%s.%s" % (ImpConstants.modellingPackageName, ImpConstants.implementationPackageName)
...
repositoryTag = "$Name:  $"
repositoryId = "$Id: XmlGen.py,v 1.10.2.2 2011/05/04 14:03:57 rhfogh Exp $"
...
class XmlGen(ModelTraverse):
    ...
    def initLeafPackage(self, package):
        ...
        if package is self.implPackage:
            ...
            for pp in self.modelPortal.leafPackagesByImport():
                ...
                # set top level map
                mapping["abstractTypes"] = abstractTypes = {
                    ".prefix": prefix,
                    ".qName": qName,
                    ".name": "abstractTypes",
                }
                mapping["exolinks"] = exolinks = {".prefix": prefix, ".qName": qName, ".name": "exolinks"}
    ...
    def processExoLink(self, clazz):
        ...
        result = {"tag": tag, "name": clazz.name, "type": "exo", "eType": "cplx", "guid": guid, "keyMaps": keyMaps}
...
def getClassElemMap(elem, globalMap):
    ...
    if inClass.__class__.__name__ == "MetaClass":
        # proc:delay for derived classes (for backwards compatibility)
        if inClass.isDerived:
            # if name not in inClass.keyNames:
            result["proc"] = "delay"
            result["isDerived"] = True
        ...
        if (
            inClass.qualifiedName() == dataObjClassName
            or parent
            and parent.valueType.container is not inClass.container
        ):
    ...
    if elem.container is inClass:
        ...
        if result is not None:
            ...
            if hicard == 1 and not elem.isDerived:
                if isDataType:
                    if (
                        not valueType.getAllConstraints()
                        and valueType.length is None
...
def getRoleMap(elem, inClass, globalMap):
    ...
    if elem.container is inClass:
        ...
        if result is not None:
            ...
            # Correct for derived target class NBNB New Rasmus 11 Apr 2011
            if valueType.isDerived:
                result["isDerived"] = True
                result["proc"] = "delay"
            ...
            if hierarchy == ImpConstants.parent_hierarchy:
                ...
                result["content"] = globalMap[vtPrefix]["abstractTypes"]
                if (
                    not inClass.isAbstract
                    and inClass.parentRole is None
                ...
                if valueType.container is package:
                    ...
                    valueType.container in package.accessedPackages
                    and elem.container.qualifiedName() != dataObjClassName
                    and elem.container.container.qualifiedName() != implPackageName
                ):
                    # wrong way exolink
