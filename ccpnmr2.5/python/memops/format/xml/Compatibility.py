...
def getMapping(packageName, fromVersion, currentMapping):
    ...
    return process(packageName, fromVersion, currentMapping, mappingMakers, reverse=True)
...
def process(packageName, fromVersion, target, infoDict, reverse=False):
    ...
    if fromVersion > currentVersion:
        ...
        raise ApiError(" Attempt to process %s file with %s  code" % (fromVersion, currentVersion))
    ...
    if funcList is None:
        ...
        try:
            ...
            raise ApiError(" No XML conversion or I/O found for package %s" % packageName)
        ...
        for vv, func in funcList:
            if vv > fromVersion or (vv == fromVersion and vv < cutoffVersion):
                if func == nonConvertible:
                    return ApiError(" %s version %s file not compatible with current XML I/O" % (packageName, target))
...
def map_ccp_BmrbEntry_1_0_210(mapping):
    ...
    dd = semideepcopy(mapping)
    dd["Entry"]["laboratories"] = {
        "type": "exo",
        "end": "/laboratories>",
        "tag": "laboratories",
        "class": ccp.api.People.Organisation,
        "sub": "exoN",
        "key1": {
            "type": "attrkey",
            "end": "/key1",
...
def map_ccp_ChemComp_1_0_113(mapping):
    """Minor changes"""
    dd = semideepcopy(mapping)
    dd["StdChemComp"]["extendedCode"] = dd["NonStdChemComp"]["extendedCode"] = {"type": "skip", "end": "/extendedCode>"}
...
def map_ccp_ChemComp_1_0_211(mapping):
    ...
    dd["ChemAtom"]["waterNmrObs"] = {"type": "attr1", "toStr": bool2str, "fromStr": str2bool, "default": True}
...
def conv_ccp_Molecule_1_0_113(headObject):
    ...
    for xx in headObject.molecules:
        for yy in xx.molResidues:
            ...
            yy.__dict__["descriptor"] = get_new_descriptor_113(chemComp, yy.linking, yy.descriptor)
...
def conv_ccp_Molecule_1_0_208(headObject):
  ...
  for molecule in headObject.molecules:
    ...
    for molecule in headObject.molecules:
        if molecule.chains or molecule.nucTargets or molecule.protTargets or molecule.molComponents:
            molecule.__dict__["isFinalised"] = True
    ...
    for molSystem in headObject.molSystems:
        ...
        for chain in molSystem.chains:
            for residue in chain.residues:
                ...
                for atom in residue.atoms:
                    ...
                    if chemAtom is None:
                        raise ApiError("residue %s atom %s not found in chemComp %s" % (residue, atom.name, chemComp))
                    else:
                            ...
                            for ccv in chemCompVars.keys():
                                if ccv not in le.chemCompVars:
                                    if len(chemCompVars) < 2:
                                        deletedMls[ml] = None
                                        print(("WARNING, MolSystemLink to %s removed: No ChemCompVar found") % residue)
                                        ml.delete()
                                    else:
                                        del chemCompVars[ccv]
        ...
        for ml in molSystem.molSystemLinks:
            ...
            for atom in ml.atoms:
                ...
                for ii in (1, 2):
                    MolSystem.MolSystemLinkEnd(residues[ii], linkCode=linkCodes[ii], molSystemLink=ml)
                continue

            # break in last loop - no link can be made, delete MolSystemLink
            print(("WARNING, MolsystemLink could not be made between atoms \n%s and %s") % ml.atoms)
            ml.delete()
...
def map_ccp_Nmr_1_0_104(mapping):
    ...
    dd = semideepcopy(mapping)
    dd2 = semideepcopy(__import__("ccp.xml.NmrConstraints", {}, {}, ["mapping"]).mapping)
    ...
    dd["ChemShiftConstraint"]["resonance"] = {
        "type": "link1",
    }
...
def conv_ccp_Nmr_1_0_104(headObject):
    ...
    from ccp.api.NmrConstraints import ChemShiftConstraintList, DihedralConstraintList, FixedAtomSet, FixedResonanceSet

    for conhead in headObject.nmrConstraintHeads:
        project = conhead.project

        # resolve link to Structures
        strucgen = StructureGeneration(headObject, nmrConstraintHead=conhead, serial=conhead.serial)
        ...
        for conlist in conhead.constraintLists:
            if isinstance(conlist, ChemShiftConstraintList):
                for con in conlist.constraints:
                    con.resonance = conv_ccp_Nmr_1_0_104_FixedResonance(conhead, con._resonance)
                    del con._resonance

            elif isinstance(conlist, DihedralConstraintList):
                for con in conlist.constraints:
                    con.resonances = [conv_ccp_Nmr_1_0_104_FixedResonance(conhead, x) for x in con._resonances]
                    del con._resonances

            else:
                for con in conlist.constraints:
                    for item in con.items:
                        item.resonances = [conv_ccp_Nmr_1_0_104_FixedResonance(conhead, x) for x in item._resonances]
        ...
        if ll:
            ...
            for frset in ll:
                frset.resonances = [conv_ccp_Nmr_1_0_104_FixedResonance(conhead, x) for x in frset._resonances]
                ...
                if res is not None:
                    ...
                    if resset is not None:
                        ...
                        for atset in resset.atomSets:
                            ...
                            if fatset is None:
                                fatset = FixedAtomSet(conhead, serial=atserial, name=atset.name, atoms=atset.atoms)
                            ll.append(fatset)
...
def conv_ccp_Nmr_1_0_113(headObject):
    ...
    for xx in headObject.resonanceGroups:
        xx.__dict__["descriptor"] = get_new_descriptor_113(xx.chemComp, xx.linking, xx.descriptor)
...
def map_ccp_Nmr_1_0_212(mapping):
    ...
    dd["SampledDataDim"]["parameterVaried"] = {"type": "text1", "end": "/parameterVaried>"}
    ...
    dd["ExpSeriesCondition"] = {
        "type": "class",
        "end": "/ExpSeriesCondition>",
        "class": MyDummyClass,
        "parent": "nmrExpSeries",
        "types": {
            "hasElements": True,
            "renames": {},
            "singleAttrs": ["serial", "value", "error"],
            "multiAttrs": [],
            "singleTextAttrs": [],
            "headerTextAttrs": [],
            "multiTextAttrs": [],
    ...
    dd["ExpSeriesCondition"]["serial"] = {
        "type": "attr1",
        "toStr": basicDataTypes.Int.toString,
        "fromStr": basicDataTypes.Int.fromString,
    }
    dd["ExpSeriesCondition"]["value"] = {
        "type": "attr1",
        "toStr": basicDataTypes.Float.toString,
        "fromStr": basicDataTypes.Float.fromString,
    }
    dd["ExpSeriesCondition"]["error"] = {
        "type": "attr1",
        "toStr": basicDataTypes.Float.toString,
        "fromStr": basicDataTypes.Float.fromString,
    }
    dd["ExpSeriesCondition"]["experiment"] = {"type": "skip"}
    dd["ExpSeriesCondition"]["access"] = {
        "type": "exo",
        "end": "/access>",
        "tag": "access",
        "class": memops.api.Implementation.AccessObject,
        "sub": "exo1",
        "prior": True,
        "key1": {"type": "textkey", "end": "/key1>", "tag": "key1", "xtag": "name", "hicard": 1},
        "keyDict": {0: "key1"},
    }
        ...
        if hasattr(nxs, "expSeriesConditions"):
            for xsc in nxs.expSeriesConditions:
                for exp in headObject.nmrExperiments:
                    ...
                    raise ("No experiment found for ExpSeriesCondition %s in %s" % (xsc.serial, nxs))
                ...
                if condset is None:
                    ...
                    condset = headObject.newSampleConditionSet(
                        experiments=(exp,),
                    ...
                    oldCond = condset.findFirstSampleCondition(condition=nxs.variableName)
                    if oldCond is None:
                        condset.newSampleCondition(
                            condition=nxs.variableName, unit=nxs.variableUnit, value=xsc.value, error=xsc.error
                        )
                    else:
                        # we already have an ExpCondition with this condition
                        if (
                            oldCond.unit == nxs.variableUnit
                            and oldCond.value == xsc.value
                            and oldCond.error == xsc.error
                        ):
...
def get_new_descriptor_113(chemComp, linking, descriptor):
    ...
    if descriptor == "neutral":
        ...
        if not (chemComp.molType == "protein" and chemComp.ccpCode == "CYS"):
            raise ApiError(
                "(1) compatibility code cannot handle %s/%s/%s/%s"
        ...
        if not (chemComp.molType == "protein" and chemComp.ccpCode == "HIS"):
            raise ApiError(
                "(2) compatibility code cannot handle %s/%s/%s/%s"
                % (chemComp.molType, chemComp.ccpCode, linking, descriptor)
            )
        ...
        if not descrs:
            if hasNeutral:
                ...
                print(
                    "WARNING, %s.%s.%s descriptor %s forced to 'neutral'"
                    % (chemComp.molType, chemComp.ccpCode, linking, descriptor)
                )
                return "neutral"
...
def parse_descriptor_113(descriptor):
    ...
    if descriptor == "neutral":
        raise ApiError("parse_descriptor_113 called with improper descriptor 'neutral'")
...
def getVersion(stream):
    ...
    for ee in elementStrings:
        ...
        raise ApiError(loadError + "not (correct) XML. Does not contain <_StorageUnit element")
    ...
    try:
        ...
        raise ApiError(loadError + "not (correct) XML. <_StorageUnit element lacks 'release' ")
...
def special_conv_1_0_204(project):
    ...
    if molDict:
        ...
        loadPackages = ("ccp.RefSampleComponent", "ccp.Annotation", "ccp.MolSystem", "ccp.Nmr", "ccpnmr.Analysis")
        for storage in project.storages:
            if storage.package in loadPackages and storage.isStored and not storage.isLoaded:
                storage.load()
                ...
                if corr:
                    ...
                    for nameMapping in nameMappings:
                        ...
                        for atomMapping in nameMapping.atomMapping:
                            if atomMapping.molSystemCode == molSystemCode and atomMapping.chainCode == chainCode:
