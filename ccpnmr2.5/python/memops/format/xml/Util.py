...
def findTopObjectPath(repositoryPath, topObject):
    ...
    topObjectDir = uniIo.joinPath(repositoryPath, *topObject.packageName.split("."))
...
def getTopObjIdFromFileName(fileName, mustBeMultipart=None):
    ...
    if mustBeMultipart is None:
        ...
        if len(ll) == 1:
            raise ApiError("TopObject fileName %s lacks field separators %s" % (fileName, keySep))

    elif len(ll) != 1:
        # must be single field (Implementation)
        raise ApiError("TopObject fileName %s has field separators %s" % (fileName, keySep))
