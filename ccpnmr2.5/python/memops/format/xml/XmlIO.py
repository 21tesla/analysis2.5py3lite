...
def save(repositoryPath, topObject, mapping=None, comment=None, simplified=True, compact=True, expanded=False):
...
def saveToStream(stream, topObject, mapping=None, comment=None, simplified=True, compact=True, expanded=False):
    ...
    if gc.isenabled():
        gc.disable()
        try:
            Implementation.saveToStream(stream, topObject, mapping, comment, simplified, compact, expanded)
        finally:
            gc.enable()
    else:
        Implementation.saveToStream(stream, topObject, mapping, comment, simplified, compact, expanded)
...
def loadFromFile(memopsRoot, filePath, partialLoad=False):
    ...
    try:
        topObjId = xmlUtil.getTopObjIdFromFileName(filePath, mustBeMultipart=True)
        topObject = loadFromStream(stream, topObject=memopsRoot, topObjId=topObjId, partialLoad=partialLoad)
    finally:
        stream.close()
...
def loadFromStream(stream, topObjId=None, topObject=None, partialLoad=False):
    ...
    if gc.isenabled():
        gc.disable()
        try:
            result = Implementation.loadFromStream(
                stream, topObjId=topObjId, topObject=topObject, partialLoad=partialLoad
            )
        finally:
            gc.enable()
    else:
        result = Implementation.loadFromStream(stream, topObjId=topObjId, topObject=topObject, partialLoad=partialLoad)
    #
    return result
