...
def correctData(topObj, delayDataDict, toNewObjDict, mapping=None):
    ...
    if pName == "ccp.molecule.Validation":
        ...
        validationStores = [x for x in topObjByGuid.values() if x.packageShortName == "VALD"]
...
def fullLoadValidationStore(topObj):
    ...
    locator = root.findFirstPackageLocator(targetName="ccp.molecule.Validation") or root.findFirstPackageLocator(
        targetName="any"
    )
    ...
    XmlIO.loadFromStream(open(filePath), topObject=topObj, topObjId=topObj.guid, partialLoad=False)
