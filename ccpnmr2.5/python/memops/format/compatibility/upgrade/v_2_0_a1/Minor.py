...
def fixImplementation(topObj, delayDataDict):
    ...
    profileLocator = topObj.newPackageLocator(targetName="ccpnmr.AnalysisProfile", repositories=(genData, refData))
...
def fixNmr(topObj, delayDataDict):
    ...
    for xpr in doGet(topObj, emptyDict).get("experiments", emptyList):
        ...
        setNmrExpPrototypeLink(xpr, "refExperiment", topObjByGuid, delayDataDict, remapPrototypeLink)
        for xd in expDict.get("expDims", emptyList):
            setNmrExpPrototypeLink(xd, "refExpDim", topObjByGuid, delayDataDict, remapPrototypeLink)
            for xdr in doGet(xd, emptyDict).get("expDimRefs", emptyList):
                setNmrExpPrototypeLink(xdr, "refExpDimRef", topObjByGuid, delayDataDict, remapPrototypeLink)
        ...
        for ds in expDict.get("dataSources", emptyList):
            for dd in doGet(ds, emptyDict).get("dataDims", emptyList):
                if dd.__class__.__name__ != "SampledDataDim":
                    allExpDimRefs = set(doGet(dd.expDim, emptyDict).get("expDimRefs", emptyList))
                    ...
                    for xdr in allExpDimRefs.difference(usedExpDimRefs):
                        ...
                        scaling = dd.newDimensionScaling(expDimRef=xdr, scalingFactors=(1.0,))
                        dimensionScalings.append(scaling)


def remapPrototypeLink(keyList):

    from memops.format.compatibility.upgrade.v_2_0_b2 import Minor as Minor20b2
    ...
    Minor20b2.remapPrototypeLink(keyList)


def setNmrExpPrototypeLink(obj, tag, topObjByGuid, delayDataDict, linkmapper):
    ...
    if inDataList:
        ...
        if oo is None:
            ...
            try:
                ...
                raise ApiError(
                    """%s.%s: NmrExpPrototype with guid %s not found or loaded""" % (clazz.__name__, tag, guid)
                )
...
def fixAnalysis(topObj, delayDataDict, toNewObjDict, mapping):
    ...
    for swin in topObjDict.get("spectrumWindows", emptyList):
        for winv in doGet(swin, emptyDict).get("spectrumWindowViews", emptyList):
            dd = doGet(winv)
            dataSource = topObj.nmrProject.findFirstExperiment(serial=dd["experimentSerial"][0]).findFirstDataSource(
            ...
            if analysisSpectrum is None:
                ...
                delayDataDict[analysisSpectrum] = {"analysisPeakLists": [], "analysisDataDims": []}
