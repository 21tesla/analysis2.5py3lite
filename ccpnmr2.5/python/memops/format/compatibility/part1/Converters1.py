...
def majorUpgradeToCurrent(oldRoot, oldVersionStr, newName=None, newDir=None, oldTags=None, doSave=True):
    """
    ...
    if newDir is None:
        ...
        newPath = uniIo.normalisePath((os.path.join(newDir, newName)), makeAbsolute=True)
    newRoot.packageLocator.findFirstRepository().url = Implementation.Url(path=newPath)
    newRoot.findFirstRepository(name="backup").url = Implementation.Url(path=newPath + "_backup")
...
class DataUpgrader:
    ...
    def __init__(self, oldVersionStr, newRoot=None, oldRoot=None, newVersionStr=None, oldTags=None):
        ...
        if newVersionStr is None:
            ...
            raise ApiError(" DataUpgrader to non-current version %s not implemented" % newVersionStr)
    ...
    def mapReferenceData(self, oldTopObj, newTopObj, fullKeyConverters, globalMapping):
        ...
        self.connectReferenceData(
            delayObjects, fullKeys, packageShortName, fullKeyConverters, str(oldTopObj), globalMapping
        )

    def connectReferenceData(
        self, delayObjects, fullKeys, packageShortName, fullKeyConverters, topObjString, globalMapping
    ):
        """Map reference data for a single TopObject.
        ...
        if delayObjects:
            ...
            ll = [x for x in oldObj.__class__.__mro__ if x.__name__ in ("MemopsObject", "MemopsBaseClass")]
            if len(ll) == 1:
                # MemopsObject for old object tree
                # - done this way to avoid import trouble
                OldMemopsObject = ll[0]
            else:
                raise ApiError("superType 'MemopsObject' not found from object %s" % oldObj)
        else:
            return
        ...
        while found:
            ...
            for ii in range(len(delayObjects) - 1, -1, -1):
                ...
                for jj, xx in enumerate(fullKey):
                    ...
                    newObj = self.newObjFromOldFullKey(
                        oldObj, fullKey, packageShortName, fullKeyConverters, globalMapping
                    )
  ...
  %s:)
  %s had unmappable keys"""
                % (topObjString, delayObjects[0])
            )
        elif nObjs:
            ...
            print(
                """\nFinished mapping reference objects
  %s:)
  %s objects of classes %s had unmappable keys"""
                % (topObjString, nObjs, ll)
            )
    ...
    def transferData(self):
        ...
        for tag in self.oldTags:
            ...
            if val:
                if isinstance(val, (set, frozenset, list, tuple)):
                    transferTopObjects.extend(x for x in val if x.packageName != "memops.Implementation")
        ...
        print("+++ Done full garbage collection - removed %s objects" % gc.collect())

        # create new data
        from memops.general.Util import transferData

        for topObj in transferTopObjects:
            transferData(newRoot, topObj, oldToNew=oldToNew, oldVersionStr=self.oldVersionStr)
            # NBNB TBD check this OK.
            topObj.__dict__["isModified"] = True

    def newObjFromOldFullKey(self, oldObj, fullKey, packageShortName, fullKeyConverters, globalMapping):
            ...
            for clazz in pp.classes:
                ...
                for role in clazz.roles:
                    if not role.isDerived:
                        ...
                        vt = role.valueType
                        pp2 = vt.container
                        if pp2 in importedPackages and pp2 is not implPackage or role.name in keyNames:
...
def modifyIoMap(oldVersionStr, globalMapping):
    ...
    for prefix, typeName, elemName, newGuid, elemType in diffMap.skipElements:
        if prefix in globalMapping and newGuid not in elemsTreatedAsOld:
            fixElementMap(newGuid, "skip", prefix, globalMapping, newElementsByGuid, typeName, elemName)
    ...
    for prefix, typeName, elemName, newName, newGuid in diffMap.renames:
        if prefix in globalMapping and newGuid not in elemsTreatedAsOld:
            fixElementMap(newGuid, "rename", prefix, globalMapping, newElementsByGuid, typeName, elemName)
            if elemName is None:
...
def removeElementName(guid, globalMapping, newElementsByGuid):
    ...
    if useObj.__class__.__name__ in ("MetaAttribute", "MetaRole"):
        if useObj.isAbstract or (useObj.isDerived and useObj.changeability == ImpConstants.frozen):
        ...
        for metaclass in subtypes:
            ...
            if containerMap is not None:
                ...
                for tag in ("simpleAttrs", "headerAttrs", "optLinks", "cplxAttrs", "children"):
        ...
        if oldMap is None:
            ...
            if (
                useObj.__class__.__name__ == "MetaRole"
                and useObj.container.container in useObj.valueType.container.importedPackages
            ):
                # interpackage link in backwards direction
                pass
            elif useObj.__class__.__name__ == "MetaRole" and useObj.hierarchy == "parent":
                pass
...
def fixExoLinkMap(newGuid, action, prefix, globalMapping, typeName):
    ...
    if action == "skip":
        ...
        newMap = {"type": "exo", "guid": newGuid, "eType": "cplx", "proc": "skip"}
...
"""
    ...
    for ss in compModules:
        if not hasattr(result, ss):
            raise ApiError("Compatibility error: No module %s found in %s" % (ss, moduleDir))
    #
    return result


def minorPostProcess(fromVersionStr, topObj, delayDataDict, toNewObjDict, mapping=None, topObjElem=None):
    ...
    if mod is not None:
        if topObjElem:
            ...
            # not need topObjElem
            mod.Minor.correctData(topObj, delayDataDict, toNewObjDict, mapping, topObjElem)
        else:
