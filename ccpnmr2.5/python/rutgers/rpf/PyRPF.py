...
class PyRpfPopup(BasePopup):
  ...
  *Huang, Y. J.; Powers, R. & Montelione, G. T. Protein NMR recall, precision,
  ...
  def body(self, guiFrame):
    ...
    tipTexts = ['A table to select which through-space peak lists are compared with structures.',
    ...
    tipText = 'How far from the homonuclear diagonal to exclude peaks and positions in the analysis'
    self.diagonalEntry = FloatEntry(frame, text=0.3, width=8, tipText=tipText)
    self.diagonalEntry.grid(row=0,column=1, sticky='w')

    label = Label(frame, text='Consider Aliased Positions?')
    label.grid(row=0,column=2, sticky='w')
    tipText = 'Whether the peak locations could be aliased, when considering which resonances they might be assigned to'
    self.aliasedSelect = CheckButton(frame, selected=True, tipText=tipText)
    self.aliasedSelect.grid(row=0,column=3, sticky='w')

    label = Label(frame, text='Prochiral Exclusion (PPM):')
    label.grid(row=0,column=4, sticky='w')
    tipText = 'The threshold within which prochiral atom pairs are considered too close to give separate peaks'
    self.prochiralEntry = FloatEntry(frame, text=0.04, width=8, tipText=tipText)
    ...
    tipTexts = ['The experiment:spectrum record that the peak list corresponds to',
                'The number of the through-space peak list for its spectrum',
                'Sets whether to use the peak list in the RPF calculation',
                'The number of peaks the peak list contains']
    ...
    tipTexts = ['The molecular system that the structure describes',
                'The id number of the structure ensemble, for its molecular system',
                'Sets whether to consider the sructure in the RPF calculation',
    ...
    tipTexts = ['Acticate the structures selected in the table, so that they will be considered.',
                'Inactivate the structures selected in the table, so that they will not be considered.']
    ...
    headingList = ['Ensemble', 'PeakList(s)',
                   ...
                   'Missing\nPeaks']
    tipTexts = ['The identity of the structure ensemble analysed',
                'The identity of the peak list analysed',
                'The fraction of peaks that are consistent with the structure; measure of unexplained peaks',
                'The fraction of short distances that are represented; measure of missing peaks',
                'F-measure score: a combination of recall and precision',
    ...
    tipTexts = ['Open the table listing all the unexplained peaks, for the selected result row.',
                'Open the table listing all the missing peak locations, for the selected result row.',
                'Open the graph of per-residue RPF quality scores',
                'Delete the selected RPF results (does not affect the original peaks or structure)']
    self.resultsButtons = ButtonList(frameC, texts=texts, commands=commands, tipTexts=tipTexts)
    ...
    label.grid(row=0,column=0, sticky='w')
    tipText = "Sets which high-level grouping of stored calulation results to show peaks for."
    self.resultPulldownA = PulldownList(frameD, self.changeResultSet, tipText=tipText)
    ...
    tipTexts = ['Select the next unexplained peak in the table',
                'Select the previous unexplained peak in the table']
    self.unexplainedPeakButtons = ButtonList(frameD, texts=texts, commands=commands, tipTexts=tipTexts)
    ...
    label.grid(row=0,column=0, sticky='w')
    tipText = "Sets which high-level grouping of stored results to show peak locations for."
    self.resultPulldownB = PulldownList(frameE, self.changeResultSet, tipText=tipText)
    self.resultPulldownB.grid(row=0,column=1, sticky='w')

    texts = ['Next Location','Prev Location']
    commands = [self.nextMissingPeak, self.prevMissingPeak]
    tipTexts = ['Select the next synthetic, missing peak in the table',
                'Select the previous synthetic, missing peak in the table']
    self.missingPeakButtons = ButtonList(frameE, texts=texts, commands=commands, tipTexts=tipTexts)
    ...
    label.grid(row=0,column=0, sticky='w')
    tipText = "Sets which high-level grouping of stored results to show scores for."
    self.resultPulldownC = PulldownList(frameF, self.changeResultSet, tipText=tipText)
    ...
    tipTexts = ['Actually run the RPF calculation using the selected peak lists and structure ensembles']
  ...
  def deleteResults(self):
    ...
    validStores = set()
    for validStore, peakList in self.resultsTable.currentObjects:
...
def calcRPF(ensembles, peakLists, tolerances, distThreshold=5.0, prochiralTolerance=0.04,
            diagonalTolerance=0.3, aliasing=True, progressBar=None,):
          ...
          if close and peak:
            ...
            if not explainedPeaks.has_key(peak):
              ...
              for resonance3, resonance4, atom3, atom4 in peakPoss[peak]:
                ...
                dist2, noe2 = rDists[resonance3].get(resonance4, (badDist, badNoe))
