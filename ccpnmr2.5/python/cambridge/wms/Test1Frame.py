...
class Test1Popup(BasePopup):
  ...
  def body(self, guiFrame):
    ...
    frame = Test1Frame(guiFrame, basePopup=self)
...
class Test1Frame(Frame):
  ...
  def __init__(self, guiParent, basePopup):
    ...
    self.button_file_in_browse = Button(self, text='Browse', command=self.tmpCall)
    self.data_widgets.append(self.button_file_in_browse)
    self.button_file_out_browse = Button(self, text='Browse', command=self.tmpCall)
  ...
  def drawFrame(self):
    ...
    if self.task_id == None:
      ...
      self.task_status_label.grid(row=3, column=1, columnspan=2, sticky='w')
      self.task_status_value.set(hm_tv['status'])
      #self.task_status_value.set('PENDING')
      self.task_status_value.grid(row=3, column=2, columnspan=2, sticky='w')

      par1 = 'N/A'
      if hm_tv['par1'] != 'valIsNullPointer':
        par1 = hm_tv['par1']

      self.task_par1_label.grid(row=4, column=1, columnspan=2, sticky='w')
      self.task_par1_value.set(par1)
      self.task_par1_value.grid(row=4, column=2, columnspan=2, sticky='w')

      par2 = 'N/A'
      if hm_tv['par1'] != 'valIsNullPointer':
        par2 = hm_tv['par2']

      self.task_par2_label.grid(row=5, column=1, columnspan=2, sticky='w')
      self.task_par2_value.set(par2)
      self.task_par2_value.grid(row=5, column=2, columnspan=2, sticky='w')
