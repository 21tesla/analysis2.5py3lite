...
from pdbe.adatah.Constants import archivesDataDir, chemCompArchiveDataDir, ebiPdbFtp
...
class PdbHandler:
  ...
  def readPdbCoordinates(self,pdbCode,chemCompArchiveDataDir = chemCompArchiveDataDir, mergeNmrStarFile = None, maxNum = 999, readPeopleCitations=True):
    ...
    if 'forceChainMappings' not in keywds:
...
def getPdbCode(pdbCode, forceGet = False, pdbDir = None, source=''):
  ...
  if source == 'EBI':
    ...
    pdbInsertDir = ""
    from pdbe.adatah.Constants import pdbFtp, pdbFtpDir
...
def getPdbChainInfo():
  ...
  for dataLine in dataLines:
    ...
    if cols and cols[0][0] == '>':
      ...
      if pdbCode not in pdbChainInfoDict:
...
def getRelatedEntriesRcsb(pdbCode):
  ...
  clusterId = None
  fullMatchPatt = re.compile(r"\>100\%\<\/a\>")
  clusterPatt = re.compile(r"cluster\=(\d+)\&amp")
