...
def filterPeakListWithCloud(argServer, peakList=None, pattern=r"t_intra_0\d+.pdb"):
...
def writeTypedPdbCloud(atomCoordList, pdbFileName, resonances):
    ...
    if hasattr(atomCoordList[0], "x"):
        for n in range(N):
            ...
            fp.write(
                "ATOM  %5d  %-3.3s HYD %5d    %8.3f%8.3f%8.3f%6.2f%6.2f\n"
...
def connectResonances(project, serials, shifts, atomTypes, isotopeCodes=("1H",), shiftList=None):
    ...
    for i in range(len(serials)):
        ...
        if resonance is None:
            ...
            resonance = project.newResonance(
                isotopeCode=isotope,
                assignNames=[
                    atomTypes[i],
                ],
...
def makeConstraintsFromStructure(
    argServer,
    structure=None,
    atomRoots=None,
    threshold=5.5,
    ...
    if resonanceDists:
        ...
        for (
            ...
            constraint = constraintList.newDistanceConstraint(
                weight=1.0, origData=dist, targetValue=dist, upperLimit=maxDist, lowerLimit=minDist, error=delta
            )
