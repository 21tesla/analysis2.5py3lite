...
def searchPosterior(
    project, residues, spinSystems, clouds, nSteps=3000, graph=None, verbose=0, preserveAssign=0, progressBar=None
):
...
def swapSpinSystem(typeScores, assignment, clouds, interDistrib, n, iteration):
    ...
    for i in range(n):
        ...
        while (
            h < 25
            and getSpinSystemScore(typeScores, residues[j], spinSystem1) > threshold
            and getResidueLikelihood(residues[j], spinSystem1, newAssignment, clouds, interDistrib) > threshold2
        ):
            j = randint(0, N - 1)
            spinSystem1 = newAssignment[residues[j]]
            h += 1
...
def getInitialAssignment(residues, spinSystems, preserveAssign, graph=None, progressBar=None):
    ...
    typeScores, bestGuess = getSpinSystemTypes(
        residues, spinSystems, preserveAssign, ("1H", "13C"), shiftList=shiftList, graph=graph, progressBar=progressBar
    )
                ...
                if atomNames4 and (atomName not in atomNames4):
                    ...
                    for atom4 in residue.atoms:
                        if not atom4.chemAtom.waterExchangeable:
                            print(
                                "     %3.3f %5.5s %.3f"
                                % (shift.value, atom4.atomSet.name, atomScores[shift][atom4.atomSet.name] or -1000.0)
                            )
...
def makeStructureDictFromXml(path, fileName):
    ...
    for atom in structure.orderedAtoms:
        ...
        dict[residue.chain.code][residue.seqId]["atoms"][atom.name] = list(coordinates[offset:next])
        offset = next
