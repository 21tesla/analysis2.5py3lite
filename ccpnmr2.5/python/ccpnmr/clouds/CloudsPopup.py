...
from ccpnmr.clouds.NoeRelaxation import disambiguateNoesyPeaks, optimiseRelaxation
from ccpnmr.clouds.ResonanceIdentification import constrainSpinSystems, getCloudsResonanceList, makeNoeAdcs
...
class CloudsPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        self.noesyPulldown = PulldownMenu(
            frame0, entries=self.getNoesys(), callback=self.setNoesy, selected_index=0, do_initial_callback=0
        )
        ...
        self.hsqcPulldown = PulldownMenu(
            frame0, entries=self.getHsqcs(), callback=self.setHsqc, selected_index=0, do_initial_callback=0
        )
        ...
        self.tocsyPulldown = PulldownMenu(
            frame0, entries=self.getTocsys(), callback=self.setTocsy, selected_index=0, do_initial_callback=0
        )
        ...
        label02.grid(row=f0row, column=0, sticky=Tkinter.NW)
        self.noesy3dPulldown = PulldownMenu(
            frame0, entries=self.getNoesy3ds(), callback=self.setNoesy3d, selected_index=0, do_initial_callback=0
        )
        ...
        self.structurePulldown = PulldownMenu(
            frame1, entries=self.getStructures(), callback=self.setStructure, selected_index=0, do_initial_callback=0
        )
        ...
        self.adcAtomsPulldown = PulldownMenu(
            frame1,
        ...
        editWidgets = [
            None,
            self.mdInitTempEntry,
            self.mdFinTempEntry,
            self.mdCoolStepsEntry,
            self.mdSimStepsEntry,
            self.mdTauEntry,
    ...
    def startMd(self):
        ...
        self.setNumClouds()
        self.setFilePrefix()
        if self.distanceConstraintList and self.antiDistConstraintList and (self.numClouds > 0) and self.filePrefix:
            resDict = {}
            ...
            startMdProcess(self.numClouds, self.distanceConstraintList, resonances, self.coolingScheme, self.filePrefix)
            ...
            self.guiParent.application.setValues(
                self.distanceConstraintList.nmrConstraintStore, "clouds", values=clouds
            )
            self.guiParent.application.setValues(
                self.distanceConstraintList.nmrConstraintStore, "cloudsResonances", values=serials
            )
            ...
            for j in range(i + 1, N):
                ...
                if dist < 5.5:
                    ...
                    item = NmrConstraint.DistanceConstraintItem(
                        constraint, resonances=[fixedResonance0, fixedResonance1]
                    )

                elif (
                    (atomSets1[0].findFirstAtom().name == "H")
                    and (atomSets0[0].findFirstAtom().name == "H")
                    and (dist > 7)
                ):
                    # else:
                    fixedResonance0 = getFixedResonance(constraintStore, resonances[i])
                    ...
                    constrDict[j] = 1
                    constraint = NmrConstraint.DistanceConstraint(
                        distConstraintList, weight=adcWeight, targetValue=75, upperLimit=175, lowerLimit=5.0, error=94.5
                    )
                    item = NmrConstraint.DistanceConstraintItem(
                        constraint, resonances=[fixedResonance0, fixedResonance1]
                    )
    ...
    def setupResonances(self):
        ...
        if self.noesyPeakList and self.noesy3dPeakList and self.tocsyPeakList and self.hsqcPeakList:
            ...
            (self.origResonances, self.noesyPeaks, null) = getCloudsResonanceList(
                self.guiParent.argumentServer,
