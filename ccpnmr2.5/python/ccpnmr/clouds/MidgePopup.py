...
from ccpnmr.clouds.NoeRelaxation import optimiseRelaxation
...
class MidgePopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        self.noesyPulldown = PulldownMenu(
            labelFrame0, entries=self.getNoesys(), callback=self.setNoesy, selected_index=0, do_initial_callback=0
        )
        ...
        self.tmix1Pulldown = PulldownMenu(
            labelFrame0, entries=self.getNoesys(), callback=self.setNoesy1, selected_index=-0, do_initial_callback=0
        )
        ...
        self.tmix2Pulldown = PulldownMenu(
            labelFrame0, entries=self.getNoesys(), callback=self.setNoesy2, selected_index=0, do_initial_callback=0
        )
        ...
        self.tmix3Pulldown = PulldownMenu(
            labelFrame0, entries=self.getNoesys(), callback=self.setNoesy3, selected_index=0, do_initial_callback=0
        )
        ...
        self.noesy3dPulldown = PulldownMenu(
            labelFrame0, entries=self.getNoesys3d(), callback=self.setNoesy3d, selected_index=0, do_initial_callback=0
        )
    ...
    def update(self):
        ...
        if self.resonances and (
            (self.noesyPeaks and self.noesyPeakList1 and self.noesyPeakList2 and self.noesyPeakList3)
            or self.noesyPeakList3d
        ):
    ...
    def calculateDistances(self):
        ...
        noeMatrix = getNoeMatrixFromPeaks(
            self.noesyPeaks, resonances, peakLists, mixingTimes, ratioHD=ratioHD, analysis=self.guiParent
        )
        ...
        if self.distanceConstraintList and self.noesyPeakList:
            ...
            if self.adcAtomTypes:
                ...
                self.antiDistConstraintList = makeNoeAdcs(
                    resonances[:], self.noesyPeakList.dataSource, constraintHead, allowedAtomTypes=allowedAtomTypes
                )
                print("Done ADCs")
