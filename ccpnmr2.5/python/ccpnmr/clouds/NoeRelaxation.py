        ...
        if matched:
            ...
            for p3 in peaks3d:
                if not matched.get(p3):
                    ...
                    region = [
                        [ppm1 - tolerances[0], ppm1 + tolerances[0]],
                        [ppm2 - tolerances[1], ppm2 + tolerances[1]],
                    ]
                    peaks = searchPeaks(
                        [
                            noesy2dPeakList,
                    ...
                    region = [
                        [ppm2 - tolerances[0], ppm2 + tolerances[0]],
                        [ppm1 - tolerances[1], ppm1 + tolerances[1]],
                    ]
                    peaks = searchPeaks(
                        [
                            noesy2dPeakList,
        ...
        for j in range(i + 1, n):
            if (amatNew[i][j] != 0) and (rmat[i][j] != 0.0):
                ...
                constraint = NmrConstraint.DistanceConstraint(
                    ...
                    weight=1,
                    targetValue=dist,
                    upperLimit=dist + (dist / 2.0),
                    lowerLimit=dist - (dist / 2.0),
                    error=dist / 5,
                )
