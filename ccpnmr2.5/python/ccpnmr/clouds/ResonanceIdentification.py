...
from ccpnmr.analysis.core.ExperimentBasic import findSpectrumDimsByIsotope, getNdSpectra, getSpectraByType
from ccpnmr.analysis.core.PeakBasic import getPeakDimPpm, getPeakHeight, searchPeaks
    ...
    if peakList:
        ...
        if spectrum.numDim > 2:
            ...
            h1Dim = (
                int(
                    askInteger(
                        "Dimension mapping", "Which dimension for peak list %s is heavy atom bound?" % text, h1Dim + 1
                    )
                    or 1
                )
                - 1
...
def assignSpinSystems(pseudoMolSystem):
    ...
    for pseudoSpinSyst in pseudoMolSystem.pseudoSpinSysts:
        ...
        spinSystem = project.newResonanceGroup(
            resonances=resonances, molType="protein", name=pseudoSpinSyst.name, details="Created for CLOUDS"
        )
...
def identify3dSideChains(pseudoMolSystem, tocsy3dPeaks, hnhaPeaks, pseudoSpinSysts):
    ...
    for peak in hnhaPeaks:
        ...
        hnhaPseudoSS.append(
            PseudoSpinSyst(pseudoMolSystem, name="3dSide", pseudoResons=[pseudoResHn, pseudoResHa, pseudoResN])
        )
    ...
    for peak in tocsy3dPeaks:
        ...
        for haPseudoSS in hnhaPseudoSS:
            (n, score, prob) = matchPairPseudoResons(
                tPseudoSS.pseudoResons, haPseudoSS.pseudoResons, [2 * stdH1err, 2 * stdH2err, 2 * stdNerr]
            )
            if n == 3:
    ...
    for tPseudoSS in tocsyPseudoSS:
        ...
        for rootPseudoSS in rootPseudoSS:
            ...
            if rootPseudoSS.name == "AsnGlnSide":
                ...
                (n, score, prob) = matchPairrootPseudoRes(
                    rootPseudoRes, [pseudoResHn, pseudoResN], [3 * stdH1err, 3 * stdNerr]
                )
                if n == 2 and prob > 0.3:
                ...
                if len(aromPseudoSS.pseudoResons) == 4:  # BS code!!!!!!!!!!!!!!!
                    ...
                    for pseudoRes2 in aromPseudoSS.pseudoResons:
                        ...
                        if n == 1:
                            ...
                            region = [
                                [pseudoResHe.ppm - e, pseudoResHe.ppm + e],
                                [pseudoRes2.ppm - e, pseudoRes2.ppm + e],
                                [pseudoResNe.ppm - e, pseudoResNe.ppm + e],
                            ]
            ...
            if len(commonCosy) == 2 and prob > 0.66:
                ...
                for tocsyPseudoSS in tocyPseudoSpinSysts:
                    ...
                    if len(common) == 2 and prob > 0.5:
                        ...
                        combinedPseudoSS = pseudoMolSystem.mergePseudoSpinSysts(
                            cosyPseudoSS1, cosyPseudoSS2, "cosyLinked"
                        )
                        cosyPseudoSpinSysts.remove(cosyPseudoSS2)
                        combinePseudoResons(commonCosy)
                        ...
                        combinedPseudoSS = pseudoMolSystem.mergePseudoSpinSysts(
                            combinedPseudoSS, tocsyPseudoSS, "cosyTocsyLinked"
                        )
                        combinePseudoResons(common)
...
def identifyAsnGlnAmides(pseudoMolSystem, hsqcPeaks, tocsy3dPeaks):
    ...
    for pair in close15NPairs:
        ...
        if returnPeak1 and returnPeak2:
            ...
            nqPseudoSS = PseudoSpinSyst(
                pseudoMolSystem, name="GlnAsnSide", pseudoResons=[pseudoResH1, pseudoResH2, pseudoResN]
            )
...
def identifyTrpD1E1(pseudoMolSystem, hsqcPeaks, cosyPeaks):
    ...
    for hsqcPeak in possibleHsqc:
        ...
        if possibleMatchesA or possibleMatchesB:
            ...
            trpPseudoSS = PseudoSpinSyst(
                pseudoMolSystem, name="TrpSide", pseudoResons=[pseudoResHe, pseudoResNe, pseudoResHd]
            )
...
def makeNoeAdcs(
    resonances, spec, constraintHead, diagExclusion=0.4, water=4.92, allowedAtomTypes=["H", "HA", "HA1", "HA2"]
):
            ...
            for j in range(i + 1, N):
                if not allowedAtomTypes or (atomTypes[j] in allowedAtomTypes):
                    ...
                    if residue1 and residue0:
                        if (residue1.chain is residue0.chain) and (abs(residue1.seqCode - residue0.seqCode) < 3):
                            continue
                    ...
                    for h in (height1, height2, height3, height4, height5):
                        ...
                        constraint = distConstraintList.newDistanceConstraint(
                            weight=1, targetValue=75, upperLimit=100, lowerLimit=5.5, error=5.0
                        )
            ...
            if resDict2.get(resonanceI):
                for j in range(i + 1, n):
                    ...
                    if resDict2.get(resonanceJ):
                        ...
                        constraint = distanceConstraintList.newDistanceConstraint(
                            weight=1, targetValue=10, upperLimit=11.5, lowerLimit=1.5, error=10
                        )
