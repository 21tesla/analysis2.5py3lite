...
from ccpnmr.clouds.CloudBasic import code1LetterDict, getCloudsFromFile, getFileNamesFromPattern
...
class CloudThreaderPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        self.filenameEntry = Entry(guiFrame, text=r"nexus15.pdb_\d+")
        ...
        self.graph = ScrolledGraph(
            guiFrame, width=300, height=200, graphType="scatter", dataColors=["#008000", "#F00000"]
        )
        self.graph.grid(row=row, column=0, columnspan=4, sticky=Tkinter.NSEW)

        row += 1
        texts = ["Run", "Assign!", "Score Current", "Fit Side Chains"]
        commands = [self.run, self.assignSpinSystems, self.scoreCurrentAssignments, self.assignSideChains]
    ...
    def scoreCurrentAssignments(self):
        ...
        if self.chain:
            ...
            files = getFileNamesFromPattern(pattern, ".")
            clouds = getCloudsFromFile(files, self.chain.root)
            score, self.scores, self.assignment = scoreAssignment(
                self.chain, clouds, spinSystems=None, graph=self.graph
            )
            self.update()
    ...
    def run(self):
        ...
        if not nSteps:
            ...
            clouds = getCloudsFromFile(files, self.chain.root)
            score, self.scores, self.assignment = fastCloudThreader(
                self.chain, clouds, nSteps=nSteps, graph=self.graph, preserveAssign=preserve, progressBar=pgb
            )
...
def cloudThreader(chain, clouds, spinSystems=None, nSteps=3000, graph=None, preserveAssign=0, progressBar=None):
    ...
    score, scores, assignment = searchPosterior(
        project,
        residues,
        spinSystems,
        clouds,
...
def swapSpinSystem(assignment, clouds, interDistrib, known, n, iteration):
    ...
    for i in range(n):
        ...
        while (
            h < 25
            and getSpinSystemScore(residues[j].ccpCode, spinSystem1) > threshold
            and getResidueLikelihood(residues[j], spinSystem1, newAssignment, clouds, interDistrib, known) > threshold2
        ):
            j = randint(0, N - 1)
            spinSystem1 = newAssignment[residues[j]]
            h += 1
...
def fitSideChain(shifts, residue, clouds, distrib, useAssignNames=False):
    ...
    from ccpnmr.analysis.core.SpinSystemTyping import getAtomShiftScore
...
def getDistDistributions(
    dataDirName="/home/tjs234/ccpn/recoord/", intraFileName="intraDistribs001.txt", interFileName="interDistribs001.txt"
):
...
def fastCloudThreader(chain, clouds, spinSystems=None, nSteps=5000, graph=None, preserveAssign=False, progressBar=None):
    ...
    score1, scores, assignment3 = searchPosterior(
        project,
        residues,
        spinSystems2,
        clouds,
    ...
    score2, scores, assignment4 = searchPosterior(
        project,
        chain.residues,
        spinSystems,
        clouds,
    ...
    if residues and spinSystems2:
        ...
        score3, scores, assignment4 = searchPosterior(
            project,
            residues,
            spinSystems2,
            clouds,
    ...
    score, scores, assignment5 = searchPosterior(
        project,
        chain.residues,
        spinSystems,
        clouds,
...
def followRoute(
    peptide, spinSystems, ensemble, inter, route=None, typeThreshold=-5.8, scoreThreshold=-5.0, totalScore=0.0
):
    ...
    for spinSystem in spinSystems:
        ...
        if typeScore > typeThreshold:
            ...
            partScore, partRoute = followRoute(
                peptide, spinSystems, ensemble, inter, route2, typeThreshold, scoreThreshold, totalScore
            )
