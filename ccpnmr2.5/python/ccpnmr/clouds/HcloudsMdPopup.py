...
from ccpnmr.analysis.core.AssignmentBasic import assignAtomsToRes, newResonance
...
class HcloudsMdPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        self.constraintSetPulldown = PulldownMenu(
            frame, callback=self.changeConstraintSet, selected_index=0, do_initial_callback=0
        )
        ...
        self.distance1Pulldown = PulldownMenu(
            frame, callback=self.changeDistance1ConstraintList, selected_index=0, do_initial_callback=0
        )
        ...
        self.distance2Pulldown = PulldownMenu(
            frame, callback=self.changeDistance2ConstraintList, selected_index=0, do_initial_callback=0
        )
        ...
        self.distance3Pulldown = PulldownMenu(
            frame, callback=self.changeDistance3ConstraintList, selected_index=0, do_initial_callback=0
        )
        ...
        self.distance4Pulldown = PulldownMenu(
            frame, callback=self.changeDistance4ConstraintList, selected_index=0, do_initial_callback=0
        )
    ...
    def startMd(self):
        ...
        self.setNumClouds()
        self.setFilePrefix()
        if (self.constrLists != [None] * 4) and (self.numClouds > 0) and self.filePrefix:
            resDict = {}
            ...
            startMdProcess(self.numClouds, constraints, resonances, self.coolingScheme, self.filePrefix)
            ...
            if nextSpinSystem:
                ...
                if ca and c and n:
                    ...
                    c_n = constraintList.newDistanceConstraint(
                        weight=1.0, origData=1.0, targetValue=1.32, upperLimit=1.35, lowerLimit=1.29, error=0.06
                    )

                    # below based on angle constraints
                    ca_n = constraintList.newDistanceConstraint(
                        weight=1.0, origData=1.0, targetValue=2.415, upperLimit=2.513, lowerLimit=2.316, error=0.197
                    )
                ...
                for resonance2 in ss.resonances:
                    if dict.get(resonance2) and resonance2.assignNames[0][:2] == "HA":
                        ...
                        h_n = constraintList.newDistanceConstraint(
                            weight=1.0, origData=1.0, targetValue=1.01, upperLimit=1.1, lowerLimit=0.9, error=0.2
                        )
                        n_ca = constraintList.newDistanceConstraint(
                            weight=1.0, origData=1.0, targetValue=1.465, upperLimit=1.50, lowerLimit=1.43, error=0.07
                        )
                        ca_c = constraintList.newDistanceConstraint(
                            weight=1.0, origData=1.0, targetValue=1.525, upperLimit=1.55, lowerLimit=1.50, error=0.05
                        )

                        # below based on angle constraints
                        n_c = constraintList.newDistanceConstraint(
                            weight=1.0, origData=1.0, targetValue=2.464, upperLimit=2.572, lowerLimit=2.356, error=0.216
                        )
                        ...
                        if ss.newAtoms.get("CAHA") is None:
                            ca_ha = constraintList.newDistanceConstraint(
                                weight=1.0, origData=1.0, targetValue=1.09, upperLimit=1.19, lowerLimit=0.99, error=0.2
                            )
                ...
                for resonance2 in ss.resonances:
                    if dict.get(resonance2) and resonance2.assignNames[0][:2] == "HB":
                        ...
                        c_c = constraintList.newDistanceConstraint(
                            weight=1.0, origData=1.0, targetValue=1.53, upperLimit=1.56, lowerLimit=1.51, error=0.05
                        )
