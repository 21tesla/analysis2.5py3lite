...
def makeJsonDict(containerObj, direction="input"):
    ...
    for datum in containerObj.sortedData():
        if datum.parameterGroup is None or not topLevel:
            ...
            if className == "ParameterGroup" and datum.parameterGroup in (None, containerObj):
...
def setStdParameters(protocol, nmrCalcRun, interface=None):
    ...
    for paramObj in protocol.sortedProtocolParameters():
        interfaceObj = interface and interface.findFirstInterfaceParameter(protocolParameter=paramObj)
    ...
    for interfacedd in interfaces:
        ...
        for tabdd in interfacedd["tabs"]:
            ...
            for pardd in ll:
                ...
                for tag in ("isOrdered", "isEditable", "locard", "hicard", "row", "col", "rowspan", "colspan"):
                    val = pardd.get(tag)
                    if val is not None:
                        dd[tag] = val
...
def interfaceTabs(protocolInterface):
    ...
    for xx in protocolInterface.sortedInterfaceParameters() + protocolInterface.sortedInterfaceLabels():
...
def parseResidueExpr(molSystem, selector):
    ...
    for chainExpr in chainExprs:
        ...
        if resExpr:
            seqIds = parseNumberExpr(resExpr, endat=max(x.seqId for x in chain.residues))
...
def parseNumberExpr(selector, startat=1, endat=None):
    ...
    if strs[-1].endswith("-"):
        if endat is None:
            raise Exception("max mumber must be specified to allow selector ending with '-'")
...
def initRunInteractive(argServer, protocolName=None, prelimProtocolName=None, masterRun=None):
    ...
    nmrCalcRun = setupRunInteractive(argServer, protocolName=protocolName, nmrCalcRun=nmrCalcRun)
    #
    return nmrCalcRun


def initRunDialogue(argServer, protocolName=None, prelimProtocolName=None, masterRun=None):
    """Interactive initialisation of NmrCalc Run starting from protocol names"""
    if masterRun is not None:
        # copy template run -
        # e.g. because we have entered data already for multistructure run
    ...
    nmrCalcRun = setupRunDialogue(argServer, protocolName=protocolName, nmrCalcRun=nmrCalcRun)
    #
    return nmrCalcRun
...
def initNmrCalcRun(entryProtocol, nmrCalcRun=None, nmrCalcStore=None, interface=None):
    ...
    if nmrCalcRun is None:
        ...
        nmrCalcRun = nmrCalcStore.newRun(details=entryProtocol.name + " Run", wmsProtocolName=entryProtocol.name)
    else:
...
def setupRunDialogue(argServer, protocolName=None, nmrCalcRun=None):
    ...
    interface = (
        entryProtocol.findFirstProtocolInterface(name=entryProtocol.name) or entryProtocol.findFirstProtocolInterface()
    )
    ...
    nmrCalcRun = (
        popup.run
    )  # ParameterEditor sets to None if Cancel button hit; otherwise it is above nmrCalcRun if Ok button hit
    popup.destroy()
        ...
        if ip.isEditable:
            ...
            if pp.container is None:
                ...
                while ip.hicard < 0 or count <= ip.hicard:
                    ...
                    dataObj = None
                    for ip2 in ll:
                        nmrCalcObj = makeNmrCalcData(argServer, nmrCalcRun, ip2.protocolParameter, dataObj=dataObj)
...
def makeNmrCalcData(argServer, run, protocolParameter, ioRole="input", dataObj=None):
    ...
    if paramType in (
        ...
        if selector == ss:
            ...
            result = run.newMolSystemData(molSystemCode=molSystem.code, chainCodes=chainCodes, **parDD)
        ...
        if selector == ss:
            result = run.newStructureEnsembleData(models=structureEnsemble.sortedModels(), **parDD)
        elif selector is None:
            return
            ...
            result = run.newStructureEnsembleData(
                structureEnsemble=structureEnsemble, modelSerials=sorted(modelNums), **parDD
            )
        ...
        if constraintStore:
            ...
            if selector == ss:
                result = run.newConstraintStoreData(constraintLists=constraintStore.sortedConstraintLists(), **parDD)
            else:
                ...
                result = run.newConstraintStoreData(
                    nmrConstraintStore=constraintStore, constraintListSerials=clistSerials, **parDD
                )
        else:
            return
    else:
...
def writeDataFiles(nmrCalcRun, targetDir):
    ...
    if xx is not None:
        ...
        keywds = intUtil.getFileFormatData(fileFormat, dataType="seq").get("IOkeywords", {}).copy()
    ...
    for obj in nmrCalcRun.sortedData():
        if obj.name == "shiftList" and obj.className == "MeasurementListData":
            ...
            if xx is not None:
                ...
                keywds = intUtil.getFileFormatData(fileFormat, dataType="shift").get("IOkeywords", {}).copy()
                keywds.update(myKeywds)
                ...
                if keywds.get("presetResonanceMapping"):
                    ...
                    if "resonanceToAtoms" not in presetInput:
                        ...
                        constraintData = nmrCalcRun.findFirstData(name="assignment", className="ConstraintStoreData")
                        if constraintData:
                            nmrConstraintStore = constraintData.nmrConstraintStore
                        else:
                            constraintStores = set(
                                x.nmrConstraintStore for x in nmrCalcRun.findAllData(className="ConstraintStoreData")
                            )
                        ...
                        dd = {}
                        for tag in ("individualAtomsIfNoSet", "individualAtoms", "ignoreChemCompsWithNoSysNames"):
                            dd[tag] = keywds[tag]

                        resonanceToAtoms = intUtil.getResonanceAtomMap(
                            namingSystemName, nmrConstraintStore, shiftListObj=obj, **dd
                        )
                ...
                handler.writeShifts(filePath, measurementList=obj.measurementList, **keywds)
    ...
    for dataObj in nmrCalcRun.sortedData():
        ...
        if dataObj.name == "noesyPeakList":
            ...
            if xx is not None:
                ...
                dataDimRefs = [x.findFirstDataDimRef() for x in peakList.dataSource.sortedDataDims()]

                handler = dataIoHandler.getIoHandler(fileFormat, dataType="peak")
                keywds = intUtil.getFileFormatData(fileFormat, dataType="peak").get("IOkeywords", {}).copy()
                keywds.update(myKeywds)
                ...
                handler.writePeaks(filePath, peakLists=[peakList], dataDimRefs=dataDimRefs, **keywds)
            ...
            if xx is not None:
                ...
                fileNames = [
                    x.textValue for x in dataObj.sortedRunParameters() if x.name == "fileName" and x.ioRole == "input"
                ]
                for ii, constraintList in enumerate(dataObj.constraintLists):
                    ...
                    keywds = intUtil.getFileFormatData(fileFormat, dataType=constraintTag).get("IOkeywords", {}).copy()
                    keywds.update(myKeywds)
...
def setupCalculation(nmrCalcRun, targetDir, protocolName=None, pluginModule=None, doGeneralAdapt=None):
    ...
    print(
        "Starting setupCalculation; protocol %s; plugin %s"  #
...
def setupMultiInteractive(argServer, protocolNames, prelimProtocolName, executeProc=False):
    """Set up or run multiple, connected protocols in one run"""

    # Get master data. shared between protocols
    masterRun = initRunInteractive(argServer, protocolName=prelimProtocolName)

    executeScripts = []
    for protocolName in protocolNames:
        executeScript = setupSingleInteractive(argServer, protocolName=protocolName, masterRun=masterRun)
...
def runSingleInteractive(argServer, protocolName, prelimProtocolName=None, masterRun=None):
    """Run single protocol for one run"""
    executeScript = setupSingleDialogue(argServer, protocolName, prelimProtocolName, masterRun)
    if executeScript:
...
def setupSingleDialogue(argServer, protocolName, prelimProtocolName=None, masterRun=None):
    """Set up single protocol for one run"""

    # get data

    nmrCalcRun = initRunDialogue(
        argServer, protocolName=protocolName, prelimProtocolName=prelimProtocolName, masterRun=masterRun
    )
...
def prepareSingleRun(nmrCalcRun, protocolName, doGeneralAdapt=None):
    ...
    if hasattr(pluginModule.write, "prepareLocalExecution"):
        commandList, scriptTargetDir, commandList2 = pluginModule.write.prepareLocalExecution(nmrCalcRun, targetDir)

        return writeExecuteScript(commandList, scriptTargetDir, commandList2, protocolName=nmrCalcRun.wmsProtocolName)

    else:
        print("WARNING, function 'prepareLocalExecution' not found in %s" % pluginModule.__path__)


def writeExecuteScript(
    commandList,
    targetDir,
    commandList2,
    scriptName="ccpnExecute.py",
    logFile="ccpnCalc.out",
    ...
    try:
        fp.write(
            """\"\"\" CCPN automatic execution script
Protocol: %s
\"\"\"
...
def doPrepareStdWmsRun(nmrCalcRun, pluginModule, targetDir=None):
    ...
    try:
        ...
        setupCalculation(nmrCalcRun, targetDir=targetDir, pluginModule=pluginModule)
...
def prepareStdWmsRun(pluginName, projectDir, nmrCalcRunId, targetDir=None):
    ...
    if nmrCalcRun is None:
        ...
        pluginModule = __import__(pluginName, globals(), locals(), ["read", "write", "Util"])

        targetDir = doPrepareStdWmsRun(nmrCalcRun, pluginModule, targetDir=targetDir)
    #
    return targetDir
...
def getDataSummary(nmrCalcRun):
    ...
    text = []
    text.append(
        "Run: %s %s %s %s" % (nmrCalcRun.wmsProtocolName, nmrCalcRun.softwareName, nmrCalcRun.status, nmrCalcRun)
    )
    text.append("    RunParameters :")
    for pp in nmrCalcRun.sortedRunParameters():
        text.append("%s %s %s %s" % (pp.name, pp.code, pp.ioRole, intUtil.getParameterValue(pp)))
        if pp.data:
            text[-1] += " dataSerial %s" % pp.data.serial

    text.append("    Data :")
    for obj in nmrCalcRun.sortedData():
        text.append("%s %s %s %s %s" % (obj.name, obj.code, obj.ioRole, obj.serial, obj.className))
...
def mergeParallelRuns(calcId, projectFiles, targetDir=None):
    ...
    for ii, path in enumerate(projectFiles):
        ...
        elif os.path.isfile(path) and (path.endswith(".tgz") or path.endswith(".tar.gz")):
        ...
        if len(ll) == 1:
            ...
            raise Exception(" %s should contain 1 CCPN project, %s found" % (target, len(ll)))

        # find active run datadir

        # First make dictionary to sort in runId order
        integratorDir = os.path.join(projDir, "integrator")
        subdirs = [x for x in os.listdir(integratorDir) if x.startswith(calcStoreGuid)]
        subdirDict = {}
        for ss in subdirs:
            if ss.endswith(".in"):
    ...
    for ii, tt in enumerate(mergeData):
        if tt[2] is None:
            if baseIndex is None:
                ...
                raise Exception(
                    "Two directories lacked mergeable data: %s, %s" % (mergeData[baseIndex][3], mergeData[ii][3])
                )
    if baseIndex is None:
        baseIndex = 0
