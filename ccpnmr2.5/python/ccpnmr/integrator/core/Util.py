...
_fileFormatData = {
    "CYANA": {"handlerClass": CyanaFormat, "seqExt": "seq", "shiftExt": "prot", "peakExt": "peaks", "IOkeywords": {}},
    "XEASY": {"handlerClass": XEasyFormat, "seqExt": "seq", "shiftExt": "prot", "peakExt": "peaks", "IOkeywords": {}},
    "NMRVIEW": {"handlerClass": NmrViewFormat, "seqExt": "seq", "shiftExt": "ppm", "peakExt": "xpk", "IOkeywords": {}},
    "NMRSTAR": {
    ...
    "SPARKY": {"handlerClass": SparkyFormat, "seqExt": "seq", "shiftExt": "ppm", "peakExt": "peaks", "IOkeywords": {}},
    "CNS": {"handlerClass": CnsFormat, "constraintExt": "tbl", "IOkeywords": {}},
}
...
_fileFormatData["TALOS"] = {"handlerClass": TalosFormat, "shiftExt": "tab", "IOkeywords": {"minShiftQuality": 0.1}}
...
runDataTags = {
    "MolSystem": frozenset(
        ("serial", "name", "code", "ioRole", "details", "molSystemCode", "symmetrySetId", "chainCodes")
    ),
    "ConstraintStore": frozenset(
        ("serial", "name", "code", "ioRole", "details", "constraintStoreSerial", "constraintListSerials")
    ),
    "PeakList": frozenset(
        ("serial", "name", "code", "ioRole", "details", "experimentSerial", "dataSourceSerial", "peakListSerial")
    ),
    "MeasurementList": frozenset(("serial", "name", "code", "ioRole", "details", "measurementListSerial")),
    "MolResidue": frozenset(
        ("serial", "name", "code", "ioRole", "details", "molSystemCode", "chainCode", "residueSeqIds", "chainCodes")
    ),
    "ViolationList": frozenset(
        ("serial", "name", "code", "ioRole", "details", "constraintStoreSerial", "violationListSerial")
    ),
    "Spectrum": frozenset(("serial", "name", "code", "ioRole", "details", "experimentSerial", "dataSourceSerial")),
    "DerivedList": frozenset(("serial", "name", "code", "ioRole", "details", "derivedDataListSerial")),
    "StructureEnsemble": frozenset(
        ("serial", "name", "code", "ioRole", "details", "molSystemCode", "ensembleId", "modelSerials")
    ),
    "External": frozenset(("serial", "name", "code", "ioRole", "details", "dataLocationStoreName", "dataStoreSerial")),
    "FloatMatrix": frozenset(("serial", "name", "code", "ioRole", "details", "shape", "data", "defaultValue")),
    # TBD:
    #'SpinSystem':(),
    #'Tensor':(),
    #'EnergyTerm':(),
...
nonDataTags = {
    "RunParameter": ("name", "code", "ioRole", "textValue", "intValue", "floatValue", "booleanValue"),
    "ParameterGroup": (),
}
...
def runIsWritable(nmrCalcRun):
    ...
    """
    result = True
    if nmrCalcRun.findAllData(ioRole="output") or nmrCalcRun.findAllRunParameters(ioRole="output"):
        result = False
    #
    return result
...
def getMeasurementList(nmrCalcRun, measurementType="ShiftList", ioRole="input"):
    ...
    """
    ll = []
    for obj in nmrCalcRun.findAllData(className="MeasurementListData", ioRole="input"):
...
def getConstraintList(nmrCalcRun, measurementType="ChemShiftConstraintList", ioRole="input"):
    ...
    ll = nmrCalcRun.findAllData(className="ConstraintStoreData", ioRole=ioRole)
    if len(ll) == 1:
        obj = ll.pop()
    ...
    for ss in selection.split(","):
        ...
        for cCode in chains:
            ...
            if singleChain is not None:
                ...
                raise ValueError("Unknown chain code %s in input %s" % (cCode, selection))
...
def filterMeasurements(measurements, atomNames=None, residues=None, minFigOfMerit=0.0):
...
def makeSingleConstraintList(constraintStore, measurements, constraintType="ChemShift", name=None, details=None):
    """Make new one-resonance ConstraintList containing measurements in
    constraintStore
    """

    createFunction = getattr(constraintStore, "new%sConstraintList" % constraintType)
    ...
    for meas in measurements:
        ...
        fixedResonance = ConstraintBasic.getFixedResonance(constraintStore, meas.resonance)
        createFunction = getattr(newList, "new%sConstraint" % constraintType)
        createFunction(targetValue=meas.value, error=meas.error, resonance=fixedResonance)
...
def makePairConstraintList(constraintStore, measurements, constraintType="JCoupling", name=None, details=None):
    ...
    for meas in measurements:
        ...
        for resonance in meas.resonances:
            fixedResonances.append(ConstraintBasic.getFixedResonance(constraintStore, resonance))
        createFunction = getattr(newList, "new%sConstraint" % constraintType)
...
def constraintListFromData(nmrCalcRun, constraintStore, data, name, code, numResonances, parameterGroup):
    """NBNB TBD"""
    return None
...
def getNmrCalcStore(memopsRoot, nmrProjectName=None, nmrCalcStoreName=None):
    ...
    if nmrCalcStoreName is None:
        ...
        result = memopsRoot.findFirstNmrCalcStore(name=nmrCalcStoreName, nmrProjectName=nmrProjectName)
    if result is None:
        # Nothing found - make a new one
        print("### ALARM making newNmrCalcStore", nmrProjectName, nmrCalcStoreName or "CCPNauto")
        result = memopsRoot.newNmrCalcStore(nmrProjectName=nmrProjectName, name=nmrCalcStoreName or "CCPNauto")
    #
    return result
...
def makeNmrCalc(memopsRoot, jsonObject):
    ...
    run = nmrCalcStore.newRun(wmsProtocolName=wmsProtocolName, details=jsonObject.get("details"))

    nmrCalcId = nmrCalcStore.guid + "+" + str(run.serial)

    # Make Top-level parameters
    for tag in jsonObject:
        if tag not in classGenerators and tag not in nonDataTags and not tag.startswith("CCPN."):
...
def wmsProjectSummary(memopsRoot, nmrProjectName=None):
    ...
    for molSystem in memopsRoot.sortedMolSystems():
        ...
        for symSet in [None] + molSystem.sortedMolSystemSymmetrySets():
            dd = {"molSystemCode": molSystemCode, "chainCodes": [x.code for x in molSystem.sortedChains()]}
            objlist.append(dd)
            if symSet:
...
def adaptNmrCalcRun(nmrCalcRun):
    ...
    constraintStoreSerials = set(
        x.constraintStoreSerial for x in nmrCalcRun.data if x.className in ("ConstraintStoreData", "ViolationListData")
    )
    if constraintStoreSerials:
        ...
        mainConstraintStore = project.newNmrConstraintStore(nmrProject=nmrProject)
    assignStoreData = nmrCalcRun.newConstraintStoreData(
        name="assignment",
        nmrConstraintStore=mainConstraintStore,
        details="Input assignment state and shifts (as constraints) " + "set by CcpNmr Integrator",
    )
    ...
    for xx in nmrCalcRun.sortedData():
        if xx.className == "ConstraintStoreData" and xx.ioRole == "input" and xx.name == "constraintLists":
    ...
    if constraintListData and restraintFormat:
        ...
        for constraintList in constraintListData.constraintLists:
            ...
            nmrCalcRun.newRunParameter(name="fileName", textValue=fileName, code="fileName", data=constraintListData)

    # peakFiles = []
    for peakListData in peakListObjs:
        if peakFormat is not None:
            nmrCalcRun.newRunParameter(name="fileFormat", textValue=peakFormat, data=peakListData)
            fileNamePeakList = objectFileName(peakListData.peakList, suffix=peakFileSuffix, compact=True)
            nmrCalcRun.newRunParameter(name="fileName", textValue=fileNamePeakList, code="fileName", data=peakListData)
        ...
        if shiftFormat is not None:
            ...
            if shiftList not in shiftLists:
                ...
                shiftListData = nmrCalcRun.newMeasurementListData(name="shiftList", measurementList=shiftList)
    ...
    if not shiftListObjs:
        for shiftList in nmrCalcRun.nmrCalcStore.nmrProject.sortedMeasurementLists():
            if shiftList.measurements and shiftList.className == "ShiftList":
                shiftLists.add(shiftList)
                shiftListObjs.append(nmrCalcRun.newMeasurementListData(name="shiftList", measurementList=shiftList))
                break
        else:
    ...
    if shiftFormat is not None:
        for shiftListData in shiftListObjs:
            nmrCalcRun.newRunParameter(name="fileFormat", textValue=shiftFormat, data=shiftListData)
            shiftList = shiftListData.measurementList
            shiftFile = objectFileName(shiftList, suffix=shiftFileSuffix, compact=True)
    ...
    for shiftList in shiftLists:
        ...
        ll = [
            x
            for x in mainConstraintStore.findAllConstraintLists(className="ChemShiftConstraintList")
...
def validateFixedAssignments(constraintStore, nmrProject):
    ...
    """
    if nmrProject is not constraintStore.nmrProject:
        ...
        if fresSet:
            ...
            if fatoms:
                ...
                if atoms:
                    if atoms != fatoms:
                        raise Exception(
                            "WARNING, incompatibility, assignment %s for %s differs from %s"
                            % (fres.name, fres, res.name)
                        )
                else:
                    print("WARNING, incompatibility, extra assignment %s for %s" % (fres.name, fres))
        if not fatoms:
...
def getAmalgamatedTolerances(dataSources, defPoints, minTol, tagOrder=("hx2", "hx1", "x1", "x2")):
    ...
    for dataSource in dataSources:
        tolerances.append(AssignmentBasic.estimateAssignmentTolerances(dataSource, defPoints=defPoints, minTol=minTol))
    ...
    for ii, codes in enumerate(dimCodes):
        ...
        for tag in tags:
            if tag in codes:
                ...
                ll.append(None)
        if "x2" in codes and "x1" not in codes:
...
def getResonanceAtomMap(namingSystemName, nmrConstraintStore, shiftListObj=None, **keywds):
    ...
    resonanceToAtoms = DataFormat.getResonanceAtomMap(namingSystemName, fixedResonances, **keywds)
    ...
    for fres, val in resonanceToAtoms.items():
        ...
        if res is None:
            vv = val[0]
            ss = "%s.%s.%s - %s" % (vv.chain.code, vv.seqId, vv.atomName or "", vv.atomSetName or "")
            print("WARNING, no resonance match for FixedResonance %s, (%s) - Skipping" % (fres.serial, ss))
        else:
            fresMap[fres] = res
...
def setRunParametersFromConfig(nmrCalcRun, confDict, ioRole="input"):
    ...
    for dd in confDict["protocol"]["protocolParameters"]:
        ...
        if paramType and "value" in dd and "relatedParameter" not in dd:
            name = dd.get("name")
            code = dd.get("code")
            runPar = None
            if name:
                runPar = nmrCalcRun.findFirstRunParameter(name=name, data=None, ioRole=ioRole)
            elif code:
                runPar = nmrCalcRun.findFirstRunParameter(code=code, data=None, ioRole=ioRole)
            else:
...
def makeDerivedRun(masterRun):
    ...
    nmrCalcRun = genUtil.copySubTree(
        masterRun,
        masterRun.nmrCalcStore,
        maySkipCrosslinks=True,
        topObjectParameters={"serial": nextSerial, "status": "provisional"},
    )
    # Remove output data
...
def linkFixedResonancesToResonances(constraintStore):
    ...
    # atoms2Resonance = []
    atoms2ResonanceSets = []
    for resonanceSets in (constraintStore.sortedFixedResonanceSets(), nmrProject.sortedResonanceSets()):
        # a2Res = {}
