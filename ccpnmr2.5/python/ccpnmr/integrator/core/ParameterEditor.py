...
class ParameterEditor(BasePopup):
    ...
    def makeInterfaceWidget(self, frame, interfaceObject):
        ...
        if clazz:
            ...
            widget = clazz(
                ...
                gridSpan=(interfaceObject.rowspan, interfaceObject.colspan),
                **kw,
            )
        ...
        if paramType in (
            ...
            if paramType == "molResidues":
                ...
                for experiment in experiments:
                    ...
                    dataValues.extend(
                        ["%s:%s:%s" % (experimentName, dataSourceName, peakList.serial) for peakList in peakLists]
                    )
                ...
                dataValues = [
                    "%s:%s" % (structureEnsemble.molSystem.code, structureEnsemble.ensembleId)
    ...
    def getInterfaceGroups(self):
        ...
        for interfaceObject in (
            protocolInterface.sortedInterfaceParameters() + protocolInterface.sortedInterfaceLabels()
        ):
