    ...
    for chain in molSystem.sortedChains():
        ...
        for link in chain.molecule.molResLinks:
            ...
            for linkEnd in link.molResLinkEnds:
                ...
                bond = chemComp.findFirstChemBond(chemAtoms=(ccLinkEnd.boundChemAtom, ccLinkEnd.boundLinkAtom))
                data.append(bondTypeMap[bond.bondType])
                result.append(data)
...
def getCosmosData(nmrCalcRun):
    ...
    if len(ll) == 1:
        ...
        for model in ensemble.sortedModels():
            ...
            coordinates.append([list(atomData[ii] + xyz) for ii, xyz in enumerate(coordTriplets)])

    else:
        raise Exception("Run must have exactly one input structure ensemble, %s found" % len(ll))
...
def mergeCosmosData(nmrCalcRun, cosmosOutput):
    ...
    nmrCalcRun.newRunParameter(name="summaryLog", code="SUMMARY_LOG", ioRole="output", textValue=summaryLog)

    if modelData:
        inputEnsembleData = nmrCalcRun.findFirstData(ioRole="input", name="structureEnsemble")
        ...
        nmrCalcRun.newStructureEnsembleData(
            ioRole="output", code="COO", name="structureEnsemble", structureEnsemble=newEnsemble
        )
        ...
        for modelDict in modelData:
            ...
            if modelLog is not None:
                nmrCalcRun.newRunParameter(
                    name="log%s" % modelSerial, code="LOG", ioRole="output", textValue=modelLog, data=newData
                )
                ...
                if data is not None:
                    if parameterGroup is None:
                        parameterGroup = nmrCalcRun.newParameterGroup(
                            name=newData.name, ioRole="output", data=(newData,)
                        )
                    ...
                    if inputModelData is None:
                        ...
                        inputModel = inputEnsembleData.structureEnsemble.findFirstModel(serial=modelSerial)
                    ...
                    constraintList = intUtil.constraintListFromData(
                        nmrCalcRun, constraintStore, data, name, code, numResonances, parameterGroup
                    )
                    if constraintList:
