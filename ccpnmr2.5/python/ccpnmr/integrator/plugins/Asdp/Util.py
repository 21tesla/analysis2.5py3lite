...
from ccp.lib.nmr.Nmr.DataSource import getDimCodes, getIsotopeCodesList
from ccpnmr.analysis.core import AssignmentBasic
from ccpnmr.integrator.core import Util as intUtil

spectrumTypes = {"aliphatic": "C13-ali", "aromatic": "C13-aro"}
...
def asdpNmrCalcRun(nmrCalcRun):
    ...
    for peakListData in peakListObjs:
        ...
        if None in dimCodes or not experiment.findFirstExpTransfer(transferType="through-space"):
        ...
        # peak file name
        fileNamePeakList = intUtil.objectFileName(peakListData.peakList, suffix=peakFileSuffix, compact=True)
        nmrCalcRun.newRunParameter(name="fileName", textValue=fileNamePeakList, code="fileNamePeak", data=peakListData)
        nmrCalcRun.newRunParameter(name="fileFormat", textValue=peakFormat, data=peakListData)

        nmrCalcRun.newRunParameter(name="numDim", code="dimension", intValue=dataSource.numDim, data=peakListData)

        # interchain NOEs not supported
        nmrCalcRun.newRunParameter(name="haveInterChain", code="haveIC", intValue=0, data=peakListData)
        nmrCalcRun.newRunParameter(name="interChain", code="IC", intValue=0, data=peakListData)
        ...
        if runParameter.textValue == "h2o":
            ...
            val = 0
        nmrCalcRun.newRunParameter(name="solventH2O", code="waterFlag", intValue=val, data=peakListData)
        runParameter.delete()

        # column data
        nmrCalcRun.newRunParameter(
            name="intensityColumn", code="col.intensity", intValue=dataSource.numDim + 4, data=peakListData
        )

        isotopeCodes = getIsotopeCodesList(dataSource)
        tolerances = AssignmentBasic.estimateAssignmentTolerances(dataSource, defPoints=defaultTol, minTol=minTol)
        ...
        for ii, dataDim in enumerate(dataSource.sortedDataDims()):
            ...
            tag = "col.%s" % dimCode
            nmrCalcRun.newRunParameter(name="PositionColumn", code=tag, intValue=ii + 2, data=peakListData)

            # Shift correction (set to default)
            tag = "%s.shift" % dimCode
            nmrCalcRun.newRunParameter(name="ShiftCorrection", code=tag, floatValue=0.0, data=peakListData)

            # sign (meaning ???) (set to default)
            tag = "%s.sign" % dimCode
            nmrCalcRun.newRunParameter(name=tag, code=tag, intValue=0, data=peakListData)

            # Assignment tolerance
            tag = "%s.tol" % dimCode
            nmrCalcRun.newRunParameter(name="AssignTolerance", code=tag, floatValue=tolerances[ii], data=peakListData)
            ...
            nmrCalcRun.newRunParameter(name="swPpm", code=tag, floatValue=swOrig, data=peakListData)
            ...
            tag = "%s.type" % dimCode
            nmrCalcRun.newRunParameter(name="dimensionType", code=tag, textValue=dimType, data=peakListData)
        ...
        if shiftListData is None:
            ...
            shiftFile = intUtil.objectFileName(shiftList, suffix=shiftFileSuffix, compact=True)
            nmrCalcRun.newRunParameter(name="fileName", textValue=shiftFile, data=shiftListData)
    ...
    if shiftListObjs:
        if len(shiftListObjs) == 1:
            ...
            nmrCalcRun.newRunParameter(
                name="fileNameShift", code="chemicalShiftFile", textValue=fileName, data=shiftData
            )
            nmrCalcRun.newRunParameter(
                name="fileFormatShift", code="fileFormatShift", textValue=fileFormat, data=shiftData
            )
        else:
