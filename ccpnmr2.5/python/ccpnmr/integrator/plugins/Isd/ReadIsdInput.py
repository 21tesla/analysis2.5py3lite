...
from ccp.format.talos.projectIO import TalosChemShiftFile, TalosSequenceFile
...
if __name__ == "__main__":
  ...
  """
    ...
    if os.path.isfile(path):
        ...
        seqReader.read()
        seqData = [(x.code1Letter, x.seqCode, x.seqInsertCode) for x in seqReader.sequences[-1].elements]
        del seqReader
        ...
        if None in ccpCodes:
            ...
            raise Exception("No residue found for code %s in position %s" % (seqData[ind][0], ind))
        ...
        shiftReader.read()
        shiftData = [(x.seqCode, x.seqInsertCode, x.resLabel, x.atomName, x.value) for x in shiftReader.chemShifts]
        del shiftReader
