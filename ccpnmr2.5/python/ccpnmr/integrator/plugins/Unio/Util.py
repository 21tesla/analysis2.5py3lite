...
spectrumTypes = {"all": "NOESY_all", "aliphatic": "NOESY_aliphatic", "aromatic": "NOESY_aromatic"}
...
def candidNmrCalcRun(nmrCalcRun):
    ...
    for peakListData in peakListObjs:
        ...
        nmrCalcRun.newRunParameter(name="fileNameShift", textValue=shiftFile, code="fileNameShift", data=peakListData)
        nmrCalcRun.newRunParameter(
            name="fileFormatShift", textValue=shiftFormat, code="fileFormatShift", data=peakListData
        )
        ...
        nmrCalcRun.newRunParameter(name="fileFormat", textValue=peakFormat, code="fileFormatPeak", data=peakListData)

    # number of data sets
    nmrCalcRun.newRunParameter(name="numDataSets", code=codeRemapping["numDataSets"], intValue=numDataSets)
    ...
    if shiftListObjs:
        if len(shiftListObjs) == 1:
            ...
            for peakListData in usePeakListObjs:
                ...
                spectrumKind = peakListData.findFirstRunParameter(name="spectrumKind").textValue
                spectrumType = peakListData.findFirstRunParameter(name="spectrumType").textValue
                shiftFileWeight = preferredShiftWeights.get(spectrumKind, 0) + preferredShiftWeights.get(
                    spectrumType, 0
                )
                if shiftFileWeight > shiftListScore:
                    shiftListScore = shiftFileWeight
                    fileName = peakListData.findFirstRunParameter(name="fileNameShift").textValue
                    fileFormat = peakListData.findFirstRunParameter(name="fileFormatShift").textValue

        # main shift list
        nmrCalcRun.newRunParameter(name="fileNameShift", code="fileNameCaCbShift", textValue=fileName)
        nmrCalcRun.newRunParameter(name="fileFormatShift", code="fileFormatCaCbShift", textValue=fileFormat)
