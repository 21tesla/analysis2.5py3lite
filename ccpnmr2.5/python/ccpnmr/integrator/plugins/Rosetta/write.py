...
def write(nmrCalcRun, targetDir):
    ...
    open(propFile, "w").write(json.dumps(jsonDict, sort_keys=True, indent=intIo.propIndent))
...
def oldwrite(nmrCalcRun, topDir=None):
    ...
    propFile = uniIo.joinPath(targetDir, intIo.propFileName)
    open(propFile, "w").write(json.dumps(propDict, sort_keys=True, indent=intIo.propIndent))
    ...
    #
    talosIo.writeShiftFile(open(talosInputFile, "w"), residues, shiftList, propDict.get("minShiftQuality"))
