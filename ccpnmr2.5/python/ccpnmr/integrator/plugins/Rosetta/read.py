...
def read(nmrCalcRun, dataDir):
    ...
    if not intUtil.runIsWritable(nmrCalcRun):
        raise Exception("Cannot read into %s - has already been used" % nmrCalcRun)
    ...
    docTemplate = "Rosetta %s for all calculated structures, sorted by corrected score"
    ss = "corrected scores"
    nmrCalcRun.newFloatMatrixData(
        code="scores", name=ss, ioRole="output", shape=(0,), data=scores, details=docTemplate % ss
    )
    ss = "raw scores"
    nmrCalcRun.newFloatMatrixData(
        code="rawScores", name=ss, ioRole="output", shape=(0,), data=rawScores, details=docTemplate % ss
    )
    ss = "shift chi2 scores"
    nmrCalcRun.newFloatMatrixData(
        code="shiftChi2", name=ss, ioRole="output", shape=(0,), data=shiftChi2, details=docTemplate % "Chemical" + ss
    )
    ...
    nmrCalcRun.newFloatMatrixData(
        ...
        shape=(0,),
        data=rms,
        details=docTemplate % "RMS to structure with best corrected score",
    )
    ...
    nmrCalcRun.newFloatMatrixData(
        ...
        shape=(0,),
        data=rawRms,
        details=docTemplate % "RMS to structure with best raw score",
    )

    # Get MolSystem
    molResidueData = nmrCalcRun.findFirstData(className="MolResidueData", ioRole="input")
    ...
    ensemble = StructureIo.getStructureFromFiles(molSystem, paths, fileType="rough")

    ss = "Rosetta calculated structures, sorted by corrected score"
    nmrCalcRun.newStructureEnsembleData(
        name="result", ioRole="output", structureEnsemble=ensemble, details=docTemplate % ss
    )
