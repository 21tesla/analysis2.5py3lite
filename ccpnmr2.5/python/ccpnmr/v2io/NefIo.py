...
from ccp.lib import MoleculeModify, StructureIo
from ccp.lib import NmrExpPrototype as NmrExpPrototypeLib
from ccp.util import Spectrum as spectrumLib
from ccpnmr.analysis.core import AssignmentBasic, ConstraintBasic, ExperimentBasic, MoleculeBasic
...
def loadProject(nefFilePath, pdbFilePaths=None, projectName=None, pdbFileType="pdb"):
    ...
    if pdbFilePaths:
        if isinstance(pdbFilePaths, str):
            StructureIo.getStructureFromFile(memopsRoot.findFirstMolSystem(), pdbFilePaths, fileType=pdbFileType)
        else:
            StructureIo.getStructureFromFiles(memopsRoot.findFirstMolSystem(), pdbFilePaths, fileType=pdbFileType)
    #
    return memopsRoot
...
class CcpnNefReader:
    ...
    def load_nef_sequence(self, memopsRoot, loop):
        ...
        for chainCode, rows in sorted(chainData.items()):
            ...
            if lastChain is None:
                ...
                newChain = copySubTree(lastChain, memopsRoot.currentMolSystem, topObjectParameters={"code": chainCode})
    ...
    def load_nef_chemical_shift_list(self, project, saveFrame):
        ...
        shiftList = project.currentNmrProject.newShiftList(name=name, details=comment, isSimulated=isSimulated)
    ...
    def load_nef_restraint_list(self, project, saveFrame):
        ...
        for row in data:
            ...
            if restraint is None:
                ...
                raise RuntimeError(
                    "One-resonance restraint type %s, is %s has more than one item" % (restraintType, serial)
                )
        ...
        if dataSource is None:
            ...
            for expDim in nmrExperiment.sortedExpDims():
                ...
                if params.get("spectralWidth"):
                    ...
                    params["valuePerPoint"] = 10.0

                dataDim = dataSource.newFreqDataDim(
                    dim=dim,
                    expDim=nmrExperiment.findFirstExpDim(dim=dim),
                    isComplex=isComplexDict.get(dim, False),
                    **params,
                )
            ...
            if loop and loop.data and nmrExperiment:
                for row in loop.data:
                    ...
                    xdr = [
                        nmrExperiment.findFirstExpDim(dim=dim).findFirstExpDimRef()
                        if nmrExperiment.findFirstExpDim(dim=dim)
                        else None
                        for dim in dims
                    ]
                    if not any(val is None for val in xdr):
                        ...
                        nmrExperiment.newExpTransfer(expDimRefs=xdr, transferType=transferType, isDirect=isDirect)
            ...
            if refExperiment is not None:
                if axisCodesRead:
                    ...
                    if None not in expDimRefs:
                        isotopeCodes = [x.isotopeCodes[0] if x.isotopeCodes else "unknown" for x in expDimRefs]
                        dimensionIds = [x.dim for x in expDims]
                        acquisitionAxisIndex = None if acquisitionDim is None else dimensionIds.index(acquisitionDim)

                        axisCodes = makeNefAxisCodes(isotopeCodes, dimensionIds, acquisitionAxisIndex, transferData)
        ...
        for row in loop.data:
            ...
            for tt in assignments:
                if all(x is None for x in tt):
                    ...
                    self.warning("Uninterpretable Peak assignment for peak %s: %s. Set to None" % (peak.serial, tt))
                    resonancesPerDimension.append(())
            if foundAssignment:
    ...
    def fetchAtomMap(self, chainCode, sequenceCode, name, isotopeCode=None, comment=None, serial=None):
    ...
    def fetchResidueMap(self, chainCode, sequenceCode, residueType=None, linkToMap=None, serial=None):
        ...
        if resonanceGroup is None:
            ...
            if serial:
                useSerial = serial
            elif offset is None and seqCode is None and seqInsertCode.startswith("@") and seqInsertCode[1:].isdigit():
            ...
            if useSerial:
                try:
                    ...
                    print(
                        "INFO: ResonanceGroup serial number %s could not be preserved. "
            ...
            if offset is None:
                if linkToMap is not None:
                    ...
                    previousResonanceGroup.newResonanceGroupProb(
                        linkType="sequential", sequenceOffset=1, isSelected=True, possibility=resonanceGroup
                    )
            else:
                ...
                if offset == 0:
                    ...
                    linkType = "sequential"
                mainResonanceGroup.newResonanceGroupProb(
                    linkType=linkType, sequenceOffset=offset, possibility=resonanceGroup, isSelected=True
                )
        #
        return result
    ...
    def load_ccpn_assignment(self, project, saveFrame):
        ...
        for row in saveFrame["nmr_residue"].data:
            ...
            if chainType == "assigned":
                ...
                raise ValueError(
                    "Invalid data, chain_code %s, sequence_code %s appear twice" % (chainCode, sequenceCode)
                )
            ...
            if offset is None:
                if chainType == "connected":
                    ...
                    linkToMap = None
                newMap = self.fetchResidueMap(
                    chainCode, sequenceCode, residueType=row["residue_name"] or None, linkToMap=linkToMap, serial=serial
                )
    ...
    def preloadAssignmentData(self, dataBlock):
            ...
            for chainCode, residueDict in sorted(assignmentData.items()):
                ...
                for sequenceCode, residueType in residueDict:
                    ...
                    if offset is None:
                        ...
                        newMap = self.fetchResidueMap(
                            chainCode=chainCode, sequenceCode=sequenceCode, residueType=residueType, linkToMap=linkToMap
                        )
                        if chainCode.startswith("#"):
                            previousConnectedMaps[chainCode] = newMap
                    else:
        ...
        for sequenceCode, residueType in postponed:
            ...
            self.fetchResidueMap(chainCode=chainCode, sequenceCode=sequenceCode, residueType=residueType)
...
def createMoleculeFromNef(project, name, sequence, defaultType="UNK"):
    ...
    for stretch in stretches:
        ...
        if len(residueTypes) > 1:
            ...
            if firstLinking in ("start", "single", "nonlinear", "dummy") or lastLinking == "end":
                isCyclic = False
            else:
                # We use isCyclic to set the ends to 'middle'. It gets sorted out below
                isCyclic = True

            molResidues = extendMolResidues(molecule, sequence=residueTypes, startNumber=startNumber, isCyclic=isCyclic)
            ...
            if not tt:
                print(
                    """Could not access ChemComp for %s - replacing with %s
NB - could be a failure in fetching remote information.
Are you off line?"""
                    % (residueTypes[0], defaultType)
                )
            ...
            if tt:
                ...
                if chemComp:
                    chemCompVar = (
                        chemComp.findFirstChemCompVar(linking="none") or chemComp.findFirstChemCompVar()
                    )  # just a default
                    if chemCompVar:
                        molecule.newMolResidue(
                            seqCode=startNumber, chemComp=chemCompVar.chemComp, chemCompVar=chemCompVar
                        )
                    else:
                        raise ValueError(
                            "No chemCompVar found for %s. Vars should be in  %s" % (chemComp, chemComp.chemCOmpVars)
                        )

                else:
                    raise ValueError(
                        "Residue type %s %s: Error in getting template information" % (residueTypes[0], tt)
                    )
...
def extendMolResidues(molecule, sequence, startNumber=1, isCyclic=False):
    ...
    if None in seqInput:
        ...
        raise ValueError("Unknown residueName %s at position %s in sequence" % (sequence[ii], ii))
    ...
    while offset1 < len(seqInput):
        ...
        if molType1 in Constants.LINEAR_POLYMER_TYPES:
            ...
            while offset2 < len(seqInput):
                ...
                if molType2 in Constants.LINEAR_POLYMER_TYPES and (molType1 == "protein") == (molType2 == "protein"):
            ...
            if offset2 - offset1 > 1:
                result.extend(
                    MoleculeModify.makeLinearSequence(
                        molecule, seqInput[offset1:offset2], seqCodeStart=startNumber + offset1, isCyclic=isCyclic
                    )
                )
                offset1 = offset2
...
def fetchDataUrl(memopsRoot, fullPath):
    ...
    standardStore = memopsRoot.findFirstDataLocationStore(name="standard") or memopsRoot.newDataLocationStore(
        name="standard"
    )
