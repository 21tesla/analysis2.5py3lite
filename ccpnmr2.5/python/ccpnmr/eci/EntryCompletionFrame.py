...
NMR_LISTS_DATA = [
    ("ShiftList", "Shift lists", "Assigned NMR chemical shifts"),
...
DERIVED_LIST_CLASSES = ["DataList", "IsotropicS2List", "PkaList", "SpectralDensityList"]

CONSTRAINT_NAME_DATA = {
    "ChemShiftConstraintList": "Chemical Shift Constraints",
...
POS_ROLES = ["principal investigator", "responsible scientist", "investigator"]
...
ORGANISMS = {
    ...
    "Bacteria F-L": [
        ("Fervidobacterium nodosum", "Fervidobacterium nodosum"),
    ...
    "Bacteria M-P": [
        ("Mastigocladus laminosus", "Mastigocladus laminosus"),
    ...
    "Viruses": [
        ("Bacillus phage phi29", "Bacillus phage phi29"),
...
EXP_ORGANISMS = [
    ...
    "Synthetic",
    "Unidentified",
    "Wheat germ - cell freeE. coli - cell free",
]

SPEC_MANU_LIST = ["Varian", "Bruker", "JEOL", "General Electric", "Chemagnetics", "Custom Made", "Other"]
...
class EntryCompletionFrame(Frame):
    ...
    def __init__(self, parent, basePopup, *args, **kw):
        ...
        self.classSubmitDict = {
            "PeakList": ("PeakList", "peakLists"),
        }
        ...
        frameM, frameA, frameB, frameC, frameD, frameE, frameF, frameG, frameH, frameI, frameJ, frameK = (
            tabbedFrame.frames
        )
        ...
        self.titleEntry = Entry(frameM, text="", width=32, tipText=tipText, returnCallback=self.updateEntryTitle)
        ...
        self.nmrMethodPulldown = PulldownList(frameM, texts=nmrMethodTypes, tipText=tipText, callback=self.setEntryType)
        ...
        self.subTypePulldown = PulldownList(
            frameM, texts=submissionTypes, tipText=tipText, callback=self.setSubmissionType
        )
        ...
        self.pdbKeywordsMulti = MultiWidget(frameM, Entry, minRows=1, useImages=False, tipText=tipText, callback=None)
    ...
    """
        ...
        tipTexts = [
            "Indicates mandatory data items for PDB or BMRB depositions - clicking in any field will take you to the correct tab for editing the data",
            "Has the mandatory data been completed? Finished data is shown in green. Clicking on any missing data (shown in red, orange, or yellow) will take you to the correct tab so that you can add this information",
        ]
        ...
        self.reportTable = ScrolledMatrix(
            frameM,
            multiSelect=False,
        ...
        texts = [
            ...
            "Export PDB Coords",
        ]  # , 'Export CNS Distance Restraints', 'Make CCPN Project as tgz File']
        commands = [
            self.addNewEntry,
            self.deleteCurrentEntry,
            self.exportNmrStar31,
            self.exportPdb,
        ]  # , self.exportCnsDistance, self.packageProject]

        tipTexts = [
            'Add a new "Entry" object that can be submitted to the PDB or BMRB',
        ...
        self.titlePulldown = PulldownList(self, texts=TITLES, callback=self.setPersonTitle)
        ...
        self.famTitlePulldown = PulldownList(self, texts=FAM_TITLES, callback=self.setFamilyTitle)
        ...
        editWidgets = [
            ...
            None,
            None,
            self.familyNameEntry,
            self.titlePulldown,
            self.givenNameEntry,
            self.initialsEntry,
            self.famTitlePulldown,
            self.personGroupPulldown,
        ]
        editGetCallbacks = [
            None,
            self.toggleAuthor,
        ...
        self.personTable = ScrolledMatrix(
            frameA,
            multiSelect=False,
            headingList=headingList,
            tipTexts=tipTexts,
        ...
        texts = [
            "Add Person",
            "Remove Person",
            "Set Person as Author",
            "Set Person As Contact",
        ...
        commands = [
            self.addPerson,
            self.removePerson,
            self.setPersonAsAuthor,
            self.setPersonAsContact,
            self.setPersonAsPrimary,
        ]
        ...
        self.orgTypePulldown = PulldownList(self, texts=ORGN_TYPES, callback=self.setOrgType)
        ...
        editWidgets = [
            ...
            self.groupCityEntry,
            self.stateProvEntry,
            self.postalCodeEntry,
            self.countryEntry,
            self.orgTypePulldown,
        ]
        ...
        self.groupTable = ScrolledMatrix(
            frame1,
            headingList=headingList,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            editWidgets=editWidgets,
            callback=self.selectGroupTable,
            grid=(0, 0),
        )

        texts = ["Add Group", "Remove Group"]

        tipTexts = ["Add a new group", "Remove the currently selected group"]
        ...
        self.positionPulldown = PulldownList(self, texts=POS_ROLES, callback=self.setPosition)

        headingList = ["Family Name", "Given Name", "Email Address", "Telephone", "FAX", "Position"]
        ...
        editWidgets = [
            None,
            None,
            None,  # self.emailAddressEntry,
            self.telephoneEntry,
            self.faxEntry,
            self.positionPulldown,
        ]

        editSetCallbacks = [
            None,
            None,
            None,  # self.setEmailAddress,
            self.setTelephone,
            self.setFax,
            self.setPosition,
        ]
        ...
        editGetCallbacks = [
            None,
            None,
            None,  # self.getEmailAddress,
            self.getTelephone,
            self.getFax,
            self.getPosition,
        ]

        self.addressTable = ScrolledMatrix(
            frame2,
            headingList=headingList,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            editWidgets=editWidgets,
            callback=self.selectAddressTable,
            grid=(0, 0),
        )
        ...
        tipTexts = [
            "Add a new email address for the person selected - the person does need to be set to being in a group using the two other frames in this tab",
        ...
        self.citationAuthorMulti = MultiWidget(self, PulldownList, minRows=1, callback=self.setCiteAuthors)
        self.citationEditorMulti = MultiWidget(self, PulldownList, minRows=1, callback=self.setCiteEditors)
        ...
        self.citeKeywordsMulti = MultiWidget(self, Entry, minRows=2, callback=self.setCiteKeywords)
        ...
        editWidgets = [
            None,
            None,
            self.citeDepositionPulldown,
            self.citationAuthorMulti,
            self.citationEditorMulti,
            self.citeYearPulldown,
            self.citeTitleEntry,
            self.citeJournalPulldown,
            self.citeIssueEntry,
            self.citeVolumeEntry,
            self.citeFirstPageEntry,
            self.citeLastPageEntry,
            self.citeStatusPulldown,
            self.citePubMedEntry,
            self.citeDoiEntry,
            self.citeKeywordsMulti,
            self.citeDetailsEntry,
            self.bookTitleText,
            self.bookSeriesEntry,
            self.bookPublisherEntry,
            self.citeCityEntry,
            self.citeCountryEntry,
            self.thesisInstitutionEntry,
            self.confTitleText,
        ]
        ...
        editGetCallbacks = [
            ...
            self.getCiteIssue,
            self.getCiteVolume,
            self.getCiteFirstPage,
            self.getCiteLastPage,
            self.getCiteStatus,
            self.getCitePubMed,
            self.getCiteDoi,
            self.getCiteKeywords,
            self.getCiteDetails,
            self.getBookTitle,
            self.getBookSeries,
            self.getBookPublisher,
            self.getCiteCity,
            self.getCiteCountry,
            self.getThesisInst,
            self.getConfTitle,
        ...
        editSetCallbacks = [
            ...
            self.setCiteIssue,
            self.setCiteVolume,
            self.setCiteFirstPage,
            self.setCiteLastPage,
            self.setCiteStatus,
            self.setCitePubMed,
            self.setCiteDoi,
            self.setCiteKeywords,
            self.setCiteDetails,
            self.setBookTitle,
            self.setBookSeries,
            self.setBookPublisher,
            self.setCiteCity,
            self.setCiteCountry,
            self.setThesisInst,
            self.setConfTitle,
        ...
        self.citationTable = ScrolledMatrix(
            frameB,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        self.softTasksMulti = MultiWidget(self, Entry, minRows=1, callback=self.setSoftTasks)
        ...
        editWidgets = [
            ...
            None,
            self.softTasksMulti,
            self.softVendorEntry,
            self.softVendorAddrEntry,
            self.softVendorWebEntry,
            self.softDetailsEntry,
        ...
        editGetCallbacks = [
            ...
            self.getSoftTasks,
            self.getSoftVendor,
            self.getSoftVendorAddr,
            self.getSoftVendorWeb,
            self.getSoftDetails,
            self.getSoftMethod,
        ]
        ...
        editSetCallbacks = [
            ...
            self.setSoftTasks,
            self.setSoftVendor,
            self.setSoftVendorAddr,
            self.setSoftVendorWeb,
            self.setSoftDetails,
            self.setSoftMethod,
        ]
        ...
        self.softwareTable = ScrolledMatrix(
            frameB,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            editWidgets=editWidgets,
        ...
        tipTexts = ["Add a new software and version of the software", "Remove the currently selected software"]
        ...
        tipTexts = [
            "Type of NMR data such as shift lists, peak lists, relaxation data, etc.",
        ...
        self.nmrListsTable = ScrolledMatrix(
            frameC,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Rapidly select all available data lists for deposition to PDB/BMRB",
        ...
        tipTexts = [
            "Indicate the type and name of each list, based on list type selectd in the top frame",
        ...
        self.nmrListSelectTable = ScrolledMatrix(
            frameC,
            multiSelect=True,
            headingList=headingList,
            tipTexts=tipTexts,
        ...
        texts = ["Select All", "Select", "Unselect", "Unselect All"]

        tipTexts = [
            "Select all lists of the selected data type",
        ...
        commands = [
            self.submitAllNmrLists,
            self.submitSelectedNmrList,
            self.withdrawSelectedNmrList,
            self.withdrawAllNmrLists,
        ]
        ...
        self.isotopePulldown = PulldownList(self, texts=STANDARD_ISOTOPES, callback=self.setShiftRefIsotope)

        self.molNamePulldown = PulldownList(self, texts=SHIFT_REF_COMPOUNDS, callback=self.setShiftRefMolName)
        ...
        self.unitPulldown = PulldownList(self, texts=STANDARD_UNITS, callback=self.setShiftRefUnit)
        ...
        editWidgets = [
            None,
            None,
            self.isotopePulldown,
            None,
            self.molNamePulldown,
            self.atomGroupEntry,
            self.valueEntry,
            self.unitPulldown,
            None,
        ...
        editGetCallbacks = [
            None,
            None,
            self.getShiftRefIsotope,
            None,
            self.getShiftRefMolName,
            self.getShiftRefAtomGroup,
            self.getShiftRefValue,
            self.getShiftRefUnit,
            self.toggleShiftRefType,
            self.getShiftRefRatio,
            self.getShiftRefGeometry,
            self.getShiftRefLocation,
            self.getShiftRefAxis,
        ]

        editSetCallbacks = [
            None,
            None,
            self.setShiftRefIsotope,
            None,
            self.setShiftRefMolName,
            self.setShiftRefAtomGroup,
            self.setShiftRefValue,
            self.setShiftRefUnit,
            None,
            self.setShiftRefRatio,
            self.setShiftRefGeometry,
            self.setShiftRefLocation,
            self.setShiftRefAxis,
        ]

        self.shiftRefTable = ScrolledMatrix(
            frameD,
            multiSelect=True,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Add standard IUPAC chemical shift references for 1H, 13C, 15N and 31P",
        ...
        colHeadings = ["#", "Isotope", "Resonance", "PPM", "SD", "BMRB\nMean", "Random\nCoil"]
        ...
        self.shiftCheckTable = ScrolledMatrix(
            frameD, multiSelect=False, headingList=colHeadings, tipTexts=tipTexts, callback=None
        )
        ...
        tipTexts = [
            "Code identifier for this chain",
        ...
        self.molSysTable = ScrolledMatrix(
            frameE,
            multiSelect=False,
            headingList=headingList,
            tipTexts=tipTexts,
        ...
        editWidgets = [
            ...
            None,
            None,
            self.moleculeDbNamePulldown,
            self.moleculeDbRefEntry,
            self.moleculeDbRefDetailsEntry,
            self.moleculeDomainEntry,
            self.moleculeECNumMulti,
            self.moleculeMutEntry,
            self.moleculeDetailsEntry,
        ]
        editGetCallbacks = [
            None,
            None,
            ...
            None,
            None,
            self.getMoleculeDbName,
            self.getMoleculeDbRef,
            self.getMoleculeDbRefDetails,
            self.getMoleculeDomain,
            self.getMoleculeECnum,
            self.getMoleculeMutation,
        ...
        editSetCallbacks = [
            ...
            None,
            None,
            self.setMoleculeDbName,
            self.setMoleculeDbRef,
            self.setMoleculeDbRefDetails,
            self.setMoleculeDomain,
            self.setMoleculeECnum,
            self.setMoleculeMutation,
            self.setMoleculeDetails,
        ]

        self.moleculeTable = ScrolledMatrix(
            frameE,
            multiSelect=False,
            headingList=headingList,
            tipTexts=tipTexts,
        ...
        self.unusualChemCompTable = ScrolledMatrix(
            frameE, multiSelect=False, headingList=headingList, tipTexts=tipTexts, callback=None
        )
        ...
        editWidgets = [
            self.expHostSciNamePulldown,
            self.expHostNameEntry,
            None,
            self.expHostProdMethodPulldown,
            self.expHostStrainEntry,
            self.expHostVecTypePulldown,
            self.expHostVecNameEntry,
            self.expHostCellLineEntry,
            self.expHostVariantEntry,
            self.expHostDetailsEntry,
        ]
        editGetCallbacks = [
            self.getExpHostSciName,
            self.getExpHostName,
            None,
            self.getExpHostProdMethod,
            self.getExpHostStrain,
            self.getExpHostVecType,
            self.getExpHostVecName,
            self.getExpHostCellLine,
            self.getExpHostVariant,
            self.getExpHostDetails,
        ]
        editSetCallbacks = [
            self.setExpHostSciName,
            self.setExpHostName,
            None,
            self.setExpHostProdMethod,
            self.setExpHostStrain,
            self.setExpHostVecType,
            self.setExpHostVecName,
            self.setExpHostCellLine,
            self.setExpHostVariant,
        ...
        tipTexts = [
            "The standard scientific name for the source organism of this molecule - choose from a list of standard recombinant organisms or 'synthetic'",
        ...
        self.expSourceTable = ScrolledMatrix(
            frameF,
            headingList=headingList,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = [
            "Adds experimental and natural sources for all polymers in the molecular system",
        ...
        editWidgets = [
            self.natSourceSciNamePulldown,
            self.natSourceNameEntry,
            None,
            self.natSourceOrgTypePulldown,
            self.natSourceStrainEntry,
            self.natSourceVariantEntry,
            self.natSourceGeneNameEntry,
            self.natSourceCellLineEntry,
            self.natSourceATCCNumEntry,
            self.natSourceOrganEntry,
            self.natSourceTissueEntry,
            self.natSourceCellTypeEntry,
            self.natSourceOrganelleEntry,
            self.natSourceDetailsEntry,
        ]
        editGetCallbacks = [
            self.getNatSourceSciName,
            self.getNatSourceName,
            None,
            self.getNatSourceOrgType,
            self.getNatSourceStrain,
            self.getNatSourceVariant,
            self.getNatSourceGeneName,
            self.getNatSourceCellLine,
            self.getNatSourceATCCNum,
            self.getNatSourceOrgan,
            self.getNatSourceTissue,
            self.getNatSourceCellType,
            self.getNatSourceOrganelle,
            self.getNatSourceDetails,
        ]
        editSetCallbacks = [
            self.setNatSourceSciName,
            self.setNatSourceName,
            None,
            self.setNatSourceOrgType,
            self.setNatSourceStrain,
            self.setNatSourceVariant,
            self.setNatSourceGeneName,
            self.setNatSourceCellLine,
            self.setNatSourceATCCNum,
            self.setNatSourceOrgan,
            self.setNatSourceTissue,
            self.setNatSourceCellType,
            self.setNatSourceOrganelle,
        ...
        tipTexts = [
            "The standard scientific name for the biological or natural source of this molecule - choose from a list of the most common organisms in the PDB, which will also set the common name",
        ...
        self.natSourceTable = ScrolledMatrix(
            frameF,
            headingList=headingList,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = [
            "Adds natural sources for all polymers in the molecular system (these are usually added when experimental sources are added)",
        ...
        self.sampleAmountEntry = FloatEntry(self, text="", width=12, returnCallback=self.setSampleAmount)

        self.sampleUnitPulldown = PulldownList(self, texts=SAMPLE_UNITS, callback=self.setSampleUnit)

        self.sampleIonicEntry = FloatEntry(self, text="", width=12, returnCallback=self.setSampleIonic)

        self.samplePhEntry = FloatEntry(self, text="", width=12, returnCallback=self.setSamplePh)

        self.sampleSolventPulldown = PulldownList(self, callback=self.setSampleSolvent)

        self.sampleDetailsEntry = Entry(self, text="", width=10, returnCallback=self.setSampleDetails)
        ...
        editWidgets = [
            None,
            None,  # self.sampleStatePulldown,
            # self.sampleAmountEntry, self.sampleUnitPulldown,
            # self.sampleIonicEntry, self.samplePhEntry,
            self.sampleSolventPulldown,
            self.sampleDetailsEntry,
        ]

        editGetCallbacks = [
            None,
            None,  # self.getSampleState,
            # self.getSampleAmount, self.getSampleUnit,
            # self.getSampleIonic, self.getSamplePh,
            self.getSampleSolvent,
            self.getSampleDetails,
        ]

        editSetCallbacks = [
            None,
            None,  # self.setSampleState,
            # self.setSampleAmount, self.setSampleUnit,
            # self.setSampleIonic, self.setSamplePh,
            self.setSampleSolvent,
            self.setSampleDetails,
        ]

        self.sampleTable = ScrolledMatrix(
            frameG,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Add a new sample to the CCPN project",
        ...
        self.sampleCompConcEntry = FloatEntry(
            self, text="", width=12, formatPlaces=7, returnCallback=self.setSampleCompConc
        )

        self.sampleCompConcErrEntry = FloatEntry(
            self, text="", width=12, formatPlaces=7, returnCallback=self.setSampleCompConcErr
        )

        self.sampleCompConcUnitPulldown = PulldownList(self, texts=SAMPLE_COMP_UNITS, callback=self.setSampleUnit)

        headingList = ["Molecular Component", "Isotopic Labelling", "Concentration", "Error", "Unit"]

        tipTexts = [
            "Common molecular name of the component from the sample selected in the top frame",
        ...
        editWidgets = [
            None,
            self.sampleCompIsotopePulldown,
            self.sampleCompConcEntry,
            self.sampleCompConcErrEntry,
            self.sampleCompConcUnitPulldown,
        ]

        editGetCallbacks = [
            None,
            self.getSampleCompIsotope,
            self.getSampleCompConc,
            self.getSampleCompConcErr,
            self.getSampleCompConcUnit,
        ]

        editSetCallbacks = [
            None,
            self.setSampleCompIsotope,
            self.setSampleCompConc,
            self.setSampleCompConcErr,
            self.setSampleCompConcUnit,
        ]

        self.sampleComponentTable = ScrolledMatrix(
            frameG,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            ...
            headingList=headingList,
            tipTexts=tipTexts,
            callback=self.selectSampleComponent,
            grid=(4, 0),
        )
        ...
        tipTexts = [
            "Add the names of all polymeric components from your molecular system",
        ...
        self.sampleConditionSetEntry = Entry(self, text="", width=10, returnCallback=self.setSampleConditionSet)

        self.sampleConditionSetDetailsEntry = Entry(
            self, text="", width=10, returnCallback=self.setSampleConditionSetDetails
        )
        ...
        self.nmrCondSetTable = ScrolledMatrix(
            frameH,
            headingList=headingList,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = [
            "Add a new condition set to the CCPN project",
        ...
        self.sampleConditionValueEntry = FloatEntry(
            self, text="", width=12, returnCallback=self.setSampleConditionValue
        )

        self.sampleConditionErrorEntry = FloatEntry(
            self, text="", width=12, returnCallback=self.setSampleConditionError
        )

        self.sampleConditionUnitEntry = Entry(self, text="", width=10, returnCallback=self.setSampleConditionUnit)

        headingList = ["Condition", "Value", "Error", "Unit"]

        tipTexts = [
            "The name of the variable used to define a specific sample condition - for example, temperature, pH, etc.",
        ...
        editWidgets = [
            None,
            self.sampleConditionValueEntry,
            self.sampleConditionErrorEntry,
            self.sampleConditionUnitEntry,
        ]

        editGetCallbacks = [
            None,
            self.getSampleConditionValue,
            self.getSampleConditionError,
            self.getSampleConditionUnit,
        ]

        editSetCallbacks = [
            None,
            self.setSampleConditionValue,
            self.setSampleConditionError,
            self.setSampleConditionUnit,
        ]

        self.nmrCondTable = ScrolledMatrix(
            frameH,
            headingList=headingList,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = [
            "This button adds the four standard condtions: temperature, pH, pressure and ionic strength",
        ...
        self.spectrometerNameEntry = Entry(self, text="", width=10, returnCallback=self.setSpectrometerName)

        self.spectrometerFreqEntry = FloatEntry(self, text="", width=12, returnCallback=self.setSpectrometerFreq)
        ...
        self.spectrometerSerialEntry = Entry(self, text="", width=10, returnCallback=self.setSpectrometerSerial)

        self.spectrometerDetailsEntry = Entry(self, text="", width=10, returnCallback=self.setSpectrometerDetails)
        ...
        editWidgets = [
            None,
            self.spectrometerNameEntry,
            None,
            self.spectrometerFreqEntry,
        ...
        editGetCallbacks = [
            None,
            self.getSpectrometerName,
            None,
            self.getSpectrometerFreq,
        ...
        editSetCallbacks = [
            None,
            self.setSpectrometerName,
            None,
            self.setSpectrometerFreq,
        ...
        self.spectrometerMatrix = ScrolledMatrix(
            frameI,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            editWidgets=editWidgets,
        ...
        tipTexts = [
            "Add a new NMR spectrometer specification to the CCPN project",
        ...
        self.probeNameEntry = Entry(self, text="", width=10, returnCallback=self.setProbeName)

        self.probeTypePulldown = PulldownList(self, texts=NMR_PROBE_TYPES, callback=self.setProbeType)

        self.probeManufacturerEntry = Entry(self, text="", width=10, returnCallback=self.setProbeManufacturer)

        self.probeModelEntry = Entry(self, text="", width=10, returnCallback=self.setProbeModel)

        self.probeSerialEntry = Entry(self, text="", width=10, returnCallback=self.setProbeSerial)

        self.probeDiameterEntry = FloatEntry(self, text=0.0, width=10, returnCallback=self.setProbeDiameter)

        self.probeDetailsEntry = Entry(self, text="", width=10, returnCallback=self.setProbeDetails)
        ...
        editWidgets = [
            None,
            self.probeNameEntry,
            self.probeTypePulldown,
            self.probeManufacturerEntry,
            self.probeModelEntry,
            self.probeSerialEntry,
            self.probeDiameterEntry,
            self.probeDetailsEntry,
        ]
        ...
        editGetCallbacks = [
            ...
            self.getProbeName,
            self.getProbeType,
            self.getProbeManufacturer,
            self.getProbeModel,
            self.getProbeSerial,
            self.getProbeDiameter,
            self.getProbeDetails,
        ]
        ...
        editSetCallbacks = [
            ...
            self.setProbeName,
            self.setProbeType,
            self.setProbeManufacturer,
            self.setProbeModel,
            self.setProbeSerial,
            self.setProbeDiameter,
            self.setProbeDetails,
        ]

        self.probeMatrix = ScrolledMatrix(
            frameI,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            editWidgets=editWidgets,
        ...
        tipTexts = [
            "Add a new NMR probe specification to the CCPN project",
        ...
        self.experimentNameEntry = Entry(self, text="", width=10, returnCallback=self.setExperimentName)
        ...
        self.expShiftRefSelect = MultiWidget(
            self, CheckButton, callback=self.setExpShiftRefs, minRows=0, useImages=False
        )
        ...
        editWidgets = [
            self.experimentNameEntry,
            None,
            None,
            self.experimentStatePulldown,
            self.experimentSamplePulldown,
            self.experimentSampleCondPulldown,
            self.expShiftRefSelect,
            None,
            self.experimentSpecPulldown,
            self.experimentProbePulldown,
            self.expChemShiftListPulldown,
        ]
        editGetCallbacks = [
            self.getExperimentName,
            self.toggleExperiment,
            None,
            self.getExperimentState,
            self.getExperimentSample,
            self.getExperimentSampleCond,
            self.getExpShiftRefs,
            None,
            self.getExperimentSpectrometer,
            self.getExperimentProbe,
        ...
        editSetCallbacks = [
            ...
            None,
            None,
            self.setExperimentState,
            self.setExperimentSample,
            self.setExperimentSampleCond,
            self.setExpShiftRefs,
            None,
            self.setExperimentSpectrometer,
            self.setExperimentProbe,
            self.setExpChemShiftList,
        ]

        self.experimentTable = ScrolledMatrix(
            frameJ,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Add a new NMR experiment record to the CCPN project",
        ...
        commands = [self.addExperiment, self.removeExperiment, self.submitAllExperiments, self.submitMinExperiments]
        ...
        editWidgets = [
            ...
            self.strucGenDetailsEntry,
            self.strucGenSoftwarePulldown,
            self.strucEnsPulldown,
        ...
        editGetCallbacks = [
            ...
            self.getStrucGenDetails,
            self.getStrucGenSoftware,
            self.getStrucEns,
        ...
        editSetCallbacks = [
            ...
            self.setStrucGenDetails,
            self.setStrucGenSoftware,
            self.setStrucEns,
        ...
        tipTexts = [
            "The serial number of the structure generation",
        ...
        self.strucGenTable = ScrolledMatrix(
            frameK,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Add a new structure generation to the CCPN project",
        ...
        editWidgets = [
            None,
            self.strucEnsDetailsEntry,
            None,
            None,
            None,
            self.strucEnsCalcEntry,
            self.strucEnsCriteriaEntry,
            self.strucEnsReprEntry,
            self.strucEnsReprCriEntry,
        ]

        editGetCallbacks = [
            None,
            self.getStrucEnsDetails,
            None,
            None,
            None,
            self.getStrucEnsCalc,
            self.getStrucEnsCriteria,
            self.getStrucEnsRepr,
            self.getStrucEnsReprCri,
        ]

        editSetCallbacks = [
            None,
            self.setStrucEnsDetails,
            None,
            None,
            None,
            self.setStrucEnsCalc,
            self.setStrucEnsCriteria,
            self.setStrucEnsRepr,
        ...
        tipTexts = [
            "The serial number of the structure ensemble",
        ...
        self.strucEnsTable = ScrolledMatrix(
            frameK,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "The serial number of the constraint list",
        ...
        self.constraintListsTable = ScrolledMatrix(
            frameK,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("", "__init__", "delete"):
            for clazz in (
                "ccp.general.Citation.JournalCitation",
                "ccp.general.Citation.BookCitation",
                "ccp.general.Citation.ThesisCitation",
                "ccp.general.Citation.ConferenceCitation",
            ):
        ...
        for func in ("", "__init__", "delete", "setShiftReferences", "addShiftReference", "removeShiftReference"):
    ...
    def updateEntries(self):
        ...
        names = []
        index = []
        store = self.project.currentNmrEntryStore or self.project.newNmrEntryStore(name="eciDefault")
    ...
    def updateReport(self):
        ...
        self.reportTable.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def setCoordDate(self, text):
        ...
        if self.entry and nmrStarText:
            ...
            if appData:
                ...
                keywds = {"application": "nmrStar", "keyword": "depRelCoord", "value": nmrStarText}
    ...
    def setRestrDate(self, text):
        ...
        if self.entry and nmrStarText:
            ...
            if appData:
                ...
                keywds = {"application": "nmrStar", "keyword": "depRelConstr", "value": nmrStarText}
    ...
    def setNmrDate(self, text):
        ...
        if self.entry and nmrStarText:
            ...
            if appData:
                ...
                keywds = {"application": "nmrStar", "keyword": "depRelNmr", "value": nmrStarText}
    ...
    def setSeqDate(self, text):
        ...
        if self.entry and nmrStarText:
            ...
            if appData:
                ...
                keywds = {"application": "nmrStar", "keyword": "depRelSeq", "value": nmrStarText}
    ...
    def updateNmrLists(self, entry=None):
        ...
        self.nmrListsTable.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
    ...
    def updateNmrListSelectTable(self):
        ...
        if self.entry:
            for obj in getNmrLists(self.nmrProject, self.entryObjType):
                ...
                datum = [getNmrListName(obj), obj.name, size, use, softName]
        ...
        self.nmrListSelectTable.update(
    ...
    def updatePeople(self):
        ...
        self.personTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def setPersonAsPrimary(self):
        ...
        keywds = {"application": "AutoDep", "keyword": "Primary", "value": "Yes"}
    ...
    def updateGroups(self):
        ...
        if entry:
            for i, group in enumerate(entry.sortedLaboratories()):
                ...
                datum = [group.name, addresses, city, stateProv, postalCode, country, orgType]

                objectList.append(group)
                textMatrix.append(datum)

        self.groupTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def addGroup(self):
        ...
        if gpName:
            organisation = affStore.findFirstOrganisation(name=gpName) or affStore.newOrganisation(name=gpName)

            group = organisation.findFirstGroup(name=gpName) or organisation.newGroup(name=gpName)
    ...
    def setStateProv(self, event):
        ...
        if self.group:
            ...
            if text:
                keywds = {"application": "nmrStar", "keyword": "State", "value": text}
    ...
    def updateAddresses(self):
        ...
        if entry:
            for i, contactPerson in enumerate(entry.sortedContactPersons()):
                ...
                datum = [contactPerson.familyName, contactPerson.givenName, email, phoneNumber, faxNumber, position]

                objectList.append(contactPerson)
                textMatrix.append(datum)

        self.addressTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def getShiftRefMolName(self, shiftReference):
        ...
        names = SHIFT_REF_COMPOUNDS + [
            "<Other>",
        ]
        objects = SHIFT_REF_COMPOUNDS + [
            None,
        ]
    ...
    def addStdIupacRef(self):
        ...
        if self.nmrProject:
            ...
            appKeywds = {"application": "nmrStar", "keyword": "refType", "value": "n/a"}
    ...
    def addInternalShiftRef(self):
        ...
        if self.nmrProject:
            ...
            self.shiftReference = newRef(
                isotopeCode="1H", molName="TSP", atomGroup="H", value=0.000, referenceType="direct"
            )
    ...
    def updateShiftCheck(self):
        ...
        self.shiftCheckTable.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def setMoleculeDomain(self, event):
        ...
        if self.molecule:
            ...
            if text != "":
                keywds = {"application": "nmrStar", "keyword": "Fragment", "value": text}
    ...
    def setMoleculeMutation(self, event):
        ...
        if self.molecule:
            ...
            if text != "":
                keywds = {"application": "nmrStar", "keyword": "Mutation", "value": text}
    ...
    def setMoleculeDbRef(self, event):
        ...
        if self.molecule:
            ...
            keywds = {"application": "nmrStar", "keyword": "authDbAcc", "value": "yes"}
    ...
    def updateChains(self):
        ...
        self.molSysTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateMolecules(self):
        ...
        self.moleculeTable.update(textMatrix=textMatrix, objectList=objectList)
        ...
        for chemComp in chemComps:
            datum = [chemComp.ccpCode, chemComp.code3Letter, chemComp.molType, chemComp.name]

            objectList.append(chemComp)
            textMatrix.append(datum)

        self.unusualChemCompTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateSources(self):
        ...
        if self.entry:
            ...
            self.expSourceTable.update(textMatrix=textMatrix, objectList=objectList)
            ...
            self.natSourceTable.update(textMatrix=textMatrix, objectList=objectList)

        else:
            self.expSourceTable.update(textMatrix=[], objectList=[])

            self.natSourceTable.update(textMatrix=[], objectList=[])
        self.waiting = False
...
"""
            ...
            if not entryMolecule:
                ...
                entryMolecule = entry.newEntryMolecule(molecule=molecule, productionMethod=prodMethod)
    ...
    def setExpHostMol(self, event):
        ...
        if entryMolecule:
            ...
            if molecule:
                ...
                entryMolecule = self.entry.newEntryMolecule(
                    molecule=molecule,
    ...
    def addNatSources(self):
        ...
        for molecule in molecules:
            ...
            natSource = tax.newNaturalSource(organismName=UNDEFINED, scientificName=UNDEFINED)
            molecule.naturalSource = natSource
    ...
    def getExpShiftRefs(self, experiment):
        ...
        for shiftReference in shiftRefs:
            data = (shiftReference.serial, shiftReference.isotopeCode, shiftReference.molName)
    ...
    def updateExperiments(self):
        ...
        if self.entry and self.nmrProject:
            ...
            for experiment in self.nmrProject.sortedExperiments():
                ...
                for shiftReference in experiment.sortedShiftReferences():
                    data = (shiftReference.isotopeCode, shiftReference.molName)
        ...
        self.experimentTable.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
  ...
  """
    ...
    def setSampleSolvent(self, event):
        ...
        if self.sample:
            ...
            if solventSys:
                keywds = {"application": "nmrStar", "keyword": "solventSys", "value": solventSys}
  ...
  """
    ...
    def updateSamples(self, obj=None):
        ...
        self.sampleTable.update(textMatrix=textMatrix, objectList=objectList)
...
"""
            ...
            if not molComp:
                ...
                while refSampleCompStore.findFirstComponent(className="MolComponent", name=name):
                    nn += 1
                    name = name0[:72] + " " + nn
                molComp = refSampleCompStore.newMolComponent(name=name, details=mol.name, molecule=mol)
    ...
    def addOtherSampleComponent(self):
        ...
        molComp = refSampleCompStore.newMolComponent(
            name=sampCompMol + ":" + self.sample.name,  # TODO: better?
            details=sampCompMol,
        )
    ...
    def updateSampleComponents(self, obj=None):
        ...
        self.sampleComponentTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateSampleConditionSets(self, obj=None):
        ...
        if self.project and self.nmrProject:
            for scs in self.nmrProject.sortedSampleConditionSets():
                ...
                datum = [scs.serial, scs.name, scs.details]

                objectList.append(scs)
                textMatrix.append(datum)

        self.nmrCondSetTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateSampleConditions(self, obj=None):
        ...
        if self.project and self.nmrProject:
            for sc in self.sampleConditionSet.sortedSampleConditions():
                datum = [sc.condition, sc.value, sc.error, sc.unit]

                objectList.append(sc)
                textMatrix.append(datum)

        self.nmrCondTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def addStdSampleCondition(self):
        ...
        for cType, cVal, cUnit in stdSampleConditions:
            ...
            self.sampleCondition = self.sampleConditionSet.newSampleCondition(condition=cType, value=cVal, unit=cUnit)
    ...
    def updateStrucGens(self):
        ...
        self.strucGenTable.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def updateStrucEns(self):
        ...
        self.strucEnsTable.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def setStrucEnsCalc(self, event):
        ...
        if self.ensemble:
            ...
            if text:
                keywds = {"application": "nmrStar", "keyword": "calculated", "value": text}
    ...
    def setStrucEnsCriteria(self, event):
        ...
        if self.ensemble:
            ...
            if text:
                keywds = {"application": "nmrStar", "keyword": "criteria", "value": text}
    ...
    def setStrucEnsRepr(self, event):
        ...
        if self.ensemble:
            ...
            if text:
                keywds = {"application": "nmrStar", "keyword": "representative", "value": text}
    ...
    def setStrucEnsReprCri(self, event):
        ...
        if self.ensemble:
            ...
            if text:
                keywds = {"application": "nmrStar", "keyword": "repr_criteria", "value": text}
    ...
    def updateConstraintLists(self):
        ...
        if not self.constSet and self.nmrProject:
            self.constSet = (
                getattr(self.project, "currentNmrConstraintStore")
                or getattr(self.project, "findFirstNmrConstraintStore")(nmrProject=self.nmrProject)
                or getattr(self.project, "newNmrConstraintStore")(nmrProject=self.nmrProject)
            )
        ...
        if self.entry and self.constSet:
            for constList in self.constSet.sortedConstraintLists():
                ...
                datum = [constList.serial, constType, constList.name, constList.details, len(constList.constraints)]

                objectList.append(constList)
                textMatrix.append(datum)
                colorMatrix.append(colors)

        self.constraintListsTable.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def updateCitations(self, obj=None):
        ...
        self.citationTable.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def updateSoftware(self, obj=None):
        ...
        self.softwareTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def exportNmrStar31(self):
        ...
        fileTypes = [FileType("STAR", ["*.str"]), FileType("All", ["*"])]

        fileSelectPopup = FileSelectPopup(
            self,
            file_types=fileTypes,
            title="Export NMR-STAR 3.1 file",
            dismiss_text="Cancel",
    ...
    def exportPdb(self):
        ...
        fileTypes = [FileType("PDB", ["*.pdb"]), FileType("PDB Entry", ["*.ent"]), FileType("All", ["*"])]

        fileSelectPopup = FileSelectPopup(
            self,
            file_types=fileTypes,
            title="Export PDB 3.20 file",
            dismiss_text="Cancel",
            selected_file_must_exist=False,
            multiSelect=False,
        )
    ...
    def exportCnsDistance(self):
        ...
        fileTypes = [FileType("CNS", ["*.tbl"]), FileType("All", ["*"])]

        fileSelectPopup = FileSelectPopup(
            self,
            file_types=fileTypes,
            title="Export CNS file",
            dismiss_text="Cancel",
            selected_file_must_exist=False,
            multiSelect=False,
        )
        ...
        if fileName:
            ...
            cnsObj.writeDistanceConstraints(
                fileName, constraintList=distConstList, resetMapping=False, minimalPrompts=True, verbose=True
            )
...
def getTopLevelStore(memopsRoot, className):
    ...
    store = (
        getattr(memopsRoot, "current" + className)
        or getattr(memopsRoot, "findFirst" + className)()
        or getattr(memopsRoot, "new" + className)(name="eciDefault")
    )
...
def getTopLevelStore2(memopsRoot, className):
    ...
    store = (
        getattr(memopsRoot, "current" + className)
        or getattr(memopsRoot, "findFirst" + className)()
        or getattr(memopsRoot, "new" + className)(namingSystem="eciDefault")
    )
...
def getNewStructureGeneration(ensemble, nmrProject):
    ...
    strucGen = nmrProject.newStructureGeneration(generationType=GENERATION_TYPES[1], name="eciDefault")
...
"""
        ...
        if not expSource:
            tax = getTaxonomy(entry.root)
            expSource = tax.findFirstNaturalSource(molecules=[]) or tax.newNaturalSource(
                scientificName=orgName, organismName=orgName
            )
...
def getPersonName(person):
    ...
    data = (person.title, person.givenName, " ".join(person.middleInitials), person.familyName, person.familyTitle)
