...
class ReadPdb:
    valueUnitPatt = re.compile(r"(\d+\.?\d*)\s*([A-Za-z]+)")
    chainCodePatt = re.compile(r"([Cc][Hh][Aa][Ii][Nn])?\:?\s+([A-Za-z])")

    softVersPatt = re.compile(r"^(.+?)\s([\d\.]+)$")

    multiSpacePatt = re.compile(r"\s{2,}")
    emptyLinePatt = re.compile(r"^(\s*)$")

    lineWithColonPatt = re.compile("^.*:.*")
    lineWithSemiColonPatt = re.compile("^.*;.*")

    concPatt = re.compile(r"([0-9\.]+)\s*(.*)")

    fieldPatt = re.compile(r"([0-9\.]+)\s*(.*)")

    unpPatt = re.compile("^[A-Z0-9]{5,6}$")  # TODO - what is this in reality
    pdbPatt = re.compile("^[0-9][A-Z0-9]{3}$")

    mutationPatt = re.compile(r"YES \[(\S+)]")

    remarkTypeToId = {"REFINEMENT.": "3", "EXPERIMENTAL DETAILS": "210"}
    ...
    expPatts = {
        "expType": re.compile(r"^EXPERIMENT TYPE\s+: (.+)$"),
        "expTemp": re.compile(r"^TEMPERATURE\s+\(KELVIN\) : (.+)$"),
        "expPh": re.compile(r"^PH\s+: (.+)$"),
        "expIon": re.compile(r"^IONIC STRENGTH\s+: (.+)$"),
        "expPress": re.compile(r"^PRESSURE\s+: (.+)$"),
        "expSample": re.compile(r"^SAMPLE CONTENTS\s+: (.+)$"),
        "nmrExpts": re.compile(r"^NMR EXPERIMENTS CONDUCTED\s+: (.+)$"),
        "specField": re.compile(r"^SPECTROMETER FIELD STRENGTH\s+: (.+)$"),
        "specModel": re.compile(r"^SPECTROMETER MODEL\s+: (.+)$"),
        "specManu": re.compile(r"^SPECTROMETER MANUFACTURER\s+: (.+)$"),
        "strucInfo": re.compile(r"^STRUCTURE DETERMINATION.\s*$"),
        "softType": re.compile(r"^SOFTWARE USED\s+: (.+)$"),
        "methType": re.compile(r"^METHOD USED\s+: (.+)$"),
        "confCalc": re.compile(r"^CONFORMERS, NUMBER CALCULATED\s+: (.+)$"),
        "confSub": re.compile(r"^CONFORMERS, NUMBER SUBMITTED\s+: (.+)$"),
        "confSel": re.compile(r"^CONFORMERS, SELECTION CRITERIA\s+: (.+)$"),
        "confBest": re.compile(r"^BEST REPRESENTATIVE CONFORMER IN THIS ENSEMBLE\s+: (.+)$"),
    ...
    def setNmrProject(self):
        ...
        self.nmrProject = self.ccpnProject.currentNmrProject = self.getTopLevelStore(
            self.ccpnProject, "NmrProject", defaultName=self.entryName
        )
    ...
    def setStructureGeneration(self):
        ...
        if not self.strucGen:
            ...
            self.strucGen = self.nmrProject.newStructureGeneration(
                name=sgTitle, generationType="refinement", nmrConstraintStore=self.nmrConstraintStore
            )  # NEW
    ...
    def setCcpnStores(self):
        ...
        self.ccpnProject.currentAffiliationStore = self.getTopLevelStore(
            self.ccpnProject, "AffiliationStore", defaultName=self.entryName
        )

        self.ccpnProject.currentCitationStore = self.getTopLevelStore(
            self.ccpnProject, "CitationStore", defaultName=self.entryName
        )

        self.ccpnProject.currentTaxonomy = self.getTopLevelStore(
            self.ccpnProject, "Taxonomy", defaultName=self.entryName
        )

        self.ccpnProject.currentSampleStore = self.getTopLevelStore(
            self.ccpnProject, "SampleStore", defaultName=self.entryName
        )

        self.ccpnProject.currentMethodStore = self.getTopLevelStore(
            self.ccpnProject, "MethodStore", defaultName=self.entryName
        )

        self.ccpnProject.currentClassification = self.getTopLevelNSStore(
            self.ccpnProject, "Classification", defaultNamingSystem=self.entryName
        )

        self.ccpnProject.currentInstrumentStore = self.getTopLevelStore(
            self.ccpnProject, "InstrumentStore", defaultName=self.entryName
        )

        self.ccpnProject.currentRefSampleComponentStore = self.getTopLevelStore(
            self.ccpnProject, "RefSampleComponentStore", defaultName=self.entryName
        )

        self.ccpnProject.currentNameMappingStore = self.getTopLevelStore(
            self.ccpnProject, "NameMappingStore", defaultName=self.entryName
        )

        self.ccpnProject.currentDatabase = self.getTopLevelStore(
            self.ccpnProject, "Database", defaultName=self.entryName
        )
    ...
    def readPdb(self, tkObj=None):
        ...
        self.pdbFormat.readSequence(
            self.pdbFile,
            molSystem=self.molSystem,
            verbose=True,
            minimalPrompts=True,
            chemCompPath=chemCompArchiveDataPath,
            numIdenticalChains=numIdenticalChains,
        )  # NEW

        if DEBUG:
            print("INP MOL_SYSTEM: [%s]" % self.molSystem)

        self.pdbFormat.readPeopleAndCitations(
            self.pdbFile, doNotMakeCcpnObjects=True, verbose=True, minimalPrompts=True
        )
    ...
    def setTitleAndKeywords(self):
        ...
        if "Keywds" in self.pdbDataFile.headerVars:
    ...
    def setMolInfo(self):
        ...
        for chainNum in range(len(chains)):
            ...
            if hasattr(pdbChain, "mutation") and pdbChain.mutation:
                ...
                keywds = {"application": self.format, "keyword": "Mutation", "value": mutation}
            ...
            if hasattr(pdbChain, "fragment") and pdbChain.fragment:
                keywds = {"application": self.format, "keyword": "Fragment", "value": pdbChain.fragment}
            ...
            if len(sourceInfo) >= chainNum:
                ...
                keywds = {"organismName": "n/a", "scientificName": "n/a"}
            ...
            if len(sourceInfo) >= 1 and sourceInfo[chainNum - 1]:
                ...
                keywds2 = {"organismName": "unknown", "scientificName": "unknown"}
    ...
    def setStructInfo(self):
        ...
        if self.ccpnEntry not in self.strucGen.sortedEntries():
    ...
    def setPeopleAndCitationInfo(self):
        ...
        for person in authors:
            ...
            if author not in self.ccpnEntry.authors:
        ...
        for citation in pdbCitations:
            ...
            for person in authors:
                ...
                if author not in keywds["authors"]:
            ...
            for person in editors:
                ...
                if editor not in keywds["editors"]:
    ...
    def setNmrExperiments(self):
        ...
        for i in range(numSamples):
            ...
            keywds = {"ph": ph, "ionicStrength": ionicStrength, "details": nmrCompoundConc}
            ...
            if not sample:
                sample = self.ccpnProject.currentSampleStore.newSample(
                    sampleCategories=[sampleCategory], **keywds
                )  # NEW - already in the project?
            ...
            if not appData and solventSys and solventSys != "NULL":
                keywds = {"application": self.format, "keyword": "solventSys", "value": solventSys}
            ...
            details = "pH [%s], temp [%s], pressure [%s], ionStrength [%s]" % (ph, temp, pressure, ionicStrength)
            ...
            if not sampleConditionSet:
                ...
                for valueName, value, defaultUnit in (
                    ("pH", str(ph), "pH"),
                    ...
                    newSamCond = sampleConditionSet.findFirstSampleCondition(
                        condition=valueName, value=value, unit=unit
                    )

                    if not newSamCond:
                        newSamCond = sampleConditionSet.newSampleCondition(
                            condition=valueName, value=value, unit=unit
                        )  # NEW
        ...
        """
            ...
            spectrometer = self.ccpnProject.currentInstrumentStore.findFirstInstrument(
                className="NmrSpectrometer", **keywds
            )
            ...
            for nmrExpName in nmrExpStrings:
                ...
                if self.molSystem not in nmrExp.sortedMolSystems():
                    nmrExp.addMolSystem(self.molSystem)  # ADD

                    if DEBUG:
                        print("ADD MOL_SYS: [%s] [%s]" % (nmrExp, self.molSystem))

                if self.ccpnEntry not in nmrExp.sortedEntries():
                    nmrExp.addEntry(self.ccpnEntry)  # ADD

                    if DEBUG:
                        print("ADD NMR_ENTRY: [%s] [%s]" % (nmrExp, self.ccpnEntry))

                if sample and scNewSamFlag and nmrExp not in sample.sortedNmrExperiments():
                    sample.addNmrExperiment(nmrExp)  # ADD

                    if DEBUG:
                        print("ADD SAMPLE: [%s] [%s]" % (nmrExp, sample))

                if sampleConditionSet and nmrExp not in sampleConditionSet.sortedExperiments():
        ...
        if self.strucGen:
            ...
            if confBest and confBest != "NULL":
                keywds = {"application": self.format, "keyword": "representative", "value": confBest}
            ...
            if confSel and confSel != "NULL":
                keywds = {"application": self.format, "keyword": "criteria", "value": confSel}
            ...
            if confCalc and confCalc != "NULL":
                keywds = {"application": self.format, "keyword": "calculated", "value": confCalc}
        ...
        for chain in self.molSystem.sortedChains():
            ...
            if len(molecule.molResidues) > 1 and molecule.isStdLinear:
                ...
                for sample in self.ccpnProject.currentSampleStore.sortedAbstractSamples():
                    molComp = self.ccpnProject.currentRefSampleComponentStore.findFirstComponent(
                        name=molName + ":" + sample.name
                    )
                    ...
                    if not molComp:
                        molComp = self.ccpnProject.currentRefSampleComponentStore.newMolComponent(
                            name=molName + ":" + sample.name, details=molName
                        )  # NEW
    ...
    def parseRefineLines(self, remarks):
        ...
        if self.remarkTypeToId[refineString] in remarks:
    ...
    def parseExpLines(self, remarks):
        ...
        if self.remarkTypeToId[expString] in remarks:
        ...
        for tperson in self.ccpnProject.currentAffiliationStore.sortedPersons():
            if tperson.familyName and tperson and tperson.familyName.upper() == familyName.upper():
                if (
                    ...
                    and firstNameGuess
                    and (
                        tperson.givenName.upper() == firstNameGuess.upper()
                        or tperson.givenName[0].upper() == firstNameGuess[0].upper()
                    )
                ) or tperson.firstInitial == firstNameGuess:
                    person = tperson
                    break
    ...
    def getTopLevelStore(self, memopsRoot, className, name=None, defaultName="readPdb"):
        ...
        if name is None:
            store = getattr(memopsRoot, "current" + className) or getattr(memopsRoot, "findFirst" + className)()
