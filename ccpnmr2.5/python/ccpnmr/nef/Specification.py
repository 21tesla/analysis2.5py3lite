...
def getCcpnSpecification(filePath):
    ...
    converter = CifDicConverter(open(filePath).read(), additionalBlocks=("ccpn_additions",))
    #
...
class CifDicConverter:
    ...
    def convertToNef(self):
        ...
        dataBlocks = [rcsbDataBlock] + list(self.specification.get(tag) for tag in self.additionalBlocks)
        if None in dataBlocks:
            ii = dataBlocks.index(None)
            raise ValueError("Specification file has no data block matching %s" % repr(self.additionalBlocks[ii - 1]))
    ...
    def extractGeneralDataFrame(self, rcsbDataBlock):
        ...
        transferLoop(
            rcsbDataBlock,
            saveFrame,
            ("_dictionary_history.version", "_dictionary_history.update", "_dictionary_history.revision"),
        )
        # ItemType loop
        typeLoop = transferLoop(
            rcsbDataBlock,
            saveFrame,
            (
    ...
    def extractSaveFrameDescription(self, inputSaveFrame):
        ...
        saveFrame.addItem("is_mandatory", inputSaveFrame.get("_category.mandatory_code") == "yes")

        saveFrame.addItem("description", inputSaveFrame.get("_category.description"))

        data = inputSaveFrame.multiColumnValues(
            (
                "_category_examples.detail",
                "_category_examples.case",
            )
        ...
        for tag in inputSaveFrame:
            if tag not in expectedTags:
                self._logging("Unexpected item in %s:" % inputSaveFrame["_category.id"], tag, inputSaveFrame.get(tag))
    ...
    def extractLoopDescription(self, inputSaveFrame):
        ...
        if parentCategory is None:
            ...
            if parent is None:
                self._logging(
                    "loop is missing parent saveFrame:", name, parentCategory, list(self._category2SaveFrame.keys())
                )
            else:
    ...
    def extractItemDescription(self, inputSaveFrame):
        ...
        if saveFrame is None:
            raise ValueError(
                "SAvFrame named %s not found in list: %s" % (category, list(self._category2SaveFrame.keys()))
            )
        ...
        if specificationLoop is None:
            specificationLoop = saveFrame.newLoop(
                "nef_item",
                (
                    "name",
                    "loop_category",
