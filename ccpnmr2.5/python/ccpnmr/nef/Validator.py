...
class Validator:
    ...
    def _validateAll(self, nef=None, validNef=None):
        ...
        for sf_name, saveframe in nef.items():
            if SF_FRAMECODE not in saveframe or saveframe.name != saveframe[SF_FRAMECODE]:
                e = self._validation_errors[SAVEFRAME]
                e += [
                    f"Saveframe.name for sf_framecode '{saveframe[SF_FRAMECODE]}' is not defined correctly.",
                ]
                break
            ...
            for vName, validFrame in validNef.items():
                ...
                if checkName and SF_CATEGORY in saveframe and saveframe[SF_CATEGORY] == checkName[0]:
                    ...
                    mandatoryFields = [
                        nm[NAME]
                        for nm in validFrame[NEF_ITEM].data
                        if nm[IS_MANDATORY] is True and nm[LOOP_CATEGORY] is None
                    ]
                    optionalFields = [
                        nm[NAME]
                        for nm in validFrame[NEF_ITEM].data
                        if nm[IS_MANDATORY] is False and nm[LOOP_CATEGORY] is None
                    ]
                    ...
                    e += self.__dict_nonallowed_keys(
                        saveframe, mandatoryFields + optionalFields + loopNames, label=sf_name
                    )
                    ...
                    for loop in loops:
                        ...
                        mandatoryLoopFields = [
                            nm[NAME]
                            for nm in validFrame[NEF_ITEM].data
                            if nm[IS_MANDATORY] is True and nm[LOOP_CATEGORY] == loop
                        ]
                        optionalLoopFields = [
                            nm[NAME]
                            for nm in validFrame[NEF_ITEM].data
                            if nm[IS_MANDATORY] is False and nm[LOOP_CATEGORY] == loop
                        ]

                        if saveframe[loop] and saveframe[loop].data:
                            # check for missing words/bad fields (keys)/malformed loops
                            e += self.__dict_missing_keys(
                                saveframe[loop].data[0], mandatoryLoopFields, label=f"{sf_name}:{loop}"
                            )
                            e += self.__dict_nonallowed_keys(
                                saveframe[loop].data[0],
                                mandatoryLoopFields + optionalLoopFields,
                                label=f"{sf_name}:{loop}",
                            )
                ...
                e = self._validation_errors[SAVEFRAME]
                e += [
                    f"No sf_category '{saveframe[SF_CATEGORY]}' found (possibly bad name defined).",
                ]
    ...
    def __dict_nonallowed_keys(self, dct, allowed_keys, label=None):
        if label is None:
            return [
                f"Field '{key}' not allowed."
                for key in dct.keys()
                if key not in allowed_keys and not key.startswith(CCPN_PREFIX)
            ]
        return [
            f"Field '{key}' not allowed in {label}."
            for key in dct.keys()
            if key not in allowed_keys and not key.startswith(CCPN_PREFIX)
        ]
