...
def defineArguments():
    ...
    parser.add_argument(
        ...
        dest="blockDirs",
        nargs=3,
        metavar=("inDir1", "inDir2", "outDir"),
        default=None,
        help="Batch mode: compare the contents of two directories",
    )
    parser.add_argument(
        "-s", "--screen", dest="screen", action="store_true", default=False, help="Output batch processing to screen"
    )
    parser.add_argument(
        "-o",
        "--overwrite",
    ...
    parser.add_argument(
        "-c", "--create", dest="createDirs", action="store_true", default=False, help="Create directories as required"
    )
    parser.add_argument(
        "-I",
        "--ignorecase",
        ...
        for compName in dSet:
            for rowIndex in range(rowRange):
                ...
                if not (
                    ...
                    or ((str(loopValue1).lower() == str(loopValue2).lower()) and options.ignoreCase)
                ):
...
def _addItem(cItem, compName, loop1, loopValue1, loopValue2, nefList, rowIndex, inWhich):
    ...
    for itm in nefList:
        for a, b in zip_longest(cItem3.list[:] + [LOOP + loop1.name], itm.list[:-1]):
...
def compareSaveFrames(saveFrame1, saveFrame2, options, cItem=None, nefList=None):
    ...
    for compName in dVSet:
        ...
        if saveFrame1[compName] != saveFrame2[compName]:
            ...
            cItem3.list.append(
                [" <Value>:  " + compName + "  -->  " + str(saveFrame1[compName]) + " != " + str(saveFrame2[compName])]
            )
            cItem3.inWhich = 3
            nefList.append(nefItem(cItem=cItem3))
