...
INFILE_NAMES = {
    "config": "%s_mars.inp",
    "shifts": "%s_cs.tab",
    "fixcon": "%s_fix_con.tab",
    "fixass": "%s_fix_ass.tab",
    "sequence": "%s_fasta.tab",
    "cyss": "%s_cyss.tab",
    "predcs": "%s_predcs.tab",
}
...
def runMars(
    ...
    marsDir, configFile = writeMarsInput(
        shiftList,
        chain,
        fragSize,
        cutoffCO,
...
def writeMarsInput(
    ...
    from ccpnmr.analysis.core.AssignmentBasic import (
        findConnectedSpinSystems,
        getSpinSystemChemComps,
        getSpinSystemResidues,
    )
    ...
    for spinSystem in nmrProject.sortedResonanceGroups():
        ...
        if (
            (useAssignment and assignedResidues)
    ...
    for spinSystem in rootSpinSystems:
        ...
        prevInSeq = [x for x in prevSpinSystems[spinSystem] if x not in prevSpinSystems]
        if prevInSeq:
            if len(prevInSeq) > 1:
    ...
    if useConnections:
        for spinSystem in rootSpinSystems:
            ...
            prevInSeq = [x for x in prevSpinSystems[spinSystem] if x in prevSpinSystems]
            if prevInSeq:
                if len(prevInSeq) > 1:
                    print("WARNING, spin system %s has more than one i-1 pseudo-neighbour")
                prevSpinSystem = prevInSeq[0]
  ...
  """
    ...
    cyss = [x for x in chain.sortedResidues() if x.ccpCode == "Cys" and "link:SG" in x.descriptor]
