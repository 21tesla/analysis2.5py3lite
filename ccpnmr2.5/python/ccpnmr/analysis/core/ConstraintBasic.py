...
from ccpnmr.analysis.core.AssignmentBasic import (
    areResonancesProchiral,
    assignAtomsToRes,
    assignResToDim,
    findMatchingPeakDimShifts,
    getBoundResonances,
    getOnebondResonance,
    getResonanceLabellingFraction,
    getResonancePairLabellingFraction,
    isShiftInRange,
)
from ccpnmr.analysis.core.ExperimentBasic import (
    findSpectrumDimsByIsotope,
    getIndirectDataDims,
    getIndirectThroughSpaceIsotopes,
    getOnebondDataDims,
    getThroughSpaceDataDims,
)
from ccpnmr.analysis.core.MoleculeBasic import areResonancesBound, getBoundAtoms
from ccpnmr.analysis.core.PeakBasic import getAliasedPeakDimPositions, pickPeak
from ccpnmr.analysis.core.UnitConverter import pnt2ppm, unit_converter
from ccpnmr.analysis.core.Util import getAnalysisPeakList, getSpectrumNoeDistanceClasses
    ...
    if resonance:
        ...
        if resonanceSet:
            ...
            if fResonance2:
                ...
                if (not resonance2) or (not resonance2.resonanceSet) or (resonance2.resonanceSet is resonanceSet):
            ...
            for peakContrib in constraint.peakContribs:
                ...
                for item in constraint.items:
                    ...
                    if assign:
                        ...
                        assignResToDim(peakDims[i], resonance0, contrib=contrib, peakContribs=peakContribs)

                        contrib = peakDims[j].findFirstPeakDimContrib(resonance=resonance1)
                        assignResToDim(peakDims[j], resonance1, contrib=contrib, peakContribs=peakContribs)

                        if (iX is not None) and bound0:
                            contrib = peakDims[iX].findFirstPeakDimContrib(resonance=bound0)
                            assignResToDim(peakDims[iX], bound0, contrib=contrib, peakContribs=peakContribs)

                        if (jX is not None) and bound1:
                            contrib = peakDims[jX].findFirstPeakDimContrib(resonance=bound1)
                            assignResToDim(peakDims[jX], bound1, contrib=contrib, peakContribs=peakContribs)
...
def makePeaksFromConstraints(constraints, spectrum=None):
    ...
    for spectrum0 in spectra:
        ...
        if not peakList:
            peakList = spectrum0.newPeakList(
                isSimulated=True, name="ConstraintsPredict", details="Constraints Predicted"
            )
    ...
    if not peakLists:
        showWarning("Failure", "Cannot make peaks from restraints. No input spectra")
        return []
            ...
            for spectrum0 in spectra:
                ...
                for spectrum1, positions, intensities in peakData:
                    ...
                    for intensityType, method, value, error in intensities:
                        peak.newPeakIntensity(method=method, intensityType=intensityType, value=value, error=error)
                    ...
                    for resonances in peakResonances:
                        ...
                        for peakDim in peak.peakDims:
                            ...
                            if resonance:
                                ...
                                if shift:
                                    delta = abs(shift.value - peakDim.value) * isotopeWeights.get(
                                        resonance.isotopeCode, 0.1
                                    )
                                    if delta > 1.0:  # Ought not be used: pathalogical case
                                        continue

                                    assignments[peakDim] = assignments.get(peakDim, []) + [
                                        resonance,
                                    ]
...
def copyFixedResonance(fixedResonance, nmrConstraintStore):
    ...
    if resonanceSet:
        ...
        if resonanceSet2:
            ...
            resonanceSet2 = cSet.newFixedResonanceSet(atomSets=atomSets2, resonances=resonanceMap.values)
        ...
        for key in resonanceDict.keys():
            ...
            if len(constraints) > 1:
                ...
                for constraint in constraints[1:]:
                    for item in constraint.items:
                        constraint0.newDihedralConstraintItem(
            ...
            if len(constraints) > 1:
                ...
                for constraint in constraints[1:]:
                    ...
                    for contrib in constraint.peakContribs:
                        serials = [
                            contrib.experimentSerial,
                            contrib.dataSourceSerial,
                            contrib.peakListSerial,
                            contrib.peakSerial,
                            contrib.peak,
                        ]
                ...
                for e, s, pl, p, peak in contribSerials:
                    if constraint0.findFirstPeakContrib(peak=peak) is None:
                        constraint0.newConstraintPeakContrib(
                            experimentSerial=e, dataSourceSerial=s, peakListSerial=pl, peakSerial=p
                        )
...
def makeStructureGeneration(structure, constraintSet=None, generationType="denovo"):
    ...
    structureGeneration = nmrProject.newStructureGeneration(generationType=generationType, structureEnsemble=structure)
...
def mergeConstraintLists(constraintListA, constraintListB):
    ...
    for constraint in constraintListB.constraints:
        copySubTree(constraint, constraintListA, objectMap=None, maySkipCrosslinks=True)
        constraint.delete()
...
def splitConstraintListAmbigUnambig(constraintList):
    ...
    if ambigList:
        ...
        constraintListB = ListClass(
            nmrConstraintStore, unit=unit, experimentSerials=experimentSerials, measureListSerials=measureListSerials
        )
...
def getStructureViolations(constraintList, structure, violationList=None):
    ...
    if constraintList.className not in ("DistanceConstraintList", "DihedralConstraintList", "HBondConstraintList"):
        return
    ...
    if constraintList.className != "DihedralConstraintList":
        ...
        for constraint in constraintList.constraints:
            ...
            if totalViols:
                ...
                if violObj:
                    ...
                    violationList.newViolation(
                        violation=aveViolAmount,
                        calcValue=calcValue,
                        calcValueError=sd,
                        fractionViolated=fracViolated,
        ...
        for constraint in constraintList.constraints:
            ...
            totalViols = 0
            for model in models:
                angle = getAtomSetsDihedral(atomSets, structure, model=model, inDegrees=inDegrees)
        ...
        for n in range(2):
            ...
            if dataDimRef:
                ...
                if not unit or unit == "None":
                    unit = "ppm"
                valRange = [
                    unit_converter[("point", unit)](1.0, dataDimRef),
...
def makeFixedResonance(nmrConstraintStore, resonance):
    ...
    fixedResonance = nmrConstraintStore.newFixedResonance(
        resonanceSerial=resonance.serial, isotopeCode=resonance.isotopeCode, name=resonance.name
    )
...
def getDistMinMax(
    intensityValue, intensityScale, resonances0, resonances1, distanceFunction, normalise=True, labelling=None
):
    ...
    if peakDims[distIndices[0]].peakDimContribs and peakDims[distIndices[1]].peakDimContribs:
        ...
        if peakDim0.dim in indirectDims and indirectDims[peakDim0.dim][1]:
            ...
            for resonance, indirect in resonances0:
                ...
                resonancesA = set(
                    x
                    for x in getBoundResonances(resonance, recalculate=True)
                    if x.isotopeCode == isotopeCode and x.resonanceSet
                )
                ...
                atomSets = set(a.atomSet for a in atoms if a.atomSet and a.chemAtom.chemElement is chemElement)

                if resonancesA:
                    # remove covalently impossible resonances
                    resonanceSets = set(y for x in atomSets for y in x.resonanceSets)
                    resonancesA = set(x for x in resonancesA if x.resonanceSet in resonanceSets)
    ...
    for peak in workingPeaks:
        ...
        if resonances0 and resonances1:
            ...
            if fResonancePairs:
                ...
                constraint.newConstraintPeakContrib(
                                ...
                                for shift in shifts:
                                    ...
                                    for shift2 in shifts2:
                                        ...
                                        if areResonancesBound(resonance, resonance2):
                                            if residueRanges:
                                                residue = (
                                                    resonance.resonanceSet.findFirstAtomSet().findFirstAtom().residue
                                                )
        ...
        if peakResonances[0] and peakResonances[1]:
            ...
            for resonance0, bound0, indirect0 in peakResonances[0]:
                ...
                for resonance1, bound1, indirect1 in peakResonances[1]:
                    ...
                    if labelling:
                        if bound0:
                            fraction0 = getResonancePairLabellingFraction(resonance0, bound0, labelling)
                        else:
                            fraction0 = getResonanceLabellingFraction(resonance0, labelling)
                        if fraction0 < minLabelFraction:
                            continue

                        if bound1:
                            fraction1 = getResonancePairLabellingFraction(resonance1, bound1, labelling)
                        else:
                            fraction1 = getResonanceLabellingFraction(resonance1, labelling)
            ...
            if not resonancePairs:
                ...
                (dist, minDistL, maxDistL) = getDistMinMax(
                    intensityValue, mean, resonances0, resonances1, distanceFunction, labelling=labelling
                )
...
def transferCovalentlyBound(sourceResonance, targetResonance):
    ...
    if sourceResonance.className == "FixedResonance":
        covalentSerials = set(x.resonanceSerial for x in sourceResonance.covalentlyBound)
        if None in covalentSerials:
            covalentSerials.remove(None)
    else:
        # normal resonance, make sure we update status
        covalentSerials = set(x.serial for x in getBoundResonances(sourceResonance, recalculate=True))

    topObj = targetResonance.topObject
    if targetResonance.className == "FixedResonance":
        boundRes = set(topObj.findFirstFixedResonance(resonanceSerial=x) for x in covalentSerials)
    else:
        boundRes = set(topObj.findFirstResonance(serial=x) for x in covalentSerials)
    if None in boundRes:
