...
from ccp.util.LabeledMolecule import (
    getExperimentAtomFractions,
    getExperimentAtomPairFractions,
    getSchemeAtomFractions,
    getSchemeAtomPairFractions,
)
...
from ccpnmr.analysis.core.ExperimentBasic import (
    ...
    getEquivalentDataDims,
    getOnebondDataDims,
    getPrimaryDataDimRef,
    getThroughSpaceDataDims,
    ppmDataDimBoundedRegion,
    ptsDataDimBoundedRegion,
)
from ccpnmr.analysis.core.MergeObjects import mergeObjects
from ccpnmr.analysis.core.MoleculeBasic import (
    DEFAULT_ISOTOPES,
    areAtomsTocsyLinked,
    getLinkedResidue,
    getNumConnectingBonds,
    getResidueObservableAtoms,
)
from ccpnmr.analysis.core.PeakFindParams import getPeakFindBoxwidth, getPeakFindMinLinewidth, getPeakFindParams
...
from memops.universal.Util import arrayOfIndex, cumulativeProductArray
...
from ccp.api.nmr.Nmr import PeakDimContribN, SampledDataDim
...
def makeTransposePeakList(peakList):
    ...
    if not dimPairList:
        showWarning("Transpose Peak List Failure", "Spectrum has no equivalent dimension pairs")
        return
...
def findShiftDistPeakMatches(peak, peakList, threshold, scaleFactorDict=None, considerAliased=True, dimMapping=None):
    ...
    peaks = findClosePeaks(peak, peakList, tolerances, considerAliased=considerAliased, dimMapping=dimMapping)
    for peakB in peaks:
        value = getPeaksOverlapScore(peak, peakB, scaleFactors, considerAliased=considerAliased, dimMapping=dimMapping)
...
def getPeakMatchRegion(peak, tolerances, considerAliased=False):
    ...
    for i in range(len(peakDims)):
        ...
        if dataDimRef:
            if considerAliased:
                ranges = [
                    getDataDimRefFullRange(dataDimRef),
                ]
...
def findClosePeaks(
    peak, peakList, tolerances=None, pickNewPeaks=False, considerAliased=False, dimMapping=None, noiseThreshold=None
):
    """
    ...
    for i in range(N):
        ...
        if tolerance is not None:
            ...
            if considerAliased:
                ...
                for point in getAliasedPeakDimPositions(
                    peakDimA,
                    [
                        fullRange,
                    ],
...
def findPositionMatches(peaks, dimDict, peakLists=None, matchTolerance=None, separateSpinSystems=False):
    """
    ...
    """
    from AssignmentBasic import getDataDimFullShiftRange
...
def copyPeakListNew(peakList, destDataSource, peakListParams=None):
    ...
    newPeakList = copySubTree(peakList, destDataSource, topObjectParameters=peakListParams, objectMap=objectMap)
        ...
        for peak in peaks:
            ...
            for peakDim in peakDims:
                ...
                if (samplePlane == 0) and (sampledDim.dataSource is spec2):
                    ...
                    newPeaks = [
                        pickPeak(destPeakList, position, "ppm", doFit=False),
                    ]
...
def makeIntermediatePeak(peaks):
    ...
    for peak in peaks:
        if peak.peakList is not peakList:
            showWarning("Error", "Selected peak from different peak list. Skipping.")
            continue
...
def propagatePeakUnaliasing(refPeak, peaks):
    ...
    for peak in peaks:
        if peak.peakList.dataSource is not refSpec:
            showWarning("Failure", "Cannot set unaliasing for peaks from different spectra")
            return
    ...
    for peakDim in peak.peakDims:
        ...
        if dataDimRef and (dataDimRef.expDimRef.measurementType == "MQShift"):
            ...
            for dataDimA in spectrum.dataDims:
                ...
                if (expDimRefA.measurementType == "Shift") and (expDimRefA.isotopes == dataDimRef.expDimRef.isotopes):
        ...
        if dataDimRef:
            ...
            if (expDimRef.measurementType == "MQShift") and (expDimRef.refExpDimRef):
                for scalingFactor in expDimRef.refExpDimRef.validScalingFactors:
                    # TBD: Int is horrid!!
                    peakDim.newPeakDimComponent(dataDimRef=dataDimRef, scalingFactor=int(scalingFactor))
    ...
    if fitMethod == PEAK_FIT_METHODS[0]:
        if doFit:
            ...
            if center is not None:
                ...
                peak.fitMethod = getMethod(
                    peak.root, task="fit peak", procedure="%s peak fit" % fitMethod, parameters=(("method", fitMethod),)
                )
        if doOtherFit:
...
def fitPeaks(peaks, fitMethod, updatePosition=True):
    ...
    apiFitMethod = getMethod(
        fittedPeaks[0].root, task="fit peak", procedure="%s peak fit" % fitMethod, parameters=(("method", fitMethod),)
    )
...
def setupPeakVolume(peak, volumeMethod=None):
    ...
    if volume is not None:
        ...
        if prevVol:
            prevVol.delete()
        method = getMethod(
            peak.root, task="fit peak volume", procedure="peak.cPeak.fitVolume", parameters=(("method", volumeMethod),)
        )
...
def findPeaks(peakList, region, parent=None, thickness=None, excludedRegions=None, intensityThreshold=None):
    """
    ...
    for dataDim in spectrum.dataDims:
        if dataDim.className == "FreqDataDim":
            ...
            min_lw = convertPosition(min_lw, getPrimaryDataDimRef(dataDim), fromUnit="Hz", relative=True)
        else:
            min_lw = 0
    ...
    for dataDim in dataDims:
        ...
        if dataDim.className == "FreqDataDim":
            ...
            (regions[dim], min_point[dim], max_point[dim]) = findDataDimRegions(
                dataDim, (r0, r1), axisUnit, thickness[dim]
            )
        else:
    ...
    for r in range(nregions):
        ...
        positions = peakList.cPeakList.findPeaks(
            first,
            last,
            block_file,
            have_high=have_high,
            ...
            diagonal_exclusions=diagonalExclusions,
...
def snapPeak(peak, doFit=True):
    ...
    positions = peak.peakList.cPeakList.findPeaks(
        first,
        last,
        block_file,
        have_high=have_high,
                    ...
                    for atomA in atomsA:
                        ...
                        if transferType == "relayed":
                            ...
                            if atomA.residue is atomB.residue:
                                ...
                                if (prevA and (atomB.residue is prevA)) or (prevB and (atomA.residue is prevB)):
                                    if bondLimit:
                                        numBonds = getNumConnectingBonds(atomA, atomB)
                                        if numBonds and (numBonds <= bondLimit):
                                            correlate = True
                                ...
                                if residueLimit == 2:
                                    ...
                                    if (prevA2 and (atomB.residue is prevA2)) or (prevB2 and (atomA.residue is prevB2)):
                                        if bondLimit:
                                            numBonds = getNumConnectingBonds(atomA, atomB)
                                            if numBonds and (numBonds <= bondLimit):
                                                correlate = True
                            ...
                            if refType in ("H[N[{CA|ca[Cali]}]]", "h{CA|Cca}NH", "H[N[CA]]"):
                                if refType == "H[N[CA]]":
                                    atomNames = ("CA",)
                                else:
                                    atomNames = (
                                ...
                                if observable is None:
                                    observable = getResidueObservableAtoms(residue, refExperiment)
...
def structurePredictNoePeakList(
    structure,
    spectrum,
    distThreshold=5.0,
    progressBar=None,
