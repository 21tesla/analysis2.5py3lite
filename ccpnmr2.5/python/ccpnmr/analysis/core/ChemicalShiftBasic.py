...
def lookupAtomProbability(project, ccpCode, atomName, shiftValue, molType=PROTEIN_MOLTYPE):
...
def getResidueProbability(
    ppms, ccpCode, elements, atomTypes=None, ppmsBound=None, prior=0.05, molType=PROTEIN_MOLTYPE, cutoff=1e-10
):
...
def getSpinSystemChainProbabilities(spinSystem, chain, shiftList):
    ...
    for ccpCode, molType in ccpCodes:
        probDict[ccpCode] = getProb(spinSystem, shiftList, ccpCode, priors[ccpCode], molType)
...
def getShiftsResidueProbability(shifts, ccpCode, prior=0.05, molType=PROTEIN_MOLTYPE):
    ...
    prob = getResidueProbability(ppms, ccpCode, elements, atomTypes, boundPpms, prior, molType)

    return prob


def getSpinSystemResidueProbability(spinSystem, shiftList, ccpCode, prior=0.05, molType=PROTEIN_MOLTYPE):
    ...
    prob = getResidueProbability(ppms, ccpCode, elements, atomTypes, prior, molType)

    return prob


def getChemAtomNmrRef(project, atomName, ccpCode, molType=PROTEIN_MOLTYPE, sourceName="RefDB"):
...
if __name__ == "__main__":
    ...
    for ccpCode in ccpCodes:
        vals[ccpCode] = getResidueProbability(
            shiftValues,
            ccpCode,
            elements,
            atomTypes,
            prior=0.0578512396694,
