...
from ccpnmr.analysis.core.ConstraintBasic import getIntensityDistanceTable, getMeanPeakIntensity, getNoeDistance
                ...
                if closeResonancesDict.get(peak) is None:
                    ...
                    for dim in hDims:
                        ...
                        shifts = findMatchingPeakDimShifts(
                            peakDim, shiftRanges=None, tolerance=tolerances[dim], aliasing=True, findAssigned=False
                        )
                                    ...
                                    if areResonancesBound(resonance1, resonance2):
                                        if labelling:
                                            fraction = getResonancePairLabellingFraction(
                                                resonance1, resonance2, expLabelling
                                            )
            ...
            if not peakDim.peakDimContribs:
                ...
                if dim2 is not None:
                    ...
                    if peakDim2.peakDimContribs:
                        ...
                        shifts2 = findMatchingPeakDimShifts(
                            peakDim2, shiftRanges=None, tolerance=tolerances[dim2], aliasing=True, findAssigned=False
                        )
                    for shift in shifts:
                        resonance1 = shift.resonance
                        for shift2 in shifts2:
...
def getCloseSingleShiftMatches(
    peakList, network, covalent, labelling=None, minLabelFraction=0.1, intensityType="height", progressBar=None
):
                ...
                if dim2 is not None:
                    ...
                    for shift in shifts:
                        ...
                        for shift2 in shifts2:
                            ...
                            if areResonancesBound(resonance1, resonance2):
                                if labelling:
                                    fraction = getResonancePairLabellingFraction(resonance1, resonance2, labelling)
                                    if fraction < minLabelFraction:
                                        continue
