...
from memops.universal.Util import cumulativeProductArray, isBigEndian

filterExpr1 = re.compile(r"\[[^A-Z]+\]")
filterExpr2 = re.compile(r"H\([0123]\)")

# TBD: put several functions from Util in here

COMMON_REF_EXPTS = set(
    [
        "H",
        "C",
        "N",
        ...
        "H_H[N].through-space",
        "H[N[CA]]",
        "H[C]_H.through-space",
        "H[N[ca[HA]]]",
        "H{[N]+[HA]}",
        "H[C]_H[C].through-space",
        "H[N]_H[C].through-space",
        "H[N]_H[N].through-space",
        "H[N]_H.relayed",
        "H_H[N].relayed",
        "HH",
        "H_H.through-space",
...
def selectPreferredRefExperiment(refExperiments, experiment=None):
    ...
    #
    refExperiments = [prot.findFirstRefExperiment(serial=ser) for (prot, ser) in foundPrototypes.items()]
...
def getRefExperimentCategories(refExperiment):
    ...
    if len(expGraphs) > 1:
        ...
        elif "J-resolved" not in cats:
            cats.add("Mixed Peak Type")

    # filtered
    if (filterExpr1.search(name) or filterExpr2.search(name)) and "Signed Peaks" not in cats:
        cats.add("Filtered")
    ...
    if isSimple and shiftOnly:
        if (
            len(prototype.expGraphs) == 1
            and "Filtered" not in cats
            ...
            for atomSiteH in atomSitesH:
                ...
                for graph in prototype.expGraphs:
                    for transferType in longRangeTransfers:
                        ...
                        transfer = graph.findFirstExpTransfer(atomSites=sitesHN, transferType="onebond")
                        ...
                        if transfer:
                            ...
                            if atomSiteCO:
                                ...
                                if graph.findFirstExpTransfer(atomSites=sitesNC, transferType="onebond"):
...
def getDataDimRefFullRange(dataDimRef):
    ...
    valRange = [converter(1, dataDimRef), converter(dataDimRef.dataDim.numPoints, dataDimRef)]
    valRange.sort()
...
def initExpTransfers(experiment, overwrite=True):
    ...
    for expGraph in experiment.refExperiment.nmrExpPrototype.expGraphs:
        for expTransfer in expGraph.expTransfers:
            if (expTransfer not in transferDict) and (expTransfer.transferType in longRangeTransfers):
                atomSiteA, atomSiteB = expTransfer.atomSites
    ...
    for refTransfer in transferDict.keys():
        ...
        if len(expDimRefs) == 2:
            ...
            if expTransfer:
                ...
                expTransfer = experiment.newExpTransfer(transferType=transferType, expDimRefs=expDimRefs)
        ...
        if experment2 is not experiment:
            ...
            for expDimRefA, expDimRefB in expDimRefPairs2:
                for expDimRefC, expDimRefD in expDimRefPairs:
                    if (expDimRefA.isotopeCodes == expDimRefC.isotopeCodes) and (
                        expDimRefB.isotopeCodes == expDimRefD.isotopeCodes
                    ):
                        # expDimRefMapping[expDimRefA] = expDimRefC
                        # expDimRefMapping[expDimRefB] = expDimRefD
                        experiments.append(experment2)
                        break
                    elif (expDimRefA.isotopeCodes == expDimRefD.isotopeCodes) and (
                        expDimRefB.isotopeCodes == expDimRefC.isotopeCodes
                    ):
...
def getAcqRefExpDimRef(refExperiment):
    ...
    if len(ll) == 1:
        ...
        raise ApiError("%s has no unambiguous RefExpDimRef for acqMeasurement (%s)" % (refExperiment, acqMeasurement))
...
def setRefExperiment(experiment, refExperiment):
    ...
    if (refExpDims.index(acqRefExpDim) * 2 < len(refExpDims)) != (expDims.index(acqExpDim) * 2 < len(expDims)):
    ...
    for expDim in expDims:
        ...
        for refExpDimRef in refExpDimRefs:
            ...
            refData.append(
                (
                    frozenset(x.isotopeCode for x in atomSites),
        ...
        for expDimRef in expDim.expDimRefs:
            inData.append(
                (
                    frozenset(expDimRef.isotopeCodes),
...
def getPossibleRefExperiments(experiment, category=None):
    ...
    if category == "use external":
        if extSource is None or not extName:
            ...
            return [x for y in memopsRoot.sortedNmrExpPrototypes() for x in y.sortedRefExperiments()]
            ...
            if extSource == "ccpn":
                ...
                for name, isReversed in nameMatches:
                    ...
                    if expPrototype is None:
                        ...
                        result.extend(
                            x
                            for y in memopsRoot.sortedNmrExpPrototypes()
                        ...
                        if isReversed is None:
                            ...
                            result.extend(expPrototype.findAllRefExperiments(isReversed=isReversed))

            else:
                raise NotImplementedError("External expt source %s not supported" % extSource)
            #
            return result
        ...
        return [x for y in memopsRoot.findAllNmrExpPrototypes(category=category) for x in y.sortedRefExperiments()]
...
def setExperimentShiftList(experiment, shiftList):
    ...
    None
    """
    from ccpnmr.analysis.core.AssignmentBasic import averageShiftValue, updateResonShift
...
def isShapeSpectrum(spectrum):
    ...
    Boolean
    """
    return isinstance(spectrum.dataStore, ShapeMatrix) or (hasattr(spectrum, "valuesList") and spectrum.valuesList)
        ...
        for isotopes, ll in sorted(nucData.items()):
            ...
            if len(ll) > 1:
                ...
                for ddr in ll[1:]:
                    ...
                    print("WARNING, %s baseFrequency differs: %s:%s; %s:%s" % (isotopes, ddr0, baseFreq, ddr, bf2))
...
def cloneExperiment(experimentOld, experimentNameNew, dataSourceNameNew=None, cloneDataFile=True):
    ...
    if dataSourceNameNew is None:
        keys += ("name",)
    if cloneDataFile:
        keys += ("isNormalStorage", "storageDetails", "compressMethod", "dataStore", "processMethod")
    ...
    for dataDimOld in dataSourceOld.sortedDataDims():
        ...
        if dataDimOld.className == "FidDataDim":
            ...
            keys = ("numPointsOrig", "phase0", "phase1", "pointOffset", "valuePerPoint", "predictMethod")
            dd.update((x, getattr(dataDimOld, x)) for x in keys)
...
def getRegionStats(spectrum, region):
    ...
    for dataDim in dataDims:
        ...
        if dataDim.className == "FreqDataDim":
            ...
            (regions[dim], min_point, max_point) = findDataDimRegions(
                dataDim, (r0, r1), axisUnit, thickness=0, addOneToUpperPoint=False
            )
        else:
