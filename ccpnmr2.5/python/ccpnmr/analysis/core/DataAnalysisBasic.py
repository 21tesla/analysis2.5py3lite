...
from ccpnmr.analysis.core.AssignmentBasic import assignResToDim, isPeakAssigned, propagatePeakAssignments
from ccpnmr.analysis.core.ExperimentBasic import getExperimentSampledDim, getSpectrumIsotopes, getSpectrumNoise
from ccpnmr.analysis.core.PeakBasic import (
    arePeaksAssignedSame,
    findClosePeaks,
    findSameAssignmentPeaks,
    getPeakDimPpm,
    pickPeak,
)
...
try:
    from memops.c.FitMethod import bootstrapData, fitData, runFit
    from memops.c.FitMethod import error as FitMethodError
except Exception as e:
    print(
        "Error, the DataAnalysisBasic module fitting functionality will not work, something is wrong with the C code."
    )
    print("Exception:", e)
...
def getExperimentConditionSet(experiment):
    ...
    if conditionSet is None:
        conditionSet = experiment.topObject.newSampleConditionSet(
            experiments=[
                experiment,
            ]
        )
        ...
        if sampleConditionSet:
            for sampleCondition in sampleConditionSet.sampleConditions:
                ...
                if condition in sampleConditionTypes:
                    conditionDict[condition] = conditionDict.get(condition, []) + [
                        sampleCondition,
                    ]
...
def makeDataList(name, unit, values, errors=None, peaks=None, peakDims=None, measurements=None):
    ...
    for i in range(N):
        derivation = dataList.newDataDerivation(inputMeasurements=measurements[i], peakDims=peakDims[i], peaks=peaks[i])
        try:
            resonances = measurements[i].resonances
        except:
            resonances = [
                measurements[i].resonance,
            ]
...
def makeRatesList(rateType, experimentOrSeries, specificType, peakGroups, values, errors=None):
    ...
    if rateType == "T1":
        measurementList = nmrProject.newT1List(unit=unit, sf=sf, coherenceType=specificType, experiments=experiments)
        for i in range(N):
            measurement = measurementList.newT1(
                value=values[i], error=errors[i], resonance=resonances[i], peaks=peakGroups[i]
            )

    elif rateType == "T1rho":
        measurementList = nmrProject.newT1RhoList(unit=unit, sf=sf, coherenceType=specificType, experiments=experiments)
        ...
        for i in range(N):
            measurement = newObjFunc(value=values[i], error=errors[i], resonance=resonances[i], peaks=peakGroups[i])

    elif rateType == "T2":
        measurementList = nmrProject.newT2List(unit=unit, sf=sf, coherenceType=specificType, experiments=experiments)
        for i in range(N):
            measurement = measurementList.newT2(
                value=values[i], error=errors[i], resonance=resonances[i], peaks=peakGroups[i]
            )
        ...
        for i in range(N):
            measurement = measurementList.newHExchRate(
                value=values[i], error=errors[i], resonance=resonances[i], peaks=peakGroups[i]
            )

    elif rateType == "H/D protection":
        measurementList = nmrProject.newHExchProtectionList(
            unit=unit, protectionType=specificType, experiments=experiments
        )
        for i in range(N):
            measurement = measurementList.newHExchProtection(
                value=values[i], error=errors[i], resonance=resonances[i], peaks=peakGroups[i]
            )
...
def matchSampledExperimentPeaks(
    refPeakList,
    experiment,
    tolerances,
    pickPeaks=True,
    ...
    dataDim = getExperimentSampledDim(experiment)
    if not dataDim:
        showWarning("Group Peaks Failure", "Experiment %s has no sampled dims" % (experiment.name))
        return
    ...
    for peak in refPeaks:
        ...
        if [] in samplePeaks:
            for peak2 in findClosePeaks(
                peak, peakList, tolerances=tolerances, pickNewPeaks=pickPeaks, noiseThreshold=noiseThreshold
            ):
...
def getFitErrorInfo():
    ...
    data = [
        ("covariance", "Calculates covariance (valid if have normally distributed errors)"),
...
def matchHnoePeaks(
    assignPeakList, refPeakList, satPeakList, tolerancesA, tolerancesB, pickNewPeaks=True, doAssignments=False
):
    ...
    for peak in assignPeakList.peaks:
        ...
        if not refPeaks:
            refPeaks = findClosePeaks(peak, refPeakList, tolerances=tolerancesA, pickNewPeaks=pickNewPeaks)
        ...
        if not satPeaks:
            satPeaks = findClosePeaks(peak, satPeakList, tolerances=tolerancesB, pickNewPeaks=pickNewPeaks)
...
def getPeakMatchRegion(peak, tolerances):
    ...
    print(
        "Warning: Deprecated use of DataAnalysisBasic getPeakMatchRegion()\n Use PeakBasic getPeakMatchRegion() instead"
    )
...
def getOverlapScore(peakA, peakB, tolerances):
    ...
    print(
        "Warning: Deprecated use of DataAnalysisBasic getOverlapScore()\n Use PeakBasic getPeaksOverlapScore() instead"
    )
