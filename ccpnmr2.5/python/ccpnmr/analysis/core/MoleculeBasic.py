...
def getResidueMapping(residue, aromaticsEquivalent=True):
    ...
    if not chainMapping:
        chainMapping = analysisProject.newChainMapping(
            molSystemCode=residue.chain.molSystem.code, chainCode=residue.chain.code
        )
...
def makeAtomSetMapping(residueMapping, name, atomSets, chemAtomSet, mappingType, resonances=None):
    ...
    atomSetMapping = residueMapping.newAtomSetMapping(
        name=guiName,
...
def makeGuiMultiAtomSet(residue, multiGuiName, guiSetsNames, elementSymbol, mappingType, chemAtomSet):
    ...
    for guiName in guiSetsNames:
        ...
        if atomSetMapping is None:
            print(
                "Non-existent group error in makeGuiMultiAtomSet for",
                residue.molResidue.ccpCode,
                residue.seqCode,
                guiName,
            )
            return
