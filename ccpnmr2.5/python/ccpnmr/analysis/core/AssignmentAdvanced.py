...
def assignSpecNonRootResonances(
    rootPeaks,
    targetPeakList,
    tolerances=None,
    diagTolerance=0.5,
    ...
    Output:
    """
    peaks, dimMapping = pickAssignSpecFromRoot(
        rootPeaks, targetPeakList, tolerances=tolerances, progressBar=progressBar, pickNew=False
    )
    ...
    resonances = assignDimNewResonances(
        peaks,
        targetDataDim.dim,
...
def assignDimNewResonances(peaks, dimNum, diagTolerance=0.5, waterMinPpm=4.88, waterMaxPpm=4.92, assignType=False):
    """Descrn:
    Inputs:
    Output:
    """
...
def pickAssignSpecFromRoot(
    rootPeaks, targetPeakList, tolerances=None, progressBar=None, pickNew=True, diagTolerance=None, waterExclusion=None
):
    ...
    if not dimMapping:
        ...
        data = (spectrum0.experiment.name, spectrum0.name, spectrum1.experiment.name, spectrum1.name)
            ...
            for peak2 in peaks:
                ...
                if not peakDim2.peakDimContribs:
                    for resonance in resonances:
                        assignResToDim(peakDim2, resonance, tolerance=10 * tolerance, doWarning=False)
...
def initialiseAmideExpts(argServer, hsqc=None, tocsy=None, noesy=None):
    ...
    if not noesyPl.peaks:
        ...
        wholeRegion = [[pnt2ppm(dd.numPointsOrig, func(dd)), pnt2ppm(0, func(dd))] for dd in dataDims]
        excludeRegion = [[0, dd.numPointsOrig] for dd in dataDims]

        dataDimRef = func(dataDims[xDim])
        excludeRegion[xDim] = [ppm2pnt(waterRegion[0], dataDimRef), ppm2pnt(waterRegion[1], dataDimRef)]
...
def possiblePeakAssigments(peak, shiftRanges=None, tolerances=None, aliasing=True, findAssigned=False):
    ...
    for ii, peakDim in enumerate(peak.sortedPeakDims()):
        ll = findMatchingPeakDimShifts(peakDim, shiftRanges[ii], tolerances[ii], aliasing, findAssigned)
                    ...
                    for res1 in matchResDict[dimNo1]:
                        for res2 in matchResDict[dimNo2]:
                            if areResonancesBound(res1, res2):
                                set1.add(res1)
                                set2.add(res2)
                    if len(set1) < len(matchResDict[dimNo1]) or len(set2) < len(matchResDict[dimNo2]):
                        changing = True
