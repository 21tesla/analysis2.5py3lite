...
from memops.universal.Util import isMacOS, isWindowsOS
...
from ccpnmr.analysis.core.ExperimentBasic import getNoiseEstimate, getPrimaryDataDimRef, getSpectra
...
def getAnalysisProject(project):
    ...
    if not analysisProject:
        ...
        analysisProject = project.newAnalysisProject(name=project.name, nmrProject=nmrProject)
...
def getAnalysisSpectrum(spectrum):
    ...
    if not analysisSpectrum or not (analysisSpectrum.posLevels or analysisSpectrum.negLevels):
...
def setupAnalysisSpectrum(analysisSpectrum):
    ...
    if not (
        ...
        or analysisSpectrum.negLevels
        or analysisSpectrum.posColors
        or analysisSpectrum.negColors
    ):
        ...
        numberLevels = application.getValue(
            spectrum, keyword="numberLevels", defaultValue=numberLevels, deleteAppData=True
        )
        levelChanger = application.getValue(
            spectrum, keyword="levelChanger", defaultValue=levelChanger, deleteAppData=True
        )
        changeMode = (
            application.getValue(spectrum, keyword="levelChangeMode", defaultValue=0, deleteAppData=True)
            and "add"
            or "multiply"
        )
...
def getAnalysisDataDim(dataDim):
    ...
    if not analysisDataDim:
        ...
        if isinstance(dataDim, FreqDataDim):
            ...
            minLw = application.getValue(dataDim, keyword="peakFindmin_lw", defaultValue=0.0)
            analysisDataDim.peakFindMinLineWidth = minLw

            boxwidth = application.getValue(dataDim, keyword="peakFindboxwidth", defaultValue=1.0)
...
def getAnalysisPeakList(peakList):
    ...
    if not analysisPeakList:
        ...
        analysisPeakList = analysisSpectrum.newAnalysisPeakList(
            peakList=peakList, symbolColor=color, textColor=color, symbolStyle=symbol, noeIntensityType=noeIntensityType
        )
...
def getSpectrumContourPriority(spectrum):
    ...
    value = application.getValue(
        spectrum, keyword="contourPriority", defaultValue=spectrumContourPriorities[0], deleteAppData=True
    )
...
def getSpectrumFont(spectrum):
    ...
    application = spectrum.root.application
    name = application.getValue(spectrum, keyword="fontName", defaultValue="Helvetica", deleteAppData=True)
    size = application.getValue(spectrum, keyword="fontSize", defaultValue=10, deleteAppData=True)
...
def getSpectrumShortcut(spectrum):
    ...
    application = project.application
    shortcut = application.getValue(spectrum, keyword="shortcut", deleteAppData=True)
...
def getNamedSpectrumView(window, name):
    ...
    for pane in window.spectrumWindowPanes:
        for view in pane.spectrumWindowViews:
            ...
            if dataSource.name == specName and dataSource.experiment.name == expName:
                return view
...
def getSpectrumActivePeakList(spectrum):
    ...
    if not peakList:
        # v1 methodology
        application = spectrum.root.application
        serial = application.getValue(spectrum, keyword="activePeakList", deleteAppData=True)
...
def getSpectrumNoeIntensityType(spectrum):
    ...
    project = spectrum.root
    application = project.application
    value = application.getValue(spectrum, keyword="noeIntensityType", defaultValue="volume", deleteAppData=True)
...
def getSampledDataDimReferencePlane(dataDim):
    ...
    application = project.application
    value = application.getValue(dataDim, keyword="sampleReferencePlane", defaultValue=None)
...
def setSpectrumNoeIntensityType(spectrum, intensityType):
    ...
    application = spectrum.root.application
    application.setValue(spectrum, keyword="noeIntensityType", value=intensityType)
...
def getSpectrumNoeDistanceClasses(spectrum):
    ...
    if hasattr(project, "application"):
        ...
        for i in range(50):  # Suitably large
            value = application.getValue(
                spectrum, keyword="%s%d" % (NOE_DIST, 1), defaultValue=None, deleteAppData=True
            )
        ...
        # New method
        for datum in spectrum.findAllApplicationData(application=application.name, keyword=NOE_DIST):
...
def setSpectrumNoeDistanceClasses(spectrum, distanceClasses):
    ...
    appData = spectrum.findAllApplicationData(application=application.name, keyword=NOE_DIST)
    ...
    for datum in distanceClasses:
        ...
        appDatum = Implementation.AppDataString(application=application.name, keyword=NOE_DIST, value=value)
...
def setSpectrumNoeResidueRanges(spectrum, ranges):
    ...
    project = spectrum.root
    application = project.application
    appData = spectrum.findAllApplicationData(application=application.name, keyword=keyword)
    ...
    for dims, chain, startRes, endRes in ranges:
        ...
        data = [chain.molSystem.code, chain.code, "seqId" + str(startRes.seqId), "seqId" + str(endRes.seqId)]
        data.extend(dimNums)
        value = "%".join(data)

        appDatum = Implementation.AppDataString(application=application.name, keyword=keyword, value=value)
        spectrum.addApplicationData(appDatum)
...
def getSpectrumNoesResidueRanges(spectrum):
    ...
    for i in range(50):  # Arbitrarily large
        value = application.getValue(spectrum, keyword="%s%d" % (keyword, i), defaultValue=None, deleteAppData=True)
        if value:
            values.append(value)
        else:
            break
...
def getSpectrumPosContourColor(spectrum):
    ...
    return application.getValue(spectrum, keyword="posContourColor", defaultValue=initialColor, deleteAppData=True)


def getSpectrumNegContourColor(spectrum):

    colors = ("magenta", "orange", "mauve", "yellow", "teal", "orange", "cyan", "midgreen")

    project = spectrum.root
    application = project.application
    initialColor = colors[(int(spectrum.experiment.serial) % 8) - 1]
    return application.getValue(spectrum, keyword="negContourColor", defaultValue=initialColor, deleteAppData=True)
...
def getSpectrumSliceColor(spectrum):
    ...
    application = spectrum.root.application
    return application.getValue(spectrum, keyword="sliceColor", defaultValue=defaultValue, deleteAppData=True)


def getSpectrumBoxVisibility(spectrum):

    application = spectrum.root.application
    return application.getValue(spectrum, keyword="isSpectrumBoxShown", defaultValue=True, deleteAppData=True)


def getSpectrumPointerVisibility(spectrum):

    application = spectrum.root.application
    return application.getValue(spectrum, keyword="isSpectrumPointerShown", defaultValue=True, deleteAppData=True)
...
def getSpectrumContourLevels(spectrum):
    ...
    if not levels:
        ...
        levels = application.getValue(spectrum, keyword="contourLevel", deleteAppData=True)
        if levels:
...
def getPeakListSymbol(peakList):
    ...
    symbol = application.getValue(peakList, keyword="symbol", defaultValue="x")
...
def getDataDimAssignmentTolerance(dataDim):
  ...
  """
    ...
    application = dataDim.root.application
    tolerance = application.getValue(dataDim, keyword="assignmentTolerance", defaultValue=default)
...
def getDataDimNoeTolerance(dataDim):
    ...
    tolerance = application.getValue(dataDim, keyword="noeTolerance", defaultValue=default)
...
def getDataDimShiftWeight(dataDim):
    ...
    application = dataDim.root.application
    weight = application.getValue(dataDim, keyword="shiftWeight", defaultValue=1.0)
...
def setTopObjectAnalysisSaveTime(topObject):
    ...
    application = topObject.root.application
    application.setValue(topObject, keyword="CcpNmrAnalysisTimeStamp", value=timeStamp)
...
def findPanelType(project, axisType, axisPanels):
    ...
    panelTypes = [panelType for panelType in analysisProject.panelTypes if panelType.axisType == axisType]
    n = len(panelTypes) + 1
...
def createAxisPanel(windowPane, axisType, region=None):
    ...
    panel = windowPane.newAxisPanel(label=label, panelType=panelType, axisUnit=axisUnit)
...
def createSpectrumWindowView(
    windowPane, spectrum, axisPanelMapping, isPosVisible=True, isNegVisible=True, isInToolbar=True, isSliceVisible=True
):

    analysisSpectrum = getAnalysisSpectrum(spectrum)

    try:
        view = windowPane.newSpectrumWindowView(
...
def defaultColors(project):
    ...
    if not analysisProfile.colorSchemes:
        for color in Color.standardColors:
            analysisProfile.newColorScheme(
                name=color.name,
                colors=[
                    color.hex,
                ],
...
def convertPosition(position, dataDimRef, fromUnit="point", toUnit="point", relative=False):
...
def findAxisTypeMatch(dataDim):
    ...
    if isinstance(dataDim, FreqDataDim):
        ...
        expDimRef = getPrimaryDataDimRef(dataDim).expDimRef
        axisType = findAxisType(isotopeCodes=expDimRef.isotopeCodes, measurementType=expDimRef.measurementType)

    elif isinstance(dataDim, SampledDataDim) and (dataDim.conditionVaried == "temperature"):
        axisType = findAxisType(name="T")

    # TBD: below is short-term hack
    elif isinstance(dataDim, SampledDataDim) and (not dataDim.conditionVaried):
...
def getSoftware(project):
    ...
    return Software.getSoftware(
        project,
        name,
        version,
        Copyright.vendorName,
        Copyright.vendorAddress,
        Copyright.vendorWebAddress,
        Copyright.details,
    )
