...
from ccp.util.LabeledMolecule import (
    atomPairFractions,
    getIsotopomerAtomPairFractions,
    getIsotopomerSingleAtomFractions,
    singleAtomFractions,
)
...
def getResonanceLabellingFraction(resonance, labelling):
    ...
    if labelling.className == "LabelingScheme":
        if resonanceSet:
            ...
            chemCompLabel = labelling.findFirstChemCompLabel(ccpCode=ccpCode, molType=molType)

            if not chemCompLabel:
                natAbun = resonance.root.findFirstLabelingScheme(name="NatAbun")

                if natAbun:
                    chemCompLabel = natAbun.findFirstChemCompLabel(ccpCode=ccpCode, molType=molType)
            ...
            if chemCompLabel:
                ...
                for atomSet in atomSets:
                    ...
                    for atom in atoms:
                        ...
                        fracDict = getIsotopomerSingleAtomFractions(isotopomers, atom.name, subType)
            ...
            if spinSystem and spinSystem.ccpCode:
                ...
                if chemCompLabel:
                    ...
                    for atomName in atomNames:
                        fracDict = getIsotopomerSingleAtomFractions(isotopomers, atomName, 1)
    ...
    if resonanceSetA and resonanceSetB:
        ...
        if labelling.className == "LabelingScheme":
            ...
            if residueA is residueB:
                chemCompLabel = findFirstChemCompLabel(ccpCode=residueA.ccpCode, molType=residueA.molType)

                if not chemCompLabel:
                    natAbun = resonanceA.root.findFirstLabelingScheme(name="NatAbun")

                    if natAbun:
                        chemCompLabel = natAbun.findFirstChemCompLabel(
                            ccpCode=residueA.ccpCode, molType=residueA.molType
                        )
                if not chemCompLabel:
...
def getSyntheticShiftLists(nmrProject):
    ...
    result = [
        x
        for x in nmrProject.sortedMeasurementLists()
        if x.className == "ShiftList" and x.findFirstExperiment() is None
...
def getBoundResonances(resonance, recalculate=False, contribs=None, doWarning=False, recursiveCall=False):
    """
                        ...
                        for resonance2 in resonanceSet2.resonances:
                            if resonance2 is resonance:  # should not happen
                                if resonance not in funnyResonances:
                                    print(
                                        "WARNING: in getBoundResonances(): resonance %d tried to be linked to itself"
                                        % resonance.serial
                                    )
    ...
    if foundBothPaired and not [x for x in expResonances if x in pairResonances]:
        ...
        if recursiveCall:
            ...
            if len(resons) > 1:
                ...
                for reson in resons:
                    boundResons = getBoundResonances(
                        reson, recalculate=True, contribs=contribs, doWarning=False, recursiveCall=True
                    )
        ...
        if resonance.isotopeCode == "1H":
            ...
            for resonance2 in resonanceSet.sortedResonances():
                if resonance2.shifts:
                    data.append((0, "%f%d" % (resonance2.findFirstShift().value, resonance2.serial), resonance2))
                else:
                    data.append((1, resonance2.serial, resonance2))
...
def getAliasedPeakDimPositions(peakDim, shiftRanges=None, ppm=None):
    ...
    if not shiftRanges:
        shiftRanges = [
            getPeakDimFullShiftRange(peakDim),
        ]
...
def getPeakDimAtomTuple(peakDim):
    ...
    for contrib in contribs:
        ...
        if resonanceSet:
            ...
            if chemAtomSet and key in chemAtomSetDict:
...
def findMatchingPeakDimShifts(peakDim, shiftRanges=None, tolerance=None, aliasing=True, findAssigned=False, ppm=None):
    """
    ...
    if ppm is None:
        ...
        if ppm is None:
            showWarning("peakDim Failure", "peakDim contains no realValue - possibly wrong experimentType")
            return shifts
    ...
    for position in positions:
        shifts.extend(findMatchingShifts(dataDimRef, position, tolerance=tolerance, findAssigned=findAssigned))
...
def assignAtomsToRes(atomSets, resonance, resonanceSet=None):
    ...
    if resonance.isotopeCode:
        if resonance.isotopeCode != "unknown":
            element = re.match(r"\d+([A-Z]\D*)", resonance.isotopeCode).group(1)
    ...
    if oldResonanceSet and (oldResonanceSet is not resonanceSet):
        if len(oldResonanceSet.resonances) == 1:
            ...
            nmrProject.newResonanceSet(atomSets=oldAtomSets, resonances=oldResonances)
    ...
    if resonanceSet:
        ...
        resonanceSet = nmrProject.newResonanceSet(
            atomSets=atomSets,
            resonances=[
                resonance,
            ],
...
def assignTentativeAtoms(atomSets, resonance, weight=1.0, doWarnings=False):
    ...
    None
    """
    spinSystem = resonance.resonanceGroup or resonance.nmrProject.newResonanceGroup(
        resonances=[
            resonance,
        ]
    )
...
def assignTentativeSpinSystemResidues(spinSystem, residues, weight=1.0, doWarnings=False):
    ...
    for residue in residues:
        residueProb = spinSystem.findFirstResidueProb(possibility=residue) or spinSystem.newResidueProb(
            possibility=residue
        )
...
def assignResToDim(peakDim, resonance=None, contrib=None, tolerance=None, doWarning=True, peakContribs=None):
    """
    ...
    Nmr.PeakDimContrib
    """
    from ccpnmr.analysis.core.PeakBasic import findNumAliasing, setPeakDimNumAliasing
    ...
    if not shiftList:
        ...
        if not shiftList:
            shiftList = nmrProject.newShiftList(name="ShiftList 1", details="Assignment Default", unit="ppm")
...
def newPeakDimContrib(peakDim, resonance, peakContribs=None):
    ...
    if len(peakContribs) < 1:
        ...
        return peakDim.newPeakDimContrib(resonance=resonance, peakContribs=peakContribs)
...
def mergeResonances(resonanceB, resonanceA):
    ...
    if isotopeA and isotopeB:
        if isotopeA != isotopeB:
            showWarning("Resonance Merge Failure", "Attempt to merge resonances with different isotope codes")
    ...
    controlData = {
        "findFirstMeasurement": ("jCouplings", "noes", "rdcs", "dipolarRelaxations"),
        "findFirstDerivedData": ("isotropicS2s", "spectralDensities", "datums"),
    ...
    for appData in resonanceA.applicationData:
        matchAppData = resonanceB.findFirstApplicationData(application=appData.application, keyword=appData.keyword)
        if matchAppData:
...
def mergeSpinSystems(spinSystemB, spinSystemA):
    ...
    for residueProb in list(spinSystemB.residueProbs):
        ...
        if not spinSystemA.findFirstResidueProb(possibility=residue):
            spinSystemA.newResidueProb(possibility=residue, weight=residueProb.weight)
...
def propagatePeakAssignments(peaks, refPeak=None, cleanNonRef=False, tolerances=None, warnUnalias=False):
    """
            ...
            if dataDimRef:
                ...
                if closeResonances:
                    ...
                    for resonance in assignResonances:
                        assignResToDim(peakDim, resonance, tolerance=tolerance, doWarning=warnUnalias)
...
def makeSeqSpinSystemLink(spinSystemA, spinSystemB, delta=1):
    ...
    link = spinSystemA.findFirstResonanceGroupProb(linkType="sequential", possibility=spinSystemB)
    ...
    if link:
        ...
        link = spinSystemA.newResonanceGroupProb(
            linkType="sequential", isSelected=True, possibility=spinSystemB, sequenceOffset=delta
        )
...
def getSpinSystemResidues(spinSystem):
    ...
    if residue is not None:
        ...
        result = [x.possibility for x in spinSystem.sortedResidueProbs() if x.weight]
    #
    return result
...
def getSpinSystemChemComps(spinSystem):
    ...
    if chemComp is not None:
        ...
        result = [x.possibility for x in spinSystem.sortedResidueTypeProbs() if x.weight]
    #
    return result
...
def estimateAssignmentTolerances(dataSource, defPoints=1.0, minTol=0.02):
    ...
    for ii, dataDim in enumerate(dataSource.sortedDataDims()):
        ...
        if not useTol:
            ...
            dataDimRefs = [x for x in dataDim.dataDimRefs if x.expDimRef.measurementType.lower() == "shift"]
