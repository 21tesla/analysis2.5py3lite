...
from ccpnmr.analysis.core import ExperimentBasic, Util
...
def getDefaultAspectRatio(axisTypes):
    ...
    weight = {"1H": 1.0, "13C": 10.0, "DQ13C": 10.0, "15N": 8.0}
...
def cloneSpectrumWindow(window, name=None):
    ...
    newWindow = createSpectrumWindow(project, name, axisTypes, sizes=sizes, regions=regions)
...
"""
                    if i > 0:
                        newAxisPanel.newAxisRegion(
                            region=axisRegion.region,
                            size=axisRegion.size,
                            isActive=axisRegion.isActive,
    ...
    for j, windowPane in enumerate(newPanes):
        ...
        for newView in windowPane.spectrumWindowViews:
            ...
            for i in range(len(oldAxisMappings)):
                if newAxisMappings[i].analysisDataDim is not oldAxisMappings[i].analysisDataDim:
            ...
            if needAxisMappingChange:
                ...
                for oldAxisMapping in oldAxisMappings:
                    makeMapping(label=oldAxisMapping.label, analysisDataDim=oldAxisMapping.analysisDataDim)


def createSpectrumWindow(project, name, axisTypes, spectrum=None, sizes=None, ncols=1, nrows=1, regions=None, **kw):
    """
    ...
    for p, paneAxisTypes in enumerate(axisTypes):
        ...
        windowPane = window.newSpectrumWindowPane(aspectRatio=aspectRatio, name=name)
        ...
        if regions:
            ...
            for n in range(2):
                ...
                if (
                    n == 1
                    and axisType.name == "value"
                    and spectrum
                    and hasattr(spectrum, "block_file")
                    and spectrum.block_file
                ):
                    ...
                    # is below still ok?
                    panelType = Util.findPanelType(project, axisType, sortedAxisPanels[:n])
        ...
        for n in range(2):
            ...
            axisPanel.panelType = Util.findPanelType(project, axisType, sortedAxisPanels[:n])
    ...
    for function, shortcut in (
        ...
        if function not in functions:
            if shortcut in shortcuts:
                shortcut = None
            macro = analysisProfile.newMacro(
                name=function, path=path, module=module, function=function, ordering=ordering, shortcut=shortcut
            )
            ...
            if axisPanel1.axisUnit:
                for j in range(i + 1, N):
                    ...
                    if axisPanel2.axisUnit:
                        if (axisPanel1.axisUnit.unit == axisPanel2.axisUnit.unit) and (
                            axisPanel1.axisType == axisPanel2.axisType
                        ):
            ...
            for axisPanel in windowPane.axisPanels:
                ...
                for dataDim in dataDims:
                    if isinstance(dataDim, Nmr.FreqDataDim):
                        ...
                        if (
                            dataDimRef
                            and (expDimRef.isotopeCodes == isotopes)
...
def displayStrips(analysis, positions, orthoPositions=None, spectrum=None, windowPane=None):
    """
...
def getSpinSystemWindowShifts(spinSystems, windowPane, shiftList=None):
    ...
    List of Nmr.Shifts
    """
    from ccpnmr.analysis.core.AssignmentBasic import getBoundResonances, getDataDimFullShiftRange
            ...
            if shift:
                ...
                for i, zIsotopes in enumerate(zIsotopeList):
                    ...
                    if resonance.isotopeCode in zIsotopes:
                        if (shift.value >= ranges[i + 2][0]) and (shift.value <= ranges[i + 2][1]):
...
def displaySpinSystemStrips(analysis, spinSystems, windowPane, shiftList=None, splitIntoCells=False):
    """
    ...
    shifts, orthoShifts = getSpinSystemWindowShifts(spinSystems, windowPane, shiftList=shiftList)
...
def getAxisRegionRegion(axisRegion):
    ...
    if hasattr(window, "inOverrideMode") and window.inOverrideMode and axisRegion.overrideRegion:
...
def setTiedAxisPanels(axisPanel, tiedAxisPanels):
    ...
    values = [
        "%s:%s" % (tiedAxisPanel.spectrumWindowPane.spectrumWindow.name, tiedAxisPanel.label)
