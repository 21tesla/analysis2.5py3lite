...
from ccpnmr.analysis.core.PeakBasic import getPeakAnnotation, getPeakDimPosition
from ccpnmr.analysis.core.PeakDrawUtil import getCPeakDrawMethod, peak_draw_methods
from ccpnmr.analysis.core.PeakFindParams import getPeakFindBoxwidth, getPeakFindParams
...
from memops.universal.Util import arrayOfIndex, cumulativeProductArray, formatDecimals, isWindowsOS
...
class WindowDraw:
    ...
    def getMinMaxFreq(self, label, view):
        ...
        if minAliasedFreq is None:
            minAliasedFreq = convertPosition(
    ...
    def drawViewTileReal(
        self, handler, view, contourLevels, contourStyle, worldPointRanges, spectrumPointRanges, row, components=None
    ):
                        ...
                        if xMapping and yMapping:
                            ...
                            if hasattr(dataSource, "block_file") and dataSource.block_file:
                                ...
                                if not hasattr(self, "_tempContourBlockFile"):
                                    self._tempContourBlockFile = getBlockFile(
                                        dataSource, mem_cache=mem_cache, writeable=writeable
                                    )
                                if self._tempContourBlockFile:
                                    contourFile = view.contourFile = ContourFile(
                                        xdim, ydim, self._tempContourBlockFile, mem_cache
                                    )
    ...
    def drawViewTilePeaks(
        ...
        for winPeakList in view.windowPeakLists:
            ...
            if self.isWinPeakListDrawn(winPeakList):
                ...
                winPeakList.cWinPeakList.drawPeaks(
                    handler,
                    xdim,
                    ydim,
                    xpix,
        ...
        for sample in range(nsampleRegions):
            ...
            for tile in range(ntiles):
                ...
                for analysisDataDim in analysisSpectrum.analysisDataDims:
                    ...
                    if not object or (hasattr(object, "doubleBuffer") and object.doubleBuffer):
                        drawViewTile(
                            handler,
                            object,
                            view,
                        ...
                        self.after_idle(
                            lambda contourLevels=contourLevels, contourStyle=contourStyle, worldPointRanges=worldPointRanges, spectrumPointRanges=spectrumPointRanges: (
                                drawViewTile(
                                    handler,
                                    object,
                                    view,
    ...
    def setHandlerFont(self, handler, name, size):
        ...
        if isinstance(handler, PrintHandler) and (
            isinstance(handler.outputHandler, Pdf) or isinstance(handler.outputHandler, PostScript)
        ):
    ...
    def drawMarkDimRuler(self, handler, object, axisLabel, dashLength, gapLength, x0, x1, y0, y1):
        ...
        for markDim in mark.markDims:
            for worldPanel in worldPanels:
                if markDim.axisType == worldPanel.axisType:
                    drawMarkDimRuler(handler, markDim, worldPanel.label, dashLength, gapLength, x0, x1, y0, y1)
    ...
    def drawRuler(self, handler, ruler, x0, x1, y0, y1):
        ...
        for worldPanel in worldPanels:
            if ruler.panelType == worldPanel.panelType:
                drawMarkDimRuler(handler, ruler, worldPanel.label, dashLength, gapLength, x0, x1, y0, y1)
