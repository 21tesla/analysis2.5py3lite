...
from memops.universal.PrintTicks import Bottom, Left, PrintTicks, Right, Top
...
def printWindow(
    windowDraw, outputHandler, tick_location, tick_placement, aspectRatio, tick_font=None, major_minor_dict=None
):
    ...
    for yAxisRegion in yRegions:
        ...
        for xAxisRegion in xAxisPanel.sortedAxisRegions():
            ...
            handler = PrintHandler(
                outputHandler, xRegion=(x0, x1), yRegion=(y0, y1), xSize=xSize, ySize=ySize, x=x, y=y
            )
    ...
    if hasattr(appObject, name):
        try:
            ...
            if key != "TickPlacement":
                print(
                    'Warning: print option "%s" not stored in data model as "%s" because of exception: %s'
                    % (key, value, str(e))
                )
