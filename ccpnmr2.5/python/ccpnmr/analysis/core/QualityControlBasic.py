...
def analyseNoeAssignments(molSystem, peakLists=None):
    ...
    if len(molSystem.chains) > 1:
        residues = [
            ("%s%5.5d" % (r.chain.code, r.seqCode), "%s%d%s" % (r.chain.code, r.seqCode, r.ccpCode), r)
            for r in contact.keys()
        ]
    else:
        residues = [("%5.5d" % (r.seqCode,), "%d%s" % (r.seqCode, r.ccpCode), r) for r in contact.keys()]
    ...
    if len(molSystem.chains) > 1:
        for residue, datum in data:
            residues2 = [("%s%5.5d" % (r.chain.code, r.seqCode), "%s%d" % (r.chain.code, r.seqCode)) for r in datum[-1]]
        ...
        for residue, datum in data:
            residues2 = [("%5.5d" % (r.seqCode), "%d" % (r.seqCode)) for r in datum[-1]]
            residues2.sort()
            datum[-1] = " ".join([x[1] for x in residues2])
...
def analyseAssignmentCompleteness(molSystem, shiftList, residueSelection=None, excludeWaterExchangeable=True):
