...
def runMultiProtocol(
    ...
    intIo.setupMultiInteractive(argServer, procNames, prelimProtocolName, executeProc=True)


def setupMultiProtocol(
    argServer,
    procNames=(
        "CYANA_SS1",
        "CYANA_PEAKLIST",
...
def runCandid(argServer):
    ...
    intIo.runSingleInteractive(argServer, protocolName="UNIO_CANDID_XPLOR_TEST", prelimProtocolName=prelimProtocolName)
...
def setupCyanaCalculation(argServer):
    ...
    intIo.setupSingleInteractive(
        argServer,
        protocolName="CYANA_SS4",
    )


def setupCyanaCalculationDialogue(argServer):

    intIo.setupCyana2CcpnDialogue(
        argServer,
        protocolName="CYANA_SS4",
    )
...
def runCyana2CcpnDialogue(argServer):
    ...
    calculationData = intIo.runCyana2CcpnDialogue(
        argServer,
        protocolName="CYANA_SS4",
    )
...
def runAsdpXplor(argServer):
    ...
    intIo.runSingleInteractive(argServer, protocolName="ASDP_XPLOR", prelimProtocolName=prelimProtocolName)
...
def setupCandid(argServer):
    ...
    intIo.setupSingleInteractive(
        argServer, protocolName="UNIO_CANDID", prelimProtocolName=prelimProtocolName, masterRun=run
    )
...
def setupCyanaPeaklist(argServer):
    ...
    intIo.setupSingleInteractive(
        argServer, protocolName="CYANA_PEAKLIST", prelimProtocolName=prelimProtocolName, masterRun=run
    )
...
def setupAsdpXplor(argServer):
    ...
    intIo.setupSingleInteractive(
        argServer, protocolName="ASDP_XPLOR", prelimProtocolName=prelimProtocolName, masterRun=run
    )


def setupRosetta(argServer):

    intIo.setupSingleInteractive(argServer, protocolName="ROSETTA_MULTI", prelimProtocolName=prelimProtocolName)
