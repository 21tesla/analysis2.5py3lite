...
class RelaxationAnalysisPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        options = [
            "%s vs %s Scatter" % (T1, T2),
            "%s,%s & NOE Graphs" % (T1, T2),
            "%s/%s Graph" % (T1, T2),
            "%s Estimate Graph" % S2,
            "Options",
        ]
        self.tabbedFrame = TabbedFrame(guiFrame, options=options, callback=self.toggleTab, grid=(1, 0))
        ...
        self.s2Graph = ScrolledGraph(
            frameE,
            title="%s vs Residue Sequence" % S2,
        ...
        self.teGraph = ScrolledGraph(
            frameE,
            title="%s vs Residue Sequence" % Te,
            xLabel="Residue number",
            yLabel="%s (ps)" % Te,
            width=500,
            height=150,
        ...
        self.rexGraph = ScrolledGraph(
            frameE,
            title="%s vs Residue Sequence" % "Rex",
            xLabel="Residue number",
            yLabel="Rex",
            width=500,
            height=150,
        ...
        label = Label(frame, text="(Order Parameter Lines)", grid=(0, 0), gridSpan=(1, 2))
        ...
        label = Label(frame, text="(Rotational Correlation Time Lines)", grid=(0, 0), gridSpan=(1, 2))
    ...
    def draw(self):
        ...
        self.t1t2Graph.update(
            self.t1List, self.t2List, cSize, cDist, sf, lenNh, ict, csaN, paramsS2, paramsRct, tBounds
        )
    ...
    def mc(self):
        ...
        for chain in chains:
            ...
            print(
                "Mean A S2:%5.3f Te:%5.1f Tm:%5.3f Rex:%5.3f T1:%5.3f %5.3f T2:%5.3f %5.3f NOE:%5.3f %5.3f %e %6d"
                % data
            )
            ...
            for j in jj:
                ...
                residue = residues[j]
                print(
                    "%3d%s" % (residue.seqCode, residue.ccpCode),
                )
...
class T1VersesT2Plot(ScrolledGraph):
    ...
    def update(self, t1List, t2List, cSize, cDist, sf, lenNh, ict, csaN, paramsS2, paramsRct, graphBounds):
    ...
    def draw(self):
        ...
        for label, x, y in self.outlierLabels:
            ...
            item = createText(x0 + 8, y0, text=label, fill="#000000", anchor="w")
    ...
    def drawContours(self, linewidth=1):
        ...
        for label, x, y in s2Labels:
            ...
            item = createText(x0 + 2, y0, text=label, fill=self.contourColorA, anchor="w")
            self.contourItems.add(item)
        ...
        for label, x, y in rctLabelsR:
            ...
            item = createText(x0 + 2, y0, text=label, fill=self.contourColorB, anchor="w")
            self.contourItems.add(item)
        ...
        for label, x, y in rctLabelsT:
            ...
            item = createText(x0, y0 + 2, text=label, fill=self.contourColorB, anchor="s")
            self.contourItems.add(item)
