...
def loadProjDefinitionFile(argServer, inputFileName=None, exptName=None):
    ...
    if format not in supportedFormats:
        print("ERROR Unknown format; %s. Supported formats: %s" % (format, supportedFormats))
        ...
        for tagin, tagout in dimParNameMap.items():
            ...
            if val is not None:
                if numDims is None:
                    ...
                    print("ERROR for %s: numDims inconsistent, should be %d" % (tagin, numDims))
                    return
    ...
    # get experiment name
    if exptName is None:
        exptName = argServer.askString(message="New Experiment Name", default=defaultExpName)
    ...
    open_spectra.scrolledMatrix.update(objectList=objectList, textMatrix=textMatrix)

    if useScalingFactors:
        extraData["scalingFactors"] = scalingFactors[0]

    refSpec = open_spectra.openSpectrum(exptName, specNames[0], fileNames[0], extraData=extraData)
    if not refSpec:
        return
    ...
    expDimRefs = [xdr for xd in refExp.sortedExpDims() for xdr in xd.sortedExpDimRefs()]
    ...
    for ii, fullPath in enumerate(fileNames):
        ...
        useDataUrl, filePath = getDataStoringFromFilepath(
            memopsRoot, fullPath, preferDataUrls=preferDataUrls, keepDirectories=keepDirectories
        )
        ...
        if format == "Bruker":
            ...
            dataPath = getMatrixFilePath(os.path.join(urlPath, filePath), refDataStore.numDims)
            path = dataPath[len(urlPath) :]
        elif format == "Azara":
            raise NotImplementedError("Azara type projection series not yet implemented")
        else:
            path = filePath
        copyPars = {"dataUrl": useDataUrl, "path": path, "nmrDataSources": []}
        newDataStore = copySubTree(refDataStore, refDataStore.dataLocationStore, topObjectParameters=copyPars)
        ...
        if useScalingFactors:
            ...
            for jj in range(len(dataDims) - 1, len(scalingFactor)):
                ...
                if dimScaling is None:
                    lastDataDim.newDimensionScaling(scalingFactors=(scalingFactor[jj],), expDimRef=expDimRef)
                else:
