...
from ccpnmr.analysis.core.ExperimentBasic import getPrimaryDataDimRef, getSpectraByType, getSpectrumIsotopes
from ccpnmr.analysis.core.MoleculeBasic import makeResidueAtomSets
from ccpnmr.analysis.core.PeakBasic import findPeaks, searchPeaks
...
def pickAssignSpecFromRoot(argServer, rootPeakList=None, targetPeakList=None):
    ...
    for isotope in rootIsotopes:
        tolerance = argServer.askFloat("%s tolerance" % isotope, toleranceDict.get(isotope, 0.1)) or toleranceDict.get(
            isotope, 0.1
        )
...
def assignPeaksUnambiguous(argServer, peaks=None, shiftRanges=None, tolerances=None, aliasing=True, findAssigned=False):
    assignPeaksAutomatic(
        argServer,
        ambiguous=False,
        peaks=peaks,
...
def assignPeaksAutomatic(
    argServer, ambiguous=True, peaks=None, shiftRanges=None, tolerances=None, aliasing=True, findAssigned=False
):
    ...
    for peak in peaks:
        resonances = possiblePeakAssigments(
            peak, shiftRanges=shiftRanges, tolerances=tolerances, aliasing=aliasing, findAssigned=findAssigned
        )
