...
from ccpnmr.analysis.core.AssignmentBasic import getShiftLists, isResidueAssigned
...
class ResidueInfoPopup(BasePopup):
    ...
    * Translate: Middle button click & drag + Control key
    ...
    def body(self, guiFrame):
        ...
        self.chainPulldown = PulldownList(guiFrame, callback=self.changeChain, grid=(row, 1), tipText=tipText)
        ...
        self.ccpCodePulldown = PulldownList(guiFrame, callback=self.changeCcpCode, grid=(row, 3), tipText=tipText)

        label = Label(guiFrame, text="Shift List:", grid=(row, 4))
        tipText = "Selects which shiftlist to get chemical shift values from"
        self.shiftListPulldown = PulldownList(guiFrame, callback=self.changeShiftList, grid=(row, 5), tipText=tipText)

        row += 1
        guiFrame.expandGrid(row, 6)

        self.residueFrame = LabelFrame(guiFrame, text="%s Information" % (self.ccpCode), grid=(row, 0), gridSpan=(1, 3))
        self.residueFrame.expandGrid(1, 0)

        self.atomsFrame = LabelFrame(
            guiFrame, text="Atom Information & Idealised Structure", grid=(row, 3), gridSpan=(1, 5)
        )
        ...
        self.assignedLabel = Label(self.residueFrame, text=self.assignedText, grid=(0, 0), sticky="ew", tipText=tipText)
        ...
        self.neighbourMatrix = ScrolledMatrix(
            self.residueFrame,
            initialRows=8,
        ...
        self.viewResidueFrame = ViewResidueFrame(
            self.atomsFrame, residue=self.residue, project=self.project, grid=(0, 0), tipText=tipText
        )
        ...
        self.utilButtons = UtilityButtonList(
            guiFrame, helpUrl=self.help_url, commands=commands, texts=texts, grid=(0, 7), tipTexts=tipTexts
        )
