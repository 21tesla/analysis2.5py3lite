...
from ccpnmr.analysis.core.Util import reloadMacro, runMacro
...
class EditProfilesPopup(BasePopup):
    ...
    **Colour Schemes**
    ...
    def body(self, guiFrame):
        ...
        self.profilePulldown = PulldownList(frameA, callback=self.selectProfile, grid=(0, 1), tipText=tipText)

        tipTexts = [
            "Copy the currently displayed profile settings into a differently named set",
        ...
        self.profileButtons = ButtonList(frameA, texts=texts, commands=commands, grid=(0, 2), tipTexts=tipTexts)

        tipTexts = [
            "The short name of the parameter that the user may set a value for",
        ...
        self.profileMatrix = ScrolledMatrix(
            frameA,
        ...
        self.macroMatrix = ScrolledMatrix(
            frameB,
            headingList=headingList,
            editWidgets=editWidgets,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = [
            "Reload the selected macro function; attempts to re-import the Python module",
        ...
        commands = [self.reloadSelectedMacro, self.runSelectedMacro, self.removeMacro, self.addMacro]
        self.bottomMacroButtons = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(1, 0))
        ...
        self.molTypePulldown = PulldownList(
            frameD, texts=molTypes, callback=self.changeMolType, grid=(row, 1), tipText=tipText
        )
        ...
        tipTexts = [
            "The official CCPN code for the residue ",
        ...
        self.resProfileMatrix = ScrolledMatrix(
            frameD,
            headingList=headingList,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        row += 1
        tipTexts = [
            "Manually trigger a complete update of all on-spectrum peak annotations in light of any residue code changes",
        ]
        commands = [
            self.refreshPeakAnnotations,
        ]
        texts = [
            "Update Peak Annotations",
        ]
        buttons = ButtonList(frameD, texts=texts, commands=commands, grid=(row, 0), gridSpan=(1, 2), tipTexts=tipTexts)
        ...
        self.colorSchemeMatrix = ScrolledMatrix(
            frameE,
        ...
        tipTexts = [
            "Create a new color scheme; the user is prompted for a name",
        ...
        self.colorSchemeButtons = ButtonList(frameE, texts=texts, grid=(1, 0), commands=commands, tipTexts=tipTexts)
        ...
        self.colorTable = ScrolledMatrix(
            labelFrame,
            headingList=headings,
        ...
        tipTexts = [
            "Add a new colour element to the current scheme, defaults to black but can be changed",
        ...
        self.colorButtons = ButtonList(labelFrame, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in (
            ...
            "setBgColor",
            "setFgColor",
            "setFont, setGraphicsHandler",
            "setWebBrowser",
            "setUseGlobalShortcuts",
            "setUseCrosshair",
            "setTwoCharShortcuts",
            "setTransientWindows",
            "setTransientDialogs",
    ...
    def copyProfile(self, profile=None):
        ...
        if profile:
            ...
            profile = copySubTree(profile, self.project, topObjectParameters={"name": name}, objectMap={})
    ...
    def updateProfile(self):
        ...
        if profile:
            ...
            textMatrix.append(
                ["Window Background", profile.bgColor, "Default colour for the background of spectrum windows"]
            )
            ...
            textMatrix.append(
                ["Window Foreground", profile.fgColor, "Default colour for the foreground of spectrum windows"]
            )
            ...
            objectList.append(
                [profile, "marksColor", self.pulldownList, self.getProfileColorScheme, self.setPulldownMenu]
            )
            colorMatrix.append(nullColors)
            ...
            objectList.append(
                [profile, "rulersColor", self.pulldownList, self.getProfileColorScheme, self.setPulldownMenu]
            )
            colorMatrix.append(nullColors)
            ...
            textMatrix.append(
                ["Graphics Handler", profile.graphicsHandler, "Whether to use OpenGL or Tk for contour graphics"]
            )
            objectList.append(
                [profile, "graphicsHandler", self.pulldownList, self.getGraphicsHandler, self.setPulldownMenu]
            )
            colorMatrix.append(nullColors)
            ...
            textMatrix.append(
                ["Transient Dialogs?", profile.transientDialogs, "Transient dialogues have no toolbar representation"]
            )
            objectList.append([profile, "transientDialogs", None, self.toggleBoolean, None])
            colorMatrix.append(nullColors)

            textMatrix.append(
                ["Transient Windows?", profile.transientWindows, "Transient windows have no toolbar representation"]
            )
            ...
            textMatrix.append(
                [
                    ...
                    profile.twoCharShortcuts,
                    "Whether to use two-character shortcuts for macros etc.",
                ]
            )
            ...
            textMatrix.append(
                ["Use Crosshairs?", profile.useCrosshair, "Whether to have crosshairs in the spectrum displays"]
            )
            ...
            textMatrix.append(
                [
                    ...
                    profile.useGlobalShortcuts,
                    "When not global, keyboard shortcuts\nonly act in the active window",
                ]
            )
            ...
            textMatrix.append(
                [
                    "Y axis decimals expand?",
                    value,
                    "Whether decimal places in y-axis tick labels expand as you zoom in\n(you must save and reopen project for this to take effect)",
                ]
            )
            ...
            textMatrix.append(
                ["Web Browser", profile.webBrowser, "The default web browser to use for help information etc."]
            )
            objectList.append([profile, "webBrowser", self.pulldownList, self.getWebBrowser, self.setPulldownMenu])
            colorMatrix.append(nullColors)

            textMatrix.append(
                ["Send Bug Reports?", bugReportsDict[profile.sendBugReports], "Whether bug reports should be sent."]
            )
            objectList.append([profile, "sendBugReports", self.pulldownList, self.getBugReports, self.setPulldownMenu])
            colorMatrix.append(nullColors)

        self.profileMatrix.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
    ...
    def updateMacros(self):
        ...
        self.macroMatrix.update(objectList=macros, colorMatrix=colorMatrix, textMatrix=textMatrix)
    ...
    def updateResCodes(self, obj=None):
        ...
        self.resProfileMatrix.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateSchemeList(self, *extra):
        ...
        self.colorSchemeMatrix.update(objectList=schemes, textMatrix=textMatrix)
    ...
    def updateColorList(self, *extra):
        ...
        self.colorTable.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
    ...
    def updateExpProfiles(self, obj=None):
        ...
        if self.profile:
            ...
            for refExpProfile in expProfiles:
                ...
                datum = [refExpProfile.name, refExpText, nColorProfiles]

                textMatrix.append(datum)

        self.expProfileMatrix.update(objectList=expProfiles, textMatrix=textMatrix)
    ...
    def updateExpColors(self, obj=None):
        ...
        self.expColorMatrix.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
