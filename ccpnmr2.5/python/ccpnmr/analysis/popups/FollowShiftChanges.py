...
from ccpnmr.analysis.core.AssignmentBasic import getResonanceName, propagatePeakAssignments
from ccpnmr.analysis.core.DataAnalysisBasic import (
    DataFitting,
    functionFitData,
    getFitErrorMethod,
    getFitErrorMethods,
    getFitMethodInfo,
    getNmrExpSeriesSampleConditions,
    makeDataList,
    setFitErrorMethod,
)
from ccpnmr.analysis.core.ExperimentBasic import getSpectrumIsotopes, newShiftList
from ccpnmr.analysis.core.PeakBasic import arePeaksAssignedSame, findClosePeaks, findSameAssignmentPeaks
...
class FollowShiftChangesPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.peakListPulldown = PulldownList(frame, self.changePeakList, grid=(0, 1), tipText=tipText)

        label = Label(frame, text=" NMR Experiment Series:", grid=(1, 0), sticky="e")
        tipText = (
            "Selects which series of NMR experiments to perform the peak-following and equation fitting analysis for"
        )
        self.expSeriesPulldown = PulldownList(frame, self.changeExpSeries, grid=(1, 1), tipText=tipText)
        ...
        label = Label(frame, text="Fitting Function:", grid=(0, 0), sticky="e")
        ...
        self.fitFuncPulldown = PulldownList(frame, self.changeFitFunc, grid=(0, 1), gridSpan=(1, 2), tipText=tipText)

        label = Label(frame, text="Followed Dimensions:", grid=(1, 0), sticky="e")
        ...
        self.dataDimsPulldown = PulldownList(frame, self.changeDataDim, grid=(1, 1), tipText=tipText)
        ...
        self.fitErrorPulldown = PulldownList(
            frame, callback=self.setFitErrorMethod, texts=fitErrorMethods, index=ind, grid=(2, 1), tipText=tipText
        )

        label = Label(frame, text="Assign groups?", grid=(3, 2), sticky="e")
        ...
        self.assignSelect = CheckButton(frame, selected=-True, grid=(3, 3), tipText=tipText)

        label = Label(frame, text="  Ignore Zero Merit Peaks?", grid=(4, 2), sticky="e")
        tipText = (
            "When grouping peaks along resonance trajectories, whether to ignore peaks with zero figure-of-merit value"
        )
        self.meritSelect = CheckButton(frame, selected=True, grid=(4, 3), tipText=tipText)

        label = Label(frame, text="  Shift Error (points):", grid=(3, 0), sticky="e")
        ...
        self.shiftErrorEntry = FloatEntry(frame, text=4.0, width=8, grid=(3, 1), tipText=tipText)

        self.proteinConcLabel = Label(
            frame,
            text="Binding site concentration:",
        )  # No grid
        ...
        self.proteinConcEntry = FloatEntry(
            frame, text=None, width=8, tipText=tipText, returnCallback=self.updateAfter
        )  # No grid
        ...
        self.minGradientEntry = FloatEntry(frame, text=None, width=8, grid=(4, 1), tipText=tipText)
        ...
        self.maxGradientEntry = FloatEntry(frame, text=None, width=8, grid=(5, 1), tipText=tipText)
        ...
        self.tolHydrogenEntry = FloatEntry(frame, text=ISOTOPE_WEIGHTS["1H"], width=8, grid=(0, 2), tipText=tipText)
        ...
        self.tolNitrogenEntry = FloatEntry(frame, text=ISOTOPE_WEIGHTS["15N"], width=8, grid=(0, 4), tipText=tipText)
        ...
        self.tolCarbonEntry = FloatEntry(frame, text=ISOTOPE_WEIGHTS["13C"], width=8, grid=(0, 6), tipText=tipText)
        ...
        self.stepHydrogenEntry = FloatEntry(frame, text=MAX_STEP_SIZES["1H"], width=8, grid=(1, 2), tipText=tipText)
        ...
        self.stepNitrogenEntry = FloatEntry(frame, text=MAX_STEP_SIZES["15N"], width=8, grid=(1, 4), tipText=tipText)
        ...
        self.stepCarbonEntry = FloatEntry(frame, text=MAX_STEP_SIZES["13C"], width=8, grid=(1, 6), tipText=tipText)
        ...
        self.bottomButtons = ButtonList(frameB, commands=commands, texts=texts, grid=(row, 0), tipTexts=tipTexts)
        ...
        tipTexts = ["", "", "", ""]
        texts = ["Group & Fit Peaks", "Re-fit All Groups", "Edit NMR Series", "Export Shifts"]
        commands = [self.groupPeaks, self.calculateAllFits, self.editExpSeries, self.exportShifts]
        self.buttonList = ButtonList(guiFrame, grid=(1, 0), tipTexts=tipTexts, commands=commands, texts=texts)
    ...
    def warnFailedPeakGroups(self, failedPeaks):
        ...
        showWarning(
            "Fit failure", "Fit failed for %d groups at reference positions %s" % (numFail, failText), parent=self
        )
    ...
    def getHeadingList(self, numDim):
        ...
        tipTexts.extend(
            [
                "The isotope-weighted chemical shift path length; following each pair of points in the trajectory of the peak group",
        ...
        for i in range(numParams):
            ...
            tipTexts.append(
                'The estimated value of parameter "%s", obtained by fitting the selected equation to the chemical shift data'
                % letter
            )
    ...
    def showFunctionFit(self, noOpen=False):
        ...
        if self.peakGroup:
            ...
            if self.fitGraphPopup:
                self.fitGraphPopup.update(
                    dataFitting=self.peakGroup, xLabel=xLabel, yLabel=yLabel, graphTitle=title, force=True
                )
                if not noOpen:
                    self.fitGraphPopup.open()
            else:
                self.fitGraphPopup = EditFitGraphPopup(
                    self,
                    self.peakGroup,
                    self.getPeakSeriesData,
                    self.updateGroup,
                    xLabel,
                    yLabel,
    ...
    def update(self):
        ...
        self.scrolledMatrix.update(
            textMatrix=textMatrix, objectList=objectList, headingList=headingList, tipTexts=tipTexts
        )
    ...
    def makeDataList(self):
        ...
        if self.peakGroups and self.expSeries:
            ...
            for dataFitting in self.peakGroups:
                ...
                shiftList = peak.peakList.dataSource.experiment.shiftList or peak.topObject.findFirstMeasurementList(
                    className="ShiftList"
                )
            ...
            dataList = makeDataList(
                name, unit, values, errors=errors, peaks=peakGroups, peakDims=peakDimGroups, measurements=shiftGroups
            )
    ...
    def changeExpSeries(self, expSeries):
        ...
        if self.expSeries is not expSeries:
            ...
            if (
                sampleConditions
                and (sampleConditions[0].unit in CONC_FACTORS)
                and (self.unitPulldown.getText() not in CONC_FACTORS)
            ):
