...
from ccpnmr.analysis.core.Util import default_axis_types_table, default_axis_units_table
...
class EditAxisPanelPopup(BasePopup):
    ...
    **Make And Edit Spectrum Window Axis Types**
    ...
    def body(self, guiFrame):
        ...
        self.axis_unit_table = ScrolledMatrix(
            frameA,
        ...
        tipTexts = ["Create a new specification of a unit of measurement", "Delete the selected measurement unit"]
        texts = ["Create", "Delete"]
        commands = [self.createAxisUnit, self.deleteAxisUnit]
        self.axis_unit_buttons = ButtonList(frameA, texts=texts, tipTexts=tipTexts, grid=(1, 0), commands=commands)
        ...
        editWidgets = [None, None, None, None, None, self.regionEntry, self.decimalEntry, self.peakSizeEntry, None]
        editGetCallbacks = [
            None,
            None,
            None,
        ...
        editSetCallbacks = [None, None, None, None, None, self.setRegion, self.setDecimal, self.setPeakSize, None]

        self.axisTypeMatrix = ScrolledMatrix(
            frameB,
            tipTexts=tipTexts,
        ...
        self.axis_type_buttons = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(1, 0))
        ...
        tipTexts = ["Row number", "Name of panel type specification", "Which kind of axis the panel is a subtype of"]
        headings = ("#", "Name", "AxisType")
        self.panelTypeMatrix = ScrolledMatrix(
            frameC,
            tipTexts=tipTexts,
            headingList=headings,
            initialRows=5,
        ...
        self.panel_type_buttons = ButtonList(frameC, texts=texts, grid=(1, 0), commands=commands, tipTexts=tipTexts)
    ...
    def selectAxisUnit(self, axisUnit=None, *extra):
        ...
        if (
            not axisUnit
            or axisUnit in axisUnits
            or axisUnit in [axisPanel.axisUnit for axisPanel in self.getAxisPanels()]
            or axisUnit.unit in [entry[0] for entry in default_axis_units_table]
        ):
            state = "disabled"
        else:
            state = "normal"
    ...
    def deleteAxisUnit(self, *event):
        ...
        if axisUnit:
            if showYesNo("Delete axis unit", 'Are you sure you want to delete axis unit "%s"?' % axisUnit.unit, self):
                axisUnit.delete()
    ...
    def selectAxisType(self, axisType=None, *extra):
        ...
        if (
            not axisType
            or axisType in [axisPanel.axisType for axisPanel in self.getAxisPanels()]
            or axisType in [markDim.axisType for markDim in markDims]
            or axisType.name in [entry[0] for entry in default_axis_types_table]
        ):
            state = "disabled"
        else:
            state = "normal"
    ...
    def deleteAxisType(self, *event):
        ...
        axisType = self.getAxisType()
        if axisType:
            if showYesNo("Delete axis type", 'Are you sure you want to delete axis type "%s"?' % axisType.name, self):
                axisType.delete()
    ...
    def selectPanelType(self, panelType=None, *extra):
        ...
        if (
            not panelType
            or panelType in [axisPanel.panelType for axisPanel in self.getAxisPanels()]
            or panelType in [ruler.panelType for ruler in self.analysisProject.rulers]
        ):
            state = "disabled"
        else:
            state = "normal"
    ...
    def deletePanelType(self, *event):
        ...
        if panelType:
            if showYesNo(
                "Delete panel type", 'Are you sure you want to delete panel type "%s"?' % panelType.name, self
            ):
                panelType.delete()
