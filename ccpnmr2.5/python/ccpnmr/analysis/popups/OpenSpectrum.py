...
from ccpnmr.analysis.core.ExperimentBasic import (
    getIsSpectrumBigEndian,
    getRefExperiments,
    isReferencingIncorrect,
    isSpectrumBigEndian,
    newShiftList,
    setAllIsotopeCodes,
    setExperimentShiftList,
    setIsSpectrumBigEndian,
    setRefExperiment,
)
...
file_formats = ("Azara", "Bruker", "Felix", "NMRPipe", "NMRView", "UCSF", "Varian", "Factorised")
...
class OpenSpectrumPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.shiftListPulldown = PulldownList(self, callback=self.setShiftList, texts=names, objects=objects)
        self.windowPulldown = PulldownList(self, texts=WINDOW_OPTS, callback=self.setWindow)
        ...
        self.formatPulldown = PulldownList(
            leftFrame, callback=self.chooseFormat, texts=file_formats, tipText=tipText, grid=(row, 1)
        )
        ...
        self.detailsSelect = CheckButton(leftFrame, selected=False, callback=self.showDetails, tipText=tipText)
        self.titleRow = row
        ...
        self.verifySelect = CheckButton(rightFrame, selected=False, grid=(row, 1), tipText=tipText)
        ...
        self.sharedExpSelect = CheckButton(
            rightFrame, selected=False, tipText=tipText, callback=self.useShared, grid=(row, 3)
        )
        ...
        bottomButtons = UtilityButtonList(
            guiFrame, texts=texts, tipTexts=tipTexts, doClone=False, commands=commands, helpUrl=self.help_url
        )
    ...
    def openSpectra(self):
        ...
        for obj in self.scrolledMatrix.objectList:
            ...
            spectrum = self.openSpectrum(obj.exptName, obj.specName, fileName, obj.window, obj.shiftListName)
            specIndex += 1
                ...
                if not experiment.refExperiment:
                    ...
                    if noVerify:
                        ...
                        if not hasattr(experiment, "category"):
                            if hasattr(experiment, "pulProgName") and hasattr(experiment, "pulProgType"):
    ...
    def chooseFiles(self, forceUpdate=False, *file):
        ...
        if format == "Bruker":
            if self.sharedExpSelect.getSelected():
                ...
                next = self.getNextExpNum(nfiles=len(fileNames), nameTemplate=nameTemplate)
            ...
            if text and text != " ":
                for data in self.scrolledMatrix.objectList:
                    ...
                    if (data.specName == text) and (data.exptName == self.currentObject.exptName):
                        showWarning(
                            "Repeated name",
                            "Spectrum name (%s) already in use for experiment (%s)" % (data.specName, data.exptName),
                            parent=self,
                        )
                        return
    ...
    def openSpectrum(self, exptName, specName, file, windowOpt=WINDOW_OPTS[2], shiftListName="<New>", extraData=None):
        ...
        if shiftListName == "<New>":
            ...
            shiftList = self.nmrProject.findFirstMeasurementList(className="ShiftList", name=shiftListName)
        ...
        try:
            ...
            showError("Reading params file", "Fatal error: " + e.error_msg, parent=self)
            return None
        ...
        if experiment:
            ...
            if experiment.findFirstDataSource(name=specName):
                showError(
                    "Duplicate name",
                    'Duplicate spectrum name "%s" in experiment %s' % (specName, experiment.name),
                    parent=self,
                )
                return None
            elif experiment.dataSources and experiment not in self.okExpSet:
                if showOkCancel(
                    "Multiple Spectra Warning",
                    "Really put multiple spectra into existing experiment %s?" % experiment.name,
                    parent=self,
                ):
            ...
            try:
                ...
                experiment = Nmr.Experiment(self.nmrProject, name=exptName, numDim=params.ndim, shiftList=shiftList)
...
%s\n"""
                ...
                if incorrect and showOkCancel("Reset Referencing?", text, parent=self):
        ...
        if self.verifySelect.getSelected():
            ...
            popup = EditSpectrumPopup(
                self.parent, transient=False, modal=True, useReducedDim=useReducedDim, spectrum=spectrum
            )
        ...
        if pulProgName is not None:
            if experiment.refExperiment is None:
                if not (hasattr(experiment, "pulProgName") and experiment.pulProgName is not None):
