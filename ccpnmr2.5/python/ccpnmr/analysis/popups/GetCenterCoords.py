...
from ccpnmr.analysis.core import ExperimentBasic, UnitConverter
...
class GetCenterCoordsPopup(BasePopup):
    ...
    def body(self, master):
        ...
        tipText = (
            "Sets the location for a new window centre position along the horizontal axis, using the below stated units"
        )
        ...
        if n > 1:
            ...
            self.x_region_menu = PulldownList(
                master, objects=indices, texts=texts, callback=self.setXCenter, tipText=tipText
            )
        ...
        tipText = (
            "Sets the location for a new window centre position along the vertical axis, using the below stated units"
        )
        ...
        if n > 1:
            ...
            indices = range(n)
            texts = ["%d" % (x + 1) for x in indices]
            self.y_region_menu = PulldownList(
                master, objects=indices, texts=texts, callback=self.setYCenter, tipText=tipText
            )
        ...
        self.unit_list = PulldownList(master, texts=units, callback=self.changedUnit, tipText=tipText)
        ...
        self.view_list = PulldownList(
            master, texts=texts, objects=views, tipText=tipText, callback=self.changedSpectrum
        )
        self.view_row = row
        row = row + 1
        ...
        texts = commands = []
        tipTexts = [
            "Move the spectrum window display so that its center, or that of the stated strip, is at the specified position",
        ]
        texts = [
            " Commit ",
        ]
        ...
        buttons = UtilityButtonList(
            master, texts=texts, commands=commands, doClone=False, helpUrl=self.help_url, tipTexts=tipTexts
        )
