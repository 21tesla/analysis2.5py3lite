...
class BrowsePeakGroupsPopup(BasePopup):
    ...
    **Display Groups of Peaks Linked by a Root Assignment**
    ...
    def body(self, guiFrame):
        ...
        commands = [self.removeGroups, self.addRulers, self.removeRulers, self.stripGroups]
        ...
        self.windowPulldown = PulldownList(frame, callback=None, grid=(1, 2), tipText=tipText)
    ...
    def updateWindows(self):
        ...
        if windowPane and groups:
            ...
            windowZPlanes = findOrthogonalWindows(windowPane, xyz, excludeQuery=False)
    ...
    def stripGroups(self):
        ...
        if groups and name and (name != "<None>"):
            ...
            displayStrips(self.parent, positions, orthoPositions=None, spectrum=None, windowPane=windowPane)
    ...
    def addRulers(self):
        ...
        if groups and self.windowPane:
            ...
            for i in range(len(positions)):
                ruler = createRuler(positions[i], panelTypes[i], dashLength=3, gapLength=1, color=color, remove=False)
                self.rulers.append(ruler)

    def update(self, groups=None, variableDim=None, peakLists=None, searchPeaks=None):
        ...
        for i, group in enumerate(self.groups):
            ...
            datum = ["%d" % (i + 1), "%f" % score, "%d" % len(peaks)]
    ...
    def getHeadingList(self):
        ...
        tipTexts = [
            "The ranking of the peak group, comparing its score with others",
