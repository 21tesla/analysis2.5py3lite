...
class NewWindowPopup(BasePopup):
    ...
    **Spectra**
    ...
    def body(self, guiFrame):
        ...
        tipText = (
            "Selects which window to use as the basis for making a new spectrum window; sets the axis types accordingly"
        )
        self.window_list = PulldownList(guiFrame, grid=(row, 1), callback=self.setAxisTypes, tipText=tipText)
        frame = LabelFrame(guiFrame, text="Strips", grid=(row, 2), gridSpan=(2, 1))
        buttons = UtilityButtonList(guiFrame, doClone=False, helpUrl=self.help_url, grid=(row, 3))
        ...
        self.nameEntry = Entry(guiFrame, width=16, grid=(row, 1), tipText=tipText)
        ...
        self.cols_menu = PulldownList(
            frame, objects=STRIP_NUMS, grid=(0, 1), texts=[str(x) for x in STRIP_NUMS], tipText=tipText
        )
        ...
        self.rows_menu = PulldownList(
            frame, objects=STRIP_NUMS, grid=(0, 3), texts=[str(x) for x in STRIP_NUMS], tipText=tipText
        )
        row += 1
        div = LabelDivider(guiFrame, text="Axes", grid=(row, 0), gridSpan=(1, 4))
        ...
        for label in AXIS_LABELS:
            ...
            if label in ("x", "y"):
                includeNone = False
                tipText = (
                    "Sets the kind of measurement (typically ppm for a given isotope) that will be used along the window %s axis"
                    % label
                )
            else:
                includeNone = True
                tipText = (
                    "Where required, sets the kind of measurement (typically ppm for a given isotope) that will be used along the window %s axis"
                    % label
                )
        ...
        row += 1
        div = LabelDivider(guiFrame, text="Viewed Spectra", grid=(row, 0), gridSpan=(1, 4))
        ...
        commands = [self.ok, self.setSelectedDisplay, self.setSelectedHide, self.setSelectedAbsent]
        buttonList = ButtonList(
            guiFrame, texts=texts, grid=(row, 0), commands=commands, gridSpan=(1, 4), tipTexts=tipTexts
        )
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in (
            ...
            "delete",
            "setName",
            "addSpectrumWindowGroup",
            "removeSpectrumWindowGroup",
            "setSpectrumWindowGroups",
        ):
    ...
    def update(self):
        ...
        for name, spectrum in self.getSpectra():
            ...
            datum = [name, isVisible, inToolbar, ",".join(["%d" % pl.serial for pl in spectrum.peakLists])]
        ...
        if window:
            ...
            if windowPanes:
                ...
                for label in AXIS_LABELS:
                    ...
                    if axisPanel and axisPanel.panelType and axisPanel.panelType.axisType:
