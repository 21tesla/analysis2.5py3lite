...
from ccpnmr.analysis.core.PeakBasic import findSymmetryPeaks, getPeakHeight, getPeakVolume, pickPeak
...
from ccpnmr.analysis.core.WindowBasic import displayPeakStrips, getPeakDimAxisMapping, getWindowPaneName
...
class LinkNoeResonancesPopup(BasePopup):
    ...
    **Caveats and Tips**
    ...
    def body(self, guiFrame):
        ...
        self.peakListPulldown = PulldownList(frame, callback=self.changePeakList, grid=(0, 1), tipText=tipText)
        ...
        self.markSelect = CheckButton(subFrame, callback=None, grid=(0, 5), selected=False, tipText=tipText)
        ...
        self.returnSelect = CheckButton(subFrame, callback=None, grid=(0, 7), selected=False, tipText=tipText)
        ...
        self.distThresholdEntry = FloatEntry(self.tolFrame, text=8.0, width=8, grid=(0, 1), tipText=tipText)
        ...
        row += 1
        tipTexts = [
            "Selects the previous peak in the table and display NOE assignment possibilities; causes navigation within spectrum windows if this is set",
        ...
        commands = [self.prevPeak, self.selectedPeak, self.nextPeak, self.stripSymmetryRelatedPeaks]
        self.peakButtons = ButtonList(frameB, texts=texts, commands=commands, grid=(row, 0), tipTexts=tipTexts)
        ...
        label = Label(frame, text="\u03a3NOE dist: ", grid=(0, 1), tipText=tipText)
        ...
        self.assignMatrix = ScrolledMatrix(
            frameB,
            headingList=headingList,
            multiSelect=True,
            grid=(row, 0),
        ...
        row += 1
        tipTexts = [
            "Assign the peak to the individual resonance+dimension possibility selected in the above table",
        ...
        self.assignButtons = ButtonList(frameB, texts=texts, commands=commands, grid=(row, 0), tipTexts=tipTexts)
        ...
        if self.peakList:
            ...
            for dataDim in spectrum.sortedDataDims():
                ...
                if dataDim.className != "SampledDataDim":
                    ...
                    tipText = (
                        "The shift scaling factor for dimension %d (%s); used to give equivalency to ppm distances for different kinds of isotope"
                        % (dim, isotope)
                    )

                    if dim > len(self.tolLabels):
                        label = Label(self.tolFrame, text=text)
                        entry = FloatEntry(self.tolFrame, text=tol, width=6, tipText=tipText)
                    ...
                    if dim > len(self.tolLabels):
                        label = Label(self.tolFrame, text=text)
                        entry = FloatEntry(
                            self.tolFrame, text=0, width=5, tipText="**Tolerance not used for sampled dimensions**"
                        )
    ...
    def updateWindows(self, obj=None):
        ...
        for windowPane in windowPanes:
            ...
            datum = [
                windowPane.spectrumWindow.serial,
        ...
        self.windowMatrix.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
        ...
        if self.peak:
            for score, resonances, dist in self.assignMatrix.currentObjects:
                ...
                for peakDim in self.peak.sortedPeakDims():
                    ...
                    assignResToDim(peakDim, resonance, tolerance=tol, contrib=contrib, peakContribs=peakContribs)
    ...
    def processAll(self):
        ...
        progressBar = ProgressBar(
            self, text="Peak progress:", progress=0, total=len(peaks), title="Assigning close resonances"
        )
        ...
        for peak in peaks:
            ...
            for shiftData in assignments:
                ...
                if resonanceSet1 and resonanceSet2:
                    dist = getAtomSetsDistance(resonanceSet1.atomSets, resonanceSet2.atomSets, structure)
                ...
                for j, peakDim in enumerate(peak.sortedPeakDims()):
                    ...
                    tol = self.tolEntries[j].get() or getAnalysisDataDim(peakDim.dataDim).assignTolerance
    ...
    def getPossibleAssignments(self, peak=None):
        ...
        if self.peakList and peak:
            ...
            for dim in self.hydroDims:
                ...
                shiftsH = findMatchingPeakDimShifts(
                    peakDim, tolerance=tolerance, aliasing=doAliasing, findAssigned=True
                )

                if dimX is not None:
                    peakDimX = peak.findFirstPeakDim(dim=dimX)
                    resonances = []
                    shiftsX = findMatchingPeakDimShifts(
                        peakDimX, tolerance=tolerances[dimX - 1], aliasing=doAliasing, findAssigned=True
                    )
    ...
    def getHeadingList(self, numDim):
        ...
        tipTexts += [
            "Distance, in the selected structure, between atom assignment possibilities, for the NOESY linked dimensions",
