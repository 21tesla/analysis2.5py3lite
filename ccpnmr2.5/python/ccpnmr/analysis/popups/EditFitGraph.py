...
from ccpnmr.analysis.core.WindowBasic import getActiveWindows, getWindowPaneName, isSpectrumInWindowPane
...
class EditFitGraphPopup(BasePopup):
    ...
    **Analyse Function Curve Fitting to Peak Series Data**
    ...
    def body(self, guiFrame):
        ...
        self.scrolledGraph = ScrolledGraph(
            guiFrame,
            width=400,
            height=300,
            symbolSize=5,
        ...
        self.methodPulldown = PulldownList(frame, self.changeMethod, grid=(0, 1), tipText=tipText)

        row += 1
        frame = Frame(guiFrame, grid=(row, 0), sticky="ew")

        tipText = "The effective equation of the final fitted graph, incorporating all parameters"
        self.equationLabel = Label(frame, text="Equation:", grid=(0, 0), tipText=tipText)

        tipText = "The error in the fit of the selected parameterised function to the experimental data"
        self.errorLabel = Label(frame, text="Fit Error:", grid=(0, 1), tipText=tipText)
        ...
        self.xOriginSelect = CheckButton(frame, callback=self.draw, grid=(0, 1), selected=False, tipText=tipText)

        label = Label(frame, text="Include y origin?:", grid=(0, 2))

        tipText = "Whether to include the y=0 point in the drawing"
        self.yOriginSelect = CheckButton(frame, callback=self.draw, grid=(0, 3), selected=False, tipText=tipText)

        label = Label(frame, text="Include y error?:", grid=(0, 4))

        tipText = "Whether to include the y error bars in the drawing (if these exist)"
        self.yErrorSelect = CheckButton(frame, callback=self.draw, grid=(0, 5), selected=False, tipText=tipText)
        ...
        self.windowPanePulldown = PulldownList(frame, self.changeWindow, grid=(0, 1), tipText=tipText)
        ...
        self.followSelect = CheckButton(
            frame, callback=self.windowPaneNavigate, grid=(0, 3), selected=False, tipText=tipText
        )
        ...
        self.markSelect = CheckButton(frame, callback=None, tipText=tipText, grid=(0, 5), selected=False)
        ...
        self.scrolledMatrix = ScrolledMatrix(
            guiFrame, headingList=headingList, callback=self.selectObject, tipTexts=tipTexts, grid=(row, 0)
        )
    ...
    def update(self, dataFitting=None, xLabel=None, yLabel=None, graphTitle=None, force=False):
        ...
        if (not force) and dataFitting and (self.dataFitting is dataFitting):
            if (self.method, self.xLabel, self.yLabel, self.noiseLevel) == (
                self.dataFitting.fitFunction,
                xLabel,
                yLabel,
                self.dataFitting.noiseLevel,
            ):
                self.waiting = False
                return
