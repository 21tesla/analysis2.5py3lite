...
from ccpnmr.analysis.core.AssignmentBasic import getResonanceResidue, makeResonanceGuiName
from ccpnmr.analysis.core.ExperimentBasic import getOnebondDataDims
from ccpnmr.analysis.core.MoleculeBasic import getChainResidueMapping, getResidueCode
...
class CalcShiftDifferencePopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        options = ["Peak List Comparison", "Shift List Comparison", "Sequence Alignment"]
        tabbedFrame = TabbedFrame(guiFrame, options=options, tipTexts=tipTexts, grid=(row, 0), callback=self.toggleTab)
        frameA, frameB, frameC = tabbedFrame.frames
        self.tabbedFrame = tabbedFrame

        utilButtons = UtilityButtonList(tabbedFrame.sideFrame, grid=(0, 0), sticky="e", helpUrl=self.help_url)
        ...
        self.assignCheck = CheckButton(
            frame, callback=self.updateAfter, tipText=tipText, grid=(0, 6), sticky="e", selected=False
        )
        ...
        self.atomEntry1 = Entry(
            frame, text="H,H1", width=10, grid=(1, 1), returnCallback=self.updateAfter, tipText=tipText
        )
        ...
        self.atomEntry2 = Entry(
            frame, text="N", width=10, grid=(1, 3), returnCallback=self.updateAfter, tipText=tipText
        )
        ...
        self.scaleEntry1 = FloatEntry(
            frame, text=1.0, width=10, grid=(2, 1), returnCallback=self.updateAfter, tipText=tipText
        )
        ...
        self.scaleEntry2 = FloatEntry(
            frame, text=0.15, width=10, grid=(2, 3), returnCallback=self.updateAfter, tipText=tipText
        )
        ...
        self.alignCheck = CheckButton(
            frame,
            callback=self.updateAlignment,
            grid=(1, 6),
            gridSpan=(2, 1),
            tipText=tipText,
            sticky="e",
        ...
        tipTexts = [
            "The residue assignment(s) of the peaks",
        ...
        self.peakCompTable = ScrolledMatrix(
            frameA, headingList=headingList, grid=(1, 0), callback=self.selectRow, multiSelect=True, tipTexts=tipTexts
        )
        ...
        self.shiftList1Pulldown = PulldownList(frame, self.changeShiftList1, grid=(0, 1), tipText=tipText)

        tipText = "Second chemical shift list used in comparison"
        label = Label(frame, text="Shift List B:", grid=(0, 2))
        self.shiftList2Pulldown = PulldownList(frame, self.changeShiftList2, grid=(0, 3), tipText=tipText)

        tipText = "Whether to use the residue sequence alignment to compare dissimilar molecules"
        label = Label(frame, text="Use Sequence\nAlignment:", grid=(0, 4))
        self.alignCheck2 = CheckButton(
            frame, callback=self.updateAlignment, tipText=tipText, grid=(0, 5), sticky="w", selected=False
        )

        tipText = 'Restricts the comparison to only specific atom types, e.g. "HA, HB"'
        label = Label(frame, text="Atom\nNames:", grid=(0, 6))
        self.atomEntryS = Entry(
            frame, text="", width=10, grid=(0, 7), returnCallback=self.updateShiftComp, tipText=tipText
        )

        tipTexts = [
            "Serial number of first resonance",
        ...
        self.shiftCompTable = ScrolledMatrix(
            frameB, headingList=headingList, grid=(1, 0), callback=self.selectRow, multiSelect=True, tipTexts=tipTexts
        )
        ...
        self.alignmentTable = ScrolledMatrix(
            frameC,
    ...
    def makeDifferenceList(self):
        ...
        if index == 0:
            data = (self.getPeakListName(self.peakList1), self.getPeakListName(self.peakList2))
            ...
            data = (
                "%s:%d" % (self.shiftList1.name, self.shiftList1.serial),
        ...
        if resonanceValues:
            sdList = self.nmrProject.newShiftDifferenceList(details=details, experiments=experiments)
            ...
            for value, resonance, peakDims, peaks in resonanceValues:
                ...
                newMeasurement(value=value, resonance=resonance, peakDims=peakDims, peaks=peaks)
    ...
    def updateAlignment(self, *event):
        ...
        self.alignmentTable.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
        self.updateAfter()
    ...
    def updatePeakComp(self):
        ...
        if self.peakList1 and self.peakList2:
            ...
            for resonance1 in resonances1:
                ...
                for resonance2 in resonances2:
                    ...
                    if resonanceB and resonanceA:
                        dataList = [
                            (resonanceA, (valueA, valueB), frozenset(measuredPeaks1), frozenset(measuredPeakDims1)),
            ...
            for resonance in resonances1:
                if resonance not in paired:
                    ...
                    if valueA and valueB:
                        dataList = [
                            (resonance, (valueA, valueB), frozenset(measuredPeaks1), frozenset(measuredPeakDims1)),
                            (None, (None, None), (), ()),
                        ]
                        objectList.append(dataList)
        ...
        self.peakCompTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateShiftComp(self, event=None):
        ...
        if self.shiftList1 and self.shiftList2:
            ...
            for shift1 in self.shiftList1.measurements:
                ...
                datum = [resonance1.serial, resonance2.serial, name, resText1, resText2, ppm1, ppm2, ppm1 - ppm2]
        ...
        self.shiftCompTable.update(textMatrix=textMatrix, objectList=objectList)
