...
from ccpnmr.analysis.core.CouplingBasic import (
    EXPT_MULTIPLETS,
    MULTIPLET_PEAK_DICT,
    assignPrimaryClusterCoupling,
    deleteCluster,
    getClusterCoupling,
    getCoupledExpDims,
    getCouplingAnnotations,
    getPeakListsPeakClusters,
    getSpectrumMultipletPattern,
    makeMultipletPeakCluster,
    setSpectrumMultipletPattern,
)
...
class CalcCouplingPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        self.multipletButtons = PartitionedSelector(frame, callback=self.setMultiplet, radio=True)
        ...
        self.peakListMatrix = ScrolledMatrix(frame, headingList=headingList, callback=None, multiSelect=False)
        ...
        self.clusterMatrix = ScrolledMatrix(
            frame, headingList=headingList, callback=self.selectCluster, multiSelect=True
        )
        ...
        commands = [
            self.clusterSelectedPeaks,
            self.assignCouplings,
            self.showPeaks,
            self.findPeaks,
            self.deleteClusters,
        ]
        self.bottomButtons = UtilityButtonList(
            guiFrame, texts=texts, expands=True, commands=commands, helpUrl=self.help_url
        )
    ...
    def updatePeakLists(self, peakList=None):
        ...
        for peakList0 in objectList:
            ...
            datum = [experiment.name, spectrum.name, peakList0.serial, coupledDims, experimentType]
    ...
    def setSpectrumMultiplet(self, spectrum, pattern):
        ...
        if prevPattern and (prevPattern != pattern):
            if showOkCancel("Query", "Really change multiplet pattern?", parent=self):
    ...
    def updateMultipletButtons(self):
        ...
        self.multipletButtons.update(objects=objects, colors=colors, labels=labels)
    ...
    def clusterSelectedPeaks(self):
        ...
        if not peaks:
            showWarning("Cluster Failure", "No peaks selected from active peak lists", parent=self)
            return

        if not self.multiplet:
            showWarning("Cluster Failure", "No multiplet pattern selected", parent=self)
            return
    ...
    def update(self):
        ...
        for cluster in clusters.keys():
            ...
            datum = [cluster.serial, cluster.annotation, len(cluster.peaks), couplingAssign]
        ...
        self.clusterMatrix.update(headingList=headingList, textMatrix=textMatrix, objectList=objectList)
