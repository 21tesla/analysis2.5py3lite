...
class EditMolLabellingPopup(BasePopup):
    ...
    *Labelling a Specific Residue*
    ...
    def body(self, guiFrame):
        ...
        options = ["Labelled Molecules", "Labelling Patterns"]

        tabbedFrame = TabbedFrame(guiFrame, options=options, tipTexts=tipTexts, grid=(0, 0), callback=self.toggleTab)
        ...
        self.mixtureNameEntry = Entry(self, text="", width=10, returnCallback=self.setMixtureName)

        self.experimentSelect = MultiWidget(self, CheckButton, minRows=0, callback=self.setExperiments, useImages=False)

        tipTexts = [
            "The name of the molecule (sequence template) that the isotope labelled sample refers to",
        ...
        editWidgets = [None, None, self.mixtureNameEntry, self.experimentSelect, None, None, None, None]
        editGetCallbacks = [None, None, self.getMixtureName, self.getExperiments, None, None, None, None]
        editSetCallbacks = [None, None, self.setMixtureName, self.setExperiments, None, None, None, None]

        self.mixtureMatrix = ScrolledMatrix(
            frameA,
            tipTexts=tipTexts,
            multiSelect=False,
            grid=(1, 0),
        ...
        tipTexts = [
            "Delete the selected isotope labelling molecule specification",
        ...
        buttonList = ButtonList(frame, tipTexts=tipTexts, texts=texts, commands=commands, grid=(0, 0))
        ...
        self.molFracWeightEntry = FloatEntry(self, text=1.0, width=8, returnCallback=self.setMolFracWeight)

        tipTexts = [
            "A code letter for a specific labelling pattern version of the molecule sample",
        ...
        headingList = ["Labelling Pattern", "Basis Scheme", "Weight", "% Composition", "Mol Mass"]
        ...
        self.molFractionMatrix = ScrolledMatrix(
            frameA,
            tipTexts=tipTexts,
            multiSelect=False,
            grid=(4, 0),
        ...
        tipTexts = [
            "Remove the selected molecule labelling pattern from the sample/mixture; the pattern is deleted if it is no longer used in any sample",
        ...
        commands = [self.deleteMolFraction, self.editMolLabel, self.addMolFraction]
        buttonList = ButtonList(frame, tipTexts=tipTexts, texts=texts, commands=commands, grid=(0, 0))
        ...
        self.addMolFracPulldown = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.updateButtons)

        tipTexts = [
            "make a complete new isotope labelling pattern for the molecule using the selected isitope labelling scheme",
        ]
        texts = ["New Pattern From Scheme:"]
        commands = [
            self.newMolFraction,
        ]
        buttonList = ButtonList(frame, tipTexts=tipTexts, texts=texts, commands=commands, grid=(0, 2))
        self.patternButtons += buttonList.buttons
        ...
        self.moleculePulldown = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changePatternMolecule)
        ...
        self.patternPulldown = PulldownList(frame, grid=(0, 3), tipText=tipText, callback=self.changeMolLabel)

        tipTexts = [
            "make a new labelling pattern (isotope version of the molecule) using the selected standard labelling scheme",
        ]
        texts = ["New Pattern From Scheme:"]
        commands = [
            self.newPattern,
        ]
        buttonList = ButtonList(frame, tipTexts=tipTexts, texts=texts, commands=commands, grid=(0, 5))
        ...
        tipTexts = [
            "The sequence ID number of the labelled residue in the molecule; a separate numbering system to the numbering in chains",
        ...
        tipTexts = [
            "Set the selected residue labelling to have natural abundance isotopic incorporation labels.",
        ...
        commands = [self.setResidueNatAbun, self.setResidueIsotopomers]
        buttonList = ButtonList(frame, tipTexts=tipTexts, texts=texts, commands=commands, grid=(0, 0))
        ...
        self.chemElementPulldown = PulldownList(
            frame, grid=(0, 1), tipText=tipText, callback=self.changeChemElement, texts=["C", "N", "H"], index=0
        )
        ...
        self.waterExchangeSelect = CheckButton(
            frame, tipText=tipText, selected=False, grid=(0, 3), callback=self.updateAtomMatrix
        )
        ...
        self.isotopomerPulldown = PulldownList(frame, grid=(0, 6), tipText=tipText, callback=self.changeIsotopomer)
        ...
        self.isotopomerFractionEntry = FloatEntry(
            frame, text=1.0, width=8, tipText=tipText, returnCallback=self.setIsotopomerWeight, grid=(0, 8)
        )
        self.isotopomerFractionEntry.bind("<Leave>", self.setIsotopomerWeight, "+")

        self.atomLabelWeightEntry = FloatEntry(self, text=1.0, width=8, returnCallback=self.setAtomLabelWeight)

        headingList, tipTexts, editWidgets, editGetCallbacks, editSetCallbacks = self.getIsotopeHeadingLists()

        self.atomMatrix = ScrolledMatrix(
            frameB,
            tipTexts=tipTexts,
            multiSelect=True,
            grid=(6, 0),
        ...
        tipTexts = [
            "Resets the selected residue to use atom isotope incorporation levels from the isotopomers of the set scheme; removes any atom level edits",
        ...
        commands = [self.resetResidue, self.propagateAbundances, self.setAtomsNatAbun]
        buttonList = ButtonList(frameB, tipTexts=tipTexts, texts=texts, commands=commands, grid=(7, 0))
    ...
    def getIsotopeHeadingLists(self):
        ...
        tipTexts = [
            "The name of the atom within its residue, for which to set isotopic abundances, within the selected labelling pattern",
        ]
        ...
        if newWeight is not None:
            ...
            for i, (atomLabel, isotope, weight) in enumerate(atomLabels):
                ...
                if (not atomLabel) or (atomLabel.className != "SingleAtomLabel"):
                    atomLabel = self.resLabel.newSingleAtomLabel(
                        atomName=atomName, massNumber=isotope.massNumber, weight=weight
                    )
                else:
        ...
        for atomName0, atomLabels0 in atomLabelTuples:
            if atomName0 != atomName:
                ...
                for massNumber in weightDict:
                    ...
                    atomLabel = resLabel.findFirstAtomLabel(atomName=atomName0, massNumber=massNumber)
                    if atomLabel is None:
                        atomLabel = resLabel.newSingleAtomLabel(
                            atomName=atomName0, massNumber=massNumber, weight=weight
                        )
                    else:
    ...
    def setAtomsNatAbun(self):
        ...
        for atomName, atomLabels in atomLabelTuples:
            for atomLabel, isotope, weight in atomLabels:
                if (not atomLabel) or (atomLabel.className != "SingleAtomLabel"):
                    atomLabel = self.resLabel.newSingleAtomLabel(
                        atomName=atomName, massNumber=isotope.massNumber, weight=isotope.abundance
                    )
                else:
        ...
        if resLabel:
            ...
            if self.resLabelFraction and self.isotopomer:
                ...
                for chemAtom in chemAtoms:
                    ...
                    if not (
                        resLabel.findFirstAtomLabel(atomName=atomName)
                        or resLabel.findFirstAtomLabel(elementName=element)
                    ):
                        # Not Overrriden
            ...
            for name in atomNames:
                ...
                if isotopeData is not None:
                    ...
                    fr = 255 * fraction
                    un = 255 - fr
                    rgb = ((r1 * un) + (fr * r2), (g1 * un) + (fr * g2), (b1 * un) + (fr * b2))
    ...
    def updateMixtureMatrix(self, mixture=None):
        ...
        self.mixtureMatrix.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateMolFractionMatrix(self):
        ...
        if mixture:
            ...
            for pattern, serial, molLabel, molLabelFraction in sortList:
                ...
                datum = [pattern, ",".join(schemes), weight, comp, molMass]

                objectList.append(molLabelFraction)
                textMatrix.append(datum)

        self.molFractionMatrix.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateResidueMatrix(self):
        ...
        if self.molLabel:
            for resLabel in self.molLabel.sortedResLabels():
                ...
                datum = [molResidue.seqCode, molResidue.ccpCode, " ".join(schemes), isotopomerText]

                objectList.append(resLabel)
                textMatrix.append(datum)

        self.residueMatrix.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("__init__", "delete", "addExperiment", "removeExperiment", "setExperiments", "setName"):
    ...
    if scheme:
        ...
        if chemCompLabel:  # Do something else otherwise?
            for isotopomer in chemCompLabel.isotopomers:
                ...
                resLabelFraction = getFraction(isotopomerSerial=serial, schemeName=scheme.name)
                weight = isotopomer.weight

                if resLabelFraction:
                    resLabelFraction.weight = weight
                else:
                    resLabel.newResLabelFraction(isotopomerSerial=serial, schemeName=scheme.name, weight=weight)
...
def setResLabelNaturalAbundance(resLabel):
    ...
    for element in elements:
        ...
        for isotope in element.isotopes:
            ...
            atomLabel = resLabel.findFirstAtomLabel(elementName=symbol, massNumber=massNumber)

            if not atomLabel:
                atomLabel = resLabel.newUniformAtomLabel(elementName=symbol, massNumber=massNumber)
