...
from ccpnmr.analysis.core.AssignmentBasic import addSpinSystemResonance, assignResToDim, findSpinSystem
...
class InitRootAssignmentsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.tolEntry = FloatEntry(frame, text=0.02, width=8, grid=(0, 3), sticky="ew", tipText=tipText)
        ...
        self.followSelect = CheckButton(frame2, callback=None, grid=(0, 1), selected=True, tipText=tipText)
        ...
        self.windowPulldown = PulldownList(frame2, self.selectWindowPane, grid=(0, 3), tipText=tipText)
        ...
        self.markSelect = CheckButton(frame2, callback=None, grid=(0, 5), selected=True, tipText=tipText)

        utilButtons = UtilityButtonList(guiFrame, grid=(row, 1), sticky="ne", helpUrl=self.help_url)
        ...
        self.initButtons = ButtonList(
            guiFrame, commands=commands, grid=(row, 0), texts=texts, gridSpan=(1, 2), tipTexts=tipTexts
        )
        self.initButtons.buttons[0].config(bg="#B0FFB0")

        row += 1
        div = LabelDivider(guiFrame, text="Amide Side Chain Peaks", gridSpan=(1, 2), grid=(row, 0))
        ...
        headingList = ["#", "Confirmed?", "15N\n\u0394ppm", "Peak 1", "Peak 2", "Shift N", "Shift H1", "Shift H2"]
        ...
        commands = [self.confirmAmidePairs, self.unconfirmAmidePairs, self.predictAmidePairs]
        texts = ["Confirm\nSelected", "Unconfirm\nSelected", "Update\nTable"]
    ...
    def update(self):
        ...
        self.amidePairMatrix.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
        self.waiting = False
