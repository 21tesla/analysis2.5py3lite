...
FIXED_TIP_TEXTS = [
    "The sequence number and residue type",
...
CSI_ERRORS = {"Pro": (4.0, 4.0, 4.0, 0.1), "Gly": (0.7, 0.5, None, 0.1), None: (0.7, 0.5, 0.7, 0.1)}
...
class SecStructureGraphPopup(BasePopup):
    ...
    *Wishart DS, Sykes BD, Richards FM.
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=options, grid=(0, 0), callback=self.selectTab, tipTexts=tipTexts)
        ...
        self.optSelector = PartitionedSelector(
            frame, self.toggleOpt, maxRowObjects=10, colors=["#C0C0FF"] * 6, grid=(0, 1), sticky="ew", tipText=tipText
        )

        self.optSelector.update(objects=OPTIONAL_COLUMNS, labels=OPTIONAL_COLUMNS, selected=OPTIONAL_COLUMNS[:-2])
        ...
        tipTexts += [
            "Read CSI information for the residue from a CSI output file",
        ...
        buttonList = ButtonList(frameA, texts=texts, gridSpan=(1, 2), commands=commands, grid=(2, 0), tipTexts=tipTexts)
        ...
        self.shiftListPulldown = PulldownList(frame, callback=self.changeShiftList, grid=(0, 3), tipText=tipText)

        label = Label(frame, text="Chart Font:", grid=(0, 4))
        tipText = "Sets the typeface to use in the main chart display"
        self.fontPulldown = FontList(frame, callback=self.changeFont, grid=(0, 5), tipText=tipText)
        ...
        self.wrapPulldown = PulldownList(
            frame,
            callback=self.changeWrap,
            texts=[str(x) for x in wraps],
            index=2,
            objects=wraps,
            grid=(0, 7),
            tipText=tipText,
        ...
        self.strongEntry = FloatEntry(frame, text=1.5, width=8, grid=(0, 1), tipText=tipText)

        label = Label(frame, text="Medium/Weak:", grid=(0, 2))
        tipText = (
            "The intensity value, relative to peak list average, that separates weak and medium NOE strength categories"
        )
        self.weakEntry = FloatEntry(frame, text=0.3, width=8, grid=(0, 3), tipText=tipText)
        ...
        row += 1
        tipTexts = [
            "The experiment:spectrum to which the through-space peak list belongs",
        ...
        buttonList = UtilityButtonList(tabbedFrame.sideFrame, helpUrl=self.help_url, grid=(0, 0), sticky="e")
    ...
    def readCsi(self):
        ...
        fileSelectPopup = FileSelectPopup(
            self, file_types=fileTypes, title="Import CSI file", dismiss_text="Cancel", selected_file_must_exist=True
        )
...
class SecStructureGraph(Frame):
    def __init__(self, parent, *args, **kw):
        ...
        self.canvasFrame = ScrolledCanvas(self, relief="groove", borderwidth=2, resizeCallback=None)
