...
from ccpnmr.analysis.core.WindowBasic import displaySpinSystemStrips, getActiveWindows, getWindowPaneName
...
class EditSpinSystemPopup(BasePopup):
    ...
    * The "Seq. Links" tab is used to display how spin systems have been
      connected as sequential neighbours.
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.statusPulldown = PulldownList(
            topFrame, callback=self.changeStatus, texts=self.assignmentTypes, grid=(0, 1), tipText=tipText
        )
        ...
        self.shiftListPulldown = PulldownList(topFrame, self.changeShiftList, grid=(0, 3), tipText=tipText)
        ...
        self.shiftListCheckButton = CheckButton(
            topFrame, text=text, callback=self.changeFilter, grid=(0, 4), tipText=tipText
        )
        ...
        tipTexts = [
            "Assign the last selected spin system, and the resonances it contains, to a specific residue in a molecular chain",
        ...
        buttonList = ButtonList(frame, commands=commands, tipTexts=tipTexts, texts=texts, grid=(0, 1))
        ...
        tipTexts = [
            "Removes the specific residue assignment form the selected spin systems, any contained resonances will be deassigned too, but will still carry atom type information",
        ...
        commands = [self.deassignSeq, self.deassignTentative, self.deassignType]
        buttonList = ButtonList(frame, commands=commands, tipTexts=tipTexts, texts=texts, grid=(0, 3))
        ...
        tipTexts = [
            "Merge the selected spin systems into one; all contained resonances will be put in the same group and the last selected spin system will have any overriding assignment",
        ...
        commands = [self.merge, self.delete, self.showPeaks, self.showResonances, self.predictType, self.newSpinSystem]
        self.bottomButtons = ButtonList(guiFrame, commands=commands, grid=(3, 0), texts=texts, tipTexts=tipTexts)
        ...
        self.tabbedFrame = TabbedFrame(guiFrame, options=options, grid=(1, 0), callback=self.update, tipTexts=tipTexts)

        genFrame, seqFrame, shiftFrame, detailFrame = self.tabbedFrame.frames

        tipTexts = [
            "Use the resonances in the selected spin systems to define positions for building strip panels in the selected spectrum window",
        ...
        buttonList = ButtonList(
            self.tabbedFrame.sideFrame, grid=(0, 0), texts=texts, commands=commands, sticky="e", tipTexts=tipTexts
        )

        self.stripsButton, self.cellsButton = buttonList.buttons

        self.windowLabel = Label(self.tabbedFrame.sideFrame, text="Window:", fg="#808080", sticky="e", grid=(0, 1))

        tipText = ""
        self.stripsPulldown = PulldownList(
            self.tabbedFrame.sideFrame, callback=self.selectWindow, sticky="e", grid=(0, 2), tipText=tipText
        )
        ...
        editGetCallbacks = [None, None, self.predictType, self.getName, self.showResonances, None]
        editSetCallbacks = [None, None, None, self.setName, None, None]
        self.generalTable = ScrolledMatrix(
            genFrame,
            tipTexts=tipTexts,
            multiSelect=True,
        ...
        colHeadings = ["#", "Chain", "Previous", "Current", "Next", "Backbone\nResonances", "Seq.\nSegment"]
        editWidgets = [None, None, None, None, None, None, self.seqSegPulldown]
        editGetCallbacks = [None, None, None, None, None, self.showResonances, self.getSeqSeg]
        editSetCallbacks = [None, None, None, None, None, None, self.setSeqSeg]
        self.seqTable = ScrolledMatrix(
            seqFrame,
            tipTexts=tipTexts,
            multiSelect=True,
            headingList=colHeadings,
        ...
        self.shiftTable = ScrolledMatrix(
            shiftFrame,
            tipTexts=tipTexts,
            multiSelect=True,
            headingList=colHeadings,
        ...
        colHeadings = ["#", "Chain", "Residue", "Details" + space]
        justifyList = ["center"] * 5
        justifyList.append("left")
        editWidgets = [None, None, None, self.detailsText]
        editGetCallbacks = [None, None, self.predictType, self.getDetailsText]
        editSetCallbacks = [None, None, None, self.setDetailsText]
        self.detailTable = ScrolledMatrix(
            detailFrame,
            tipTexts=tipTexts,
            multiSelect=True,
            headingList=colHeadings,
    ...
    def administerNotifiers(self, notify):
        ...
        for func in ("__init__", "delete"):
            for clazz in ("ccp.molecule.MolSystem.Chain", "ccp.nmr.Nmr.ResonanceSet", "ccp.nmr.Nmr.Resonance"):
        ...
        for func in ("setName", "setAssignNames", "addAssignName", "removeAssignName", "setResonanceGroup"):
    ...
    def displayStrips(self):
        ...
        if windowPane and self.spinSystem:
            ...
            displaySpinSystemStrips(self.guiParent, spinSystems, windowPane, self.shiftList)
    ...
    def delete(self, *event):
        ...
        if len(spinSystems) > 0:
            ...
            if showOkCancel(
                "Delete Spin Systems", "Do you want to delete %d spin systems" % len(spinSystems), parent=self
            ):
            ...
            if assigned > 0:
                if assigned == 1:
                    showWarning(
                        "Delete failed",
                        "Cannot delete %d Spin Systems: Spin Systems still contain resonances" % assigned,
                        parent=self,
                    )
                else:
                    showWarning(
                        "Delete failed", "Cannot delete Spin System: Spin System still contain resonances", parent=self
                    )
    ...
    def _getUbiquitousInfo(self, spinSystem, status):
        ...
        if residue:
            if status in (STATUS_TYPED, STATUS_UNASSIGN, STATUS_TENTATIVE, STATUS_ORPHAN):
                return
    ...
    def getTableSelected(self):
        ...
        tableDict = {0: self.generalTable, 1: self.seqTable, 2: self.shiftTable, 3: self.detailTable}
    ...
    def update(self, index=None):
        ...
        updateFuncs = {0: self._updateGeneral, 1: self._updateSeq, 2: self._updateShifts, 3: self._updateDetails}
    ...
    def _updateDetails(self):
        ...
        for spinSystem in self.spinSystems:
            ...
            data = [serial, molText, resText, spinSystem.details]

            objectList.append(spinSystem)
            textMatrix.append(data)

        self.detailTable.update(objectList=objectList, textMatrix=textMatrix)
    ...
    def _updateShifts(self):
        ...
        tipFormat = (
            "The chemical shift of the resonance(s) assigned to the %s atom(s), according to the selected shift list"
        )
        ...
        self.shiftTable.update(objectList=objectList, textMatrix=textMatrix, headingList=headingList, tipTexts=tipTexts)
        if self.spinSystem:
    ...
    def _updateSeq(self):
        ...
        for spinSystem in self.spinSystems:
            ...
            data = [serial, molText, prevText, resText, nextText, resonanceNames, seqSegText]

            objectList.append(spinSystem)
            textMatrix.append(data)

        self.seqTable.update(objectList=objectList, textMatrix=textMatrix)
    ...
    def _updateGeneral(self):
        ...
        for spinSystem in self.spinSystems:
            ...
            data = [serial, molText, resText, spinSystem.name, resonanceNames, numPeaks]

            objectList.append(spinSystem)
            textMatrix.append(data)

        self.generalTable.update(objectList=objectList, textMatrix=textMatrix)
