...
from ccpnmr.analysis.core.StructureBasic import alignStructures, getAtomSetsDihedral, getAtomSetsDistance
...
class ViewStructurePopup(BasePopup):
    ...
    * Translate: Middle button click & drag + Control key
    ...
    def body(self, guiFrame):
        ...
        self.peakListPulldown = PulldownList(frame, callback=self.setPeakList, grid=(0, 8), sticky="e", tipText=tipText)
        ...
        self.structFrame = ViewStructureFrame(
            guiFrame,
        ...
        self.paramButtons = ButtonList(frame, texts=texts, commands=commands, grid=(0, 0), tipTexts=tipTexts)
        ...
        self.bottomButtons = UtilityButtonList(
            frame, texts=texts, commands=commands, helpUrl=self.help_url, grid=(0, 3), tipTexts=tipTexts
        )
            ...
            for value, atoms in atomParams:
                ...
                for atom in atoms:
                    ...
                    if not label:
                        ...
                        label = "%d%s%s %.3f" % (residue.seqCode, residue.ccpCode, atom.name, value)
                    else:
                        label = atom.name
    ...
    def showPeakConnection(self, peak):
        ...
        if len(atomSetsPairs):
            for atomSetsI, atomSetsJ in atomSetsPairs:
                value = getAtomSetsDistance(atomSetsI, atomSetsJ, self.structure, self.model, method=self.distMethod)
    ...
    def showResonancesConnection(self, resonance1, resonance2):
        ...
        if resonance1.resonanceSet and resonance2.resonanceSet:
            ...
            value = getAtomSetsDistance(atomSets1, atomSets2, self.structure, self.model, method=self.distMethod)
