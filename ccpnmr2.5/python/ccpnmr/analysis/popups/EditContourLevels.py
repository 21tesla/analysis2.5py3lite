...
class EditContourLevelsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.expt_spectrum = PulldownList(specFrame, grid=(0, 0), tipText=tipText, callback=self.setSpectrumDetails)

        tipText = (
            "Open the edit spectrum table to view and change other properties associated with the selected spectrum"
        )
        button = Button(
            specFrame,
        ...
        button = Button(
            globalFrame, borderwidth=1, text="10^5", tipText=tipText, command=self.defaultGlobalScale, grid=(1, 0)
        )
        ...
        self.base_entry = FloatEntry(
            self.autoFrame, returnCallback=self.applyAuto, width=10, grid=(frow, 1), sticky="ew", tipText=tipText
        )
        ...
        button = Button(
            self.autoFrame, borderwidth=1, text="/2", tipText=tipText, command=command, grid=(frow, 2), sticky="ew"
        )
        ...
        button = Button(
            self.autoFrame, borderwidth=1, text="*2", tipText=tipText, command=command, grid=(frow, 3), sticky="ew"
        )
        ...
        button = Button(
            self.autoFrame, borderwidth=1, text="-1", tipText=tipText, command=command, grid=(frow, 2), sticky="ew"
        )

        command = lambda w=1: self.changeNumberLevels(w)
        tipText = "Increase the number of contour levels in the series by one"
        button = Button(
            self.autoFrame, borderwidth=1, text="+1", tipText=tipText, command=command, grid=(frow, 3), sticky="ew"
        )
        ...
        frow += 1
        self.change_label = Label(self.autoFrame, text="%s:" % multiplier_text, grid=(frow, 0), sticky="e")
        ...
        selected = (True, False)
        self.which_buttons = CheckButtons(
            frame,
            entries,
            selected=selected,
        ...
        self.change_mode_buttons = RadioButtons(
            frame, entries, grid=(0, 1), sticky="ew", select_callback=self.modeChanged, tipTexts=tipTexts
        )

        row += 1
        manualFrame = LabelFrame(guiFrame, text="Positive levels", grid=(row, 0), gridSpan=(1, 2))
        ...
        self.posLevelsEntry = FloatEntry(
            manualFrame,
            isArray=True,
            width=60,
        ...
        row += 1
        manualFrame = LabelFrame(guiFrame, text="Negative levels", grid=(row, 0), gridSpan=(1, 2))
        manualFrame.expandGrid(None, 0)
        ...
        self.negLevelsEntry = FloatEntry(
            manualFrame,
            isArray=True,
            width=60,
            returnCallback=self.applyManual,
