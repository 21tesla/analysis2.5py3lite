...
class IsotopeSchemeEditor(BasePopup):
    ...
    * Translate: Middle button click & drag + Control key
    ...
    def body(self, guiFrame):
        ...
        editWidgets = [None, self.schemeNameEntry, self.schemeDetailsEntry, None]
        editGetCallbacks = [None, self.getSchemeName, self.getSchemeDetails, None]
        editSetCallbacks = [None, self.setSchemeName, self.setSchemeDetails, None]

        self.schemeMatrix = ScrolledMatrix(
            frameA,
        ...
        self.schemeButtons = ButtonList(frameA, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
        ...
        self.schemePulldown = PulldownList(frame, callback=self.setLabellingScheme, grid=(0, 1), tipText=tipText)
        row += 1
        ...
        self.abundanceWidget = MultiWidget(
            self, FloatEntry, relief="raised", borderwidth=2, callback=self.setDefaultAbundances, useImages=False
        )
        ...
        self.molTypePulldown = PulldownList(
            frame, callback=self.setMolType, texts=entries, grid=(0, 3), tipText=tipText
        )
        ...
        self.isotopomerButtons = ButtonList(frame, texts=texts, commands=commands, grid=(0, 0), tipTexts=tipTexts)
        tipText = "Selects which kind of residue isotopomer may be added to the current isotope scheme"
        self.ccpCodePulldown = PulldownList(frame, callback=None, grid=(0, 1), sticky="e", tipText=tipText)
        ...
        self.elementPulldown = PulldownList(frame, callback=self.changeChemElement, grid=(0, 1), tipText=tipText)
        ...
        self.exchangeCheck = CheckButton(
            frame, callback=self.updateAtomLabelsAfter, grid=(0, 3), selected=False, tipText=tipText
        )
        row += 1
        # Tip texts set on update
        headingList = ["Atom\nName", "Weighting\n13CWeighting\n12C", "%12C", "%13C"]
        self.atomLabelTupleWeightEntry = FloatEntry(self, text="", width=6, returnCallback=self.setAtomLabelWeight)

        self.atomsMatrix = ScrolledMatrix(
            frameB, headingList=headingList, callback=self.selectAtomLabel, multiSelect=True, grid=(row, 0)
        )
        ...
        self.atomButtons = ButtonList(frameB, texts=texts, commands=commands, grid=(row, 0), tipTexts=tipTexts)
        ...
        self.viewCcpCodePulldown = PulldownList(frame, callback=self.selectViewCcpcode, grid=(0, 1), tipText=tipText)
        ...
        self.viewIsotopomerPulldown = PulldownList(
            frame, callback=self.selectViewIsotopomer, grid=(0, 3), tipText=tipText
        )
    ...
    def getCcpCodeIsotopomers(self, ccpCode):
        ...
        chemCompLabel = self.scheme.findFirstChemCompLabel(molType=self.molType, ccpCode=ccpCode)
    ...
    def updateSchemes(self, obj=None):
        ...
        for labelingScheme in self.project.sortedLabelingSchemes():
            ...
            line = [labelingScheme.name, labelingScheme.longName, labelingScheme.details, saveLocation]
    ...
    def copyScheme(self):
        ...
        if self.scheme:
            ...
            if name:
                newScheme = copySubTree(self.scheme, self.project, topObjectParameters={"name": name})
        ...
        for chemAtom in chemComp.chemAtoms:
            if chemAtom.elementSymbol:
                ...
                for isotope in chemElement.isotopes:
                    ...
                    isotopomer.newAtomLabel(
                        name=chemAtom.name, subType=chemAtom.subType, isotopeCode=code, weight=weight
                    )
    ...
    def getChemCompLabel(self, molType, ccpCode):
        ...
        chemCompLabel = None
        if self.scheme:
            chemCompLabel = self.scheme.findFirstChemCompLabel(molType=molType, ccpCode=ccpCode)
            if not chemCompLabel:
                chemCompLabel = self.scheme.newChemCompLabel(molType=molType, ccpCode=ccpCode)
    ...
    def updateIsotopomers(self):
        ...
        if self.scheme:
            chemCompLabels = [(label.ccpCode, label) for label in self.scheme.chemCompLabels]
            chemCompLabels.sort()
        ...
        if self.isotopomer:
            ...
            for atomName, subType in atomNames:
                ...
                for isotope in isotopeCodes:
                    atomLabel = self.isotopomer.findFirstAtomLabel(name=atomName, subType=subType, isotopeCode=isotope)
        ...
        if value is not None:
            ...
            if chemAtom and (atomLabel is None):
                ...
                atomLabel = self.isotopomer.newAtomLabel(
                    name=chemAtom.name, subType=chemAtom.subType, isotopeCode=isotopeCode, weight=value
                )
...
if __name__ == "__main__":
    ...
    if os.path.isfile(projectFile):
        ...
        labelingSchemeLocator = project.findFirstPackageLocator(targetName="ccp.molecule.ChemCompLabel")
