...
standardTipTexts = (
    "Spectrum dimension or sub dimension",
...
reducedDimHeadingList = (
    standardHeadingList[:1]
    + ("Sub Dim\nName",)
    + standardHeadingList[1:2]
    + ("Scaling\nFactors",)
    + standardHeadingList[2:]
)

reducedTipTexts = (
    standardTipTexts[:1]
    + ("Name of sub-dimension",)
    + standardTipTexts[1:2]
    + ("Scaling factors",)
    + standardTipTexts[2:]
)
...
class EditSpectrumPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def __init__(self, parent, spectrum=None, modal=False, useReducedDim=False, *args, **kw):
    ...
    def body(self, guiFrame):
        ...
        if self.isModal:
            ...
            options = ["Spectra", "Display Options", "Referencing", "Tolerances", "File Details", "Data Locations"]
            tipTexts = [
            ...
            self.tabbedFrame = TabbedFrame(
                guiFrame, options=options, tipTexts=tipTexts, callback=self.updateSpectrumButtons
            )
        ...
        if not self.isModal:
            ...
            self.expNameEntry = Entry(self, width=20, returnCallback=self.setExperimentName)
            self.specNameEntry = Entry(self, width=20, returnCallback=self.setSpectrumName)
            ...
            editGetCallbacks = [
                ...
                self.getSpectrumName,
                None,
                self.getActivePeakList,
                None,
                self.getScale,
                self.getNoise,
                None,
            ...
            editSetCallbacks = [
                ...
                self.setSpectrumName,
                None,
                self.setActivePeakList,
                None,
                self.setScale,
                self.setNoise,
                None,
            ...
            self.spectrumTable = ScrolledMatrix(
                frameA,
            ...
            commands = [self.deleteSpectra, self.editFileDetails, self.editReferencing, self.editTolerances]
            ...
            editWidgets = [
                None,
                None,
                self.posColorPulldown,
                self.negColorPulldown,
                self.sliceColorPulldown,
                None,
                self.shortcutPulldown,
                self.rankEntry,
                None,
                None,
                None,
            ...
            editGetCallbacks = [
                ...
                self.getNegColor,
                self.getSliceColor,
                self.toggleBoxVisible,
                self.getShortcut,
                self.getRank,
                self.getContourLevels,
                self.getContourLevels,
                self.toggleTryPrecalcContours,
                self.getSpectrumFont,
            ...
            editSetCallbacks = [
                ...
                None,
                None,
                self.setSpectrumFont,
                None,
            ]

            self.displayTable = ScrolledMatrix(
                frameB,
                headingList=headingList,
            ...
            texts = ["Contour\nLevels", "Contour\nFiles", "Propagate\nContours", "Set Ranks\nFrom Order"]
            tipTexts = [
                "Open popup for detailed setting of contour levels",
            ...
            commands = [self.editLevels, self.editContourFiles, self.propagateContours, self.orderRanks]
        ...
        self.refSpecPulldown = PulldownList(frameC, callback=self.selectRefSpectrum, tipText=tipText, grid=(row, 1))

        label = Label(frameC, text=" Use reduced\ndimensionality options:")
        label.grid(row=row, column=2, sticky="w")
        self.reducedDimSelect = CheckButton(
            frameC,
            callback=self.toggleReducedDim,
            tipText="Used for reduced dimensionality or " + "projected experiments when you have sub-dimensions",
        )
        ...
        if not self.isModal:
            ...
            entries = ["point", "ppm"]
            tipTexts = [
                "If you change the referencing then peak position in points stays the same",
        ...
        self.scaleFactorsWidget = MultiWidget(self, FloatEntry, callback=self.setDimScaling, minRows=0, useImages=False)
        ...
        self.dataDimRefTable = ScrolledMatrix(
            frameC, headingList=headingList, callback=self.selectDataDimRef, tipTexts=tipTexts
        )
        ...
        tipTexts = [
            "Add a new sub dimension (e.g. reduced dimensionality component) by copying the selected sub-dimension",
            "Remove selected sub-dimension",
        ]
        commands = [self.addSubDim, self.removeSubDim]

        self.refButtons = ButtonList(frameC, texts=texts, tipTexts=tipTexts, commands=commands, expands=True)
        ...
        if not self.isModal:
            ...
            self.tolTableAll = ScrolledMatrix(
                frameD,
                headingList=headingList,
                tipTexts=tipTexts,
            ...
            tipTexts = [
                "Propagate peak vs shift assignment tolerance from last selected spectrum to other selected spectra in Spectrum Overview table",
            ...
            buttons = ButtonList(
                frameD, texts=texts, tipTexts=tipTexts, commands=commands, grid=(4, 0), gridSpan=(1, 2)
            )
        ...
        tipText = (
            "Whether binary numbers in data are big endian (e.g. Mac PowerPC, SGI)\n"
        ...
        tipTexts = [
            "Select a new spectrum data file using a dialog, rather than typing it above",
        ...
        self.fileDimTable = ScrolledMatrix(
            frameE,
            headingList=headingList,
            tipTexts=tipTexts,
            initialRows=5,
    ...
    def updateSpectra(self):
        ...
        self.spectrumTable.update(textMatrix=textMatrix, objectList=spectra)
    ...
    def doEditMarkExtraRules(self, analysisSpec, row, col):
        ...
        if hasattr(dataSource, "block_file") and dataSource.block_file and analysisSpec.storedContours:
            return True
    ...
    def updateAnalysisSpectra(self):
        ...
        self.displayTable.update(textMatrix=textMatrix, objectList=analysisSpectra, colorMatrix=colorMatrix)
    ...
    def setRank(self, *extra):
        ...
        if replaceSpec and (replaceSpec is not self.analysisSpec):
            sortSpectra = [
                (asp.rank, asp) for asp in self.analysisProject.analysisSpectra if asp is not self.analysisSpec
            ]
            sortSpectra.sort()
    ...
    def updateTolerances(self):
        ...
        if self.tolSpectrum:
            ...
            for add in analysisDataDims:
                ...
                datum = [dataDim.dim, isotopes, add.assignTolerance, add.chemShiftWeight]

                textMatrix.append(datum)

        self.tolTable.update(textMatrix=textMatrix, objectList=analysisDataDims)
        ...
        if self.nmrProject:
            ...
            for experiment in self.nmrProject.sortedExperiments():
                ...
                for spectrum in experiment.sortedDataSources():
                    ...
                    datum = [i, name, " ".join(tols), " ".join(weights)]

                    textMatrix.append(datum)
                    objectList.append(spectrum)
                    i += 1

        self.tolTableAll.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def propagateAssignTols(self):
        ...
        if showOkCancel("Confirm", msg, parent=self):
            ...
            for dataDim in primary.sortedDataDims():
                ...
                if isotopes not in isotopeDict:
    ...
    def updateReferencing(self, spectrum=None):
        ...
        for dataDim in dataDims:
            ...
            if isinstance(dataDim, Nmr.FreqDataDim):
                ...
                for dataDimRef in dataDimRefs:
                    ...
                    swhz = (dataDimRef.localValuePerPoint or dataDim.valuePerPoint) * dataDim.numPoints
                    sf = expDimRef.sf
                    if sf:
                        swppm = swhz / sf
                    else:
        ...
        if self.useReducedDim:
            ...
            editWidgets = [
                None,
                self.subDimNameEntry,
                self.isotopeMenu,
                self.scaleFactorsWidget,
                self.sfEntry,
                None,
                self.swEntry,
                self.refppmEntry,
    ...
    def updateRefButtons(self):
        ...
        if ddr and self.useReducedDim:
            ...
            if (self.isModal and len(self.spectrum.experiment.dataSources) == 1) or (
                nDdr < len(ddr.expDimRef.expDim.expDimRefs)
            ):
    ...
    def addSubDim(self):
        ...
        if self.dataDimRef:
            ...
            if self.isModal and len(self.spectrum.experiment.dataSources) == 1:
                useExpDimRef = expDim.newExpDimRef(
                    sf=edr.sf,
                    unit=edr.unit,
    ...
    def setDimScaling(self, *event):
        ...
        if dimensionScaling and values:
            ...
            dimensionScaling = dataDim.newDimensionScaling(expDimRef=expDimRef, scalingFactors=values)
    ...
    def apply(self):
        ...
        if spectrum:
            ...
            for dataDimRef in dataDimRefs:
                ...
                known = isotopeCode in chemShiftRefRatios
                if ratio:
                    ...
                    if (known or possibleIsotopeCode) and (isotopeCode != possibleIsotopeCode):
                        msg = "Given sf, the isotope (%s) looks incorrect for dim %s" % (isotopeCode, dimId)
    ...
    def updateFileDetails(self, spectrum=None):
        ...
        if spectrum:
            ...
            for ii, dataDim in enumerate(dataDims):
                datum = [dataDim.dim, dataDim.numPoints, blockSizes[ii]]
        ...
        self.fileDimTable.update(objectList=dataDims, textMatrix=textMatrix)
    ...
    def checkEndianess(self):
        ...
        test = isDataBigEndian(fileName, numberType=numberType, fileHeaderSize=fileHeaderSize, nbytes=nByte)
