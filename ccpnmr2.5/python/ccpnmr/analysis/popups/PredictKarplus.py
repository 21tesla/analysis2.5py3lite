...
couplingAtoms = [("H", "HA"), ("H", "C"), ("H", "CB"), ("C", "HA"), ("C", "C"), ("C", "CB")]
...
class PredictKarplusPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        self.couplingMatrix = ScrolledMatrix(
            frame,
        ...
        texts = []
        commands = []
        self.bottomButtons = UtilityButtonList(guiFrame, commands=commands, texts=texts, helpUrl=self.help_url)
    ...
    def setJCoupling(self, index=None):
        ...
        if self.residue and atomNames:
            ...
            if value is None:
                if coupling:
                    if showOkCancel("Confirm", "Really remove J coupling?", parent=self):
                        coupling.delete()

            else:
                if not coupling:
                    self.setResidueJCoupling(
                        self.measureJCouplingList, self.residue, atomNames, value, isSimulated=False
                    )
