...
from ccpnmr.analysis.core.WindowBasic import getWindowPaneName, windowPaneHasValueAxis
...
MIN_MAX_HEADINGS = ["Axis", "Region\nNumber", "Override Region\nMin (ppm)", "Override Region\nMax (ppm)"]

CENTER_WIDTH_HEADINGS = ["Axis", "Region\nNumber", "Override Region\nCenter (ppm)", "Override Region\nWidth (ppm)"]
    ...
    **Print Window to Output File**
    ...
    def body(self, guiFrame):
        ...
        self.windowPulldown = PulldownList(
            guiFrame, grid=(0, 1), tipText="The window that will be printed out", callback=self.selectWindow
        )
        texts = ["Save Print File"]
        tipTexts = ["Save the printout to the specified file"]
        commands = [self.saveFile]
        buttons = UtilityButtonList(
            guiFrame, helpUrl=self.help_url, grid=(row, 2), commands=commands, texts=texts, tipTexts=tipTexts
        )
        ...
        self.overrideSpectrum = CheckButton(
            frame,
        ...
        button = Button(
            frame, text="Reset Selected", tipText=tipText, command=self.resetSelected, grid=(0, 1), sticky="e"
        )

        headings = ["Spectrum", "Pos. Contours\nDrawn", "Neg. Contours\nDrawn"]
        tipTexts = [
            "Spectrum in window",
            "Whether the positive contours should be drawn",
            "Whether the negative contours should be drawn",
        ]
        ...
        self.overridePeakList = CheckButton(
            frame,
            text="Use below settings when printing",
            tipText="Use below settings when printing instead of the window values",
            grid=(0, 0),
        ...
        self.overrideButton = CheckButton(
            frame, text="Use override region when printing", tipText=tipText, callback=self.toggledOverride, grid=(0, 0)
        )
        ...
        self.useButtons = RadioButtons(
            frame, entries=USE_ENTRIES, tipTexts=tipTexts, select_callback=self.changedUseEntry, grid=(1, 0)
        )

        texts = ("Set Region from Window", "Set Center from Window", "Set Width from Window")
        tipTexts = (
            "Set the override region to be the current window region",
        ...
        self.setRegionButton = ButtonList(frame, texts=texts, tipTexts=tipTexts, commands=commands, grid=(2, 0))
        ...
        self.regionTable = RegionScrolledMatrix(
            frame,
            headingList=headings,
            editWidgets=editWidgets,
            editGetCallbacks=editGetCallbacks,
    ...
    def resetPeakList(self):
        ...
        for windowPeakList in windowPeakLists:
            ...
            PrintBasic.setPrintOption(
                windowPeakList, "PeakFont", windowPeakList.spectrumWindowView.analysisSpectrum.font
            )
    ...
    def togglePos(self, spectrumWindowView):
        ...
        PrintBasic.setPrintOption(
            spectrumWindowView,
            "PositiveOn",
            not PrintBasic.getPrintOption(
                spectrumWindowView, "PositiveOn", defaultValue=spectrumWindowView.isPosVisible
            ),
        )
    ...
    def toggleNeg(self, spectrumWindowView):
        ...
        PrintBasic.setPrintOption(
            ...
            "NegativeOn",
            not PrintBasic.getPrintOption(
                spectrumWindowView, "NegativeOn", defaultValue=spectrumWindowView.isNegVisible
            ),
        )
    ...
    def saveFile(self):
        ...
        if isOverrideSpectrumSelected:
            ...
            for spectrumWindowView in spectrumWindowViews:
                spectrumWindowView.printPositive = PrintBasic.getPrintOption(
                    spectrumWindowView, "PositiveOn", spectrumWindowView.isPosVisible
                )
                spectrumWindowView.printNegative = PrintBasic.getPrintOption(
                    spectrumWindowView, "NegativeOn", spectrumWindowView.isNegVisible
                )
        ...
        if isOverridePeakListSelected:
            ...
            for windowPeakList in windowPeakLists:
                windowPeakList.printPeaks = PrintBasic.getPrintOption(
                    windowPeakList, "PeaksOn", windowPeakList.isSymbolDrawn
                )
                if windowPeakList.isAnnotationDrawn:
                    default = windowPeakList.analysisPeakList.analysisSpectrum.font
                else:
        ...
        dyy = abs(yrr[0] - yrr[1])
        try:
            outputHandler = printFrame.getOutputHandler(
                pixelWidth, pixelHeight, unitWidth, unitHeight, fonts=printNames
            )
    ...
    def selectWindow(self, windowPane):
        ...
        if windowPane:
            ...
            application.setValue(self.analysisProject, keyword="printWindowWindow", value=window.name)
    ...
    def updateSpectrumTable(self):
        ...
        if windowPane:
            ...
            for spectrumWindowView in spectrumWindowViews:
                ...
                positiveOn = PrintBasic.getPrintOption(
                    spectrumWindowView, "PositiveOn", spectrumWindowView.isPosVisible
                )
                text.append(positiveOn and "Yes" or "No")
                negativeOn = PrintBasic.getPrintOption(
                    spectrumWindowView, "NegativeOn", spectrumWindowView.isNegVisible
                )
                text.append(negativeOn and "Yes" or "No")
                textMatrix.append(text)

        self.spectrumTable.update(objectList=spectrumWindowViews, textMatrix=textMatrix)
    ...
    def updatePeakListTable(self):
        ...
        self.peakListTable.update(objectList=windowPeakLists, textMatrix=textMatrix)
    ...
    def updateRegionTable(self):
        ...
        self.regionTable.update(headingList=headings, objectList=axisRegions, textMatrix=textMatrix)
