...
class ViewRamachandranPopup(BasePopup):
    ...
    **Display Protein Backbone Phi & Psi Angles**
    ...
    def body(self, guiFrame):
        ...
        self.molSystemPulldown = PulldownList(frame, callback=self.setMolSystem, grid=(0, 1), tipText=tipText)
        ...
        self.structurePulldown = PulldownList(frame, callback=self.setStructure, grid=(0, 3), tipText=tipText)
        ...
        self.modelPulldown = PulldownList(frame, callback=self.setModel, grid=(0, 5), tipText=tipText)
        ...
        self.labelPulldown = PulldownList(
            frame, texts=LABEL_MODES, index=1, callback=self.updateLabels, grid=(0, 7), sticky="e", tipText=tipText
        )
        ...
        self.ccpCodePulldown = PulldownList(frame, callback=self.setCcpCode, grid=(1, 1), tipText=tipText)

        label = Label(frame, text=" Residue:", grid=(1, 2))
        tipText = "Allows the display of phi/psi points from only a single residue"
        self.residuePulldown = PulldownList(frame, callback=self.setResidue, grid=(1, 3), tipText=tipText)
        ...
        self.spotSizePulldown = PulldownList(
            frame, texts=texts, objects=sizes, callback=self.updateAfter, index=1, grid=(1, 5), tipText=tipText
        )

        label = Label(frame, text=" Colours:", grid=(1, 6))
        tipText = (
            "Selects which colour scheme to use to distinguish phi/psi points from different conformational models"
        )
        self.schemePulldown = PulldownList(frame, callback=self.selectColorScheme, grid=(1, 7), tipText=tipText)
        ...
        self.regionLabel = Label(guiFrame, text=REGION_TEXT % (0.0, 0.0, 0.0), grid=(row, 0), tipText=tipText)
        ...
        self.plot = ViewRamachandranFrame(guiFrame, relief="sunken", bgColor=self.cget("bg"))
        self.plot.grid(row=row, column=0, sticky="nsew")

        row += 1
        tipTexts = [
            "Show phi/spi spots for the previous residue in the sequence; when there is no current residue, starts fron the first in sequence there is data for",
        ...
        self.bottomButtons = ButtonList(guiFrame, commands=commands, texts=texts, grid=(row, 0), tipTexts=tipTexts)
