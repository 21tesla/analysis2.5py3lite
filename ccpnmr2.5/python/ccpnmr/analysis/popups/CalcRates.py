...
from ccpnmr.analysis.core.DataAnalysisBasic import (
    ...
    getPeakSampledDimIntensities,
    getPeakSeriesIntensities,
    makeRatesList,
    matchSampledExperimentPeaks,
    matchSeriesPeaks,
    setFitErrorMethod,
)
from ccpnmr.analysis.core.ExperimentBasic import getExperimentSampledDim, getSampledDimExperiments, getSpectrumNoise
...
ALLOWED_CONDITION_TYPES = ("delay time", "Time", "num delays", "mixing time", "pulsing frequency", GRADIENT_STRENGTH)
...
class CalcRatesPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tipTexts = [
            "Allows the user to setup how peak intensities are followed and which experiments to analyse",
        ...
        self.fitErrorPulldown = PulldownList(
            frame, callback=self.setFitErrorMethod, texts=fitErrorMethods, index=ind, tipText=tipText, grid=(0, 5)
        )
        ...
        tipText = (
            "Whether to pick new peaks, according to the reference peaks, should none of the right assignment be found"
        )
        ...
        self.assignSelect = CheckButton(frame, callback=self.setSkipMerit, grid=(0, 7), selected=True, tipText=tipText)
        ...
        tipTexts = [
            "Spectrum dimension for tolerance setting",
        ...
        tipTexts = [
            "Remove the selected peak groups from the table",
        ...
        texts = ["Remove\nSelected Groups", "Re-fit\nSelected", "Show Fit\nGraph", "Show\nPeaks", "Make Rates List"]
        commands = [self.removeGroup, self.recalcRates, self.showFunctionFit, self.showPeaks, self.makeRatesList]
        bottomButtons = ButtonList(frameB, expands=True, tipTexts=tipTexts, commands=commands, texts=texts)
        ...
        tipTexts = [
            "Search for peaks near to reference positions to create peak groups",
        ...
        texts = ["Group & Fit Peaks", "Re-fit All Groups", "Edit NMR Series", "Export Data"]
        commands = [self.matchPeaks, self.calcAllRates, self.editExpSeries, self.exportData]
        buttonList = ButtonList(guiFrame, commands=commands, texts=texts, tipTexts=tipTexts)
    ...
    def showFunctionFit(self, noOpen=0):
        ...
        if self.fitGraphPopup:
            self.fitGraphPopup.update(
                dataFitting=dataFitting, xLabel=self.rateType, yLabel=self.intensityType, graphTitle=title
            )
            if not noOpen:
    ...
    def getPeakSeriesData(self, dataFitting):
        ...
        if self.expSeries:
            ...
            useTotalDelay = False
            if expSeries.className == "Experiment":
                (xData, yData, xWidths, yWidths, refIntensity) = getPeakSampledDimIntensities(
                    peaks, expSeries, self.intensityType
                )
                ...
                (xData, yData, xWidths, yWidths) = getPeakSeriesIntensities(
                    peaks, expSeries, self.intensityType, sampleConditionType
                )
    ...
    def matchPeaks(self):
        ...
        expSeries = self.expSeries
        if expSeries.className == "Experiment":
            groups = matchSampledExperimentPeaks(
                self.peakList,
                expSeries,
                tolerances,
                self.pickPeaks,
    ...
    def makeRatesList(self):
        ...
        if self.peakGroups and self.expSeries:
            ...
            measurementList = makeRatesList(
                self.rateType, self.expSeries, self.seriesType, peakGroups, values, errors=errors
            )
...
"""
    ...
    def getHeadingList(self, numDim):
        ...
        if seriesType == GRADIENT_STRENGTH:
            ...
            tipTexts.extend(
                [
                    "Diffucsion coeffient in m^2/s; calculated using the DOSY parameters and fitted decay",
            ...
            tipTexts.extend(
                [
                    "Time constant, e.g. T1 or T2; one over the decay rate for exponential fits",
        ...
        tipTexts.extend(
            [
                "Error in the fit of the peak intensities to the fitting function",
