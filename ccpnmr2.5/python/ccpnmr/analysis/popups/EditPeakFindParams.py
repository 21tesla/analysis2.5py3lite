...
from ccpnmr.analysis.core import ExperimentBasic, PeakBasic, PeakFindParams, UnitConverter
from ccpnmr.analysis.core.Util import getIsotopeExclusion, getNmrIsotopes, setIsotopeExclusion
...
class EditPeakFindParamsPopup(BasePopup):
    ...
    ** Region Peak Find **
    ...
    def body(self, guiFrame):
        ...
        tipTexts = ["", "", "", ""]
        options = ["Find Parameters", "Spectrum Widths", "Diagonal Exclusions", "Region Peak Find"]
        ...
        row = 0
        label = LabelFrame(frameA, text="Extrema to search for:", grid=(row, 0), gridSpan=(1, 2))
        ...
        self.extrema_buttons = RadioButtons(
            label, entries=entries, select_callback=self.apply, direction="horizontal", grid=(0, 0), tipTexts=tipTexts
        )

        row += 1
        label = LabelFrame(frameA, text="Nearby points to check:", grid=(row, 0), gridSpan=(1, 2))
        ...
        self.adjacent_buttons = RadioButtons(
            label, entries=entries, select_callback=self.apply, direction="horizontal", grid=(0, 0), tipTexts=tipTexts
        )

        row += 1
        labelFrame = LabelFrame(frameA, text="Other parameters:", grid=(row, 0), gridSpan=(1, 2))
        labelFrame.expandGrid(5, 2)

        frow = 0
        label = Label(labelFrame, text="Scale relative to contour levels:", grid=(frow, 0), sticky="e")
        ...
        self.scale_entry = FloatEntry(labelFrame, grid=(frow, 1), tipText=tipText, returnCallback=self.apply, width=10)
        self.scale_entry.bind("<Leave>", self.apply, "+")

        frow += 1
        label = Label(labelFrame, text="Exclusion buffer around peaks (in points):", grid=(frow, 0), sticky="e")
        tipText = (
            "The size of the no-pick region, in data points, around existing picked peaks; eliminates duplicate picking"
        )
        self.buffer_entry = IntEntry(labelFrame, returnCallback=self.apply, grid=(frow, 1), width=10, tipText=tipText)
        self.buffer_entry.bind("<Leave>", self.apply, "+")

        frow += 1
        label = Label(labelFrame, text="Extra thickness in orthogonal dims (in points):", grid=(frow, 0), sticky="e")
        tipText = "Sets whether to consider any additional planes (Z dimension) when calculating peak volume integrals"
        self.thickness_entry = IntEntry(
            labelFrame, returnCallback=self.apply, width=10, grid=(frow, 1), tipText=tipText
        )
        self.thickness_entry.bind("<Leave>", self.apply, "+")

        frow += 1
        label = Label(labelFrame, text="Minimum drop factor (0.0-1.0):", grid=(frow, 0), sticky="e")
        tipText = ""
        self.drop_entry = FloatEntry(labelFrame, returnCallback=self.apply, width=10, grid=(frow, 1), tipText=tipText)
        self.drop_entry.bind("<Leave>", self.apply, "+")

        frow += 1
        label = Label(labelFrame, text="Volume method:", grid=(frow, 0), sticky="e")
        ...
        self.method_menu = PulldownList(
            labelFrame, texts=PeakBasic.PEAK_VOLUME_METHODS, grid=(frow, 1), callback=self.apply, tipText=tipText
        )
        ...
        self.expt_spectrum = PulldownList(frameB, tipText=tipText, callback=self.setSpectrumProperties)
        ...
        self.isotopeMatrix = ScrolledMatrix(
            frameC,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            editWidgets=editWidgets,
        ...
        self.regionPeakListPulldown = PulldownList(
            frameD, callback=self.changeRegionPeakList, grid=(0, 1), tipText=tipText
        )
        ...
        self.conditionMenu = PulldownList(self, texts=("include", "exclude"), callback=self.setCondition)
        ...
        texts = ["Whole Region", "Add Region", "Delete Region", "Adjust Params", "Find Peaks!"]
        commands = [self.wholeRegion, self.addCondition, self.deleteCondition, self.adjustParams, self.regionFindPeaks]
        ...
        utilButtons = UtilityButtonList(tabbedFrame.sideFrame, grid=(0, 0), helpUrl=self.help_url, sticky="e")
        ...
        for i in range(len(dataDims)):
            ...
            if isinstance(dataDim, Nmr.FreqDataDim):
                ...
                textMatrix.append(
                    [
                        dataDim.dim,
                        ExperimentBasic.getPrimaryDataDimRef(dataDim).expDimRef.isotopeCodes[0],
    ...
    def changeRegionPeakList(self, peakList):
            ...
            if spectrum1.numDim == spectrum2.numDim:
                for n in range(spectrum1.numDim):
                    ...
                    if (dataDim1.className == "FreqDataDim") and (dataDim2.className == "FreqDataDim"):
                        if (
                            ExperimentBasic.getPrimaryDataDimRef(dataDim1).expDimRef.isotopeCodes
                            != ExperimentBasic.getPrimaryDataDimRef(dataDim2).expDimRef.isotopeCodes
                        ):
                            break
                else:
        ...
        for dataDim in dataDims:
            ...
            tipTexts.append(
                "Lower value bound of peak picking inclusion/exclusion region for spectrum dimension %s" % dim
            )
            tipTexts.append(
                "Upper value bound of peak picking inclusion/exclusion region for spectrum dimension %s" % dim
            )
            (rMin, rMax) = self.getWholeRegion(dataDim)
            textRow.append(rMin)
            textRow.append(rMax)
