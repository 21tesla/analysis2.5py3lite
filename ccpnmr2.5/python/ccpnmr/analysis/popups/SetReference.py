...
from ccpnmr.analysis.core.ExperimentBasic import getPrimaryDataDimRef, isSpectrum
...
class SetReferencePopup(BasePopup):
    def __init__(self, parent, dataDim, position):
        ...
        BasePopup.__init__(self, parent=parent, title="Set spectrum referencing", modal=True, transient=True)
    ...
    def body(self, guiFrame):
        ...
        label = Label(guiFrame, text=spectrum.experiment.name, grid=(row, 1), tipText=tipText)

        row += 1
        label = Label(guiFrame, text="Spectrum:", grid=(row, 0), sticky="e")
        tipText = "The name of the spectrum to be re-referenced"
        label = Label(guiFrame, text=spectrum.name, grid=(row, 1), tipText=tipText)
        ...
        label = Label(guiFrame, text=self.dataDimText(dataDim), grid=(row, 1), tipText=tipText)
        ...
        self.refppm_entry = FloatEntry(guiFrame, width=30, grid=(row, 1), sticky="ew", tipText=tipText)
        ...
        self.datadim_list = ScrolledMatrix(
            guiFrame,
