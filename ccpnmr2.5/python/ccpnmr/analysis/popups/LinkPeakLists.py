...
class LinkPeakListsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=options, grid=(row, 0), tipTexts=tipTexts)
        ...
        self.peakListPulldown = PulldownList(frame, callback=self.changeRootPeakList, grid=(0, 1), tipText=tipText)
        ...
        self.rootPanePulldown = PulldownList(frame, callback=self.changeRootWindow, grid=(0, 3), tipText=tipText)

        div = LabelDivider(frameA, text="Target Windows", grid=(1, 0), gridSpan=(1, 2))

        tipTexts = [
            "Remove the selected target window from consideration, so that it is no longer used for navigation and assignment",
        ...
        self.windowButtons = ButtonList(frameA, texts=texts, tipTexts=tipTexts, commands=commands, grid=(2, 0))

        tipText = (
            "Selects a spectrum window, from those not already selected, that may be used for assignment/navigation"
        )
        self.windowPulldown = PulldownList(frameA, callback=None, grid=(2, 1), tipText=tipText)
        ...
        self.windowMatrix = ScrolledMatrix(
            frameA,
        ...
        tipTexts = [
            'The "experiment:spectrum" name for the spectrum which may be used for peak picking & assignment; must be present in the above windows',
        ...
        self.spectrumMatrix = ScrolledMatrix(
            frameA,
            headingList=headingList,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        self.tolEntry1 = FloatEntry(
            frame, returnCallback=self.setDataDimTolerances, width=8, grid=(0, 1), tipText=tipText
        )
        ...
        self.tolEntry2 = FloatEntry(
            frame, returnCallback=self.setDataDimTolerances, width=8, grid=(1, 1), tipText=tipText
        )
        ...
        self.minWaterEntry = FloatEntry(frame, width=8, text=4.95, grid=(2, 1), tipText=tipText)
        ...
        self.maxWaterEntry = FloatEntry(frame, width=8, text=4.95, grid=(3, 1), tipText=tipText)
        ...
        self.marksEntry = IntEntry(frame, width=8, text=1, grid=(4, 1), tipText=tipText)
        ...
        self.diagEntry = FloatEntry(frame, width=8, text=0.25, grid=(5, 1), tipText=tipText)
        ...
        self.followRootSelect = CheckButton(frame, callback=None, tipText=tipText, grid=(0, 1), selected=True)
        ...
        self.followTargetSelect = CheckButton(frame, callback=None, tipText=tipText, grid=(0, 3), selected=True)
        ...
        self.assignTypeSelect = CheckButton(frame, callback=None, tipText=tipText, grid=(0, 5), selected=False)
        ...
        self.rootPeakMatrix = ScrolledMatrix(
            frameC, headingList=headingList, callback=self.selectRootPeak, grid=(2, 0), tipTexts=tipTexts
        )

        tipTexts = [
            "If a peak is selected, select the next peak in the table and navigate to that peaks root location",
        ...
        self.prevNextButtons = ButtonList(frameC, texts=texts, tipTexts=tipTexts, commands=commands, grid=(3, 0))

        div = LabelDivider(frameC, text="Pick & Assign Functions", grid=(4, 0))

        tipTexts = [
            "Using the selected root peak as the source of assignment, spread assignments to the active spectra, within stated tolerances",
        ...
        texts = ["Assign\nRoot Resonances", "Pick & Assign\nRoot Resonances", "Assign Non-root\nResonances"]
        commands = [self.assignTarget, self.pickAssignTarget, self.assignNonRootTarget]
        self.rootPeakButtons = ButtonList(frameC, texts=texts, tipTexts=tipTexts, commands=commands, grid=(5, 0))

        div = LabelDivider(frameC, text="Process-all Functions", grid=(6, 0))

        tipTexts = [
            "Using all peaks in the root peak list as an assignment source, spread assignments to the active spectra, within stated tolerances",
        ...
        commands = [self.assignAllTarget, self.pickAssignAllTarget, self.assignNonRootAllTarget]
        self.bottomButtons = ButtonList(
            frameC, commands=commands, texts=texts, grid=(7, 0), sticky="ew", tipTexts=tipTexts
        )

        buttons = UtilityButtonList(tabbedFrame.sideFrame, helpUrl=self.help_url, grid=(0, 0), sticky="e")
    ...
    def navigateToTarget(self):
        ...
        if self.rootPeak and self.rootPane and self.followTargetSelect.get():
            position = getPeakWindowPosition(self.rootPeak, self.rootPane, useDefault=False)
            ...
            windowZplanes = findOrthogonalWindows(
                self.rootPane,
                [
                    position,
                ],
            ...
            for planeName0, windowPane, positions in windowZplanes:
                planeName = planeName0.split(" in ")[0]
                if (windowPane in self.windowPanes) and (planeName == self.selDimMapping.get(windowPane)):
                ...
                for peakList in self.targetPeakLists:
                    if self.nonRootDict.get(self.getPeakListId(peakList)):
                        ...
                        self.progressBar.setText(
                            "Working on %s:%s:%s" % (experiment.name, spectrum.name, peakList.serial)
                        )
                        assignSpecNonRootResonances(
                            self.peakList.peaks,
                            peakList,
    ...
    def setAppDataOptions(self):
        ...
        if self.rootPane:
            options["rootWindow"] = (self.rootPane.spectrumWindow.serial, self.rootPane.serial)
        if self.peakList:
    ...
    def togglePeakList(self, peakList):
        ...
        if peakList:
            ...
            if onViews:
                for view in onViews:
                    toggleSpectrum(view.spectrumWindowPane.spectrumWindow, spectrum=view.analysisSpectrum.dataSource)
    ...
    def updateSpectra(self, object=None):
        ...
        for peakList in peakLists:
            ...
            datum = ["%s:%s" % (experiment.name, spectrum.name), isActive, yesNo, refExpName, " ".join(isotopes)]

            textMatrix.append(datum)
            objectList.append(peakList)

        self.spectrumMatrix.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
        self.setAppDataOptions()
    ...
    def updateWindows(self, windowPane=None):
        ...
        self.updateSpectra()
        self.windowMatrix.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def update(self):
        ...
        self.rootPeakMatrix.update(
            textMatrix=textMatrix, objectList=objectList, headingList=headingList, tipTexts=self.rootPeakTipTexts
        )
