...
class ViewNoeMatrix(BasePopup):
    ...
    **Tips**
    ...
    def body(self, guiFrame):
        ...
        self.noeMatrix = NoeMatrix(frameA, borderwidth=1, relief="flat", background="darkGrey", labelAxes=False)
        ...
        self.constraintSetPulldown = PulldownList(frameC, self.changeConstraintSet, grid=(0, 1), tipText=tipText)
        ...
        tipText = (
            "Selects which molecular chain to use as the sequence along the horizontal axis of the interaction matrix"
        )
        self.xMolPulldown = PulldownList(frameD, callback=self.changeMolX, grid=(0, 1), tipText=tipText)
        tipText = (
            "Selects which molecular chain to use as the sequence along the vertical axis of the interaction matrix"
        )
        self.yMolPulldown = PulldownList(frameD, callback=self.changeMolY, grid=(1, 1), tipText=tipText)

        tipText = "Sets the number of the first residue that appears at the start (left) of the horizontal matrix axis"
        self.xEntryStart = IntEntry(
            frameD, text="", returnCallback=self.setMolRanges, width=5, grid=(0, 2), tipText=tipText
        )
        tipText = "Sets the number of the first residue that appears at the start (bottom) of the vertical matrix axis"
        self.yEntryStart = IntEntry(
            frameD, text="", returnCallback=self.setMolRanges, width=5, grid=(1, 2), tipText=tipText
        )
        ...
        self.xEntryStop = IntEntry(
            frameD, text="", returnCallback=self.setMolRanges, width=5, grid=(0, 4), tipText=tipText
        )
        tipText = "Sets the number of the last residue that appears at the end (top) of the vertical matrix axis"
        self.yEntryStop = IntEntry(
            frameD, text="", returnCallback=self.setMolRanges, width=5, grid=(1, 4), tipText=tipText
        )
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("__init__", "delete", "setName"):
            for clazz in ("ccp.nmr.NmrConstraint.DistanceConstraintList", "ccp.nmr.NmrConstraint.HBondConstraintList"):
    ...
    def updateConstraintLists(self, constraintList=None):
        ...
        self.constraintMatrix.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
