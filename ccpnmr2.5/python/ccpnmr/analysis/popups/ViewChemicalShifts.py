...
class ViewChemicalShiftsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.otherShiftsSelect = CheckButton(frame, callback=self.draw, grid=(0, 5), tipText=tipText)

        utilButtons = UtilityButtonList(frame, helpUrl=self.help_url, grid=(0, 7))
        ...
        self.oneLetterSelect = CheckButton(frame, callback=self.draw, grid=(0, 1), selected=False, tipText=tipText)
        ...
        self.protonPrecisionSelect = PulldownList(
            frame, texts=texts, objects=precisions, callback=self.draw, index=1, grid=(0, 3), tipText=tipText
        )

        label = Label(frame, text="  Other precision:")
        label.grid(row=0, column=4, sticky="w")
        tipText = (
            "Specifies how many decimal places to use when displaying chemical shift values for isotopes other than 1H"
        )
        self.otherPrecisionSelect = PulldownList(
            frame, texts=texts, objects=precisions, callback=self.draw, index=1, grid=(0, 5), tipText=tipText
        )
        ...
        self.optSelector = PartitionedSelector(
            frame, self.toggleOpt, tipText=tipText, maxRowObjects=10, grid=(0, 1), sticky="ew"
        )
        options = ["H", "N", "C", "CA", "CB", "CG"]
        self.optSelector.update(objects=options, labels=options, selected=["H", "N", "CA"])

        row += 1
        guiFrame.expandGrid(row, 0)
        self.canvasFrame = ScrolledCanvas(
            guiFrame, relief="groove", width=650, borderwidth=2, resizeCallback=None, grid=(row, 0), padx=1, pady=1
        )
        ...
        buttonList = ButtonList(guiFrame, commands=commands, texts=texts, grid=(row, 0), tipTexts=tipTexts)
    ...
    def update(self):
        ...
        self.optSelector.update(objects=options, labels=options, colors=colors, selected=list(selected))
