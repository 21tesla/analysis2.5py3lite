...
from ccpnmr.analysis.core.ContourStore import createStoredContour, saveSpectrumContours
...
class CreateContourFilePopup(BasePopup):
    ...
    **Create Contour Files for Project**
    ...
    def body(self, master):
        ...
        self.expt_spectrum = PulldownList(frame, grid=(row, 1), tipText=tipText, callback=self.update)
        ...
        self.dim_menu = PulldownList(frame, grid=(row, 3), tipText=tipText, callback=self.updateFile)
        ...
        editWidgets = [None]
        self.conditionTable = ScrolledMatrix(
            master,
            initialRows=6,
            grid=(row, 0),
            gridSpan=(1, 2),
        ...
        row = row + 1
        tipTexts = [
            "Add a new contour region for including in or excluding from the contour file ",
...
"""
    ...
    def updateConditionTable(self, *extra):
        ...
        if spectrum:
            ...
            ndim = 0
        tipTexts = [
            "Whether to include or exclude the region in the contour file",
        ]
        headingList = ["Condition"]
...
"""
    ...
    def continueIfFileNameUsed(self, fileName):
        ...
        if storedContoursToDelete:
            ...
            result = showYesNo(
                "File used",
                "Stored contour%s already use%s this fileName, and will be deleted: continue?" % (s, t),
                parent=self,
            )
            if result:
    ...
    def contourAndSaveSpectrum(self):
        ...
        try:
            ...
            saveSpectrumContours(
                spectrum, fullPath, xdim, ydim, levels, firstInt, lastInt, mem_cache=self.parent.mem_cache
            )
        except Exception as e:
