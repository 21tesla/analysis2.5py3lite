...
from ccpnmr.analysis.core.ExperimentBasic import (
    cloneExperiment,
    defaultExperiment,
    getFilteredRefExperiments,
    getPrimaryExpDimRef,
    getRefExperimentCategories,
    getRefExperiments,
    initExpTransfers,
    isSpectrum,
    newShiftList,
    setExperimentShiftList,
    setRefExperiment,
)
...
class EditExperimentPopup(BasePopup):
    ...
    *A nomenclature and data model to describe NMR experiments.
    ...
    def body(self, guiFrame):
        ...
        options = ["Experiments", "Experiment Types", "Experimental Details", "Shift References", "NMR Instruments"]
        if self.isModal:
            options[1] = "Specify " + options[1]

        tabbedFrame = TabbedFrame(guiFrame, options=options, tipTexts=tipTexts, grid=(0, 0))
        ...
        editWidgets = [
            ...
            None,
            self.acqDimPulldown,
            self.shiftListPulldown,
            None,
            self.molSystemSelect,
            self.shiftRefSelect,
            None,
            None,
            self.detailsEntry,
        ...
        row += 1
        tipTexts = [
            "Show a table of NMR measurement lists (chemical shifts, T1, J-coupling etc) that relate to the selected experiment",
        ...
        commands = [self.showMeasurementLists, self.cloneExperiment, self.newExperiment, self.deleteExperiment]
        self.experimentButtons = ButtonList(frameA, commands=commands, texts=texts, tipTexts=tipTexts, grid=(row, 0))
        ...
        self.categoryPulldown = PulldownList(self, texts=self.getCategories(), callback=self.setCategory)
        self.extSourcePulldown = PulldownList(self, texts=self.getExtSources(), callback=self.setExtSource)
        ...
        editWidgets = [
            None,
            None,
            self.extSourcePulldown,
            self.extNameEntry,
            self.categoryPulldown,
            self.synonymPulldown,
            self.fullTypePulldown,
            None,
        ]
        editGetCallbacks = [
            None,
        ...
        self.expTypeMatrix = ScrolledMatrix(
            frameB,
        ...
        row += 1
        tipTexts = [
            "Spread the experiment type specification, where possible, from the last selected experiment to all highlighted experiments",
        ...
        self.expTypeButtons = ButtonList(
            frameB, commands=commands, grid=(row, 0), gridSpan=(1, 2), texts=texts, tipTexts=tipTexts
        )
        ...
        tipTexts = [
            "The first recorded experimental dimension involved in a magnetisation transfer",
        ...
        self.transferMatrix = ScrolledMatrix(
            frame,
            headingList=headingList,
        ...
        tipTexts = [
            "The number of the experimental dimension",
        ...
        self.refExpDimMatrix = ScrolledMatrix(
            frame,
            headingList=headingList,
        ...
        row = 0
        self.scansEntry = IntEntry(self, text=0, width=10, returnCallback=self.setNumScans)
        ...
        self.statePulldown = PulldownList(self, texts=SAMPLE_STATES, callback=self.setSampleState)

        self.volumeEntry = FloatEntry(self, text=0.0, width=10, returnCallback=self.setSampleVolume)

        self.unitPulldown = PulldownList(self, texts=VOLUME_UNITS, index=1, callback=self.setSampleVolUnit)

        self.tubeEntry = Entry(self, text="", width=10, returnCallback=self.setNmrTube)

        self.spinRateEntry = FloatEntry(self, text=0.0, width=10, returnCallback=self.setSpinRate)

        self.spinAngleEntry = FloatEntry(self, text=0.0, width=10, returnCallback=self.setSpinAngle)
        tipTexts = [
            "The serial number of the experiment",
        ...
        editGetCallbacks = [
            None,
            None,
            self.getSpectrometer,
            self.getProbe,
            self.getNumScans,
            self.getSampleState,
            self.getSampleVolume,
            self.getSampleVolUnit,
            self.getNmrTube,
            self.getSpinAngle,
            self.getSpinRate,
        ]
        ...
        editSetCallbacks = [
            ...
            self.setNumScans,
            self.setSampleState,
            self.setSampleVolume,
            self.setSampleVolUnit,
            self.setNmrTube,
            self.setSpinAngle,
            self.setSpinRate,
        ]

        self.detailsMatrix = ScrolledMatrix(
            frameC,
            multiSelect=True,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "The type of experimental condition that is described for the sample",
        ...
        self.conditionsMatrix = ScrolledMatrix(
            frameC,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Open a table to view and edit the specification of experimental and sample conditions",
        ]
        commands = [
            self.editConditions,
        ]
        ...
        row += 1
        tipTexts = [
            "Display a table to view and administer NMR spectrometer and probe specifications",
        ...
        self.expDetailsButtons = ButtonList(
            frameC, texts=texts, commands=commands, tipTexts=tipTexts, grid=(row, 0), gridSpan=(1, 2)
        )
        ...
        self.ratioEntry = FloatEntry(self, text=0.0, width=18, formatPlaces=16, returnCallback=self.setShiftRefRatio)
        self.unitPulldown2 = PulldownList(self, texts=STANDARD_UNITS, callback=self.setShiftRefUnit)
        ...
        self.shiftRefMatrix = ScrolledMatrix(
            frameD,
            multiSelect=True,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        self.shiftRefButtons = ButtonList(frameD, texts=texts, tipTexts=tipTexts, commands=commands, grid=(1, 0))
        ...
        self.probeButtons = ButtonList(frameE, texts=texts, tipTexts=tipTexts, commands=commands, grid=(row, 0))
        ...
        self.spectrometerModelEntry = Entry(self, text="", width=10, returnCallback=self.setSpectrometerModel)
        ...
        self.spectrometerButtons = ButtonList(frameE, texts=texts, tipTexts=tipTexts, commands=commands, grid=(row, 0))
        ...
        if self.isModal:
            dismissText = "Done"
        bottomButtons = UtilityButtonList(self.tabbedFrame.sideFrame, helpUrl=self.help_url, closeText=dismissText)
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("setShiftReferences", "addShiftReference", "removeShiftReference"):
    ...
    def close(self):
        ...
        if self.isModal:
            ...
            if names:
                if len(names) == 1:
                    msg = "Experiment %s does not have a reference experiment type set. Try again?" % names[0]
                else:
...
"""
    ...
    def deleteExperiment(self, *event):
        ...
        if len(experiments) == 1:
            if showOkCancel(
                "Delete Experiment", "Really delete experiment and\nany spectra, lists, peaks etc... ?", parent=self
            ):
            ...
            if showOkCancel(
                "Delete Experiment",
                "Really delete %d experiments and\nany spectra, lists, peaks etc... ?" % (len(experiments)),
                parent=self,
            ):
    ...
    def propagateExpType(self):
        ...
        if self.typeExpt and (len(experiments) > 1):
            ...
            if len(refExpDict.keys()) > 1:
                showWarning("Propagate type failed", "Ambiguous experiment types in current selection", parent=self)
                return

            if not refExperiment:
                showWarning("Warning", "No experiment type to propagate", parent=self)
                return
                ...
                for experiment in experiments:
                    ...
                    if refExperiment not in refExperiments:
                        ...
                        showWarning(
                            ...
                            "Experiment type %s not possible for experiment %s" % data,
                            parent=self,
                        )
                        continue
    ...
    def getCategories(self):
        ...
        names = ["<None>", "through-bond", "through-space", "quantification", "use external", "other"]
    ...
    def getFullTypes(self, experiment=None):
        ...
        if experiment:
            if experiment.refExperiment:
                if not (
                    hasattr(experiment, "pulProgName")
                    and hasattr(experiment, "pulProgType")
                    and hasattr(experiment, "category")
                    and experiment.category == "use external"
                ):
                    ...
                    if synonym:
                        names = [
                            (rx.name, rx)
                            for rx in getRefExperiments(experiment)
    ...
    def getCategory(self, experiment):
        ...
        if names:
            ...
            if experiment:
                ...
                elif hasattr(experiment, "pulProgName") and hasattr(experiment, "pulProgType"):
    ...
    def setCategory(self, null):
        ...
        if self.typeExpt:
            if name == "<None>":
                ...
                if self.typeExpt.refExperiment and self.typeExpt.refExperiment.nmrExpPrototype.category != name:
                    setRefExperiment(self.typeExpt, None)
    ...
    def setRefExpDim(self, null):
        ...
        if expDimRef:
            ...
            if not expDimRef.refExpDimRef or (expDimRef.refExpDimRef is not refExpDimRef):
                expDimRef2 = None
                refExpDimRef2 = expDimRef.refExpDimRef
                for expDim in self.typeExpt.expDims:
                    for expDimRef3 in expDim.expDimRefs:
                        if expDimRef3.refExpDimRef and (expDimRef3.refExpDimRef is refExpDimRef):
    ...
    def updateExpTypes(self, experiments=None):
        ...
        for experiment in self.typeExps:
            ...
            datum = [experiment.serial, experiment.name, extSource, extName, category, synonym, fullType, altTypeStr]
        ...
        if self.typeExpt:
            for expDim in self.typeExpt.sortedExpDims():
                for expDimRef in expDim.sortedExpDimRefs():
                    ...
                    datum = [expDimText, refExpDimText, ",".join(expDimRef.isotopeCodes), measurementText]
    ...
    def updateExpTransfers(self):
        ...
        self.transferMatrix.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def updateExpDetails(self):
        ...
        if self.detailsExp and self.detailsExp.sampleConditionSet:
            for sampleCondition in self.detailsExp.sampleConditionSet.sampleConditions:
                datum = [sampleCondition.condition, sampleCondition.value, sampleCondition.error, sampleCondition.unit]
...
class NewExperimentPopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        if not experiment:
            ...
            self.numDimList = PulldownList(
                guiFrame,
                texts=texts,
                objects=objects,
                index=1,
                tipText="The number of dimensions of the experiment",
                callback=self.updateIsotopeLists,
                grid=(row, 1),
                sticky="w",
            )
            row += 1
        ...
        name = ""
        self.exptNameEntry = Entry(
            guiFrame, text=name, tipText="The name of the experiment", width=30, grid=(row, 1), sticky="ew"
        )
        row += 1

        self.fileCheckButton = CheckButton(
            guiFrame,
            text="Include link for data file?",
        ...
        self.buttons = UtilityButtonList(
            guiFrame, commands=commands, texts=texts, tipTexts=tipTexts, doClone=False, grid=(row, 1), sticky="e"
        )
    ...
    def updateIsotopeLists(self, *extra):
        ...
        for m in range(n, numDim):
            ...
            isotopeList = PulldownList(
                self.isotopeFrame, texts=texts, tipText="The isotope for dimension %s" % (m + 1), grid=(0, m)
            )
    ...
    def apply(self):
        ...
        if experimentOld:
            ...
            popup = EditSpectrumPopup(
                self.guiParent, transient=False, modal=True, useReducedDim=False, spectrum=dataSourceNew
            )
