...
from ccpnmr.analysis.core import Util, WindowBasic
...
class EditWindowPopup(BasePopup):
    ...
    **Window Groups**
    ...
    def body(self, guiFrame):
        ...
        tipTexts = [
            "For the below table, show all of the spectrum windows in the current project, irrespective of the active group",
            "For the below table, show only spectrum windows that are in the currently active window group",
        ]
        self.which_windows = RadioButtons(
            frame, entries=ACTIVE_OPTIONS, tipTexts=tipTexts, select_callback=self.updateWindowList
        )
        ...
        editGetCallbacks = [
            ...
            self.getWindowName,
            self.getAspectRatio,
            self.toggleXSliceVisible,
            self.toggleYSliceVisible,
            self.getSliceRange,
            self.toggleLabelVisible,
            self.toggleMidpointVisible,
        ]  # , self.toggleZeroLineVisible]
        ...
        self.windowTable = WindowListScrolledMatrix(
            frameA,
        ...
        row += 1
        tipTexts = [
            "Open and display the selected spectrum window",
        ...
        commands = [self.openWindow, self.createWindow, self.cloneWindow, self.deleteWindow]
        self.window_buttons = ButtonList(frameA, texts=texts, commands=commands, grid=(row, 0), tipTexts=tipTexts)
        ...
        row += 1
        tipTexts = [
            'A short label for identifying the axis within its window, e.g. "x", "y", "z1", "z2"',
        ...
        editWidgets = [None, None, self.panelTypeWidget, None, None, self.sliceThicknessWidget, self.tiedAxesWidget]

        editGetCallbacks = [
            None,
            None,
            self.getPanelType,
            self.toggleScrollbarVisible,
            self.toggleSliceVisible,
            self.getThickness,
            self.getTiedAxes,
        ]

        editSetCallbacks = [None, None, self.setPanelType, None, None, self.setThickness, self.setTiedAxes]

        self.axisTable = AxisListScrolledMatrix(
            frameA,
            headingList=headings,
            initialRows=4,
        ...
        row += 1
        tipTexts = [
            "For the currently selected spectrum window, add a new axis",
        ]
        ...
        self.axis_buttons = ButtonList(frameA, texts=texts, tipTexts=tipTexts, commands=commands, grid=(row, 0))
        ...
        self.mapWindowPulldown = PulldownList(frame, callback=self.changeMapWindow, grid=(0, 1), tipText=tipText)

        texts = ("All On", "All Off", "Invert")
        commands = (
            lambda: self.setAllSpectraVisibility(True),
            lambda: self.setAllSpectraVisibility(False),
            self.invertSpectraVisibility,
        )
        tipTexts = (
            "Turn on all spectra (contours, etc.) and peak lists for this window",
        ...
        tipTexts = [
            "The name of the experiment record for the window-mappable spectrum",
            ...
            "How much the spectrum is offset in the x axis",
            "How much the spectrum is offset in the y axis",
        ]
        headings = (
            "Experiment",
            "Spectrum",
        ...
        editGetCallbacks = [
            None,
            None,
            self.toggleInToolbar,
            self.togglePosContoursVisible,
            self.toggleNegContoursVisible,
            self.toggleViewSliceVisible,
            self.toggleContourLineVisible,
            self.changeViewMapping,
            self.getXAxisOffset,
            self.getValueAxisOffset,
        ]
        ...
        row += 1
        tipTexts = [
            "Opens a table allowing editing of per-spectrum (rather than window) parameters",
        ]
        texts = ["Edit Spectra"]
        commands = [self.editSpectrum]
        self.spectrumButtons = ButtonList(
            frameB, texts=texts, commands=commands, grid=(row, 0), gridSpan=(1, 2), tipTexts=tipTexts
        )
        ...
        tipTexts = [
            "The name of the experiment record for the window-mappable peak list",
        ...
        headings = ("Experiment", "Spectrum", "Peak list", "Symbol\nDrawn?", "Annotation\nDrawn?")
        editWidgets = 5 * [None]
        editGetCallbacks = [None, None, None, self.togglePeakSymbol, self.togglePeakAnnotation]
        editSetCallbacks = 5 * [None]

        self.peakListMatrix = ScrolledMatrix(
            frameB,
            headingList=headings,
        ...
        row += 1
        tipTexts = [
            "Show a table of the peaks within the selected peak list",
        ]
        texts = [
            "Edit Peak List",
        ]
        ...
        self.peakListButtons = ButtonList(
            frameB, texts=texts, commands=commands, grid=(row, 0), gridSpan=(1, 2), tipTexts=tipTexts
        )
        ...
        row += 1
        self.windowGroupNameWidget = Entry(self, width=10, returnCallback=self.setGroupName)
        ...
        self.windowGroupTable = ScrolledMatrix(
            frameC,
            headingList=headings,
            initialRows=8,
        ...
        row += 1
        tipTexts = [
            "Sets the selected window group as the active one; only windows within this group will be displayable (in menus etc.)",
        ...
        self.windowGroup_buttons = ButtonList(frameC, texts=texts, commands=commands, grid=(row, 0), tipTexts=tipTexts)
        ...
        self.groupWindowsTable = ScrolledMatrix(
            frameC,
            headingList=headings,
            initialRows=8,
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("__init__", "delete", "setName", "addSpectrumWindow", "removeSpectrumWindow"):
    ...
    def updateWindowList(self, *extra):
        ...
        for n, windowPane in enumerate(windowPanes):
            ...
            datum = [
                n + 1,
                WindowBasic.getWindowPaneName(windowPane),
...
"""
        ...
        self.windowTable.update(objectList=windowPanes, textMatrix=textMatrix)
...
"""
    ...
    def updateAxisList(self, *extra):
        ...
        for axisPanel in axisPanels:
            ...
            if hasVisibleSlicePanel(axisPanel):
                ...
                thickness = Util.greaterOrEqualEntry(axisPanel.slicePanel.thickness, SLICE_HEIGHT_VALS)
        ...
        self.axisTable.update(objectList=axisPanels, textMatrix=textMatrix)
    ...
    def getPanelTypes(self, axisPanel=None):
        ...
        panelTypes = [panelType for panelType in panelTypes if panelType.axisType == axisPanel.axisType]
    ...
    def _getTiedNames(self, axisPanels):
        ...
        return [
            "%s:%s" % (axisPanel.spectrumWindowPane.spectrumWindow.name, axisPanel.label) for axisPanel in axisPanels
        ]
    ...
    def updateMappings(self):
        ...
        if hasValueAxis:
            ...
            editGetCallbacks = (
                editGetCallbacks1 + editGetCallbacks3 + editGetCallbacks4 + editGetCallbacks5 + editGetCallbacks6
            )
            n = len(headings) - 2
        ...
        for view in views:
            try:
                ...
                text = [spectrum.experiment.name, spectrum.name, view.isInToolbar and "Yes" or "No"]
        ...
        for view in views:
            ...
            for winPeakList in winPeakLists:
                try:
                    ...
                    text = [
                        view.analysisSpectrum.dataSource.experiment.name,
    ...
    def updateGroupList(self, notifyObj=None):
        ...
        self.windowGroupTable.update(objectList=groups, textMatrix=textMatrix)
    ...
    def updateGroupWindows(self, *extra):
        ...
        self.groupWindowsTable.update(objectList=windows, textMatrix=textMatrix)
