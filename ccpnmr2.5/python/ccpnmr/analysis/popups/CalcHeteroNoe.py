...
from ccpnmr.analysis.core.PeakBasic import getPeakAnnotation, getPeakIntensity, getPeakSeqCodes
...
class CalcHeteroNoePopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.pickPeaksSelect = CheckButton(frame0a, tipText=tipText, grid=(0, 5), selected=True)

        tipText = (
            "Whether to assign peaks in the peaks in the reference & NOE saturation lists, if not already assigned"
        )
        ...
        self.intensPulldown = PulldownList(
            frame0a, texts=["height", "volume"], callback=self.setIntensityType, tipText=tipText
        )
        self.intensPulldown.grid(row=0, column=9, sticky="w")

        divider = LabelDivider(frameA, text="Peaks", grid=(3, 0), gridSpan=(1, 6))

        tipTexts = [
            "Show the selected intensity reference peaks in the below table",
        ...
        texts = ["Show Ref Peaks", "Show Sat Peaks", "Show Assign Peaks", "Separate Peak Table"]
        commands = [self.viewRefPeakList, self.viewSatPeakList, self.viewAssignPeakList, self.viewSeparatePeakTable]
        self.viewPeaksButtons = ButtonList(frameA, expands=True, tipTexts=tipTexts, texts=texts, commands=commands)
        self.viewPeaksButtons.grid(row=4, column=0, columnspan=6, sticky="nsew")

        self.peakTable = PeakTableFrame(frameA, self.guiParent, grid=(5, 0), gridSpan=(1, 6))
        ...
        tipTexts = [
            "Force a manual update of the table; pair-up NOE saturation and reference peaks according to assigned peak positions",
        ...
        texts = ["Refresh Table", "Remove Pairs", "Show Peak Pair", "Create Hetero NOE List"]
        commands = [self.matchPeaks, self.removePair, self.showPeakPair, self.makeNoeList]
        self.pairButtons = ButtonList(frameB, tipTexts=tipTexts, grid=(1, 0), texts=texts, commands=commands)
    ...
    def makeNoeList(self):
        ...
        if self.refPeakList is self.satPeakList:
            showWarning("Same Peak List", "Ref Peak List and Sat Peak List cannot be the same", parent=self)
            return
