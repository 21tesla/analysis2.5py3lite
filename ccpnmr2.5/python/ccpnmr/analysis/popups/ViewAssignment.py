...
from ccpnmr.analysis.core.MoleculeBasic import STANDARD_ISOTOPES, areAtomsBound, getResidueCode, unicodeGreek
from ccpnmr.analysis.popups.BasePopup import BasePopup
from memops.gui.ButtonList import ButtonList, UtilityButtonList
from memops.gui.CheckButton import CheckButton
from memops.gui.Color import hsbToRgb, inverseGrey, standardColors
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(
            guiFrame,
            options=["Assignment & Connections Chart", "Options"],
        ...
        self.molPulldown = PulldownList(topFrame, callback=self.changeMolecule, grid=(0, 1), tipText=tipText)

        label = Label(topFrame, text=" Connection Type:", grid=(0, 2))
        texts = ["Intra-residue", "Sequential", "Within 4 Residues", "2 to 4 Residues", "Long Range", "All Connections"]
        tipText = "Restricts the assignment connection lines to atom groups with particular sequential relationships"
        self.modePulldown = PulldownList(
            topFrame,
            callback=self.showSetConnections,
            texts=texts,
            objects=range(len(texts)),
            index=2,
            grid=(0, 3),
            tipText=tipText,
        )

        self.canvasFrame = ScrolledCanvas(
            graphFrame, relief="groove", borderwidth=2, resizeCallback=None, height=200, grid=(1, 0)
        )
        ...
        self.specSelector = PartitionedSelector(
            self.specOptFrame, self.toggleSpectrum, maxRowObjects=5, grid=(1, 0), tipText=tipText, gridSpan=(1, 2)
        )
        ...
        tipText = (
            "Toggles particular kinds of atom group on and off in the chart (for all residues with that kind of group) "
        )
        ...
        tipTexts = [
            'Switches on assignment display and connections for atoms in "set 1"',
        ...
        buttonList = ButtonList(self.atomsOptFrame, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
        ...
        self.residueSelector = PartitionedSelector(
            self.residueFrame, self.toggleResidueType, maxRowObjects=10, gridSpan=(1, 3), grid=(0, 0), tipText=tipText
        )

        tipTexts = [
            "Set all residue types, and any of the selected atom sites they contain, to be visible in the main chart",
        ...
        buttonList = ButtonList(self.residueFrame, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
        ...
        self.labellingSchemePulldown = PulldownList(frame, self.changeLabellingScheme, grid=(0, 1), tipText=tipText)
        ...
        self.isotopePulldown = PulldownList(
            frame,
            texts=STANDARD_ISOTOPES,
            index=1,
            objects=isotopes,
            grid=(1, 1),
            callback=self.changeIsotope,
            tipText=tipText,
        )
        ...
        self.minFractionEntry = FloatEntry(frame, text=0.25, width=8, grid=(0, 3), tipText=tipText)
        ...
        self.labelColorPulldown = PulldownList(
            frame,
            callback=self.setLabelColor,
        ...
        self.unlabelColorPulldown = PulldownList(
            frame,
            callback=self.setUnlabelColor,
            texts=[c.name for c in standardColors],
            objects=standardColors,
            colors=[c.hex for c in standardColors],
            grid=(1, 5),
        ...
        self.resColorPulldown = PulldownMenu(
            self.optionFrame,
            self.changeResColor,
        ...
        tipTexts = [
            "Manually trigger a redraw of the main chart, in light of new assignments etc",
        ]
        texts = [
            "Update",
        ]
    ...
    def updateButtons(self):
        ...
        if self.chain:
            ...
            if self.chain.molecule.molType == "protein":
                self.set1AtomsButton.configure(fg="black", state="normal", text="backbone, alpha & beta")
                self.set2AtomsButton.configure(fg="black", state="normal", text="backbone, alpha - delta")
                toolTipA.text = tip + "in the protein backbone and at alpha and beta positions"
                toolTipB.text = (
                    tip + "in the protein backbone and at side chain positions from alpha to delta, inclusive"
                )
    ...
    def greekSort(self, dataList):
        ...
        for x, molType in dataList:
            ...
            sortName = re.sub(r"^(\d)", "zz@\\1", sortName)
            sortName = re.sub(r"N(\S*)", "''@N\\1", sortName)
    ...
    def updateAtomSelection(self):
        ...
        self.atomSelector.update(
            objects=[x[0] for x in self.atomNamesList], labels=nameList, colors=colors, selected=selected
        )
    ...
    def updateSpecSelection(self):
        ...
        self.specSelector.update(objects=spectra, labels=spectraNames, colors=colors, fonts=fonts)
        for i in range(len(spectra)):
    ...
    def getIsotopeColors(self, isoScheme, residue, atomGroup, minIsoFrac=0.1, colorGrad=True):
        ...
        if not atomNames:
            ...
            if isoScheme.className == "LabelingScheme":
                ...
                chemCompLabel = isoScheme.findFirstChemCompLabel(ccpCode=ccpCode, molType=molType)
    ...
    def updateResidueAtoms(self, residue):
        ...
        for j, atomName in enumerate(atomNames):
            ...
            if rDict.get("inactive"):
                ...
                color1, color2, color3 = getIsotopeColors(isoScheme, residue, atomGroup, minIsoFrac, colorGrad)
    ...
    def makeGraphAtomList(self, residue):
        ...
        for heavy in heavies:
            ...
            if name == "N":
                ...
                newName = re.sub(r"^(\d)", "zz@\\1", newName)
    ...
    def createAtomCircle(self, canvas, centerX, centerY, radius, fill=None, outline="black", width=1):
