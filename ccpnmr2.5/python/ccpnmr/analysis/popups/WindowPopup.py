    ...
    def body(self, master):
        ...
        if windowPaneHasValueAxis(self.window.findFirstSpectrumWindowPane()):
            ...
            if len(self.window.spectrumWindowPanes) > 1:
                ...
                commands = [self.selSpectra, self.selPeaks, self.selShapes, self.getCenterCoords]
                tipTexts = [TOP_BAR_TIP_TEXTS.get(x) for x in options]

            else:
                options = ["Spectra", "Peaks", "Strips", "x,y"]
                commands = [self.selSpectra, self.selPeaks, self.selStrips, self.getCenterCoords]
                tipTexts = [TOP_BAR_TIP_TEXTS.get(x) for x in options]

        else:
            options = ["Spectra", "Contours", "Peaks", "Strips", "x,y"]
            commands = [self.selSpectra, self.selContours, self.selPeaks, self.selStrips, self.getCenterCoords]
            tipTexts = [TOP_BAR_TIP_TEXTS.get(x) for x in options]
        ...
        self.buttonList = UtilityButtonList(
            frame,
            texts=options,
            commands=commands,
        ...
        tipTexts = [
            "Move the contour base level, for displayed spectra, up (away from zero)",
        ...
        buttonList = ButtonList(frame, texts=texts, tipTexts=tipTexts, commands=commands, grid=(0, 0))
        ...
        self.spectrumToggleSelector = PartitionedSelector(
            frame, self.toggleSpectrum, font=self.font, tipText=tipText, grid=(0, 0)
        )
        ...
        self.peaksToggleSelector = PartitionedSelector(
            frame, self.togglePeaks, font=self.font, tipText=tipText, grid=(0, 0), sticky="ew"
        )
        ...
        self.peakListCheck = CheckButton(
            frame,
            text="Multi-list",
            font=self.font,
            callback=self.setMultiPeakLists,
            tipText=tipText,
            selected=self.window.useMultiplePeakLists,
            grid=(0, 1),
        )
        ...
        tipTexts = [
            "Add a new strip to the window (parallel sub-divisions)",
        ...
        texts = ["", "", "", "", ""]
        commands = [self.addStrip, self.deleteStrip, self.moveStripsLeft, self.moveStripsRight, self.toggleStripDir]

        self.stripsButtons = ButtonList(frame, texts=texts, commands=commands, tipTexts=tipTexts)
        ...
        tipTexts = [
            "Delete all vertical strips, or sub-divisions, from the window",
            "Delete all horizontal strips, or sub-divisions, from the window",
        ]
        texts = ["", ""]
        commands = [self.deleteVert, self.deleteHoriz]

        clearButtons = ButtonList(frame, texts=texts, grid=(0, 2), commands=commands, tipTexts=tipTexts)
        ...
        self.stripsToggleSelector = PartitionedSelector(
            frame, self.toggleStrips, radio=True, font=self.font, grid=(0, 4), tipText=tipText
        )
  ...
  """
    ...
    def setStripsSelector(self):
        ...
        if N == 0:
            ...
            self.stripsToggleSelector.update(
                objects=[
                    axisRegions[0],
                ],
                labels=[
    ...
    def configTopBar(self, frame, buttonIndex):
        ...
        for frameOther in (self.contourFrame, self.peakFrame, self.stripFrame, self.specFrame):
