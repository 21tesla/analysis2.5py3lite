...
from ccpnmr.analysis.core.Util import getAnalysisDataDim, getAnalysisSpectrum, getHueSortedColorSchemes
...
class EditExperimentSeriesPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.conditionNamesPulldown = PulldownList(self, callback=self.setConditionName, texts=self.getConditionNames())
        self.unitPulldown = PulldownList(self, callback=self.setUnit, texts=self.getUnits())
        ...
        self.seriesMatrix = ScrolledMatrix(
            frame0,
            tipTexts=tipTexts,
        ...
        tipTexts = [
            "Make a new, blank NMR series specification in the CCPN project",
        ...
        texts = ["Add Series", "Delete Series", "Auto Colour Spectra"]
        commands = [self.addExpSeries, self.deleteExpSeries, self.autoColorSpectra]

        self.seriesButtons = ButtonList(frame0, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
        ...
        tipTexts = [
            "The kind of experimental parameter that is being used to define the NMR series",
        ...
        self.conditionPointsMatrix = ScrolledMatrix(
            frame1,
            grid=(0, 0),
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = [
            "Add a new point to the NMR series with an associated parameter value and experiment",
        ...
        self.conditionPointsButtons = ButtonList(frame1, texts=texts, commands=commands, tipTexts=tipTexts, grid=(1, 0))
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("__init__", "delete", "setName", "setConditionNames", "addConditionName", "removeConditionName"):
    ...
    def checkAddSampleCondition(self, experiment):
        ...
        if not condition:
            condition = conditionSet.newSampleCondition(condition=conditionName, unit=unit, value=0.0, error=0.0)
    ...
    def setConditionName(self, obj):
        ...
        if self.expSeries:
            if self.expSeries.className == "Experiment":
                ...
                self.expSeries.setConditionNames(
                    [
                        name,
                    ]
                )
    ...
    def addExpSeries(self):
        ...
        expSeries = self.nmrProject.newNmrExpSeries(
            conditionNames=[
                "delay time",
            ]
        )
        ...
        if self.conditionPoint and (type(self.conditionPoint) != type(())):
            ...
            if conditionSet is not self.conditionPoint.parent:
                ...
                if self.conditionPoint:
                    ...
                    self.conditionPoint = conditionSet.newSampleCondition(
                        condition=condition, unit=unit, value=value, error=error
                    )
    ...
    def updateConditions(self):
        ...
        if self.expSeries:
            if self.expSeries.className == "Experiment":
                ...
                for i in range(dataDim.numPoints):
                    ...
                    pointText = ":%3d" % (i + 1)
                    if i == refPlane:
                        datum = ["* Ref Plane *", expName + pointText, None, None, None]
                        colorMatrix.append(["#f08080"] * nCols)
                    else:
                        datum = [conditionVaried, expName + pointText, pointValues[i], error, unit]
        ...
        self.conditionPointsMatrix.update(objectList=objectList, colorMatrix=colorMatrix, textMatrix=textMatrix)
    ...
    def update(self):
        ...
        for experiment in getSampledDimExperiments(self.nmrProject):
            ...
            datum = [None, experiment.name, 1, sampledDim.conditionVaried, sampledDim.numPoints]
