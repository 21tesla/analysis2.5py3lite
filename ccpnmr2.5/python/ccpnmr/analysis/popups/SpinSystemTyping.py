...
class SpinSystemTypingPopup(BasePopup):
    ...
    **Finding the Optimal Arrangement of Residue Types**
    ...
    def body(self, guiFrame):
        ...
        self.chainPulldown = PulldownList(guiFrame, self.changeChain, grid=(row, 1), tipText=tipText)
        ...
        self.shiftListPulldown = PulldownList(guiFrame, callback=self.setShiftList, grid=(row, 3), tipText=tipText)
        ...
        frow = 0
        label = Label(frame, text="Keep existing types?", grid=(frow, 0), sticky="e")
        tipText = (
            "Whether any existing residue type information should be preserved, when predicting the type of others"
        )
        self.preserveTypesSelect = CheckButton(
            frame, grid=(frow, 1), selected=False, callback=self.selectPreserveTypes, tipText=tipText
        )

        label = Label(frame, text="Assignment threshold: ", grid=(frow, 2), sticky="e")
        ...
        self.thresholdEntry = FloatEntry(frame, text=self.threshold, width=8, grid=(frow, 3), tipText=tipText)

        frow += 1
        label = Label(frame, text="Ensemble size: ", grid=(frow, 0), sticky="e")
        tipText = (
            "The number of best scoring residue type mappings, from the Monte Carlo search, to use un the prediction"
        )
        self.ensembleEntry = IntEntry(frame, text=20, width=4, grid=(frow, 1), tipText=tipText)
        ...
        self.stepsEntry = IntEntry(frame, text=100000, width=8, tipText=tipText, grid=(frow, 3))

        frow += 1
        label = Label(frame, text="Isotope shifts to consider:", grid=(frow, 0), gridSpan=(1, 4))
        ...
        tipText = (
            "Selects which kinds of resonances, in terms of isotope, the residue type predictions will be made with"
        )
        self.isotopeCheckButtons = PartitionedSelector(
            frame,
            labels=isos,
            objects=isos,
            colors=colors,
        ...
        row += 1
        tipTexts = [
            "Execute the Monte Carlo search that will make the residue type predictions for the spin systems",
        ...
        commands = [self.run, self.assign, self.individualScore, self.showPeaks]
        bottomButtons = ButtonList(
            guiFrame, texts=texts, commands=commands, grid=(row, 0), gridSpan=(1, 5), tipTexts=tipTexts
        )
    ...
    def run(self):
        ...
        if self.chain and spinSystems:
            ...
            typeScores, cc0 = getSpinSystemTypes(
                residues,
                spinSystems,
                self.preserveTypes,
...
class TypingEnsemblePopup(ProgressBar):
    ...
    def body(self, guiFrame):
        ...
        for i in range(self.ensembleSize):
            label = Label(frame, text="", pady=0, font="Courier 10", borderwidth=0, grid=(row, 0), sticky="ew")
            self.labels.append(label)
            row += 1

        guiFrame.grid_rowconfigure(2, weight=1)
        self.graph = ScrolledGraph(
            guiFrame,
            width=450,
            height=300,
            graphType="scatter",
