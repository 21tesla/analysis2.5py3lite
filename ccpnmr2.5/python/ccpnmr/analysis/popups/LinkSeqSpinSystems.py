...
from ccpnmr.analysis.core.WindowBasic import (
    displayStrips,
    getPeakDimAxisMapping,
    getSpinSystemWindowShifts,
    getWindowPaneName,
)
...
SeqNoesyExperiments = [
    "H[N]_H.through-space",
    "H_H[N].through-space",
    "H[{c|n}]_H[N].through-space",
    "h[C]_H[N].through-space",
]
...
class LinkSeqSpinSystemsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=options, tipTexts=tipTexts, callback=self.selectTab, grid=(0, 0))
        ...
        div = LabelDivider(frameA, text="Query Windows and Spectra", grid=(row, 0), tipText=tipText)
        ...
        self.queryPanePulldownC = PulldownList(frame, self.changeQueryPaneC, grid=(0, 1), tipText=tipText)
        ...
        self.queryPanePulldownH = PulldownList(frame, self.changeQueryPaneH, grid=(0, 3), tipText=tipText)
        ...
        self.rootPanePulldown = PulldownList(frame, callback=self.changeRootPane, grid=(0, 6), tipText=tipText)

        self.toleranceEntry = FloatEntry(self, returnCallback=self.setTolerance, text=0.1, width=6)

        tipTexts = [
            'The "experiment:spectrum" name of the spectrum for a potential query peak list; used as the source of peak positions for a spin system',
        ...
        editGetCallbacks = [self.toggleQueryPeakList, None, self.toggleQueryPeakList, self.getTolerance, None]
        editSetCallbacks = [None, None, None, self.setTolerance, None]

        self.querySpecTable = ScrolledMatrix(
            frame,
            gridSpan=(1, 7),
            highlightType=2,
            tipTexts=tipTexts,
        ...
        div = LabelDivider(frameA, text="Match Windows and Spectra", grid=(row, 0), tipText=tipText)
        ...
        self.matchPanePulldownC = PulldownList(frame, callback=self.changeMatchPaneC, grid=(0, 1), tipText=tipText)
        ...
        self.matchPanePulldownH = PulldownList(frame, callback=self.changeMatchPaneH, grid=(0, 3), tipText=tipText)

        tipTexts = [
            'The "experiment:spectrum" name of the spectrum that may be searched for potential matches to the peak positions from the query spin systems',
        ...
        headingList = ["Spectrum", "Peak List", "Use?", "Tolerance", "Expt Type"]
        editWidgets = [None, None, None, self.toleranceEntry, None]
        editGetCallbacks = [self.toggleMatchPeakList, None, self.toggleMatchPeakList, self.getTolerance, None]
        editSetCallbacks = [None, None, None, self.setTolerance, None]
        self.matchSpecTable = ScrolledMatrix(
            frame,
            gridSpan=(1, 5),
            highlightType=2,
            tipTexts=tipTexts,
        ...
        headingList = ["#", "Name", "i-1", "i+1", "i+0", "Seq, Segment", "Position"]
        ...
        row += 1
        tipTexts = [
            "Show peak matches locations for the next spin system in the table, assuming a spin system is currently selected",
        ...
        texts = ["Next Row", "Previous Row", "Goto i-1", "Goto i+1"]
        commands = [self.nextSpinSystem, self.prevSpinSystem, self.prevLinkedSpinSystem, self.nextLinkedSpinSystem]
        self.spinSystemButtons = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(row, 0))

        row += 1
        tipTexts = [
            "For the selected spin system, clear any connections to spin systems set as being one earlier in the sequence",
        ...
        texts = ["Clear i-1 Links", "Clear i+1 Links", "Clear All Links"]
        commands = [
            self.clearPrevLinks,
            self.clearNextLinks,
            self.clearAllLinks,
        ]
        self.spinSystemButtons2 = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(row, 0))
        row += 1
        tipTexts = [
            "Considering how peaks in different spectra overlap, add non-root (HA/CO/CA/CB) resonance assignments to peaks in the selected spin system",
        ...
        texts = ["Add HA/CO/CA/CB", "Show Peaks", "Predict Type"]
        commands = [self.addResonances, self.showPeaks, self.showResonances]
        self.spinSystemButtons3 = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(row, 0))

        row += 1
        tipText = (
            "The locations of the amide spin systems that are potential sequence neighbours to the query spin system"
        )
        div = LabelDivider(frameB, text="Matched Peak Positions", grid=(row, 0), tipText=tipText)

        row += 1
        frameB.grid_rowconfigure(row, weight=1)
        tipTexts = [
            "The rank of the spin system with regards to how well its peaks match the peaks in the query",
        ...
        self.matchMatrix = ScrolledMatrix(
            frameB,
            headingList=headingList,
            multiSelect=True,
            grid=(row, 0),
        ...
        row += 1
        tipTexts = [
            "Manually force a refresh of the above table",
        ...
        texts = ["Refresh Matches", "Discard Selected", "Set Seq Link", "Clear Rulers"]
        commands = [self.findMatches, self.discardMatches, self.setSeqLink, self.clearRulers]
        self.matchButtons = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(row, 0))
        ...
        divL = LabelDivider(frame, text="Sequence Locations", grid=(0, 0), tipText=tipText)

        tipTexts = [
            "The ranking how well the query and selected match spin system fits a region of protein sequence",
        ...
        tipTexts = [
            "Assign the query spin system and any sequentially linked spin systems to the residue sequence at the selected location",
        ...
        self.assignButtons = ButtonList(frame, texts=texts, commands=commands, grid=(2, 0), tipTexts=tipTexts)

        tipText = "The predicted residue types for the selected spin system and any sequence neighbours"
        divR = LabelDivider(frame, text="Residue Types", grid=(0, 1), tipText=tipText)

        tipTexts = [
            "The score for predicting the residue type of the i-2 spin system; connected as two sequence positions earlier than the query spin system",
        ...
        self.typeMatrix = ScrolledMatrix(
            frame,
            headingList=headingList,
            multiSelect=True,
            grid=(1, 1),
        ...
        self.chainPulldown = PulldownList(frame, callback=self.changeChain, grid=(0, 1), tipText=tipText)
        ...
        tipText = (
            "Sets the maximum number of matching spin systems to display in strips of the selected spectrum window"
        )
        self.maxMatchesEntry = IntEntry(frame, width=2, text=7, grid=(0, 1), tipText=tipText)
        ...
        self.focusEntryC = FloatEntry(frame, width=8, text=DEFAULT_FOCUS_WIDTH, grid=(1, 1), tipText=tipText)
        ...
        self.focusEntryH = FloatEntry(frame, width=8, text=DEFAULT_FOCUS_WIDTH_H, grid=(2, 1), tipText=tipText)
        ...
        self.filterSelectC = CheckButton(frame, callback=self.findMatches, grid=(4, 1), selected=True, tipText=tipText)
        ...
        self.filterSelectH = CheckButton(frame, callback=self.findMatches, grid=(5, 1), selected=True, tipText=tipText)

        label = Label(frame, text="Focus 13C matches?", grid=(6, 0))
        tipText = (
            "Whether to zoom in on the 13C peak locations using the above stated focus width for the window region"
        )
        self.focusSelectC = CheckButton(frame, callback=self.findMatches, grid=(6, 1), selected=True, tipText=tipText)
        ...
        self.focusSelectH = CheckButton(frame, callback=self.findMatches, grid=(7, 1), selected=True, tipText=tipText)
        ...
        self.matchOrderPulldown = PulldownList(
            frame, texts=MATCH_ORDERS, grid=(8, 1), tipText=tipText, index=index, callback=self.changeMatchOrder
        )
    ...
    def updateResTypes(self):
        ...
        self.typeMatrix.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
    ...
    def updateSeqLocations(self, scoreMatrix=None, spinSystems=None):
        ...
        self.seqMatrix.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
    ...
    def setAppDataOptions(self):
        ...
        if self.queryPaneC:
            options["queryPaneC"] = (self.queryPaneC.spectrumWindow.serial, self.queryPaneC.serial)
        if self.matchPaneC:
            options["matchPaneC"] = (self.matchPaneC.spectrumWindow.serial, self.matchPaneC.serial)
        if self.queryPaneH:
            options["queryPaneH"] = (self.queryPaneH.spectrumWindow.serial, self.queryPaneH.serial)
        if self.matchPaneH:
            options["matchPaneH"] = (self.matchPaneH.spectrumWindow.serial, self.matchPaneH.serial)
    ...
    def updateQueryPanes(self):
        ...
        if windowDataC:
            names = [
                "<None>",
            ] + [x[0] for x in windowDataC]
            windows = [
                None,
            ] + [x[1] for x in windowDataC]
        ...
        if windowDataH:
            names = [
                "<None>",
            ] + [x[0] for x in windowDataH]
            windows = [
                None,
            ] + [x[1] for x in windowDataH]
        ...
        if windowData:
            names = [
                "<None>",
            ] + [x[0] for x in windowData]
            windows = [
                None,
            ] + [x[1] for x in windowData]
    ...
    def updateQuerySpectra(self):
        ...
        self.querySpecTable.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
    ...
    def updateMatchSpectra(self):
        ...
        self.matchSpecTable.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
    ...
    def updateMatches(self):
        ...
        windowPanes = ((self.matchPaneC, self.queryPaneC), (self.matchPaneH, self.queryPaneH))
        ...
        for matchPane, queryPane, orthoAxis, axisLabels, positions in axesAndPanes:
            if positions and not self.refreshSeqSeg:
                displayStrips(self.guiParent, positions, spectrum=None, windowPane=matchPane)
    ...
    def getQueryOrthoPositions(self, queryPane, matchPane):
        ...
        if self.spinSystem:
            ...
            peaks, dimDict, dimMappingDict = self.getMatchPeaks(
                self.shifts, queryPane, matchPane, self.queryPeakLists, self.matchPeakLists
            )
            orthoPos = []
            tolerances = []
            for peak in peaks:
    ...
    def getMatchPeaks(self, shifts, queryPane, matchPane, queryPeakLists, matchPeakLists):
        ...
        for position, panelType in panelPositions:
            ruler = createRuler(position, panelType, dashLength=3, gapLength=1, color=self.color, remove=False)
    ...
    def findMatches(self, event=None):
        ...
        data = (
            (queryPaneC, matchPaneC, queryPeakListsC, matchPeakListsC, filterByTypeC),
        ...
        for queryPane, matchPane, queryPeakLists, matchPeakLists, filterByType in data:
            ...
            peaks, dimDict, dimMappingDict = self.getMatchPeaks(
                ssShifts, queryPane, matchPane, queryPeakLists, matchPeakLists
            )
            ...
            matches = findPositionMatches(peaks, dimDict, matchPeakLists, matchTolerance=None, separateSpinSystems=True)
            matches = self.filterMatches(
                matches, dimDict, querySpinSystem, queryPeakLists, matchPeakLists, filterByType
            )
    ...
    def filterMatches(self, matches, dimDict, querySpinSystem, queryPeakLists, matchPeakLists, filterByType):
        ...
        if queryPanes:
            ...
            for spinSystem in spinSystems:
                ...
                for queryPane in queryPanes:
                    shiftDicts, yShifts = getSpinSystemWindowShifts(
                        [
                            spinSystem,
                        ],
                        queryPane,
