...
from ccpnmr.analysis.core import ExperimentBasic, PeakBasic
...
class EditPeakListsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=options, callback=self.toggleTab, grid=(0, 0), tipTexts=tipTexts)
        ...
        self.peakColorWidget = PulldownList(
            self,
        ...
        symbols = ["+", "x", "o", "*"]
        self.peakSymbolWidget = PulldownList(self, callback=self.setSymbol, texts=symbols)

        frameA.grid_columnconfigure(0, weight=1)
        frameA.grid_rowconfigure(0, weight=1)

        tipTexts = [
            "The name of the experiment that gave rise to the spectrum & hence contains the peak list",
        ...
        editSetCallbacks = [None, None, None, None, self.setColor, self.setSymbol, None, None, None, self.setDetails]
        ...
        tipTexts = [
            "Show a table of the individual peaks within the selected peak list",
        ...
        texts = ["Edit Peaks", "Delete", "Add Sister List", "Copy Peaks", "Subtract Peaks", "Shift Whole Peak List"]
        commands = [
            self.editPeaks,
            self.deleteList,
            self.addList,
        ...
        self.peakListButtons = ButtonList(frameA, texts=texts, tipTexts=tipTexts, commands=commands, grid=(1, 0))
        ...
        self.peakTableFrame = PeakTableFrame(frameB, self.guiParent, nmrProject=self.nmrProject, grid=(0, 0))
        ...
        self.predFromShiftsButton = Button(
            frameC,
            borderwidth=1,
            text="Predict from shifts",
        ...
        self.shiftsSpectrumPulldown = PulldownList(subFrame, self.changeShiftsSpectrum, grid=(0, 1), tipText=tipText)
        ...
        self.shiftListPulldown = PulldownList(subFrame, self.changeShiftList, grid=(1, 1), tipText=tipText)
        ...
        self.molSystemPulldown = PulldownList(subFrame, self.changeMolSystem, grid=(2, 1), tipText=tipText)
        ...
        self.useUnassignedSelect = CheckButton(subFrame, grid=(3, 1), selected=False, tipText=tipText)
        ...
        self.throughSpaceBondsPulldown = PulldownList(
            subFrame, None, texts, values, index=0, grid=(4, 1), tipText=tipText
        )
        ...
        self.labellingSchemePulldownA = PulldownList(
            subFrame, self.changeLabellingScheme, grid=(0, 3), gridSpan=(2, 1), tipText=tipText
        )
        ...
        self.minFractionEntryA = FloatEntry(subFrame, text=0.25, width=8, grid=(2, 3), gridSpan=(2, 1), tipText=tipText)
        ...
        self.throughSpaceResiduePulldown = PulldownList(
            subFrame, None, texts, objs, index=1, grid=(4, 3), tipText=tipText
        )
        ...
        self.predFromStructButton = Button(
            frameC,
            borderwidth=1,
            text="Predict from structure",
        ...
        self.structSpectrumPulldown = PulldownList(subFrame, self.changeStructSpectrum, grid=(0, 1), tipText=tipText)
        ...
        self.structurePulldown = PulldownList(subFrame, self.changeStructure, grid=(1, 1), tipText=tipText)
        ...
        self.distThresholdEntry = FloatEntry(subFrame, text=5.0, width=8, grid=(2, 1), tipText=tipText)
        ...
        self.strucShiftListPulldown = PulldownList(subFrame, self.changeStrucShiftList, grid=(3, 1), tipText=tipText)
        ...
        self.labellingSchemePulldownB = PulldownList(
            subFrame, self.changeLabellingScheme, grid=(0, 3), gridSpan=(2, 1), tipText=tipText
        )
        ...
        self.minFractionEntryB = FloatEntry(subFrame, text=0.25, width=8, grid=(2, 3), gridSpan=(2, 1), tipText=tipText)
        ...
        self.transposeButton = Button(
            frameC,
            borderwidth=1,
            text="Make transpose list",
        ...
        self.transposeSourcePulldown = PulldownList(subFrame, self.changeTransposeSource, grid=(0, 1), tipText=tipText)
        ...
        self.constraintSpectrumPulldown = PulldownList(
            subFrame, self.changeConstraintSpectrum, grid=(0, 1), tipText=tipText
        )
        ...
        self.constraintSetPulldown = PulldownList(subFrame, self.changeConstraintSet, grid=(1, 1), tipText=tipText)
        ...
        self.constraintListPulldown = PulldownList(subFrame, self.changeConstraintList, grid=(2, 1), tipText=tipText)
    ...
    def getStructSpectra(self):
        ...
        spectraDict = {}
        peakLists = ExperimentBasic.getThroughSpacePeakLists(self.nmrProject.root, excludeSimulated=False)
    ...
    def synthesiseFromShifts(self):
        ...
        if self.shiftsSpectrum:
            ...
            peakList = PeakBasic.makePeakListFromShifts(
                self.shiftsSpectrum,
    ...
    def synthesiseFromStruct(self):
        ...
        if self.structSpectrum and self.structure:
            ...
            peakList = PeakBasic.structurePredictNoePeakList(
                self.structure,
                self.structSpectrum,
    ...
    def selectCell(self, peakList, row, col):
        ...
        if self.cloneSourceList:
            ...
            if pointsA != pointsB:
                showWarning(
                    "Stop",
                    "Peak clone not possible. Spectra must have the same number of points for each dimension",
                    parent=self,
                )
                return
            ...
            if N > 500:
                if not showOkCancel(
                    "Warning", "Copy will duplicate %d peaks. This may take some time." % N, parent=self
                ):
                    self.cancelCopy()
                    return
            ...
            if self.peakList.dataSource is not self.subtractPeakList.dataSource:
                showError
                showError(
                    "Different spectra", "For now can only subtract peaks from peak list in same spectrum", parent=self
                )

            N = len(self.subtractPeakList.peaks)
            if N > 500:
                if not showOkCancel(
                    "Warning", "Subtraction is checking %d peaks. This may take some time." % N, parent=self
                ):
                    self.cancelSubtract()
                    return
    ...
    def deleteList(self, *event):
        ...
        if self.peakList:
            ...
            if len(spectrum.peakLists) == 1:
                ...
                showWarning("Delete failed", "Cannot delete the only peak list in spectrum %s:%s" % data, parent=self)
                return
    ...
    def update(self):
        ...
        self.peakListTable.update(objectList=peakLists, textMatrix=textMatrix, colorMatrix=colorMatrix)
...
class SelectedPeaksPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        texts = []
        commands = []
        bottomButtons = UtilityButtonList(frame, commands=commands, grid=(0, 1), texts=texts, helpUrl=self.help_url)
