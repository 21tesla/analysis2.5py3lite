...
from ccpnmr.analysis.core.AssignmentBasic import getResidueResonances, propagatePeakAssignments
...
from ccpnmr.analysis.core.MoleculeBasic import duplicateResidueAssignments, getChainResidueMapping, getResidueCode
from ccpnmr.analysis.core.PeakBasic import arePeaksAssignedSame, findShiftDistPeakMatches, getIsotopeWeightedTolerances
...
class CopyAssignmentsPopup(BasePopup):
    ...
    **Copying Assignments Between Molecular Chains**
    ...
    def body(self, guiFrame):
        ...
        self.sourcePulldown = PulldownList(frame, tipText=tipText, grid=(0, 1), callback=self.changeSourcePeakList)

        label = Label(frame, text="Target Peak List:", grid=(0, 2))
        tipText = "Selects the peak list which will be assigned"
        self.targetPulldown = PulldownList(frame, grid=(0, 3), tipText=tipText, callback=self.changeTargetPeakList)
        ...
        self.followSelect = CheckButton(
            frame, callback=self.setShowCopied, grid=(1, 1), selected=False, tipText=tipText
        )
        ...
        self.overwriteSelect = CheckButton(
            frame, callback=self.setOverwrite, grid=(1, 3), selected=True, tipText=tipText
        )
        ...
        self.followSelect = CheckButton(frame, callback=self.setFollow, grid=(2, 1), selected=True, tipText=tipText)
        ...
        self.restrictSelect = CheckButton(frame, callback=self.setRestrict, grid=(2, 3), selected=True, tipText=tipText)
        ...
        self.aliasedSelect = CheckButton(
            frame, callback=self.setConsiderAliased, grid=(3, 1), selected=True, tipText=tipText
        )
        ...
        self.windowPulldown = PulldownList(frame, grid=(4, 1), tipText=tipText, callback=self.setWindowPane)
        ...
        self.thresholdEntry = FloatEntry(
            frame, text=0.08, width=7, tipText=tipText, returnCallback=self.changeThreshold, grid=(4, 3)
        )
        ...
        self.scaleEntry1H = FloatEntry(
            frame, text=1.0, width=7, tipText=tipText, returnCallback=self.changeScaleEntry, grid=(0, 1)
        )
        ...
        self.scaleEntry15N = FloatEntry(
            frame, text=5.0, width=7, tipText=tipText, returnCallback=self.changeScaleEntry, grid=(0, 3)
        )
        ...
        self.scaleEntry13C = FloatEntry(
            frame, text=10.0, width=7, tipText=tipText, returnCallback=self.changeScaleEntry, grid=(0, 5)
        )
        ...
        self.sourcePeakMatrix = ScrolledMatrix(
            frameA, headingList=headingList, tipTexts=tipTexts, callback=self.selectSourcePeak, grid=(row, 0)
        )
        ...
        self.targetPeakMatrix = ScrolledMatrix(
            frameA, headingList=headingList, tipTexts=tipTexts, callback=self.selectTargetPeak, grid=(row, 0)
        )

        row += 1
        tipTexts = [
            "Copy assignments from the selected source peak to the selected target",
        ...
        self.peakButtons = ButtonList(frameA, commands=commands, texts=texts, grid=(row, 0), tipTexts=tipTexts)
        ...
        self.sourceChainPulldown = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changeSourceChain)

        label = Label(frame, text="Target Chain:", grid=(0, 2))
        tipText = "Selects the molecular chain which will receive new atom assignments"
        self.targetChainPulldown = PulldownList(frame, grid=(0, 3), tipText=tipText, callback=self.changeTargetChain)

        row += 1
        div = LabelDivider(frameB, text="Peak Assignment Transfers", grid=(row, 0))

        row += 1
        self.exptChainPulldown = PulldownList(
            self, callback=self.setExptChain, objects=(0, 1, 2), texts=COPY_OPERATIONS
        )
        tipTexts = [
        ...
        self.exptChainMatrix = ScrolledMatrix(
            frameB,
        ...
        tipTexts = [
            "Sets the selected experiments so that their assignments will be left unaltered",
        ...
        commands = [
            self.setSelectedExperimentsSource,
            self.setSelectedExperimentsTarget,
            self.setSelectedExperimentsDuplicate,
        ]
        self.exptChainButtons = ButtonList(
            frame, texts=COPY_OPERATIONS, tipTexts=tipTexts, commands=commands, grid=(0, 1), expands=True
        )
        ...
        tipTexts = [
            "The residue from which assignments may be copied",
        ...
        row += 1
        tipTexts = [
            "Go ahead and copy assignments between chains using the selected options",
        ...
        self.residueButtons = ButtonList(frameB, texts=texts, commands=commands, grid=(row, 0), tipTexts=tipTexts)
        self.residueButtons.buttons[0].config(bg="#A0FFA0")

        self.chainButtons = UtilityButtonList(tabbedFrame.sideFrame, helpUrl=self.help_url, grid=(0, 0), sticky="e")
    ...
    def updateExperimentChains(self, dummy=None):
                ...
                for molSystem in experiment.molSystems:
                    for chain in molSystem.sortedChains():
                        if chain is self.sourceChain:
                            ...
                            datum = [experiment.serial, experiment.name, COPY_OPERATIONS[operation]]
        ...
        self.exptChainMatrix.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
    ...
    def updateResMapping(self):
        ...
        self.residueMatrix.update(textMatrix=textMatrix, colorMatrix=colorMatrix, objectList=objectList)
        self.updateResidueButtons()
        self.waitingChain = False
    ...
    def updatePeaks(self):
        ...
        if self.sourcePeakList and self.targetPeakList and (self.sourcePeakList is not self.targetPeakList):
            ...
            for peak in self.sourcePeakList.peaks:
                matches = findShiftDistPeakMatches(
                    peak,
                    self.targetPeakList,
                    threshold,
                    scaleFactors,
    ...
    def updatePeakButtons(self):
        ...
        if self.sourcePeakList and self.targetPeakList and self.sourcePeakList.peaks and self.targetPeakList.peaks:
    ...
    def assignSelected(self):
        ...
        if self.targetPeak and self.sourcePeak:
            ...
            tolerances, null = getIsotopeWeightedTolerances(
                self.targetPeak, threshold, scaleFactorDict=self.getScaleFactorDict()
            )
            propagatePeakAssignments(
                [
                    self.targetPeak,
                ],
                refPeak=self.sourcePeak,
    ...
    def assignSingles(self):
        ...
        if self.assignChecks() and self.sourcePeakList and self.targetPeakList:
            ...
            tolerances, null = getIsotopeWeightedTolerances(peak, threshold, scaleFactorDict=scaleFactors)
