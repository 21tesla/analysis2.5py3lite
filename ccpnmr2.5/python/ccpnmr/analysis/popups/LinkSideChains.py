...
from ccpnmr.analysis.core.AssignmentBasic import assignAtomsToRes, assignResToDim, getResonanceName
...
from ccpnmr.analysis.core.MoleculeBasic import getResidueCode, greekSortAtomNames
...
class LinkSideChainsPopup(BasePopup):
    def __init__(self, parent, *args, **kw):
        ...
        for func in ("__init__", "delete", "setResidue", "setResonances", "addResonance", "removeResonance", "setName"):
    ...
    def body(self, guiFrame):
        ...
        self.spectrumPulldown = PulldownMenu(frame, callback=self.changeSpectrum, do_initial_callback=False)
        self.spectrumPulldown.grid(row=0, column=1, sticky="w")

        label = Label(frame, text="Peak List")
        label.grid(row=0, column=2, sticky="w")
        self.peakListPulldown = PulldownMenu(frame, callback=self.changePeakList, do_initial_callback=False)
        self.peakListPulldown.grid(row=0, column=3, sticky="w")

        label = Label(frame, text="Window:")
        label.grid(row=0, column=4, sticky="w")
        self.windowPulldown = PulldownMenu(frame, callback=self.changeMainWindow, do_initial_callback=False)
        self.windowPulldown.grid(row=0, column=5, sticky="w")

        # label = Label(frame, text='Window 2:')
        # label.grid(row=1, column=2, sticky='w')
        self.orthogWindowPulldown = PulldownMenu(frame, callback=self.changeOrthogWindow, do_initial_callback=False)
        ...
        self.matchMatrix = ScrolledMatrix(frame, headingList=headingList, callback=self.selectMatch)
        ...
        self.resonanceMatrix = ScrolledMatrix(
            frame, headingList=headingList, multiSelect=True, callback=self.selectResonance
        )
        ...
        commands = [
            ...
            self.updateMatchLocations,
            self.commitAssignments,
            self.commitAllAssignments,
        ]
        texts = [
            "Add Known\nPeak Locations",
            "Search\nStrips",
        ...
        self.bottomButtons = UtilityButtonList(guiFrame, helpUrl=self.help_url, commands=commands, texts=texts)
    ...
    def updateResonances(self):
        ...
        if self.spinSystem:
            ...
            for atomH, atomC in getSideChainAtomSets(self.spinSystem):
                ...
                datum = ["%s %s" % (nameH, nameC), shiftH, shiftC, ",".join(serials)]
    ...
    def displayKnownLocations(self):
        ...
        if self.wPane1 and self.spinSystem and self.dataDims:
            ...
            shifts, orthoShifts = getSpinSystemWindowShifts(
                [
                    self.spinSystem,
                ],
                self.wPane1,
            ...
            # Make strips
            displayStrips(self.parent, shifts, orthoPositions=orthoPositions, windowPane=self.wPane1)
                            ...
                            for peak in searchPeaks(
                                ...
                                for i in rootDims:
                                    ...
                                    for k in nonRootDims:
                                        if "1H" in isotopes[k]:
                                            for resonance in [
                                                contrib.resonance for contrib in peakDims[k].peakDimContribs
                                            ]:
    ...
    def updateMatchLocations(self):
            ...
            if self.wPane1:
                ...
                for atomSite in indirectPositions:
                    ...
                    for i in range(len(nonRootDims)):
                        ruler = createRuler(
                            ppms[i], panelTypes[i], dashLength=2, gapLength=2, color=self.color, remove=False
                        )
      ...
      """
            ...
            assignments = dynamicBranchPrune(
                I,
                J,
                linkScoreMatrix,
                linkThreshold,
    ...
    def displayMatchLocations(self):
        ...
        if self.assignment and self.spectrum and self.wPane1:
            ...
            displayStrips(self.parent, locations, orthoPositions=None, windowPane=self.wPane1)
            self.update_idletasks()
            displayStrips(self.parent, locations, orthoPositions=None, windowPane=self.wPane1)
