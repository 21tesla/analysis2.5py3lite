...
class BrowseReferenceShiftsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        ccpCodes = self.getCcpCodes(self.molType) or [
            None,
        ]
        self.ccpCode = "Ala"
        ...
        self.molTypePulldown = PulldownList(
            frameA, callback=self.changeMolType, texts=molTypes, grid=(row, 1), tipText=tipText
        )
        ...
        self.ccpCodePulldown = PulldownList(
            frameA,
        ...
        self.atomTypePulldown = PulldownList(
            frameA, callback=self.changeAtomType, texts=ATOM_TYPES, tipText=tipText, grid=(row, 5)
        )

        row += 1

        tipText = "The selection of atom name to display distributions for"
        self.atomSelector = PartitionedSelector(frameA, self.toggleAtom, tipText=tipText, maxRowObjects=20)
        ...
        self.scrolledGraph = ScrolledGraph(
            ...
            width=650,
            height=300,
            title="Chemical shift distribution",
        ...
        self.cacbMatrix = ScrolledDensityMatrix(
            frameB,
            matrix=matrix,
            boxSize=14,
            title=title,
        ...
        x0, y0 = (470, 370)
        sdm.canvas.create_rectangle(
            x0 + 170, y0 - 6, x0 + 182, y0 + 6, fill="#4040A0", outline=sdm.borderColor, width=1
        )
        sdm.canvas.create_text(x0 + 200, y0, text="CA", font=font)
        sdm.canvas.create_rectangle(
            x0 + 220, y0 - 6, x0 + 232, y0 + 6, fill="#A04040", outline=sdm.borderColor, width=1
        )
        ...
        self.sourcePulldown = PulldownList(
        ...
        self.bottomButtons = UtilityButtonList(
            tabbedFrame.sideFrame, expands=True, helpUrl=self.help_url, sticky="e", grid=(0, 2)
        )
        ...
        if chemCompNmrRef:
            ...
            if chemCompVarNmrRef:
                for chemAtomNmrRef in chemCompVarNmrRef.chemAtomNmrRefs:
                    ...
                    elif (atomType == "Hydrogen" and element == "H") or (atomType == "Heavy" and element != "H"):
                        dataDict[atomName] = chemAtomNmrRef
