...
from ccp.util.NmrCalc import DATA_MISSING, getDataObjText, getRangeString
...
class EditCalculationPopup(BasePopup):
    ...
    *This popup is somewhat of a prototype, so its functionality is neither
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=options, grid=(0, 0), callback=self.update)
        ...
        self.calcStoreMatrix = ScrolledMatrix(
            frameA,
        ...
        tipTexts = [
            "Add a new grouping (e.g. by program name) to contain calculation specifications",
            "Delete the selected calculation group (program settings) and all calculation run settings it contains",
        ]
        texts = ["Add New Calculation Group", "Delete Calculation Group"]
        commands = [self.addCalcStore, self.deleteCalcStore]
        self.calcStoreButtons = ButtonList(frameA, texts=texts, tipTexts=tipTexts, commands=commands, grid=(1, 0))
        ...
        self.editRunFrame = NmrCalcRunFrame(frameB, self.project, self.calcStore, grid=(1, 0), gridSpan=(1, 2))

        tipText = "Selects which program (or other calculation grouping) a set of calculations relate to"
        self.calcStorePulldown = PulldownList(frameB, callback=self.changedCalcStore, grid=(0, 1), tipText=tipText)
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("__init__", "delete"):
            for clazz in ("ccp.nmr.NmrCalc.NmrCalcStore", "ccp.nmr.NmrCalc.Run"):
...
class NmrCalcRunFrame(Frame):
    def __init__(
        ...
        **kw,
        ...
        label = Label(tabbedFrame.sideFrame, text="Run #:", grid=(0, 0), sticky="e")

        tipText = 'Selects which calculation job or "run" is currently being viewed or edited'
        self.runPulldown = PulldownList(
            tabbedFrame.sideFrame, callback=self.changeRun, grid=(0, 1), sticky="e", tipText=tipText
        )
        ...
        runButtons = ButtonListClass(
            tabbedFrame.sideFrame, texts=texts, tipTexts=tipTexts, commands=commands, sticky="e", grid=(0, 2)
        )
        ...
        self.molSystemPulldown = PulldownList(subframe, callback=self.changeMolSystem, grid=(0, 1), tipText=tipText)
        ...
        if chainSelection:
            ...
            self.chainPulldown = PulldownList(subframe, callback=self.changeChain, grid=(0, 3), tipText=tipText)

        label = Label(subframe, text="Protocol name:", grid=(0, 4))
        tipText = "Name of calculation protocol for the current calculation"
        self.protocolEntry = Entry(subframe, text="", grid=(0, 5), width=32, tipText=tipText)
        ...
        self.detailsEntry = Entry(subframe, text="", grid=(0, 7), width=32, tipText=tipText)
        ...
        if STRUCTURE_DATA in inputTypes:
            ...
            self.structureTable = ScrolledMatrix(
                frame,
                grid=(0, 0),
                gridSpan=(1, 4),
            ...
            tipTexts = [
                "Remove the selected structural models from consideration (does not delete the coordinates)",
            ...
            self.structureButtons = ButtonList(frame, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
            ...
            tipTexts = [
                "Add the selected conformational model to the calculation input",
            ]
            texts = ["Add Model:"]
            commands = [self.addModel]
            self.modelButtons = ButtonList(frame, texts=texts, commands=commands, grid=(1, 2), tipTexts=tipTexts)
        ...
        if MEASUREMENT_DATA in inputTypes:
            ...
            self.measurementsTable = ScrolledMatrix(
                frame,
                grid=(0, 0),
                gridSpan=(1, 2),
            ...
            tipTexts = [
                "Remove the selected measurement list from consideration (does not delete the data)",
            ...
            buttons = ButtonList(frame, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
        ...
        if PEAK_DATA in inputTypes:
            ...
            self.peakListTable = ScrolledMatrix(
                frame,
                grid=(0, 0),
                gridSpan=(1, 2),
            ...
            tipTexts = [
                "Remove the selected peak list from consideration (does not delete the data)",
        ...
        if CONSTRAINT_DATA in inputTypes:
            ...
            self.constraintListTable = ScrolledMatrix(
                frame,
                tipTexts=tipTexts,
            ...
            tipTexts = [
                "Remove the selected restraint list from consideration (does not delete the data)",
        ...
        if self.__class__ is NmrCalcRunFrame:
            ...
            self.paramsTable = ScrolledMatrix(frameB, headingList=headingList, grid=(0, 0), tipTexts=tipTexts)
        ...
        self.outputTable = ScrolledMatrix(frameC, headingList=headingList, grid=(0, 0), tipTexts=tipTexts)
    ...
    def update(self, calcStore=None):
        ...
        if self.calcStore:
            ...
            self.nmrProject = (
                ...
                or self.project.findFirstNmrProject()
                or self.project.newNmrProject(name="default")
            )
        ...
        if STRUCTURE_DATA in self.inputTypes:
            ...
            for i, dataObj in enumerate(structureData):
                ...
                if structure:
                    ...
                    datum = [i + 1, structure.molSystem.code, structure.ensembleId, rangeText]

                else:
                    datum = [i + 1, DATA_MISSING, None, None]
        ...
        if MEASUREMENT_DATA in self.inputTypes:
            ...
            for i, dataObj in enumerate(measurementData):
                ...
                if mList:
                    datum = [i + 1, mList.className, mList.serial, mList.name, len(mList.measurements), mList.details]

                else:
                    datum = [i + 1, DATA_MISSING, None, None, None, None]
        ...
        if PEAK_DATA in self.inputTypes:
            ...
            for i, dataObj in enumerate(peakListData):
                ...
                if peakList:
                    ...
                    datum = [i + 1, experiment.name, spectrum.name, serial, len(peakList.peaks)]

                else:
                    datum = [i + 1, DATA_MISSING, None, None, None]
        ...
        if self.__class__ is NmrCalcRunFrame:
            ...
            if run:
                objectList = [x for x in run.sortedRunParameters() if x.ioRole == "input"]
        ...
        if run:
            ...
            outputPars = [
                x for x in run.sortedRunParameters() if x.ioRole == "output" and x.data is None and x.name != "summary"
            ]
    ...
    def addEnsemble(self):
        ...
        if run and ensemble:
            ...
            if not run.findFirstData(className=STRUCTURE_DATA, molSystemCode=msCode, ensembleId=eId, ioRole="input"):
                run.newStructureEnsembleData(molSystemCode=msCode, ensembleId=eId, ioRole="input")
    ...
    def addModel(self):
        ...
        if self.run and model:
            ...
            data = self.run.findFirstData(className=STRUCTURE_DATA, structureEnsemble=ensemble, ioRole="input")

            if not data:
                data = self.run.newStructureEnsembleData(structureEnsemble=ensemble, ioRole="input")
    ...
    def updateConstraintLists(self, obj=None):
        ...
        if run:
            for cList in self.getConstraintLists():
                ...
                data = run.findFirstData(
                    className=CONSTRAINT_DATA,
        ...
        if self.run and constraintList:
            ...
            if not data:
                ...
                self.run.newConstraintStoreData(
                    constraintStoreSerial=cSet, constraintListSerials=[constraintList.serial], ioRole="input"
                )
    ...
    def updatePeakLists(self, obj=None):
        ...
        if run:
            for peakList in self.getPeakLists():
                data = run.findFirstData(className=PEAK_DATA, peakList=peakList, ioRole="input")
    ...
    def addPeakList(self):
        ...
        if self.run and peakList:
            data = self.run.findFirstData(className=PEAK_DATA, peakList=peakList, ioRole="input")
            ...
            if not data:
                ...
                self.run.newPeakListData(
                    experimentSerial=eSerial, dataSourceSerial=sSerial, peakListSerial=pSerial, ioRole="input"
                )
    ...
    def updateMeasurementLists(self, obj=None):
        ...
        if run:
            ...
            for mList in nmr.sortedMeasurementLists():
                data = self.run.findFirstData(className=MEASUREMENT_DATA, measurementList=mList, ioRole="input")
    ...
    def addMeasurementList(self):
        ...
        if self.run and mList:
            ...
            if not data:
                self.run.newMeasurementListData(measurementListSerial=mList.serial, ioRole="input")
    ...
    def changeMolSystem(self, molSystem):
        ...
        if self.run and molSystem:
            data = self.run.findFirstData(className="MolSystemData", ioRole="input")
            ...
            if not data:
                self.run.newMolSystemData(molSystemCode=molSystem.code, ioRole="input")
    ...
    def updateMolSystemPulldown(self, obj=None):
        ...
        run = self.run
        if run:
            data = run.findFirstData(className="MolSystemData", ioRole="input") or run.findFirstData(
                className="MolResidueData", ioRole="input"
            )
