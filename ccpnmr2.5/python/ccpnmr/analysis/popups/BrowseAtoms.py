...
from ccpnmr.analysis.core.MoleculeBasic import (
    ...
    getUnicodeAtomName,
    greekSortAtomNames,
    makeResidueAtomSetsEquivalent,
    makeResidueAtomSetsNonEquivalent,
)
...
MAP_TYPE_OPTS = {"nonstereo": 1, "stereo": 0, "ambiguous": 0, "simple": 1}
...
class BrowseAtomsPopup(BasePopup):
    ...
    **Tips and Caveats**
    ...
    def body(self, guiParent):
        ...
        self.tabbedFrame = TabbedFrame(guiParent, options=options, callback=self.toggleTab, grid=(0, 0))
        frameA, frameB = self.tabbedFrame.frames
        frameA.expandGrid(1, 0)
        frameB.expandGrid(4, 0)

        frameT = self.tabbedFrame.sideFrame
        self.bottomButtons = UtilityButtonList(frameT, doClone=False, grid=(0, 0), helpUrl=self.help_url, sticky="e")
        # Atom Table
        ...
        self.chainPulldown = PulldownList(frame, self.changeChain, grid=(0, 1), tipText=tipText)

        tipText = "Select the type of residue to display atoms for, defaults to all types"
        label = Label(frame, text="Residue:", grid=(0, 2))
        self.resPulldown = PulldownList(frame, self.changeResCode, grid=(0, 3), tipText=tipText)
        ...
        self.nucleotidePulldown = PulldownList(
            frame,
        ...
        labels = ["H", "N", "C", "P"]
        self.elementSelect = PartitionedSelector(
            frame,
            labels=labels,
            objects=labels,
            callback=self.toggleChemElement,
            selected=[],
            grid=(1, 1),
            gridSpan=(None, 5),
            tipText=tipText,
        ...
        tipTexts = ["Sequence number code", "Residue type code"]
        ...
        tipTexts = [
            "Toggle whether the symmetric aromatic ring of selected residue\nflips quickly on the NMR timescale",
        ...
        texts = ["Set Ring Flip\nEquivalency", "Remove Atom\nAssignments", "Show\nPeaks", "Show\nResonances"]
        # commands = [self.minimAtomMatrix,self.changeStereo,self.getInfo]
        commands = [self.setAromaticEquivalency, self.removeAssignments, self.showPeaks, self.showResonances]
        self.bottomButtons = ButtonList(frameA, texts=texts, tipTexts=tipTexts, commands=commands, grid=(2, 0))
        ...
        self.experimentTypePulldown = PulldownList(frameB, self.changeExperimentType, tipText=tipText)
        ...
        self.jBondsSelect = PartitionedSelector(
            frameB, labels=numbers, objects=numbers, tipText=tipText, callback=self.toggleJbonds, selected=range(1, 3)
        )
        ...
        tipText = (
            "Whether to show distinct but stereoscopically ambiguous assignment options.\n"
            + 'For example you may not know if a Serine Hb is "Hb2" or "Hb3"'
        )
        labelNonstereo = Label(frame, text="Non-Stereospecific ", grid=(0, 0))
        self.checkNonstereo = CheckButton(frame, callback=self.setNonstereo, grid=(0, 1), tipText=tipText)
        ...
        self.checkStereo = CheckButton(frame, callback=self.setStereo, grid=(0, 3), tipText=tipText)

        tipText = (
            "Whether to include multi-atom ambiguous options like Ser Hb*.\n"
        ...
        self.checkAmbiguous = CheckButton(frame, callback=self.setAmbiguous, grid=(0, 5), tipText=tipText)
        ...
        tipTexts = [
            "Show all atom options irrespective of assignments",
        ...
        self.assignSelect = RadioButtons(
            frame, entries=entries, grid=(0, 0), select_callback=self.selectAssign, gridSpan=(1, 3), tipTexts=tipTexts
        )
        ...
        self.shiftListPulldown = PulldownList(frame, self.changeShiftList, grid=(1, 1), sticky="w", tipText=tipText)
    ...
    def removeAssignments(self):
        ...
        if self.atomSetMapping and self.atomSetMapping.resonanceSerials:
            ...
            text = "%d%s%s" % (
                residue.seqCode,
                ccpCode,
                self.getUnicodeAtomName(self.atomSetMapping, residue.molResidue.molType),
            )
            ...
            if atom and atom.atomSet:
                if self.isAromaticDEAssigned(self.residue):
                    if len(atom.atomSet.atoms) > 1:
                        if showOkCancel(
                            "Confirm",
                            "Are you sure? Any current assignments will become non-stereospecific.",
                            parent=self,
                        ):
                            makeResidueAtomSetsNonEquivalent(self.residue)
                    else:
                        if showOkCancel(
                            "Confirm", "Are you sure? Any current assignments may be duplicated", parent=self
                        ):
    ...
    def updateShiftLists(self, *obj):
        ...
        shiftListNames = [
            "<Ignore>",
        ] + self.getShiftListNames(shiftLists0)
    ...
    def updateChemElements(self):
        ...
        if self.chain:
            ...
            self.elementSelect.update(
                objects=chemElements, labels=chemElements, colors=colors, selected=self.chemElements
            )
        ...
        if self.chain:
          ...
          """
                ...
                if refExperiment or labelling:
                    obsAtoms = getResidueObservableAtoms(
                        residue,
                        refExperiment,
                        labelling,
                        minFraction,
