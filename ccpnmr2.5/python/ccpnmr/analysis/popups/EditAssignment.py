...
from memops.gui.MessageReporter import showMulti, showOkCancel, showWarning, showYesNo
from memops.gui.PulldownList import PulldownList
from memops.gui.ScrolledMatrix import ScrolledMatrix
from memops.universal.BlockData import arrayOfIndex, cumulativeArray
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.structurePulldown = PulldownList(
            peakFrame, grid=(0, 3), tipText=tipText, callback=self.setStructure, sticky="e"
        )

        utilButtons = UtilityButtonList(peakFrame, doClone=False, helpUrl=self.help_url, grid=(0, 4))
        ...
        self.aliasSelect = CheckButton(frame, callback=self.setAlias, selected=True, grid=(0, 1), tipText=tipText)

        label = Label(frame, text="Restrict Mol System", grid=(0, 2))
        tipText = (
            "Whether to restrict assignment possibilities to only those within a single molecular system (complex)"
        )
        self.molSysSelect = CheckButton(
            frame, callback=self.setRestrictMolSys, selected=True, grid=(0, 3), tipText=tipText
        )
        ...
        self.boundAtomsSelect = CheckButton(
            frame, callback=self.setBoundAtoms, selected=True, grid=(1, 1), tipText=tipText
        )
        ...
        self.doubleTolSelect = CheckButton(
            frame, callback=self.setDoubleTolerance, selected=False, grid=(1, 3), tipText=tipText
        )
        ...
        self.intraResSelect = CheckButton(
            frame, callback=self.setIntraResidue, selected=False, grid=(0, 5), tipText=tipText
        )
        ...
        self.groupsSelect = CheckButton(frame, callback=self.toggleGroups, selected=False, grid=(1, 5), tipText=tipText)

        label = Label(frame, text="Show Mol System", grid=(2, 0))
        tipText = "Whether to show molecular system in the proposed resonance table"
        self.showMolSysSelect = CheckButton(
            frame, callback=self.setShowMolSys, selected=False, grid=(2, 1), tipText=tipText
        )
        ...
        self.labellingPulldown = PulldownList(frame2, grid=(0, 1), tipText=tipText, callback=self.changeLabellingScheme)

        label = Label(frame2, text="Min Isotope Fraction:", grid=(0, 2))
        tipText = (
            "The lower limit for spin active isotope incorporation at an atom site for a resonance to be considered"
        )
        self.minIsoFracEntry = FloatEntry(
            frame2, grid=(0, 3), width=7, text=0.1, returnCallback=self.updateAfterEntry, tipText=tipText
        )
        ...
        commands = [
            ...
            self.deassignResonance,
            self.deassignSpinSystem,
            self.clearContrib,
        ]
        tipTexts = [
            "Assign the resonance selected in a peak dimension table to atoms",
        ...
        functionButtons = ButtonList(guiFrame, commands=commands, texts=texts, grid=(row, 0), tipTexts=tipTexts)
    ...
    def addDimension(self, dim):
        ...
        headingList = ["F%d" % dim]
        tipTexts = [
            "The peak dimension number and position",
    ...
    def updateLabellingSchemes(self, obj=None):
        ...
        names = [
            ...
            "<None>",
        ] + ["Scheme: %s" % sc.name for sc in schemes[2:]]
    ...
    def setAssignOpt(self, event):
        ...
        if option == 1:
            ...
            name = askString("Query", "Enter Resonance Name:", resonance.name or "", parent=self)

            if name is not None:
                name = name.strip() or None
                resonance.setName(name)

        elif spinSystem and (option == 5):
            name = askString("Query", "Enter Spin System Name:", spinSystem.name or "", parent=self)
    ...
    def addSpinSystem(self):
        ...
        if self.peak:
            AssignmentBasic.addPeakResonancesToSpinSystem(
                [
                    self.peak,
                ]
            )
    ...
    def assignResidueType(self, residue):
        ...
        if not spinSystem:
            spinSystem = self.nmrProject.newResonanceGroup(
                resonances=[
                    resonance,
                ]
            )
        ...
        AssignmentBasic.assignSpinSystemType(spinSystem, residue.ccpCode, residue.molType)
    ...
    def assignResidueTentative(self, residue):
        ...
        AssignmentBasic.assignTentativeSpinSystemResidues(
            spinSystem,
            [
                residue,
            ],
    ...
    def assign(self, object, component=None):
        ...
        if object is None:
            ...
            for isotope in peakDim.dataDimRef.expDimRef.isotopeCodes:
                elements.append(re.match(r"\d+([A-Z]\D*)", isotope).group(1))
            ...
            for atomSetMapping in atomSetMappings:
                for element in elements:
                    if element != atomSetMapping.elementSymbol:
                        ...
                        showWarning(
                            "Assignment failed", msg % (elementString, atomSetMapping.elementSymbol), parent=self
                        )
                        return
        ...
        for i in range(len(resonances)):
            ...
            if self.contrib:
                if resonance is None:
                    ...
                    atomStr = ",".join(
                        ["%d%s %s" % (residue.seqCode, getResidueCode(residue), ass.name) for ass in atomSets0]
                    )
                    if shift and (abs(peakDim.realValue - shift.value) > tolerance):
                        ...
                        if component:
                            CouplingBasic.assignPeakDimComponentResonance(component, resonance, tolerance=tolerance)
                        else:
                            assignResToDim(peakDim, resonance, tolerance=tolerance, doWarning=True)
                        ...
                        if existingShift:
                            ...
                            AssignmentBasic.mergeResonances(resonance, resonance2)
                            print(
                                "NOTICE: Atom assignment %s existed in a different shift list for a different resonance."
                                % atomStr
                            )
                            print(
                                "NOTICE: Resonance [%d] has been merged with resonance [%d]."
                                % (resonance2.serial, resonance.serial)
                            )
            ...
            if self.checkSpinSystem(resonance, atomSets0):
                if (not atomSets0) or self.checkBmrbShift(resonance, atomSets0[0], peakDim):
                    ...
                    if component:
                        ...
                        AssignmentBasic.assignPeakDim(resonance, peakDim, atomSets0, self.contrib, tolerance)
    ...
    def checkBmrbShift(self, resonance, atomSet, peakDim):
        ...
        chemAtomNmrRef = getChemAtomNmrRef(project, name, ccpCode, molType=molType)
    ...
    def update(self, peak=None):
        ...
        if peak:
            ...
            for peakDim in peakDims:
                ...
                peakDimComponents = [
                    None,
                ] + peakDim.sortedPeakDimComponents()
                ...
                for component in peakDimComponents:
                    ...
                    if component:
                        ...
                        if expDimRef.measurementType in PPM_MEASUREMENT_TYPES:
                            ...
                            data = (peakDim.dim, component.serial, isotopesCode, ppm, peakDim.value)
                            ...
                            data = (peakDim.dim, component.serial, isotopesCode, hz, dataDimRef.expDimRef.unit)
                        ...
                        if showGroups and peakContribs:
                            ...
                            for contrib in contribs:
                                ...
                                if groupContribs:
                                    if (groupContribs == frozenset(peakContribs)) and (len(groupContribs) > 1):
                                        group = "*"
                                    else:
                    ...
                    tipTexts = [
                        "The peak dimension number, isotope and position",
                        "The (mutual exclusion) assignment group to for the linked resonance",
                    ]
                    self.peakDimPanels[i].update(
                        headingList=headingList,
                        objectList=contribs,
            ...
            data = (experiment.name, spectrum.name, peak.peakList.serial, peak.serial, experiment.shiftList.serial)
                                ...
                                for resonance1 in resonances[j]:
                                    ...
                                    if resonance0 and resonance1:
                                        if areResonancesBound(resonance0, resonance1):
                                            ...
                                            if labelling:
                                                fraction = getFrac(resonance0, resonance1, labelling)
                                                if fraction < minFraction:
                                                    continue
