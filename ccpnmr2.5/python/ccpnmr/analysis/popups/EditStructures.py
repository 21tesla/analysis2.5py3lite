...
from ccpnmr.analysis.core.StructureBasic import (
    alignStructures,
    compareEnsembles,
    copyModelToEnsemble,
    getResiduePhiPsi,
    getStructureFromFile,
    makeEmptyEnsembleCopy,
    makeEnsemble,
    makePdbFromStructure,
)
from ccpnmr.analysis.core.ValidationBasic import ANALYSIS_RMSD_CONTEXT, RMSD_KEYWORDS, getEnsembleValidationStore
...
from memops.gui.MessageReporter import showError, showOkCancel, showWarning
...
GRAPH_COLORS = (("#0000A0", "#A08000"), ("#008080", "#A00000"), ("#800080", "#80A000"), ("#008000", "#A00000"))
...
class EditStructuresPopup(BasePopup):
    ...
    **Model Superposition & RMSD Calculation**
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=options, tipTexts=tipTexts, callback=self.changeTab, grid=(0, 0))
        ...
        self.systemPulldown = PulldownList(structFrame, callback=self.changeMolSystem, grid=(0, 1), tipText=tipText)
        ...
        colHeadings = ["#", "Chains", "Residues", "Models", "RMSD", "Generation\nRun", "Violation\nLists"]
        self.structMatrix = ScrolledMatrix(
            structFrame,
            initialRows=7,
            tipTexts=tipTexts,
            ...
            multiSelect=True,
            deleteFunc=self.deleteStructure,
            grid=(1, 0),
            gridSpan=(1, 2),
        )

        tipTexts = [
            "Show a table of atom coordinates for a model from the selected ensemble",
        ...
        self.structButtons = ButtonList(
            structFrame, texts=texts, commands=commands, grid=(2, 0), gridSpan=(1, 2), tipTexts=tipTexts
        )
        ...
        self.modelsEnsemblePulldown = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changeEnsemble)

        commands = [self.alignEnsembleM, self.deleteModel]
        tipTexts = [
            "Calculate all-atom and backbone atom coordinate root mean square deviations for the selected ensemble; fills the graph",
            "Delete the currently selected structural model from the ensemble",
        ]
        texts = ["Superpose & Calculate RMSDs", "Delete Model"]
        buttons = ButtonList(frame, grid=(0, 3), commands=commands, texts=texts, tipTexts=tipTexts)
        ...
        editWidgets = [None, self.modelNameEntry, None, self.modelDetailsEntry]
        editGetCallbacks = [None, self.getModelName, None, self.getModelDetails]
        editSetCallbacks = [None, self.setModelName, None, self.setModelDetails]

        tipTexts = [
            "The number of the conformational model within the structure ensemble",
        ...
        self.modelMatrix = ScrolledMatrix(
            modelFrame,
        ...
        self.resEnsemblePulldown = PulldownList(frame, callback=self.changeEnsemble, grid=(0, 1), tipText=tipText)
        ...
        self.ccpCodePulldown = PulldownList(frame, callback=self.changeCcpCode, grid=(0, 3), tipText=tipText)
        ...
        self.angleCheck = CheckButton(frame, selected=True, tipText=tipText, callback=self.updateResidues, grid=(0, 1))
        ...
        self.rmsdCheck = CheckButton(frame, selected=True, tipText=tipText, callback=self.updateResidues, grid=(0, 3))

        label = Label(frame, text="Rama.\nparams", grid=(0, 4))
        tipText = "Whether to show the Ramachandran plot regional classification using backbone angles"
        self.ramaCheck = CheckButton(frame, selected=True, tipText=tipText, callback=self.updateResidues, grid=(0, 5))

        label = Label(frame, text="Validation\nresults", grid=(0, 6))
        tipText = (
            "Whether to show per-residue structure validation results (of various types where calculated) in the table"
        )
        self.validCheck = CheckButton(frame, selected=False, tipText=tipText, callback=self.updateResidues, grid=(0, 7))

        label = Label(frame, text="Physical\nparams")
        # label.grid(row=0, column=6, sticky='w')
        self.physCheck = CheckButton(frame, selected=False, callback=self.updateResidues)
        # self.physCheck.grid(row=0, column=7, sticky='w')

        tipTexts = [
            "The code of the molecular chain to which the residue belongs",
        ...
        self.residueMatrix = ScrolledMatrix(
            resFrame,
        ...
        self.ensemblePulldown = PulldownList(coordsFrame, callback=self.changeEnsemble, grid=(0, 1), tipText=tipText)
        ...
        self.modelPulldown = PulldownList(coordsFrame, callback=self.changeModel, grid=(0, 3), tipText=tipText)
        ...
        colHeadings = ["#", "Atom", "Residue", "Chain", "SeqId", "x", "y", "z", "Occupancy", "B Factor", "Locn Code"]
        self.coordsMatrix = ScrolledMatrix(
            coordsFrame,
            headingList=colHeadings,
        ...
        self.systemPulldown2 = PulldownList(frame, callback=self.changeMolSystem2, grid=(0, 1), tipText=tipText)

        tipTexts = ("Compare all atoms", "Compare only backbone atoms")
        self.compareRadio = RadioButtons(
            frame, entries=COMPARE_OPTIONS, select_callback=self.changeComparison, tipTexts=tipTexts, grid=(0, 3)
        )
        ...
        buttons = ButtonList(frame, grid=(0, 4), commands=commands, texts=texts, tipTexts=tipTexts)

        tipTexts = ["The ensembles"]
        headingList = ["Ensembles"]
        self.compareMatrix = ScrolledMatrix(compareFrame, headingList=headingList, grid=(1, 0), tipTexts=tipTexts)

        self.compareGraph = ScrolledGraph(
            compareFrame,
            symbolSize=2,
            reverseX=False,
            width=550,
            height=220,
            title="Residue comparison RMSDs",
            xLabel="Residue",
            yLabel="RMSD",
            grid=(2, 0),
        )
    ...
    def updateRmsdGraph(self):
        ...
        if self.structure:
            ...
            validStore = getEnsembleValidationStore(self.structure, ANALYSIS_RMSD_CONTEXT, RMSD_KEYWORDS)
            ...
            for i, chain in enumerate(chains):
                ...
                for residue in chain.sortedResidues():
                    ...
                    for j in (0, 1):
                        validObj = findValid(
                            validationStore=validStore, context=ANALYSIS_RMSD_CONTEXT, keyword=RMSD_KEYWORDS[j]
                        )
            ...
            self.rmsdGraph.update(dataSets=dataSets, dataColors=dataColors, dataNames=dataNames)
    ...
    def updateModels(self):
        ...
        if self.structure:
            ...
            for model in objectList:
                rmsdValid = getModelValidation(validStore, model, ANALYSIS_RMSD_CONTEXT, RMSD_KEYWORDS[1])
        ...
        self.modelMatrix.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def deleteModel(self):
        ...
        if self.model:
            ...
            if showOkCancel("Confirm", msg, parent=self):
                ...
                validStore = getEnsembleValidationStore(ensemble, ANALYSIS_RMSD_CONTEXT, RMSD_KEYWORDS)
    ...
    def updateResidues(self, obj=None):
        ...
        if doRmsd:
            ...
            tipTexts.append(
                "The coordinate root mean square deviation for the residue within the ensemble, considering only backbone atoms"
            )
            tipTexts.append(
                "The coordinate root mean square deviation for the residue within the ensemble, considering all atoms"
            )
        ...
        if self.structure:
            ...
            rmsdStore = getEnsembleValidationStore(self.structure, ANALYSIS_RMSD_CONTEXT)
                ...
                for residue in chain.sortedResidues():
                    ...
                    if doRmsd:
                        for keyword in RMSD_KEYWORDS[:2]:
                            validObj = findValid(
                                validationStore=rmsdStore, context=ANALYSIS_RMSD_CONTEXT, keyword=keyword
                            )
                    ...
                    if doValid:
                        for context, keyword, validStore in validKeys:
                            validObj = findValid(validationStore=validStore, context=context, keyword=keyword)
            ...
            if doRmsd and rmsdStore.findFirstValidationResult(
                className="ResidueValidation", context=ANALYSIS_RMSD_CONTEXT
            ):
                for keyword in RMSD_KEYWORDS[:2]:
                    validKeys.append((ANALYSIS_RMSD_CONTEXT, keyword, rmsdStore))

        self.residueMatrix.update(
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("__init__", "delete", "setX", "setY", "setZ", "setOccupancy", "setBFactor", "setaltLocationCode"):
            for clazz in ("ccp.molecule.MolStructure.Coord",):
                notifyFunc(self.updateCoordsAfter, clazz, func)

        for func in ("__init__", "delete", "addMolStructure", "removeMolStructure", "setMolStructures", "setDetail"):
            for clazz in ("ccp.nmr.Nmr.StructureGeneration", "ccp.nmr.NmrConstraint.ViolationList"):
    ...
    def mergeModels(self):
        ...
        if ensembles and self.molSystem:
            ...
            if models:
                makeEnsemble(models, ensemble=models[0].structureEnsemble, replaceStructures=True)
    ...
    def importStructure(self, fileNames=None):
        ...
        for file in fileNames:
            ...
            if self.molSystem is None:
                ...
                self.molSystem = self.project.newMolSystem(
                    code="MS%d" % codeNum, name="Default", details="Default made for structures"
                )
    ...
    def alignEnsemble(self, structure=None):
        ...
        if structure:
            ...
            msg = "Really superpose %d models of structure ensemble %s? (The change will persist)" % (
                len(models),
                structure.ensembleId,
            )

            if showOkCancel("Confirm", msg, parent=self):
                structures, error, structureRmsds, atomRmsdDict = alignStructures(
                    [
                        structure,
                    ]
                )
    ...
    def exportStructure(self):
        ...
        fileTypes = [FileType("PDB", ["*.pdb"]), FileType("All", ["*"])]
        fileSelectPopup = FileSelectPopup(
            self, file_types=fileTypes, title="Export PDB file", dismiss_text="Cancel", selected_file_must_exist=False
        )
    ...
    def update(self):
        ...
        for structure in ensembles:
            ...
            datum = [structure.ensembleId, " ".join(codes), numResidues, len(models), rmsd, strucGenStr, len(violLists)]
    ...
    def updateCompareTable(self):
        ...
        self.compareMatrix.update(
            headingList=headingList, objectList=structures, textMatrix=textMatrix, tipTexts=tipTexts
        )
                ...
                for structure2 in structures[i + 1 :]:
                    ...
                    if key in rmsdsDict:
                        ...
                        for chain in chains:
                            ...
                            if len(chains) > 1:
                                dataNames.append(
                                    "%d-%d (chain %s)" % (structure1.ensembleId, structure2.ensembleId, chain.code)
                                )
                            else:
                                dataNames.append("%d-%d" % (structure1.ensembleId, structure2.ensembleId))

            self.compareGraph.update(dataSets=dataSets, dataColors=dataColors, dataNames=dataNames)
