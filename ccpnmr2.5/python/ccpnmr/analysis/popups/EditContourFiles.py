...
class EditContourFilesPopup(BasePopup):
    ...
    **Manage Contour Files in Project**
    ...
    def body(self, master):
        ...
        self.buttons = UtilityButtonList(master, texts=texts, doClone=False, commands=commands, helpUrl=self.help_url)
...
"""
    ...
    def deleteContourFile(self, *extra):
        ...
        if showYesNo(
            ...
            "Are you sure you want to delete selected %sstored contour%s from the project?" % (s, t),
            parent=self,
        ):
            if showYesNo(
                "Delete underlying contour file%s on disk" % t,
                "Do you also want to delete underlying contour file%s on disk (normally yes)?" % t,
                parent=self,
            ):
