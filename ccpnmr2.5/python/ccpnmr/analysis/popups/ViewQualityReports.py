...
from ccpnmr.analysis.core.QualityControlBasic import (
    analyseAssignmentCompleteness,
    analyseChemicalShifts,
    analyseNoeAssignments,
    analysePeaks,
)
...
class ViewQualityReportsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tipTexts = ["", "", "", ""]
        options = ["Atom Assign Status", "Through-Space Status", "Resonances", "Peaks"]

        tabbedFrame = TabbedFrame(guiFrame, options=options, callback=self.selectTab, grid=(0, 0), tipTexts=tipTexts)
        ...
        self.molSystemPulldownA = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changeMolSystemA)
        ...
        self.shiftListPulldownA = PulldownList(frame, grid=(0, 3), tipText=tipText, callback=self.changeShiftListA)
        ...
        self.residueEntry = Entry(frame, text="", grid=(0, 6), tipText=tipText, returnCallback=self.reportAssignStatus)

        text = "Include water exchangeable atoms"
        tipText = "Whether to include water exchangeable atoms in the statistics"
        self.waterExchangeableCheckButton = CheckButton(
            frame, grid=(1, 0), text=text, tipText=tipText, callback=self.refresh
        )

        tipTexts = [
            "The name of a category into which atoms of the selected chain/residue have been grouped",
        ...
        headingList = ["Category", "Available", "Assigned", "% Assigned", "Min\nShift", "Mean\nShift", "Max\nShift"]
        ...
        self.molSystemPulldownB = PulldownList(frame, callback=self.changeMolSystemB, grid=(0, 1), tipText=tipText)
        ...
        self.peakListPulldownB = PulldownList(frame, callback=self.changePeakListB, grid=(0, 3), tipText=tipText)

        tipTexts = [
            "The identity of the chain or residue that the data in the row pertains",
        ...
        self.shiftListPulldownC = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changeShiftListC)
        ...
        self.resAlertCheck = CheckButton(
            frame, selected=False, grid=(0, 4), callback=self.reportChemShifts, tipText=tipText
        )
        ...
        self.peakListPulldownD = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changePeakListD)

        label = Label(frame, text="Show only alerts:", grid=(0, 3))
        tipText = "Sets whether to show only rows in the table for peaks that carry warnings"
        self.peakAlertCheck = CheckButton(
            frame, grid=(0, 4), selected=False, callback=self.reportPeaks, tipText=tipText
        )
        ...
        self.peakMatrix = ScrolledMatrix(
            frameD,
            headingList=headingList,
            multiSelect=True,
            callback=self.selectObject,
        ...
        label = Label(frame, text="Mol System:", grid=(0, 0))
        tipText = "Selects which molecular system (group of chains) to display assignment status information for"
        self.molSystemPulldownD = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changeMolSystemD)

        tipTexts = [
            "The row number",
            "The chain code",
        ] + 7 * [""]
        headingList = ["#", "Chain", "Seq Code", "Ccp Code", "Atom", "Resonances", "Shifts", "Delta", "Stdev"]

        self.peakAtomMatrix = ScrolledMatrix(
            frameD,
            headingList=headingList,
            multiSelect=True,
            callback=self.selectObject,
        ...
        sideButtons = UtilityButtonList(tabbedFrame.sideFrame, helpUrl=self.help_url, grid=(0, 0), sticky="e")

        tipTexts = [
            "Manually update the currently viewed table in light of assignment changes etc.",
        ...
        self.bottomButtons = ButtonList(guiFrame, texts=texts, tipTexts=tipTexts, commands=commands, grid=(1, 0))
    ...
    def selectTab(self, i):
        ...
        commands = [self.reportAssignStatus, self.reportNoe, self.reportChemShifts, self.reportPeaksAndAtoms]
    ...
    def getResidueSelection(self):
        ...
        for resRange in resRanges:
            ...
            matchObj = match(r"([a-zA-Z_]*)\s*(-*\d+)\s*-\s*([a-zA-Z_]*)\s*(-*\d+)", resRange)
            ...
            if matchObj:
                ...
                matchObj = match(r"([a-zA-Z_]*)\s*(-*\d+)", resRange)
    ...
    def reportAssignStatus(self, *event):
        ...
        if self.molSystemA and self.shiftListA:
            ...
            data = analyseAssignmentCompleteness(
                self.molSystemA, self.shiftListA, residueSelection, excludeWaterExchangeable
            )
        ...
        self.atomMatrix.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def reportNoe(self):
        ...
        self.noeMatrix.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def boundResonancesOfCorrectIsotopeCode(self, peak, dataDim, isotopeCode):
        ...
        for resonance in resonances:
            ...
            resonanceSet.update(
                [boundResonance for boundResonance in boundResonances if isotopeCode == boundResonance.isotopeCode]
            )
    ...
    def reportPeaks(self, *event):
        ...
        if self.peakListD:
            ...
            for peak, datum in data:
                ...
                datum = [serial, annotation, ",".join(assignErrors), location, volume, nVolSd, height, nHtSd]
    ...
    def reportAtoms(self, *event):
        ...
        self.peakAtomMatrix.update(textMatrix=textMatrix, objectList=objectList)
