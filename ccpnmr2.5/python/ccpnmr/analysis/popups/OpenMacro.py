    ...
    **Locate Python Macro Scripts On Disk**
    ...
    def body(self, guiParent):
        ...
        self.file_select = FileSelect(
            guiParent, file_types=file_types, single_callback=self.chooseFile, double_callback=self.chooseFile
        )
        ...
        self.scrolledMatrix = ScrolledMatrix(
            guiParent, initialRows=4, headingList=headingList, callback=self.selectFunction
        )
        ...
        buttons = UtilityButtonList(guiParent, texts=texts, commands=commands, helpUrl=self.help_url)
    ...
    def updateFunctions(self, file):
        ...
        textMatrix = []
        while line:
            match = re.match(r"\s*def\s+(\w+)\s*\(.*argServer", line)
            if match:
    ...
    def loadMacro(self):
        ...
        if self.module and self.function and self.path:
            ...
            m1 = analysisProfile.newMacro(
                name=name, path=self.path, function=self.function, module=self.module, ordering=1
            )
