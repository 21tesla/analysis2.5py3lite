...
class EditMarksPopup(BasePopup):
    ...
    **Setup Spectrum Marker and Ruler Line Options**
    ...
    def body(self, guiParent):
        ...
        row = 0
        div = LabelDivider(guiParent, text="Multi-dimensional Marks", grid=(row, 0), gridSpan=(1, 4))
        ...
        tipText = (
            "Sets the maximum number of multi-dimensional cross-marks that the user can add to spectrum window displays"
        )
        self.marks_menu = PulldownList(
            guiParent,
            callback=self.setMaxMarks,
            grid=(row, 1),
            texts=entries,
            objects=values,
            index=value - 1,
        ...
        tipText = (
            "Sets the line colour of the multi-dimensional cross-marks, excepting those that go through peak centers"
        )
        self.marksSchemePulldown = PulldownList(guiParent, callback=self.setMarksColor, grid=(row, 3), tipText=tipText)

        row += 1
        label = Label(guiParent, text="(Non-peak marks only)", grid=(row, 2), gridSpan=(1, 2))

        row += 1
        div = LabelDivider(guiParent, text="1-dimensional Rulers", grid=(row, 0), gridSpan=(1, 5))
        ...
        tipText = (
            "Sets the maximum number of 1-dimensional ruler lines that the user can add to spectrum window displays"
        )
        self.rulers_menu = PulldownList(
            guiParent,
            callback=self.setMaxRulers,
            grid=(row, 1),
            texts=entries,
            objects=values,
            index=value - 1,
        ...
        self.rulersSchemePulldown = PulldownList(
            guiParent, callback=self.setRulersColor, grid=(row, 3), tipText=tipText
        )
