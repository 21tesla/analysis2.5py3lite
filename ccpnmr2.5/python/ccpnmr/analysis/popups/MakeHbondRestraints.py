...
from ccpnmr.analysis.core.MoleculeBasic import getBoundAtoms, getResidueCode
...
NUCLEIC_HBONDS = {
    frozenset(["G", "C"]): ({"G": "O6", "C": "H42"}, {"G": "H1", "C": "N3"}, {"G": "H22", "C": "O2"}),
    frozenset(["A", "T"]): ({"A": "N1", "T": "H3"}, {"A": "H62", "T": "O4"}),
    frozenset(["A", "U"]): ({"A": "N1", "U": "H3"}, {"A": "H62", "U": "O4"}),
    frozenset(["I", "A"]): ({"I": "06", "A": "H62"}, {"I": "H1", "A": "N1"}),
    frozenset(["I", "C"]): ({"I": "06", "C": "H42"}, {"I": "H1", "C": "N3"}),
    frozenset(["I", "U"]): ({"I": "06", "U": "H3"}, {"I": "H1", "U": "O4"}),
    frozenset(["I", "T"]): ({"I": "06", "T": "H3"}, {"I": "H1", "T": "O4"}),
}
...
class MakeHbondRestraintsPopup(BasePopup):
    ...
    **Evidence**
    ...
    def __init__(self, parent, *args, **kw):
        ...
        BasePopup.__init__(self, parent=parent, title="Structure : Make H Bond Restraints", **kw)
    ...
    def body(self, guiFrame):
        ...
        self.lowerEntry = FloatEntry(frame, text=DEFAULT_LOWER, grid=(1, 1), width=8, tipText=tipText)
        ...
        self.targetEntry = FloatEntry(frame, text=DEFAULT_TARGET, grid=(1, 3), width=8, tipText=tipText)
        ...
        self.upperEntry = FloatEntry(frame, text=DEFAULT_UPPER, grid=(1, 5), width=8, tipText=tipText)
        ...
        self.bondEntry = FloatEntry(frame, text=DEFAULT_BOND, grid=(1, 7), width=8, tipText=tipText)
        ...
        self.hbondTypePulldown = PulldownList(
            frame, grid=(1, 2), texts=texts, objects=HBOND_TYPES, tipText=tipText, callback=self.changeHbondType
        )

        div = LabelDivider(tFrame, text="Hydrogen Bond Restraints", gridSpan=(1, 2), grid=(1, 0))

        self.distLowerEntry = FloatEntry(self, text=0.0, width=8, returnCallback=self.setLower)

        self.distTargetEntry = FloatEntry(self, text=0.0, width=8, returnCallback=self.setTarget)

        self.distUpperEntry = FloatEntry(self, text=0.0, width=8, returnCallback=self.setUpper)
        ...
        tipTexts = [
            "The donor hydrogen atom involved in the hydrogen bond restraint",
        ...
        editWidgets = [
            self.donorPulldown,
            self.acceptorPulldown,
            self.distLowerEntry,
            self.distTargetEntry,
            self.distUpperEntry,
        ]
        editGetCallbacks = [
            self.getAtomA,
        ...
        self.hbondTable = ScrolledMatrix(
            tFrame,
        ...
        tipTexts = [
            "Add a new hydrogen bond restraint to the selected list; the acceptor and donor atoms etc. are set subsequently",
        ...
        texts = ["Add New", "Delete Selected", "Set Default\nDistances", "Delete List", "Make Co-linear\nRestraints"]
        commands = [
            self.newRestraint,
            self.deleteRestraints,
            self.setDefaultDistances,
            self.deleteRestraintList,
            self.makeCoLin,
        ]
        self.restraintButtons = ButtonList(
            tFrame, texts=texts, gridSpan=(1, 2), commands=commands, grid=(3, 0), tipTexts=tipTexts
        )
        ...
        self.helixTypePulldown = PulldownList(self, callback=self.setHelixType, objects=[4, 5, 3], texts=HELIX_TYPES)
        ...
        editWidgets = [None, self.helixStartPulldown, self.helixEndPulldown, self.helixTypePulldown]
        editGetCallbacks = [None, self.getHelixStart, self.getHelixEnd, self.getHelixType]
        editSetCallbacks = [None, self.setHelixStart, self.setHelixEnd, self.setHelixType]
        self.helixTable = ScrolledMatrix(
            pFrame,
            headingList=headingList,
        ...
        self.helixButtons = ButtonList(pFrame, texts=texts, commands=commands)  # , grid=(2,0))
        # Sheets table
        ...
        self.strandTypePulldown = PulldownList(self, callback=self.setStrandType, objects=[-1, 1], texts=SHEET_TYPES)
        ...
        editWidgets = [
            None,
            self.strandStart1Pulldown,
            self.strandEnd1Pulldown,
            self.strandStart2Pulldown,
            self.strandEnd2Pulldown,
            self.strandTypePulldown,
        ]
        editGetCallbacks = [
            None,
            self.setStrand1Start,
            self.setStrand1End,
            self.setStrand2Start,
            self.setStrand2End,
            self.setStrandType,
        ]
        editSetCallbacks = [
            None,
            self.getStrand1Start,
            self.getStrand1End,
            self.getStrand2Start,
            self.getStrand2End,
            self.getStrandType,
        ]
        self.sheetTable = ScrolledMatrix(
            pFrame,
            headingList=headingList,
        ...
        self.sheetButtons = ButtonList(pFrame, texts=texts, commands=commands)  # , grid=(5,0))
        ...
        commands = [self.makeRestraintsFromHelices, self.makeRestraintsFromSheets, self.makeRestraintsFromNonCovalent]

        self.pepRestraintButtons = ButtonList(pFrame, texts=texts, commands=commands)  # , grid=(7,0))
        ...
        self.duplexTable = ScrolledMatrix(
            nFrame,
            headingList=headingList,
            callback=None,
            editWidgets=editWidgets,
        ...
        self.basePairTable = ScrolledMatrix(
            nFrame,
            headingList=headingList,
            callback=None,
            editWidgets=editWidgets,
        ...
        self.restraintSetPulldown = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.selectRestraintSet)
        ...
        self.restraintListPulldown = PulldownList(
            frame, grid=(0, 3), tipText=tipText, callback=self.selectRestraintList
        )
        ...
        self.molSystemPulldown = PulldownList(frame, grid=(0, 5), tipText=tipText, callback=self.selectMolSystem)
    ...
    def makeCoLin(self):
        ...
        if restraintList:
            for restraint in restraintList.constraints:
                ...
                if r1.isotopeCode == "1H":
                    ...
                    boundH1 = [a for a in getBoundAtoms(atom1) if a.chemAtom.elementSymbol == "H"]
                    boundH2 = [a for a in getBoundAtoms(atom2) if a.chemAtom.elementSymbol == "H"]
        ...
        for key in triples:
            ...
            if restraintH is None:
                targetX = restraintX.targetValue or (0.5 * (restraintX.upperLimit + restraintX.lowerLimit))

                target = targetX - bondLen
                upper = target * restraintX.upperLimit / targetX
                lower = target * restraintX.lowerLimit / targetX
                restraintH = restraintList.newHBondConstraint(targetValue=target, upperLimit=upper, lowerLimit=lower)
                r1 = findResonance(atomH)
                r2 = findResonance(atomA)
                restraintH.newHBondConstraintItem(resonances=(r1, r2))

            elif restraintX is None:
                targetH = restraintH.targetValue or (0.5 * (restraintH.upperLimit + restraintH.lowerLimit))
    ...
    def updateRestraintLists(self, obj=None):
        ...
        if self.restraintSet:
            ...
            restraintLists = [
                None,
            ] + [x[1] for x in sortList]
            names = [
                "<New>",
            ] + [x[0] for x in sortList]
    ...
    def findResonance(self, atom):
        ...
        if not resonance:
            ...
            resonanceSet = nmrProj.newResonanceSet(atomSets=(atomSet,), resonances=(resonance,))
    ...
    def newRestraint(self):
        ...
        if self.restraintSet:
            ...
            restraint = self.restraintList.newHBondConstraint(targetValue=target, upperLimit=upper, lowerLimit=lower)
    ...
    def updateMain(self):
        ...
        if self.restraintList:
            for restraint in self.restraintList.sortedConstraints():
                ...
                datum = [donor, acceptor, restraint.lowerLimit, restraint.targetValue, restraint.upperLimit]

                textMatrixAppend(datum)
                objectListAppend(restraint)

        self.hbondTable.update(textMatrix=textMatrix, objectList=objectList)
    ...
    def getTempResonanceH(self, restraintSet):
        ...
        if not self.tempResonanceH:
            fResonance = restraintSet.findFirstFixedResonance(name="HBondTemp", isotopeCode="1H")

            if not fResonance:
                fResonance = restraintSet.newFixedResonance(name="HBondTemp", isotopeCode="1H")
    ...
    def getTempResonanceX(self, restraintSet):
        ...
        if not self.tempResonanceX:
            fResonance = restraintSet.findFirstFixedResonance(name="HBondTemp", isotopeCode="unknown")

            if not fResonance:
                fResonance = restraintSet.newFixedResonance(name="HBondTemp", isotopeCode="unknown")
            fResonance.guiName = "?"
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in ("__init__", "delete", "setLowerLimit", "setTargetValue", "setUpperLimit"):
        ...
        if self.restraintSet and self.molSystem:
            ...
            for element in elements:
                for ncBond in element.nonCovalentBonds:
                    ...
                    restraint = makeRestraint(targetValue=target, upperLimit=upper, lowerLimit=lower)
    ...
    def getElementPosition(self, element, residue0, pulldown):
        ...
        molSystem = self.molSystem
        names = [
            "%s%s" % (residue0.seqCode, getResidueCode(residue0)),
        ]
        residues = [
            residue0,
        ]
...
class Duplex:
    ...
    def __init__(self, molSystem, startResidueA=None, endResidueA=None, startResidueB=None, endResidueB=None):
...
class Sheet:
    ...
    def __init__(
        self, molSystem, startResidueA=None, endResidueA=None, startResidueB=None, endResidueB=None, register=-1
    ):
...
def getPairedHbondSheets(molSystem):
    ...
    for cluster in clusters:
        ...
        sheet = Sheet(molSystem, startA.residue, endA.residue, startB.residue, endB.residue, delta)
