...
from ccpnmr.analysis.core.MoleculeBasic import copyMolecule, getChemCompOverview, getMolTypeCcpCodes, getResidueCode
...
class EditMoleculesPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tabbedFrame = TabbedFrame(guiFrame, options=options, tipTexts=tipTexts, grid=(0, 0), callback=self.updateTab)
        ...
        tipTexts = [
            "The short name of the molecular system to which the chain belongs; a grouping of chains, e.g. in a complex",
        ...
        self.editChainDetlEntry = Entry(self, width=12, text="", returnCallback=self.setChainDetl)
        ...
        self.chainsMatrix = ScrolledMatrix(
            frameA,
            grid=(0, 0),
            tipTexts=tipTexts,
        ...
        tipTexts = [
            "Show a table containing the residue sequence for the chain, together with associated linking and protonation states",
        ...
        texts = ["Show\nSeq", "Delete\nChain", "Copy\nChain", "Add\nSequence", "Edit Molecule\nTemplates"]
        commands = [self.showChainSequence, self.deleteChain, self.copyChain, self.chainAddSequence, self.editMolecule]

        self.chainButtons = ButtonList(frameA, texts=texts, grid=(1, 0), commands=commands, tipTexts=tipTexts)
        ...
        tipText = (
            "Selects which molecular system group to place new chains in (when constructed with a sequence template)"
        )
        self.chainMolSystemPulldown = PulldownList(
            frame, grid=(0, 1), tipText=tipText, callback=self.selectChainMolSystem
        )
        ...
        self.chainMolPulldown = PulldownList(frame, grid=(0, 3), tipText=tipText, callback=self.setChainMolecule)

        tipTexts = [
            "Using the selected molecule template make a new chain withing the stated molecular system group; the chain made can be used for NMR assignments",
        ]
        texts = [
        ...
        self.chainButtons2 = ButtonList(frame, texts=texts, grid=(0, 4), commands=commands, tipTexts=tipTexts)
        ...
        self.editStartNumlEntry = IntEntry(self, text=0, width=6, returnCallback=self.setStartNum)
        ...
        self.fragmentMatrix = ScrolledMatrix(
            frameA,
            grid=(4, 0),
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = [
            "A sort textual code for the molecular system grouping; set when the molecular system is first created",
        ...
        colHeadings = ["Code", "Name", "Chains", "Molecules", "Mol Types", "Structures", "Details"]

        self.editMolSysNameEntry = Entry(self, text="", width=8, returnCallback=self.setMolSysName)
        self.editMolSysDetlEntry = Entry(self, text="", width=12, returnCallback=self.setMolSysDetl)

        editWidgets = [None, self.editMolSysNameEntry, None, None, None, None, self.editMolSysDetlEntry]

        editGetCallbacks = [None, self.getMolSysName, None, None, None, None, self.getMolSysDetl]

        editSetCallbacks = [None, self.setMolSysName, None, None, None, None, self.setMolSysDetl]

        self.molSysMatrix = ScrolledMatrix(
            frameB,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Add a new, blank molecular system to the CCPN project; which will then go on to contain chains of residues and their atoms",
        ...
        self.molSysButtons = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(1, 0))
        ...
        self.moleculePulldown = PulldownList(frameC, tipText=tipText, grid=(0, 1), callback=self.selectResMolecule)

        tipText = "An indication of which chains the template has been used to construct (inside the molecular systems)"
        self.molResChainInfoLabel = Label(frameC, tipText=tipText, grid=(0, 2), text="Chains with this sequence:")
        ...
        tipTexts = [
            "The number of the residue in the sequence; separate to the chain (fragment) numbering which can be edited by the user",
        ...
        editWidgets = [None, None, self.descriptorPulldown, None, self.linkingPulldown, None]
        editGetCallbacks = [None, None, self.getDescriptor, None, self.getLinking, self.showResLinks]
        editSetCallbacks = [None, None, self.setDescriptor, None, self.setLinking, None]

        self.molResidueMatrix = ScrolledMatrix(
            frameC,
            grid=(1, 0),
            gridSpan=(1, 3),
            tipTexts=tipTexts,
        ...
        tipTexts = [
            "Add a bio-polymer sequence to construct a new molecular template, from which chains may be constructed",
        ...
        texts = ["Add Polymer", "Copy Sequence", "Add Compound", "Edit Links", "Unlock Molecule"]

        commands = [
            self.addResPolymer,
            self.copyResSequence,
            self.addResCompound,
            self.showResLinks,
            self.toggleMoleculeLock,
        ]

        self.molResidueButtons = ButtonList(
            frameC, texts=texts, tipTexts=tipTexts, commands=commands, grid=(2, 0), gridSpan=(1, 3)
        )
        ...
        self.longNameEntry = Entry(self, text="", width=16, returnCallback=self.setLongName)
        self.molDetailsEntry = Entry(self, text="", width=16, returnCallback=self.setMolDetails)
        self.seqMolDetailsEntry = Entry(self, text="", width=16, returnCallback=self.setSeqDetails)

        tipTexts = [
            "The short name for the molecule template, for use in graphical displays; set when the template is first created",
        ...
        colHeadings = ["Name", "Long Name", "Mol Type", "Chains", "Details", "Seq\nDetails"]
        #                'Common\nNames','Keywords','Functions',

        editWidgets = [None, self.longNameEntry, None, None, self.molDetailsEntry, self.seqMolDetailsEntry]
        editGetCallbacks = [None, self.getLongName, None, None, self.getMolDetails, self.getSeqDetails]
        editSetCallbacks = [None, self.setLongName, None, None, self.setMolDetails, self.setSeqDetails]

        self.moleculeMatrix = ScrolledMatrix(
            frameD,
            grid=(0, 0),
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Show a table containing the residue sequence of the selected molecule template",
        ...
        commands = [
            self.showMoleculeSequence,
            self.addMolecule,
            self.copyMolecule,
            self.deleteMolecule,
            self.addMolPolymer,
            self.addMoleculeCompound,
        ]

        self.moleculeButtons = ButtonList(frameD, texts=texts, grid=(1, 0), tipTexts=tipTexts, commands=commands)
        ...
        tipTexts = [
            "Various characteristics and attributes of the selected molecule",
            "The value of the parameter described on each row",
        ]
        colHeadings = ["Attribute", "Value"]

        self.molDataMatrix = ScrolledMatrix(frameD, grid=(3, 0), headingList=colHeadings, tipTexts=tipTexts)
        ...
        self.linkMoleculePulldown = PulldownList(
            subFrame, tipText=tipText, grid=(0, 1), callback=self.changeLinkMolecule
        )
        ...
        self.linkCcpCodesPulldown = PulldownList(subFrame, tipText=tipText, grid=(0, 3), callback=self.setLinkCcpCode)
        ...
        self.linkResiduePulldown = PulldownList(subFrame, tipText=tipText, grid=(0, 5), callback=self.setLinkMolResidue)

        colHeadings = ["Source's\nLink", "Destination\n Residue", "Destination's\nLink"]

        tipTexts = [
            'A name for the kind of link relative to the selected "source" residue; the one which all links emanate from',
        ...
        self.molResLinkMatrix = ScrolledMatrix(
            frameE,
            tipTexts=tipTexts,
            headingList=colHeadings,
        ...
        div = LabelDivider(frameE, text="Inter Molecule Links", tipText=tipText, grid=(3, 0))
        ...
        self.polymerMolTypePulldown = PulldownList(
            frame,
            texts=texts,
            grid=(0, 1),
        ...
        self.polymerStartEntry = IntEntry(
            frame, text=self.seqStartNum, grid=(0, 3), width=5, tipText=tipText, returnCallback=self.tidyPolymerSeq
        )
        ...
        self.cyclicSelect = CheckButton(frame, grid=(0, 5), tipText=tipText, selected=False)

        label = Label(frame, text="Input Type:", grid=(0, 6))
        tipTexts = [
            'The sequence will be specified in one-lette code form like "GCAT" or "TEAANDCAKE"',
            'The sequence will be specified in three-letter (or full CCPN) code form like "ALA GLY MET SER"',
        ]
        self.seqCodeModeButtons = RadioButtons(
            frame,
            RES_CODE_MODES,
            self.changeCodeMode,
            selected_index=0,
        ...
        self.seqMoleculePulldown = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changeSeqMolecule)
        ...
        self.seqMolSystemPulldown = PulldownList(frame, grid=(0, 3), tipText=tipText, callback=self.changeSeqMolSystem)
        ...
        self.ccpCodeSeqPulldown = PulldownList(
            frame, texts=ccpCodes, grid=(0, 5), callback=self.setPolymerSeqCcpCode, sticky="e", tipText=tipText
        )
        ...
        self.seqTextBox = ScrolledText(
            frameF, width=60, height=10, grid=(3, 0), text=self.inputSeq, xscroll=False, tipText=tipText
        )

        tipTexts = [
            "Add the states sequence to the selected, or new, molecule making a new chain if a molecular system is specified",
        ...
        self.polymerSeqButtons = ButtonList(frameF, texts=texts, tipTexts=tipTexts, commands=commands, grid=(4, 0))
        ...
        self.molTypePulldown = PulldownList(
            frameG,
            texts=texts,
            tipText=tipText,
        ...
        tipTexts = [
            "Import a non-standard compound definition from a CCPN ChemComp XML file, e.g. something made by CcpNmr ChemBuild",
        ]
        texts = [
            "Import XML File",
        ]
        ...
        buttons = ButtonList(frameG, texts=texts, commands=commands, grid=(0, 3), tipTexts=tipTexts)
        ...
        self.compoundMatrix = ScrolledMatrix(
            frameG,
            tipTexts=tipTexts,
        ...
        self.compMoleculePulldown = PulldownList(frame, tipText=tipText, grid=(0, 1), callback=self.changeCompMolecule)

        tipTexts = [
            "Add the selected chemical compound to the above selected, or new, molecule template; when done the template can be used to make an assignable chain",
        ]
        texts = [
            "Add Compound",
        ]
        ...
        buttons = ButtonList(frameG, texts=texts, commands=commands, gridSpan=(1, 3), grid=(3, 1), tipTexts=tipTexts)
        ...
        self.bottomButtons = UtilityButtonList(tabbedFrame.sideFrame, helpUrl=self.help_url, grid=(0, 0), sticky="e")
    ...
    def _chooseChemCompCoord(self, obj, row, col):
        ...
        for sourceName in SOURCE_NAMES:
            chemCompCoord = self.project.findFirstChemCompCoord(sourceName=sourceName, molType=molType, ccpCode=ccpCode)
    ...
    def chooseChemComp(self, obj, row, col):
        ...
        if chemComp:
            chemCompVar = (
                chemComp.findFirstChemCompVar(linking="middle", descriptor="neutral")
    ...
    def updateMolResidues(self):
        ...
        if self.molecule:
            ...
            if not self.molecule.isFinalised:
                editWidgets = [None, self.ccpCodePulldown, self.descriptorPulldown, None, self.linkingPulldown, None]
                editGetCallbacks = [None, self.getCcpCode, self.getDescriptor, None, self.getLinking, self.showResLinks]
                editSetCallbacks = [None, self.setCcpCode, self.setDescriptor, None, self.setLinking, None]

        self.molResChainInfoLabel.set(chainText)

        self.molResidueMatrix.update(
    ...
    def updateMolecules(self):
        ...
        self.moleculeMatrix.update(objectList=objectList, textMatrix=textMatrix)
    ...
    def updateCompoundTable(self):
        ...
        for molType in self.molTypes:
            ...
            for ccpCode in ccpCodes:
                ...
                name = re.sub(r"(.{50}\w+\W)", r"\1\n", name)
                textMatrix.append([ccpCode, code3Letter, name])
                objects.append((ccpCode, molType))
                colorMatrix.append(colors)

        self.compoundMatrix.update(objectList=objects, textMatrix=textMatrix, colorMatrix=colorMatrix)
    ...
    def readFile(self):
        ...
        fileSelectPopup = FileSelectPopup(
            self, title="Read sequence file", dismiss_text="Cancel", selected_file_must_exist=True
        )
            ...
            if start != initial:
                ...
                for i in range(start, end + 1):
                    ...
                    if testResidue and (testResidue not in residues):
                        showWarning(
                            "Failure", "This numbering would overlap with residues from other fragments.", parent=self
                        )
        ...
        if molResidue:
            ...
            if descriptor != molResidue.chemCompVar.descriptor:
                chemCompVar = chemComp.findFirstChemCompVar(
                    descriptor=descriptor, linking=molResidue.chemCompVar.linking
                )
                if not chemCompVar:
        ...
        if self.linkMolResidue and self.linkMolecule:
            ...
            if (linkEnd is None) and selLinkEnd and selLinkEnd.molResLink:
                ...
                if selLinkEnd and selLinkEnd.molResLink:
                    if selLinkEnd.molResLink.findFirstMolResLinkEnd(molResidue=molResidue, linkCode=linkCode):
                        return
    ...
    def commitPolymerSeq(self):
        ...
        addMolResidues(molecule, molType, sequence, startNum=startNum, isCyclic=isCyclic)
