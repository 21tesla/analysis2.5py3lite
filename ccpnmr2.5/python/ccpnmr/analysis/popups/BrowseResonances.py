...
from ccpnmr.analysis.core.AssignmentBasic import (
    ...
    assignResonanceType,
    clearResonancePeakDimContribs,
    deassignResonance,
    findResonanceSet,
    getOnebondResonances,
    getResidueResonances,
    getResonanceAtomTuple,
    getResonanceResidue,
    getShiftLists,
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        if not self.resonances:
            ...
            tipText = (
                "Selects which chemical shift list to display resonances for, and hence which\n"
            ...
            self.isotopePulldown = PulldownList(frame, self.changeIsotope, ISOTOPE_NAMES, grid=(0, 3), tipText=tipText)
            ...
            self.statusPulldown = PulldownList(frame, self.changeStatus, statusNames, grid=(0, 5), tipText=tipText)
        ...
        div = LabelDivider(frame, text="Navigation & Marks", tipText=tipText, grid=(0, 0))
        ...
        self.stripButton = Button(frame, text="Strip Selected", tipText=tipText, command=self.stripSelected, bd=1)
        ...
        self.gotoButton = Button(frame, text="Go To Position", tipText=tipText, command=self.gotoPosition, bd=1)
        ...
        self.windowPulldown = PulldownList(frame, callback=self.changeWindow, grid=(0, 4), tipText=tipText)

        tipText = "Put multi-dimensional cross marks at the selected resonance positions"
        self.markButton = Button(
            frame, text="Mark Selected", grid=(0, 6), command=self.markSelected, bd=1, sticky="ew", tipText=tipText
        )

        tipText = "Remove the resonance-located cross marks"
        self.clearMarksButton = Button(
            frame,
            text="Clear Marks",
            grid=(
                0,
        ...
        div = LabelDivider(frame, text="Strip Residue Resonances", tipText=tipText, grid=(0, 0))
        ...
        self.stripResButton = Button(
            frame,
            text="Make Residue Strips",
            grid=(1, 0),
            gridSpan=(1, 2),
            command=self.stripResidueResonances,
            bd=1,
            sticky="ew",
            tipText=tipText,
        )
        ...
        self.markResButton = Button(
            frame,
            text="Mark Residue Resonances",
            grid=(1, 2),
            gridSpan=(1, 2),
            command=self.markResidueResonances,
            bd=1,
            sticky="ew",
            tipText=tipText,
        )
        ...
        self.clearMarksButton = Button(
            frame,
            text="Clear Residue Marks",
            grid=(
                1,
                4,
            ),
            gridSpan=(1, 2),
            command=self.clearResidueMarks,
            bd=1,
            sticky="w",
            tipText=tipText,
        )
        ...
        div = LabelDivider(guiFrame, text="Resonance Table", tipText=tipText, grid=(row, 0))
        ...
        div = LabelDivider(guiFrame, text="Atom Assignment", grid=(row, 0), tipText=tipText)

        row += 1
        tipTexts = [
            "Remove any atomic assignments from the selected resonances",
        ...
        buttonList = ButtonList(
            guiFrame, texts=texts, grid=(row, 0), tipTexts=tipTexts, commands=commands, expands=True, gridSpan=(1, 10)
        )
        ...
        div = LabelDivider(guiFrame, text="General & Information", grid=(row, 0), tipText=tipText)

        row += 1
        tipTexts = [
            "Delete the selected resonances form the CCPN project",
        ...
        commands = [
            ...
            self.deleteOrphans,
            self.mergeOrSplitResonances,
            self.showPeaks,
            self.showShiftListPeaks,
            self.showInfo,
        ]
        self.bottomButtons = ButtonList(
            guiFrame, texts=texts, commands=commands, grid=(row, 0), gridSpan=(1, 10), tipTexts=tipTexts
        )
    ...
    def updateResWindows(self, *windowPane):
        ...
        for windowPanes, windowPulldown in (
            (windowPanesND, self.windowRes1Pulldown),
    ...
    def stripSelected(self):
        ...
        if self.resonance and self.windowPane and self.shift:
            ...
            displayStrips(self.parent, positions, windowPane=self.windowPane)
            ...
            for i, resonance in enumerate(stripResonances):
                ...
                for resonance2 in resonances2:
                    gotoResonancePosition(self.guiParent, resonance2, self.windowPane, shiftList)

                gotoResonancePosition(self.guiParent, resonance, self.windowPane, shiftList)
    ...
    def gotoPosition(self):
        ...
        if self.resonance and self.windowPane and self.shift:
            shiftList = self.shift.parentList
            gotoResonancePosition(self.guiParent, self.resonance, self.windowPane, shiftList)

            bound = list(self.resonance.covalentlyBound) or []
            if len(bound) == 1:
                gotoResonancePosition(self.guiParent, bound[0], self.windowPane, shiftList)
    ...
    def gotoResonancesPosition(self, windowPane, resonances, **rowColDict):
        ...
        if self.shift:
            ...
            for resonance in resonances:
                gotoResonancePosition(self.guiParent, resonance, windowPane, shiftList, **rowColDict)
    ...
    def updateAfter(self, obj=None):
        ...
        funcs = {
            "Resonance": self.updateResonanceAfter,
    ...
    def update(self, resonances=None, shiftList=None):
        ...
        for i, resonance in enumerate(resonances):
            ...
            colorMatrixAppend([color, None, None, None, None, None, None, None, None])
        ...
        self.scrolledMatrix.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
        self.refresh = False
    ...
    def checkSpinSystem(self, resonance, atomSet):
        ...
        if spinSystem:
            ...
            if spinSystem.residue is not residue:
                ...
                data = (makeResonanceGuiName(resonance), residue.seqCode, getResidueCode(residue))
...
class ResonanceInfoPopup(BasePopup):
    ...
    **Detailed Information About a Resonance's Peak Assignments**
    ...
    def body(self, guiFrame):
        ...
        self.mainLabel = Label(guiFrame, text="", grid=(0, 0), tipText=tipText)

        tipTexts = [
            "For the peak assignments selected in the upper table show a table of the full peak records",
        ]
        ...
        self.detailsEntry = Entry(
            detailsFrame, text=text, returnCallback=self.updateResonanceDetails, grid=(0, 1), sticky="ew"
        )

        tipText = "A table of the peak dimensions, and hence positions, to which the current resonance is assigned"
        div = LabelDivider(guiFrame, text="Peak Assignments", gridSpan=(1, 2), grid=(2, 0), tipText=tipText)

        tipTexts = [
            "The peak list of the peak to which the current resonance is assigned (experiment:spectrum:list number)",
        ...
        self.peakDimTable = ScrolledMatrix(
            guiFrame,
            grid=(3, 0),
        ...
        div = LabelDivider(guiFrame, text="Chemical Shifts", gridSpan=(1, 2), grid=(4, 0), tipText=tipText)

        headingList = ["Shift List", "Shift", "SD"]
        tipTexts = [
            "The shift list that records a chemical shift value for the resonance",
    ...
    def update(self, resonance=None):
        ...
        self.peakDimTable.update(textMatrix=textMatrix, objectList=peakDimContribs)
        ...
        if self.resonance:
            ...
            for shift in shifts:
                shiftList = shift.parentList
                datum = ["%s:%d" % (shiftList.name, shiftList.serial), shift.value, shift.error]

                textMatrix.append(datum)

        self.shiftTable.update(textMatrix=textMatrix, objectList=shifts)
