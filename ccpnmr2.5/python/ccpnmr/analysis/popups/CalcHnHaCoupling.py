...
from math import acos, atan, cos, exp, radians, sqrt
...
from ccpnmr.analysis.core.ConstraintBasic import getFixedResonance, makeNmrConstraintStore
from ccpnmr.analysis.core.CouplingBasic import setResidueJCoupling
...
from ccpnmr.analysis.core.MoleculeBasic import getLinkedResidue, getRandomCoilShift
...
ATOM_NAME_DICT = {"H[{N,HA}]": ("H", "N", "CA", "HA"), "H{[N]+[HA]}": ("H", "N", "CA", "HA")}
...
class CalcHnHaCouplingPopup(BasePopup):
    ...
    *G. W. Vuister and A. Bax (1993). "Quantitative J correlation: a new approach
    ...
    def body(self, guiFrame):
        ...
        self.intensityPulldown = PulldownList(
            frame, texts=["height", "volume"], callback=self.changeIntensity, tipText=tipText
        )
        ...
        self.karplusAEntry = FloatEntry(frame, text=6.51, width=6, tipText=tipText, returnCallback=self.updateAfter)
        ...
        self.karplusBEntry = FloatEntry(frame, text=-1.76, width=6, tipText=tipText, returnCallback=self.updateAfter)
        ...
        self.karplusCEntry = FloatEntry(frame, text=1.60, width=6, tipText=tipText, returnCallback=self.updateAfter)
        ...
        self.angleErrorEntry = FloatEntry(frame, text=30.0, width=6, returnCallback=None, tipText=tipText)
        ...
        self.karplusCurve = ScrolledGraph(
            frameA,
            title="Karplus Curve",
            width=400,
            height=200,
            xLabel="Angle (degrees)",
            yLabel="3J Coupling (Hz)",
            grid=(row, 0),
            symbolSize=1,
            graphType="scatter",
        ...
        tipTexts = [
            "Assigned name of the root (amide) spin system",
        ...
        tipTexts = [
            "Assign amide H and alpha H resonances to the indirect 1H peak dimensions",
        ...
        commands = [
            self.assignNonRootDim,
            self.makeJCouplingList,
            self.makeAngleConstraints,
            self.makeCouplingConstraints,
            self.updateAfter,
        ]
        self.tableButtons = ButtonList(frameB, texts=texts, commands=commands, tipTexts=tipTexts, grid=(1, 0))
    ...
    def getJCouplingLists(self):
        ...
        jCouplingLists = [
            None,
        ] + list(self.nmrProject.findAllMeasurementLists(className="JCouplingList"))
    ...
    def getConstraintSets(self):
        ...
        constraintSets = [
            None,
        ] + self.nmrProject.sortedNmrConstraintStores()
    ...
    def makeCouplingConstraints(self):
        ...
        constraintList = self.constraintSet.newJCouplingConstraintList(
            name=name, unit="Hz", experimentSerials=(eSerial,), measureListSerials=(mSerial,)
        )
        ...
        for jCoupling in self.jCouplingList.measurements:
            ...
            constraint = constraintList.newJCouplingConstraint(
                targetValue=targetValue, upperLimit=upperLimit, lowerLimit=lowerLimit, error=error
            )
            ...
            for peak in jCoupling.peaks:
                peakContrib = constraint.newConstraintPeakContrib(
                    experimentSerial=eSerial, dataSourceSerial=sSerial, peakListSerial=pSerial, peakSerial=peak.serial
                )
    ...
    def makeAngleConstraints(self):
        ...
        constraintList = self.constraintSet.newDihedralConstraintList(
            name=name, unit="degrees", experimentSerials=(eSerial,)
        )
        ...
        for spinSystem, peaks, couplingValue, couplingError, phis in self.spinSystemMatrix.objectList:
            ...
            constraint = constraintList.newDihedralConstraint(resonances=fixedResonances, origData=couplingValue)
            ...
            for phi in phis:
                ...
                for i in range(n):
                    ...
                    if ((phiMin >= phiMin2) and (phiMin <= phiMax2)) or ((phiMax >= phiMin2) and (phiMax <= phiMax2)):
            ...
            for phiMin, phi, phiMax in phiRanges:
                constraint.newDihedralConstraintItem(targetValue=phi, upperLimit=phiMax, lowerLimit=phiMin)
    ...
    def update(self, recalc=True):
        ...
        self.spinSystemMatrix.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
        self.updateButtons()
        self.waiting = False
    ...
    def extractHnHaCouplingData(self):
        ...
        for spinSystem in ssPeaks:
            ...
            sigmaSq = (
                ...
                * noise
                * (1 + (ratio * ratio))
                / (-1 * f * ratio * heightA * heightA * (1 - ratio) * (1 - ratio))
            )
        ...
        if invalid:
            ...
            if self.inBody:
                ...
                invalidReasons.insert(0, msg1)
                error = "\n".join(invalidReasons)
                showWarning("Warning", error, parent=self)
