...
from ccpnmr.analysis.core.AssignmentBasic import getPeakDimResonances, getShiftLists
from ccpnmr.analysis.core.ConstraintBasic import (
    findPeakConstraintPairs,
    getDistMinMax,
    getNoeDistance,
    makeAmbigDistConstraints,
    makeDistConstraints,
)
from ccpnmr.analysis.core.ExperimentBasic import (
    getCompatibleHsqcPeakLists,
    getDiagonalBondedDataDims,
    getOnebondDataDims,
    getSpectrumIsotopes,
...
from ccpnmr.analysis.core.Util import (
    getAnalysisDataDim,
    getSpectrumNoesResidueRanges,
    getSpectrumNoesShiftRanges,
    setSpectrumNoeResidueRanges,
    setSpectrumNoeShiftRanges,
)
...
from memops.gui.MessageReporter import showMulti, showOkCancel, showWarning
...
GROUP_TIP_TEXTS = {
    "Matched": tip
    + "the peaks that were successfully matched to chemical shifts, and which would result in a restraint",
    "Assigned": tip
    + "the peaks that were already assigned to specific resonances and so are not included for shift match restraint generation",
    "Diagonal": tip + "the peaks that were excluded because they were too close to the homonuclear diagonal",
    "Out of range": tip
    + "the peaks that were excluded because they only matched assignments were not in the stated residue range ",
    "Poor Merit": tip
    + "the peaks that were excluded because they have a figure-of-merit value below the stated threshold",
    "Too Distal": tip
    + "the peaks that were excluded because they only match assignments that are too far apart given the stated guide structure",
    "Unmatchable": tip
    + "the peaks that were excluded because they do not match any chemical shifts in the stated ranges",
}
...
class CalcDistConstraintsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        self.peakListPulldown = PulldownList(frame, grid=(0, 1), tipText=tipText, callback=self.changePeakList)
        ...
        self.includeSimulatedSelect = CheckButton(frame, grid=(0, 1), tipText=tipText, callback=self.updatePeakLists)
        ...
        self.constraintSetPulldown = PulldownList(
            optFrame, grid=(0, 1), tipText=tipText, callback=self.changeConstraintSet
        )
        ...
        self.labellingSchemePulldown = PulldownList(
            optFrame, grid=(0, 3), tipText=tipText, callback=self.changeLabellingScheme
        )

        tipText = (
            "Threshold for peak figure-of-merit value, below which peaks will not be considered for making restraints"
        )
        label = Label(optFrame, text="Minimum\nPeak Merit: ", grid=(1, 0))
        self.minMeritEntry = FloatEntry(
            optFrame, text="1.0", grid=(1, 1), tipText=tipText, returnCallback=self.setMinPeakMerit, width=8
        )
        ...
        self.structurePulldown = PulldownList(optFrame, grid=(0, 5), tipText=tipText, callback=self.changeStructure)
        ...
        self.noeFrame = NoeDistParamsFrame(
            optFrame,
            self.nmrProject,
        ...
        label = Label(
            peakFrame,
            text="Table below allows you to specify alternatives to ref intensity (cells highlighted in green are used)",
            grid=(0, 0),
            gridSpan=(1, 2),
        )

        tipTexts = (
            "Always use the ref intensity no matter what the table below says",
            "Use the setting for each peak as specified in the table below",
        )
        self.intensityChoiceButton = RadioButtons(
            peakFrame, entries=(USE_REF_INTENSITY, USE_TABLE_INTENSITY), tipTexts=tipTexts, grid=(1, 0)
        )

        tipTexts = [
            "Update the table (but most changes should be recognised automatically)",
        ...
        self.hsqcPeakListPulldown = PulldownList(frame2, grid=(0, 1), tipText=tipText, callback=self.changeHsqcPeakList)
        ...
        self.peaksMatrix = ScrolledMatrix(
            peakFrame,
            multiSelect=True,
        ...
        self.molSysPulldown = PulldownList(resFrame, callback=self.changeMolSystem, grid=(0, 1), tipText=tipText)

        tipTexts = [
            "Which spectrum dimensions the residue range filter applies to",
        ...
        self.rangeMatrix = ScrolledMatrix(
            resFrame,
            headingList=headingList,
            callback=self.selectResidueRange,
            initialRows=2,
            editWidgets=editWidgets,
            tipTexts=tipTexts,
            editSetCallbacks=editSetCallbacks,
        ...
        tipTexts = ["Add a new allowable residue range to the table", "Remove the currently selected residue range"]
        ...
        self.copySpecButton = Button(
            tolFrame, text="Copy From:", tipText=tipText, command=self.copySpecTols, bd=1, grid=(0, 5)
        )
        self.copySpecPulldown = PulldownList(tolFrame, callback=None, tipText=tipText, grid=(0, 6))

        tipTexts = [
            "The spectrum dimension that the tolerance applies to",
        ...
        self.toleranceMatrix = ScrolledMatrix(
            tolFrame,
            headingList=headingList,
            editWidgets=editWidgets,
            tipTexts=tipTexts,
        ...
        tipTexts = [
            "The spectrum dimension to which an allowed chemical shift range applies",
        ...
        self.shiftsMatrix = ScrolledMatrix(
            shiftFrame,
            headingList=headingList,
        ...
        tipTexts = [
            "Add a new chemical shift range for matching peak positions to chemical shifts",
        ...
        tipText = (
            "Whether to assign the peaks of the input peak lists according to the filtered assignment possibilities"
        )
        ...
        tipText = (
            "Selects which kinds of NOE network information can filter assignment options;\n"
        ...
        entries = ["Low: Any residue support", "Normal: Sequential residue support", "High: Same residue support"]
        self.strictnessPulldown = PulldownList(
            frame, texts=entries, grid=(0, 5), callback=None, index=1, tipText=tipText
        )
        ...
        self.tolHydrogenEntry = FloatEntry(subFrame, text=TOLERANCE_DICT["1H"], grid=(0, 1), tipText=tipText, width=7)
        ...
        self.tolNitrogenEntry = FloatEntry(subFrame, text=TOLERANCE_DICT["15N"], tipText=tipText, grid=(0, 3), width=7)
        ...
        self.tolCarbonEntry = FloatEntry(subFrame, text=TOLERANCE_DICT["13C"], grid=(0, 5), tipText=tipText, width=7)

        tipText = "Selects which list of chemical shifts to compare to peak positions"
        label = Label(subFrame, text="Shift List", grid=(0, 6))
        self.shiftListPulldown = PulldownList(subFrame, tipText=tipText, grid=(0, 7), callback=self.changeNetShiftList)
        ...
        tipTexts = [
            "The experiment: spectrum of the input peak list",
        ...
        self.networkPeakListsMatrix = ScrolledMatrix(
            netFrame,
            headingList=headingList,
        ...
        tipTexts = [
            "Make restraints using the assigned peaks in the selected peak list, according to the settings",
        ...
        commands = [self.calcNormRestraints, self.calcShiftMatchRestraints, self.testShiftMatch, self.runAnchoring]
        self.bottomButtons = ButtonList(
            guiFrame, commands=commands, texts=texts, grid=(2, 0), gridSpan=(1, 3), tipTexts=tipTexts
        )
    ...
    def updateNetPeakLists(self, obj=None):
        ...
        self.networkPeakListsMatrix.update(colorMatrix=colorMatrix, objectList=objectList, textMatrix=textMatrix)
    ...
    def getResidueRanges(self):
        ...
        if not self.residueRanges:
            ...
            if residueRanges:
                ...
                self.residueRanges = [
                    ResidueRowObject(dataDims, chain, startRes, endRes)
                    for (dataDims, chain, startRes, endRes) in residueRanges
                ]
    ...
    def getTolerances(self):
        ...
        if not self.tolerances:
            ...
            if self.spectrum:
                ...
                self.tolerances = [
                    ToleranceRowObject(dataDimInfo, minTol, multiplier, maxTol)
                    for (dataDimInfo, minTol, multiplier, maxTol) in tolerances
                ]
    ...
    def getShiftRanges(self):
        ...
        if not self.chemShiftRanges:
            ...
            if self.spectrum:
                ...
                self.chemShiftRanges = [
                    ShiftRowObject(dataDim, isotopes, start, end) for (dataDim, isotopes, start, end) in chemShiftRanges
                ]
    ...
    def testShiftMatch(self):
        ...
        if self.spectrum and self.molSystem:
            ...
            constraintList = makeAmbigDistConstraints(
                self.peakList,
                tolerances,
                chemShiftRanges,
                testOnly=True,
    ...
    def checkPeakListIntensities(self, peakList, intensityType):
        ...
        return showOkCancel(
            "Warning",
            "%d peaks from %d in the peak list are missing a %s value. Continue?" % (c, N, intensityType),
            parent=self,
        )
    ...
    def calculateConstraints(self, doShiftMatch=False):
        ...
        if self.spectrum and self.molSystem and self.peakList:
            ...
            if doShiftMatch:
                ...
                constraintList = makeDistConstraints(
                    self.peakList,
                    constraintSet=self.constraintSet,
                    labelling=self.labellingScheme,
                    intensityType=intensityType,
    ...
    def updateLabellingSchemes(self, obj=None):
        ...
        schemes = [
            True,
            None,
        ] + self.project.sortedLabelingSchemes()
        scheme = self.labellingScheme
        names = [
            "Automatic from sample",
            "<None>",
        ] + [sc.name for sc in schemes[2:]]
    ...
    def runAnchoring(self):
        ...
        if peakLists:
            ...
            constraintList = networkAnchorAssign(
                peakLists,
                intensityType=intensityType,
                ...
                minLabelFraction=minLabelFrac,
                threshold=threshold,
                isotopeTolerances=TOLERANCE_DICT,
    ...
    def findPeakDistance(self, peak, intensityScale):
        ...
        (dist, minDist, maxDist) = getDistMinMax(
            intensityValue, intensityScale, resonances0, resonances1, distanceFunction
        )
    ...
    def updatePeakFrameMatrix(self, *extra):
        ...
        self.peaksMatrix.update(textMatrix=textMatrix, objectList=peaks, colorMatrix=colorMatrix)
