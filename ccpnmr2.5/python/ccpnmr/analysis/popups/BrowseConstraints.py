...
from ccpnmr.analysis.core.ConstraintBasic import (
    exportAriaTbl,
    getConstraintStoreResonances,
    getStructureViolations,
    makePeaksFromConstraints,
    mergeConstraintLists,
    mergeDuplicateConstraints,
    splitConstraintListAmbigUnambig,
    splitConstraintListViol,
    updateDistConstraintFromPeakAssign,
    updatePeaksFromConstraints,
)
from ccpnmr.analysis.core.StructureBasic import getAtomSetsDihedral, getAtomSetsDistance
...
class BrowseConstraintsPopup(BasePopup):
    ...
    **Caveats & Tips**
    ...
    def body(self, guiFrame):
        ...
        tipText = (
            "Which set of restraints is currently active in the popup;\n"
        ...
        self.constraintSetPulldown = PulldownList(
            frame, tipText=tipText, grid=(0, 1), callback=self.changeConstraintSet
        )

        utilButtons = UtilityButtonList(frame, grid=(0, 2), sticky="e", helpUrl=self.help_url)

        options = ["Restraint Sets", "Restraint Lists", "Restraints", "Violations Lists", "Violations"]
        self.tabbedFrame = TabbedFrame(guiFrame, options=options, grid=(1, 0), callback=self.updateTabbedFrame)
        ...
        self.constraintSetMatrix = ScrolledMatrix(
            frameA,
            multiSelect=True,
            grid=(0, 0),
        ...
        tipTexts = [
            "**Disabled option**, import restraints from file. Use FormatConverter",
            "Delete the selected restraint set with all its restraints and violations",
        ]
        texts = ["Import", "Delete"]
        commands = [None, self.deleteRestraintSet]
        self.constraintSetButtons = ButtonList(
            frameA, texts=texts, tipTexts=tipTexts, expands=True, commands=commands, grid=(1, 0)
        )
        ...
        self.constraintListMatrix = ScrolledMatrix(
            frameB,
            multiSelect=True,
        ...
        tipTexts = [
            "Show a table of individual restraints for the selected restraint list",
        ...
        texts = ["Show\nRestraints", "Export\nARIA list", "Create\nEquiv. Peaks", "Delete\nSelected Lists"]
        commands = [self.viewConstraints, self.exportAriaList, self.makeRestraintPeaks, self.deleteRestraintList]
        self.restraintListButtons = ButtonList(frameB, texts=texts, grid=(1, 0), tipTexts=tipTexts, commands=commands)
        # self.restraintListButtons.buttons[1].disable()

        tipTexts = [
            "Merge the selected restraint lists into a single list (if the same type)",
        ...
        commands = [self.mergeRestraintLists, self.splitRestraintListAmbig, self.splitRestraintListViol]
        self.restraintListButtons2 = ButtonList(frameB, texts=texts, tipTexts=tipTexts, commands=commands, grid=(2, 0))
        ...
        self.constraintListPulldown = PulldownList(
            frameC, grid=(0, 1), tipText=tipText, callback=self.changeRestraintList
        )
        ...
        self.violationListPulldownA = PulldownList(
            frameC, tipText=tipText, grid=(0, 3), callback=self.changeViolationListA
        )

        tipText = "Select which loaded structure to use for restraint value comparison"
        label = Label(frameC, text=" Structure:", grid=(0, 4))
        self.structurePulldown = PulldownList(frameC, callback=self.changeStructure, grid=(0, 5), tipText=tipText)

        tipText = (
            'Select how values are generated from a structure: "NOE sum" is used where\n'
            + "ambiguous distance restraint options all contribute NOE peak intensity"
        )
        label = Label(frameC, text=" Value Method:", grid=(0, 6))
        self.valueMethodPulldown = PulldownList(frameC, callback=self.setValueMethod, grid=(0, 7), tipText=tipText)

        frameC.grid_rowconfigure(1, weight=1)

        self.detailsRestraintEntry = Entry(self, text="", width=12, returnCallback=self.setRestraintDetails)
        ...
        self.constraintsMatrix = ScrolledMatrix(
            frameC,
            multiSelect=True,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Delete the selected restraint items; assignment possibility not necessarily whole restraint",
        ...
        self.restraintButtons = ButtonList(
            frameC, tipTexts=tipTexts, texts=texts, commands=commands, grid=(2, 0), gridSpan=(1, 8)
        )

        tipTexts = [
            "Show a table of individual violation records for selected list",
        ...
        commands = [
            self.viewViolationList,
            self.calcViolations,
            self.exportAriaList,
            self.deleteRestraintListSingle,
            self.setupResonances,
            self.mergeDuplicates,
            self.updatePeakAssign,
        ]

        self.restraintButtons2 = ButtonList(
            frameC, commands=commands, grid=(3, 0), texts=texts, tipTexts=tipTexts, gridSpan=(1, 8)
        )
        ...
        self.violationListMatrix = ScrolledMatrix(
            frameD,
            multiSelect=True,
            headingList=colHeadings,
            grid=(0, 0),
        ...
        tipTexts = [
            "Show a table of individual violation entries for the selected list",
        ...
        self.violationListButtons = ButtonList(frameD, texts=texts, tipTexts=tipTexts, grid=(1, 0), commands=commands)
        ...
        self.violationListPulldownB = PulldownList(
            frameE1, grid=(0, 1), tipText=tipText, callback=self.changeViolationListB
        )

        tipTexts = [
            "The restraint list that the violations were calculated for",
        ...
        self.violationsMatrix = ScrolledMatrix(
            frameE, tipTexts=tipTexts, headingList=colHeadings, callback=self.selectViolationCell
        )
        self.violationsMatrix.grid(row=1, column=0, sticky="nsew")

        tipTexts = [
            "Assign any peak from which the violated restraint may be derived",
        ...
        commands = [self.assignViolationPeak, self.showViolationPeaks, self.showViolationRestraint]
        self.violationButtons = ButtonList(frameE, expands=True, texts=texts, commands=commands)
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        for func in (
            ...
            for clazz in (
                "ccp.nmr.NmrConstraint.ChemShiftConstraint",
        ...
        for func in ("__init__", "delete", "setError", "setLowerLimit", "setTargetValue", "setUpperLimit"):
        ...
        for func in ("__init__", "delete", "setResonances"):
            for clazz in (
                "ccp.nmr.NmrConstraint.DistanceConstraintItem",
    ...
    def updateTabbedFrame(self, index):
        ...
        # Updates when viewed
        funcDict = {
            0: self.updateConstraintSetsAfter,
    ...
    def updateConstraintSets(self):
        ...
        if self.tabbedFrame.selected == 0:
            ...
            self.constraintSetMatrix.update(objectList=objectList, textMatrix=textMatrix)
    ...
    def updateConstraintListTable(self, *opt):
        ...
        self.constraintListMatrix.update(objectList=objectList, textMatrix=textMatrix)
    ...
    def updateViolationLists(self):
        ...
        if self.constraintSet:
            ...
            violationListsA = violationListsC + [
                None,
            ]
    ...
    def updateViolationListTable(self):
        ...
        if constraintSet:
            for violationList in constraintSet.sortedViolationLists():
                ...
                datum = [violationList.serial, len(violationList.violations), strucText, violationList.details]

                objectList.append(violationList)
                textMatrix.append(datum)

        self.violationListMatrix.update(objectList=objectList, textMatrix=textMatrix)
    ...
    def updateRestraints(self):
            ...
            for constraint in constraints:
                ...
                if struct:
                    ...
                    if len(atomSets) == 4:
                        ...
                        strucVal = quickStrucDictGet(atomSets) or getAtomSetsDihedral(atomSets, struct)
                ...
                for item in items:
                    ...
                    if struct:
                        ...
                        for i, resonance in enumerate(resonances):
                            ...
                            atomSets = tuple(atomSets)
                            strucVal = quickStrucDictGet(atomSets) or getAtomSetsDistance(
                                atomSets[0], atomSets[1], struct, method=distMethod
                            )
                            quickStrucDict[atomSets] = strucVal
        ...
        self.constraintsMatrix.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
    ...
    def updateViolations(self):
        ...
        self.violationsMatrix.update(objectList=objectList, textMatrix=textMatrix)
    ...
    def updateStructures(self, structure=None):
        ...
        if self.tabbedFrame.selected == 2:
            ...
            names = [
                "<None>",
            ] + [str(x.ensembleId) for x in structures[1:]]
    ...
    def calcViolations(self):
        ...
        if self.constraintListB and self.structure:
            ...
            self.violationListA = getStructureViolations(self.constraintListB, self.structure, self.violationListA)
    ...
    def exportAriaList(self):
        ...
        fileSelectPopup = FileSelectPopup(
            self, file_types=fileTypes, title="Export ARIA file", dismiss_text="Cancel", selected_file_must_exist=False
        )
    ...
    def exportCnsList(self):
        ...
        fileSelectPopup = FileSelectPopup(
            self, file_types=fileTypes, title="Export CNS file", dismiss_text="Cancel", selected_file_must_exist=False
        )
