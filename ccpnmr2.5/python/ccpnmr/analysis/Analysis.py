...
from ccpnmr.analysis.core import (
    AssignmentBasic,
    CouplingBasic,
    ExperimentBasic,
    MarkBasic,
...
class Analysis:
    def __init__(self, cache_size):
        ...
        alternativeNames = ("CCPNMR_Analysis2", "CCPNMR_Analysis")  # previous names
        ...
        self.versionInfo = "%s %s Version %s.%s.%s Release %s (%s)" % (
            Copyright.suite,
            Copyright.program,
            version.major,
            version.minor,
    ...
    def updateTiedAxes(self):
        ...
        self.after_idle(
            lambda: self.registerNotify(self.updateTiedAxesAfter, "ccpnmr.Analysis.AxisRegion", "setRegion")
        )
        self.changedAxisPanel = None
    ...
    def setCPeakText(self, peak):
        ...
        if analysisProject.doMinimalAnnotations:
            ...
            text = PeakBasic.getPeakAnnotation(peak, noPeakDimAnnotationChar="", joinChar="", doPeakDims=False)
      ...
      """
            ...
            if self.argServer and self.argServer.inGui:
                ...
                if not analysisProject.findFirstChainMapping(chain=chain):
                    ...
                    progressBar = self.argServer.getProgressBar(text=text % data, total=len(chain.residues))
        ...
        if self.analysisProject.contourToUnaliased:
            ...
            for dataDim in spectrum.dataDims:
                if (dataDim.className == "FreqDataDim") and dataDim.dataDimRefs:
                    ...
                    valRange = [
                        unitConvert[("point", unit)](1.0, dataDimRef),
    ...
    def initBlockFile(self, spectrum, updateContourLevels=False):
        ...
        if ExperimentBasic.isShapeSpectrum(spectrum):
            ...
            block_file = getBlockFile(spectrum, mem_cache=self.mem_cache, writeable=writeable)
            if not block_file:
                return
        ...
        if updateContourLevels:
            Util.defaultContourLevels(spectrum, updateContourLevels=updateContourLevels)
            analysisSpectrum = spectrum.analysisSpectrum
            Util.updateSpectrumLevelParams(analysisSpectrum, analysisSpectrum.posLevels, analysisSpectrum.negLevels)
    ...
    def checkCreateSpectrumViews(self, spectrum):
        ...
        if not createdView:
            ...
            if None not in axisTypes:
                ...
                window = WindowBasic.createSpectrumWindow(
                    self.project,
                    name,
                    [
                        axisTypes,
    ...
    def createSpectrumView(self, windowPane, spectrum, axisPanelMapping):
        ...
        if isVisible == 1:  # This deliberate: Map spectrum to first window view only.
            isVisible = True
        ...
        view = Util.createSpectrumWindowView(
            windowPane,
            spectrum,
            axisPanelMapping,
            isPosVisible=isVisible,
            isNegVisible=isVisible,
            isInToolbar=isInToolbar,
            isSliceVisible=isVisible,
        )
        if view:
            self.changedViewAxisMapping(view)
    ...
    def initPeakList(self, peakList, initialising=False):
        ...
        if not Util.isTopObjectAnalysisInitialised(peakList.topObject):
            fitMethod = Util.getMethod(
                project, task="fit peak center", procedure="peak.cPeak.fitCenter", parameters=(("method", "parabolic"),)
            )
            manMethod = Util.getMethod(project, task="set peak intensity", procedure="manual")
            volMethod = Util.getMethod(
                project, task="fit peak volume", procedure="peak.cPeak.fitVolume", parameters=(("method", "gaussian3"),)
            )
            hytMethod = Util.getMethod(project, task="find peak height", procedure="peak.cPeak.getIntensity")
            ...
            for windowPane in window.spectrumWindowPanes:
                ...
                if view:  # exists only if spectrum is mapped onto window
                    ...
                    if windowPeakList:
                        ...
                        windowPeakList = view.newWindowPeakList(
                            analysisPeakList=analysisPeakList, isSymbolDrawn=isDrawn, isAnnotationDrawn=isDrawn
                        )
    ...
    def initSpectrumWindowView(self, view):
        ...
        for analysisPeakList in analysisSpectrum.analysisPeakLists:
            ...
            view.newWindowPeakList(analysisPeakList=analysisPeakList, isSymbolDrawn=isDrawn, isAnnotationDrawn=isDrawn)
    ...
    def changedPeakTextOffset(self, owner):
        ...
        appData = owner.findFirstApplicationData(application=self.application.name, keyword="textOffset")
        if appData:
            value = appData.value
        else:
            value = None
