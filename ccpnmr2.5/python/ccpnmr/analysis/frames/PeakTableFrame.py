...
from ccpnmr.analysis.core import PeakBasic, UnitConverter, WindowBasic
...
from memops.gui.MessageReporter import showError, showMulti, showOkCancel, showWarning, showYesNo
...
class PeakTableFrame(Frame):
    def __init__(self, parent, analysis, nmrProject=None, peakList=None, peaks=None, simplified=False, *args, **kw):
        ...
        texts = ["Any", "Fully Assigned", "Part Assigned", "Unassigned", "Intra Spin System"]
        #'Sequential', 'Within 4 Residues', '2 to 4 Residues', 'Long Range']
        self.statusPulldown = PulldownList(
            ssFrame,
            callback=self.changeStatus,
            texts=texts,
            index=0,
            grid=(0, 1),
        ...
        self.structPulldown = PulldownList(
            ssFrame, grid=(0, 3), callback=self.setStructure, tipText="Structure to calculate distances from"
        )
        ...
        if self.nmrProject:
            ...
            self.listPulldown = PulldownList(
                frame, callback=self.changeList, grid=(0, 1), tipText="Select peak list to display peaks from"
            )

            label = Label(frame, text=" Position Unit:", grid=(0, 2))
            self.unitPulldown = PulldownList(
                frame,
...
"""
        self.findButton = CheckButton(
            topFrame,
            text=" Find Peak ",
            borderwidth=1,
            padx=2,
            pady=1,
            callback=lambda isSelected: self.findPeak(),
            tipText="Locate the currently selected peak in the specified window",
        )
        self.windowLabel = Label(topFrame, text=" Window:", fg="grey")
        self.windowPulldown = PulldownList(
            topFrame, callback=self.selectWindowPane, tipText="Choose the spectrum window for locating peaks or strips"
        )
        self.markFoundLabel = Label(topFrame, text="Mark Found")
        self.markFoundCheck = CheckButton(
            topFrame, tipText="Whether to put a cross-mark though peaks found in a given window"
        )
        self.markFoundCheck.set(True)

        self.orthStripButton = Button(
            topFrame,
            borderwidth=1,
            padx=2,
            pady=1,
            text="Strip Locations",
            command=self.gotoOrthogStrips,
...
"""
        self.orthNavButton = CheckButton(
            topFrame,
            borderwidth=1,
            padx=2,
            pady=1,
            text="Go To Position",
            callback=lambda isSelected: self.gotoOrthogPlane(),
            tipText="Use the selected peak position to navigate to a location in a higher dimensionality window",
        )
        self.orthWinPulldown = PulldownList(
            topFrame,
            callback=None,
        ...
        self.scrolledMatrix = ScrolledMatrix(
            self, headingList=headingList, multiSelect=True, callback=self.selectCell, deleteFunc=self.peakRemove
        )
        ...
        tipTexts = [
            "Add a new peak, specifying its position",
        ...
        row += 1
        self.bottomButtons1 = ButtonList(self, texts=texts, commands=commands, tipTexts=tipTexts)
        ...
        row += 1
        self.bottomButtons2 = ButtonList(self, texts=texts, expands=True, commands=commands, tipTexts=tipTexts)
    ...
    def gotoOrthogStrips(self):
        ...
        if self.peak and location:
            ...
            if positions:
                WindowBasic.displayStrips(
                    self.analysisApp, positions, orthoPositions=None, spectrum=None, windowPane=windowPane
                )
    ...
    def recalcPeakFit(self):
        ...
        doFit = showYesNo(
            "Fit position?", "Should position be fit (otherwise only height and linewidth are)?", parent=self
        )
...
"""
            ...
            if notSimplified:
                ...
                datum.extend([peak.figOfMerit, peak.details])
    ...
    def getHeadings(self, n):
        ...
        if self.nmrProject:
            ...
            tipTexts = ["Experiment:spectrum of peak", "Peak list number", "Peak serial number"]
        ...
        tipTexts += [
            'Figure of merit value for peak; zero: "bad" one: "good"',
        ...
        if not self.simplified:
            ...
            tipTexts += ["Method used to fit the peak location", "Method used to calculate the peak volume integral"]
