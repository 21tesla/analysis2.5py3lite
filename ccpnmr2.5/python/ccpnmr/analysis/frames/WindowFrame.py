...
from memops.gui.Color import hexToRgb, hexXor, inverseGrey
...
from memops.gui.Scrollbar import bottom_right_trough_mode, top_left_trough_mode
from memops.gui.ScrolledWindow import ScrolledWindow, alt_key_state, ctrl_key_state, no_key_state, shift_key_state
from memops.universal.Region1D import Region1D
from memops.universal.Util import OSButton, formatDecimals, isMacOS, isWindowsOS, useWheelMouse
...
if not GlHandler and not TkHandler:
    print(
        "Error, the WindowFrame module will not work, something is wrong with both the GlHandler and TkHandler C code."
    )

from ccpnmr.analysis.core import Util
from ccpnmr.analysis.core.AssignmentBasic import (
    addPeakResonancesToSeqSpinSystems,
    addPeakResonancesToSpinSystem,
    clearSeqSpinSystemLinks,
    propagatePeakAssignments,
)
from ccpnmr.analysis.core.ExperimentBasic import getDataDimRefFullRange, getSpectrumIsotopes
from ccpnmr.analysis.core.MarkBasic import createNonPeakMark, createPeakMark, createRuler, removeMarks, removeRulers
from ccpnmr.analysis.core.PeakBasic import (
    PEAK_FIT_METHODS,
    addPeak,
    addPeakToSelected,
    copyPeaksToPeakList,
    ...
    fitPeaks,
    getPeakDimPosition,
    makeIntermediatePeak,
    makePeakAnnotation,
    propagatePeakUnaliasing,
    removePeakFromSelected,
    removePeaksAssignment,
    searchPeaks,
    setPeakPosition,
    snapPeaks,
    uniteResonancePeakPositions,
)
from ccpnmr.analysis.core.PeakFindParams import getPeakFindParams
from ccpnmr.analysis.core.WindowBasic import (
    VALUE_AXIS_OFFSET,
    X_AXIS_OFFSET,
    centerAxisRegion,
    ...
    displayPeakStrips,
    displayStrips,
    findOrthogonalWindows,
    findWindowTransposes,
    getDataDimAxisMapping,
    getEquivalentWindows,
    getPeakWindowPosition,
    getSpectrumWindowView,
    getWindowPaneName,
    isSpectrumInWindowPane,
    refinePeakLabelPositions,
    resetPeakLabelPositions,
    setWindowPeakListState,
    zoomToShowPeaks,
)
...
class WindowFrame(Frame, WindowDraw):
    ...
    def __init__(self, parent, windowPane):
        ...
        if not self.hasValueAxis:
            scrolledWindow.sliceMenuBind(
                button=OSButton(3), menu_items=self.slice_menu_items, update_func=self.updateSliceMenuState
            )
    ...
    def moveStripToWindow(self, sourceNum, windowPane=None, targetNum=0):
        ...
        if windowPane:
            ...
            window = createSpectrumWindow(
                self.windowPopup.project,
                name,
                [
                    axisTypes,
                ],
        ...
        for window in windows:
            for windowPane in window.sortedSpectrumWindowPanes():
                ...
                for i in range(len(axisRegions) + 1):
                    ...
                    item = {"kind": "command", "label": label, "shortcut": "%d" % (i + 1), "command": cmd}
                    items.append(item)
    ...
    def updateMenuState(self):
        ...
        if windowZplanes and currentPeaks:
            for planeName, pane, positions in windowZplanes:
                ...
                peak_strip_items.append({"kind": "command", "label": planeName, "state": state, "command": cmd})
        ...
        if len(windowPane.findFirstAxisPanel(label=stripAxis).axisRegions) > 1:
            ...
            if stripAxis == "x":
                strip_items = [
                    ...
                    {
                        "kind": "command",
                        "label": "Add horizontal separator",
                        "shortcut": "h",
        ...
        propsister_items = []
        if peak:
            label = "Assign %s:%s:%s:%s" % (
                peak.peakList.dataSource.experiment.name,
            ...
            for peakDim in peak.sortedPeakDims():
                    ...
                    for spectrum in experiment.sortedDataSources():
                        ...
                        for isotope in expDimRef.isotopeCodes:
                            if isotope in isotopes:
                                ...
                                spec_find_items.append(
                                    {
                                        "kind": "command",
                                        "label": specText,
                                        "tipText": "Search for peaks with a similar dimension %d position in this spectrum"
                                        % i,
                ...
                text = "F%d: %s %7.3f %s" % (
                    i,
                    isotopesCode,
                    getPeakDimPosition(peakDim, toUnit=expDimRef.unit),
                    expDimRef.unit,
                )
                find_items.append(
                    {
                        "kind": "cascade",
                        "label": text,
                        "tipText": "Find other peaks that have a similar dimension %d position to the peak at the cursor"
                        % i,
                        "shortcut": "%d" % i,
                        "submenu": spec_find_items,
                    }
                )
                i += 1
                    ...
                    dim_items.append(
                        {
                            ...
                            "command": lambda event, p=currentPeaks, i=i, pl=peakList: self.findPositionMatches(
                                p, i, pl
                            ),
                        }
                    )
                ...
                dim_items.append(
                    {
                        "kind": "command",
                        "label": text,
                        "tipText": "Match selected peaks to all spectra displayed in the window using dimension %d"
                        % (i + 1),
            ...
            for fitType in PEAK_FIT_TYPES:
                for fitMethod in PEAK_FIT_METHODS[1:]:
                    fit_items.append(
                        {
                            "kind": "command",
                            "label": "%s, %s" % (fitMethod, fitType),
                            "tipText": "Fit the selected peaks %s using the %s method" % (fitType, fitMethod),
                            "command": lambda event, fitMethod=fitMethod, fitType=fitType: self.fitPeaks(
                                fitMethod, fitType
                            ),
                            "shortcut": fitMethod[0],
                        }
                    )
                ...
                fit_items.append(
                    {
                        "kind": "command",
                        "label": "%s" % fitMethod,
                        "command": lambda event, fitMethod=fitMethod: self.fitPeaks(fitMethod),
                        "shortcut": fitMethod[0],
                    }
                )
                ...
                move_items.append(
                    {
                        "kind": "command",
                        "label": label,
                        "command": lambda event, peakList=peakList: self.movePeaksToPeakList(peakList),
                        "shortcut": shortcut,
                    }
                )
        ...
        if windowZplanes:
            for planeName, pane, positions in windowZplanes:
                cmd = lambda event, w=pane, position=positions[0]: self.gotoOrthogonalPlane(w, position)
                goto_items.append(
                    {
                        "kind": "command",
                        "label": planeName,
        ...
        if transposes:
            ...
            if len(transposes) > 1:
                ...
                for label, position in transposes:
                    cmd = lambda event, pos=position, col=col, row=row: self.gotoPosition(pos, row, col)
                    transpose_items.append(
                        {
                            "kind": "command",
                            "label": label,
        ...
        macro_items = [
            {
                ...
                "command": self.reloadMouseMacros,
                "shortcut": "R",
            }
        ]
                ...
                macro_items.append(
                    {
                        "kind": "command",
                        "label": macro.name,
                        "tipText": 'Launch the macro called "%s"; any positional information comes from the cursor location'
                        % macro.name,
    ...
    def setMenuItems(self):
        ...
        zoom_items = [
            {
                ...
                "command": lambda event: self.zoom(event.widget, 2.0),
                "shortcut": "O",
            },
            {
                "kind": "command",
                ...
                "command": lambda event: self.zoom(event.widget, 0.5),
                "shortcut": "I",
            },
            {
                "kind": "command",
                "label": "Zoom to selected peaks",
                "tipText": "Center the viewed region of the contour display to cover any peaks selected in the window",
                "command": self.zoomToSelectedPeaks,
                "shortcut": "Z",
            },
            {
                "kind": "command",
                "label": "Zoom to spin system peaks",
                "shortcut": "s",
        ...
        mark_items = [
            {
                ...
                "command": self.createMark,
                "shortcut": "M",
            },
            {
                "kind": "command",
                "label": "Add Horizontal ruler",
                "tipText": "Add a single horizontal line at the current cursor location",
                "command": self.createHorizontalRuler,
                "shortcut": "H",
            },
            {
                "kind": "command",
                "label": "Add Vertical ruler",
                "tipText": "Add a single vertical line at the current cursor location",
                "command": self.createVerticalRuler,
                "shortcut": "V",
            },
            {
                "kind": "command",
            ...
            {"kind": "separator"},
            {
                ...
                "command": lambda event: self.topPopup.editMarks(),
                "shortcut": "O",
            },
        ]
        ...
        strip_items = [
            {
                "kind": "command",
                "label": "Add Vertical strip",
                "tipText": "Add a new vertical strip/sub-division to the window",
                "command": lambda: self.addCol(),
                "shortcut": "V",
            },
            {
                "kind": "command",
                "label": "Add Horizontal strip",
                "tipText": "Add a new Horizontal strip/sub-division to the window",
                "command": lambda event: self.addRow(),
                "shortcut": "H",
            },
            {
                "kind": "command",
                "label": "Delete vertical strip",
                "tipText": "Delete the vertical strip currently under the cursor",
                "command": lambda event: self.deleteCol(),
                "shortcut": "v",
            },
            {
                "kind": "command",
                "label": "Delete horizontal strip",
                "tipText": "Delete the horizontal strip currently under the cursor",
                "command": lambda event: self.deleteRow(),
                "shortcut": "h",
            },
            {
                "kind": "command",
        ...
        merit_items.append(
            {
                ...
                "command": lambda event, m=0.0: self.setPeakMerit(m),
                "shortcut": "0",
            }
        )
        merit_items.append(
            {
                ...
                "command": lambda event, m=0.5: self.setPeakMerit(m),
                "shortcut": "5",
            }
        )
        merit_items.append(
            {
                ...
                "command": lambda event, m=1.0: self.setPeakMerit(m),
                "shortcut": "1",
            }
        )
        ...
        assn_items = [
            ...
            },
            {
                ...
                "command": self.propagatePeakAssignments,
                "shortcut": "P",
            },
            {
                "kind": "cascade",
                "label": "Propagate to sister peak lists",
            ...
            },
            {
                ...
                "command": self.uniteResonancePositions,
                "shortcut": "n",
            },
        ]
        ...
        peak_symm_items = [
            {
                ...
                "command": self.stripSymmetryRelatedPeaks,
                "shortcut": "s",
            },
            {
                "kind": "command",
                ...
                "command": self.findSymmetryRelatedPeaks,
                "shortcut": "t",
            },
        ]
        ...
        peak_items = [
            ...
            },
            {
                ...
                "command": self.deleteSelectedPeaks,
                "shortcut": "D",
            },
            {
                "kind": "command",
            ...
            },
            {
                ...
                "command": self.snapSelectedPeaks,
                "shortcut": "t",
            },
            {
                "kind": "command",
        ...
        peak_nav_items = [
            ...
            },
            {
                ...
                "command": self.peakCenterOrthogonalPlanes,
                "shortcut": "C",
            },
            {
                "kind": "cascade",
                "label": "Find close peaks",
            ...
            },
            {
                ...
                "submenu": peak_symm_items,
                "shortcut": "r",
            },
            {
                "kind": "command",
                "label": "Strip plot selected peaks",
        ...
        contour_items = [
            {
                ...
                "command": lambda event: self.changeScale(2.0),
                "shortcut": "U",
            },
            {
                "kind": "command",
                ...
                "command": lambda event: self.changeScale(0.5),
                "shortcut": "D",
            },
            {
                "kind": "command",
                ...
                "command": lambda event: self.windowPopup.generalContours(),
                "shortcut": "G",
            },
        ]
    ...
    def setPeakMerit(self, merit):
        ...
        if len(peaks) > 1:
            if not showYesNo(
                "Set merit", "Do you really want to set merit to %.1f for %d peaks?" % (merit, len(peaks)), parent=self
            ):
                return
    ...
    def uniteResonancePositions(self, event):
        ...
        if len(peaks) > 1:
            if not showOkCancel(
                "Confirm",
                "Do you really want to unite resonance positions for the %d selected peaks? This will align other peaks which carry the same assignments."
                % len(peaks),
                parent=self,
            ):
                return
    ...
    def clearSeqLinks(self, event):
        ...
        if peaks:
            if showOkCancel(
                "Confirm",
                "Clear all sequential spin system connections for %d selected peaks?" % len(peaks),
                parent=self,
            ):
                spinSystems = []
                for peak in peaks:
                ...
                if view.isPosVisible or view.isNegVisible:
                    ...
                    for axisMapping in view.axisMappings:
                        ...
                        if region0 < 0 or region1 > dataDim.numPoints:
                            print(
                                "Outside fundamental region for spectrum %s:%s and noise calculation only works inside that"
    ...
    def selectSingle(self, canvas, a, b, x, y, button=1, state=no_key_state, newSelection=True, event=None):
    ...
    def createNewPeak(self, a, b, row, col):
        ...
        for view in activeViews:
            ...
            for axis in position.keys():
                if axis in axisMapping:  # might not if lower dimensional spectrum
                ...
                if dataDim:  # Pseudo 3Ds etc.
                    ...
                    for point in points:
                        ...
                        addPeak(peakList, position=specPosition, tile=specTile, parent=self.topPopup, doFit=False)
        ...
        if state == shift_key_state or state == no_key_state:
            ...
            for view in views:
                ...
                if peakList:
                    ...
                    if winPeakList and winPeakList.isSymbolDrawn:
                        peakLists = [peakList]
                        searchPeaks(
                            peakLists, region=spectrum_region, parent=self.topPopup, thickness=spectrum_thickness
                        )
            ...
            for view in views:
                ...
                if result:
                    n, avg, std = result
                    print(
                        "spectrum=%s:%s, npts=%d, sum=%.3e, avg=%.3e, std=%.3e"
                        % (spectrum.experiment.name, spectrum.name, n, n * avg, avg, std)
                    )
                else:
            ...
            for view in views:
                ...
                for axisMapping in view.axisMappings:
                    ...
                    findPeaks(peakList, region=spectrum_region, parent=self.topPopup, thickness=spectrum_thickness)
    ...
    def selectMulti(self, canvas, a0, b0, a1, b1, x0, y0, x1, y1, button=1, state=no_key_state, event=None):
    ...
    def dragBox(self, canvas, a0, b0, a1, b1, x0, y0, x1, y1, button=1, state=no_key_state, event=None):
    ...
    def initSpectrumWindowView(self, spectrumWindowView):
        ...
        self.after_idle(
            lambda spectrumWindowView=spectrumWindowView: self.changedSpectrumWindowView(spectrumWindowView)
        )
    ...
    def minAxisPanelThickness(self, axisPanel):
        ...
        if axisPanel.axisUnit:
            ...
            for view in self.getSpectrumViews():
                ...
                if axisMapping:
                    t = Util.convertPosition(
                        1.0, getPrimaryDataDimRef(axisMapping.analysisDataDim.dataDim), toUnit=unit, relative=True
                    )
                    t = abs(t)
    ...
    def getOrthogonalWorldRegion(self, axisPanel):
        ...
        for view in views:
            ...
            minAliasedFreq = expDimRef.minAliasedFreq
            if minAliasedFreq is None:
                minAliasedFreq = Util.convertPosition(
                    float(dataDim.numPoints), dataDimRef, toUnit=axisType.findFirstAxisUnit(unit="ppm").unit
                )
            if r0 is None:
                r0 = minAliasedFreq
            else:
            ...
            maxAliasedFreq = expDimRef.maxAliasedFreq
            if maxAliasedFreq is None:
                maxAliasedFreq = Util.convertPosition(
                    1.0, dataDimRef, toUnit=axisType.findFirstAxisUnit(unit="ppm").unit
                )
            if r1 is None:
                r1 = maxAliasedFreq
            else:
    ...
    def createRegionSelector(self, axisPanel):
        ...
        callback = lambda view_region, axisPanel=axisPanel: self.regionSelectorCallback(view_region, axisPanel)

        if axisType.isSampled:
            numButtons = int(w1)
            # state = w0*[False] + (w1-w0+1)*[True]
            state = self.getPseudoState(axisPanel)
            axisPanel.region_selector = ButtonScrollbar(
                self, numButtons=numButtons, state=state, label="Plane", callback=callback
            )
        else:
            # if True:
            label = axisType.name
            ...
            axisPanel.region_selector = RegionSelector(
                self,
                label=label,
                callback=callback,
                text_decimals=axisType.numDecimals,
                world_region=world_region,
    ...
    def curateNotifiers(self, notify):
        ...
        for func in (
            "setPeakDrawMethod",
            "setPeakPixelSize",
            "setPeakIntensityScale",
            "setPeakVolumeScale",
            "setPeakFindThickness",
    ...
    def changedSliceRange(self, windowPane):
        ...
        if self.hasValueAxis:
            ...
            window = windowPane.spectrumWindow
            if (
                (window.isXSliceDrawn or window.isYSliceDrawn)
    ...
    def motion(self, event):
        ...
        if not (hasattr(canvas, "canvas_width") and hasattr(canvas, "canvas_height")):
            ...
            if axisUnit.unit == "ppm":
                ...
                for view in views:
                    ...
                    if spinningRate:
                        ...
                        if minAliasedFreq is None:
                            minAliasedFreq = Util.convertPosition(
                                float(dataDim.numPoints), dataDimRef, toUnit=axisUnit.unit
                            )
                        if maxAliasedFreq is None:
    ...
    def drawViewTile(
        ...
        if canvas.doubleBuffer:
            ...
            self.drawViewTileReal(
                handler, view, contourLevels, contourStyle, worldPointRanges, spectrumPointRanges, row, components
            )
            ...
            try:
                ...
                self.drawViewTileReal(
                    handler, view, contourLevels, contourStyle, worldPointRanges, spectrumPointRanges, row
                )
    ...
    def drawViewSlice(self, slice, view, sliceRange=None):
        ...
        if isWindowsOS():
            if hasattr(view, "_createSliceFileObjectX"):
                ...
                self._tempblock_fileX = getBlockFile(dataSource, mem_cache=mem_cache, writeable=writeable)
            ...
            if hasattr(view, "_createSliceFileObjectY"):
                ...
                dataSource = view.analysisSpectrum.dataSource
                writeable = hasattr(dataSource, "writeable") and dataSource.writeable
                self._tempblock_fileY = getBlockFile(dataSource, mem_cache=mem_cache, writeable=writeable)
        ...
        for axisMapping in view.axisMappings:
            ...
            if dataDim.className == "FreqDataDim":
                ...
                p = Util.convertPosition(p, getPrimaryDataDimRef(dataDim), fromUnit=axisPanel.axisUnit.unit) - 1
    ...
    def splitWindowVertical(self, event):
        ...
        if r0 > r1:
            (r0, r1) = (r1, r0)
        Util.addAxisPanelRegion(
            self.windowPane.findFirstAxisPanel(label="x"), region=(r0, r1), size=new_canvas.winfo_width()
        )
    ...
    def addRow(self, row=-1, centerPositionDict=None):
        ...
        Util.addAxisPanelRegion(yAxisPanel, region=(r0, r1), size=canvas.winfo_height())
    ...
    def deleteRow(self, row=-1):
        ...
        if N > 1:
            ...
            if (
                ...
                and (active_row != None)
                and (active_row != "all")
                and self.windowPane.spectrumWindow.stripAxis == "y"
            ):
                row = active_row
    ...
    def addCol(self, col=-1, centerPositionDict=None):
        ...
        Util.addAxisPanelRegion(
            self.windowPane.findFirstAxisPanel(label="x"), region=(r0, r1), size=canvas.winfo_width()
        )
    ...
    def deleteCol(self, col=-1):
        ...
        if N > 1:
            ...
            if (
                ...
                and (active_col != None)
                and (active_col != "all")
                and self.windowPane.spectrumWindow.stripAxis == "x"
            ):
                col = active_col
    ...
    def makeOrthogonalStrips(self, windowPane, positions):
        ...
        N = len(positions)
        if N > 10 and not showOkCancel(
            "Warning",
            "%d positions selected. Really make %d strips in window %s" % (N, N, getWindowPaneName(windowPane)),
            parent=self,
        ):
            return
        ...
        if positions:
            displayStrips(self.topPopup, positions, orthoPositions=None, spectrum=None, windowPane=windowPane)
                        ...
                        if axisMapping.label not in ("x", "y"):
                            for peakDim in peak.peakDims:
                                if peakDim.dataDim is axisMapping.analysisDataDim.dataDim:
                                    dataDimRef = peakDim.dataDimRef
                                    if dataDimRef:
                                        position[axisMapping.label] = getPeakDimPosition(
                                            peakDim, toUnit=dataDimRef.expDimRef.unit
                                        )
                                        break
                                    else:
