...
class ViewResidueFrame(Frame):
    ...
    def __init__(
        self, parent, residue=None, resizeCallback=None, project=None, tipText=None, shiftList=None, *args, **kw
    ):
        ...
        self.varFrame = ViewChemCompVarFrame(
            self, chemCompVar=self.chemCompVar, project=project, tipText=tipText, grid=(row, 0), gridSpan=(1, 3)
        )
    ...
    def setResidue(self, residue):
        ...
        if residue:
            ...
            self.label.set(
                "Residue: %d%s ( %s %s )"
                % (self.residue.seqCode, getResidueCode(self.residue), chain.molSystem.code, chain.code)
            )
