...
from ccpnmr.analysis.core.Register import isRegistered, updateRegister
from ccpnmr.analysis.core.WindowBasic import (
    cloneSpectrumWindow,
    copySpectrumWindowRegions,
    copySpectrumWindowViewProperties,
    initWindowMacros,
    isActiveWindow,
    togglePeakList,
    toggleSpectrum,
...
class AnalysisPopup(BasePopup, Analysis):
    ...
    def __init__(self, root, cache_size=64, glDirect=None):
        ...
        self.program_name = " ".join([Copyright.suite, Copyright.program, ".".join([str(x) for x in versionData])])
    ...
    def queryDeleteSelected(self, originator=None):
        ...
        if self.selected_objects:
            ...
            if showYesNo("Delete object%s" % s1, "Delete %sselected %s%s?" % (s2, className, s1), parent=originator):
                self.deleteSelected()
    ...
    def queryDeletePeaks(self, peaks, originator=None):
        ...
        if peaks:
            ...
            if showYesNo("Delete Peak%s" % s1, "Delete %sPeak%s?" % (s2, s1), parent=originator):
    ...
    def setProjectMenu(self):
        ...
        fontsMenu = FontMenu(
            self.menubar, self.selectFont, sizes=(8, 10, 12, 14), doItalic=False, doBoldItalic=False, tearoff=0
        )
        ...
        prefsMenu.add_command(
            ...
            compound="left",
            shortcut="R",
            tipText="User-specified codes that override the standard residue names",
        ...
        menu.add_cascade(label="Preferences", shortcut="P", image=self.iconPrefs, compound="left", menu=prefsMenu)
        menu.add_command(
            label="Register",
            shortcut="R",
            image=self.iconTool,
    ...
    def setExperimentMenu(self):
        ...
        menu_items = self.menu_items[ExperimentMenu] = [
            "Open Spectra",
            "Spectra",
            "Experiments",
            "NMR Series",
    ...
    def setResonanceMenu(self):
        ...
        menu.add_separator()
        menu.add_command(
            label="Reference Chemical Shifts",
            shortcut="C",
            image=self.iconChart,
            compound="left",
    ...
    def setChartMenu(self):
        ...
        menu.add_command(
            label="Residue Interaction Matrix",
            shortcut="I",
            image=self.iconChart,
            compound="left",
    ...
    def setWindowMenu(self):
        ...
        menu_items = self.menu_items[WindowMenu] = ["Windows", "Axes", "New Window", "Print Window", "Marks and Rulers"]
        ...
        if hasattr(analysisProfile, "userName"):
            if isRegistered(analysisProfile):
                if not self.printedRegistration:
                    print(
                        "Registered (%s, %s, %s)"
                        % (analysisProfile.userName, analysisProfile.userOrganisation, analysisProfile.userEmail)
                    )
    ...
    def registerCallback(self, funcname, callback):
        ...
        if funcname not in self.callbacksDict:
    ...
    def openPopup(self, popup_name, clazz, oldStyle=False, *args, **kw):
        ...
        if popup:
            ...
            if oldStyle:
                ...
                popup = self.popups[popup_name] = clazz(
                    self, project=self.project, popup_name=name, transient=transient, *args, **kw
                )
    ...
    def editExpPrototype(self):
        ...
        self.openPopup("edit_experiment_prototypes", NmrExpPrototypePopup, title="Experiment: Experiment Prototypes")
    ...
    def browseResonanceInfo(self, resonance=None):
        ...
        popup = self.openPopup("browse_resonance_info", ResonanceInfoPopup, resonance=resonance)
    ...
    def backupProject(self):
        ...
        self.openPopup("backup_project", BackupProjectPopup, help_url=determineHelpUrl(name="BackupProjectPopup"))

    def archiveProject(self):

        self.openPopup("archive_project", ArchiveProjectPopup, help_url=determineHelpUrl(name="ArchiveProjectPopup"))
    ...
    def importNmrStar211(self):
        ...
        if self.project:
            ...
            fileSelectPopup = FileSelectPopup(
                self,
                file_types=fileTypes,
                title="Import NMR-STAR 2.1.1 file",
                dismiss_text="Cancel",
                selected_file_must_exist=True,
                multiSelect=False,
            )
    ...
    def importNmrStar31(self):
        ...
        if self.project:
            ...
            fileSelectPopup = FileSelectPopup(
                self,
                file_types=fileTypes,
                title="Import NMR-STAR 3.1 file",
                dismiss_text="Cancel",
                selected_file_must_exist=True,
                multiSelect=False,
            )
    ...
    def importCoordinates(self):
        ...
        if self.project:
            ...
            fileSelectPopup = FileSelectPopup(
                self,
                file_types=fileTypes,
                title="Import PDB-style Coordinates",
                dismiss_text="Cancel",
                selected_file_must_exist=True,
                multiSelect=True,
                directory=projPath,
    ...
    def validateProject(self):
        ...
        if showYesNo("Project : Validate", "Do a complete validation (can take some time)?", parent=self):
            complete = True
        else:
            complete = False
    ...
    def quit(self):
        ...
        if not showYesNo("Quit " + self.program_name, "Quit " + self.program_name + "?", parent=self):
            return
  ...
  """
    ...
    def initMacros(self):
        ...
        if analysisProfile.findFirstMacro(function=function) is None:
            m = analysisProfile.newMacro(
                name="browseAtoms", path=path, function=function, module=module[-1], shortcut="b", ordering=1
            )
        ...
        if macro is None:
            m = analysisProfile.newMacro(
                name="clearMarksAndRulers", path=path, function=function, module="MarkBasic", shortcut="n", ordering=1
            )
