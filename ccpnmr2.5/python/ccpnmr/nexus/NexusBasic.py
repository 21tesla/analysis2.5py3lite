...
from ccpnmr.analysis.core.ExperimentBasic import getOnebondDataDims, getPrimaryDataDimRef, getSeqAssignRefExperiments
...
CARBONYL_TYPE = set(
    ("H[N[CO]]", "H[N[ca[CO]]]", "CONH", "HNCO", "HNcaCO", "H[N[coca][ca[CO]]", "H[N_[ca[CO]].onebond)")
)
...
CACB_TYPE = set(("h{CA|Cca}coNH", "h{CA|Cca}NH", "{CA|Cca}coNH", "{CA|Cca}NH", "h{CA|Cca}NH.seq"))

HA_TYPE = set(("HA[ca[N[H]]]", "H[N[ca[HA]]]", "HAcaNH", "HNcaHA", "HNcocaH", "HcacoNH", "H{[N]+[HA]}"))

HB_TYPE = set(("H[N[HB]]",))

HAHB_SIGN_TYPE = set(
    ("H[N[co[{ca[H]|ca[c[H]]}]]]", "H[N[co[{ca[H]|cac[H]}]]]", "H[N[{ca[H]|ca[c[H]}]]", "H[N[{ca|cac}[H]]]")
)
...
def linkSpinSystemInterIntraResonances(spinSystem, activeLists, tolerances=None):
            ...
            for peakDimB, contribB, isotopeB, isInterB, ppmB, i in untyped:
                ...
                if contribB:
                    ...
                    if resonance is not contribB.resonance:
                        ...
                        assignResToDim(peakDimB, resonance, tolerance=tolerance)
    ...
    for atomType in nonMatched.keys():
        ...
        if not contrib:
            ...
            contrib = assignResToDim(peakDim, resonance, tolerance=2 * tolerance)
