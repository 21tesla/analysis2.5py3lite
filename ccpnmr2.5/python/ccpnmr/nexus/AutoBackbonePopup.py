...
from ccpnmr.analysis.core.AssignmentBasic import (
    ...
    assignTentativeSpinSystemResidues,
...
from ccpnmr.nexus.NexusBasic import (
    getAmideSpinSystems,
    getSpinSystemInterIntraResonances,
    isResonanceAmide,
    linkSpinSystemInterIntraResonances,
)
...
PROGRAM_ATTRIBUTES = [(NEXUS, "nexusPrediction"), (MARS, "marsPrediction")]

PROGRAM_THRESHOLDS = {
    NEXUS: (0.75, 0.5),
    MARS: (0.9, 0.4),
}  # proposed by Zweckstetter
# MARS: (0.8, 0.4),}

SCORE_COLORS = ["#8080D0", "#E0E000", "#E00000"]

COMPARISONS = ["Current Assignment", NEXUS + " Prediction", MARS + " Prediction"]
...
STRIP_ACTIONS = ("Any Available", "Confirmed Only", "Best Match", "1st Alterative (if available)")
...
class AutoBackbonePopup(BasePopup):
    ...
    def body(self, guiFrame):
        ...
        options = ["Settings", "Spin Systems", "Automation", "Score Graph", "Predictions", "Instructions", "Comparison"]
        ...
        self.spectraMatrix = ScrolledMatrix(
            frame,
        ...
        headingList = ["#", "Residue", "Intra residue\nresonances", "Inter residue\nresonances", "Comments"]
        self.spinSystemsMatrix = ScrolledMatrix(frameB, headingList=headingList, callback=self.selectSpinSystem)
        ...
        if not HAVE_NUMPY:
            ...
            self.nexusIterationsPulldown = PulldownList(frame, objects=objects, texts=texts, index=1, grid=(1, 3))
            ...
            self.nexusVarianceEntryH = FloatEntry(frame, text=0.04, width=8, grid=(1, 1))

            buttonList = ButtonList(
                frame,
                commands=[self.runNexus],
                texts=[
                    "Run Nexus",
                ],
                grid=(0, 4),
        ...
        if not marsDir:
            ...
            self.marsDeuteratedSelect = CheckButton(frame, callback=None, grid=(row, 3), selected=False, sticky="w")

            row += 1

            label = Label(frame, text="Keep existing assignments?", grid=(row, 0))
            self.marsKeepAssignSelect = CheckButton(frame, callback=None, grid=(row, 1), selected=False, sticky="w")

            label = Label(frame, text="Correct shifts for disulfides?", grid=(row, 2))
            self.marsDisulfideSelect = CheckButton(frame, callback=None, grid=(row, 3), selected=False, sticky="w")

            row += 1

            label = Label(frame, text="Keep existing connectivity?", grid=(row, 0))
            self.marsKeepConnSelect = CheckButton(frame, callback=None, grid=(row, 1), selected=False, sticky="w")
            ...
            self.marsCoCutoffEntry = FloatEntry(subframe, grid=(1, 0), width=8, text=MARS_TOLERANCES.get("CO"))
            self.marsCaCutoffEntry = FloatEntry(subframe, grid=(1, 1), width=8, text=MARS_TOLERANCES.get("CA"))
            self.marsCbCutoffEntry = FloatEntry(subframe, grid=(1, 2), width=8, text=MARS_TOLERANCES.get("CB"))
            self.marsHaCutoffEntry = FloatEntry(subframe, grid=(1, 3), width=8, text=MARS_TOLERANCES.get("HA"))
            ...
            label = Label(
                frame,
                text="Native MARS has more options, including RDC and coupling input.",
                grid=(row, 0),
                gridSpan=(1, 4),
            )
        ...
        self.scoreGraph = ScrolledGraph(frameD, graphType="histogram", width=700, height=200)
        self.scoreGraph.grid(row=0, column=0, sticky="nsew")

        self.scoreGraph2 = ScrolledGraph(frameD, graphType="histogram", width=700, height=200)
        ...
        self.probRegionsCheck = CheckButton(frame, grid=(0, 1), callback=self.updatePredictionsAfter)

        label = Label(frame, text="Prediction Method:", grid=(0, 3))
        self.methodPulldown = PulldownList(
            frame, grid=(0, 4), texts=(NEXUS, MARS), index=0, callback=self.selectPredictionMethod
        )
        ...
        editWidgets = [None, None, self.assignPulldown, None, None, None, None]
        editGetCallbacks = [None, None, self.getResidueSpinSystems, None, None, None, None]
        editSetCallbacks = [None, None, self.setResidueSpinSystem, None, None, None, None]
        self.assignmentMatrix = ScrolledMatrix(
            frameE,
            headingList=headingList,
            editWidgets=editWidgets,
            editSetCallbacks=editSetCallbacks,
        ...
        texts = ["Clear Confirmation", "Confirm Selected", "Confirm Above Score Threshold:"]

        commands = [self.clearSelected, self.setSelectedResidues, self.setAboveThreshold]

        self.confirmButtons = ButtonList(frame, texts=texts, commands=commands, grid=(0, 0))

        threshold = PROGRAM_THRESHOLDS[self.predictMethod][0]
        self.assignThresholdEntry = FloatEntry(frame, width=8, grid=(0, 1), text=threshold)
        ...
        texts = ["Commit Assignments", "Assign Tentatively", "Strip Selected Using:"]

        commands = [self.commitAssignments, self.tentativeAssignments, self.gotoPredictionStrips]

        self.assignButtons = ButtonList(frame, texts=texts, commands=commands, grid=(0, 0))

        self.stripPulldown = PulldownList(frame, texts=STRIP_ACTIONS, grid=(0, 1), objects=range(len(STRIP_ACTIONS)))
        ...
        self.compXPulldown = PulldownList(
            frameG, texts=COMPARISONS, grid=(0, 1), objects=[None, NEXUS, MARS], callback=self.updateComparison
        )

        label = Label(frameG, text="Y Axis:", grid=(0, 2))
        self.compYPulldown = PulldownList(
            frameG, texts=COMPARISONS, grid=(0, 3), objects=[None, NEXUS, MARS], callback=self.updateComparison
        )

        self.compGraph = ScrolledGraph(frameG, graphType="scatter", width=500, height=500)
    ...
    def getResidueSpinSystems(self, residue):
        ...
        if residue:
            ...
            names = [
                "<None>",
            ] + [getSpinSystemName(ss) for ss in spinSystems[1:]]
    ...
    def updatePredictions(self):
        ...
        for i, residue in enumerate(residues):
            ...
            if score is not None:
                if score > tGood:
                    if onlyProbRegions:
                        ...
                        if not (((prev is not None) and (prev < tGood)) or ((next is not None) and (next < tGood))):
                            continue
                    index = 0
                elif score < tBad:
                    index = 2
            ...
            if residue not in self.assignments:
        ...
        self.assignmentMatrix.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
    ...
    def mergeResonances(self, ss1, ss2):
        ...
        typeDict[atomType] = typeDict.get(atomType, []) + [
            resonance,
        ]
    ...
    def runNexus(self):
        ...
        progressBar = ProgressBar(self, text="Assigning Backbone", total=len(self.spinSystems))
    ...
    def updateSpectraMatrix(self):
        ...
        for peakList in objectList:
            ...
            textMatrix.append((listName, experiment.refExperiment.name, isUsed, len(peakList.peaks)))
            colorMatrix.append(colors)

        self.spectraMatrix.update(objectList=objectList, textMatrix=textMatrix, colorMatrix=colorMatrix)
    ...
    def updateSpinSystemsMatrix(self):
        ...
        for spinSystem in spinSystems:
            ...
            datum = [getSpinSystemName(spinSystem), resCode, " ".join(intra), " ".join(inter), ",".join(comments)]
        ...
        self.spinSystemsMatrix.update(textMatrix=textMatrix, objectList=objectList, colorMatrix=colorMatrix)
    ...
    def setupResonances(self):
        ...
        progressBar = ProgressBar(self, text="Curating intra and inter residue resonances", total=len(spinSystems))
    ...
    def gotoPredictionStrips(self):
        ...
        options = ((True, True, False), (True, False, False), (False, True, False), (False, True, True))

        useConfirmed, useMatch, useAlternative = options[index]

        self.gotoSpinSystemStrips(useConfirmed, useMatch, useAlternative)

    def gotoSpinSystemStrips(self, useConfirmed=True, useMatch=True, useAlternative=False):
    ...
    def updateSpectraMatrixAfter(self, obj=None):
        ...
        if obj:
            if obj.className == "DataSource":
                ...
                if obj.isDeleted and (obj in self.activeLists):
    ...
    def updateSpinSystemsAfter(self, obj=None):
        ...
        if obj:
            if obj.className == "PeakDim":
                if obj.peak.peakList not in self.activeLists:
                    return
