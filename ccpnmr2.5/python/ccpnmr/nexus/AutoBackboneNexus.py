...
def autoBackboneNexus(
    ...
    cMatrix = getConnectivityMatrix(spinSystems, peakLists, sqrt(varianceC), sqrt(varianceH), progressBar)
    ...
    for i in range(n):
        ...
        if prev and (prev.ccpCode in PROLYL):
            ...
            if spinSystemsA and spinSystemsB:
                ...
                connPrev = getSpinSystemLikelihood(spinSystemA, spinSystemB, peakLists)
        ...
        if next and (next.ccpCode in PROLYL):
            ...
            if spinSystemsB and spinSystemsC:
                ...
                connNext = getSpinSystemLikelihood(spinSystemB, spinSystemC, peakLists)
...
def getInitialAssignMatrix(chain, spinSystems, peakLists, shiftList, keepExisting=False, progressBar=None):
...
def getConnectivityMatrix(spinSystems, peakLists, varianceC=0.09, varianceH=0.04, progressBar=None):
    ...
    for i, spinSystemA in enumerate(spinSystems):
        for j, spinSystemB in enumerate(spinSystems):
            p = getSpinSystemLikelihood(spinSystemA, spinSystemB, peakLists, varianceC, varianceH)
...
def getSpinSystemLikelihood(spinSystemA, spinSystemB, peakLists, sigmaC=0.3, sigmaH=0.2, limit=3.0, cache={}):
