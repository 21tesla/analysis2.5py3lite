...
from ccpnmr.update.UpdateAgent import (
...
class UpdateAdministratorPopup(BasePopup, UpdateAgent):
    ...
    def _fillCompareList(self, currentPath=os.getcwd()):
        ...
        if filter:
            ...
            self._allFiles = [
                filePath for filePath in self._allFiles if os.path.splitext(filePath)[-1].lower() in filter.split()
            ]
    ...
    def _checkBranchServer(self):
        ...
        if self._filterEntry.get():
            filter = self._filterEntry.get()
            filePaths = [
                filePath for filePath in filePaths if os.path.splitext(filePath)[-1].lower() in filter.split()
