    ...
    def fcSetPeaksInformation(self, formatName, filePath, addKeywords):
        ...
        if not fileRead:
            ...
            if len(peakListNames) > 1:
                if "peakListName" not in addKeywords:
                    raise self.FcWorkFlowError(
        ...
        if "expDimToPeakDim" not in addKeywords or not addKeywords["expDimToPeakDim"]:
            (matches, uniqueMatches) = self.formatConversion.matchFormatAndCcpnPeakDims(
...
class FcProject_1bc6(FcWorkFlow):
    ...
    def fcImportAllData(self):
        ...
        self.fcImportFile(
            "coordinates", "pdb", joinPath(self.importDir, "1bc6.pdb"), addKeywords={"forceDefaultChainMapping": True}
        )
