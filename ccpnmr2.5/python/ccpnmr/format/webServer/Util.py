...
def is_current_session_user():
    ...
    if SESSION_KEY in cherrypy.session and cherrypy.session[SESSION_KEY].username == cherrypy.request.login:
        validUser = True
...
def check_credentials(username, password):
    ...
    if username in usersDict:
        if password == usersDict[username]:
            if ( SESSION_KEY not in cherrypy.session) or (not cherrypy.session[SESSION_KEY]):
                error_msg = None
...
<body>
    ...
    def login(self, username=None, password=None, from_page="/", checkType="login"):
        ...
        if error_msg:
            ...
            if SESSION_KEY in cherrypy.session and cherrypy.session[SESSION_KEY]:
                # Doublecheck
