...
class SequenceLinkPopup(TemporaryBasePopup):
    ...
    def __init__(self, parent, project, createMoleculeDict):
        ...
        TemporaryBasePopup.__init__(
            self,
            parent=parent,
            title="Project '%s': " % project.name + "Import original sequence",
            modal=False,
            transient=True,
        )
    ...
    def setupWidgets(self, row=None):
        ...
        for molName in self.molNames:
            ...
            self.origSeqButton[molName] = Tkinter.Button(
                ...
                command=lambda molName=molName: self.popupSequenceChemCompSelect(molName),
            )
            ...
            self.molNameEntry[molName] = Tkinter.Button(
                self.frameMaster, text=molName, command=lambda molName=molName: self.changeMolName(molName)
            )
        ...
        self.buttons = createDismissHelpButtonList(
            self.frameMaster, texts=texts, commands=commands, dismiss_text="Cancel", help_url=self.help_url
        )
    ...
    def popupSequenceChemCompSelect(self, molName):
        ...
        popup = SequenceChemCompSelect(
            self, self.project, molName, self.createMoleculeDict, self.localCreateMoleculeDict
        )
