...
class ProcFilePopup(BasePopup):
    ...
    def __init__(self, parent, acqProcPars, format):
        ...
        self.writeSetup[self.nmrPipe] = [
            ["Write conversion file", lambda: self.writeFile(self.nmrPipe, ["conversion"])],
            ["Write processing file", lambda: self.writeFile(self.nmrPipe, ["processing"])],
            ["Write all", lambda: self.writeFile(self.nmrPipe, ["conversion", "processing"])],
        ]
        ...
        self.writeSetup[self.azara] = [
            ["Write processing parameters file", lambda: self.writeFile(self.azara, ["processingPars"])],
            ["Write processing script file", lambda: self.writeFile(self.azara, ["processingScript"])],
            ["Write all", lambda: self.writeFile(self.azara, ["processingPars", "processingScript"])],
        ]

        BasePopup.__init__(
            self,
            parent=parent,
            title="Project '%s': " % self.project.name + "%s processing write" % format,
            modal=False,
            transient=False,
        )
    ...
    def body(self, master):
        ...
        for buttonIndex in range(0, len(self.fileSetup[self.format])):
            ...
            self.buttons.append(
                Tkinter.Button(master, text=fileName, command=(lambda x=buttonIndex: self.selectFile(self.format, x)))
            )
