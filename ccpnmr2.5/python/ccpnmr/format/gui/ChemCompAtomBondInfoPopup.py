...
class ChemCompAtomBondInfoPopup(TemporaryBasePopup):
    ...
    def __init__(self, parent, chemCompInfo, chemCompAtoms, chemCompBonds):
        ...
        TemporaryBasePopup.__init__(
            self, parent=parent, title="ChemComp atom and bond creation", modal=False, transient=True
        )
    ...
    def body(self, master):
        ...
        row = 0
        label = Label(
            master, text="Molecule type '%s', ccp code '%s'" % (self.chemCompInfo[0][1], self.chemCompInfo[1][1])
        )
        ...
        if self.obligatoryAtoms:
            ...
            label = Label(
                master, text="Warning: have to define atoms: %s!" % string.join(self.obligatoryAtoms, ", "), fg="red"
            )
            label.grid(row=row, column=0, columnspan=self.columnspan, sticky=Tkinter.EW)
                ...
                if attrName == "bondedAtoms":
                    ...
                    if attrName in self.nonEntryAttributes:
                        ...
                        if widgetInfo[0] == PulldownMenu:
                            self.atomWidgets[-1].append(
                                PulldownMenu(
                                    frame,
                                    entries=widgetInfo[1],
                                    selected_index=widgetInfo[1].index(chemCompAtomInfo[1]),
                                )
                            )
    ...
    def apply(self, checkAtoms=True):
        ...
        if checkAtoms and obligatoryAtoms:
            showWarning(
                self,
                "Obligatory atoms: %s still missing - please rename accordingly." % string.join(obligatoryAtoms, ", "),
            )
            return False
