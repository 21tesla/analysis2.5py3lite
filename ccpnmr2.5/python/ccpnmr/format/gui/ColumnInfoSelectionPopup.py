...
class ColumnInfoSelectionPopup(TemporaryBasePopup):
    ...
    def body(self, master):
        ...
        for colIndex in range(0, self.columnLen):
            ...
            label = Label(
                master, text="%s - %s" % (str(self.columnData[colIndex][0]), str(self.columnData[colIndex][1]))
            )
            label.grid(row=row, column=1, sticky=Tkinter.EW)
    ...
    def apply(self):
        ...
        for colIndex in range(0, self.columnLen):
            ...
            if columnInfoLongName != "None" and columnInfoLongName in columnInfoLong:
                showError(
                    "Unique names", "Each column needs a unique content - %s repeated twice." % columnInfoLongName
                )
                return False
