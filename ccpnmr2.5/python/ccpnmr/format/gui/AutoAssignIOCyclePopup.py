...
class AutoAssignIOCyclePopup(BasePopup):
    ...
    def __init__(self, parent, project):
        ...
        BasePopup.__init__(
            self,
            parent=parent,
            title="Project '%s': " % project.name + "AutoAssign export/import cycle.",
            modal=True,
            transient=True,
        )
    ...
    def body(self, master):
        ...
        label = Label(
            master,
            fg="red",
            text="Popup to export %s data from the CCPN data model,\nrun %s, then re-import the output."
        ...
        self.selectExportFileButton = Tkinter.Button(
            master, text="Select export file", command=(lambda: self.selectExportProjectFile())
        )
        ...
        self.selectImportFileButton = Tkinter.Button(
            master, text="Select import file", command=(lambda: self.selectImportShiftFile())
        )
    ...
    def doExport(self):
        ...
        if self.exportFileName:
            ...
            if not returnValue:
                showError(
                    "No export file written", "There were problems while exporting the %s project file." % self.format
                )
            else:
                showInfo("File written", "File written successfully")
                cdtsTag = "%s-%s" % (self.chainSelect.getSelected(), self.dateTimeFlag)
                if cdtsTag not in self.chainDateTimeSelect.entries:
            ...
            showError(
                "No export file defined", "Please define a name for the %s project file to export to." % self.format
            )
    ...
    def doImport(self):
        ...
        if self.importFileName:
            ...
            if cdtsTag not in self.chainDateTimeStampDict:
                            ...
                            for appDatum in appData:
                                if appDatum and appDatum.value == curDateTimeStamp:
                                    ...
                                    if peakList.findFirstApplicationData(
                                        application=self.format, keyword=ioCycleTag_kw, value=rootPeakListTag
                                    ):
                                        for peak in peakList.sortedPeaks():
                                            appData = peak.findFirstApplicationData(
                                                application=self.format, keyword=ioCycleTag_kw
                                            )
                                            if appData:
                ...
                for rawChemShift in autoAssignChemShiftFile.chemShifts:
                    ...
                    if seqCode in seqCodeToResidue:
                    ...
                    self.autoAssignFormat.setSeqAssignTag(
                        atom, rawChemShift.value, AppDataClass=Implementation.AppDataFloat
                    )
                    ...
                    if rawChemShift.peakId:
                        ...
                        if peakNum in rootPeakNumToPeak:
                            peak = rootPeakNumToPeak[peakNum]
                            self.autoAssignFormat.setSeqAssignTag(peak, str((chain.code, residue.seqId)))

        else:
            showError(
                "No import file defined", "Please define a name for the %s shift output file to import." % self.format
            )
