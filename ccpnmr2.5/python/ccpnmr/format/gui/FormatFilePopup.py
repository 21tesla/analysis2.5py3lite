...
from memops.universal.Io import normalisePath, splitPath
...
class FormatFilePopup(BasePopup):
    ...
    def body(self, master):
        ...
        if self.component in fileTypeDict:
            if self.format and self.format in fileTypeDict[self.component]:
                file_types.extend([FileType(self.format, fileTypeDict[self.component][self.format])])

            if "generic" in fileTypeDict[self.component]:
        ...
        self.file_select = FileSelect(
            master, file_types=file_types, directory=directory, double_callback=self.ok, multiSelect=self.multiSelect
        )
