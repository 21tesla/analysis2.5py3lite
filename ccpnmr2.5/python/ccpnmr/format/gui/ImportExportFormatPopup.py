...
from memops.universal.Util import returnFloat, returnInt  # Do memopsWord type stuff as well?
...
def setIoInfo(master, setupInfo, project, fileDefs, forceCreation=False, appendNone=False):
    ...
    if widgetType[-5:] == "Entry":
        addKeywds = {}
        if extraDefs and "width" in extraDefs:
        ...
        if widgetType[:4] == "File":
            ...
            widget = Tkinter.Button(
                master,
                text=IOdefault,
                command=lambda comp=component, format=format, buttonKeyword=IOkeywd, defaultText=IOdefault, fileComponent=fileComponent, multiSelect=multiSelect: (
                    selectFile(comp, format, buttonKeyword, defaultText, fileComponent, multiSelect=multiSelect)
                ),
            )
            ...
            widget = Tkinter.Button(
                master,
                text=IOdefault,
                command=lambda comp=component, format=format, buttonKeyword=IOkeywd: selectDir(
                    comp, format, buttonKeyword
                ),
            )
...
class GenericFormatPopup(BasePopup):
    def __init__(self, parent, format, component=None, threading=True):
        ...
        self.formatModule = __import__(
            "ccpnmr.format.converters.%sFormat" % self.formatLabel,
            {},
            {},
            ["%sFormat" % self.formatLabel, "IOkeywords"],
        )
            ...
            if format in fileDict:
                ...
                if validFlag:
                    ...
                    if validFlag == 1:
                        ...
                        for widgetInfo in self.widgetSetup[component]:
                            if widgetInfo[1] == "fileName":
                                self.IOkeywords[component]["fileName"] = (
                                    None,
                                    True,
                                    "The file name to be %sed." % self.importExportFlag,
                                )
                                break
                            elif widgetInfo[1] == "fileNames":
                                self.IOkeywords[component]["fileNames"] = (
                                    None,
                                    True,
                                    "The file names to be %sed." % self.importExportFlag,
                                )
                                break
                        ...
                        if self.function[component] in self.formatModule.IOkeywords:
                    ...
                    #
                        ...
                        self.IOkeywords[component]["showText"] = "Please use %s to %s %s." % (
                            validFlag,
                            self.importExportFlag,
                            component,
                        )
                        addInfo = None
                ...
                if addInfo:
                    ...
                    for addItem in addInfo:
                        ...
                        if inputKeyWord not in self.IOkeywords[component]:
                            print(
                                "  Error: invalid %s keyword '%s' for %s!"
                                % (self.importExportFlag, inputKeyWord, component)
                            )
                            continue
    ...
    def body(self, master):
        ...
        for component in self.components:
            ...
            if "showText" in self.IOkeywords[component]:
                row += 1
            ...
            for widgetIndex in range(0, len(self.widgetSetup[component])):
                ...
                if widgetInfo[1] in self.IOkeywords[component] and self.IOkeywords[component][widgetInfo[1]][1]:
                    if not widget:
                        if component in self.widgets:
                            showError(
                                "Error",
                                "Could not set up export menu for %s: no '%s' information available."
                                % (component, widgetInfo[3]),
                                self,
                            )
                ...
                if widget:
                    ...
                    infoButton = Tkinter.Button(
                        ...
                        width=0,
                        height=0,
                        command=lambda text=labelText: self.doInfoPopup(text),
                    )
            ...
            if component in self.widgets and self.widgets[component]:
                if self.importExportFlag == "export":
                    row += 1
                    # print 'export button row %d' % row

                    self.exportButton[component] = Tkinter.Button(
                        _frame,
                        text="Export %s file." % component,
                        command=lambda comp=component: self.importExportFile(comp),
                    )
                ...
                if toggleInfoLabelRow != None:
                    ...
                    toggleInfoLabel = ToggleLabel(
                        _frame,
                        text="Additional options",
                        callback=lambda hidden, curRow=toggleInfoLabelRow, toggleWidgets=toggleWidgets: self.toggleInfo(
                            hidden, curRow, toggleWidgets
                        ),
                    )
                ...
                if componentToggleRow != None:
                    toggleInfoLabel = ToggleLabel(
                        _frame,
                        text="Click here for the %s export menu." % component,
                        callback=lambda hidden, curRow=componentToggleRow, allWidgets=allWidgets: self.toggleInfo(
                            hidden, curRow, allWidgets
                        ),
                    )
    ...
    def importExportFile(self, checkComponent):
        ...
        for component in self.components:
            ...
            if checkComponent and checkComponent != component:
                ...
                if self.importExportFlag == "import" and not self.component:
                    if component not in self.checkButton:
                        continue
            ...
            if self.widgets[component]:
                for inputKeyWord in self.widgets[component].keys():
                    ...
                    if inputKeyWord in ("fileName", "fileNames"):
                        ...
                        if (
                            widgetInfo[0] not in ("FileButton", "FileMultiButton", "DirButton")
                            or widgetInfo[3] != value
                        ):
                        ...
                        if self.IOkeywords[component][inputKeyWord][1]:
                            showWarning(
                                "Error",
                                "Nothing selected for %s writing, mandatory input %s!" % (component, inputKeyWord),
                                self,
                            )
                            inputValid = False
                            break
                ...
                if returnValue:
                    ...
                    if conversionSuccess:
                        if fileNameText:
                            showInfo(
                                "Success",
                                "Successfully %sed %s\n\n%s" % (self.importExportFlag, fileNameText, convertInfoString),
                                self,
                            )
                        ...
                        if fileNameText:
                            showInfo(
                                "Not finished",
                                "The %s did not finish correctly for %s\n\n%s"
                                % (self.importExportFlag, fileNameText, convertInfoString),
                                self,
                            )
        ...
        if self.importExportFlag == "import":
            ...
            if returnValue and self.project.molSystems:
                if self.project.currentNmrProject.resonances or self.project.currentNmrProject.nmrConstraintStores:
                    runLinkResonances = showYesNo(
                        "Run linkResonances",
                        "You have a defined molecular system and NMR atom assignment information in your project. To connect this information to each other you have to run linkResonances. Do you want to run this program now?",
                    )
...
class ImportFormatComponentPopup(GenericFormatPopup):
    ...
    def setImportExportFlag(self):
        ...
        self.importExportFlag = "import"
        self.titleText = "Project '%s': " % self.project.name + "%s %s %s" % (
            self.format,
            self.component,
            self.importExportFlag,
        )
