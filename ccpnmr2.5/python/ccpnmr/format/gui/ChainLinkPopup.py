...
class ChainLinkPopup(TemporaryBasePopup):
    ...
    def __init__(
        ...
        TemporaryBasePopup.__init__(
            self, parent=parent, title="Project '%s': " % project.name + "Link chains", modal=False, transient=True
        )
    ...
    def body(self, master):
        ...
        for ccpChainLabel in ccpChainLabels:
            ...
            self.ccpCodeLow[ccpChainLabel] = ScrolledListbox(
                master, initial_list=ccpChainSeqIdLabels[ccpChainLabel], width=4, height=4, xscroll=False
            )
            self.ccpCodeLow[ccpChainLabel].grid(row=row, column=1, sticky=Tkinter.EW)

            self.formatChainMenu[ccpChainLabel] = PulldownMenu(
                master, entries=self.formatChainList, selected_index=self.formatChainList.index(self.defaultFormatChain)
            )
            ...
            self.formatRemoveCode[ccpChainLabel] = Tkinter.Button(
                master, text="Remove", command=(lambda x=ccpChainLabel, row=row: self.removeSeqCode(x, row))
            )
            ...
            self.formatChainMenu[ccpChainLabel].callback = (
                lambda x=-1, y=self.defaultFormatChain, z=ccpChainLabel, row=row: self.updateSeqCodes(x, y, z, row)
            )
    ...
    def updateSeqCodes(self, formatChainIndex, formatChainText, ccpChainLabel, row):
        ...
        if formatChainText == "Do not link":
            ...
            if chainDictKey not in self.localFormatChainDict:
            ...
            if selection != "Do not link":
                ...
                if not selectedItems:
                    showError(
                        ...
                        "Please select a sequence ID on the left side for chain %s!" % ccpChainLabel,
                    )
                    return False
                ...
                (formatChainSeqCodeLow, seqInsertCode) = getSeqAndInsertCode(
                    self.formatChainCodeLow[ccpChainLabel].getSelectedItems()[0]
                )
