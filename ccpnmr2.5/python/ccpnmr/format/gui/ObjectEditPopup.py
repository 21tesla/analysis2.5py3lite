...
from ccp.api.general import Affiliation, Citation
...
class ObjectEditPopup(TemporaryBasePopup):
    ...
    def __init__(
        self, parent, project, selectedObject=None, createCallback=None, ccpnParent=None, ccpnParentSelect=None
    ):
        ...
        if ccpnParentSelect:
            ...
            if parentList:
                interaction = SelectionListPopup(
                    self.parent,
                    parentList,
                    title="Select associated %s" % objectName,
                    text="Select associated %s" % objectName,
                    dismissText="Create new",
                    selectionDict=parentDict,
                    dismissButton=True,
                    modal=True,
                )
            ...
            if not ccpnParent:
                ...
                if storeObject:
                    ...
                    interaction = globals()["%sCreatePopup" % objectName](
                        self.parent, self.project, storeObject, objectName
                    )
        ...
        if self.initialCreate:
            ...
            TemporaryBasePopup.__init__(
                self,
                parent=parent,
                title="Project '%s': Edit %s" % (project.name, self.objectName),
                modal=False,
                transient=True,
            )
    ...
    def body(self, master):
        ...
        self.objectWidget = PulldownMenu(
            master, entries=objectIds, callback=self.updateCallback, do_initial_callback=False
        )
    ...
    def setAttrSelectionList(
        ...
        linkName,
        initialize,
        listParentSelect=None,
        listParent=None,
        forceDisplay=False,
        createNew=None,
        isChild=False,
        parentObjectLink=None,
    ):
        ...
        if initialize:
            ...
            self.widgets[attrName] = ScrolledListbox(
                self.selfMaster,
                width=50,
                height=5,
            ...
            if createNew:
                ...
                self.widgets[creationWidgetName].config(
                    command=self.getCreationCommand(createNew, attrName, listParent, listParentSelect)
                )
    ...
    def setAttrPulldown(
        ...
        if initialize:
            ...
            self.widgets[attrName] = self.setSelectionList(
                labelText, columnspan, self.varLists[attrName], selectedIndex
            )
    ...
    def setNameFuncs(self, metaAttr, attrName):
        ...
        if dataTypeName not in self.nameFuncs:
            self.nameFuncs[dataTypeName] = []

        if attrName not in self.nameFuncs[dataTypeName]:
    ...
    def setAttribute(self, object, attrName, attrLabel, initialize, subClassName=None):
        ...
        if widgetName not in self.otherWidgets:
        ...
        if not metaDataType.enumeration:
            if metaAttr.hicard == 1:
                ...
                if initialize:
                    ...
                    self.widgets[widgetName] = Entry(
                        self.selfMaster, text=value, width=50, returnCallback=self.changesMade
                    )
                    self.widgets[widgetName].grid(
                        row=self.row, column=1, columnspan=(self.columnspan - 1), sticky=Tkinter.W
                    )
                ...
                if initialize:
                    ...
                    self.widgets[widgetName] = ScrolledListbox(
                        self.selfMaster,
                        width=50,
                        height=5,
                        selectmode=Tkinter.MULTIPLE,
                    ...
                    button = Tkinter.Button(
                        self.selfMaster, text="Add value", command=lambda x=widgetName: self.addAttrListValue(x)
                    )
                    ...
                    button = Tkinter.Button(
                        ...
                        command=lambda x=widgetName: self.deleteAttrListSelected(x),
                    )
            ...
            if initialize:
                ...
                self.widgets[widgetName] = self.setSelectionList(
                    "%s (select from list):" % attrLabel, columnspan, valueList, selectedIndex
                )
        ...
        if subClassName and object._metaclass.name != subClassName:
            ...
            if widget not in self.toggledWidgets:
                self.widgets[widgetName].grid()
                for widget in self.otherWidgets[widgetName]:
                    widget.grid()
        ...
        if self.changedObject:
            for objectId in self.objectDict.keys():
                if self.objectDict[objectId] == self.changedObject:
                    break
            if showYesNo(
                "Update %s" % self.objectName.lower(),
                "Update changes to %s '%s' first?" % (self.objectName.lower(), objectId),
            ):
    ...
    def deleteObject(self):
        ...
        if objectId:
            ...
            if object:
                if showYesNo(
                    "Delete %s" % self.objectName.lower(),
                    "Really delete %s '%s'?" % (self.objectName.lower(), objectId),
                ):
                    object.delete()

            else:
                showInfo(
                    "No such %s" % self.objectName.lower(), "%s '%s' does not exist." % (self.objectName, objectId)
                )
    ...
    def createNewObject(self, objectName, attrName, ccpnParent, ccpnParentSelect):
        ...
        if not popup:
            try:
                self.popups[objectName] = globals()["%sEditPopup" % objectName](
                    self,
                    self.project,
                    ccpnParent=ccpnParent,
                    ccpnParentSelect=ccpnParentSelect,
                    createCallback=lambda x=objectName, y=attrName, createdObject=None: self.setOtherObject(
                        x, y, createdObject
                    ),
                )
            except:
    ...
    def setOtherObject(self, objectName, attrName, createdObject):
        ...
        elif objectName in self.popups:
    ...
    def startToggle(self, toggleName):
        ...
        if toggleName not in self.toggleWidgetInfo:
            self.row += 1
            ...
            self.toggleWidgetInfo[toggleName]["widget"] = ToggleLabel(
                self.selfMaster,
                text="Edit %s" % toggleName,
                callback=lambda hidden, name=toggleName: self.toggleInfo(hidden, name),
            )
    ...
    def endToggle(self, toggleName):
        ...
        if "endRow" not in self.toggleWidgetInfo[toggleName]:
        ...
        if hidden:
            ...
            for widget in gridWidgets:
                ...
                if toggleEndRow >= widgetRow >= toggleStartRow:
                    if widget not in self.toggleWidgetInfo[toggleName]["toggleWidgets"]:
                        self.toggleWidgetInfo[toggleName]["toggleWidgets"][widget] = [
                            widgetRow,
                            int(widgetInfo["column"]),
                            int(widgetInfo["rowspan"]),
                            int(widgetInfo["columnspan"]),
                            widgetInfo["sticky"],
                        ]
...
class ObjectCreatePopup(TemporaryBasePopup):
    def __init__(self, parent, project, ccpnParent, objectName, metaClass=None):
        ...
        TemporaryBasePopup.__init__(
            self,
            parent=parent,
            title="Project '%s': " % project.name + "Create %s" % objectName,
            modal=True,
            transient=True,
        )
    ...
    def body(self, master):
        ...
        buttons = createDismissHelpButtonList(
            master, texts=texts, commands=commands, dismiss_text="Skip", help_url=self.help_url
        )
...
class EntryEditPopup(ObjectEditPopup):
    ...
    def setObjectSpecificWidgets(self, entry, initialize):
        ...
        for attrLabel, attrName in (("Title for this entry", "title"), ("Entry details", "details")):
        ...
        self.setAttrPulldown(
            entry,
            "study",
            "study for this entry",
            "currentNmrEntryStore.sortedStudies()",
            initialize,
            listParent=self.project.currentNmrEntryStore,
            forceDisplay=True,
            createNew="Study",
        )
        ...
        self.setAttrSelectionList(
            self.project.currentAffiliationStore,
            "organisations",
            "organisations in entry",
            "currentAffiliationStore.sortedOrganisations()",
            initialize,
            listParent=self.project.currentAffiliationStore,
            forceDisplay=True,
        ...
        self.setAttrSelectionList(
            entry,
            "laboratories",
            "groups generating this entry",
            "currentAffiliationStore.sortedOrganisations().sortedGroups()",
            initialize,
            listParent=self.project.currentAffiliationStore,
            listParentSelect=("Organisation", "project.currentAffiliationStore.sortedOrganisations()"),
            forceDisplay=True,
            createNew="Group",
        )
        ...
        self.setAttrSelectionList(
            ...
            "currentAffiliationStore.sortedPersons()",
            initialize,
            listParent=self.project.currentAffiliationStore,
            forceDisplay=True,
            createNew="Person",
        ...
        self.setAttrPulldown(
            entry,
            "primaryCitation",
            "primary citations relevant for this entry",
            "currentCitationStore.sortedCitations()",
            initialize,
            listParent=self.project.currentCitationStore,
            forceDisplay=True,
            createNew="Citation",
        )
        self.setAttrSelectionList(
            entry,
            "otherCitations",
            "other citations relevant for this entry",
            "currentCitationStore.sortedCitations()",
            initialize,
            listParent=self.project.currentCitationStore,
            forceDisplay=True,
        ...
        self.setAttrSelectionList(
            ...
            "experiments",
            "NMR experiments",
            "currentNmrProject.sortedExperiments()",
            initialize,
            listParent=self.project.currentNmrProject,
        )
        ...
        self.setAttrSelectionList(
            ...
            "currentNmrProject.sortedStructureGenerations()",
            initialize,
            listParent=self.project.currentNmrProject,
        )
        ...
        self.setAttrSelectionList(
            entry,
            "measurementLists",
            "measurement lists (e.g. shifts)",
            "currentNmrProject.sortedMeasurementLists()",
            initialize,
            listParent=self.project.currentNmrProject,
        )
        ...
        self.setAttrSelectionList(
            entry,
            "derivedDataLists",
            "derived data lists (e.g. pKa's)",
            "currentNmrProject.sortedDerivedDataLists()",
            initialize,
            listParent=self.project.currentNmrProject,
        )
...
class PersonEditPopup(ObjectEditPopup):
    ...
    def setObjectSpecificWidgets(self, person, initialize):
        ...
        self.setAttrSelectionList(
            person,
            "personInGroups",
            "groups where this person works",
            "person.sortedPersonInGroups()",
            initialize,
            listParent=person,
            forceDisplay=True,
            createNew="PersonInGroup",
            isChild=True,
        )
        ...
        self.setAttrPulldown(
            ...
            "current group that employs this person",
            "person.sortedPersonInGroups()",
            initialize,
            listParent=person,
            forceDisplay=True,
...
class PersonCreatePopup(AffiliationObjectCreatePopup):
    ...
    def setLabels(self):
        ...
        self.labels = [("Family name :", "familyName"), ("Given name :", "givenName")]
...
class OrganisationEditPopup(ObjectEditPopup):
    ...
    def setObjectSpecificWidgets(self, organisation, initialize):
        ...
        self.setAttrSelectionList(
            ...
            "organisation.sortedGroups()",
            initialize,
            listParent=organisation,
            forceDisplay=True,
            createNew="Group",
            isChild=True,
        )
...
class CitationEditPopup(ObjectEditPopup):
    ...
    def setObjectSpecificWidgets(self, citation, initialize):
        ...
        self.setAttrSelectionList(
            ...
            "currentAffiliationStore.persons",
            initialize,
            listParent=self.project.currentAffiliationStore,
            forceDisplay=True,
            createNew="Person",
        ...
        self.setAttrSelectionList(
            citation,
            "editors",
            "editors of this citation (if any)",
            "currentAffiliationStore.persons",
            initialize,
            listParent=self.project.currentAffiliationStore,
            forceDisplay=True,
    ...
    def createSpecificObject(self):
        ...
        return CitationCreatePopup(
            self.parent, self.project, self.ccpnParent, self.objectName, metaClass=Citation.Citation._metaclass
        )
