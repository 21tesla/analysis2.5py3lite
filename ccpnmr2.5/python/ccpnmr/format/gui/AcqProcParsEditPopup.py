...
from ccpnmr.format.general.Constants import IoSetupList, ccpNmr_kw, importGeneral_kw, varianAcqOrdInfo
from ccpnmr.format.general.Io import getHelpUrlDir
from ccpnmr.format.general.Util import createPpmFreqDataSource, getAzaraBlockSizes, setDataDimRefs
...
from memops.universal.Util import isBigEndian, returnFloat, returnInt
...
class AcqProcParsEditPopup(TemporaryBasePopup):
    ...
    def __init__(self, parent, experiment=None, newFidDataSources=None):
        ...
        if self.experiment.rawData:
            ...
            self.rawDataDims = self.experiment.rawData.sortedDataDims()
        else:
            self.rawData = None
        ...
        if newFidDataSources:
            for formatName in newFidDataSources:
                if formatName not in self.freqFormats:
                    dataSource = createPpmFreqDataSource(
                        self.experiment, formatName, "processed", self.experiment.numDim
                    )
        ...
        TemporaryBasePopup.__init__(
            self,
            parent=parent,
            title="Project '%s': " % self.experiment.root.name + "Edit parameters",
            modal=False,
            transient=True,
        )
    ...
    def body(self, master):
        ...
        if self.rawData:
            ...
            button = Button(
                master,
                text="Help",
                command=lambda top=self.guiParent, url=self.help_urls["main"]: showHelpUrl(
                    self.guiParent, self.help_urls["main"]
                ),
            )
        ...
        if self.rawData:
            ...
            self.fidParsButton = ToggleButton(
                master,
        ...
        self.refParsButton = ToggleButton(
            master,
            text="Reference data",
            toggledDict=self.toggledDict,
            untoggledDict=self.untoggledDict,
  ...
  """
    ...
    def createFreqDataSourceTable(self, row, formatName, formatNames, master):
        ...
        if self.rawData and formatName in self.writeFormats:
            ...
            button = Tkinter.Button(
                master, text="Write processing files", command=(lambda: self.procFileWrite(formatName))
            )
        ...
        if len(formatNames) > 1:
            for otherFormat in formatNames:
                if otherFormat != formatName and otherFormat != importGeneral_kw:
                    button = Tkinter.Button(
                        master,
                        text="Copy data to %s" % otherFormat,
                        command=(lambda: self.setupDataSource(otherFormat, fromFormat=formatName)),
                    )
                    getattr(self, widgetsName).append([button, row, 0, self.columnsTotal])
                    row += 1
    ...
    def setupDataSource(self, toFormat, fromFormat=None):
        ...
        if dataSource:
            ...
            appData = dataSource.findFirstApplicationData(
                application=self.applicationName, keyword=self.scriptName, value=True
            )
        ...
        if not fromFormat:
            if importGeneral_kw in self.freqDataDims:
                ...
                for expDimRef in expDim.expDimRefs:
                    ...
                    if createNew:
                        dataDimRef = Nmr.DataDimRef(
                            dataDim, refPoint=refRefPoint, refValue=refRefValue, expDimRef=expDimRef
                        )
                    else:
        ...
        if toFormat == "azara":
            dataSourceItems = {"isBigEndian": isBigEndian(), "nByte": 4, "numberType": "float", "fileHeaderSize": 0}
    ...
    """
        ...
        dsFreq.addApplicationData(
            Implementation.AppDataBoolean(application=self.applicationName, keyword=self.scriptName, value=True)
        )
        ...
        if parsListKey == "fidPars":
            ...
            self.parsList["fidPars"] = [
                ...
                [
                    ...
                    lambda x=dim, value=value: setattr(
                        self.rawDataDims[x],
                        "valuePerPoint",
                        returnFloat((self.rawDataDims[x].isComplex + 1) / (2 * returnFloat(value))),
                    ),
                    allDims,
                    {},
                ],
        ...
        # TODO: truncated FT for processed
            ...
            self.parsList[nmrPipe] = [
                [
                    ...
                    allDims,
                    {
                        "entries": powerTwoList,
                        "callback": (lambda x=dim, y=dim: self.updateOwnWidget("Number of complex points for FT")),
                    },
                ],
                [
                    ...
                    Label,
                    lambda x=dim: (
                        str(self.freqDataDims[nmrPipe][x].valuePerPoint) + " " + self.freqDataDims[nmrPipe][x].unit
                    ),
                    lambda x=dim, value=value: self.void(),
                    allDims,
                    {},
                ...
                [
                    ...
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[nmrPipe][x].findFirstApplicationData(
                            application=self.applicationName, keyword="deleteImaginary"
                        ),
                        "value",
                        value,
                    ),
                ...
                [
                    ...
                    Entry,
                    lambda x=dim: (
                        self.freqDataDims[nmrPipe][x]
                        .findFirstDataDimRef(expDimRef=self.expDimRefs[x])
                        .findFirstApplicationData(application=nmrPipe, keyword="label")
                        .value
                    ),
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[nmrPipe][x]
                ...
                [
                    ...
                    Entry,
                    lambda x=dim: (
                        self.freqDataDims[nmrPipe][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]).refPoint
                    ),
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[nmrPipe][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]),
                        "refPoint",
                        returnFloat(value),
                    ),
                    allDims,
                ...
                [
                    ...
                    Entry,
                    lambda x=dim: (
                        self.freqDataDims[nmrPipe][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]).refValue
                    ),
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[nmrPipe][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]),
                        "refValue",
            ...
            self.parsList[azara] = [
                ...
                [
                    ...
                    Label,
                    lambda x=dim: (
                        str(self.freqDataDims[azara][x].valuePerPoint) + " " + self.freqDataDims[azara][x].unit
                    ),
                    lambda x=dim, value=value: self.void(),
                    allDims,
                    {},
                ...
                [
                    ...
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[azara][x].findFirstApplicationData(
                            application=self.applicationName, keyword="deleteImaginary"
                        ),
                        "value",
                        value,
                ...
                [
                    ...
                    Entry,
                    lambda x=dim: (
                        self.freqDataDims[azara][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]).refPoint
                    ),
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[azara][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]),
                        "refPoint",
                        returnFloat(value),
                    ),
                    allDims,
                ...
                [
                    ...
                    Entry,
                    lambda x=dim: (
                        self.freqDataDims[azara][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]).refValue
                    ),
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[azara][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]),
                        "refValue",
            ...
            self.parsList[generic] = [
                ...
                [
                    ...
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[generic][x],
                        "valuePerPoint",
                        returnFloat(
                            (2 * returnFloat(value))
                            / (
                                (self.freqDataDims[generic][x].isComplex + 1)
                                * self.freqDataDims[generic][x].numPointsOrig
                            )
                        ),
                    ),
                    allDims,
                ...
                [
                    ...
                    Label,
                    lambda x=dim: (
                        str(self.freqDataDims[generic][x].valuePerPoint) + " " + self.freqDataDims[generic][x].unit
                    ),
                    lambda x=dim, value=value: self.void(),
                    allDims,
                    {},
                ...
                [
                    ...
                    Entry,
                    lambda x=dim: (
                        self.freqDataDims[generic][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]).refPoint
                    ),
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[generic][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]),
                        "refPoint",
                        returnFloat(value),
                    ),
                    allDims,
                ...
                [
                    ...
                    Entry,
                    lambda x=dim: (
                        self.freqDataDims[generic][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]).refValue
                    ),
                    lambda x=dim, value=value: setattr(
                        self.freqDataDims[generic][x].findFirstDataDimRef(expDimRef=self.expDimRefs[x]),
                        "refValue",
    ...
    def getDeleteImaginary(self, format, x):
        ...
        deleteImaginary = self.freqDataDims[format][x].findFirstApplicationData(
            application=self.applicationName, keyword="deleteImaginary"
        )
        ...
        if deleteImaginary:
            ...
            value = True
            self.freqDataDims[format][x].addApplicationData(
                Implementation.AppDataBoolean(application=self.applicationName, keyword="deleteImaginary", value=value)
            )
    ...
    def setupObjectsTable(self, master, parsListKey):
        ...
        return ObjectsTable(
            master,
            objects=self.objects[parsListKey],
  ...
  """
    ...
    def selectFile(self):
        ...
        format = string.lower(self.experiment.rawData.dataStore.fileType)
  ...
  """
    ...
    def parameterUpdate(self, *args):
        ...
        if self.rawData and "azara" in self.parsList:
    ...
    def getSensEnhanceMethod(self):
        ...
        if self.experiment.rawData.dataStore.fileType == "Varian":
            method = "ranceKay"
        else:
            method = "echoAntiEcho"
