...
class GuiMultiDialog(MultiDialog):
    ...
    def waitWindow(self, guiParent, popup, kw=None):
        ...
        if kw and ( "modal" not in kw or kw["modal"] == False):
            waitStatus = True
    ...
    def chainExportFunction(self, guiParent, requireChainCode):
        ...
        popup = ChainExportPopup(guiParent, self.ccpChainLabelDict, self.ccpChainSeqIdCodes, requireChainCode)
    ...
    def ResonanceSelectFunction(self, guiParent, resName, multi, title):
        ...
        return ResonanceSelectPopup(
            guiParent, self.resonanceList, self.resonanceDict, self.ccpCode, resName, self.infoText, multi, title=title
        )

    def ResonanceGroupFunction(self, guiParent, title):

        return ResonanceGroupPopup(guiParent, self.resonanceNameList, self.resonanceLineDict, self.ccpCode, title=title)
    ...
    def MeasurementSelectFunction(self, guiParent):
        ...
        popup = SelectionListPopup(
            ...
            topText=self.topMessage,
            text=self.question,
            urlFile="MeasurementSelect",
            selectionDict=self.measurementValueDict,
            dismissButton=False,
        )
