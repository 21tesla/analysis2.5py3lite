...
class ProchiralStatusPopup(TemporaryBasePopup):
    ...
    def __init__(self, parent, chainList, chainAtomSetsDict):
        ...
        for resParent in self.chainAtomSetsDict.keys():
            ...
            for chain in self.chainList:
                ...
                if chain in self.chainAtomSetsDict[resParent] and self.chainAtomSetsDict[resParent][chain]:
                    chainLabel = "Molecular system '%s':Chain '%s' (molecule '%s')" % (
                        chain.molSystem.code,
                        chain.code,
                        chain.molecule.name,
                    )
        ...
        TemporaryBasePopup.__init__(
            self,
            parent=parent,
            title="Project '%s': " % self.project.name + "Set status prochiral atoms/resonances",
            modal=False,
            transient=True,
        )
    ...
    def body(self, master):
        ...
        self.resParentSelect = PulldownMenu(
            master, entries=self.resParentList, callback=self.setupResParentList, do_initial_callback=False
        )
        ...
        for i in range(0, len(self.chainAtomSetsDict[self.resParent][chain])):
            ...
            if hasResonances and isValidCase:
                ...
                checkButton = CheckButton(
                    frame,
                    selected=isStereo,
                    callback=lambda status=isStereo, fr=frameRow, item=i, setup=False: self.toggleResonances(
                        status, fr, item, setup
                    ),
                )
                ...
                self.resonanceObjects[frameRow] = {True: [[], []], False: [[], []]}
                ...
                resonanceList = PulldownMenu(
                    ...
                    callback=lambda selInd=x, selItem=y, fr=frameRow, item=i: self.setProchiralLabels(
                        selInd, selItem, fr, item
                    ),
    ...
    def setProchiralLabels(self, selInd, selItem, fr, item):
        ...
        self.chainResonancesDict[self.resParent][self.chain][item][2] = not self.chainResonancesDict[self.resParent][
            self.chain
        ][item][2]
    ...
    def toggleResonances(self, status, frameRow, item, setup):
        ...
        if not setup:
            ...
            self.chainResonancesDict[self.resParent][self.chain][item][1] = not self.chainResonancesDict[
                self.resParent
            ][self.chain][item][1]
    ...
    def setStereo(self, chain=None):
        ...
        if self.chainResonancesDict[self.resParent][chain]:
            for i in range(0, len(self.chainResonancesDict[self.resParent][chain])):
                atomSetGroup = self.chainAtomSetsDict[self.resParent][chain][i]
                (stereoStatus, stereoStatusChanged, resChanged, resonances) = self.chainResonancesDict[self.resParent][
                    chain
                ][i]
                ...
                if stereoStatusChanged or (stereoStatus and resChanged):
                    ...
                    if stereoStatusChanged:
                        ...
                        print(
                            "  Warning: Changed '%s.%d' prochiral resonances for atoms %s from %s"
                                ...
                                if not resonances[0]:
                                    ...
                                    if isinstance(self.resParent, NmrConstraint.NmrConstraintStore):
                                        newResonanceSet = self.resParent.newFixedResonanceSet(
                                            resonances=[resonances[1]], atomSets=[atomSetGroup[1]]
                                        )
                                    else:
                        ...
                        print(
                            "  Warning: Changed resonance assignment for '%s.%d' stereospecifically assigned atoms %s."
