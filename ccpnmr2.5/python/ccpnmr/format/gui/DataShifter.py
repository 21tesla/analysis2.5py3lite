...
from ccpnmr.format.gui.ImportExportFormatPopup import getCcpnObjects, getIoInfo, setIoInfo
...
copyOrderDict = {
    ...
    "MolSystem": [
        ...
        ("chainInteractions", 1),
        ("chains.residues", 0),
        ("chains.chainFragments", -1),
        ("chains.residues.atoms", 0),
        ("chains.residues.molSystemLinkEnds", 1),
        ("molSystemLinks", 1),
    ],
    "Method": [("parameters", 1)],
    ...
    "NmrConstraintStore": [
        ...
        ("violationLists", 1),
        ("constraintLists", 1),
        ("constraintLists.constraints", 1),
    ],  # ('fixedResonances',1),('constraintLists.constraints.resonances',1)
    "NumericMatrix": [("dataUrl", -1)],
    "Organisation": [("groups", 1)],
    "PeakList": [
        ("peaks", 1),
        ("peaks.peakIntensities", 1),
        ("peaks.peakDims", 0),
        ("peaks.peakContribs", 1),
        ("peaks.peakDims.peakDimContribs", 1),
        ("peaks.peakDims.peakDimComponents", 1),
    ],
...
class CopyObjectData:
    def __init__(
        ...
        dialogInfo,
        toLinkedObjectData,
        createdObjectDict=None,
        ignorePresets=False,
        ignoreAttrs=None,
        extendedCopy=True,
        verbose=False,
        postponedObjects=None,
    ):
        ...
        if ccpnObject:
            ...
            if self.objectClassName in copyOrderDict:
                self.copyObject()
            else:
    ...
    def copyObject(self):
        ...
        self.toObject = createInProject(
            ...
            self.toProjectResInfo,
            self.dialogInfo,
            self.toLinkedObjectData,
            postponedObjects=self.postponedObjects,
            ignorePresets=self.ignorePresets,
            verbose=self.verbose,
            ignoreAttrs=self.ignoreAttrs,
        )
    ...
    def initializeParent(self):
        ...
        if self.parentClassName not in self.createdObjectDict:
            self.setParent()

    def setParent(self):

        addClassToCreatedObjectDict(self.createdObjectDict, self.parentClassName)
        addToCreatedObjectDict(
            self.createdObjectDict[self.parentClassName], self.ccpnObject.parent, self.parentToObject
        )
        ...
        if self.extendedCopy:
            for dataSource in self.ccpnObject.dataSources:
                CopyDataSource(
                    ...
                    self.dialogInfo,
                    self.toLinkedObjectData,
                    createdObjectDict=self.createdObjectDict,
                    verbose=self.verbose,
                )
...
class CopyDataSource(CopyNmrProjectData):
    ...
    def setSpecificInfo(self):
        ...
        if self.extendedCopy:
            ...
            if selectionList and showYesNo(
                "Copy peakLists",
                "Do you also want to copy the peakList(s) associated with dataSource '%d:%s'?"
                % (self.ccpnObject.serial, self.ccpnObject.name),
            ):
...
class CopyPeakList(CopyNmrProjectData):
    def initializeParent(self):
        ...
        if not experiment:
            ...
            if selectionList and not showYesNo(
                "New experiment",
                "Do you also want to create a new NMR experiment from scratch? Press No if you want to link to an existing one.",
            ):
                ...
                if interaction.isSelected:
                    ...
                    if experiment:
                        ...
                        addToCreatedObjectDict(
                            self.createdObjectDict["Experiment"], self.ccpnObject.dataSource.experiment, experiment
                        )

                        expCopyOrder = copyOrderDict["Experiment"]
                        createInProject(
                            self.ccpnObject.dataSource.experiment,
                            self.toProject,
                            expCopyOrder,
                            self.createdObjectDict,
                            self.toProjectResInfo,
        ...
        if not isCreated or (experiment and not experiment.dataSources):
            ...
            if not dataSource:
                ...
                if selectionList and not showYesNo(
                    "New dataSource",
                    "Do you also want to create a new dataSource from scratch? Press No if you want to link to an existing one.",
                ):
                    interaction = self.dialogInfo[1].SelectionList(
                        self.dialogInfo[0],
                        selectionList,
                    ...
                    if interaction.isSelected:
                        ...
                        if dataSource:
                            ...
                            addToCreatedObjectDict(
                                self.createdObjectDict["DataSource"], self.ccpnObject.dataSource, dataSource
                            )
...
def createInProject(
    ...
    for linkName, doCopy in copyOrder:
        ...
        for i in range(len(ccpnObjectStrings)):
            ...
            sortedLinkName = "sorted%s%s" % (
                ccpnObjectStrings[i : i + 1][0][0].upper(),
                ccpnObjectStrings[i : i + 1][0][1:],
            )
            ...
            for tempCcpnObject in ccpnObjects:
                try:
                    ...
                    try:
                        ...
                        print(
                            "Warning: object %s does not have attribute/link %s... ignored."
                            % (tempCcpnObject.className, ccpnObjectStrings[i])
                        )
...
def getMetaInfo(ccpnObject):
    ...
    for metaRole in metaClass.getAllRoles():
        if (
            not metaRole.isDerived
            and not metaRole.hierarchy == "child"
            and not metaRole.hierarchy == "parent"
            and not metaRole.isImplementation
        ):
      ...
      """
            ...
            for refObject in otherObjects:
                if refObject.className == "Method":
                    ...
                    setPresetLinks(
                        refObject._metaclass.getElement("software"),
                        [refObject.software],
                        toProject,
                        {},
                        createdObjectDict,
                ...
                if createdObjectDict == None:
                    createdObjectDict = {}
                if "Project" not in createdObjectDict:
                ...
                newObject = mapObject(
                    ...
                    dialogInfo,
                    toLinkedObjectData,
                    forceAttrOrRoles=forceAttrOrRoles,
                    verbose=verbose,
                )
                ...
                if newObject:
                    ...
                    CopyClass(
                        ...
                        dialogInfo,
                        toLinkedObjectData,
                        createdObjectDict=createdObjectDict,
                        ignorePresets=ignorePresets,
                        verbose=verbose,
                    )
            ...
            if "Project" not in createdObjectDict:
                addClassToCreatedObjectDict(createdObjectDict, "Project")
                addToCreatedObjectDict(createdObjectDict["Project"], refObject.root, toProject)
            if refObject.parent.className not in createdObjectDict:
            ...
            if resClassName not in createdObjectDict:
                    ...
                    if resonanceSet:
                        for atomSet in resonanceSet.atomSets:
                            ...
                            for i in range(4):
                                ...
                                if linkedObject and linkedObject.className in createdObjectDict:
                                    if linkedObject in createdObjectDict[linkedObject.className]["fromObjects"]:
                                        refIndex = createdObjectDict[linkedObject.className]["fromObjects"].index(
                                            linkedObject
                                        )
                ...
                for resonance in checkResonances:
                    ...
                    if resonanceSet:
                        for atomSet in resonanceSet.atomSets:
                            ...
                            for atom in atomSet.atoms:
                                ...
                                if fromResidue in fromObjectDict["Residue"]:
                            ...
                            if toAtomList == toResonanceAtoms[tempToResonance]:
                                ...
                                if toResonance not in createdObjectDict[resonance.className]["toObjects"]:
                                    if verbose:
                                        ...
                                        print("    ", toAtomList)
                                    addToCreatedObjectDict(
                                        createdObjectDict[resonance.className], resonance, toResonance
                                    )
                                    break
                    ...
                    if not toResonance:
                        ...
                        if toAtomList:
                            ...
                            toResonanceSet = createInProject(
                                resonanceSet,
                                toProject,
                                [],
                                createdObjectDict,
                            ...
                            if resonanceSet:
                                ...
                                for atomSet in resonanceSet.atomSets:
                                    ...
                                    resNames.append(
                                        getResName(
                                            chainCode, residue.seqCode, atomName, seqInsertCode=residue.seqInsertCode
                                        )
                                    )

                                # Specific Analysis case...
                                # TODO need to put these strings in ccpnmr.general.Constants or something! Shared...
                                analysisAppData = resonance.findFirstApplicationData(
                                    application="Analysis", keyword="ambigProchiralLabel"
                                )
                                ...
                                for resName in resNames:
                                    ...
                                    if not toResonance.findFirstApplicationData(application="ccpn", keyword=assign_kw):
                                        ...
                                        toResonance.addApplicationData(
                                            Implementation.AppDataString(
                                                application="ccpn", keyword=assign_kw, value=resName
                                            )
                                        )
                                        break

                                if not toResonance.parent.findFirstApplicationData(keyword=newResonances_kw):
                                    toResonance.parent.addApplicationData(
                                        Implementation.AppDataBoolean(
                                            application="ccpn", keyword=newResonances_kw, value=True
                                        )
                                    )
...
def mapObject(
    ...
    for metaAttrOrRole in metaInfo[1] + metaInfo[2]:
        ...
        if (
            metaClass.name not in autoCreatedClasses
            and metaAttrOrRole.locard >= 1
            and metaAttrOrRole.name not in keyNames
        ):
    ...
    for keyName in keyNames:
        ...
        if (keyName, linkedData) not in fromData:
    ...
    if toParent:
        ...
        for toLinkedObject in getattr(toParent, linkName):
            ...
            if (toParent, linkName, toLinkedObject in toLinkedObjectData):
                ...
                for keyName in keyNames:
                    ...
                    if (keyName, linkedData) not in toData:
            ...
            if toData == fromData:
                ...
                if ccpnObject.className in confirmMappingClasses.keys():
                    ...
                    if not dialogInfo[2].showYesNo(
                        "Mapping to existing object",
                        "%s object with %s '%s' is being mapped to '%s' in the other project. Is this mapping correct?"
                        % (className, attrName, str(fromAttrValue), str(toAttrValue)),
                    ):
                        doMapping = False
                ...
                if doMapping:
                    if metaClass.name not in createdObjectDict:
                            ...
                            if not existingValue:
                                ...
                                if value != None:
                                    ...
                                    try:
                                        ...
                                        if verbose:
                                            print(
                                                "  mapObjects set %s.%s to '%s'"
                                                % (objectMapped.className, attrOrRoleName, str(value))
                                            )
                                    except:
                                        print(
                                            "  Error: mapObjects cannot set %s for %s... API error '%s'"
                                            % (
                                                attrOrRoleName,
                                                objectMapped.className,
                                ...
                                print(
                                    "  Warning: mapObjects NOT setting %s.%s, is '%s'"
                                    % (objectMapped.className, attrOrRoleName, str(existingValue))
                                )
...
def getToProjectObject(fromObject, createdObjectDict):
    ...
    if metaClassName in createdObjectDict:
...
def copyObject(
    ...
    for metaRole in metaInfo[2]:
        ...
        if metaClass.name != otherMetaClassName:  # Have to postpone links to same class.
            for otherObject in otherObjects:
                if otherMetaClassName in createdObjectDict:
            ...
            for otherIndex in range(len(otherObjects)):
                if not validToProjectObjects[otherIndex]:
                    ...
                    if newObjects:
                        ...
                        if otherMetaClassName not in createdObjectDict:
                            addClassToCreatedObjectDict(createdObjectDict, otherMetaClassName)

                        if otherObject not in createdObjectDict[otherMetaClassName]["fromObjects"]:
        ...
        if roleName not in keywds:
            linkValue = getattr(ccpnObject, roleName)

            if linkValue:
                if ccpnObject not in postponedLinkDict:
                    postponedLinkDict[ccpnObject] = {}

                if roleName not in postponedLinkDict[ccpnObject]:
    ...
    if not newObject:
        ...
        if newParent:
            try:
                if ccpnObject.className in confirmMappingClasses.keys():
                    ...
                    while getattr(newParent, "findFirst%s" % searchClass)(**searchKwd):
                        attrValue = dialogInfo[3].askString(
                            "Enter %s %s" % (className, attrName),
                            "Enter a new %s for the copy of the %s in the other project:" % (attrName, className),
                            attrValue,
                            dialogInfo[0],
                        )
            ...
            print(
                "  ERROR: copyObjects could not create new %s... parent missing in other project."
                % ccpnObject.className
            )
            print(objectParent)
...
class DataShifter(BasePopup):
    ...
    def __init__(self, parent, project1=None, project2=None, testMode=False, verbose=False):
        ...
        self.widgetDefs = [
            ...
            [
                ...
                "molSystem",
                "Select a molecular system to copy:",
            ...
            [
                "SelectionListWithNone",
                "measurementList",
                "Select measurement list to copy:",
                "project.currentNmrProject.sortedMeasurementLists()",
                None,
            ],
            ["Separator", None, None, None, None],
            [
                ...
                "Select Structure Generation to copy:",
            ...
            [
                "SelectionListWithNone",
                "Constraint set",
                "Select NmrConstraintStore to copy:",
                "project.currentNmrProject.sortedNmrConstraintStores()",
                None,
            ],
            ["Separator", None, None, None, None],
            ...
            [
                ...
                "project.currentMethodStore.sortedSoftware()",
                None,
            ],
            [
                "SelectionListWithNone",
                "organisations",
                "Select organisation to copy:",
                "project.currentAffiliationStore.sortedOrganisations()",
                None,
            ],
            [
                "SelectionListWithNone",
                "persons",
                "Select person to copy:",
                "project.currentAffiliationStore.sortedPersons()",
                None,
            ],
            [
                "SelectionListWithNone",
                "citations",
                "Select citation to copy:",
                "project.currentCitationStore.sortedCitations()",
                None,
            ],
            ["Separator", None, None, None, None],
            [
    ...
    def body(self, master):
        ...
        for projectNum, colNum in ((1, 0), (2, 3)):
            ...
            widget = Tkinter.Button(
                ...
                command=lambda projectId=projectId: self.selectProjectDir(projectId=projectId),
            )
            ...
            for widgetIndex in range(len(self.widgetDefs)):
                ...
                (label, widget, selectionDict) = setIoInfo(
                    master,
                    widgetInfo,
                    project,
                    (projectId, "Project", self.selectFile, None),
                    forceCreation=True,
                    appendNone=True,
                )
            ...
            button = Tkinter.Button(
                master, text="Save %s" % projectId, command=lambda projectId=projectId: self.saveProject(projectId)
            )
        ...
        for widgetIndex in range(len(self.widgetDefs)):
            ...
            for shiftCol, shiftLabelText in ((1, "<-"), (2, "->")):
                button = Tkinter.Button(
                    master,
                    text=shiftLabelText,
                    command=lambda buttonKeyword=buttonKeyword, toProjectNum=shiftCol: self.shiftData(
                        buttonKeyword, toProjectNum
                    ),
                    state=buttonState,
                )
        ...
        if project:
            try:
                ...
                try:
                    ...
                    showError(
                        "Project could not be saved", "The project could not be saved. Sorry - try again from scratch."
                    )
                    print(traceback.format_exception_only(sys.exc_type, sys.exc_value))
            except:
                showError(
                    "Project not valid",
                    "The project is not valid and will not be saved. Sorry - try again from scratch.",
                )
                print(traceback.format_exception_only(sys.exc_type, sys.exc_value))
    ...
    def shiftData(self, buttonKeyword, toProjectNum):
        ...
        ccpnObject = getIoInfo(
            self.widgets[fromProjectId][buttonKeyword],
            setupInfo,
            getattr(self, fromProjectId),
            self.widgetInfo[fromProjectId][buttonKeyword],
        )
            ...
            try:
                ...
                for resonanceParent in toProject.findAllNmrProjects().union(toProject.findAllNmrConstraintStores()):
                    ...
                    if newResonancesFlag and toProject.molSystems:
                        if showYesNo(
                            "Run linkResonances", "Unlinked resonances are present - do you want to link them now?"
                        ):
    ...
    def selectProjectDir(self, projectId):
        ...
        popup = OpenProjectPopup(
            self, callback=lambda project, projectId=projectId: self.initProject(project, projectId=projectId)
        )
