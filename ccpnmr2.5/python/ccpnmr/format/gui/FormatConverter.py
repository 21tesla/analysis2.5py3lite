...
from ccpnmr.format.general.Constants import IoSetupList, allFormatsDict, formatComponents
from ccpnmr.format.general.Io import TextPipe, getHelpUrlDir
...
from ccpnmr.format.gui.ImportExportFormatPopup import ExportFormatPopup, ImportFormatComponentPopup, ImportFormatPopup
...
class FormatConverter(BasePopup):
  ...
  """
                ...
                for formatName in self.importFormats:
                    ...
                    for infoList in IoSetupList:
                        if compName == infoList[0]:
                            ...
                            if formatName in fileDict.keys():
                                if fileDict[formatName][1]:
                                    sub_menu_item["submenu"].append(
                                        {
                                            "kind": "command",
                                            "label": formatLabel,
                                            "command": lambda format=formatName, comp=compName: self.importExportFormat(
                                                "import", format, comp
                                            ),
                                        }
                                    )
    ...
    def setSetupMenu(self):
        ...
        self.menu_items[SetupMenu] = [
            "Edit default experiment types",
    ...
    def setOtherMenu(self):
        ...
        self.menu_items[OtherMenu] = ["General Editor", "Create/edit Entry", "Edit acq/proc pars"]
  ...
  """
    ...
    def importExportFormat(self, action, format, component):
        ...
        if action == "import":
            if component:
                self.popups[action + format + component] = ImportFormatComponentPopup(
                    self, format, threading=self.threading, component=component
                )
    ...
    def cleanAppData(self):
        ...
        if showYesNo(
            ...
            "Remove all FormatConverter related application data? This will reduce the size of your project, but all information on previous import/export to external formats will be lost.",
        ):
    ...
    def cleanResonanceAppData(self):
        ...
        if interaction.isSelected:
            ...
            if showYesNo(
                "Clean resonance application data",
                "Remove all FormatConverter related Resonance linked application data for format %s?" % formatName,
            ):
...
if __name__ == "__main__":
    ...
    if inputArgs:
        ...
        askdir = lambda title, prompt, initial_value, default_dir: askDir(
            title, prompt, initial_value, parent=root, extra_dismiss_text="Skip", default_dir=default_dir
        )
        askfile = lambda title, prompt, initial_value: askFile(
            title, prompt, initial_value, parent=root, extra_dismiss_text="Skip"
        )
