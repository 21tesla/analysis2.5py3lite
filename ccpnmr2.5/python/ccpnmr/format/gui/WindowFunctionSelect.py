    ...
    def __init__(self, parent, format, dim, getValue):
        ...
        self.windowFunctions[self.azara] = [
            ...
            [
                "gaussian (fixed)",
                "gaussian",
                [["Total points fraction for max", 0.25, "frac"], ["End value", 1, "end"]],
            ],
            [
                "gaussian",
                "gaussian_sw",
        ...
        self.windowFunctions[self.nmrPipe] = [
            [
                "sine",
                "SP",
                [["Start sine at (pi)", 0, "off"], ["End sine at (pi)", 1, "end"], ["Sine to power", 1, "pow"]],
            ],
            ["exponential", "EM", [["Line broadening (Hz)", 10, "lb"]]],
            [
                "lorentz-gauss",
                "GM",
                [["Inverse exp width (Hz)", 0, "g1"], ["Gaussian width (Hz)", 0, "g2"], ["Center (0-1)", 0, "g3"]],
            ],
            ...
            [
                "triangle",
                "TRI",
                [["Point loc of apex", 0, "loc"], ["Left edge height", 0, "lHi"], ["Right edge height", 0, "rHi"]],
            ],
        ]
    ...
    def getApplDataAndReturnString(self):
        ...
        for argName, argValue, argShort in self.windowFunctions[self.format][self.winFuncIndex][2]:
            applData = self.freqDataDim.findFirstApplicationData(
                application=self.format, keyword=keyword + winFuncName + argShort
            )
