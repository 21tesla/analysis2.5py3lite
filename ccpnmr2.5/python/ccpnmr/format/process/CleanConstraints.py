...
class CleanConstraints:
    ...
    def cleanAllDistanceConstraintLists(self):
        ...
        for strucGen in self.project.currentNmrProject.findAllStructureGenerations():
            for constraintList in strucGen.nmrConstraintStore.findAllConstraintLists(
        ...
        while i < len(constraints):
            ...
            while j < len(constraints):
                if refConstraint == constraints[j]:
                    ...
                    print(
                        "  Constraint %s (items %s) is duplicate of %s - deleted"
                        % (constraintRefs[j].serial, items, constraintRefs[i].serial)
                    )
                    ...
                    for otherDihedralItem in constraintRefs[j].items:
                        ...
                        for dihedralItem in constraintRefs[i].items:
                            if (
                                otherDihedralItem.upperLimit == dihedralItem.upperLimit
                                and otherDihedralItem.lowerLimit == dihedralItem.lowerLimit
                            ):
                                itemExists = 1
                                break
                    ...
                    for attribute in ["upperLimit", "targetValue", "lowerLimit"]:
                        if getattr(constraintRefs[i], attribute) and getattr(constraintRefs[j], attribute):
                            setattr(
                                constraintRefs[i],
                                attribute,
                                min(getattr(constraintRefs[i], attribute), getattr(constraintRefs[j], attribute)),
                            )
