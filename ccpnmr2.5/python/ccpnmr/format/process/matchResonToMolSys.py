...
def doMatchResidues(
    bestMatch, comparisonList, comparisonListIndex, residues, resResidues, resResidueDictForChain, fixMatch, test
):

    (residueCodes, atomNames, matchScore, scoreMultiplier) = comparisonList[comparisonListIndex]

    (bestFit, correctFit) = matchResidues(
        residueCodes, residues, atomNames, resResidues, resResidueDictForChain, matchScore=matchScore, test=test
    )
    ...
    if correctFit[0] == None or correctFit[1] == None:
        ...
        if len(comparisonList) > comparisonListIndex:
            (correctFit, bestMatch) = doMatchResidues(
                bestMatch,
                comparisonList,
                comparisonListIndex,
                residues,
                resResidues,
                resResidueDictForChain,
                fixMatch,
                test,
            )
...
def matchResonToMolSys(resonances, molSystem, assignFormat=None, test=False, matchMode=False):
    ...
    if (
        not resonances
        or (type(resonances) not in (type(("",)), type([])))
        or not (isinstance(resonances[0], Nmr.Resonance) or isinstance(resonances[0], NmrConstraint.FixedResonance))
    ):
        ...
        if not resonanceLinked:
            ...
            for resName in resNames:
                ...
                if seqCode != None:
                    ...
                    if chainCode not in resChainDict:
...
def matchToMolSys(resChainDict, resResidueDict, molSystem, test=0, matchMode=0):
    ...
    for resChainCode in resChainDict.keys():
        ...
        if len(resChainDict[resChainCode]) > 1:
            ...
            while seqCodeKey != resChainDict[resChainCode][-1]:
                ...
                if seqCodeKey[0] == (oldSeqCodeKey[0] + 1) or (
                    seqCodeKey[0] == oldSeqCodeKey[0] and seqCodeKey[1] != oldSeqCodeKey[1]
                ):
                    pass
                else:
        ...
        for seqCodeKey in resChainDict[resChainCode]:
            if seqCodeKey in resResidueDict[resChainCode]:
    ...
    for chainCode in chainCodes:
        ...
        for resChainCode in resChainCodes:
            ...
            for seqAttr in ("seqId", "seqCode"):
                if (
                    getattr(residues[0], seqAttr) == resResidues[0][0]
                    and residues[0].seqInsertCode == resResidues[0][1]
                    and getattr(residues[-1], seqAttr) == resResidues[-1][0]
                    and residues[-1].seqInsertCode == resResidues[-1][1]
                ):
        ...
        if len(resChainsMapped) != len(resChainCodes):
            ...
            for resChainCode in resChainCodes:
                if resChainCode not in resChainsMapped:
                    ...
                    (match, bestMatch) = doMatchResidues(
                        (None, None, -999),
                        comparisonList,
                        0,
                        residues,
                        resResidues,
                        resResidueDict[resChainCode],
                        0,
                        test,
                    )
    ...
    for chainCode, resChainCode in resMapDictKeys:
        ...
        if chainCode in chainDict:
            del chainDict[chainCode]
        if resChainCode in resChainDict:
...
def printMissingInfo(resResidueDict):
    ...
    for resChainCode in resChainCodes:
        ...
        for resSeqCode in resSeqCodes:
            ...
            if scoreTexts:
                print(
                    "      %-4s%s:" % (str(resSeqCode[0]), resSeqCode[1]),
                )
...
def complexChainMapping(
    chainDict, resChainDict, resResidueDict, unmappedCodes, resChainCodes, chainMappingDict, test=0, matchMode=0
):
    ...
    if test:
        ...
        for mol in molecules:
            print(
                " %s (%d)," % (mol.name, len(mol.molResidues)),
            )
        print()
            ...
            #
                ...
                for tempResChainCode in tempResChainCodes:
                    ...
                    (match, bestMatch) = doMatchResidues(
                        ...
                        residues,
                        resResidues,
                        resResidueDict[tempResChainCode],
                        1,
                        test,
                    )
                    (resId, offset, score) = match
                    ...
                    if matchMode:
                        ...
                        print(
                            "  Best match for CCPN code '%s' to code '%s': %s"
                            % (tempChainCodes[0], tempResChainCode, matchText)
                        )
...
def matchResidues(resType, residues, atomNames, resResidues, resAtomDict, matchScore=10, test=0):
    ...
    if test:
        print(
            "  endMatch %5d %-20s %5d %5d (%5d,%5d)"
            % (bestMatch, str(bestFit[:2]), bestMatchLen, bestResStart, moleculeMapLen, resonanceMapLen)
        )
    ...
    if bestMatch > min(moleculeMapLen, resonanceMapLen):
        if test:
            print(
                "  Resetting, bestMatch %d, from mininum of %d, %d (map lengths for molecule and resonance)"
                % (bestMatch, moleculeMapLen, resonanceMapLen)
            )
