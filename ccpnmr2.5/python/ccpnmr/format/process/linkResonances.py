...
from ccp.general.Util import (
    ...
    getDescriptorDict,
    getDescriptorFromDict,
    mergeDescriptorDicts,
    setUniqueAppData,
)
...
from ccpnmr.format.general.userInteraction import setupMessageReporter, setupMultiDialog
from ccpnmr.format.general.Util import (
    ...
    getStereoCode,
    returnProchiralAtom,
    returnProchiralAtomGroup,
    setCcpNmrResonanceName,
    updateResonanceNamesDict,
)
...
class ResidueAtomToResName:
    ...
    def setAllAtomGroups(self):
        ...
        for nmrAtom in self.nmrAtoms:
            ...
            if chemAtomOrSetName not in self.allAtomGroups:
...
class AtomToResName:
    ...
    def setNmrAtomGroups(self):
        ...
        for nmrAtom in self.nmrAtoms:
            ...
            if chemAtomSet:
                if chemAtomSet not in tempDict:
...
class linkResonances(TopShared):
    ...
    keywordInfo = {
        ...
        "makeMethylsEquivalent": (
            True,
            False,
            """
        True:   Methyl protons are equivalent, e.g. make HG1* if only HG11 given for valine.
        False:  Methyl portons are not equivalent, e.g. in ILV labelling experiments.""",
        ),
        "setSingleProchiral": (
            None,
            False,
        ...
        Example: [['A','B',1,4]] maps the CCPN chain 'A' to the external format chain 'B'. Residue 1 of the CCPN chain maps to sequence code 5 (1+4) for the relevant chain in the external format.""",
        ),
    ...
    tempNamingSystemNames = ("PDB_REMED", "IUPAC", "MSD", "PDB")

    checkProtonationStates = {
        #'Asn': ('HD21','HD22'),
        "Asp": ("HD2",),
        #'Gln': ('HE21','HE22'),
        "Glu": ("HE2",),
    ...
    def run(self, project, keywds):
        ...
        if not project.molSystems:
            self.messageReporter.showError(
                "Error", "Unable to link resonances: need project with molecule data!", parent=self.guiParent
            )
            self.status = False
            return
        ...
        if not self.nmrConstraintStore and not self.nmrProject:
            raise self.LinkResonancesError(
                "No nmrConstraintStore or nmrProject passed in, and no currentNmrProject available. Aborting!"
            )
        ...
        if not self.resUnassignedNameList:
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning", "No unassigned resonances... aborting linkResonances", parent=self.guiParent
                )
            return False
        ...
        if not self.setChainLinks(forceChainMappings=self.forceChainMappings):
            self.messageReporter.showWarning(
                "Warning", "No chain selected... aborting linkResonances", parent=self.guiParent
            )
            return False
        ...
        if not self.matchNamingSystems():
            self.messageReporter.showWarning(
                "Warning", "No naming system selected... aborting linkResonances", parent=self.guiParent
            )
            return False
    ...
    def setKeywords(self, keywds, verbose=True):
        ...
        for keyword in self.keywordInfo.keys():
            ...
            if mandatory and keyword not in keywds:
    ...
    def getStatusOtherProchiral(self, atomSets, resName):
        ...
        if self.setSingleProchiral == None:
            ...
            if self.useSinglePossEquivMapping:
                if resStatusMapping and resStatusMapping.isSingleProchiral != None:
                    ...
                    if self.verbose:
                        print(
                            "  Set single prochiral status for '%s' to %d from mapping."
                            % (resName, singleProchiralStatus)
                        )
        ...
        if self.verbose:
            if singleProchiralStatus == 0:
                print(
                    "  Adding resonance for prochiral atom/group %s (from residue %s %d, resonance %s)..."
                    % (otherAtomName, ccpCode, seqCode, self.resonance.name)
                )

            elif singleProchiralStatus == 1:
                print(
                    "  Setting resonance %s (%s %d) to non-stereospecifically assigned..."
                    % (self.resonance.name, ccpCode, seqCode)
                )
    ...
    def getStatusResonance(self, refAtom, otherAtomName, resName):
        ...
        if self.setSinglePossEquiv == None:
            ...
            if self.useSinglePossEquivMapping:
                if resStatusMapping and resStatusMapping.isSinglePossEquiv != None:
                    ...
                    if self.verbose:
                        print(
                            "  Set single possible equivalent status for '%s' to %d from mapping."
                            % (resName, singleResonanceStatus)
                        )
        ...
        if self.verbose and singleResonanceStatus == 0:
            print(
                "  Changing status to equivalent for aromatic/amide atom %s (from residue %s %d)..."
                % (otherAtomName, ccpCode, seqCode)
            )
            ...
            if resonanceSet:
                for atomSet in resonanceSet.atomSets:
                    ...
                    for atom in atomSet.atoms:
                        if atom not in self.atomResonances:
            ...
            if not resonanceLinked:
                    ...
                    if self.specificResNameMappings:
                        if resName in self.specificResNameMappings:
                            ...
                            for newResName in newResNames:
                                if newResName not in self.resUnassignedNames:
                ...
                for resName in resNames:
                    ...
                    if seqCode != None:
                        ...
                        if not self.resUnassignedNames[resName]:
    ...
    def createResonanceGroups(self):
        ...
        if self.missingAtomNames and not self.nmrConstraintStore:
            ...
            if len(self.resUnassignedNameList) == skippedResonances:
                self.messageReporter.showWarning(
                    "Warning",
                    "LinkResonance will not finish because no resonances were found with associated atom names. All resonances were linked to the relevant ResonanceGroup, but not to any atoms. This problem probably occurs because the original imported file does not contain any atom names.",
                ...
                self.messageReporter.showWarning(
                    "Warning",
                    "Some resonances do not have associated atom names. These resonances were linked to the relevant ResonanceGroup.",
                    parent=self.guiParent,
                )
    ...
    def getResonanceGroup(self, nmrRes):
        ...
        if not resonanceGroup:
            resonanceGroup = Nmr.ResonanceGroup(
                self.nmrProject, chains=[nmrRes.chain], residue=nmrRes, details="Created by formatConverter"
            )
    ...
    def matchNamingSystems(self):
        ...
        if self.namingSystemName:
            ...
            #
            # Note that added a CUSTOM naming system for automatically created ChemComps
            #

            resNameFound = {"CUSTOM": 0}

            refChemComps = (
                self.project.findFirstChemComp(molType="protein", ccpCode="Ala"),
            ...
            for refChemComp in refChemComps:
                for namingSystem in refChemComp.namingSystems:
                    ...
                    if namingSystemName not in resNameFound:
            ...
            for resName in self.resUnassignedNameList:
                ...
                if self.chemComp not in chemCompAtomSysNames:
                    ...
                    for namingSystem in self.chemComp.namingSystems:
                        ...
                        for chemAtom in self.chemComp.chemAtoms:
                            ...
                            if atomSysName:
                                chemCompAtomSysNames[self.chemComp][namingSystem][atomSysName] = (
                                    atomSysName.sysName,
                                ) + atomSysName.altSysNames
            ...
            if noResidue != len(self.resUnassignedNameList):
                if not self.minimalPrompts and self.allowPopups:
                    ...
                    if len(bestNamingSystemNames) > 1:
                        for namingSystemOrder in (
                            ("IUPAC", "PDB", "GROMOS"),
                            ("AQUA", "CYANA2.1", "PDB_REMED", "XPLOR", "MSD", "DISGEO", "SYBYL"),
                        ):
    ...
    def setProchiralGroup(self, atomSets, otherResonance=None):
        ...
        if not otherResonance:
            ...
            copyResonanceInfo(
                resonance, otherResonance, nmrShiftSelect=self.nmrShiftSelect, copyTrack=True, verbose=self.verboseCopy
            )
    ...
    def updateResonance(self, resName, verbose=1):
        ...
        if chainCode != self.chainCode:
            if chainCode not in self.formatChainCodeDict:
                if verbose:
        ...
        if seqCode != self.seqCode or seqInsertCode != self.seqInsertCode:
            ...
            for firstSeqCodeKey, lastSeqCodeKey in seqCodes:
                ...
                if (
                    self.seqCode >= firstSeqCode
                    and self.seqInsertCode == seqInsertCode
                    and (lastSeqCode == None or self.seqCode <= lastSeqCode)
                ):
                ...
                if nmrRes:
                    if nmrRes not in self.ignoreResidues:
                        ...
                        if verbose:
                            ...
                            if not resNameText:
                                resNameText = ",".join(
                                    [getResNameText(res) for res in self.resUnassignedNames[self.resName]]
                                )

                            print(
                                "  Error: ignoring file resonance '%s'... it is linked to a residue that is listed as not valid."
                                % resNameText
                            )
        ...
        if not self.globalStereoAssign:
            ...
            if stereoAssign == None and len(self.resUnassignedNames[self.resName]) == 1:
                ...
                if stereoCode == 1:
                    ...
                    if not resStatusMapping:
                        resStatusMapping = NameMapping.ResonanceStatusMapping(
                            self.nameMapping, resonanceName=self.resName, isStereo=True
                        )
            ...
            for resName in tempResUnassignedNameList:
                if not self.updateResonance(resName, verbose=0):
                    ...
                    if self.useCommonNames:
                        commonNameMappings = self.chemComp.findAllApplicationData(
                            application=ccpNmr_kw, keyword="commonNameMapping"
                        )
                        self.setApplDataNameMappings(commonNameMappings, self.useCommonNames)

                    if self.useAmbiguity:
                        ambiguityMappings = self.chemComp.findAllApplicationData(
                            application=ccpNmr_kw, keyword="ambiguityMapping"
                        )
                    ...
                    if self.addNameMappings:
                        ...
                        if ccpCode in self.addNameMappings or "ALL" in self.addNameMappings.keys():
                            nameMappingList = []

                            if ccpCode in self.addNameMappings:
            ...
            if setStatus == 1:
                for i in range(1, len(nameMappings)):
                    ...
                    if i == 1:
                        ...
                        for refResonance in refResonances:
                            ...
                            copyResonanceInfo(
                                refResonance,
                                newResonances[-1],
                                nmrShiftSelect=self.nmrShiftSelect,
                                verbose=self.verboseCopy,
                            )
                    ...
                    for newResonance in newResonances:
                        newResonance.addApplicationData(
                            Implementation.AppDataBoolean(application=self.format, keyword="resetToIupac", value=1)
                        )
                    ...
                    if newResName in self.resUnassignedNames:
                    ...
                    if self.verbose:
                        print(
                            "  Warning: using name %s for %s - %s to %s resName %s."
                            % (newAtomName, self.resName, actionText, resNameText, newResName)
                        )
    ...
    def setupAtomToResName(self):
        ...
        for resName in self.resUnassignedNameList:
            ...
            #
            # Here reset protonation state of a chemCompVar, but ONLY if there is a clear atom name match!
            #

            if self.ccpCode in self.checkProtonationStates:
                atomNames = self.checkProtonationStates[self.ccpCode]
                if self.atomName in atomNames:
                    atom = self.nmrRes.findFirstAtom(name=self.atomName)
                    if not atom:
                        origDesc = self.nmrRes.descriptor
                        descDict = getDescriptorDict(origDesc)
                        mergeDescriptorDicts(descDict, {"prot": [self.atomName]})
                        newDesc = getDescriptorFromDict(descDict)
                        self.nmrRes.descriptor = newDesc
                        self.nmrRes.newAtom(name=self.atomName)
                        self.chemCompVar = self.nmrRes.chemCompVar

                        print(
                            "  Warning: reset protonation state from %s to %s for %s (%s,%d) based on format atom name %s!"
                            % (origDesc, newDesc, self.ccpCode, self.chain.code, self.nmrRes.seqId, self.atomName)
                        )
            ...
            if self.chemComp not in self.ccNamingSystems:
                ...
                if namingSystem and self.hasAtomSysNames(namingSystem):
                    ...
                    for tempNamingSystemName in self.tempNamingSystemNames:
                        ...
                        if tempNamingSystem and self.hasAtomSysNames(tempNamingSystem):
                            ...
                            print(
                                "  Warning: using local naming system %s for %s because no %s atom names available!"
                                % (tempNamingSystemName, self.chemComp.ccpCode, self.namingSystemName)
                            )
                            break
            ...
            if self.fixSingleAtoms and not chemAtomSysNames:
                if self.atomName and self.atomName[-1] in "*#%+":
                    ...
                    if chemAtomSysNames:
                        print(
                            "  Warning: reset atom name %s to %s for residue %s - is single atom."
                            % (self.atomName, tryAtomName, self.resName)
                        )
      ...
      """
            ...
            if not chemAtomSysNames and not foundChemAtom:
                print(
                    "  Error: No matches at all for resonance name %s (%s)" % (resName, self.resName),
                )
                if self.nmrRes:
            ...
            if not foundChemAtom:
                ...
                if not self.useLinkResonancePopup or not self.allowPopups or not self.getAtomMatch():
                    continue
    ...
    def setAtomToResNameList(self):
        ...
        if newAtomToResName:
            ...
            self.atomToResNameList.append(
                AtomToResName(
                    nmrAtoms, chemAtomDict, self.resName, self.chemAtomSets, isotopeCode, verbose=self.verbose, **keywds
                )
            )
        ...
        for i in range(0, len(self.atomToResNameList) - 1):
            ...
            for j in range(i + 1, len(self.atomToResNameList)):
                ...
                if comparisonStatus == 0:
                    ...
                    if self.verbose:
                        print(
                            "  Warning: Full subset detected for %s. Added to full set %s."
                            % (str(subSet.resNameList), str(fullSet.resNameList))
                        )
                    ...
                    for resName in subSet.resNameList:
                        ...
                        if resName in subSet.singleAtoms:
                    ...
                    for nmrAtoms in atomToResName.nmrAtomGroups:
                        ...
                        if atomSetResName:
                            for nmrAtom in nmrAtoms:
                                ...
                                chemAtomSysName = findChemAtomOrSetSysName(namingSystem, chemAtom)
                                ...
                                for resName in atomToResName.hasEqChemAtomSets:
                                    ...
                                    if chemAtomSysName == atomName:
                                        if self.verbose:
                                            print(
                                                "  Found equivalent atom group name match for %s - collapsing info on %s"
                                                % (resName, atomSetResName)
                                            )
        ...
        for atomToResName in self.atomToResNameList:
            ...
            if self.verbose:
                ...
                if printResidueInfo:
                    ...
                    print(
                        drawBox(
                            "Assignments for residue %d, chain '%s' (%s, code %s)"
                            % (self.nmrRes.seqId, self.nmrRes.chain.code, self.nmrRes.ccpCode, self.nmrRes.seqCode),
                            indent="  ",
                        )
                    )
                    print()
                ...
                if self.existingResonances.resonances:
                    ...
                    for resonance in self.existingResonances.resonances:
                        copyResonanceInfo(
                            ...
                            nmrShiftSelect=self.nmrShiftSelect,
                            deleteOldInfo=1,
                            toResName=refResName,
                            verbose=self.verboseCopy,
                        )
                        for resName in atomToResName.resNameList[1:]:
                            self.deleteResonance(resonance, [resName])
                    ...
                    resonanceSet = self.resonanceSetClass(
                        self.resonanceParent, resonances=[refResonance], atomSets=atomSets
                    )
            ...
            #
                ...
                if atomToResName.isProchiral:
                    ...
                    if atomToResName.hasProchChemAtomSets:
                        ...
                        if not self.linkToExistingResonances(
                            nmrAtoms, atomSets, atomToResName.resNameList, resRequired=2
                        ):
                            if self.verbose:
                                print(
                                    "  Set %s as %s %s prochiral (with multi group)"
                                    % (resNameText, prochiralType, stereoStatusText)
                                )
                        ...
                        if otherChemAtomSets:
                            ...
                            for nmrAtom in nmrAtoms:
                                if nmrAtom not in self.atomResonances:
                    ...
                    if len(nmrAtoms) > singleNumberAtoms:
                        ...
                        if len(otherChemAtomSets) > 2:
                            ...
                            interaction = self.multiDialog.ResonanceGroup(
                                self.guiParent, otherChemAtomSets, self.nmrRes, self.allowPopups
                            )
                            ...
                            if not resNameGroups:
                                print(
                                    "   Error: Not grouping resonances with name %s because automatic grouping failed or no popup allowed."
                                    % (otherChemAtomSets)
                                )
                            ...
                            for resNameGroup in resNameGroups:
                                if resNameGroup:
                                    ...
                                    for resName in resNameGroup[1:]:
                                        ...
                                        if self.verbose:
                                            print(
                                                "  Warning: Collapsing group resonances for %s to %s"
                                                % (resName, refResName)
                                            )
                                        ...
                                        if resName in atomToResName.stereoAtoms:
                        ...
                        if len(otherChemAtomSets) == 2:
                            ...
                            if not self.stereoAssign:
                                ...
                                if not self.linkToExistingResonances(
                                    ...
                                    [resNames[0]],
                                    resRequired=2,
                                    otherResonance=otherResonance,
                                    otherResNames=[resNames[1]],
                                ):
                                    if self.verbose:
                                        print(
                                            "  Set %s as %s prochiral, non-stereo, two resonances."
                                            % (resNameText, prochiralType)
                                        )
                                ...
                                if len(atomToResName.stereoAtoms) == 1:
                                    ...
                                    if len(nmrAtoms) == 1:
                                        atomToResName.stereoAtoms[otherResName] = [
                                            returnProchiralAtom(nmrAtoms[0], chemCompVar=self.nmrRes.chemCompVar)
                                        ]

                                    else:
                                        atomToResName.stereoAtoms[otherResName] = returnProchiralAtomGroup(
                                            nmrAtoms, chemCompVar=self.nmrRes.chemCompVar
                                        )[0]
                                ...
                                for resName in resNames:
                                    ...
                                    if not self.linkToExistingResonances(nmrAtoms, atomSets, [resName], resRequired=1):
                                        if self.verbose:
                                            print(
                                                "  Set %s as %s prochiral, stereo, two resonances."
                                                % (resName, prochiralType)
                                            )

                                        atomSet = nmrAtoms[0].atomSet
                                        resonanceSet = self.resonanceSetClass(
                                            self.resonanceParent, resonances=[self.resonance], atomSets=[atomSet]
                                        )
                            ...
                            if not self.stereoAssign:
                                ...
                                if not self.linkToExistingResonances(
                                    nmrAtoms, atomSets, atomToResName.resNameList, resRequired=resRequired
                                ):
                                    if not singleProchiralStatus:
                                        if self.verbose:
                                            print(
                                                "  Set %s as %s prochiral, non-stereo, one resonance (copied)."
                                                % (resNameText, prochiralType)
                                            )
                                        ...
                                        if self.verbose:
                                            print(
                                                "  Set %s as %s prochiral, non-stereo, one resonance (kept single)."
                                                % (resNameText, prochiralType)
                                            )

                                        resonanceSet = self.resonanceSetClass(
                                            self.resonanceParent, resonances=[self.resonance], atomSets=atomSets
                                        )
                                        resonanceName = createNonStereoName(atomSets[0].name, "a")
                                        setCcpNmrResonanceName(
                                            self.resonance, atomSets[0].findFirstAtom(), atomName=resonanceName
                                        )
                ...
                #
                    ...
                    if atomToResName.hasEqChemAtomSets:
                        ...
                        if atomToResName.singleAtoms and atomToResName.isEquivalent == None:
                            ...
                            for nmrAtomGroup in atomToResName.nmrAtomGroups:
                                ...
                                if resNamesFound:
                                    ...
                                    singleResonanceStatus = self.getStatusResonance(
                                        nmrAtom, otherAtomName, resNamesFound[index][0]
                                    )
                        ...
                        if (
                            not atomToResName.singleAtoms
                            and len(atomToResName.nmrAtoms) > 1
                            and atomToResName.isEquivalent == False
                        ):
                            #
                            ...
                            if atomToResName.nmrAtoms[0].chemAtom.chemAtomSet:
                                ...
                                if (
                                    deepChemAtomSet
                                    and deepChemAtomSet.isEquivalent == False
                                    and deepChemAtomSet.isProchiral == False
                                ):
                                    isGroupedAmide = 0
                            ...
                            if isGroupedAmide:
                                ...
                                if not self.linkToExistingResonances(nmrAtoms, atomSets, [refResName], resRequired=2):
                                    ...
                                    for i in range(0, 2):
                                        setCcpNmrResonanceName(
                                            resonances[i],
                                            atomSets[i].findFirstAtom(),
                                            atomName=atomSets[i].findFirstAtom().name,
                                        )
                        ...
                        if not atomToResName.isEquivalent and len(atomToResName.nmrAtoms) > 1:
                            ...
                            if atomToResName.singleAtoms:
                                ...
                                for nmrAtom in nmrAtomList:
                                    ...
                                    for singleAtomResName in atomToResName.singleAtoms.keys():
                                        if nmrAtom == atomToResName.singleAtoms[singleAtomResName]:
                                            self.resonance = self.joinResonanceFromResNames(
                                                [singleAtomResName], isotopeCode
                                            )
                                            setCode = "Set"
                                            break
                                    ...
                                    if not self.resonance:
                                        self.resonance = self.resonanceClass(
                                            self.resonanceParent, isotopeCode=isotopeCode
                                        )
                                        singleAtomResName = ""
                                    ...
                                    if not self.linkToExistingResonances(
                                        [nmrAtom], None, [singleAtomResName], resRequired=1
                                    ):
                                        ...
                                        if self.verbose:
                                            print(
                                                "  %s '%s' as single aromatic/amide atom" % (setCode, singleAtomResName)
                                            )
                                    ...
                                    if nmrAtom in atomToResName.chemAtomDict:
                                        chemAtom = atomToResName.chemAtomDict[nmrAtom]
                                    else:
                                ...
                                for resName in tempGroupChemAtomSets:
                                    ...
                                    for chemAtomSet in chemAtomSets:
                                        if chemAtomSet in deleteChemAtomSets:
                                            ...
                                            for chemAtom in chemAtomSet.chemAtoms:
                                                if chemAtom in chemAtomResonances:
                                    ...
                                    if not atomToResName.groupChemAtomSets[resName]:
                                        del atomToResName.groupChemAtomSets[resName]
                                        index = atomToResName.hasGroupChemAtomSets.index(resName)
                                        atomToResName.hasGroupChemAtomSets.pop(index)
                            ...
                            if atomToResName.hasGroupChemAtomSets:
                                ...
                                for resName in atomToResName.hasGroupChemAtomSets:
                                    ...
                                    for chemAtomSet in chemAtomSets:
                                        ...
                                        if chemAtomSetName not in chemAtomSetNamesResNames:
                                ...
                                if len(chemAtomSetNames) == 2:
                                    if (
                                        chemAtomSetNamesResNames[chemAtomSetNames[0]]
                                        == chemAtomSetNamesResNames[chemAtomSetNames[1]]
                                    ):
                                        #
                                ...
                                for i in range(0, len(chemAtomSetNames)):
                                    ...
                                    if i == 1 and createNewResonance:
                                        ...
                                        refResName = ""
                                        self.resonance = self.resonanceClass(
                                            self.resonanceParent, isotopeCode=resonance.isotopeCode
                                        )
                                        loopStart = 0
                                        setCode = "Added"
                                    ...
                                    if hasAtomSets and len(hasAtomSets) == 2:
                                        resonance = self.joinResonanceFromResNames(
                                            atomToResName.resNameList, isotopeCode
                                        )

                                        newResonance = self.resonanceClass(
                                            self.resonanceParent, isotopeCode=isotopeCode
                                        )
                                        copyResonanceInfo(
                                            resonance, newResonance, copyTrack=True, verbose=self.verboseCopy
                                        )

                                        allResonances = [resonance, newResonance]

                                        for j in range(len(allResonances)):
                                            ...
                                            if not self.linkToExistingResonances(
                                                [nmrAtom], [atomSet], [resonance.name], resRequired=1
                                            ):
                                                if self.verbose:
                                                    print(
                                                        "  Forced %s as single atom (atomSet already present)"
                                                        % resNameText
                                                    )
                                    ...
                                    #
                                        ...
                                        ccpName = setCcpNmrResonanceName(
                                            self.resonance, nmrAtomGroup[0], chemAtomSetName
                                        )
                                        ...
                                        for resName in chemAtomSetNamesResNames[chemAtomSetName][loopStart:]:
                                            ...
                                            if self.verbose:
                                                print(
                                                    "  Warning: Collapsing grouped aromatic/amide resonances for %s to %s"
                                                    % (resName, refResName)
                                                )
                            ...
                            if not self.linkToExistingResonances(
                                nmrAtoms, None, atomToResName.resNameList, resRequired=1
                            ):
                                if self.verbose:
            ...
            for resonance in resonanceList:
                if not resonance:
                    if self.verbose:
                        print("ERROR: resonance information missing.")
                    continue
                ...
                for tResonance in resonances:
                    if tResonance not in allLinkedResonances:
                        allLinkedResonances.append(tResonance)
                ...
                if self.verbose:
                    ...
                    if resonanceResidueLink:
                        outText[i] += " Residue '%s.%d' -- all atoms" % (
                            resonanceResidueLink.chain.code,
                            resonanceResidueLink.seqCode,
                        )
                    else:
    ...
    def linkToExistingResonances(
        self, nmrAtoms, atomSets, resNames, resRequired=1, otherResonance=None, otherResNames=None
    ):
        """
            ...
            if not otherResonance:
                ...
                if self.existingResonances.numResonances == 1:
                    ...
                    copyResonanceInfo(
                        ...
                        existingResonance,
                        nmrShiftSelect=self.nmrShiftSelect,
                        toResName=str(self.existingResonances.resNames[0]),
                        verbose=self.verboseCopy,
                    )
                    ...
                    if (sharedChemAtomSet or len(self.existingResonances.resonanceSets) == 1) and resRequired == 2:
                        ...
                        for i in range(0, len(self.existingResonances.resonances)):
                            ...
                            copyResonanceInfo(
                                ...
                                existingResonance,
                                nmrShiftSelect=self.nmrShiftSelect,
                                toResName=str(self.existingResonances.resNames[i]),
                                copyTrack=True,
                                verbose=self.verboseCopy,
                            )
                ...
                if len(resonances) == 2 and len(self.existingResonances.resonanceSets) == 1:
                    if len(self.existingResonances.resonances) == 2:
                        ...
                        if len(existingResNames) <= 1:
                            ...
                            if self.verbose:
                                ...
                                print(
                                    "    Warning: special case - autoconnecting resonances %s%s."
                                    % (str(resNamesList), addText)
                                )
                                    ...
                                    for nrnIndex in range(0, 2):
                                        ...
                                        if len(newResNames) == 1:
                                            ...
                                            for i in range(2, 0, -1):
                                                if self.patt["onlydigit"].search(atomName[-i:]) and self.patt[
                                                    "onlydigit"
                                                ].search(newAtomName[-i:]):
                            ...
                            if sameResNames == 2:
                                ...
                                if self.verbose:
                                    print(
                                        "    Warning: special case - autoconnecting resonances %s with same double name."
                                        % (str(resNamesList))
                                    )
                                ...
                                if self.verbose:
                                    print(
                                        "    Warning: special case - autoconnecting resonances %s to resonances with matching atomName index."
                                        % (str(resNamesList))
                                    )
                    ...
                    #
                        ...
                        if len(existingResNames) == 0:
                            ...
                            if self.verbose:
                                print(
                                    "    Warning: special case - autoconnecting resonances %s (no existing resNames, creating one new resonance to existing resonanceSet)."
                                    % (str(resNamesList))
                                )
                            ...
                            if self.verbose:
                                print(
                                    "    Warning: special case - autoconnecting resonances %s (trying to match to existing resNames, creating one new resonance to existing resonanceSet)."
                                    % (str(resNamesList))
                                )
                ...
                #
                    ...
                    self.autoConnectStereo
                    and len(resonances) == 2
                    and len(self.existingResonances.resonanceSets) == 2
                    and len(self.existingResonances.resonances) == 2
                ):
                    matchResNameList = []
                    newResNameList = [resNames, otherResNames]
                        ...
                        if len(existingResSet.atomSets) == 1:
                            ...
                            for nrnIndex in range(0, 2):
                                ...
                                if len(newResNames) == 1:
                                    ...
                                    chemAtomSysNames = self.getAtomSysNames(
                                        newAtomName, self.namingSystemName, self.nmrRes.chemCompVar
                                    )
                    ...
                    if len(matchResNameList) == 2 and matchResNameList[0] != matchResNameList[1]:
                        ...
                        if self.verbose:
                            print(
                                "    Warning: special case - autoconnecting resonances %s (using atom names of stereospecific existing resonances to connect)."
                                % (str(resNamesList))
                            )
    ...
    def selectResonances(self, resNames, multi=0):
        ...
        if not selectedResonances:
            if self.allowPopups:
                interaction = self.multiDialog.ResonanceSelect(
                    self.guiParent, self.existingResonances, self.nmrRes, resName, multi
                )
                ...
                for selectedResonance in selectedResonances:
                    selectedResonanceNames.extend(
                        getApplResNames(ccpNmr_kw, [selectedResonance], keyword=assign_kw).keys()
                    )

                selectedResonanceNames.sort()
                resLinkMapping = NameMapping.ResonanceLinkMapping(
                    self.nameMapping, resonanceName=resName, selectResonanceNames=selectedResonanceNames
                )
                ...
                print(
                    "  ERROR: no resonances selected from list %s because no user interaction allowed." % str(tResNames)
                )
    ...
    def deleteResonance(self, resonance, resNameList):
        ...
        if not resonance.isDeleted:
            ...
            print(
                "  Warning: attempt to delete already deleted %s %s..."
                % (resonance.className, getResNameText(resonance))
            )
    ...
    def findResonancesMatchingAtoms(self, nmrAtoms):
        ...
        for atom in nmrAtoms:
            if atom in self.atomResonances:
    ...
    def getMatchingAtoms(self):
        ...
        if self.atomsFound == self.chemAtoms:
            if self.verbose:
                print(
                    "No match with real atoms for chemAtoms %s (%s,%s)"
                    % (str(chemAtomNames), self.seqCode, self.nmrRes.molResidue.ccpCode)
                )
            return 0
            ...
            if self.verbose:
                print(
                    "Error: ChemAtoms  %s do not match real atoms (%s,%s)."
                    % (str(chemAtomNames), self.seqCode, self.nmrRes.molResidue.ccpCode)
                )
            return 0
    ...
    def hasAtomSysNames(self, namingSystem, coverage=0.8):

        atomSysNamesFound = False

        chemComp = namingSystem.chemComp
        chemAtoms = chemComp.sortedChemAtoms()
        chemAtomSysNames = findAllSysNamesByChemAtomOrSet(chemComp, chemAtoms, namingSystem.name)

        if len(chemAtomSysNames) * 1.0 / len(chemAtoms) > coverage:
            atomSysNamesFound = True

        if not atomSysNamesFound and self.useEmptyNamingSystems:
            atomSysNamesFound = True

        return atomSysNamesFound
    ...
    def getAtomSysNames(self, atomName, namingSystemName, chemCompVar):
        ...
        if chemCompVar not in self.ccvSysNames:
            self.ccvSysNames[chemCompVar] = [[ca.name for ca in chemCompVar.chemAtoms], {}]

        if namingSystem not in self.ccvSysNames[chemCompVar][1]:
            self.ccvSysNames[chemCompVar][1][namingSystem] = {}

            nsChemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                chemCompVar.chemComp, chemCompVar.chemAtoms, namingSystemName
            )
            nsChemAtomSysNames.extend(
                findAllSysNamesByChemAtomOrSet(chemCompVar.chemComp, chemCompVar.chemAtomSets, namingSystemName)
            )
            ...
            for nsChemAtomSysName in nsChemAtomSysNames:
                self.ccvSysNames[chemCompVar][1][namingSystem][nsChemAtomSysName] = [
                    nsChemAtomSysName.atomName,
                    nsChemAtomSysName.sysName,
                    nsChemAtomSysName.altSysNames,
                ]
    ...
    def getIupacChemAtomSysNames(self):
        ...
        if chemAtomSysNames:
            for chemAtomSysName in chemAtomSysNames:
                chemAtom = self.chemCompVar.findFirstChemAtom(
                    name=chemAtomSysName.atomName, subType=chemAtomSysName.atomSubType
                )
    ...
    def getAtomMatch(self):
        ...
        interaction = self.multiDialog.AtomSelect(
            self.guiParent, self.nmrRes, self.resName, self.resUnassignedNames[self.resName], modal=True
        )
    ...
    def useAtomMapping(self):
        ...
        if self.nameMapping.namingSystem == "Unknown":
            ...
            if self.nameMapping.namingSystem != self.namingSystemName:
                self.messageReporter.showWarning(
                    "Warning",
                    "Naming system mismatch!! Using %s for mapping (not %s). This might lead to errors in the mapping..."
                    % (self.nameMapping.namingSystem, self.namingSystemName),
                    parent=self.guiParent,
                )
        ...
        for atomMapping in self.nameMapping.atomMappings:
            ...
            if molSystemCode:
                if molSystemCode != self.molSystem.code:
                    if self.verbose:
                        print(
                            "  Warning: molSystem code %s in mapping does not match current molSystem." % molSystemCode
                        )
            ...
            for mapChain in tempMapChains:
                if mapChain not in self.ccpChainDict:
            ...
            for tempResName in self.resUnassignedNames:
                if tempResName in tempCodes:
                    for tempResonance in self.resUnassignedNames[tempResName]:
                        mappingData = tempResonance.findFirstApplicationData(
                            application=self.format, keyword=ccpNmrMapping_kw
                        )
                        ...
                        if mappingData and mappingData.value != atomMappingName:
                            if self.verbose:
                                print(
                                    "  Warning: propagating mapping to %s (previous mapping to atomName %s overwritten)."
                                    % (tempResName, mappingData.value)
                                )
                            ...
                            mappingData = Implementation.AppDataString(
                                application=self.format, keyword=ccpNmrMapping_kw, value=atomMappingName
                            )
    ...
    def setAtomMapping(self, chain=None, molecule=None, molSystem=None, ccpCode=None, seqId=None):
        ...
        self.atomMapping = self.nameMapping.findFirstAtomMapping(
            name=self.atomName, formatName=self.atomMappingName, **keywds
        )

        if not self.atomMapping and self.atomMappingName:
            self.atomMapping = NameMapping.AtomMapping(
                self.nameMapping, name=self.atomName, formatName=self.atomMappingName, **keywds
            )
            return 1
            ...
            for mapChain in mapChains:
                ...
                for tempNmrRes in mapChain.sortedResidues():
                    if tempNmrRes.molResidue.seqCode == seqId:
                        ...
                        tempCodes.append(
                            getResName(mapChainCode, str(formatSeqCode), atomName, seqInsertCode=seqInsertCode)
                        )
    ...
    def getMatchingChemAtoms(self, chemAtomSysNames):
        ...
        for chemAtomSysName in chemAtomSysNames:
            ...
            if chemAtom:
                ...
                chemAtomSet = self.chemCompVar.findFirstChemAtomSet(
                    name=chemAtomSysName.atomName, subType=chemAtomSysName.atomSubType
                )
        ...
        if not validChemAtomSysNames:
            ...
            if self.verbose:
                print(
                    "  Error: no match for chemAtomSysName %s, atom(s) %s! (molType %s, ccpCode %s)"
                    % (chemAtomSysName.sysName, str(atomNames), chemAtomSysName.namingSystem.chemComp.molType, ccpCode)
                )
        ...
        if len(validChemAtomSysNames) > 1:
            if self.verbose:
                print(
                    "  Warning: Multiple matches for search on atom name %s (molType %s, ccpCode %s). Picked first valid atom"
                    % (self.atomName, chemAtomSysName.namingSystem.chemComp.molType, ccpCode),
                )
            ...
            if chemAtomSet:
                if not self.stereoAssign and chemAtomSet.isProchiral:
                    ...
                    for resonance in self.resUnassignedNames[self.resName]:
                        ...
                        setUniqueAppData(
                            "AppDataString", resonance, self.format, "mappingSingleMatch", chemAtomSysName.atomName
                        )
                ...
                if chemAtomSet.isEquivalent:
                    #
                    # If protons are considered equivalent, need the chemAtomSet unless makeMethylsEquivalent is False
                    # This is necessary for checking if only an HG11 is reported for a Val HG1* for example
                    # Also use ALL atoms for this set (basically adding them here)
                    #
                    ...
                    if deepChemAtomSet:
                        if not self.stereoAssign and deepChemAtomSet.isProchiral and self.makeMethylsEquivalent:
                            ...
                            for resonance in self.resUnassignedNames[self.resName]:
                                setUniqueAppData(
                                    "AppDataString", resonance, self.format, "mappingSingleMatch", chemAtomSet.name
                                )
    ...
    def nmrShiftSelect(self, shift, otherShift, toResName):
        ...
        if not self.forceShiftMerge and abs(shiftValue - otherShiftValue) > chemShiftAllowedDiff and self.allowPopups:
            interaction = self.multiDialog.MeasurementSelect(
                self.guiParent, toResName, toRes, shiftValue, shift.parentList, "Shift"
            )
            ...
            if self.verbose:
                print(
                    "      Merging shift values %.3f and %.3f for resonance %s."
                    % (shiftValue, otherShiftValue, toResName)
                )
                print()
        ...
        for formatChainCode in formatChainCodeList:
            ...
            for mapToChain in mapToChainList:
                ...
                for origResName in origResNamesList:
                    ...
                    if matchName in mapResNamesDict:
                        if self.verbose:
                        ...
                        for origResonance in origResonances:
                            for matchResonance in matchResonances:
                                copyResonanceInfo(
                                    origResonance, matchResonance, toResName=matchName, verbose=self.verboseCopy
                                )
                        ...
                        for origResonance in origResonances:
                            matchResonance = self.resonanceClass(
                                self.resonanceParent, isotopeCode=origResonance.isotopeCode
                            )
                            matchResonance.addApplicationData(
                                Implementation.AppDataString(
                                    application=self.format, keyword=assign_kw, value=matchName
                                )
                            )
                            copyResonanceInfo(
                                origResonance, matchResonance, toResName=matchName, verbose=self.verboseCopy
