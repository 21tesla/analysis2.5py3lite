...
from ccpnmr.format.general.Constants import allFormatsDict, newline
from ccpnmr.format.general.Util import getApplResNames, getNameInfo
from memops.universal.Util import drawBox, returnInt
...
def writeMappingFile(project, outFile, originalFormats=None):
    ...
    for format in formatList:
        ...
        for resonanceName in resonancesDict.keys():
            ...
            if chainCode not in resonancesSortDict:
                resonancesSortDict[chainCode] = {}

            if seqCode not in resonancesSortDict[chainCode]:
                resonancesSortDict[chainCode][seqCode] = {}

            if seqInsertCode not in resonancesSortDict[chainCode][seqCode]:
        ...
        if resonancesDict != {}:
            ...
            for resonanceName in resonanceNames:
                ...
                for resonance in resonances:
                    ...
                    if resonanceSet:
                        for atomSet in resonanceSet.atomSets:
                            ...
                            resCode = (
                                residue.molResidue.ccpCode + str(residue.seqCode) + string.strip(residue.seqInsertCode)
                            )
                ...
                if atomNames:
                    ...
                    fout.write(
                        "  %-15s -> %-15s  %s" % ("'" + resonanceName + "'", string.join(atomNames, ","), addText)
                    )
