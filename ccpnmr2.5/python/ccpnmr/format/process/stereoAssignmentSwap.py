    ...
    for resonance in resAtomSetDict:
        ...
        if len(atomSetList) == 1:
            ...
            if not refAtom.chemAtom:
                print(
                    "  Error: no chemAtom for atom %s in residue %s, %s"
                    % (refAtom.name, refAtom.residue.molType, refAtom.residue.ccpCode)
                )
                continue
...
def getAtomsOtherProchiral(atom, prochiralResonancesDict, resonance):
    ...
    if chemAtomSet:
        ...
        #
            ...
            (ccpCode == "Asn" and chemAtomSet.name == "HD2*")
            or (ccpCode == "Gln" and chemAtomSet.name == "HE2*")
            or (ccpCode in ("Phe", "Tyr") and chemAtomSet.name in ("HD*", "HE*"))
            or (ccpCode in ("G", "A", "C", "G", "T", "U", "DUR") and chemAtomSet.name == "H2'*")
            or (ccpCode in ("A") and chemAtomSet.name == "H6*")
            or (ccpCode in ("C") and chemAtomSet.name == "H4*")
        ):
            sortedChemAtomsList = list(chemAtomSet.sortedChemAtoms())
            chemAtomIndex = sortedChemAtomsList.index(chemAtom)
            prochiralChemAtomSet = chemAtomSet
...
def getDistConstrSumAvg(
    distConstr, resAtomDict, atomCoordDict, factor, resAtomSwapDict, prochiralResonancesDict, swap=False
):
    ...
    for sideCode in sideCodes:
        if avgSum[sideCode]:
            ...
            for prochiralKey in prochiralAvgSum[sideCode].keys():
                ...
                prochiralContribs[sideCode][prochiralKey] = (
                    tempProchiralAvgSum / avgSum[sideCode],
                    prochiralAvgSum[sideCode][prochiralKey][1],
                )

    return avgDist, prochiralContribs


def setLocalSumContribs(
    resIndex,
    sideCodes,
    item,
    resAtomDict,
        ...
        if atom in atomCoordDict:
            ...
            for i in range(0, len(otherAtomLists)):
                ...
                for otherAtom in otherAtomList:
                    if (
                        atom != otherAtom
                        and otherAtom in atomCoordDict
                        and atomCoordDict[atom]
                        and atomCoordDict[otherAtom]
                    ):
                        avgSumContrib = 1.0 / math.pow(
                            getDistanceFromCoordinates(atomCoordDict[atom], atomCoordDict[otherAtom]), 6
                        )
            ...
            if avgLocalSum:
                if resonance in prochiralResonancesDict.keys():
                    ...
                    if prochiralKey not in prochiralAvgSum[sideCode]:
...
class StereoAssignmentSwapCheck:
    ...
    def checkSwapsAndClean(self, method="SUM_AVERAGING", violationCodes=None, swapFraction=0.75, deassignAll=False):
        ...
        (self.resAtomDict, self.resAtomSetDict) = createResonanceAtomAndAtomSetDict(
            self.distanceConstraintLists[0].parent.fixedResonances
        )
        if self.verbose:
    ...
    """
            ...
            for distanceConstraintList in self.distanceConstraintLists:
                for distConstr in distanceConstraintList.sortedConstraints():
                    ...
                    if method == "SUM_AVERAGING":
                        ...
                        (avgDist, prochiralContribs) = getDistConstrSumAvg(
                            distConstr,
                            self.resAtomDict,
                            self.atomCoordDict,
                            factor,
                            self.resAtomSwapDict,
                            self.prochiralResonancesDict,
                        )
        ...
        for i in range(len(structureViolations)):
            ...
            for prochiralKey in prochiralViolationDict["orig"]["total"]:
                ...
                if (
                    prochiralViolationDict["orig"]["total"][prochiralKey]
                    > prochiralViolationDict["swap"]["total"][prochiralKey]
                ):
                    if prochiralKey not in prochiralSwaps:
        ...
        for prochiralKey in prochiralViolationDict["orig"]["total"]:
            ...
            if prochiralKey in prochiralSwaps and prochiralSwaps[prochiralKey] > cutoff:
                swapStatus = "swap"
                percent = prochiralSwaps[prochiralKey] * 100 / len(structureViolations)
                self.infoText["swap"].append(
                    "Swapping stereo assignment status chain %s, residue %3d group %s: %.3f%%"
                    % (prochiralKey[0], prochiralKey[1], prochiralKey[2].name, percent)
                )
            ...
            for violationCode in self.violationCodes.keys():
                ...
                for prochiralViolationDict in structureViolations:
                    for distConstr in prochiralViolationDict[swapStatus][violationCode][prochiralKey].keys():
                        if distConstr not in constraintsViolated:
                            constraintsViolated[distConstr] = [0.0, []]
                        constraintsViolated[distConstr][0] += 1.0

                        for constrItem in prochiralViolationDict[swapStatus][violationCode][prochiralKey][distConstr][
                            1
                        ]:
                ...
                for distConstr in distConstraints:
                    ...
                    if fractionViolated >= cutoffFraction:
                        ...
                        if len(prochiralResonances) == 1:
                            otherProchiralResonance = self.nmrConstraintStore.newFixedResonance(
                                isotopeCode=prochiralResonances[0].isotopeCode
                            )
                            print("NEW1 resonance %d" % otherProchiralResonance.serial)
                            otherAtoms = self.resAtomSwapDict[prochiralResonances[0]]
                            otherAtomSet = otherAtoms[0].findFirstFixedAtomSet(
                                nmrConstraintStore=self.nmrConstraintStore
                            )
                            if not otherAtomSet:
                                otherAtomSet = self.nmrConstraintStore.newFixedAtomSet(atoms=otherAtoms)

                            otherProchiralResonanceSet = self.nmrConstraintStore.newFixedResonanceSet(
                                resonances=[otherProchiralResonance], atomSets=[otherAtomSet]
                            )  # @UnusedVariable
                        ...
                        self.resetConstraintItems(
                            violatedItems, prochiralResonances, prochiralKey, violationCode, fractionViolated
                        )
                        ...
                        if self.deassignAll:
                            ...
                            for prochiralResonance in prochiralResonances:
                                ...
                                self.resetConstraintItems(
                                    ...
                                    violationCode,
                                    fractionViolated,
                                    basedOnOtherConstraint=distConstr.serial,
                                )
    ...
    def resetConstraintItems(
        ...
        if len(prochiralResonances) == 1:
            ...
            if len(resSet.atomSets) != 1 or len(resSet.sortedAtomSets()[0].atoms) != 2:
                ...
                otherProchiralResonance = self.nmrConstraintStore.newFixedResonance(
                    name=otherAtoms[0].name, isotopeCode=prochiralResonance.isotopeCode, resonanceSet=resSet
                )
                        ...
                        if not constraintExists:
                            ...
                            if verbose:
                                if basedOnOtherConstraint:
                                    infoLine = (
                                        "  + based on constraint %d, deassigned constraint %d to prochiral %s.\n"
                                        % (basedOnOtherConstraint, distConstr.serial, prochiralText)
                                    )
                                    infoLine += "     --> added new item for '%s' to '%s'" % (
                                        getResNameText(newResonances[0]),
                                        getResNameText(newResonances[1]),
                                    )
                                else:
                                    infoLine = (
                                        "Deassigned constraint %d to prochiral %s: violation > %.1f in %.1f%% of structures.\n"
                                        % (
                                            distConstr.serial,
                                            prochiralText,
                                            self.violationCodes[violationCode]["violation"],
                                            fractionViolated * 100,
                                        )
                                    )
                                    infoLine += "     --> added new item for '%s' to '%s'" % (
                                        getResNameText(newResonances[0]),
                ...
                for prochiralKey in prochiralContribs[sideCode]:
                    ...
                    for violationCode in self.violationCodes.keys():
                        ...
                        if violContrib > violLimit:
                            ...
                            if distConstr not in prochiralViolation:
...
class StereoAssignmentCleanup(StereoAssignmentSwapCheck):
    ...
    VIOLATION_CODE_REPORTING_LIST = [
        VIOLATION_CODE_REPORTINGS_STR,
        VIOLATION_CODE_REPORTINGL_STR,
        VIOLATION_CODE_REPORTINGX_STR,
    ]
  ...
  """
    ...
    def storeToAppData(self, star_text):
        appData1 = Implementation.AppDataString(
            value=star_text, application="FormatConverter", keyword="stereoAssignmentCorrectionsFile"
        )
        self.nmrConstraintStore.addApplicationData(appData1)
        print("Added STAR file to application data within nmrConstraintStore")

    def checkSwapsAndClean(
        self,  # For comparison the NRG tags and defaults on March 2nd, 2011 are presented.
        ...
        self.violationCodes["l"] = {"violation": multiModelCutoff, "fraction": multiModelPercentageCutoff / 100.0}
        self.violationCodes[self.VIOLATION_CODE_REPORTINGX_STR] = {
            "violation": singleModelCutoff,
            "fraction": negativeFraction,
        }
        self.violationCodes[self.VIOLATION_CODE_REPORTINGL_STR] = {
            "violation": multiModelCutoff,
            "fraction": negativeFraction,
        }
        ...
        for violationCode in self.violationCodeList:
            if violationCode not in self.violationCodes:
        ...
        (self.resAtomSwapDict, self.prochiralResonancesDict) = createResAtomSwapDict(
            self.resAtomSetDict, compareWithWattos=self.compareWithWattos
        )
        if self.verbose:
            print("Made resAtomSwapDict,prochiralResonancesDict")
        ...
        for res in resList:
            ...
            if prochiralKey not in self.triplets:
                self.triplets[prochiralKey] = {}

            if atomTuple not in self.triplets[prochiralKey]:
        ...
        for prochiralKey in prochiralKeys:
            ...
            for atomTuple in atomTuples:
                ...
                for resonance in self.triplets[prochiralKey][atomTuple]:
                    ...
                    for constraintItem in resonance.pairwiseConstraintItems:
                        ...
                        if constraint not in otherItems:
                        ...
                        if (
                            otherResonance not in otherItems[constraint]
                        ):  # Use this now for future Python3 compatibility
                        ...
                        if constraint.className in ("DistanceConstraint", "HBondConstraint"):
                            if constraint not in connectedConstraints:
                                ...
                                if (
                                    otherResonance.resonanceSet
                                    and len(otherResonance.resonanceSet.atomSets) == 1
                                    and otherResonance in self.prochiralResonancesDict
                                ):
            ...
            priorityKey = (
                ...
                len(unambiguousStereoConstraints),
                chainIdCcpn,
                resIdCcpn,
                chemAtomSetName,
            )
            #      print "DEBUG: priorityKey:", priorityKey
            if priorityKey not in prochiralPriority:
            ...
            for prochiralKey in prochiralPriority[priorityKey]:
                ...
                for violationCode, violationList in violResultTupleList:
                    if violationCode == self.VIOLATION_CODE_REPORTINGS_STR:  # Includes any possible violation.
                        orgMaxViolation[prochiralKey] = max(
                            orgMaxViolation[prochiralKey], max(violationList)
                ...
                doSwapCount = 0.0
                totalEnergyHighState = (
                    0.0  # actual high state will be determined after next loop. For now assume state 0 (unswapped)
                )
                ...
                for modelIndex in range(self.numModels):
                    ...
                    if (
                        energyHighState > energyLowState
                    ):  # swapping needed because for this model the assumption on the unswapped being the highest energy state was correct
                        doSwapCount += 1.0
                ...
                if (
                    totalEnergyHighState > 0.0
                ):  # Strange in Wattos code there's no safety on totalEnergyHighState being zero. Added here.
                ...
                if totalEnergyHighState <= totalEnergyLowState:
                    msg = "criterium not met: totalEnergyHighState > totalEnergyLowState: %.3f and %.3f" % (
                        totalEnergyHighState,
                        totalEnergyLowState,
                    )
                elif percentageModelFavoured < swapPercentage:
                    msg = "criterium not met: percentageModelFavoured >= swapPercentage: %.1f %.1f" % (
                        percentageModelFavoured,
                        swapPercentage,
                    )
                elif energyDifference < energy_abs_criterium:  # If diff is close to zero do nothing.
                    msg = "criterium not met: energyDifference >= energy_abs_criterium: %.3f and %.3f" % (
                        energyDifference,
                        energy_abs_criterium,
                    )
                elif energyDifferencePercentage < energy_rel_criterium:
                    msg = "criterium not met: energyDifferencePercentage >= energy_rel_criterium: %.1f %.1f" % (
                        energyDifferencePercentage,
                ...
                if swapAssignment:
                    ...
                    for resList in self.triplets[prochiralKey].values():
                        for resonance in resList:
                            if resonance not in prochiralResonances:
                                prochiralResonances.append(resonance)
        ...
        for priorityKey in priorityKeys:
            ...
            for prochiralKey in prochiralPriority[priorityKey]:
                ...
                (prochiralViolationInfo, allConstraintItems) = self.checkProchiralKeyConstraints(
                    prochiralKey, debug=debug
                )
                ...
                for violationCodeToTest in self.violationCodeList:
                    ...
                    for violationCode, violationList in violResultTupleList:
                        ...
                        if fractionFound >= fractionByViolationCode:  # inclusive
                            if debug:
                                print(
                                    "DEBUG:    DEASSIGNING BASED ON %s %s"
                                    % (
                                        violationCode,
                                        str(prochiralViolationInfo[finalSwapType][self.REQUIRES_DEASSIGNMENT_STR]),
                                    )
                                )
                            deassign = True
                            Deassign_count += 1
                ...
                #        if chainCode in mapChainId2Idx:
...
""")
        ...
        if outputType == "NMRSTAR":
            ...
            if validTripletCount != validTripletCount2:
                print(
                    "Error: found number of triplets with restraints %d but number of report list %d"
                    % (validTripletCount, validTripletCount2)
                )
            #          return True
        ...
        for constraint in constraints:
            ...
            if self.method == "SUM_AVERAGING":
                ...
                if self.useLowestAromaticViolation:
                    ...
                    for item in constraint.items:
                        ...
                        for tmpResonance, tmpAtomList in tmpList:
                            if (
                                tmpResonance in self.prochiralResonancesDict
                                and prochiralKey != self.prochiralResonancesDict[tmpResonance]
                                and len(tmpAtomList) == 1
                            ):
                                otherProchiralChemAtomSet = self.prochiralResonancesDict[tmpResonance][2]
                                if otherProchiralChemAtomSet.name in (
                                    "HD*",
                                    "HE*",
                                ) and otherProchiralChemAtomSet.chemComp.ccpCode in ("Phe", "Tyr"):
                                    ...
                                    if setItem:
                                        ...
                                        checkAromaticLowestViolations[item] = (
                                            (otherInfo[1], tmpAtomList),
                                            otherLowestViolationAtom,
                                            itemResonances.index(tmpResonance),
                                        )
                ...
                for item in constraint.items:
                    ...
                    if atomListsAdded == 2:
                        print(
                            "  Warning: restraint item between prochiral resonances of same triplet %s.%d.%s. Ignoring... ."
                            % (prochiralKey[0], prochiralKey[1], prochiralKey[2].name)
                        )
                        continue
                            ...
                            for atom in atomList:
                                if atom in self.atomCoordDict:
                                    for otherAtom in otherAtomList:
                                        ...
                                        if otherAtom in self.atomCoordDict:
                                            if self.atomCoordDict[atom] and self.atomCoordDict[otherAtom]:
                                                for i in range(self.numModels):
                                                    avgSumContrib = 1.0 / math.pow(
                                                        getDistanceFromCoordinates(
                                                            self.atomCoordDict[atom][i],
                                                            self.atomCoordDict[otherAtom][i],
                                                        ),
                                                        6,
                                                    )
                                                    ...
                                                    if checkAromaticAtomInfo:
                                                        otherAvgSumContrib = 1.0 / math.pow(
                                                            getDistanceFromCoordinates(
                                                                self.atomCoordDict[checkAromaticAtomInfo[0]][i],
                                                                self.atomCoordDict[checkAromaticAtomInfo[1]][i],
                                                            ),
                                                            6,
                                                        )

                                                    # Take shortest distance (highest avgSumContrib)
                                                    avgLocalSums[localSumsIndex][i] += max(
                                                        avgSumContrib, otherAvgSumContrib
                                                    )

                                            else:
                                                print(
                                                    "  Warning: empty coordinate for atom %s or %s" % (atom, otherAtom)
                                                )
                                        else:
                ...
                for i in range(len(self.swapTypes)):
                    ...
                    for violationCode in self.violationCodeList:
                        ...
                        if violationCount and (violationFraction > self.violationCodes[violationCode]["fraction"]):
                            prochiralViolationInfo[swapType][self.REQUIRES_DEASSIGNMENT_STR].append(
                                (violationCode, violationInfo[violationCode])
                            )
                    # end for violationCode
                    if debug:
...
def ccpnDistanceRestraintToString(ccpnConstraint):
    ...
    for constItem in ccpnConstraint.sortedItems():
        ...
        if resonanceListLength != 2:
            print(
                "ERROR: expected a pair but found number: %d for ccpnConstraint %s"
                % (resonanceListLength, ccpnConstraint)
            )
            return None
