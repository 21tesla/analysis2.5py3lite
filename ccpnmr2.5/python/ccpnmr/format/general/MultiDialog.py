...
from ccp.format.general.Constants import atomOrder, defaultSeqInsertCode
from ccp.general.Util import setCurrentStore
from ccpnmr.format.general.Constants import assign_kw, newline, stereoText
from ccpnmr.format.general.userInteraction import setupMessageReporter
from ccpnmr.format.general.Util import createExperiment, getNameInfo
...
class MultiDialog:
  ...
  """
    ...
    def ExperimentTypesList(self, guiParent, project, *args, **kw):
        ...
        if self.messageReporter.showYesNo(
            "Edit reference experiments",
            "You will directly edit the reference experiments. If done incorrectly this might results in problems later on. Do you want to continue?",
        ):
  ...
  """
    ...
    def ProcParToExpDimInitialize(self, *args, **kw):
        ...
        for dim in range(0, len(fPars["nucleus"])):
            if not linkDict[dim]:
                linkDict[dim] = expDims[0].sortedExpDimRefs()[0]
                expDims.pop(0)
  ...
  """
    ...
    def LinkResonancesSetup(self, guiParent, project, nmrConstraintStore, *args, **kw):
        ...
        if self.linkResonancesFindFormat(project, nmrConstraintStore):
            ...
            if ("fastLinking" in kw and kw["fastLinking"]) or self.messageReporter.showYesNo(
                "Use fast linking", "Click Yes if you want to link automatically with default settings."
            ):
                interaction = self.DefaultLinkResonancesSetup()

            else:
                interaction = self.LinkResonancesSetupFunction(
                    guiParent, self.formatNamesList, self.formatNamesDict, title
                )
            ...
            self.messageReporter.showError(
                "No external format data",
                "There are unlinked resonances present, but they do not have information from external format files (and were probably created by Analysis). LinkResonances quitting.",
            )
    ...
    def DefaultLinkResonancesSetup(self):
        ...
        nonGuiClass.globalStereoAssign = False
        nonGuiClass.useIupacMatching = True
        nonGuiClass.useLinkResonancePopup = False
        nonGuiClass.minimalPrompts = True
        nonGuiClass.forceShiftMerge = True
        nonGuiClass.useAmbiguity = False
        nonGuiClass.verbose = True
        nonGuiClass.setSingleProchiral = True
        nonGuiClass.setSinglePossEquiv = True
        nonGuiClass.makeMethylsEquivalent = True
    ...
    def linkResonancesFindFormat(self, project, nmrConstraintStore):
        ...
        if formatsFound != {}:
            ...
            for format in formatsFound.keys():
                ...
                if percentage not in formatPercentages:
  ...
  """
    ...
    def ChainLink(self, guiParent, formatChains, molSystem, forceDefaultChainMapping=0, doInteraction=True):
        ...
        if forceDefaultChainMapping:
            if len(self.ccpChainLabelDict) == 1 and len(formatChains) == 1:
                ...
                if foundMatch:
                    nonGuiClass.chainDict[self.ccpChainLabelDict[ccpChainLabel]] = [
                        formatChainCode,
                        ccpCodeLow,
                        formatChains[formatChainCode],
                    ]
                    return nonGuiClass
    ...
    def ChainLinkFunction(self, guiParent, formatChains):
        ...
        for formatChainText in self.formatChainList[:-1]:
            ...
            print(
                "Pick corresponding Ccp chain for '%s', %d residues, %d first seqCode:"
                % (formatChainCode, len(seqCodesList), seqCodesList[0][0])
            )
            ...
            if chosenChain != self.defaultFormatChain:
                ...
                print(
                    "Sequence code %d%s for chain '%s' corresponds to which sequence Id for %s?"
                    % (formatFirstSeqCode, seqInsertCodeText, formatChainCode, chosenChain)
                )
                ...
                nonGuiClass.chainDict[self.ccpChainLabelDict[chosenChain]] = [
                    formatChainCode,
                    chosenSeqId,
                    seqCodesList,
                ]
    ...
    def ChainLinkInitialize(self, formatChains, molSystem):
        ...
        self.ccpChainCcpCodes = {}

        for chain in molSystem.sortedChains():
            if len(chain.residues) == 1 and chain.findFirstResidue().chemCompVar.chemComp.ccpCode == "Hoh":
                continue
            ...
            self.ccpChainCcpCodes[chainLabel] = []
            ...
            for residue in chain.sortedResidues():
                ...
                self.ccpChainCcpCodes[chainLabel].append(residue.ccpCode)
            ...
            for seqInsertCode in seqInsertCodes:
                ...
                if seqInsertCode and len(seqCodeStretchDict[seqInsertCode]) > 1:
                    for seqCodeStretch in seqCodeStretchDict[seqInsertCode]:
                        ...
                        formatChainText = "Link to code '%s' (range %s-%s)" % (
                            formatChainCode,
                            startSeqCodeText,
                            endSeqCodeText,
                        )
  ...
  """
    ...
    def namingSystemPercentages(self, resNameFound, numResonances):
        ...
        for namingSystemName in resNameFound:
            ...
            if atomPercent not in atomPercentDict:
  ...
  """
    ...
    def coordinateAtomSelectDefineElementAtomSets(self, nmrRes):
        ...
        for atom in nmrRes.atoms:
            ...
            if elementSymbol not in self.elementAtomSets:
    ...
    def coordinateAtomSelectSuggestMatch(self, atomName):
        ...
        if atomMatchElement != None:
            if atomMatchElement in self.elementAtomSets:
        ...
        if not suggestElementAtomSets:
            if not defaultElementAtomSets:
    ...
    def atomSelectDefineElementAtomSets(self, nmrRes):
        ...
        for atom in nmrRes.atoms:
            ...
            if chemAtomSet != None:
                ...
                if deepChemAtomSet:
                    if deepChemAtomSet not in self.elementAtomSets[elementSymbol]:
                        self.elementAtomSets[elementSymbol][deepChemAtomSet] = ElementAtomSet(
                            chemAtom, nmrRes, chemAtomSet=chemAtomSet, deepChemAtomSet=deepChemAtomSet
                        )
                    ...
                    if chemAtomSet not in self.elementAtomSets[elementSymbol]:
                        self.elementAtomSets[elementSymbol][chemAtomSet] = ElementAtomSet(
                            chemAtom, nmrRes, chemAtomSet=chemAtomSet
                        )
  ...
  """
    ...
    def resonanceSelectInitialize(self, existingResonances, nmrRes, multi):
        ...
        for resonance in matchedResonances.keys():
            ...
            listName = "Resonance '%s' (to atoms '%s', orig names '%s')" % (
                resonance.name,
                string.join(otherAtomsNameList, ","),
                listResName,
            )
  ...
  """
    ...
    def ResonanceGroup(self, guiParent, resNameList, nmrRes, allowInteraction):
        ...
        if not returnClass.resNameGroups and allowInteraction:
            ...
            print(
                "  Warning: autogrouped %s and %s."
                % (str(returnClass.resNameGroups[0]), str(returnClass.resNameGroups[1]))
            )
        ...
        for i in range(1, len(self.resonanceLineDict)):
            ...
            for tempGroup in tempGroups:
                ...
                if seqCode == refSeqCode and seqInsertCode == refSeqInsertCode:
                    if (refAtomName.count(atomName) or atomName.count(refAtomName)) or (
                        atomName[-2] in numberString
                        and atomName[:-1] == refAtomName[:-1]
                        and atomName[-2] == refAtomName[-2]
                    ):
                        if (
                            atomName[-1] in numberString + wildcardString
                            and refAtomName[-1] in numberString + wildcardString
                        ):
  ...
  """
    ...
    def singleProchiralStatusInitialize(self, refAtomSet, otherAtomName):
        ...
        self.optionList = [
            "%s is assigned and has the same chemical shift as %s." % (otherAtomName, refAtomSet.name),
            "%s is not assigned, the chemical shift is unknown." % otherAtomName,  # ,
            # "%s not assigned, this one stereo assigned" % otherAtomName
        ]

        self.message = (
            "Atom(set) %s is prochiral, but nothing is known about the other prochiral atom(set) %s.\nPlease select its status below."
            % (refAtomSet.name, otherAtomName)
        )
        self.title = "Project '%s': " % self.nmrRes.root.name + "Prochiral status selection"
  ...
  """
    ...
    def SingleResonanceStatusInitialize(self, refAtom, otherAtomName):
        ...
        self.optionList = [
            "%s and %s have the same chemical shift (equivalent)." % (otherAtomName, refAtom.name),
            "%s and %s have a different chemical shift (not equivalent)." % (otherAtomName, refAtom.name),
        ]

        self.message = (
            "The atoms %s and %s usually have the same chemical shift,\nbut in this case some separate information was identified.\nPlease select their status below."
            % (refAtom.name, otherAtomName)
        )
  ...
  """
    ...
    def MeasurementSelect(self, guiParent, resName, resonance, newValue, measurementList, measurementApiCode):
        ...
        selectValue = self.measurementSelectInitialize(
            resonance, measurementList, resName, newValue, measurementApiCode
        )
    ...
    def measurementSelectInitialize(self, resonance, measurementList, resName, newValue, measurementApiCode):
        ...
        if selectValue == None:
            ...
            self.topMessage += "Resonance '%s' %shas %s %.3f, " % (
                resName,
                atomNameText,
                measurementText,
                measurementValue,
            ...
            if otherMeasurementValues:
                ...
                for i in range(0, len(otherMeasurementValues)):
                    ...
                    self.topMessage += (
                        "(other resonance %s connected to these atoms has %s %.3f)" % (name, measurementText, value)
                        + newline
                    )
  ...
  """
    ...
    def prochiralStatusInitialize(self, project):
                    ...
                    while atomList:
                        ...
                        for resParent in resParents:
                            ...
                            if atomSet:
                                ...
                                if resParent not in atomSetsList:
                                ...
                                if prochiralChemAtoms:
                                    ...
                                    for resParent in atomSetDict.keys():
                                        ...
                                        for i in range(0, len(atomSetsList[resParent])):
                                            if atomOrder.index(atomPositionCode) < atomOrder.index(
                                                atomSetsList[resParent][i][0].name[1]
                                            ):
                                    ...
                                    if otherAtomList:
                                        for resParent in atomSetDict.keys():
                                            if isinstance(resParent, NmrConstraint.NmrConstraintStore):
                                                otherAtomSet = otherAtomList[0].findFirstFixedAtomSet(
                                                    nmrConstraintStore=resParent
                                                )
                                            ...
                                            if not otherAtomSet:
                                                ...
                                                otherAtomSet = atomSetClass(
                                                    resParent, atoms=otherAtomList, name=atomSetName
                                                )

                                            if atomSet.name < otherAtomSet.name:
                                                atomSetsList[resParent][atomSetsListIndex[resParent]].append(
                                                    otherAtomSet
                                                )
                                            else:
                                                atomSetsList[resParent][atomSetsListIndex[resParent]].insert(
                                                    0, otherAtomSet
                                                )
        ...
        for resParent in resParents:
            if resParent in self.chainAtomSetsDict:
                for molSys in project.sortedMolSystems():
                    for chain in molSys.sortedChains():
                        if (
                            chain in self.chainAtomSetsDict[resParent]
                            and not self.chainAtomSetsDict[resParent][chain]
                        ):
...
class ElementAtomSet:
    ...
    def addChemAtomSet(self, chemAtom, chemAtomSet):
        ...
        if chemAtomSet not in self.chemAtomSets:
