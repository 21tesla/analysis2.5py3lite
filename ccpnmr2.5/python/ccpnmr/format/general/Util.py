...
from ccp.general.Util import setCurrentStore, setUniqueAppData
...
from memops.universal.Util import returnFloat, returnInt, returnLong
...
def setNonDictKeywords(function, IOkeywordList, numMandatory):
    ...
    for IOkeyword, default, mandatory, infoText in (
        (
            "fileName",
            None,
            True,
            "Always first keyword, is name of file to %s. The file name includes the directory path." % actionType,
        ),
        (
            "fileNames",
            None,
            True,
            "Always first keyword, is list of names of file to %s. The file names include the directory path."
            % actionType,
        ),
...
def getFormatConverterInfoDict():
    ...
    automaticKeywords = {"Constraints": "constraintType", "Measurements": "measurementType"}
    ...
    for formatName in formatNames:
            ...
            for superClass in subClasses.keys():
                if functionName in ("read%s" % superClass, "write%s" % superClass):
                    ...
                    for subClass in subClasses[superClass]:
                        ...
                        if subClassFunctionName not in subClassedFunctions:
                        ...
                        if automaticKeyword in formatObject.IOkeywords[subClassFunctionName]:
                            (default, mandatory, infoText) = formatObject.IOkeywords[subClassFunctionName][
                                automaticKeyword
                            ]
                            formatObject.IOkeywords[subClassFunctionName][automaticKeyword] = (
                                default,
                                mandatory,
                                True,
                                infoText,
                        ...
                        for IOkeyword, default, mandatory, isAutomatic, infoText in addIOkeywordList:
                            formatObject.IOkeywords[subClassFunctionName][IOkeyword] = (
                                default,
                                mandatory,
                                False,
                                infoText,
        ...
        for functionName in functionNames:
                ...
                if functionName in validFunctionNames:
                    ...
                    if formatName in formatsDict:
                        ...
                        if formatsDict[formatName][functionIndex + 1]:
                            ...
                            if functionName not in functionWidgetInfo:
            ...
            if actionType not in functionNameInfo:
        ...
        for actionType in functionNameInfo.keys():
            ...
            for component in functionNameInfo[actionType].keys():
                ...
                if functionName in fileNameInfo:
                ...
                for IOkeyword, default, mandatory, isAutomatic, infoText in functionNameInfo[actionType][component][1]:
                    if IOkeyword in functionWidgetInfo[functionName]:
                        (widgetType, defaults, label) = functionWidgetInfo[functionName][IOkeyword]
                    else:
                        (widgetType, defaults, label) = (None, None, None)

                    formatInfoDict[formatName][actionType][component][2].append(
                        (IOkeyword, default, mandatory, isAutomatic, infoText, widgetType, defaults, label)
                    )
...
def getValidExportFormats(project):
        ...
        for className in measurementListClassNames:
            for measurementList in nmrProject.findAllMeasurementLists(className=className):
                if measurementList.measurements:
                    if className not in ccpnObjects:
    ...
    for nmrConstraintStore in project.nmrConstraintStores:
        ...
        for className in constraintListClassNames:
            if className in hasConstraints:
                continue
    ...
    for objectType in projectExportRequirements.keys():
        ...
        if objectType == "chain" and not hasChains:
            ...
            hasInfo = False
        elif objectType == "shiftList" and "Shift" not in hasMeasurements:
            hasInfo = False
        ...
        for formatName in projectExportRequirements[objectType]:
            if formatName not in hasProjectInfoPerFormat:
    ...
    for formatName in fcInfo.keys():
        ...
        if "export" in actionTypes:
            ...
            if (
                "project" in components
                and formatName in hasProjectInfoPerFormat
                and hasProjectInfoPerFormat[formatName]
            ):
...
def getResName(chainCode, seqCode, atomName, seqInsertCode="", spinSystemId=None, fixChainCode=True):
    """NBNB TBD Rsmus July 2012:
    What happens if atomName is an empty string?
    Can come about e.g. in Ansig if only sequence is asssigned for a PeakDim
    """
...
def createSelection(ccpnObjectList):
    ...
    if ccpnObjectList:
        ...
        if type(ccpnRefObject) in [Nmr.Experiment, Affiliation.Organisation]:
        ...
        elif type(ccpnRefObject) in [
            NmrConstraint.RdcConstraintList,
    ...
    if resonance.name == None or forceChange == 1:
        ...
        if assignData:
            if assignData.value != resName:
                if forceChange == 0:
                    print(
                        "  Warning: couldn't set ccpNmr resonance name %s (name already set to %s)"
                        % (resName, assignData.value)
                    )
                else:
                    assignData.delete()
                    resonance.addApplicationData(
                        Implementation.AppDataString(application=ccpNmr_kw, keyword=assign_kw, value=resName)
                    )
        ...
        if resonance.name != atomName:
            print(
                "  Warning: couldn't set ccpNmr name %s for resonance %s (name already set)"
...
def getApplResNames(format, resonances, keyword=assign_kw):
    ...
    for resonance in resonances:
        ...
        for applDatum in applData:
            ...
            if resName not in resonanceNames:
...
def updateResonanceNamesDict(resonance, resonanceNames, assignFormat=None):
    ...
    for resonanceNameFound in resonanceNamesFound:
        if resonanceNameFound not in resonanceNames:
...
def getOriginalData(format, parent, rawData, formatName, setFormatName=None):
    ...
    if applData:
        ...
        try:
            ...
            if (
                type(value) == type([])
                and hasattr(rawData, setFormatName)
                and type(getattr(rawData, setFormatName)) == type([])
            ):
...
def createExperiment(proj, expName, refExperiment, unit="ppm"):
    ...
    for i in range(0, numDim):
        ...
        for refExpDimRef in refExpDim.refExpDimRefs:
            ...
            expDimRef.addApplicationData(
                Implementation.AppDataBoolean(application=ccpNmr_kw, keyword="default", value=True)
            )
            ...
            for otherAtomSiteList in atomSiteList[i + 1 :]:
                for otherExpDimRef, otherAtomSites in otherAtomSiteList:
                    for expGraph in atomSites[0].nmrExpPrototype.expGraphs:
                        ...
                        if refExpTransfer:
                            expTransfer = exp.findFirstExpTransfer(
                                transferType=refExpTransfer.transferType, expDimRefs=expDimRefSet
                            )
                            if not expTransfer:
                                expTransfer = exp.newExpTransfer(
                                    transferType=refExpTransfer.transferType, expDimRefs=expDimRefSet
                                )
...
def createPpmFreqDataSource(exp, dsName, dsType, numDim):
    ...
    for dim in range(0, numDim):
        ...
        freqDataDim.addApplicationData(
            Implementation.AppDataBoolean(application=ccpNmr_kw, keyword="default", value=True)
        )
    ...
    if protonFreq == 0:
        if otherFreqs:
            ...
            for isotopeCode in isotopeCodes:
                if isotopeCode in otherFreqRefPpm and isotopeCode in INDIRECT_FREQ_CONV:
    ...
    for softwareName in softwareDefinitions.keys():
        ...
        for taskName in softwareDefs["methods"].keys():
            for procedureName in softwareDefs["methods"][taskName]:
                method = project.currentMethodStore.findFirstMethod(
                    software=software, task=taskName, procedure=procedureName
                )

                if not method:
                    method = Method.Method(
                        project.currentMethodStore, software=software, task=taskName, procedure=procedureName
                    )
