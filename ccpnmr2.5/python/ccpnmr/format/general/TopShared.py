...
class TopShared:
        ...
        if not self.molSystem:
            ...
            while molSysExists:
                molSysCode = self.dataEntry.askString(
                    "Molecular system name", "Code for new molecular system:", molSysCode, self.guiParent
                )

                if self.project.findFirstMolSystem(name=molSysCode):
                    self.messageReporter.showError(
                        "Error", "Name for molecular system already exists - select another.", parent=self.guiParent
                    )

                elif " " in molSysCode:
                    self.messageReporter.showError(
                        "Error", "Name for molecular system cannot have spaces.", parent=self.guiParent
                    )
    ...
    def getNameMapping(self, namingSystemName=None, isOriginalImport=False, resetMapping=False):
        ...
        nameMappings = self.project.currentNameMappingStore.findAllNameMappings(
            formatName=self.format, namingSystem=namingSystemName
        )

        if nameMappings:
            if len(nameMappings) > 1:
                print(
                    "  Warning: multiple %s %s nameMappings available - picking first one."
                    % (self.format, namingSystemName)
                )
            ...
            if namingSystemName != "Unknown":
                ...
                self.nameMapping = self.project.currentNameMappingStore.findFirstNameMapping(
                    formatName=self.format, namingSystem="Unknown"
                )
        ...
        if not self.nameMapping:
            self.nameMapping = self.project.currentNameMappingStore.newNameMapping(
                formatName=self.format, namingSystem=namingSystemName, isOriginalImport=isOriginalImport
            )
    ...
    def setChainLinks(self, forceChainMappings=None, resetMapping=False, verbose=0):
        ...
        if not self.nameMapping:
            ...
            self.getNameMapping(namingSystemName=namingSystemName, isOriginalImport=True, resetMapping=resetMapping)
        ...
        if forceChainMappings:
            ...
            for forceChainMapping in forceChainMappings:
                ...
                if not chain:
                    print(
                        "  Error: forceChainMappings chain code '%s' is not present in molSystem! Aborting..."
                        % chainCode
                    )
                    forceMapping = 0
                    break
            ...
            if forceMapping:
                ...
                for forceChainMapping in forceChainMappings:
                    ...
                    self.setChainMappingDicts(
                        chain, formatChainCode, firstSeqId=firstSeqId, offset=offset, seqInsertCode=seqInsertCode
                    )
        ...
        if self.nameMapping.chainMappings:
                ...
                if chain:
                    ...
                    if hasattr(self, "resUnassignedNames"):
                        ...
                        for resName in self.resUnassignedNames.keys():
                            (chainCode, seqCode, spinSystemId, seqInsertCode, atomName) = getNameInfo(
                                resName, verbose=0
                            )
                            ...
                            if (seqCode, seqInsertCode) not in residueList:
                                ...
                                if (
                                    ...
                                    and seqCode >= formatFirstSeqCode
                                    and seqCode <= formatLastSeqCode
                                ):
                                    residueMatches += 1

                    if residueMatches == totalResidues:
                        setChainMappings.append((chain, chainMapping.formatChainCode, chainMapping))
                        if chainMapping.formatChainCode not in deleteFormatChains:
            ...
            if doSet:
                ...
                for formatChainCode in deleteFormatChains:
                    ...
                    if formatChainCode in self.formatChains:
                        # print " DELETED",
                        ...
                        if len(chain.residues) - resPos >= len(self.formatChains[formatChainCode]):
                            ...
                            if residue.seqCode == seqCode and residue.seqInsertCode == seqInsertCode:
                                self.setSeqAndInsertCodeToChainMapping(
                                    formatChainCode, self.formatChains[formatChainCode], chain, resPos, verbose=verbose
                                )
        ...
        if linkChains == 1:
            ...
            if hasattr(interaction, "chainDict") and interaction.chainDict != {}:
                for chain in interaction.chainDict:
                    ...
                    self.setSeqAndInsertCodeToChainMapping(
                        formatChainCode, seqAndInsertCodesList, chain, firstSeqId - firstResidueSeqId, verbose=verbose
                    )
                ...
                if not self.setMappingFromChainMappings:
                    ...
                    print(
                        "  Missing mappings for format chains: %s. Continuing because other mappings were set."
                        % str(self.formatChains.keys())
                    )
    ...
    def getChainMappingsSeqCodeInfo(self, chainMappings):
        ...
        specificInfo = None
        if not hasattr(self, "specialSeqCodeMapping") or not self.specialSeqCodeMapping:
            specificInfo = self.getSpecificChainMappingsSeqCodeInfo(chainMappings)

        if specificInfo:
            (seqCodeToIdMapping, seqCodeToChainMapping, seqCodes) = specificInfo
    ...
    def setChainMappingDicts(
        ...
        if not chainMapping:
            ...
            keywds = {
                ...
                "formatChainCode": returnMemopsLine(formatChainCode),
                "firstSeqId": firstSeqId,
            ...
            chainMapping = self.nameMapping.newChainMapping(**keywds)
