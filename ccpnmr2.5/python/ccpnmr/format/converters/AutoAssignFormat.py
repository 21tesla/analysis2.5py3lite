...
from ccpnmr.format.general.Constants import height_kw, volume_kw
...
class AutoAssignFormat(DataFormat):
    ...
    def createFullProject(self, fileName, peakListInfo=None, tolerances=None, setTag=None):
        ...
        if not tolerances or not peakListInfo:
            self.messageReporter.showError(
                "Error",
                "Cannot write %s project in non-GUI enviroment without passing in peakListInfo list and tolerances dictionary."
                % self.format,
        ...
        for specType, peakList, phase in peakListInfo:
            ...
            if not peakList.peaks:
                self.messageReporter.showError(
                    "Error",
                    "Cannot export peak list of type %s - no peaks available!" % specType,
                    parent=self.guiParent,
                )
                continue
            ...
            for dataDimRef in dataDimRefs:
                ...
                if expDimRef.refExpDimRef:
                    ...
                    if not atomInfo:
                        print(
                            "  Warning: no atomSite info for expDimRef dim %d of reference experiment %s - guessing information from expDimRef."
                            % (expDimRef.expDim.dim, expMeasurement.nmrExpPrototype.name)
                        )
                ...
                if not atomInfo:
                    ...
                    if "1H" in expDimRef.isotopeCodes:
                        ...
                        if not atomInfo:
                            print(
                                "  Warning: no specific 13C atom type found in experiment type '%s' - set to 'C13'."
                                % specType
                            )
                            atomInfo.append("C13")
                        ...
                        print(
                            "  Warning: no %s atom info set for expDimRef with isotope codes '%s'."
                            % (self.format, str(expDimRef.isotopeCodes))
                        )
                        atomInfo.append("X")
                ...
                if expDimRef.minAliasedFreq:
                    ...
                    lowPpm = (
                        dataDimRef.refValue
                        + (dataDimRef.refPoint - 1 - freqDataDim.numPoints) * dataDimRef.valuePerPoint
                    )
            ...
            self.peakFile.setProjectSpectrumInfo(
                specType, rootSpecName, intra, sequential, throughSpace, phase, dimInfo
            )
    ...
    def setSequenceFileElements(self):
        ...
        self.sequence.elements.append(
            self.sequenceIO.AutoAssignSequenceElement(self.seqCode, self.getResidueCode1Letter(self.residue))
        )
    ...
    def setPeakIntensity(self):
        ...
        if self.rawPeak.intensity != 0:
            ...
            peakInt = Nmr.PeakIntensity(
                self.peak, value=self.rawPeak.intensity, method=self.methods["unknown"]["Intensity"]
            )
    ...
    def createPeakFilePeak(self):
        ...
        self.peakFile.peaks.append(self.peaksIO.AutoAssignPeak(self.peakNum, self.ppm, self.intensity))
