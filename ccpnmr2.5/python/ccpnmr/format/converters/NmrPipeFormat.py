...
from ccpnmr.format.converters.BrukerFormat import getBrukerDQDdata, getDMXtypeSpectrometer
...
class NmrPipeFormat(DataFormat):
    ...
    def createProcParsFile(self):
        ...
        if not self.outputFile:
            ...
            self.messageReporter.showError(
                "Error", "No output file given for %s conversion parameters" % (self.format), parent=self.guiParent
            )
        ...
        if self.expOrigin == "varian":
            ...
            self.messageReporter.showError(
                "Error",
                "%s conversion parameters unknown for %s" % (self.format, self.expOrigin),
                parent=self.guiParent,
            )
            return False

        self.procParsFile = self.conversionParsIO.NmrPipeConversionFile(
            self.fileName, self.inputFile, self.outputFile, self.numDim, convCommand
        )
    ...
    def createProcScriptFile(self):
        ...
        self.procScriptFile = self.processingScriptIO.NmrPipeProcessingScriptFile(
            self.fileName, self.inputFile, self.outputFile, self.numDim
        )
    ...
    def setProcParsSpecific(self):
        ...
        for dataDimRef in self.dataDimRefs:
            ...
            values = [
                fidDataDim.numPointsValid * complexFactor,
                fidDataDim.numPointsValid,
                "Complex",
                fidDataDim.spectralWidth,
                expDimRef.sf,
                nominalRefValue,
                label,
            ]
            ...
            for i in range(0, len(values)):
                value = values[i]
                self.procParsFile.dataValues[expDimIndex].append(
                    self.conversionParsIO.NmrPipeConversion(self.procParsFile, i, value)
                )
        ...
        for dataDimRef in self.dataDimRefs:
            ...
            for i in range(0, len(values)):
                ...
                self.procScriptFile.dataValues[expDimIndex].append(
                    self.processingScriptIO.NmrPipeProcessingScript(self, i, value)
                )
