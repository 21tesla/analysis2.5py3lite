...
from ccp.general.Util import findChemAtomSetSysName, findChemAtomSysName, getResonancesFromPairwiseConstraintItem
...
class ConcoordFormat(DataFormat):
    ...
    def setRawDistanceConstraintItemMembers(self):
                ...
                if not chemAtom:
                    ...
                    if chemAtomSetSysName:
                        chemAtomSet = chemCompVar.findFirstChemAtomSet(
                            name=chemAtomSetSysName.atomName, subType=chemAtomSetSysName.atomSubType
                        )
                    else:
                            ...
                            for chemAtomSysName in chemAtomSysNames:
                                if chemAtomSysName:
                                    atomNameLists[-1].append(
                                        getResName(chainCode, seqCode, chemAtomSysName, seqInsertCode=seqInsertCode)
                                    )
                                else:
                                    print(
                                        "  Error: problems with decompressing Concoord name %s - sysNames missing for %s"
        ...
        for resSetName0 in atomNameLists[0]:
            ...
            resLabel0 = (
                resonance.resonanceSet.findFirstAtomSet().findFirstAtom().residue.molResidue.chemComp.ccpCode.upper()
            )
            ...
            for resSetName1 in atomNameLists[1]:
                ...
                resLabel1 = (
                    resonance.resonanceSet.findFirstAtomSet()
                    .findFirstAtom()
                    .residue.molResidue.chemComp.ccpCode.upper()
                )
