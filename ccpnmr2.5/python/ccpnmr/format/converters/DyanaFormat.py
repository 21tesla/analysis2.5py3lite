...
from ccp.general.Util import createMoleculeTorsionDict, getResonancesFromPairwiseConstraintItem
from ccpnmr.format.converters.DataFormat import DataFormat, IOkeywords
from ccpnmr.format.general.Constants import allResidueAtoms_kw, distanceConstraintDefaultLowerLimit
...
IOkeywords["writeRdcConstraints"]["singleAtom"] = (
    False,
    False,
    "Write out RDC constraints with single atom name (old format).",
)
...
class DyanaFormat(DataFormat):
        ...
        if self.constraintType in ["distance", "hBond"]:
            ...
            if not self.lowerLimitFile and not self.minimalPrompts:
                getLowerDist = self.messageReporter.showYesNo(
                    "%s lower limit constraint file" % string.capitalize(messageType),
    ...
    def setSpecificConstraintFileWriteInfo(self):

        if self.constraintType == "rdc" and hasattr(self, "singleAtom"):
            self.writeKeywds["singleAtom"] = self.singleAtom
    ...
    def setRawDistanceConstraintItemMembers(self):
        ...
        for i in range(0, 2):
            ...
            self.rawConstraintItem.members.append(
                self.rawConstraintItemMemberClass(chainCode, seqCode, resLabel, atomName)
            )
    ...
    def setRawDihedralConstraint(self):
        ...
        self.constraintFile.constraints.append(
            self.rawConstraintClass(self.constraint.serial, None, self.constraintFile.cyanaLib)
        )
    ...
    def setRawDihedralConstraintItemMembers(self):
        ...
        if molecule not in self.moleculeTorsions:
            if self.verbose:
        ...
        if not molRes or molRes not in self.moleculeTorsions[molecule]:
            if self.verbose:
        ...
        matchInfo = (
            ("forward", (0, 1, 2, 3)),
            ("backward", (3, 2, 1, 0)),
            ("forward_inout", (1, 0, 3, 2)),
            ("backward_inout", (2, 3, 0, 1)),
        )
        torsion = None
        ...
        if matchPatterns or not torsion:
            ...
            for pattern in ("0111", "1110", "0110"):
                for matchType, matchIndexes in matchInfo:
                    if pattern in matchPatterns[matchType]:
            ...
            if torsion:
                ...
                for coordSysName in ("ideal", "pdb"):
                    ccc = self.project.findFirstChemCompCoord(
                        sourceName=coordSysName, molType=molRes.molType, ccpCode=molRes.ccpCode
        ...
        if torsion:
            ...
            if namingSystem:
                ...
                if torsionSysName:
                    ...
                    print(
                        "  Error: could not find a %s sysName for %s, angle name %s"
                        % (self.namingSystemName, molRes.ccpCode, torsion.name)
                    )
            elif self.verbose:
                print(
                    "  Error: could not find a %s naming system for chemComp %s"
            ...
            if self.verbose:
                if None not in chemAtoms:
                    print(
                        "  Error: could not define a torsion angle name for %s, atoms %s-%s-%s-%s."
                        % (molRes.ccpCode, chemAtoms[0].name, chemAtoms[1].name, chemAtoms[2].name, chemAtoms[3].name)
                    )
                else:
                    print(
                        "  Error: could not define a torsion angle name for %s because of missing chemAtoms"
                        % (molRes.ccpCode)
                    )
    ...
    def setRawRdcConstraint(self):
        ...
        self.constraintFile.constraints.append(
            self.rawConstraintClass(self.constraint.serial, self.constraintFile.cyanaLib)
        )
    ...
    def setRawCoordinate(self):
        ...
        modelCoordinate = self.coordinatesIO.DyanaCoordinate(
            self.coordinateSerial, atomName, seqCode, resName, "", self.x, self.y, self.z, insertionCode=seqInsertCode
        )
