...
from ccpnmr.format.general.Constants import ccpNmr_kw, defaultMolCode, height_kw, tagSep, volume_kw
...
IOkeywords["readPeaks"]["assignTagSep"] = (
    " ",
    False,
    "Separator for ambiguous assignments (e.g. the ' ' in '3.HA 4.HA').",
)
IOkeywords["readPeaks"]["processingParsFile"] = (
    None,
    False,
    "NmrView .par file for setting parameters during experiment creation.",
)
IOkeywords["writePeaks"]["writeMissingPeaks"] = (
    True,
    False,
    "Write out peaks with status -1 in between\n peaks with non-sequential peak numbers.",
)
IOkeywords["writePeaks"]["assignTagSep"] = (
    " ",
    False,
    "Separator for ambiguous assignments (e.g. the ' ' in '3.HA 4.HA').",
)
...
class NmrViewFormat(DataFormat):
    ...
    def setSequenceFileElements(self):
        ...
        self.sequence.elements.append(
            self.sequenceIO.NmrViewSequenceElement(self.seqCode, self.residue.molResidue.chemComp.ccpCode)
        )
        ...
        if self.rawPeak.assign[self.rawPeakDimIndex] not in [None, "?", "null", ""]:
            try:
                ...
                try:
                    ...
                    print(
                        "  Problems handling %s assignment name %s... ignored."
                        % (self.format, self.rawPeak.assign[self.rawPeakDimIndex])
                    )
    ...
    def setPeakFileInfo(self):
        ...
        if self.useXeasyDimCodes:
            # NBNB Rasmus 5 Aug 2012. Replaced with XEASY style, so that CYANA can read it
            # This would work globally, but the if statement avoids breaking normal use.
            peakDimCodes = getXEasyDimCodes(self.dataSource)
    ...
    def setPeakFilePeakDimInfo(self):
        ...
        if self.writeAssignments and not (self.skipMultiAssignments and len(peakContribs) > 1):
    ...
    def setMeasurementFileGenericValue(self):
        ...
        if self.atomName in self.atomNamesDict:
            ...
            if resonanceToAtom.chemAtomSet and resonanceToAtom.chemAtomSet.isEquivalent:
                ...
                atomNames = self.getChemAtomSysNamesFromSet(
                    resonanceToAtom.chemAtomSet, resonanceToAtom.getResidue().chemCompVar
                )
            ...
            for atomName in atomNames:
                self.measurementFileValues.append(
                    self.rawMeasurementClass(measurement.value, atomName, self.seqCode, defaultMolCode)
                )
