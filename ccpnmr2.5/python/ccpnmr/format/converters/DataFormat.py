...
Copyright (C) 2008-2012 Wim Vranken (Vrije Universiteit Brussel)
...
from ccp.api.general import Affiliation, Citation, Instrument, Method
from ccp.api.molecule import ChemComp, Molecule, MolStructure, MolSystem
from ccp.api.nmr import Nmr, NmrConstraint, NmrEntry
from ccp.format.general.Constants import (
    bioPolymerCodes,
    chainCodeString,
    defaultAltLoc,
...
from ccp.general.Constants import (
    ...
    refRepositoryName,
    secStrucInfo_kw,
    shiftIdentityTolerance,
    standardBackboneAtoms,
)
from ccp.general.Geometry import calcPlanarCoord, convertDegreeRadian
from ccp.general.Io import getChemComp, getChemCompCoord, getDataSourceFileName, setDataSourceDataStore
from ccp.general.Util import (
    MakeAtomSets,
    findAllSysNamesByChemAtomOrSet,
    findChemAtomOrSetSysName,
    findChemAtomSysName,
    findChemCompVarSysName,
    getAmbigNamingPriority,
    getChemAtomSetFromAtomSets,
    getDescriptorElementMatches,
...
# Moved functions:
from ccp.lib.StructureLib import checkChemAtomsForDeepSet, makeEnsemble
from ccpnmr.api import NameMapping
from ccpnmr.format.general.Constants import (
    IoSetupList,
    allFormatsDict,
    assign_kw,
    atomSerial_kw,
...
from memops.general.Util import returnMemopsLine, returnMemopsWord
from memops.universal.Io import joinPath, splitPath

#
# Import general functions (normal and Api-dependent)
#
from memops.universal.Util import drawBox, isBigEndian, returnFloat, returnInt, returnInts
...
class ResonanceToAtoms:
    ...
    def clone(self):
        ...
        result = ResonanceToAtoms(*(getattr(self, tag) for tag in tags))

        for tag in ("chemAtom", "chemAtomSet", "originalResName", "atomSetName", "useAtomSetName", "atomType"):
            setattr(result, tag, getattr(self, tag))

        for tag in ("otherLinkedResonances", "otherGroupResonances"):
            setattr(result, tag, list(getattr(self, tag)))
        #
        return result
...
IOkeywords = {
    "readSequence": {
        ...
        "numIdenticalChains": (
            0,
            False,
            "Number of identical chains for a homodimer, homotrimer, ...\n when reading in a single (monomer) sequence.",
        ),
        ...
        "useOriginalChainCode": (
            None,
            False,
            "Use original (author defined) chain code if available. Relevant for archiving formats.",
        ),
        "addNameMappings": (None, False, "Dictionary that can be passed in to map atom names to CCPN atoms."),
        "autoCreateChemComps": (
            False,
            False,
            "If set to True (on), unknown chemComps will be automatically created (if atom names are available!).",
        ),
        "ignoreUnknownChemComps": (
            False,
            False,
            "If set to True (on), unknown chemComps will be automatically ignored.",
        ),
        "ignoreSingleOther": (
            False,
            False,
            "If set to True (on), single residue molecules of type 'other' will be ignored. Useful if, for example, a PDB file with has lots of waters that you want to ignore.",
        ),
        "ignoreResNames": (
            None,
            False,
            "List of residue names that should be ignored when reading sequence (e.g. ['HOH']).",
        ),
        "autoLink": (
            False,
            False,
            "If set to True (on), the sequence elements will be fully automatically determined.",
        ),
        "chemCompMappings": (
            None,
            False,
            "Dictionary that can be passed in to map chemComp names from the format to CCPN chemComp names. Takes the form: {'formatCode': (molType,ccpCode)}",
        ),
        "chemCompPath": (
            None,
            False,
            "Path to directory that contains all chemComp information (if locally available).",
        ),
        "saveChemCompLocal": (
            True,
            False,
            "Save any downloaded chemComps locally. If set to False, will save in refData directory.",
        ),
        "minimalPrompts": (
            False,
            False,
            "If set to True (on), will use automatic settings as much as possible.\n This will reduce the number of popups that appear but is less user-oriented.",
        ),
    },
    "writeSequence": {
        "version": (None, False, "The format version number as string (if relevant, it will be used)."),
        ...
        "resetMapping": (
            True,
            False,
            "If set to False (off), the existing chain mapping will be used for export\n (otherwise it can be reset).",
        ),
        "useCcpnChainInfo": (
            False,
            False,
            "If set to True (on), the CCPN chain and sequence codes will be directly used for export.",
        ),
        "useOriginalData": (
            False,
            False,
            "If set to True (on), original format-specific data (from the import)\n will be used for export.",
        ),
        "forceDefaultChainMapping": (
            False,
            False,
            "If set to True (on), the default chain mapping\n (between the format and the data model)\n will be automatically used.",
        ),
    ...
    "readChemComps": {
        ...
        "makeNamingSystem": (
            None,
            False,
            "If set, will create a naming system for all the atom names based on the original name.",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "writeChemComp": {
        ...
        "copyCoordinates": (
            True,
            False,
            "If set to False (off), the chemCompCoord files will not be copied if found in the archive or online.",
        ),
        "chemCompVar": (None, False, "ChemCompVar to write to the external format."),
        "forceNamingSystemName": (
            None,
            False,
            "If set, will use this naming system instead of the default one for the atom names. Leave as None for default!",
        ),
        "coordSystem": (None, False, "Name of the coordinate system to use for getting the atom coordinates."),
        "writeLinkAtoms": (
            True,
            False,
            "If set to False (off), will not try to create atoms and coordinates for link atoms",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "readCoordinates": {
        ...
        "strucGen": (
            None,
            False,
            "The new structures will automatically be linked\n to this StructureGeneration object.",
        ),
        "structureEnsemble": (
            None,
            False,
        ...
        "matchToAtomNames": (
            False,
            False,
            "If set to True (on), will try to match the names of the coordinate atoms to the actual atom names if none found for the naming system.",
        ),
        "forceChainMappings": (
            None,
            False,
            "Dictionary that can be passed in to automate the coordinate\n to data model chain/residue mapping.",
        ),
        "forceDefaultChainMapping": (
            False,
            False,
        ...
        "forceNamingSystemName": (
            None,
            False,
            "If set, will use this naming system instead of the default one. Leave as None for default!",
        ),
        "addNameMappings": (
            None,
            False,
            "Dictionary that can be passed in to map coordinate atom names to CCPN atoms.",
        ),
        "autoCreateChemComps": (
            False,
            False,
        ...
        "ignoreUnknownChemComps": (
            False,
            False,
            "If set to True (on), unknown chemComps will be automatically ignored (only relevant if sequence read).",
        ),
        "ignoreSingleOther": (
            False,
            False,
        ...
        "ignoreResNames": (
            None,
            False,
            "List of residue names that should be ignored when reading coordinates (e.g. ['HOH']).",
        ),
        "chemCompMappings": (
            None,
            False,
        ...
        "trackAtomSerials": (
            False,
            False,
            "If set to True (on), it will track the atom serials of the first model of the file that is read in.",
        ),
    ...
    "readMeasurements": {
        ...
        "presetResonanceMapping": (
            False,
            False,
            "If set to True (on), will keep using first (or previously passed-in) Resonance:Atom mapping",
        ),
    },
    ...
    "readShifts": {  # Specific to chemical shift reading only
        "autoMergeShifts": (
            False,
            False,
            "If set to True (on), shifts belonging to the same resonance and shift list will be automatically merged and get the average value.",
        ),
        "measurementAllowedDiff": (
            chemShiftAllowedDiff,
            False,
    ...
    "readT1Rhos": {  # Specific for T1 rho relaxation
        ...
        "tempCalibMethod": (
            None,
            False,
            "Method used to calibrate the temperature unit on the NMR spectrometer used to collect the experimental data",
        ),
        "tempControlMethod": (
            None,
            False,
            "Method used to control the temperature of the sample to eliminate heating caused by the long spin-lock pulse",
        ),
    },
    ...
    "writeMeasurements": {
        ...
        "useOriginalResNames": (
            False,
            False,
            "If set to True (on), original atom names (from the import)\n will be used for export.",
        ),
        "compressResonances": (
            True,
            False,
            "If set to False (off), the output atom names will not be compressed\n (e.g. HB2 and HB3 will be written out separately,\n even if they share the same value for a measurement).",
        ),
        "individualAtoms": (
            False,
            False,
            "If set to True (on), atom names will be fully written out (e.g. an Ala HB* is written in individual HB1, HB2, HB3 contributions)",
        ),
        "individualAtomsIfNoSet": (
            False,
            False,
            "If set to True (on), atom names for an atom set will be fully written out if no correct atomSysName found (e.g. an Ala HB* that has no matching name in the selected naming system is written in individual HB1, HB2, HB3 contributions)",
        ),
        "ignoreChemCompsWithNoSysNames": (
            False,
            False,
            "If set to True (on), residues that do not have the required atom naming system information will not be exported. Default (False) is to use atom names from CCPN instead.",
        ),
        "noWrite": (False, False, "If set to True (on), no file will be written."),
        "forceDefaultChainMapping": (
            False,
    ...
    "readPeaks": {
        ...
        "dataSource": (
            None,
            False,
            "A valid Nmr.DataSource object. The new peak list\n will be created using this object.",
        ),
        "dataDimRefs": (
            None,
            False,
            "A list of Nmr.DataDimRef objects. The peak list\n will be linked to these DataDimRefs.",
        ),
        "peakList": (
            None,
            False,
            "A valid Nmr.PeakList object. New information will be \nadded to this list (and/or old information replaced).\n Use with caution!",
        ),
    ...
    "writePeaks": {
        ...
        "useOriginalNumbers": (
            True,
            False,
            "If set to False (off), original format-specific peak numbers\n (from the import) will NOT be used for export.",
        ),
        "useOriginalData": (
            False,
            False,
            "If set to True (on), original format-specific data\n (from the import) will be used for export.",
        ),
        "useOriginalResNames": (
            False,
            False,
            "If set to True (on), original atom names\n (from the import) will be used for export.",
        ),
        "compressResonances": (
            True,
            False,
        ...
        "writeAssignments": (
            True,
            False,
            "If set to False (off), peak assignments are not written to the output file.",
        ),
        "skipMultiAssignments": (
            False,
            False,
            "If set to True (on), multiple peak assignments are not written to the output file. even for the format(s) taht support this.",
        ),
        "useXeasyDimCodes": (
            False,
            False,
            "If set to True (on), peakList dimCodes will be set in Xeasy format (NmrView only).",
        ),
        "noWrite": (False, False, "If set to True (on), no file will be written."),
        "forceDefaultChainMapping": (
            False,
        ...
        "forcePositiveVolumes": (
            False,
            False,
            "If set to True (on), all peaks will be exported with positive volumes/intensities.",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "readPeakAssignments": {
        ...
        "peakList": (
            None,
            False,
            "A valid Nmr.PeakList object. The assignment information\n will be added to this list (and/or old information replaced).\n Use with caution!",
        ),
        "addMode": (
            False,
            False,
            "If set to True (on), will add assignment possibilities to the peaks in the peak list.",
        ),
        "overwriteMode": (
            False,
            False,
            "If set to True (on), will overwrite assignment possibilities\n for the peaks in the peak list.",
        ),
        "useOriginalData": (
            False,
            False,
        ...
        "resonanceMatching": (
            True,
            False,
            "If set to False (off), the script will not try to match the newly\n created resonances to existing ones (in the Data Model).",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "writePeakAssignments": {
        ...
        "dataDimRefs": (
            None,
            False,
            "A list of Nmr.DataDimRef objects. The peak assignments\n will be exported using the order of these dataDimRefs.",
        ),
    ...
    "readConstraints": {
        ...
        "constraintList": (
            None,
            False,
            "A valid subclass of a ConstraintList (e.g. DistanceConstraintList, ...). New information will be added to this list (and/or old information replaced).",
        ),
        "constraintListName": (None, False, "A name for the new constraint list, if one is created."),
        "nmrConstraintStore": (
            None,
            False,
            "The new constraint list will automatically be part of this NmrConstraintStore object.",
        ),
        "strucGen": (
            None,
            False,
            "The new constraint list will automatically be linked to this StructureGeneration object.",
        ),
        "resonanceMatching": (
            True,
            False,
            "If set to False (off), the script will not try to match the newly created resonances to existing ones (in the Data Model).",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "readDihedralConstraints": {
        "newConstraintForDuplicate": (
            False,
            False,
            "Set to True (on) if you want to create a new constraint when, for a set of resonances that already has a dihedral restraint, a new angle range is found (instead of adding as additional item for the existing one).",
        ),
    },  # Specific for dihedral constraints
    ...
    "writeConstraints": {
        ...
        "exportAll": (
            False,
            False,
            "If set to True (on), conversion warnings are exported\n with the constraints (currently NMR-STAR only).",
        ),
        "sortForExport": (
            False,
            False,
            "If set to True (on), the constraints will be exported sorted by the chain code and sequence ID of the resonances in the first constraint item (or the constraint itself for dihedrals).",
        ),
        "useOriginalData": (
            False,
            False,
        ...
        "useOriginalResNames": (
            False,
            False,
            "If set to True (on), original atom names (from the import) will be used for export.",
        ),
        "compressResonances": (
            True,
            False,
        ...
        "retainOriginalCompression": (
            True,
            False,
            "If set to False (off), the original compression of atom names (based on whether resonances were copied or not) will not be retained.",
        ),
        "individualAtoms": (
            False,
            False,
    ...
    "writeRdcConstraints": {  # Specific for RDC constraints
        "negateValues": (
            False,
            False,
            "If set to True (on), the RDC values will be negated in the export. This is to avoid problems with programs that use different sign conventions.",
        ),
    },
    ...
    "readAcqPars": {
        ...
        "freqDataSource": (
            None,
            False,
            "An Nmr.DataSource object of type processed to link the acquisition parameters to.",
        ),
        "overwritePars": (
            False,
            False,
            "Determines whether existing parameters are overwritten. Set to 1 (on) for overwriting.",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "readProcPars": {
        ...
        "aliasing": (
            None,
            False,
            "Aliasing range for a dimension. Is a dictionary with as key the dimension number,\n as value a tuple (minValue,maxValue).",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "writeProcPars": {
        ...
        "dataDimRefs": (
            None,
            True,
            "A list of Nmr.DataDimRef objects. The processing parameter information\n will be referenced using these dataDimRefs.",
        ),
        "freqDataSource": (
            None,
            False,
            "An Nmr.DataSource object of type processed to write the processing parameters from.",
        ),
    ...
    "readPeopleAndCitations": {
        "useExistingPersons": (
            True,
            False,
            "If set to False (off), will not try to match new persons to existing ones.",
        ),
        "useExistingCitations": (
            True,
            False,
            "If set to False (off), will not try to match new citations to existing ones.",
        ),
        "doNotMakeCcpnObjects": (
            False,
            False,
            "If set to True (on), will only make the FormatConverter objects and not the CCPN ones",
        ),
        "minimalPrompts": (
            False,
            False,
    ...
    "readAssignment": {
        ...
        "chain": (
            None,
            True,
            "The chain the assignment refers to.",
        ),  # TODO this might be specific to MARS, but keeping it for now
        "minimalPrompts": (
            False,
            False,
            "If set to True (on), will use automatic settings as much as possible.\n This will reduce the number of popups that appear but is less user-oriented.",
...
class DataFormat(TopShared):
    ...
    def __init__(
        ...
        **keywds:        other keywords - these will be passed on to self.readProject()
        ...
        # Track newly created resonances. Note that this is a custom variable that needs to be reset
        # by external code (or not) depending on the purpose.
        self.newResonances = []
    ...
    def setFormatIOs(self):
        ...
        for componentInfo in IoSetupList + [
            ("general", None),
            ("chemShifts", None),
            ("conversionPars", None),
            ("hetNoe", None),
  ...
  """
    ...
    def readProject(self, fileName=None, checkFileExists=True, **keywds):
        ...
        try:
            ...
            self.messageReporter.showError(
                "Error", "Error reading %s project file: %s" % (self.format, str(e)), parent=self.guiParent
            )
            return None
    ...
    def readSequence(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error", "Error reading %s sequence file: %s" % (self.format, str(e)), parent=self.guiParent
                )
                return None
    ...
    def writeSequence(self, fileName=None, verbose=True, **keywds):
        ...
        if not isinstance(self.chains, (list, tuple)):
            self.messageReporter.showError(
                "Error", "Need valid list of chain(s) to write sequence.", parent=self.guiParent
            )
            return
            ...
            for chain in self.chains:
                if not isinstance(chain, MolSystem.Chain):
                    self.messageReporter.showError(
                        "Error",
                        "Need valid chain object(s) to write sequence. Removed invalid %s" % chain.__class__,
    ...
    def readCoordinates(self, fileNames=None, verbose=True, **keywds):
        ...
        self.modelSet = None
        self.coordResidues = {}
        self.specialSeqCodeMapping = (
            False  # This is set to True for NMR-STAR imports where the sequence codes are reset - for NRG project
        )
        ...
        for self.fileName in self.fileNames:
            if self.fileName:
                ...
                try:
                    ...
                    self.messageReporter.showError(
                        "Error", "Error reading %s coordinates file: %s" % (self.format, str(e)), parent=self.guiParent
                    )
                    return None

            elif not self.file:
                self.messageReporter.showError(
                    "Error", "Need a valid filename to read coordinates.", parent=self.guiParent
                )
                return None
            ...
            if not self.structureNumber:
                ...
                if not self.forceNamingSystemName or self.format in ("pseudoPdb",):
        ...
        self.specialSeqCodeMapping = False  # Reset, just to be sure.
    ...
    def writeCoordinates(self, fileName=None, verbose=True, **keywds):
        ...
        if self.structures:
            if not isinstance(self.structures, (list, tuple)):
                self.messageReporter.showError(
                    "Error", "Need valid list of structures to write coordinates.", parent=self.guiParent
                )
                return
                ...
                for structure in self.structures:
                    if not isinstance(structure, MolStructure.Model):
                        self.messageReporter.showError(
                            "Error",
                            "Need valid structure object(s) to write sequence. Removed invalid %s"
                            % structure.__class__,
                            parent=self.guiParent,
                        )
    ...
    def readChemComps(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error",
                    "Error reading %s chemical compound file: %s" % (self.format, str(e)),
                    parent=self.guiParent,
                )
                return None
    ...
    def writeChemComp(self, fileName=None, verbose=True, **keywds):
        ...
        if self.chemCompVar:
            ...
            self.messageReporter.showError(
                "Error", "Need valid chemCompVar to write chemComp file.", parent=self.guiParent
            )
            return
    ...
    def setMeasurementCodes(self, readWrite, keywds, verbose=True):
        ...
        if self.measurementType == "shift":
            ...
            self.formatFileClassName = (
                "ChemShift"  # This is the name of the File class in the raw format measurement file
            )
            ...
            self.messageReporter.showError(
                "Unrecognized type", "Unrecognized measurement type %s." % str(self.measurementType)
            )
            return False
    ...
    def readMeasurements(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            self.messageReporter.showError(
                "Error", "Need a valid filename to read %ss." % self.measurementType, parent=self.guiParent
            )
            return
        ...
        if not self.measurementFile or len(self.measurementFileValues) == 0:
            self.messageReporter.showError(
                "No %ss" % self.measurementType,
                "No %s values found in file %s: not used" % (self.measurementType, fileName),
                self.guiParent,
            )
            return None
        ...
        if self.measurementList:
            if not isinstance(self.measurementList, self.ApiMeasurementList):
                self.messageReporter.showWarning(
                    "Invalid %ss" % self.measurementType,
                    "Invalid %s list passed in (%s). Creating new one."
                    % (self.measurementType, self.measurementList.__class__),
                    self.guiParent,
                )
                self.measurementList = None

        if self.experiments:
            if not isinstance(self.experiments, (list, tuple)):
                self.messageReporter.showWarning(
                    "Invalid experiment list", "No list of experiments passed in: are ignored.", self.guiParent
                )
                ...
                for experiment in self.experiments:
                    if not isinstance(experiment, Nmr.Experiment):
                        self.messageReporter.showWarning(
                            "Invalid experiment",
                            "Invalid experiment object passed in (%s). Ignored." % experiment.__class__,
                            self.guiParent,
                        )
        ...
        if self.method:
            if not isinstance(self.method, (list, tuple)):
                self.messageReporter.showWarning(
                    "Invalid method list", "No list of method passed in: are ignored.", self.guiParent
                )
                self.method = None
                ...
                for method in self.method:
                    if not isinstance(method, Method.Method):
                        self.messageReporter.showWarning(
                            "Invalid method",
                            "Invalid method object passed in (%s). Ignored." % method.__class__,
                            self.guiParent,
                        )
    ...
    def writeMeasurements(self, fileName=None, verbose=True, **keywds):
        ...
        if not self.measurementList or not isinstance(
            self.measurementList, getattr(Nmr, "%sList" % self.measurementApiCode)
        ):
            self.messageReporter.showError(
                "Error",
                "No or invalid %s list provided for writing %s: file not written." % (self.measurementType, fileName),
                parent=self.guiParent,
            )
            return
        ...
        if hasattr(self, "chains") and self.chains:
            if not isinstance(self.chains, (list, tuple)):
                self.messageReporter.showWarning(
                    "Invalid chain list", "No list of chains passed in: are ignored.", self.guiParent
                )
                self.chains = None
                ...
                for chain in self.chains:
                    if not isinstance(chain, MolSystem.Chain):
                        self.messageReporter.showWarning(
                            "Invalid chain",
                            "Invalid chain object passed in (%s). Ignored." % chain.__class__,
                            self.guiParent,
                        )
                        self.chains.pop(self.chains.index(chain))
    ...
    def readPeaks(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error", "Error reading %s peak list file: %s" % (self.format, str(e)), parent=self.guiParent
                )
                return None
        ...
        hasSoftwareDefinitions = False

        if self.project.currentMethodStore:
            if self.project.currentMethodStore.findFirstSoftware(name=self.format):
                hasSoftwareDefinitions = True
        ...
        if self.dataSource and not isinstance(self.dataSource, Nmr.DataSource):
            self.messageReporter.showError(
                "Error", "Need valid dataSource object to read peaks.", parent=self.guiParent
            )
            return

        if not isinstance(self.dataDimRefs, (list, tuple)):
            self.messageReporter.showError(
                "Error", "Need valid list of dataDimRefs to read peaks.", parent=self.guiParent
            )
            return
            ...
            for dataDimRef in self.dataDimRefs:
                if not isinstance(dataDimRef, Nmr.DataDimRef):
                    self.messageReporter.showError(
                        "Error", "Need valid dataDimRef object(s) to read peaks.", parent=self.guiParent
                    )
                    return
            ...
            if self.checkPeakListValid():
                ...
                if not self.peakList:
                    ...
                    if not self.minimalPrompts:
                        peakListName = self.dataEntry.askString(
                            "Enter peak list name", "New peak list name", self.specName, self.guiParent
                        )
    ...
    def writePeaks(self, fileName=None, verbose=True, **keywds):
        ...
        if not isinstance(self.peakLists, (list, tuple)):
            self.messageReporter.showError(
                "Error",
                "Invalid list of peaks provided for writing %s: file not written." % fileName,
                parent=self.guiParent,
            )
            return
            ...
            for peakList in self.peakLists:
                if not peakList or not isinstance(peakList, Nmr.PeakList):
                    self.messageReporter.showError(
                        "Error", "Invalid peak list provided for writing %s: deleted." % fileName, parent=self.guiParent
                    )
                    self.peakLists.pop(self.peakLists.index(peakList))

            if len(self.peakLists) == 0:
                self.messageReporter.showError(
                    "Error", "No valid peak lists given - aborting.", parent=self.guiParent
                )
                return
        ...
        if self.dataDimRefs and len(self.peakLists) == 1:
            if not isinstance(self.dataDimRefs, (list, tuple)):
                self.messageReporter.showWarning(
                    "Invalid dataDimRef list", "No list of dataDimRefs passed in: are ignored.", self.guiParent
                )
                ...
                for dataDimRef in self.dataDimRefs:
                    if not isinstance(dataDimRef, Nmr.DataDimRef) or dataDimRef.dataDim.dataSource != self.dataSource:
                        self.messageReporter.showWarning(
                            "Invalid dataDimRef",
                            "Invalid dataDimRef object passed in (%s). All ignored." % dataDimRef.__class__,
                            self.guiParent,
                        )
                        self.dataDimRefs = None
                        break
        ...
        for self.peakList in self.peakLists:
            ...
            if not self.dataDimRefs:  # or (len(self.peakLists) > 1 and self.minimalPrompts):
                ...
                if self.guiParent:
                    ...
                    self.messageReporter.showError(
                        "No peakDim mapping",
                        "No dataDimRefs passed in: is necessary for writePeaks in non-GUI environment. Aborting.",
                        self.guiParent,
                    )
                    return
    ...
    def readPeakAssignments(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error", "Error reading %s peak assignments file: %s" % (self.format, str(e)), parent=self.guiParent
                )
                return None

        elif not self.file:
            self.messageReporter.showError(
                "Error", "Need a valid filename to read peak assignments.", parent=self.guiParent
            )
            return
    ...
    def writePeakAssignments(self, fileName=None, verbose=True, **keywds):
        ...
        if not self.peakList or not isinstance(self.peakList, Nmr.PeakList):
            self.messageReporter.showError(
                "Error",
                "No or invalid peak list provided for writing %s: file not written." % fileName,
                parent=self.guiParent,
            )
            return
        ...
        if not self.dataDimRefs or not self.minimalPrompts:
            if self.guiParent:
                ...
                self.messageReporter.showError(
                    "No peakDim mapping",
                    "No dataDimRefs passed in: is necessary for writePeakAssignments in non-GUI environment. Aborting.",
                    self.guiParent,
                )
                return
    ...
    def setConstraintCodes(self, readWrite, keywds, verbose=True):
        ...
        if self.constraintType == "distance":
            ...
            self.messageReporter.showError(
                "Unrecognized type", "Unrecognized constraint type %s." % str(self.constraintType)
            )
            return False
    ...
    def readConstraints(self, fileName=None, verbose=True, **keywds):
        ...
        if not self.constraintList:
            ...
            if not self.constraintListName:
                ...
                if not self.minimalPrompts:
                    newConstraintListName = self.dataEntry.askString(
                        "Enter name for constraint list",
                        "New %s constraint list name" % self.constraintType,
                        self.constraintListName,
                        self.guiParent,
                    )
            ...
            self.constraintList = self.ApiConstraintList(
                self.nmrConstraintStore, name=self.constraintListName, **constraintListPars
            )
            ...
            if isinstance(self.constraintList, self.ApiConstraintList):
                ...
                self.messageReporter.showError(
                    "Not valid",
                    "Constraint list of type %s (should be %s)"
                    % (self.constraintList.className, self.constraintApiCode),
                )
                return False
        ...
        if fileName:
            ...
            self.messageReporter.showError(
                "Error", "Need a valid filename to read constraint lists.", parent=self.guiParent
            )
            return False
    ...
    def writeConstraints(self, fileName=None, verbose=True, **keywds):
        ...
        if not self.constraintList or self.constraintList.className != self.constraintApiCode + "ConstraintList":
            self.messageReporter.showError(
                "Error",
                "No or invalid %s constraint list provided for writing %s: file not written."
                % (self.constraintType, fileName),
                parent=self.guiParent,
            )
            return
    ...
    def readAcqPars(self, fileName=None, verbose=True, **keywds):
        ...
        if self.fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error", "Error reading %s acquisition file: %s" % (self.format, str(e)), parent=self.guiParent
                )
                return None

        elif not self.file:
            self.messageReporter.showError(
                "Error", "Need a valid filename to read acquisition parameters.", parent=self.guiParent
            )
            return
        ...
        self.setSpectrometer(
            manufacturer=self.formatLabel,
    ...
    def readProcPars(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error",
                    "Error reading %s processing parameters file: %s" % (self.format, str(e)),
                    parent=self.guiParent,
                )
                return None
            ...
            self.messageReporter.showError(
                "Error", "Need a valid filename to read processing parameters.", parent=self.guiParent
            )
            return
        ...
        if not self.experiment:
            if not self.expName and (
                len(self.project.currentNmrProject.experiments) == 1
                and self.numDim == self.project.currentNmrProject.sortedExperiments()[0].numDim
                and self.minimalPrompts
            ):
                self.experiment = self.project.currentNmrProject.sortedExperiments()[0]
                ...
                ll = [x for x in self.project.currentNmrProject.sortedExperiments() if x.numDim == self.numDim]
                (selectionList, selectionDict) = createSelection(ll)
            ...
            if not self.experiment:
                while not self.experiment:
                    ...
                    popup = self.multiDialog.ExperimentCreate(
                        self.guiParent,
                        numDim=self.numDim,
    ...
    def readPeopleAndCitations(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error", "Error reading %s citation from file: %s" % (self.format, str(e)), parent=self.guiParent
                )
                return None
    ...
    def readAssignment(self, fileName=None, verbose=True, **keywds):
        ...
        if fileName:
            ...
            try:
                ...
                self.messageReporter.showError(
                    "Error", "Error reading %s assignment file: %s" % (self.format, str(e)), parent=self.guiParent
                )
                return None
    ...
    def readShiftsAndPeaks(self, verbose=True, **keywds):
        ...
        measurementList = self.readShifts(
            self.shiftFile, verbose=False, minimalPrompts=self.minimalPrompts, resetIOkeywords=False
        )

        if not measurementList:
            return False

        peakLists = self.readPeaks(
            self.peakFile, verbose=False, minimalPrompts=self.minimalPrompts, resetIOkeywords=False
        )
        ...
        for peakList in peakLists:
            if peakList.dataSource.experiment not in measurementList.experiments:
        ...
        if hasattr(self, "peakAssignmentFile") and self.peakAssignmentFile:
            peakList = self.readPeakAssignments(
                self.peakAssignmentFile,
                verbose=False,
    ...
    def setIOkeywords(self, functionName, keywds, lockUpdate=False, verbose=True):
        ...
        if functionName in self.IOkeywords:
            ...
            for IOkeyword in self.IOkeywords[functionName].keys():
                ...
                if mandatory and IOkeyword not in keywds:
                    if doReset:
                        print(
                            "  Error: mandatory keyword '%s' for '%s' missing! Cannot continue."
                            % (IOkeyword, functionName)
                        )
                        returnStatus = False
                        break
                        ...
                        print(
                            "  Error: mandatory keyword '%s' for '%s' was not set! Cannot continue."
                            % (IOkeyword, functionName)
                        )
                        returnStatus = False
                        break
        ...
        # Rasmus Fogh 31/7/2012 :
        if hasattr(self, "presetResonanceMapping") and hasattr(self, "compressResonances"):
            if self.presetResonanceMapping and self.compressResonances:
                raise self.FormatConverterError("presetResonanceMapping and compressResonances are not compatible")
    ...
    def initConvertCount(self, dataType):
        ...
        if dataType == "sequence":
            ...
            if self.mainCode not in self.convertCount:
                if self.molSystemCreated:
                    startCount = 1
                else:
                    startCount = 0

                self.convertCount[self.mainCode] = [
                    startCount,
                    {"molecule(s)": 0, "molecule residue(s)": 0, "chain(s)": 0, "chain residue(s)": 0},
                ]
    ...
    def linkResonances(self, **kw):
        ...
        if "allowPopups" not in kw:
            kw["allowPopups"] = self.allowPopups

        if "assignFormat" not in kw:
    ...
    def checkUnlinkedResonances(self, nmrConstraintStore=None, exportMode="atoms", doPopup=True):
        ...
        if numUnlinkedResonances != 0:
            ...
            if numUnlinkedResonances / len(resonances) > 0.05:
                if doPopup:
                    runLinkResonances = self.messageReporter.showYesNo(
                        "Start linkResonances",
                        "There are %d resonances with no connection to atoms - do you want to link them first?"
                        % numUnlinkedResonances,
                    )
  ...
  """
    ...
    def setSequenceFileClass(self):
        try:
            ...
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning",
                    " No sequence IO available in format %s... aborting import/export." % (self.formatLabel),
                    self.guiParent,
                )
            self.quit()
    ...
    def getSequence(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readSequence available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def createSequenceFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writeSequence available for %s..." % self.formatLabel, self.guiParent
        )
        self.sequenceFile = None
        self.quit()
    ...
    def getFullProject(self, fileName=None):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readProject available for %s..." % self.formatLabel, self.guiParent
        )
        self.quit()
    ...
    def createFullProject(self, fileName=None):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writeProject available for %s..." % self.formatLabel, self.guiParent
        )
        self.quit()
    ...
    def setCoordinateFileClass(self):
        try:
            ...
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning",
                    " No coordinate IO available in format %s... aborting import/export." % (self.formatLabel),
                    self.guiParent,
                )
            self.quit()

    def getCoordinates(self, noSequenceRead=False):
        """
        ...
        self.messageReporter.showWarning(
            "Warning", " No readCoordinates available for %s..." % self.formatLabel, self.guiParent
        )
        self.coordinateFile = None
        self.quit()

    def getCoordinatesGeneric(self, noSequenceRead=False):
        """
        ...
        if not noSequenceRead and (not chains or self.forceReadSequence):
            #
            ...
            #
            # print "SEQUENCE READ", readSeqKeywds
    ...
    def createCoordinateFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writeCoordinates available for %s..." % self.formatLabel, self.guiParent
        )
        self.coordinateFile = None
        self.quit()
    ...
    def setChemCompFileClass(self):
        try:
            ...
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning",
                    " No chemComp IO available in format %s... aborting import/export." % (self.formatLabel),
                    self.guiParent,
                )
            self.quit()
    ...
    def getChemComps(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readChemComps available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def createChemCompFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writeChemComps available for %s..." % self.formatLabel, self.guiParent
        )
        self.chemCompFile = None
        self.quit()
    ...
    def setMeasurementFileClass(self):
        try:
            ...
            self.MeasurementFileClass = getattr(
                self.measurementsIO, "%s%sFile" % (self.formatLabel, self.formatFileClassName)
            )
        except:
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning",
                    " No IO available for %s in format %s... aborting import/export."
                    % (self.formatFileValueKey, self.formatLabel),
                    self.guiParent,
                )
            self.quit()
    ...
    def getMeasurements(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No read%ss available for %s..." % (self.measurementApiCode, self.formatLabel), self.guiParent
        )
    ...
    def getMeasurementsGeneric(self):
        ...
        try:
            ...
            readKeywds = self.getMeasurementsSetFormatSpecificReadKeywds()
            self.measurementFile.read(**readKeywds)

            sequenceText = self.getMeasurementsSequenceRead()

            if self.verbose == 1:
                print(
                    "Reading %ss%s from %s file %s"
                    % (self.measurementType, sequenceText, self.formatLabel, self.fileName)
                )
    ...
    def getMeasurementsSetFormatSpecificReadKeywds(self):
    ...
    def createMeasurementFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No write%ss available for %s..." % (self.measurementApiCode, self.formatLabel), self.guiParent
        )
        self.measurementFile = None
        self.quit()
    ...
    def createMeasurementFileGeneric(self):
        ...
        try:
            ...
            self.messageReporter.showWarning(
                "Warning",
                " No write%s available for %s..." % (self.measurementApiCode, self.formatLabel),
                self.guiParent,
            )
    ...
    def setPeakFileClass(self):
        ...
        try:
            ...
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning",
                    " No peaks IO available in format %s... aborting import/export." % (self.formatLabel),
                    self.guiParent,
                )
            self.quit()
    ...
    def getPeaks(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readPeaks available for %s..." % self.formatLabel, self.guiParent
        )
        self.peakFile = None
        self.quit()
    ...
    def createPeakFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writePeaks available for %s..." % self.formatLabel, self.guiParent
        )
        self.peakFile = None
        self.quit()
    ...
    def getPeakAssignments(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readPeakAssignments available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def createPeakAssignmentFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writePeakAssignments available for %s..." % self.formatLabel, self.guiParent
        )
        self.peakAssignmentsFile = None
        self.quit()
    ...
    def setConstraintFileClass(self):
        ...
        try:
            ...
            self.ConstraintFileClass = getattr(
                self.constraintsIO, "%s%sConstraintFile" % (self.formatLabel, self.constraintFileCode)
            )
        except:
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning",
                    " No IO available for %s constraints in format %s... aborting import/export."
                    % (self.constraintType, self.formatLabel),
                    self.guiParent,
                )
            self.quit()
    ...
    def getConstraints(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readConstraints available for %s..." % self.formatLabel, self.guiParent
        )
        ...
        try:
            ...
            if self.verbose:
                self.messageReporter.showWarning(
                    ...
                    % (self.constraintApiCode, self.formatLabel, errorMessage),
                    self.guiParent,
                )
    ...
    def createConstraintFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writeConstraints available for %s..." % self.formatLabel, self.guiParent
        )
        self.constraintFile = None
        self.quit()
    ...
    def createConstraintFileGeneric(self):
        ...
        try:
            ...
            self.messageReporter.showWarning(
                "Warning",
                " No write%sConstraints available for %s...:\n%s"
                % (self.constraintApiCode, self.formatLabel, errorMessage),
                self.guiParent,
            )
            self.quit()
    ...
    def getAcqPars(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readAcqPars available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def getProcPars(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readProcPars available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def createProcParsFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writeProcPars available for %s..." % self.formatLabel, self.guiParent
        )
        self.procParsFile = None
        self.quit()
    ...
    def createProcScriptFile(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No writeProcScript available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def getPeopleAndCitations(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readPeopleAndCitations available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def setAssignmentFileClass(self):
        try:
            ...
            if not self.minimalPrompts:
                self.messageReporter.showWarning(
                    "Warning",
                    " No assignment IO available in format %s... aborting import/export." % (self.formatLabel),
                    self.guiParent,
                )
            self.quit()
    ...
    def getAssignment(self):
        ...
        self.messageReporter.showWarning(
            "Warning", " No readAssignment available for %s..." % self.formatLabel, self.guiParent
        )
    ...
    def setSingleResonanceMeasurementsGeneric(self):
        ...
        for self.rawMeasurement in self.measurementFileValues:
            ...
            if resName in self.resonanceNames:
                #
            ...
            if makeNewResonance == 1:
                ...
                if self.rawMeasurement.seqCode == None and hasattr(self.rawMeasurement, "spinSystemId"):
                    ...
                    if spinSystemId not in spinSystems:
                    ...
                    if self.measurement:
                        applData = self.measurement.findFirstApplicationData(
                            application=self.format, keyword=assign_kw, value=resName
                        )

                        if not applData:
                            self.measurement.addApplicationData(
                                Implementation.AppDataString(application=self.format, keyword=assign_kw, value=resName)
                            )
        ...
        for self.rawMeasurement in self.measurementFileValues:
            ...
            for resIndex in range(1, self.measurementResNum + 1):
                ...
                if resName in self.resonanceNames:
                    ...
                    if getattr(self.rawMeasurement, "seqCode%d" % resIndex) == None and hasattr(
                        self.rawMeasurement, "spinSystemId%d" % resIndex
                    ):
            ...
            if len(resNames) == self.measurementResNum:
                ...
                for resonanceComb in resonanceCombs:
                    ...
                    # Avoid having same resonance in here - meaningless. Occurs due to problems in input files
                    elif resonanceComb.count(resonanceComb[0]) != self.measurementResNum:
                        ...
                        applData = self.measurement.findFirstApplicationData(
                            application=self.format, keyword=assign_kw, value=str(resNames)
                        )
                        ...
                        print("  Warning: measurement linked to same resonance with name %s - ignored." % resNames[0])
        #
    ...
    def createSpinSystems(self, spinSystems, addSpinSystemInfo):
        ...
        for spinSystemId in spinSystemIds:
            ...
            resGroup = Nmr.ResonanceGroup(
                self.project.currentNmrProject,
                name=str(spinSystemId),
                ccpCode=ccpCode,
                resonances=spinSystems[spinSystemId][-1],
                details="Set by DataFormat from %s format file" % self.format,
            )
            ...
            if "prevSpinSystemId" in addSpinSystemInfo:
                ...
                if prevSpinSystemId != None:
                    ...
                    resGroupProb = Nmr.ResonanceGroupProb(
                        resGroup, linkType="sequential", sequenceOffset=-1, possibility=prevResGroup
                    )
    ...
    def checkMolecules(self):
        ...
        for self.sequence in self.sequenceFile.sequences:
            ...
            if not self.sequence.elements and not self.minimalPrompts:
                self.messageReporter.showError(
                    "Error", "No sequence elements for %s - ignoring." % (self.sequence.molName), parent=self.guiParent
                )
                self.addWarning("Molecule %s not created because it has no sequence elements." % self.sequence.molName)
                continue

            #
            # Check if water, treat differently...
            #

            isWater = False
            if len(self.sequence.elements) == 1:
                seqEl = self.sequence.elements[0]
                if hasattr(seqEl, "code3Letter") and seqEl.code3Letter in ("Hoh", "HOH"):
                    isWater = True
            ...
            for origMolName in origSequenceDict.keys():
                ...
                if isWater:
                    otherSeqEl = otherSequence.elements[0]
                    if hasattr(otherSeqEl, "code3Letter") and otherSeqEl.code3Letter in ("Hoh", "HOH"):
                        if otherSequence.originalChainCode == self.sequence.originalChainCode:
                            sameMolecule = True

                elif len(self.sequence.elements) == len(otherSequence.elements):
                    sameMolecule = True
                    for seqIndex in range(len(self.sequence.elements)):
            ...
            if sameMolecule:
                if not isWater:
                    ...
                    if origMolName in createMoleculeDict:
                        createMoleculeDict[origMolName][1].append((self.sequence, []))
                        for seqIndex in range(len(self.sequence.elements)):
                            (otherSeqElement, chemComp, chemCompVar) = createMoleculeDict[origMolName][1][0][1][
                                seqIndex
                            ]
                            createMoleculeDict[origMolName][1][-1][1].append(
                                (self.sequence.elements[seqIndex], chemComp, chemCompVar)
                            )

                    continue

                else:
                    if self.sequence.originalChainCode == origSequenceDict[origMolName][0][0].originalChainCode:
                        """
            (otherSeqElement,chemComp,chemCompVar) = createMoleculeDict[origMolName][1][0][1][0]
            createMoleculeDict[origMolName][1][-1][0].elements.append(self.sequence.elements[0])
            createMoleculeDict[origMolName][1][-1][1].append(self.sequence.elements[0],chemComp,chemCompVar)
            """
            ...
            while origMolName in origSequenceDict:
            ...
            # Modify for water
            if isWater:
                origMolName = "%s_%s" % (origMolName, self.sequence.originalChainCode)
            ...
            if not origSequenceDict[origMolName][1]:
                ...
                if hasattr(self.sequence, "polymerType") and self.sequence.polymerType != "unknown":
                    print(
                        "  Molecule %s set to type %s from sequence file information."
                        % (origMolName, self.sequence.polymerType)
                    )
                            ...
                            if hasattr(sequenceElement, "code1Letter"):
                                ...
                                for bioPolymerType in bioPolymerCodes.keys():
                                    ...
                                    # Then on 1 letter codes, EXCEPT for proteins - this should not happen
                                        ...
                                        code3Letter in bioPolymerCodes[bioPolymerType][0]
                                        and bioPolymerType != "protein"
                                    ):
                                        unknownSequenceElements[-1][-1].append(bioPolymerType)
                                    ...
                                    # Finally a dictionary check...
                                        ...
                                        for tempCcpCode in chemCompStdDict[bioPolymerType].keys():
                                            if (
                                                code3Letter == tempCcpCode
                                                or code3Letter == chemCompStdDict[bioPolymerType][tempCcpCode][1]
                                            ):
                                                unknownSequenceElements[-1][-1].append(bioPolymerType)
                    ...
                    if unknownSequenceElements:
                        ...
                        for bioPolymerType in nucleotideList:
                            ...
                            if fraction == 1:
                                ...
                                molType = bioPolymerType
                                print(
                                    "  Partial match for unknown residues with molecule type %s (%.1f %%)"
                                    % (molType, fraction * 100)
                                )
                                break
                                ...
                                if hasattr(sequenceElement, "code3Letter") and sequenceElement.code3Letter:
                                    ...
                                    for tmpMolType in bioPolymerCodes.keys() + ["carbohydrate"]:
                                        file = joinPath(
                                            "molecule",
                                            "ChemComp",
                                            "%s.%s.xml" % (tmpMolType, sequenceElement.code3Letter.capitalize()),
                                        )
                                    ...
                                    if not chemCompFound:
                                        for tmpMolType in bioPolymerCodes.keys() + ["carbohydrate"]:
                                            for tempCcpCode in chemCompStdDict[tmpMolType].keys():
                                                if (
                                                    sequenceElement.code3Letter == tempCcpCode
                                                    or sequenceElement.code3Letter
                                                    == chemCompStdDict[tmpMolType][tempCcpCode][1]
                                                ):
                                                    origSequenceDict[origMolName][1][sequenceElementIndex][-1] = (
                                                        tmpMolType
                                                    )
                                                    chemCompFound = True
                                                    molType = tmpMolType
                                    ...
                                    if not chemCompFound:
                                        ...
                                        file = joinPath(
                                            "molecule",
                                            "ChemComp",
                                            "other.%s.xml" % sequenceElement.code3Letter.capitalize(),
                                        )
                                        for repository in repositoryList:
                                            if os.path.exists(joinPath(repository.url.path, file)):
                                                origSequenceDict[origMolName][1][sequenceElementIndex][-1] = tmpMolType
                                        ...
                                        if not chemCompFound:
                                            ...
                                            for tempCcpCode in chemCompOverview[tmpMolType].keys():
                                                if (
                                                    sequenceElement.code3Letter == tempCcpCode
                                                    or sequenceElement.code3Letter
                                                    == chemCompOverview[tmpMolType][tempCcpCode][1]
                                                ):
                                                    origSequenceDict[origMolName][1][sequenceElementIndex][-1] = (
                                                        tmpMolType
                                                    )
        ...
        for origMolName in origMolNames:
            ...
            for seqElIndex in range(seqLength):
                            ...
                            if bondType in sequenceElement.bonds:
                                ...
                                for atomName in atomNames:
                                    for bondedSeqEl, bondedAtomName in sequenceElement.bonds[bondType][atomName]:
                                        ...
                                        if sequenceElement not in bondDict:
                                            bondDict[sequenceElement] = {}

                                        if atomName not in bondDict[sequenceElement]:
                ...
                if self.chemCompMappings:
                    ...
                    if code in self.chemCompMappings:
                ...
                if not chemComp:
                    if residueType in ["protein"] + nucleotideList:
                        ...
                        if not chemComp and hasattr(sequenceElement, "code1Letter") and sequenceElement.code1Letter:
                            ...
                            if residueType in code1LetterToCcpCodeDict and code1Letter in code1LetterToCcpCodeDict[residueType]:
                ...
                #
                # If required, auto create new chemComps. These will have minimal information, only atom names, no bonds, but
                # should be sufficient to link resonance and/or coordinate information.
                #

                if (
                    not chemComp
                    and self.autoCreateChemComps
                    and hasattr(sequenceElement, "atomNames")
                    and sequenceElement.atomNames
                ):
                    if not residueType:
                        if len(sequence.elements) == 1:
                            ...
                            seqElIndex != 0
                            and sequence.elements[seqElIndex - 1].seqCode != sequence.elements[seqElIndex].seqCode - 1
                        ):
                            ccMolType = "other"
                        else:
                            ccMolType = molType
                    else:
                        ccMolType = residueType

                    if hasattr(sequenceElement, "code3Letter"):
                        code = sequenceElement.code3Letter.capitalize()

                    elif hasattr(sequenceElement, "code1Letter"):
                        code = sequenceElement.code1Letter

                    self.rawChemComp = GenericChemComp(code, ccMolType, sequenceElement.atomNames)

                    for i in range(len(sequenceElement.atomNames)):
                        atomName = sequenceElement.atomNames[i]
                        atomSerial = i + 1

                        self.rawChemComp.atoms.append(GenericAtom(atomSerial, atomName))

                    self.rawChemCompList = [self.rawChemComp]
                    ...
                    convertCount = self.convertCount
                    mainCode = self.mainCode

                    # Initialise fake stuff
                    self.initConvertCount("chemComp")
                    self.allowPopups = self.createAtomSets = False
                    self.saveChemComp = True
                    self.molTypes = self.ccpCodes = self.chemComps = []
                    self.makeNamingSystem = "CUSTOM"
                    ...
                    self.convertCount = convertCount
                ...
                #
                # If nothing found or created, then ask user which chemComp
                #

                if not chemComp and not self.ignoreUnknownChemComps and self.allowPopups:
                    selectNew = True
                    if not self.minimalPrompts:
                        selectNew = self.messageReporter.showYesNo(
                            "Select a chemComp",
                            "Unrecognized %s chemComp %s code %s for position %d in %s. Do you want to select a chemComp for this residue?"
                            % (residueType, codeText, code, seqCode, self.fileName),
                            parent=self.guiParent,
                        )

                    if selectNew:
                        interaction = self.multiDialog.ChemCompSelection(
                            self.guiParent, self.project, chemCompEntries=["ChemComp"], origCode=code
                        )
                ...
                if not chemComp:
                    ...
                    if not chemComp:
                        print(
                            "  Warning: no %s chemComp %s code %s selected. Will be ignored."
                            % (residueTypeText, codeText, code)
                        )
                        self.addWarning(
                            "No CCPN reference compound could be found for molecular type %s, compound %s with code %s."
                            % (residueType, codeText, code)
                        )
                        continue

                    else:
                        print(
                            "  Warning: No %s chemComp %s code %s selected, replaced by Xxx chemComp to maintain chain sequence numbering."
                            % (residueTypeText, codeText, code)
                        )
    ...
    print("""
        ...
        for origMolName in origMolNames:
            ...
            for sequenceIndex in range(0, len(chemCompDict[origMolName])):
                ...
                if sequenceIndex == 0:
                    ...
                    while molName in createMoleculeDict:
                        addIndex += 1
                        molName = "%s_%d" % (origMolName, addIndex)
                    ...
                    if chemComp:
                        ...
                        if not namingSystem:
                            for backupNamingSystemName in ("IUPAC", "MSD", "CUSTOM"):
                                ...
                                if namingSystem:
                                    print(
                                        "  Warning: selected %s naming system for chemComp %s,%s because %s naming system does not exist."
                                        % (
                                            backupNamingSystemName,
                                            chemComp.molType,
                                            chemComp.ccpCode,
                                            self.namingSystemName,
                                        )
                                    )
                                    break
                        ...
                        if not namingSystem:
                            print(
                                "  Warning: no %s naming system or backup naming systems found for chemComp %s,%s... aborting linking/descriptor selection."
                                % (self.namingSystemName, chemComp.molType, chemComp.ccpCode)
                            )
                            continue
                        ...
                        for bondDict in (moleculeBonds, molSystemBonds):
                            if seqElement in bondDict:
                                ...
                                for molTypes, removeCcpnAtomNames in (
                                    (("protein",), ("C", "N")),
                                    (("DNA", "RNA"), ("P", "O3'")),
                                ):
                        ...
                        if linkedAtomNames:
                            ...
                            if chemComp.molType in ("protein", "DNA", "RNA"):
                                ...
                                linking = "link:%s" % ",".join(linkedAtomNames)
                        ...
                        if hasattr(seqElement, "formatCode") and seqElement.formatCode:
                            ...
                            for chemCompSysName in chemCompSysNames:
                                chemCompVar = chemCompSysName.findFirstChemCompVar(
                                    linking=linking, isDefaultVar=True
                                ) or chemCompSysName.findFirstChemCompVar(linking=linking)
                        ...
                        if (
                            not chemCompVar
                            and hasattr(seqElement, "atomNames")
                            and seqElement.atomNames
                            and namingSystem
                        ):
                            chemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                                chemComp, chemComp.chemAtoms, namingSystem.name
                            )
                            chemCompVars = list(chemComp.findAllChemCompVars(linking=linking))

                            if not chemCompVars and origLinking != linking:
                                print(
                                    "  Warning: could not find a chemCompVar for (%s,%s), linking '%s'. Switching to linking '%s'."
                                    % (chemComp.molType, chemComp.ccpCode, linking, origLinking)
                                )
                                linking = origLinking
                                chemCompVars = list(chemComp.findAllChemCompVars(linking=linking))
                            ...
                            chemCompVars = getDescriptorElementMatches(
                                chemCompVars, descriptorElements, verbose=self.verbose
                            )
                            ...
                            if self.addNameMappings and chemComp.ccpCode in self.addNameMappings:
                            ...
                            for sysAtomName in seqElement.atomNames:
                                ...
                                for chemAtomSysName in chemAtomSysNames:
                                    if (
                                        sysAtomName == chemAtomSysName.sysName
                                        or sysAtomName in chemAtomSysName.altSysNames
                                    ):
                                        atomNames.append(chemAtomSysName.atomName)
                                    elif (
                                        mappedSysAtomName
                                        and mappedSysAtomName == chemAtomSysName.sysName
                                        or mappedSysAtomName in chemAtomSysName.altSysNames
                                    ):
                                        atomNames.append(chemAtomSysName.atomName)
                            ...
                            if selectedChemCompVars:
                                ...
                                if len(selectedChemCompVars) > 1:
                                    print(
                                        "  Warning: %d chemCompVar '%s','%s' matches based on atomnames - picking first match"
                                        % (len(selectedChemCompVars), chemComp.ccpCode, linking)
                                    )
                        ...
                        if not chemCompVar:
                            ...
                            if not chemCompVar:
                                chemCompVars = getDescriptorElementMatches(
                                    linkingChemCompVars, descriptorElements, verbose=False
                                )
                        ...
                        if not chemCompVar and not forceUserInteraction:
                            print(
                                "  ERROR: no chemCompVar %s,%s,%s found while automatically creating molecule - need user interaction."
                                % (chemComp.molType, chemComp.ccpCode, linking)
                            )
                            forceUserInteraction = True
                        ...
                        for origSequenceNum in range(0, len(origSequenceDict[origMolName][0])):
                            ...
                            createMoleculeDict[molName][1][origSequenceNum][1].append(
                                (seqElement, chemComp, chemCompVar)
                            )
                        ...
                        print(
                            "  Warning: no chemComp selected for %s, seqCode %s%s..."
                            % (origMolName, seqElement.seqCode, resNameText)
                        )
        ...
        for molName in molNames:
            ...
            print(
                "Created %smolecule %s (molType %s, %d chemComps)"
                % (addText, newMolName, molecule.molType, len(molecule.molResidues))
            )
    ...
    def getChemComp(self, residueType, residueCode, download=True):
        ...
        if residueType in ("other",):
            ...
            chemCompInfoDict = chemCompOverview[residueType]
        else:
        ...
        if ccpCode not in chemCompInfoDict.keys():
        ...
        # get ChemComp:
        chemComp = self.project.findFirstChemComp(molType=residueType, ccpCode=ccpCode)

        if not chemComp:
            # Do a doublecheck for specific molType classes if residueType is 'other'!
            if residueType == "other" and ccpCode not in chemCompInfoDict:
                for tempMolType in chemCompStdDict.keys():
                    # uses getByNavigation to avoid calling refreshTopObjects again
                    chemComp = self.project.getByNavigation(("chemComps", (tempMolType, ccpCode)))
                    if chemComp:
                        break
            ...
            while 1:
                if not molSysCodeSet:
                    molSysCode = self.dataEntry.askString(
                        "Enter molecular system name", "Molecular system name", molSysCode, self.guiParent
                    )

                if self.project.findFirstMolSystem(code=returnMemopsWord(molSysCode)):
                    self.messageReporter.showError(
                        "Error",
                        "Molecular system code '%s' already exists in this project - choose another"
                        % returnMemopsWord(molSysCode),
                        parent=self.guiParent,
                    )
                    molSysCodeSet = 0

                elif molSysCode:
                    self.molSystem = MolSystem.MolSystem(
                        self.project, code=returnMemopsWord(molSysCode), name=molSysCode
                    )
    ...
    def checkChains(self, createMoleculeDict, molSystemBonds):
        ...
        for origMolName in origMolNames:
            ...
            if molType == None:
                ...
                for molRes in molecule.molResidues:
                    if molRes.molType not in molTypeCount:
                ...
                if len(molTypeCount.keys()) > 2:
                    for tMolType in molTypeOrderList:
                        if tMolType in molTypeCount:
                            if molTypeCount[tMolType] * 1.0 / molTypeCount["total"] > 0.9:
                                molType = tMolType
                                break

            if molType not in origMolNamesByMolType:
            ...
            for sequenceIndex in range(numSequences):
                ...
                if self.sequence and hasattr(self.sequence, "chainCode"):
                    chainCode = self.sequence.chainCode
                    if chainCode not in existingChainCodeList:
                        existingChainCodeList.append(chainCode)
        ...
        for molType in molTypeOrderList:
            if molType in origMolNamesByMolType:
        ...
        for origMolName in origMolNames:
            ...
            if not self.molecule.molResidues:
                print(
                    "  Error: molecule %s does not have any molResidues! Chain creation aborted..." % self.molecule.name
                )
                self.addWarning(
                    "Molecule %s not created because its sequence elements could not be identified."
                    % self.molecule.name
                )
                continue

            chainLoop = True  # to make sure findChainMatch is not called when multiple chain creation requested...
            numSequences = len(createMoleculeDict[origMolName][1])
            ...
            for sequenceIndex in range(numSequences):
                ...
                if chains:
                    ...
                    while newChain:
                        ...
                        elif hasattr(sequenceElements[0][0], "seqInsertCode"):
                            formatSeqInsertCode = sequenceElements[0][0].seqInsertCode
                        else:
                        ...
                        if self.sequence and hasattr(self.sequence, "chainCode"):
                            ...
                            self.useOriginalChainCode
                            and hasattr(self.sequence, "originalChainCode")
                            and self.sequence.originalChainCode
                        ):
                        ...
                        if not chainCode or (
                            chainCode == defaultMolCode and (not self.minimalPrompts or self.forceNonEmptyChainCode())
                        ):
                            chainCode = None
                            characters = [
                                "",
                            ] + [x for x in chainCodeString]

                            for char1 in characters:
                                for char2 in characters:
                                    for char3 in chainCodeString:
                                        tempChainCode = char1 + char2 + char3
                            ...
                            if not self.minimalPrompts and sequenceElements[0][1].ccpCode != "Hoh":
                        ...
                        self.chains.append(
                            self.molSystem.newChain(
                                molecule=self.molecule, code=chainCode, pdbOneLetterCode=pdbOneLetterCode
                            )
                        )
                        ...
                        if chainMapping:
                            ...
                            for chainMapping in chainMappings:
                                seqIdToCodeMapping[chainMapping.firstSeqId] = (
                                    chainMapping.formatFirstSeqCode,
                                    chainMapping.formatSeqInsertCode,
                                )
                            ...
                            for resPos in range(seqLen):
                                ...
                                if not residue:
                                    print(
                                        "  Warning: residue information missing for seqId %d in chain '%s'! Ignored."
                                        % (resPos + 1, self.chains[-1].code)
                                    )
                                    continue
                                ...
                                elif hasattr(seqEl, "seqInsertCode"):
                                    seqInsertCode = seqEl.seqInsertCode
                                else:
                        ...
                        for resPos in range(0, seqLen):
                            ...
                            if residue:
                                if residue.seqId in seqIdToCodeMapping:
                            ...
                            if seqEl not in seqElToChainRes:
                                seqElToChainRes[seqEl] = []

                            seqElToChainRes[seqEl].append(residue)

                        print(
                            "Created chain '%s', start seqCode %s, end seqCode %s, molecule '%s'..."
                            % (self.chains[-1].code, str(firstSeqCode), str(lastSeqCode), self.molecule.name)
                        )
                        ...
                        if formatChainCode and not chainMapping:
                            ...
                            for firstSeqId in seqIds:
                                ...
                                if hasattr(self.sequence, "originalChainCode"):
                                    chainMapping.addApplicationData(
                                        Implementation.AppDataString(
                                            application=ccpNmr_kw,
                                            keyword="originalFormatChainCode",
                                            value=self.sequence.originalChainCode,
                                        )
                                    )
                        ...
                        if numSequences > sequenceIndex:
                            if (
                                ...
                                and self.sequence == createMoleculeDict[origMolName][1][sequenceIndex + 1][0]
                            ):
                                newChain = False
                            elif (
                                sequenceIndex > 0
        ...
        for seqEl in seqElToChainRes.keys():
            if seqEl.secStrucInfo:
                ...
                for residue in seqElToChainRes[seqEl]:
                    residue.addApplicationData(
                        Implementation.AppDataString(
                            application=self.format, keyword=secStrucInfo_kw, value=str(seqEl.secStrucInfo)
                        )
                    )
    ...
    def duplicateChain(self):
        ...
        if not self.minimalPrompts:
            ...
            newChain = self.messageReporter.showYesNo(
                ...
                "Do you want to create another chain for molecule '%s' in molecular system '%s' (already %d chain(s) present)?"
                % (molecule.name, self.molSystem.code, numChains),
            )
        ...
        if self.sequence and hasattr(self.sequence, "spinSystems"):
            for spinSystem in self.sequence.spinSystems:
                ...
                if hasattr(spinSystem, "code"):
                    resGroup.addApplicationData(
                        Implementation.AppDataInt(application=self.format, keyword="code", value=spinSystem.code)
                    )
    ...
    def checkMeasurementListExists(self):
        ...
        for measurement in self.measurementFileValues:
            #
            # Ignore if no info available (yet)...
            #
            ...
            for resMeasurementFound in resMeasurementsFound:
                if len(resMeasurementFound) == self.measurementResNum:
                    ...
                    if existingMeasurementList not in resonanceMatches:
            ...
            if resonanceMatches[existingMeasurementList] == len(self.measurementFileValues):
                if not self.minimalPrompts:
                    if self.messageReporter.showYesNo(
                        "Use existing list",
                        "A %s list exists with exactly the same values as are being read in: use this list instead?"
    ...
    def setMeasurementList(self):
        ...
        if not self.measurementList:
            if not self.measurementListName:
                ...
                if not self.minimalPrompts:
                    newMeasurementListName = self.dataEntry.askString(
                        "Enter %s list name" % self.measurementType,
                        "New %s list name" % self.measurementType,
                        self.measurementListName,
                        self.guiParent,
                    )
            ...
            if "unit" not in self.measurementListKeywds or not self.measurementListKeywds["unit"]:
                self.measurementListKeywds["unit"] = "default"

            # print 'KEYWDS: [%s]' % self.measurementListKeywds

            self.measurementList = self.ApiMeasurementList(
                self.project.currentNmrProject, name=self.measurementListName, **self.measurementListKeywds
            )
        ...
        if not self.dataSource:
            ...
            if not self.dataSource:
                if not self.experiment:
                    ...
                    while not self.experiment:
                        popup = self.multiDialog.ExperimentCreate(
                            self.guiParent, expName=self.specName, numDim=self.numDim, project=self.project
                        )
                ...
                if not self.minimalPrompts:
                    newDataSourceName = self.dataEntry.askString(
                        "Enter dataSource name",
                        "Enter name for processed dataSource (experiment %s)" % self.experiment.name,
                        dataSourceName,
                        self.guiParent,
                    )
                ...
                self.dataSource = createPpmFreqDataSource(
                    self.experiment, dataSourceName, "processed", self.experiment.numDim
                )
            ...
            if self.experiment.numDim != self.numDim:
                self.messageReporter.showWarning(
                    "Warning", "Mismatch in number of dimensions for peak list and given experiment", self.guiParent
                )

        if not self.dataSource:
            raise self.FormatConverterError("No datasource selected!")
    ...
    def getSpecificExperimentList(self):
        ...
        if self.numDim:
            ...
            experimentList = [x for x in self.project.currentNmrProject.sortedExperiments() if x.numDim == self.numDim]
            interactionText = "Existing %sD experiment:" % (self.numDim)
  ...
  """
    ...
    def fileExists(self, fileName, verbose=True):
        ...
        if not os.path.exists(fileName):
            if verbose:
                self.messageReporter.showError(
                    "Error", "File %s does not exist - aborting." % fileName, parent=self.guiParent
                )
            value = 0

        return value

    def createResonance(self, resName, isotopeCode="unknown", fixed=False, origSource=None, resLabel=None):
        """
        ...
        if not fixed:
            resonance = self.project.currentNmrProject.newResonance(isotopeCode=isotopeCode)
        else:
            resonance = self.nmrConstraintStore.newFixedResonance(isotopeCode=isotopeCode)
        ...
        if not resLabel and origSource and hasattr(origSource, "resLabel") and origSource.resLabel:
            resLabel = origSource.resLabel

        if resLabel:
            setUniqueAppData("AppDataString", resonance, self.format, "origResLabel", resLabel)

        self.newResonances.append(resonance)
            ...
            if not self.minimalPrompts:
                ...
                if self.project.currentNmrProject.experiments:
                    ...
                    interaction = self.multiDialog.MultiSelectionList(
                        ...
                        + "Select experiments linked to %s lists" % self.measurementType,
                        text="Existing experiments:",
            ...
            if not self.minimalPrompts:
                ...
                if self.project.currentMethodStore.methods:
                    ...
                    interaction = self.multiDialog.SelectionList(
                        ...
                        selectionDict=selectionDict,
                        title="Project '%s': " % self.project.name
                        + "Select software linked to %s lists" % self.measurementType,
    ...
    def setT2ListSpecific(self):
        ...
        if self.tempCalibMethod:
            self.measurementListKeywds["tempCalibMethod"] = self.tempCalibMethod
        elif hasattr(self.measurementFile, "tempCalibMethod"):
            self.measurementListKeywds["tempCalibMethod"] = self.measurementFile.tempCalibMethod

        if self.tempControlMethod:
            self.measurementListKeywds["tempControlMethod"] = self.tempControlMethod
        elif hasattr(self.measurementFile, "tempControlMethod"):
            self.measurementListKeywds["tempControlMethod"] = self.measurementFile.tempControlMethod
    ...
    def setSingleResonanceMeasurementSpecific(self, resName, resonance):
        ...
        if abs(self.measurement.value - self.rawMeasurement.value) > self.measurementAllowedDiff:
            interaction = self.multiDialog.MeasurementSelect(
                self.guiParent,
                resName,
                resonance,
                self.rawMeasurement.value,
                self.measurementList,
                self.measurementApiCode,
            )
    ...
    def setShiftSpecific(self, resName, resonance):
        ...
        if self.autoMergeShifts or self.minimalPrompts:
            if not self.autoMergeShifts:
                print(
                    "  Warning: merged chemical shift values %.3f and %.3f for resonance %s"
                    % (self.measurement.value, self.rawMeasurement.value, resName)
                )
    ...
    def setMultipleResonanceMeasurementSpecific(self, resNames, resonances):
        ...
        if (
            self.rawMeasurement.value != None
            and abs(self.measurement.value - self.rawMeasurement.value) > self.measurementAllowedDiff
        ):
    ...
    def createSingleResonanceMeasurement(self, resonance, rawMeasurement):
        ...
        for attrName, keywd in (("error", "valueError"), ("figOfMerit", "figOfMerit")):
            if hasattr(rawMeasurement, keywd):
                keywds[attrName] = getattr(rawMeasurement, keywd)

        if self.thisAssignmentValid():
            ...
            print(
                "For the residue %s%s assignment probability %.2f is below threshold"
            ...
            for molecule in self.project.sortedMolecules():
                if len(sequenceChemComps) == len(molecule.molResidues):
                    ...
                    if sequenceChemComps == molSequenceChemComps:
                        ...
                        molSequenceChemCompVars = [
                            molResidue.chemCompVar for molResidue in molecule.sortedMolResidues()
                        ]
                        ...
                        if not fullMatch or not self.minimalPrompts:
                            ...
                            useMolecule = self.messageReporter.showYesNo(
                                "Use molecule",
                                "Sequence:\n%s\ncorresponds to molecule template with name '%s'.\n%sUse this molecule template for sequence setup?"
                                % (seqText, molecule.name, matchText),
                                parent=self.guiParent,
                            )
    ...
    def askMoleculeName(self, molName):
        ...
        if self.minimalPrompts == 0:
            ...
            while molNameExists:
                molName = self.dataEntry.askString(
                    "Enter molecule name", "Molecule template name", molName, self.guiParent
                )

                if self.project.findFirstMolecule(name=molName):
                    self.messageReporter.showError(
                        "Error",
                        "Molecule template name '%s' already exists in this project - choose another" % molName,
                        parent=self.guiParent,
                    )
                elif molName:
                    molNameExists = 0
    ...
    def createMolResidues(self, createMoleculeInfo, moleculeBonds):
        ...
        for resPos in range(0, numRes):
            ...
            if chemComp and chemCompVar:
                ...
                if hasattr(seqElement, "authorSeqCode") and seqElement.authorSeqCode != None:
                    keywds = {
        ...
        for seqEl in seqEls:
            if seqEl in seqElToRes.keys():
                for residue in seqElToRes[seqEl]:
                    ...
                    if not namingSystem:
                        print(
                            "  Warning: no %s naming system available for %s,%s - aborting molResLink creation."
                            % (self.namingSystemName, chemComp.molType, chemComp.ccpCode)
                        )
                        continue
                                ...
                                if not prevLink:
                                    ...
                                    if ccLinkEnd:
                                        ...
                                        if not molResLinkEnd:
                                            ...
                                            print(
                                                "  Error: could not set MolSystem LinkEnd '%s' for residue '%s.%d' because already exists on Molecule level!"
                                                % (chemAtomName, residue.chain.code, residue.seqCode)
                                            )
                                ...
                                for boundSeqEl, boundSysAtomName in bondDict[seqEl][sysAtomName]:
                                    if boundSeqEl in seqElToRes.keys():
                                        for boundResidue in seqElToRes[boundSeqEl]:
                                            ...
                                            boundNamingSystem = boundChemComp.findFirstNamingSystem(
                                            ...
                                            if not boundNamingSystem:
                                                print(
                                                    ...
                                                    % (
                                                        self.namingSystemName,
                                                        boundChemComp.molType,
                                                        boundChemComp.ccpCode,
                                                    )
                                                )
                                                continue

                                            boundChemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                                                boundChemComp, boundChemComp.chemAtoms, self.namingSystemName
                                            )

                                            boundChemAtomSysName = None
                                            for tChemAtomSysName in boundChemAtomSysNames:
                                                if (
                                                    boundSysAtomName == tChemAtomSysName.sysName
                                                    or boundSysAtomName in tChemAtomSysName.altSysNames
                                                ):
                                            ...
                                            if boundChemAtomSysName:
                                                ...
                                                if Package == MolSystem:
                                                    nextLink = boundResidue.findFirstMolSystemLinkEnd(
                                                        linkCode=boundChemAtomName
                                                    )
                                                    if not nextLink:
                                                        ccLinkEnd = boundResidue.molResidue.chemComp.findFirstLinkEnd(
                                                            linkCode=boundChemAtomName
                                                        )

                                                        if ccLinkEnd:
                                                            nextLink = MolSystem.MolSystemLinkEnd(
                                                                boundResidue, linkCode=boundChemAtomName
                                                            )

                                                elif Package == Molecule:
                                                    nextLink = boundResidue.findFirstMolResLinkEnd(
                                                        linkCode=boundChemAtomName
                                                    )
                                                ...
                                                if nextLink:
                                                    ...
                                                    if not bondLink:
                                                        ...
                                                        if not bondLink:
                                                            searchDict[searchDictKey].reverse()
                                                            try:
                                                                bondLink = getattr(Package, objectName)(
                                                                    parent, **searchDict
                                                                )
                                                                print(
                                                                    "  Set %sbond between (%s%s.%d,%s) and (%s%s.%d,%s)"
                                                                    % (
                                                                        infoString,
                                                                        residueInfo,
                                                                        residue.ccpCode,
                                                                        ...
                                                                        chemAtomName,
                                                                        boundResidueInfo,
                                                                        boundResidue.ccpCode,
                                                                        boundResidue.seqCode,
                                                                        boundChemAtomName,
                                                                    )
                                                                )
                                                            except:
                                                                print(
                                                                    "  Error: failed to set %sbond between (%s%s.%d,%s) and (%s%s.%d,%s)"
                                                                    % (
                                                                        infoString,
                                                                        residueInfo,
                                                                        residue.ccpCode,
                                                    ...
                                                    print(
                                                        "  Warning: could not set molResLink %s because does not exist for (%s,%s). ChemComp info probably needs updating!"
                                                        % (
                                                            boundChemAtomName,
                                                            boundResidue.molType,
                                                            boundResidue.ccpCode,
                                                        )
                                                    )
                                                ...
                                                print(
                                                    "  Warning: could not set molecular bond because no %s sysName found for %s (%s,%s)."
                                                    % (
                                                        self.namingSystemName,
                                                        boundSysAtomName,
                                                        boundResidue.molType,
                                        ...
                                        print(
                                            "  Warning: could not set molecular bond because sequence element (%s,%d) not converted into molResidue."
                                            % (boundSeqEl.residueType, boundSeqEl.seqCode)
                                        )

                            else:
                                print(
                                    "  Warning: could not set molResLink %s because does not exist for (%s,%s). ChemComp info probably needs updating!"
                                    % (chemAtomName, residue.molType, residue.ccpCode)
                                )

                        else:
                            print(
                                "  Warning: could not set molecular bond because no %s sysName found for %s (%s,%s)."
                                % (self.namingSystemName, sysAtomName, residue.molType, residue.ccpCode)
                            )

            else:
                print(
                    "  Warning: could not find reference dictionary information for (%s,%s) to set bonds."
                    % (seqEl.seqCode, str(seqEl.formatCode))
                )
    ...
    def askChainCode(self, chainCode):
        ...
        while chainCodeExists:
            chainCode = self.dataEntry.askString(
                "Enter chain code", "New chain code\n(molecule %s)" % self.molecule.name, chainCode, self.guiParent
            )
            ...
            if self.molSystem.findFirstChain(code=chainCode):
                self.messageReporter.showError(
                    "Error",
                    "Chain code '%s' already exists in this project - choose another" % chainCode,
                    parent=self.guiParent,
                )
            elif chainCode:
                chainCodeExists = 0
        ...
        if not self.dataDimRefs:
            ...
            if self.guiParent:
                ...
                while not self.dataDimRefs:
                    popup = self.multiDialog.DataDimRefSelectByPeakDim(
                        self.guiParent, self.dataSource, rawPeaks, order=self.peakDimOrder
                    )
                ...
                self.messageReporter.showError(
                    "Error",
                    "Have to pass in dataDimRefs for reading peaks in non-GUI environment. Aborting.",
                    parent=self.guiParent,
                )
                return
    ...
    def thisAssignmentValid(self):
        ...
        Check whether this assignment is valid. Default is true - is redefined for some formats.
    ...
    def setPeak(self):
        ...
        if newPeak:
            ...
            self.peak.addApplicationData(
                Implementation.AppDataInt(application=self.format, keyword=peakNum_kw, value=self.rawPeak.num)
            )
    ...
    def setPeakDimResonance(self):
        ...
        for resNameIndex in range(len(self.resNames)):
            ...
            if self.resName not in self.resonanceNames:
                ...
                if hasattr(self, "resLabels"):
                    resLabel = self.resLabels[resNameIndex]
                else:
                    resLabel = None

                resonance = self.createResonance(self.resName, resLabel=resLabel)
        ...
        if self.numPeakLists > 1:
            ...
            if self.specName and self.peakFile.specNames.count(self.specName) == 1:
                ...
                if specNameList and not self.minimalPrompts and self.guiParent:
                    textMessage = (
                        "The imported peak list has data for multiple experiments.\nSelect which data in this list "
                    )

                    if self.dataSource:
                        textMessage += "belongs to\nexperiment '%s', datasource '%s':" % (
                            self.dataSource.experiment.name,
                            self.dataSource.name,
                        )
                    else:
                        textMessage += "you want to convert to the data model."

                    popup = self.multiDialog.MultiSelectionList(
                        self.guiParent,
                        specNameList,
    ...
    def peakListGetDimMapping(self):
        ...
        if self.dataDimRefs == None:
            ...
            if self.peakList.dataSource.numDim == 1:
                ...
                tempDataDimRefs = numValidDims * [None]

                for mapping in self.peakList.findAllApplicationData(application=self.format, keyword="dimMapping"):
                    (origPeakFileDim, peakDimIndex) = returnInts(mapping.value.split(tagSep))
    ...
    def peakDimSelect(self):
        ...
        while not tempDataDimRefs:
            popup = self.multiDialog.PeakDimSelect(
                self.guiParent, self.peakList, self.format, dataDimRefs=self.dataDimRefs, order=self.peakDimOrder
            )
                ...
                for peakDim in self.peak.sortedPeakDims():
                    ...
                    if peakDimContribs:
                        for peakDimContrib in peakDimContribs:
                            if peakDimContrib.resonance in self.resonanceToAtoms:
                        ...
                        if contrib[:i] == otherContrib[:i] and contrib[i + 1 :] == otherContrib[i + 1 :]:
                            ...
                            if resToAtom == otherResToAtom:
                                ...
                                if resToAtom.getAtomId()[:-1] == otherResToAtom.getAtomId()[:-1]:
                                    ...
                                    (self.compressResonances)
                                    and resToAtom.getAtomSetId()
                                    and otherResToAtom.getAtomSetId()
                                    and resToAtom.getAtomSetId()[:-1] == otherResToAtom.getAtomSetId()[:-1]
                                ):
    ...
    def createAtomMeasurements(self):
        ...
        if self.measurementResNum != 1:
            self.messageReporter.showError(
                "Not supported",
                "Output of measurement list with %d resonances not supported yet!" % self.measurementResNum,
            )
            return
    ...
    def getChainMapping(self, resetMapping=0):
        ...
        if chainList:
            ...
            if not mappingChainDict or (resetMapping and not self.minimalPrompts):
                ...
                if self.format in exportChainCodeDict:
    ...
    def getSortedChainList(self):
        ...
        # simplified code. Rasmus 22/7/2011
        self.chainList = [tt[1] for tt in sorted((item[1], item[0]) for item in self.chainDict.items())]
    ...
    def selectStructureGeneration(self):
        ...
        if self.nmrConstraintStore:
            strucGenList = [
                self.project.currentNmrProject.findFirstStructureGeneration(nmrConstraintStore=self.nmrConstraintStore)
            ]
        else:
        ...
        if not strucGen:
            ...
            if not self.minimalPrompts:
                strucGenName = self.dataEntry.askString(
                    "Enter structure generation name", "Name of structure generation", strucGenName, self.guiParent
                )
    ...
    def selectStructureEnsemble(self):
        ...
        if not self.structureEnsemble:
            ...
            newEnsembleId += 1
            self.structureEnsemble = self.project.newStructureEnsemble(
                molSystem=self.molSystem, ensembleId=newEnsembleId, atomNamingSystem=self.namingSystemName
            )
    ...
    def orderByAtomNames(self, residueResonanceToAtomList):
        ...
        if self.presetResonanceMapping:
            self.compressPresetResidueResonanceToAtomList(residueResonanceToAtomList)

        elif self.compressResonances and not self.useOriginalResNames and not self.individualAtoms:
        ...
        if self.useOriginalResNames:
            ...
            if not namingSystem:
                print(
                    "  Warning: no %s naming system for chemComp %s,%s!"
                    % (self.namingSystemName, chemComp.molType, chemComp.ccpCode)
                )
            ...
            for chemAtomOrSetList in (chemCompVar.sortedChemAtoms(), chemCompVar.sortedChemAtomSets()):
                for chemAtomOrSet in chemAtomOrSetList:
                    chemAtomSysName = findChemAtomOrSetSysName(namingSystem, chemAtomOrSet)
    ...
    def getMeasurementSpinSystemList(self):
        ...
        for resonanceToSpinSystem in self.atomMeasurements.keys():
            ...
            if spinSystem not in self.spinSystemToResonanceDict:
    ...
    def getAtomNameMatch(self, atomName, residueResonanceToAtomList):
        ...
        for resonanceToAtom in residueResonanceToAtomList:
            if atomName == resonanceToAtom.atomName:
                if atomName in self.atomNamesDict:
                    print(
                        "  ERROR: Atom name confusion for '%s.%d.%s'!" % (self.chain.code, self.residue.seqId, atomName)
                    )
                self.atomNamesDict[atomName] = resonanceToAtom
        ...
        for resonanceToAtom in residueResonanceToAtomList[:]:
            ...
            if chemAtomSet:
                for otherResonanceToAtom in residueResonanceToAtomList:
                    ...
                    if (
                        chemAtomSet == otherChemAtomSet
                        and resonanceToAtom in self.atomMeasurements
                        and otherResonanceToAtom in self.atomMeasurements
                    ):
                        measurement = self.atomMeasurements[resonanceToAtom]
                        otherMeasurement = self.atomMeasurements[otherResonanceToAtom]

                        if abs(measurement.value - otherMeasurement.value) < shiftIdentityTolerance:
                            chemAtomSetSysName = self.getChemAtomOrSetSysName((chemAtomSet, namingSystem))

                            if chemAtomSetSysName:
                                newAtomName = chemAtomSetSysName.sysName

                                self.setResonanceToAtoms(
                                    resonanceToAtom.resonance, resonanceToAtom.chain, resonanceToAtom.seqId, newAtomName
                                )
                                ...
                                print(
                                    "  Warning: %s:%s compressed %s and %s to %s name..."
                                    % (
                                        self.chemComp.ccpCode,
                                        self.residue.seqId,
                                        listAtomNames[0],
                                        listAtomNames[1],
                                        newAtomName,
    ...
    def compressPresetResidueResonanceToAtomList(self, residueResonanceToAtomList):
        """compress ResidueResonanceToAtomList for preset resonances
        Here shift comparison has been done up front, and useAtomSetName has been set
        """
        ...
        for resonanceToAtom in residueResonanceToAtomList[:]:
            # r2a = resonanceToAtom
            # print ('### PRESET', r2a.atomName, r2a.atomSetName,
            #                r2a.chemAtom and  r2a.chemAtom.name,
            #                r2a.chemAtomSet and  r2a.chemAtomSet.name,
            #                r2a.useAtomSetName, r2a.resonance,
            #                [x for x in r2a.otherLinkedResonances],
            #                [x for x in r2a.otherGroupResonances])

            if resonanceToAtom.useAtomSetName:
                bigChemAtomSet = resonanceToAtom.getChemAtomSet()
                chemCompVar = resonanceToAtom.getResidue().chemCompVar

                if bigChemAtomSet and bigChemAtomSet is not resonanceToAtom.chemAtomSet:
                    chemAtomSetSysName = self.getChemAtomOrSetSysName((bigChemAtomSet, namingSystem))

                    if chemAtomSetSysName:
                        newAtomName = chemAtomSetSysName.sysName

                        self.setResonanceToAtoms(
                            resonanceToAtom.resonance,
                            resonanceToAtom.chain,
                            resonanceToAtom.seqId,
                            newAtomName,
                            chemAtomOrSet=bigChemAtomSet,
                        )
                        newResonanceToAtom = self.resonanceToAtoms[resonanceToAtom.resonance][-1]

                        measurement = self.atomMeasurements.pop(resonanceToAtom)
                        del self.origAtomMeasurements[resonanceToAtom]
                        self.atomMeasurements[newResonanceToAtom] = measurement
                        self.origAtomMeasurements[newResonanceToAtom] = measurement

                        ii = 0
                        while ii < len(residueResonanceToAtomList):
                            otherResonanceToAtom = residueResonanceToAtomList[ii]

                            if otherResonanceToAtom == resonanceToAtom:
                                residueResonanceToAtomList[ii] = newResonanceToAtom
                                ii += 1

                            elif (
                                bigChemAtomSet == otherResonanceToAtom.getChemAtomSet()
                                and resonanceToAtom in self.atomMeasurements
                                and otherResonanceToAtom in self.atomMeasurements
                            ):
                                # We have a match - remove second copy
                                del residueResonanceToAtomList[ii]
                                del self.atomMeasurements[otherResonanceToAtom]
                                del self.origAtomMeasurements[otherResonanceToAtom]

                                self.setResonanceToAtoms(
                                    otherResonanceToAtom.resonance,
                                    otherResonanceToAtom.chain,
                                    otherResonanceToAtom.seqId,
                                    newAtomName,
                                    chemAtomOrSet=bigChemAtomSet,
                                )
    ...
    def checkAtomMeasurements(self):
        ...
        if self.verbose and self.atomMeasurements != {}:
            ...
            for resonanceToAtom in self.atomMeasurements.keys():
                ...
                if self.useOriginalResNames:
                    outputLines.append(
                        "  Original resonance name: '%s', %s %.3f"
                        % (resonanceToAtom.originalResName, self.measurementType, measurement)
                    )

                else:
                    outputLines.append(
                        "  Ccp identification: chain %s, seqId %d, atomName %s, %s %.3f"
                        % (
                            resonanceToAtom.chain.code,
                            resonanceToAtom.seqId,
                            resonanceToAtom.atomName,
                            self.measurementType,
                            measurement,
                        )
                    )
    ...
    def getConstraintItems(self):
        ...
        for item in self.rawConstraint.items:
            if len(item.members) != self.constraintFile.constraintElements:
                print(
                    "  %s constraint item with %d members (Id %d)... ignored"
                    % (self.constraintApiCode, len(item.members), self.rawConstraint.Id)
                )
                continue
    ...
    def setConstraintItems(self):
            ...
            for fixedResonance1 in resonances1:
                for fixedResonance2 in resonances2:
                    if fixedResonance1 == fixedResonance2:
                        print(
                            "  Error: duplicate resonance %s found - constraint not imported"
                            % getResNameText(fixedResonance1)
                        )
                        self.setConversionError(
                            self.constraintList,
                            self.rawConstraint.Id,
                            1,
                            "Not handling item from distance restraint (original ID %d) - duplicate resonances",
                        )
                        continue
                    ...
                    for constrItem in fixedResonance1.findAllPairwiseConstraintItems(
                        className="%sConstraintItem" % self.constraintApiCode, constraint=self.constraint
                    ):
                    ...
                    if resonanceMatch == 1:
                        ...
                        self.constraintItem = self.ApiConstraintItem(
                            self.constraint, resonances=fixedResonances, firstResonance=fixedResonance1
                        )
        ...
        if not self.constraint.items:
            ...
            self.setConversionError(
                ...
                self.rawConstraint.Id,
                1,
                "Distance restraint (original ID %d) deleted - no constraint items left.",
            )
            self.convertCount[self.mainCode][1][self.valueCode] -= 1
    ...
    def setDistanceConstraint(self):
        ...
        if "origId" in self.constraintPars:
    ...
    def setChemShiftConstraint(self):
        ...
        if not self.constraint:
            ...
            self.setConversionError(
                ...
                self.rawConstraint.Id,
                1,
                "Restraint (original ID %d) not created - already exists or not enough information.",
            )
    ...
    def setDihedralConstraint(self):
        ...
        for i in range(0, self.constraintResNum):
            ...
            if fixedResonance not in curFixedResonances:
                ...
                print(
                    "  Error: duplicate resonance %s found - constraint not imported" % getResNameText(fixedResonance)
                )
                self.setConversionError(
                    self.constraintList,
                    self.rawConstraint.Id,
                    1,
                    "Not handling dihedral restraint (original ID %d) - duplicate resonances",
                )
                self.constraint = None
        ...
        if len(curFixedResonances) == self.constraintResNum:
            ...
            if resonanceMatch and not self.newConstraintForDuplicate:
                #
                # In this case means the dihedral constraint already exists - set it
                #
                print(
                    "  Warning: Additional value found for dihedral constraint %s - added as extra item"
                    % (self.rawConstraint.Id)
                )
                self.constraint = self.constraintList.findFirstConstraint(resonances=dihedConstr.resonances)

            else:
                self.constraint = self.ApiConstraint(
                    self.constraintList, resonances=curFixedResonances, **self.constraintPars
                )
                self.convertCount[self.mainCode][1][self.valueCode] += 1
    ...
    def createPeakContribAtomLinks(self):
        ...
        # Now set the info for each resonance with regard to atom, residue, ... name
        self.getResonanceAtomLinks()
    ...
    def getFormatNamingSystemName(self, format=None, version=None):
        ...
        if not format:
            format = self.format
        if not version and hasattr(self, "version"):
            version = self.version

        namingSystemName = format

        if (
            hasattr(self, "forceNamingSystemName")
            and self.forceNamingSystemName
            and self.forceNamingSystemName != "None"
        ):
            namingSystemName = self.forceNamingSystemName

        elif (format, version in formatNamingDict):
            namingSystemName = formatNamingDict[(format, version)]

        elif (format, None in formatNamingDict):
    ...
    def setResonanceToAtoms(self, resonance, *args, **keywds):
        ...
        if resonance not in self.resonanceToAtoms:
    ...
    def getResonanceAtomLinks(self):
        ...
        # for tag in ('compressResonances', 'individualAtomsIfNoSet', 'minimalPrompts',
        #            'useCcpnChainInfo', 'presetResonanceMapping'):
        #   if (hasattr(self, tag)):
        #     print '###', tag, getattr(self,tag)

        # Return if new resonanceToAtoms not needed
        if hasattr(self, "resonanceToAtoms") and self.resonanceToAtoms and self.presetResonanceMapping:
        ...
        if self.useOriginalResNames:
            ...
            if self.project.currentNameMappingStore:
                nameMappings = self.project.currentNameMappingStore.findAllNameMappings(
                    namingSystem=self.namingSystemName, isOriginalImport=True
                )
                ...
                if not nameMappings:
                    ...
                    nameMappings = self.project.currentNameMappingStore.findAllNameMappings(
                        namingSystem="Unknown", isOriginalImport=True
                    )

                    if nameMappings:
                        for nameMapping in nameMappings:
                            print(
                                "  Warning: using original format %s with unknown naming system"
                                % nameMapping.formatName
                            )
            ...
            if not self.allowedFormats:
                print(
                    "  Warning: no allowed existing formats found for %s export in %s naming system - original names cannot be used"
                    % (self.format, self.namingSystemName)
                )
        ...
        # Added sorting to make calculation (and output) deterministic
        for resonance in sorted(self.resonancesToLink, key=operator.attrgetter("serial")):
            #
            ...
            if self.useOriginalResNames:
                ...
                for allowedFormat in self.allowedFormats:
                    ...
                    if resNames:
                        (seqCodeToIdMapping, seqCodeToChainMapping, seqCodes) = self.getChainMappingsSeqCodeInfo(
                            self.allowedFormatChainMappings[allowedFormat]
                        )

                        for resName in resNames:
                            (formatChainCode, formatSeqCode, formatSpinSystemId, formatSeqInsertCode, atomName) = (
                                getNameInfo(resName)
                            )
                            ...
                            for firstSeqCodeKey, lastSeqCodeKey in seqCodes:
                                ...
                                if (
                                    formatSeqCode >= firstSeqCode
                                    and formatSeqInsertCode == seqInsertCode
                                    and (lastSeqCode == None or formatSeqCode <= lastSeqCode)
                                ):
                                    ...
                                    if chainMapping.formatChainCode == formatChainCode:
                                        ...
                                        if chain:
                                            ...
                                            seqId = (
                                                formatSeqCode
                                                - chainMapping.formatFirstSeqCode
                                                + chainMapping.firstSeqId
                                            )
                                            self.setResonanceToAtoms(
                                                resonance, chain, seqId, atomName, originalResName=resName
                                            )
            ...
            if not self.useOriginalResNames and resonance not in self.resonanceToAtoms:
                #
                ...
                if resonanceSet:
                    ...
                    # Rasmus Fogh 27/6/12 replaces previous code

                    resSetResonances = resonanceSet.sortedResonances()

                    # get common residue and naming system
                    allAtoms = set(x for y in atomSets for x in y.atoms)
                    residues = set(x.residue for x in allAtoms)
                    rr = residues.pop()
                    if residues:
                    ...
                    namingSystem = rr.chemCompVar.chemComp.findFirstNamingSystem(name=self.namingSystemName)

                    if not namingSystem:
                        print(
                            "  Warning: No %s naming system available for resonance %s"
                            % (self.namingSystemName, getResNameText(resonance)),
                        )
                    ...
                    if len(resSetResonances) == 1 and residue and allAtoms == residue.atoms:
                        # Residue-level constraint. Set it.
                        self.setResonanceToAtoms(resonance, residue.chain, residue.seqId, None, atomSetName=None)

                    elif residue and chemAtomSet and len(atomSets) == 2 and len(resSetResonances) <= 2:
                        #
                        # Case where 1/2 resonances, 2 atomSets, one chemAtomSet
                        #

                        # Rasmus Fogh 27/6/12 end replacement block
                        ...
                        if (
                            hasattr(self, "compressResonances")
                            and self.compressResonances
                            and self.individualAtomsIfNoSet
                        ):
                            ...
                            if atomSysName:
                                ...
                                print(
                                    "  Warning: no chemAtomSetSysName found for %s ... ChemAtomSet not used."
                                    % exportName
                                )

                        else:
                            atomSysName = self.getChemAtomOrSetSysName(
                                (chemAtomSet, namingSystem), exportName, findIupac=True
                            )
                        ...
                        if chemAtomSet.chemAtomSets:
                            chemAtomOrSetList = chemAtomSet.sortedChemAtomSets()
                        else:
                            chemAtomOrSetList = chemAtomSet.sortedChemAtoms()

                        chemAtomOrSetNames = [caos.name for caos in chemAtomOrSetList]
                        # chemAtomOrSetNames.sort() # Objects already sorted by name
                        ...
                        # test mapping matches first, to guard against incorrect appdata
                        # that gives the same name for both resonances.
                        mappingMatches = None
                        ll = [x.findFirstApplicationData(keyword="mappingSingleMatch") for x in resSetResonances]
                        if None not in ll:
                            ll = [x.value for x in ll]
                            if len(ll) == len(set(ll)):
                                # no duplicates - use mapping data
                                mappingMatches = ll

                        for ii, resonance in enumerate(resSetResonances):
                            #
                            # Already mapped as covalently bound prochiral resonance?
                            #

                            if resonance in preferredAtomNameDict:
                                ...
                                if mappingMatches:
                                    preferredAtomNames.append(mappingMatches[ii])
                                ...
                                #
                                    ...
                                    index = getAmbigNamingPriority(resonance, chemAtomSet)
                                    preferredAtomNames.append(chemAtomOrSetNames[index])

                        if len(set(preferredAtomNames)) != len(preferredAtomNames):
                            raise Exception(
                                """Inconsistent assignment: 
resonances %s map onto atom names %s
Cannot map atom names for export.
"""
                                % (([x.serial for x in resSetResonances]), preferredAtomNames)
                            )
                        #
                        # Reorganise...
                        #
                        ...
                        for chemAtomOrSet in chemAtomOrSetList:
                            ...
                            if chemAtomOrSetName in preferredAtomNames:
                                ...
                                del resSetResonances[resonanceIndex]
                                del preferredAtomNames[resonanceIndex]
                                ...
                                validIndices = range(len(preferredAtomNames))

                                for tempChemAtomOrSet in chemAtomOrSetList:
                                    ss = tempChemAtomOrSet.name
                                    if ss in preferredAtomNames:
                                        removeIndex = preferredAtomNames.index(ss)
                                        validIndices.pop(validIndices.index(removeIndex))

                                if validIndices:
                                    useIndex = validIndices[0]
                                    resonance = resSetResonances.pop(useIndex)
                                    del preferredAtomNames[useIndex]
                            ...
                            if (
                                hasattr(resonance, "covalentlyBound") and resonance.covalentlyBound
                            ):  # Temporary fix because FixedResonance has no covalentlyBound
                                #
                                    ...
                                    if covalentResSet:
                                        for covalentAtomSet in covalentResSet.atomSets:
                                            ...
                                            for refChemBond in refChemAtom.chemBonds:
                                                refChemBondAtoms = set(refChemBond.chemAtoms)
                                                refChemBondAtoms.remove(refChemAtom)
                                                otherRefChemAtom = refChemBondAtoms.pop()

                                                if isinstance(otherRefChemAtom, ChemComp.ChemAtom):
                                                    if chemAtomOrSet == otherRefChemAtom.chemAtomSet:
                                                    ...
                                                    if covalentPreferredAtomName:
                                                        preferredAtomNameDict[covalentResonance] = (
                                                            covalentPreferredAtomName
                                                        )
                                                        break
                            ...
                            # Decide whether to split this list into individual component Atom(Set)s
                            if chemAtomOrSet.className == "ChemAtomSet":
                                splitChemAtomSet = False
                                if self.individualAtoms:
                                    splitChemAtomSet = "deep"
                                elif self.individualAtomsIfNoSet:
                                    if not self.getChemAtomOrSetSysName((chemAtomOrSet, namingSystem)):
                                        splitChemAtomSet = "shallow"

                                if splitChemAtomSet:
                                    # split to direct content of ChemAtom(Set)s
                                    tempChemAtomOrSetList = (
                                        chemAtomOrSet.sortedChemAtomSets() or chemAtomOrSet.sortedChemAtoms()
                                    )
                                    if (
                                        splitChemAtomSet == "deep"
                                        and tempChemAtomOrSetList[0].className == "ChemAtomSet"
                                    ):
                                        # split all the way down to individual atoms
                                        ii = 0
                                        while ii < len(tempChemAtomOrSetList):
                                            caos = tempChemAtomOrSetList[ii]
                                            if caos.className == "ChemAtomSet":
                                                del tempChemAtomOrSetList[ii]
                                                tempChemAtomOrSetList.extend(caos.sortedChemAtomSets())
                                                tempChemAtomOrSetList.extend(caos.sortedChemAtoms())
                                            else:
                                                # we have a ChemAtom here.
                                                # We could probably break, but just to be safe
                                                ii += 1

                            for tempChemAtomOrSet in tempChemAtomOrSetList:
                                atomSysName = self.getChemAtomOrSetSysName(
                                    (tempChemAtomOrSet, namingSystem), exportName, findIupac=True
                                )
                                ...
                                if not atomSysName:
                                    ...
                                    print(
                                        "  Warning: no %s chemAtomSysName found... using CCPN name."
                                        % (tempChemAtomOrSet.name)
                                    )
                                ...
                                self.setResonanceToAtoms(
                                    ...
                                    seqId,
                                    refSysName,
                                    atomSetName=chemAtomSetName,
                                    chemAtomOrSet=tempChemAtomOrSet,
                                    atomType=tempChemAtomOrSet.elementSymbol,
                                )
                    ...
                    elif residue and len(atomSets) == 1 and len(resSetResonances) == 1:
                        #
                        # 1 resonance, 1 atomset. Simplest case.
                        # Needs special handling to deal with isopropyl groups
                        #

                        atomSet = atomSets[0]
                        resonance = resSetResonances[0]

                        seqId = residue.seqId
                        chain = residue.chain

                        exportName = getResName(
                            chain.code, residue.seqCode, atomSet.name or atomSet.findFirstAtom().name
                        )

                        chemAtom = atomSet.findFirstAtom().chemAtom

                        # NB all atoms should ALWAYS point to the same ChemAtomSet,
                        # but it costs nothing ot be careful.
                        aSet = set(x.chemAtom.chemAtomSet for x in atomSet.atoms)
                        if len(aSet) == 1:
                            chemAtomSet = aSet.pop()
                        else:
                            chemAtomSet = None

                        if len(atomSet.atoms) == 1:
                            atomSysName = self.getChemAtomOrSetSysName((chemAtom, namingSystem))
                            if atomSysName:
                                refSysName = atomSysName.sysName
                            else:
                            ...
                            if chemAtomSet:
                                if (
                                    ...
                                    chemAtomSetSysName = self.getChemAtomOrSetSysName(
                                        (chemAtomSet, namingSystem), exportName, findIupac=True
                                    )
                                    if chemAtomSetSysName:
                                        chemAtomSetName = chemAtomSetSysName.sysName
                                        ...
                                        chemAtomSetName = chemAtomSet.name
                                        print(
                                            "  Warning: no chemAtomSetSysName found for %s ... using CCPN name."
                                            % exportName
                                        )
                        ...
                        elif chemAtomSet and not self.individualAtoms:
                            # assert chemAtomSet and len(atomSet.atoms) > 1
                            ...
                            deepSet = chemAtomSet.chemAtomSet
                            if deepSet:
                                chemAtomSet = deepSet
                            ...
                            # assert (individualAtoms or not chemAtomSet) and len(atomSet.atoms) > 1
                            # Deal with atoms individualy
                            # chemAtomSet should always exist here, but it costs nothing to be prepared.
                            ...
                            for chemAtom in chemAtomSet.chemAtoms:
                                ...
                                atomSysName = self.getChemAtomOrSetSysName(
                                    (chemAtom, namingSystem), exportName, findIupac=True
                                )
                                ...
                                if atomSysName:
                                    ...
                                    print(
                                        "  Warning: no %s sysName for %s... using CCPN name."
                                ...
                                self.setResonanceToAtoms(
                                    ...
                                    refSysName,
                                    atomSetName=chemAtomSetName,
                                    chemAtomOrSet=chemAtomOrSet,
                                    atomType=chemAtomOrSet.elementSymbol,
                                )

                            # Done with this resonance
                            continue

                        # set resonanceToAtoms
                        chemAtomOrSet = chemAtomSet or chemAtom
                        self.setResonanceToAtoms(
                            resonance,
                            chain,
                    ...
                    #
                    # This is the generic case: ambiguous and not prochiral.
                    #
                        ...
                        for atomSet in atomSets:
                            ...
                            if len(atomSet.atoms) > 1:
                                ...
                                if deepSet and not self.individualAtoms:
                                    #
                                    # First check if valid...
                                    #

                                    # chemAtomsFound = checkChemAtomsForDeepSet(chemAtomSet.chemAtomSet,atomSets)
                                    chemAtomsFound = checkChemAtomsForDeepSet(deepSet, atomSets)
                                    ...
                                    if chemAtomsFound and namingSystem:
                                        ...
                                        # chemAtomSetSysName = self.getChemAtomOrSetSysName((chemAtomSet,namingSystem))
                                        chemAtomSetSysName = self.getChemAtomOrSetSysName((deepSet, namingSystem))

                                        if chemAtomSetSysName:
                                            # sysNamesLink = [(chemAtomSetSysName.sysName,chemAtomSet)]
                                            sysNamesLink = [(chemAtomSetSysName.sysName, deepSet)]
                                            chemAtomSet = None
                                            chemAtomSetName = chemAtomSetSysName.sysName
                                ...
                                exportName = getResName(chain.code, residue.seqCode, chemAtomSetName)

                                if not sysNamesLink and not self.individualAtoms:
                                    ...
                                    if atomSysName:
                                        sysNamesLink = [(atomSysName.sysName, chemAtomSet)]

                                    else:
                                        if chemAtomSet.chemAtomSet and not self.individualAtomsIfNoSet:
                                            print("  Warning: using CCPN name for atomSet '%s'..." % exportName)
                                            sysNamesLink = [(chemAtomSet.name, chemAtomSet)]

                                        else:
                                            print(
                                                "  Warning: no %s sysNames for %s... using individual atom name(s)."
                                                % (self.namingSystemName, exportName)
                                            )
                            ...
                            if not sysNamesLink:
                                if chemAtomSet:
                                    ...
                                    # chemAtoms = [chemAtom] # NB if we have more than one, we surely want them all
                                    chemAtoms = [x.chemAtom for x in atomSet.sortedAtoms()]
                                ...
                                if (
                                    not chemAtomSetName
                                    and namingSystem
                                    and not self.individualAtoms
                                    and chemAtom.chemAtomSet
                                    and hasattr(self, "compressResonances")
                                    and self.compressResonances
                                ):
                                    chemAtomSetSysName = self.getChemAtomOrSetSysName(
                                        (chemAtom.chemAtomSet, namingSystem)
                                    )
    ...
    def setResonanceToSpinSystem(self, resonance, *args, **keywds):
        ...
        if resonance not in self.resonanceToSpinSystems:
    ...
    def getResonanceSpinSystemLinks(self):
        ...
        # Added sorting to make result (and output) deterministic
        for resonance in sorted(self.resonancesToLink, key=operator.attrgetter("serial")):
            ...
            if resonance not in self.resonanceToSpinSystems:
                ...
                self.setResonanceToSpinSystem(
                    resonance, resonanceGroup, assignNames, linkedSpinSystems=linkedResonanceGroups
                )  # atomSetName = chemAtomSetName, chemAtomOrSet = chemAtomOrSet, atomType = refChemAtom.elementSymbol)
    ...
    def sortConstraintsForExport(self, constraints, getResonancesFunc):
        ...
        for constraint in constraints:
            ...
            for resonance in getResonancesFunc(constraint):
                ...
                if resonance in self.resonanceToAtoms:
        ...
        for self.constraint in constraints:
            ...
            while it < len(constraintItemList):
                ...
                for x in range(2):
                    ...
                    if not self.compressResonances and self.item.findFirstApplicationData(
                        application="ccp", keyword="copied", value=x
                    ):
                        isCopied = True
                    ...
                    if fixedResonance in self.resonanceToAtoms:
                        ...
                        if len(self.resonanceToAtoms[fixedResonance]) == 1 and atomSetId:
                            ...
                            while tempIt < len(constraintItemList):
                                ...
                                if matchFixedResonance in self.resonanceToAtoms:
                                    matchAtomSetId = self.resonanceToAtoms[matchFixedResonance][0].getAtomSetId()

                                    if (
                                        matchAtomSetId
                                        and len(self.resonanceToAtoms[matchFixedResonance]) == 1
                                        and atomSetId[:-1] == matchAtomSetId[:-1]
                                    ):
                                        #
                                        ...
                                        if (
                                            otherFixedResonance.resonanceSet
                                            and otherFixedResonance == matchOtherFixedResonance
                                        ):
                                            #
                                            ...
                                            if not self.compressResonances:
                                                #
                                                # Check whether was copied
                                                #

                                                if not isCopied and tempItem.findFirstApplicationData(
                                                    application="ccp", keyword="copied", value=x
                                                ):
                                                    isCopied = True
                                            ...
                                            if self.compressResonances or (
                                                self.compressResonances == None and isCopied
                                            ):
                                                otherResidue = (
                                                    otherFixedResonance.resonanceSet.findFirstAtomSet()
                                                    .findFirstAtom()
                                                    .residue
                                                )
                                                ...
                                                seqIds = [atomSetId[1], otherResidue.seqId, matchAtomSetId[1]]

                                                print(
                                                    "  %s constraint %d: Removed %s.%d.%s,%s.%d.%s based on %s.%d.%s,%s.%d.%s"
                                                    % (
                                                        constraintApiCode,
                                                        ...
                                                        chainCodes[1],
                                                        seqIds[1],
                                                        otherFixedResonance.name,
                                                        chainCodes[2],
                                                        seqIds[2],
                                                        matchFixedResonance.name,
                                                        chainCodes[1],
                                                        seqIds[1],
                                                        matchOtherFixedResonance.name,
                                                    )
                                                )
                            ...
                            if x == 1 and resSetNames[1]:
                                if resSetNames[0] and len(resSetNames[0]) == 1:
                                    ...
                                    while tempIt < len(constraintItemList):
                                        ...
                                        if matchFixedResonance not in self.resonanceToAtoms:
                                            tempIt += 1
                                            continue
                                        ...
                                        if (
                                            matchOtherFixedResonance in self.resonanceToAtoms
                                            and len(self.resonanceToAtoms[matchOtherFixedResonance]) == 1
                                        ):
                                            matchOtherAtomSetId = self.resonanceToAtoms[matchOtherFixedResonance][
                                                0
                                            ].getAtomSetId()

                                            if (
                                                matchAtomSetId
                                                and matchOtherAtomSetId
                                                and atomSetId[0:2] == matchAtomSetId[0:2]
                                                and matchOtherAtomSetId[:-1] == resSetNames[0][0][:-1]
                                            ):
                                                #
                                                # If self.compressResonances is default then output depends on
                                                # whether an item was duplicated... should retain original 'compression'
                                                ...
                                                if self.compressResonances or (
                                                    ...
                                                    print(
                                                        "  %s constraint %d: Removed %s.%d.%s,%s.%d.%s based on %s.%d.%s,%s.%d.%s (SET MATCH!)"
                                                        % (
                                                            constraintApiCode,
                                                            self.constraint.serial,
                                                            chainCodes[0],
                ...
                if None in resSetNames:
                    ...
                    warningText = "Not handling constraint %d, item %d, resonance(s) '%s' not assigned." % (
                        self.constraint.serial,
                        it,
                        str(text),
                    )
                ...
                for atomOrSetId in resSetNames[0]:
                    ...
                    for otherAtomOrSetId in resSetNames[1]:
                        ...
                        if otherAtomOrSetId:
                            ...
                            otherSeqCode = self.getExportSeqCode(
                                self.chainDict[otherChain][1], otherChain.findFirstResidue(seqId=otherSeqId)
                            )
                        ...
                        getattr(self, "setRaw%sConstraintItem" % constraintApiCode)()

                        # Possible that constraint item creation fails... only relevant if rawConstraintItem exists!
                        if not hasattr(self, "rawConstraintItem") or self.rawConstraintItem:
                            for funcFormat in ("setRaw%sConstraintItemMembers", "setRaw%sConstraintItemInfo"):
    ...
    def checkRawConstraintItems(self, delete=True):
        ...
        if not self.rawConstraint.items:
            if delete:
    ...
    def setDihedralConstraintFileConstraints(self):
        ...
        for self.constraint in constraints:
            ...
            if badConstraint or "" in self.resSetNames:
                ...
                warningText = "Not handling dihedral restraint %d, resonance(s) '%s' not assigned." % (
                    self.constraint.serial,
                    text,
                )
            ...
            if len(self.constraint.items) > 1:
                print(
                    "  Warning: more than one item for dihedral constraint %d. Written as separate ones."
                    % self.constraint.serial
                )
    ...
    def setConversionError(self, parent, constraintId, errorType, errorText):
        ...
        parent.addApplicationData(
            Implementation.AppDataString(application=self.format, keyword="ConversionError", value=str(value))
        )
    ...
    def setPeakDimContribs(self, peakAssignDim, peakDims=None, resonances=None):
        ...
        if len(self.assignCombination) == peakAssignDim:
            ...
            for i in range(0, len(peakDims)):
                ...
                if resonance in self.resonancesPeakContrib[i]:
            ...
            for i in range(0, len(peakDims)):
                ...
                if resonance not in self.resonancesPeakContrib[i]:
                    peakDim = peakDims[i]
            ...
            for resonance in self.peakContribResonances[peakAssignDim][resName]:
                self.setPeakDimContribs(
                    peakAssignDim + 1, peakDims=peakDims + [peakDim], resonances=resonances + [resonance]
                )
    ...
    def createPeakAssignmentResonances(self):
        ...
        for resName in self.peakAssignment.assignments[self.peakAssignDim]:
            if resName not in self.resonanceNames:
                #
                # Create resonance if doesn't exist
                #
    ...
    def peakAssignmentCheckPeakList(self):
        ...
        if not self.peakList and self.guiParent:
            ...
            if self.peakList.dataSource.numDim != self.peakAssignmentsFile.numDim:
                self.messageReporter.showError(
                    "Error", "Number of dimensions for peak list and peak assignment file do not match.", self.guiParent
                )
                return None
    ...
    def peakAssignmentSetDimMapping(self):
        ...
        for origDim in self.peakAssignmentsFile.assignmentsDim:
            peakListDimFound = None

            for mapping in self.peakList.findAllApplicationData(application=self.format, keyword="dimMapping"):
                (origPeakFileDim, peakListDim) = returnInts(mapping.value.split(tagSep))

                if origPeakFileDim == origDim:
                    peakListDimFound = peakListDim
                    break

            self.assignDim.append(peakListDimFound)
    ...
    def getChainLinks(self):
        ...
        if modelNums:
            ...
            for coord in self.coordinateFile.modelCoordinates[refModel]:
                ...
                elif chainId not in badChainIdList:
                    badChainIdList.append(chainId)
        ...
        if badChainIdList:
            print(
                "  Warning: no coordinate chain correspondence for chain(s) with codes %s!"
                % str(["'%s'" % chainCode for chainCode in badChainIdList])
            )
    ...
    def autoSelectCoordNamingSystem(self):
        ...
        for coordinate in curCoordinates:
            ...
            if refChainId != oldChainId:
                ...
                if oldChainId in self.formatChainCodeDict:
            ...
            if coordSeqCode != oldSeqCode or coordinate.insertionCode != oldInsertionCode:
                ...
                if multipleChains:
                    ...
                    for firstSeqCodeKey, lastSeqCodeKey in seqCodes:
                        ...
                        if (
                            oldSeqCode >= firstSeqCode
                            and oldInsertionCode == seqInsertCode
                            and (lastSeqCode == None or oldSeqCode <= lastSeqCode)
                        ):
                            chainMapping = seqCodeToChainMapping[(firstSeqCode, seqInsertCode)]
                            chain = self.chainMappingChain[chainMapping]
                            break
            ...
            for namingSystem in chemComp.namingSystems:
                if namingSystem.name not in namingSystemNames:
                    namingSystemNames[namingSystem.name] = 0

                sysNameKey = (chemComp.ccpCode, namingSystem.name)

                if sysNameKey in atomSysNameDict:
                    chemAtomSysNames = atomSysNameDict[sysNameKey]
                else:
                    chemAtomSysNames = findAllSysNamesByChemAtomOrSet(chemComp, chemCompAtoms, namingSystem.name)
        ...
        if self.namingSystemName not in bestHits and bestHits:
            print(
                "  Resetting naming system to %s for coordinates - has higher atom name match (%.f%%)"
                % (bestHits[0], highestMatch)
            )
            self.namingSystemName = bestHits[0]
        else:
            print(
                "  Selected naming system %s has %.f%% match with atom names." % (self.namingSystemName, highestMatch)
            )

    def mapCoordinateAtoms(self):
        """June 2011 Rasmus Fogh: Changed to fit new model:
        Map only first model - all models must have same atoms
        Added altLocation to mapping key
        """
        ...
        # Track mapping problems, and connect to structureEnsemble.
        mappingErrors = {"chain": [], "residue": [], "atom": []}
        ...
        for coordinate in curCoordinates:
            ...
            mappingKey = (refChainId, coordSeqCode, coordinate.insertionCode, coordAtomName, altLoc)
            coordId = "%s.%d%s.%s.%s" % mappingKey
            ...
            if coordSeqCode != oldSeqCode or coordinate.insertionCode != oldInsertionCode:
                ...
                if multipleChains:
                    ...
                    for firstSeqCodeKey, lastSeqCodeKey in seqCodes:
                        ...
                        # print "  ",firstSeqCodeKey
                        # print "  ",lastSeqCodeKey
                        # print
                    ...
                    if not chain:
                        print(
                            "  Error: no chain/residue mapping for seqCode %d, chainCode '%s' - ignoring."
                            % (oldSeqCode, oldChainId)
                        )
                    ...
                    if not validSeqCode:
                        ...
                        self.ignoredSeqCodes.append((oldChainId, oldSeqCode, oldInsertionCode))
                        mappingErrors["residue"].append(
                            ((oldChainId, oldSeqCode, oldInsertionCode), "Not a valid sequence code")
                        )
                        oldSeqCode = ""
                ...
                if not residue:
                    if len(chain.residues) == 1:
                        ...
                        print(
                            "  Warning: single residue chain '%s', picking only residue for seqCode %d (seqId %d)"
                            % (chain.code, oldSeqCode, residue.seqId)
                        )

                    if not residue:
                        print(
                            "  ERROR: can't handle seqCode %d in chain '%s' - no matching residue! Searched chain %s, seqId %d."
                            % (oldSeqCode, oldChainId, chain.code, seqId)
                        )
                        self.ignoredSeqCodes.append((oldChainId, oldSeqCode, oldInsertionCode))
                        mappingErrors["residue"].append(
                            ((oldChainId, oldSeqCode, oldInsertionCode), "Cannot find residue match")
                        )
                        oldSeqCode = ""
                        continue
                ...
                if chemCompVar in chemAtomSysNamesByVar:
                    (chemAtomSysNames, backupChemAtomSysNames) = chemAtomSysNamesByVar[chemCompVar]
                else:
                    chemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                        chemComp, chemCompVar.sortedChemAtoms(), self.namingSystemName
                    )
                    backupChemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                        chemComp, chemCompVar.sortedChemAtoms(), "IUPAC"
                    )

                if not chemAtomSysNames:
                    chemAtomSysNames = backupChemAtomSysNames
                    backupChemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                        chemComp, chemCompVar.sortedChemAtoms(), "MSD"
                    )

                    if not chemAtomSysNames:
                        chemAtomSysNames = backupChemAtomSysNames
                        backupChemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                            chemComp, chemCompVar.sortedChemAtoms(), "CUSTOM"
                        )

                if backupChemAtomSysNames:
                    chemAtomSysNames = chemAtomSysNames + backupChemAtomSysNames

                if chemCompVar not in chemAtomSysNamesByVar:
            ...
            if mappingKey in self.coordAtomMapping:
                continue
            ...
            #
                ...
                if (oldChainId, seqLabel, coordAtomName in self.setMapping):
                ...
                #
                    ...
                    if self.addNameMappings:
                        for nameMappingKey in (residue.ccpCode, "ALL"):
                            if nameMappingKey in self.addNameMappings:
                    ...
                    # Make sure that original atom name comes SECOND after mapped one - this is to avoid problems with
                    # HB1/HB2 to HB2/HB3 type mappings for methylenes.
                    searchNames.append(coordAtomName)
                ...
                if atom:
                    ...
                    if self.linkAtoms:
                        if not atomStatus:
                            ...
                            interaction = self.multiDialog.CoordinateAtomSelect(
                                self.guiParent, residue, coordAtomName, seqLabel, headerLines=headerLines
                            )
        ...
        #
        # Now tag the structure ensemble with the mapping error info. Put in as dictionary!
        #

        hasMappingErrors = False
        for mappingErrorKey in mappingErrors.keys():
            if mappingErrors[mappingErrorKey]:
                hasMappingErrors = True
                break

        if hasMappingErrors:
            self.structureEnsemble.addApplicationData(
                Implementation.AppDataString(application=ccpNmr_kw, keyword="mappingErrors", value=str(mappingErrors))
            )
    ...
    def setCoordinates(self):
        ...
        #
        # Set index mapping and create coord Atoms (etc.) if necessary
        #
        ...
        indexMapping = {}
        nextIndex = 0
        orderedCoordAtoms = self.structureEnsemble.orderedAtoms

        curCoordinates = self.coordinateFile.modelCoordinates[modelNums[0]]
        for coordinate in curCoordinates:
            #
            # get mappingKey
            #
            ...
            mappingKey = (chainId, seqCode, seqInsertCode, atomName, altLoc)

            #
            # set up mapping and create coord aAtoms as necessary
            #
            tt = self.coordAtomMapping.get(mappingKey)
            if tt is not None:
                # get data for working
                (chain, residue, atom, atomStatus) = tt
                ...
                if chainId != oldChainId or oldChain != chain:
                    ...
                    elif cChain not in self.coordResidues:
                        self.coordResidues[cChain] = {}
                        for cResidue in cChain.sortedResidues():
                            residueKey = (cResidue.seqCode, cResidue.seqInsertCode)
                ...
                if seqCode != oldSeqCode or seqInsertCode != oldSeqInsertCode:
                    ...
                    if residueKey in self.coordResidues[cChain]:
                        cResidue = self.coordResidues[cChain][residueKey]

                    else:
                        cResidue = cChain.newResidue(
                            seqId=residue.seqId, seqCode=residueKey[0], seqInsertCode=residueKey[1]
                        )
                ...
                #
                cAtom = cResidue.findFirstAtom(name=atom.name, altLocationCode=altLoc)

                if cAtom is None:
                    # No cAtom - make one

                    if orderedCoordAtoms:
                        raise self.FormatConverterError(
                            "Cannot create new Atoms in pre-existing Ensemble - %s" % (mappingKey,)
                        )

                    # make new cAtom
                    cAtom = cResidue.newAtom(name=atom.name, altLocationCode=altLoc)

                    # set index mapping
                    indexMapping[mappingKey] = nextIndex
                    nextIndex += 1
                ...
                elif not orderedCoordAtoms:
                    # raise self.FormatConverterError(
                    # "Duplicate atom record found while setting up new ensemble - %s"
                    # % (mappingKey,)
                    # )
                    # print (
                    # "WARNING Duplicate atom record found while setting up new ensemble - %s"
                    # % (mappingKey,)
                    # )
                    pass

                else:
                    # cAtom found. set mapping

                    try:
                        indexMapping[mappingKey] = orderedCoordAtoms.index(cAtom)
                    except ValueError:
                        raise self.FormatConverterError("No mapping found for atom - %s" % (mappingKey,))

        #
        # Create models and add their data
        #
        ...
        nAtoms = len(indexMapping)
        iiii = 0
        skipcount = 0

        for modelNum in modelNums:
            indmap = indexMapping.copy()

            coordData = [0.0] * (nAtoms * 3)
            occData = [0.0] * nAtoms
            bfacData = [0.0] * nAtoms
            ...
            model.addApplicationData(
                Implementation.AppDataString(application=ccpNmr_kw, keyword=originalFormat_kw, value=self.format)
            )
            self.structures.append(model)

            # loop over coordinates to get data
            # (NB done to process in order and delete temporary data as we go)
            ...
            while curCoordinates:
                ...
                # get record index
                offset = indmap.get(mappingKey)
                if tt is None or offset is None:
                    # set warning records and skip record
                    ...
                    if atomName[0] == "Q":
                        mk = pseudoAtomMessage
                    else:
                        mk = mappingKey

                    if mk not in noMappingKeys:
                        noMappingKeys[mk] = []

                    noMappingKeys[mk].append(modelNum)
                    continue

                else:
                    # delete mapping entry to avoid duplicates
                    del indmap[mappingKey]
                    # add data to data arrays
                    self.convertCount[self.mainCode][1]["coordinates"] += 1
                    iiii += 1

                    tag = "bFactor"
                    if hasattr(coordinate, tag):
                        hasBFactors = True
                        bfacData[offset] = getattr(coordinate, tag)

                    tag = "occupancy"
                    if hasattr(coordinate, tag):
                        hasOccupancies = True
                        occData[offset] = getattr(coordinate, tag)

                    # coordinates
                    ii = offset * 3
                    for tag in ("x", "y", "z"):
                        coordData[ii] = getattr(coordinate, tag)
                        ii += 1

            # set data in model
            if hasBFactors:
                model.setSubmatrixData("bFactors", bfacData)
            if hasOccupancies:
                model.setSubmatrixData("occupancies", occData)
            model.setSubmatrixData("coordinates", coordData)
        ...
        for key, modelNums in sorted(noMappingKeys.items()):
            print("  Error: no mapping found for %s (models %s)" % (key, ",".join(str(x) for x in modelNums)))

    def setCoordChainOrigData(self, cChain, coordinate):

        chainId = coordinate.chainId
        cChain.addApplicationData(
            Implementation.AppDataString(application=self.format, keyword="originalChainCode", value=chainId)
        )

    def setCoordResidueOrigData(self, cResidue, coordinate, seqCode, seqInsertCode):

        cResidue.addApplicationData(
            Implementation.AppDataInt(application=self.format, keyword="originalSeqCode", value=seqCode)
        )
        cResidue.addApplicationData(
            Implementation.AppDataString(application=self.format, keyword="originalSeqInsertCode", value=seqInsertCode)
        )

        if hasattr(coordinate, "resName"):
            cResidue.addApplicationData(
                Implementation.AppDataString(application=self.format, keyword="originalLabel", value=coordinate.resName)
            )
    ...
    def setCoordAtomOrigData(self, cAtom, coordinate, atomName, atomStatus):
        ...
        if not atomStatus:
            ...
            cAtom.addApplicationData(
                Implementation.AppDataString(application=self.format, keyword="originalName", value=atomName)
            )

    def getCoordinateChains(self):

        if self.exportChains and isinstance(self.exportChains, (list, tuple, set, frozenset)):
            ...
            for model in self.structures:
                for coordChain in model.structureEnsemble.coordChains:
                    if coordChain.chain and coordChain.chain not in self.chains:
                        self.chains.append(coordChain.chain)

    def createCoordinates(self):

        attrList1 = ["atom", "seqCode", "atomName", "exportChainCode", "atomType"]
        attrList2 = ["origChainCode", "origSeqCode", "origSeqInsertCode", "origResLabel", "origAtomName"]

        if self.useOriginalData:
            attrListAll = attrList1 + attrList2
        else:
            attrListAll = attrList1

        # Sort by serial
        structures = sorted(self.structures, key=operator.attrgetter("serial"))

        # get all models into same ensemble.
        # This ensures that atom names and ordering fit
        aset = set(x.structureEnsemble for x in structures)
        if len(aset) == 1:
            newEnsemble = None
        else:
            newEnsemble = makeEnsemble(structures, replaceStructures=False)

        try:
            # necessary to clean up new ensemble in case of error

            # set up data structures
            model = structures[0]
            structureEnsemble = model.structureEnsemble
            data = {}
            coordAtoms = structureEnsemble.orderedAtoms
            for tag in attrListAll:
                data[tag] = [None] * len(coordAtoms)
            ...
            # loop over coordAtoms to get atom data
            for ii, coordAtom in enumerate(coordAtoms):
                ...
                # get chain info
                data["exportChainCode"][ii] = self.chainDict[chain][0]

                # get residue info
                data["seqCode"][ii] = self.getExportSeqCode(self.chainDict[chain][1], residue)
                chemCompVar = residue.chemCompVar
                chemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                    chemCompVar.chemComp, chemCompVar.chemAtoms, self.namingSystemName
                )
                ...
                data["atomType"][ii] = atom.chemAtom.elementSymbol
                ...
                if origFormat is not None:
                    ...
                    if origResLabel:
                        data["origResLabel"][ii] = origResLabel.value

                    origChemAtomSysNames = findAllSysNamesByChemAtomOrSet(
                        chemCompVar.chemComp, chemCompVar.chemAtoms, origNamingSysName
                    )
                    ...
                    if origAtomName:
                        ...
                        if not origAtomName:
                            origAtomName = atomName
                    data["origAtomName"][ii] = origAtomName

            # do actual setting of coordinate data for all models.
            coordinateSerial = 0
            modelId = 0
            for model in structures:
                ...
                # set input data and call coordinate setup
                for jj in indices:
                    coordinateSerial += 1
                    offset = jj * 3
                    atom = data["atom"][jj]
                    ...
                    params = {
                        "modelId": modelId,
                        "coordinateSerial": coordinateSerial,
                        "x": coordinates[offset],
                        "y": coordinates[offset + 1],
                        "z": coordinates[offset + 2],
                        "bFactor": bFactors[jj],
                        "occupancy": occupancies[jj],
                        "residue": residue,
                        "chain": residue.chain,
                    }

                    # set attributes on self for transfer to setRawCoordinate
                    # Horrible, horrible, but too much work to redo
                    for tag, val in params.items():
                        setattr(self, tag, val)
                    for tag in attrListAll:
                        setattr(self, tag, data[tag][jj])

                    self.setRawCoordinate()

        finally:
            # clean up
            if newEnsemble is not None:
                newEnsemble.delete()
            # raise
    ...
    def setChemCompInfo(self):
        ...
        while not valuesChecked:
            interaction = self.multiDialog.ChemCompInfo(
                self.guiParent, self.formula, len(self.rawChemComp.bonds), self.chemCompInfo
            )
            ...
            for attrName, value in self.chemCompInfo:
                if attrName == "ccpCode":
                    ...
                    for repository in repositoryList:
                        curChemCompPath = os.path.join(
                            repository.url.path,
                            "molecule",
                            "ChemComp",
                            "%s.%s.xml" % (self.chemCompInfo[0][1], self.chemCompInfo[1][1]),
                        )

                        if os.path.exists(curChemCompPath):
                            self.messageReporter.showError(
                                "Error",
                                "Molecule type '%s', ccpCode '%s' already exists!"
                                % (self.chemCompInfo[0][1], self.chemCompInfo[1][1]),
                                parent=self.guiParent,
                            )
    ...
    def checkAtomAndBondInfo(self):
        ...
        for rawAtom in self.rawChemComp.atoms:
            if hasattr(rawAtom, "name") and rawAtom.name:
                ...
                if rawAtom.atomType not in atomNames:
        ...
        if self.allowPopups:
            interaction = self.multiDialog.ChemCompAtomBondInfo(
                self.guiParent, self.chemCompInfo, self.chemCompAtoms, self.chemCompBonds
            )
    ...
    def createLinkAndVars(self):
        ...
        if not self.minimalPrompts and self.allowPopups:
            descriptor = self.dataEntry.askString(
                "Enter chemCompVar descriptor",
                "Descriptor for the state of this chemCompVar",
                descriptor,
                self.guiParent,
            )

        self.chemCompVars = [(descriptor, chemCompByLink)]
        ...
        while addDescriptor:
            ...
            addDescriptor = self.messageReporter.showYesNo(
                "Add/change descriptor",
                "Do you want to add/change a chemCompVar? '%s' exist already." % string.join(descriptors, ", "),
            )
            ...
            if addDescriptor:
                ...
                while getDescriptor:
                    descriptor = self.dataEntry.askString(
                        "Enter chemCompVar descriptor", "Give a descriptor name:", descriptor, self.guiParent
                    )
                    ...
                    if not descriptor:
                        ...
                        if self.messageReporter.showYesNo(
                            "Error",
                            "Descriptor '%s' already exists! Do you want to overwrite?" % descriptor,
                            parent=self.guiParent,
                        ):
                            changeDescriptor = 1
                            getDescriptor = 0
    ...
    def makeChemComp(self):
        ...
        if self.chemCompCoords:
            keywds = {
                "sourceName": coordinateImportSourceName,
        ...
        if self.setLinkEnds:
            ...
            for linkCode, posRange in linkCodes:
                ...
                for chemAtomName in linkEndChemAtomNames:
                    chemAtom = chemComp.findFirstChemAtom(name=chemAtomName)
                    if not chemAtom:
                        createLinkAtom = False
            ...
            for bondType, stereoChem, bondAtomNames in linkBonds[self.molTypeKey]:
                ...
                if None not in chemAtomList and not chemComp.findFirstChemBond(chemAtoms=tuple(chemAtomList)):
                    linkBond = ChemComp.ChemBond(
                        chemComp, bondType=bondType, chemAtoms=chemAtomList, stereochem=stereoChem
                    )
            ...
            for linkCode, posRange in linkCodes:
                ...
                for chemAtomName in linkEndChemAtomNames:
                    chemAtom = chemComp.findFirstChemAtom(name=chemAtomName)
                    linkEndChemAtoms.append(chemAtom)
                    if not chemAtom:
                        missingAtoms.append(chemAtomName)

                for pos in range(1, posRange + 1):
                    name = "%s_%d" % (linkCode, pos)
                    chemAtom = chemComp.findFirstChemAtom(name=name)
                    linkEndLinkAtoms.append(chemAtom)
                    if not chemAtom:
                        missingAtoms.append(chemAtomName)

                if None not in linkEndChemAtoms + linkEndLinkAtoms:
                    if not chemComp.findFirstLinkEnd(linkCode=linkCode):
                        if len(linkEndLinkAtoms) == 2:
                            remoteLinkAtom = linkEndLinkAtoms[1]
                        else:
                            remoteLinkAtom = None

                        linkEnd = ChemComp.LinkEnd(
                            chemComp,
                            linkCode=linkCode,
        ...
        for descriptor, chemCompByLink in self.chemCompVars:
            for linking, atomRemovedList in chemCompByLink:
                ...
                if isValid:
                    ...
                    chemCompVar = ChemComp.ChemCompVar(
                        chemComp,
                        linking=linking,
                        formalCharge=0,
                        isAromatic=0,
                    ...
                    print(
                        "Could not create %s,%s: %s,%s. Link info missing"
                        % (chemComp.ccpCode, chemComp.molType, descriptor, linking)
                    )
        ...
        if self.chemCompCoords:
            ...
            for atomName in atomNames:
                ...
                chemCompCoord.newChemAtomCoord(
                    name=chemAtom.name, subType=chemAtom.subType, chemCompVarCoords=chemCompVarCoords, x=x, y=y, z=z
                )
    ...
    def getChemCompCoord(self):
        ...
        localKeywds = {}
        if self.noWrite or not self.copyCoordinates:
            localKeywds["copyFile"] = False

        ccc = getChemCompCoord(self.project, self.coordSystem, chemComp.molType, chemComp.ccpCode, **localKeywds)
        ...
        if ccc:
            self.chemCompVarCoord = ccc.findFirstChemCompVarCoord(
                linking=self.chemCompVar.linking, descriptor=self.chemCompVar.descriptor
            )

        if not self.chemCompVarCoord:
            print(
                "  Warning: could not find chemCompVarCoord for %s, %s, %s, %s, %s!"
                % (
                    self.coordSystem,
                    chemComp.molType,
                    chemComp.ccpCode,
                    self.chemCompVar.linking,
                    self.chemCompVar.descriptor,
                )
            )
    ...
    def createRawChemComp(self):
        ...
        for chemAtom in self.chemCompVar.sortedChemAtoms():
            ...
            if not atomSysName:
                print(
                    "  Warning: no %s chemAtomSysName for atom %s - using atom name instead."
                    % (self.namingSystemName, chemAtom.name)
                )
        ...
        for atomNames, bondType in chemBondInfo:
            ...
            for atomName in atomNames:
                if atomName in rawAtomSerialDict:
    ...
    def makeInitialsForCcpn(self, initials):
        ...
        return (
            initials.translate(string.maketrans(string.punctuation, " " * len(string.punctuation)))
            .replace(" ", "")
            .upper()
        )
    ...
    def setPeople(self):
        ...
        for rawPerson in self.peopleAndCitationsFile.persons:
            ...
            if rawPerson.familyTitle:
                if rawPerson.familyTitle[:2].upper() in ("JR", "SR"):
                    familyTitle = rawPerson.familyTitle.capitalize()
                else:  # TO avoid things like 'Iii'
                    familyTitle = rawPerson.familyTitle.upper()
            else:
                familyTitle = None
            ...
            if self.useExistingPersons and self.project.currentAffiliationStore:
                ...
                if person and not person.middleInitials and initials and isinstance(initials, str):
            ...
            if not person:
                ...
                if hasattr(rawPerson, "serial") and rawPerson.serial is not None:
                    keywds2 = {"application": self.format, "keyword": "Serial", "value": rawPerson.serial}
            ...
            if rawPerson not in self.rawPersonToCcpnPerson:
    ...
    def findVaguePerson(self, familyName, firstName, initials, familyTitle):
        ...
        for tperson in self.project.currentAffiliationStore.sortedPersons():
            if tperson.familyName and familyName and tperson.familyName.upper() == familyName.upper():
                if (tperson.givenName and firstName and tperson.givenName.upper() == firstName.upper()) or (
                    firstName
                    and (
                        (tperson.firstInitial and tperson.firstInitial.upper() == firstName[0].upper())
                        or (tperson.givenName and tperson.givenName[0].upper() == firstName[0].upper())
                    )
                ):
        ...
        if personsFound:
            for tperson in personsFound:
                ...
                if tperson.middleInitials == tuple(initials) and (
            ...
            if not person:
                if len(personsFound) > 1:
                    print(
                        "  Warning: match (but no full match) found for person %s %s %s - picking first match."
                        % (firstName, initials, familyName)
                    )
        ...
        for rawCitation in self.peopleAndCitationsFile.citations:
            ...
            if not citation:
                ...
                if rawCitation.type == "journal":
                    ...
                    for ccpnKeywd, attrName in (
                        ("journalAbbreviation", "pubShortName"),
                        ("journalFullName", "pubLongName"),
                        ("issue", "issue"),
                    ):
                    ...
                    if rawCitation.editors:
                        ...
                        for rawPerson in rawCitation.editors:
                            ...
                            for person in self.rawPersonToCcpnPerson[rawPerson]:
                                if person not in keywds["editors"]:
                ...
                if rawCitation.authors:
                    ...
                    for rawPerson in rawCitation.authors:
                        for person in self.rawPersonToCcpnPerson[rawPerson]:
                            if person not in keywds["authors"]:
                ...
                if rawCitation.keywords:
                    ...
                    for keyword in rawCitation.keywords:
                        if keyword not in keywds["keywords"]:
                ...
                if rawCitation.pubMedId:
                    keywds["pubMedId"] = rawCitation.pubMedId

                if rawCitation.doi:
                    keywds["doi"] = rawCitation.doi

                if rawCitation.medline:
                    keywds["medlineUiCode"] = rawCitation.medline
    ...
    def findVagueCitation(self, className, title, year):
        ...
        for tcitation in citations:
            if tcitation.className == className and (
                tcitation.title and title and tcitation.title.upper() == title.upper()
            ):
        ...
        for assignment in self.assignmentFile.assignments:
            if self.assignmentValid(assignment):
                ...
                if resonanceGroup and residue:
                    ...
                    print(
                        "  Error: could not set assignment:",
                    )
                    if not resonanceGroup:
    ...
    def setFidDataSource(self):
        ...
        if not self.fidDataSource:
            ...
            if not self.dataSource:
                ...
                self.dataSource = Nmr.DataSource(self.experiment, name=self.format, numDim=self.numDim, dataType="FID")
    ...
    def setExpDimRefs(self):
        ...
        for i in range(0, self.numDim):
            ...
            for expDimRef in expDim.expDimRefs:
                ...
                if nucleus in self.acqParsData.specRefs:
    ...
    def setSpectrometer(self, manufacturer="Bruker", model="unknown", protonFreq=500.00):
        ...
        if not self.experiment.spectrometer:
            ...
            if not self.spectrometer:
                ...
                self.spectrometer = Instrument.NmrSpectrometer(
                    self.project.currentInstrumentStore,
                    name=manufacturer + "_" + model,
                    model=model,
                    nominalFreq=nominalFreq,
                    protonFreq=protonFreq,
                )
    ...
    def setFreqDataSource(self):
        ...
        if not self.freqDataSource:
            ...
            if not self.dataSource:
                self.dataSource = Nmr.DataSource(
                    self.experiment, name=importGeneral_kw, numDim=self.numDim, dataType="processed"
                )
    ...
    def setProcParsGeneral(self):
        ...
        self.dataSource = Nmr.DataSource(self.experiment, name=dataSourceName, numDim=self.numDim, dataType="processed")
        ...
        if "inputFile" in fPars and fPars["inputFile"]:
        ...
        if "bigEndian" in fPars and fPars["bigEndian"] is not None:
            dataStore.isBigEndian = fPars["bigEndian"]
        elif "swap" in fPars and fPars["swap"]:
            dataStore.isBigEndian = not isBigEndian()
        else:
            dataStore.isBigEndian = isBigEndian()

        if "integer" in fPars and fPars["integer"]:
            dataStore.numberType = "int"
        else:
            dataStore.numberType = "float"

        if "headerSize" in fPars and fPars["headerSize"]:
    ...
    def setProcParsDimInfo(self):
        ...
        for dim in range(0, len(expDimRefs)):
            ...
            if self.aliasing and dim in self.aliasing:
            ...
            dataDimRef = Nmr.DataDimRef(
                freqDataDim, refPoint=fPars["refPoint"][dim], refValue=fPars["refPpm"][dim], expDimRef=expDimRef
            )
    ...
    def validDataDimRefs(self):
        ...
        if isinstance(self.dataDimRefs, tuple):
            self.dataDimRefs = list(self.dataDimRefs)

        if isinstance(self.dataDimRefs, list):
            for dataDimRef in self.dataDimRefs:
                if not isinstance(dataDimRef, Nmr.DataDimRef):
                    self.messageReporter.showError(
                        "Error",
                        "Need valid dataDimRef object(s) to write processing parameters.",
                        parent=self.guiParent,
                    )
                    return False
            ...
            self.messageReporter.showError(
                "Error", "Need list of dataDimRef object(s) to write processing parameters.", parent=self.guiParent
            )
            return False

        return True

    def getChemAtomOrSetSysName(self, searchKey, exportName=None, findIupac=False):

        if searchKey in self.atomSysNamesByChemAtom:
            atomSysName = self.atomSysNamesByChemAtom[searchKey]

        else:
            (chemAtomOrSet, namingSystem) = searchKey
            atomSysName = findChemAtomOrSetSysName(namingSystem, chemAtomOrSet, exportName, findIupac=findIupac)
    ...
    def getChemAtomSysNamesFromSet(self, chemAtomSet, chemCompVar, findIupac=False):
        ...
        for chemAtom in chemAtoms:
            chemAtomSysName = self.getChemAtomOrSetSysName((chemAtom, namingSystem))

            if findIupac and not chemAtomSysName:
                chemAtomSysName = self.getChemAtomOrSetSysName((chemAtom, iupacNamingSystem))
...
def filteredNameInfo(resonanceToAtoms, mode=None):
    """get list of chain,seqId,atomName tuples
    free of duplicates, compressed according to compression mode:
    'compress', 'preset' or None
    Used in FormatConverter
    """

    resonanceToAtoms = list(resonanceToAtoms)

    result = list()

    if mode in ("compress", "preset"):
        # Merge resonances from shared atomSets
        # if mode is 'preset' only do it if r2a.useAtomSet

        chemAtomSets = [r2a.getChemAtomSet() for r2a in resonanceToAtoms]

        # for r2a in resonanceToAtoms:
        #  print ('   +++ ', r2a.resonance.serial, r2a.seqId, r2a.atomName, r2a.atomSetName,
        #                  r2a.chemAtom and  r2a.chemAtom.name,
        #                  r2a.chemAtomSet and  r2a.chemAtomSet.name,
        #                  r2a.useAtomSetName,
        ...
        ii = 0
        while ii < len(resonanceToAtoms):
            r2a = resonanceToAtoms[ii]
            atomName = r2a.atomName
            if mode == "compress" or r2a.useAtomSetName:
                chemAtomSet = chemAtomSets[ii]
                if chemAtomSet:
                    chain = r2a.chain
                    seqId = r2a.seqId
                    jj = ii + 1
                    while jj < len(resonanceToAtoms):
                        otherR2a = resonanceToAtoms[jj]
                        if otherR2a.chain == chain and otherR2a.seqId == seqId and chemAtomSets[jj] == chemAtomSet:
                            # resonanceToAtoms share a chemAtomSet. Merge
                            if r2a.atomSetName:
                                # use atomSetName instead of atomName
                                atomName = r2a.atomSetName
                            # remove duplicates
                            del resonanceToAtoms[jj]
                            del chemAtomSets[jj]
                        else:
                            jj += 1
            key = (r2a.chain, r2a.seqId, atomName)
            tt = keyMap.get(key)
            if tt is None or tt[0].atomSetName == atomName or not tt[0].useAtomSetName or not tt[0].getChemAtomSet():
                # favour pseudoatoms by selecting later, or incompletely set r2a
                keyMap[key] = (r2a, atomName)
            ii += 1

        result = [keyMap[x] for x in sorted(keyMap)]

    else:
        # no compression, just transform, remove duplicates, and sort
        result = [(r2a, r2a.atomName) for r2a in resonanceToAtoms]

    #
    # transform to list, sort, and return
    return sorted(result)


def cleanAtomNameLists(resNames, resAtomSetNames):
    ...
    # First remove individual atoms if their atomSetNames are also present
    ii = 0
    while ii < len(resNames):
        ss = resNames[ii]
        jj = ii + 1
        while jj < len(resNames):
            if resAtomSetNames[jj] == ss:
                del resNames[jj]
                del resAtomSetNames[jj]
            else:
                jj += 1
        ii += 1
    ...
    ii = 0
    while ii < len(resNames):
        resAtomSetName = resAtomSetNames[ii]
        if resAtomSetName is not None:
            jj = ii + 1
            while jj < len(resNames):
                if resAtomSetNames[jj] == resAtomSetName:
                    del resNames[jj]
                    del resAtomSetNames[jj]
                    resNames[ii] = resAtomSetName
                else:
                    jj += 1
        ii += 1
...
def getResonanceAtomMap(
    namingSystemName,
    resonances,
    individualAtomsIfNoSet=True,
    individualAtoms=False,
    ignoreChemCompsWithNoSysNames=False,
    compressResonances=False,
):
    """get resonance: ResonanceToAtoms map for specific naming system
    resonances is list or tuple of resonances or fixedResonances
    Stand-alone replacement for DataFormat.getResonanceAtomLinks,
    ...
    if not resonances:
        return resonanceToAtoms

    # set high level variables
    project = resonances[0].root

    usedResonanceSets = set()

    for resonance in resonances:
        # Make assignment map for resonance
        resonanceSet = resonance.resonanceSet
        if resonanceSet:
            # We have assignment, so dd to map (otherwise skip)

            # Only do each ResonanceSet once
            if resonanceSet in usedResonanceSets:
                continue
            else:
                usedResonanceSets.add(resonanceSet)
            ...
            exportName = getResName(
                residue.chain.code, residue.seqCode, (chemAtomSet or atomSets[0].findFirstAtom()).name
            )

            namingSystem = residue.chemCompVar.chemComp.findFirstNamingSystem(name=namingSystemName)
            if not namingSystem:
                print("  Warning: No %s naming system available for resonance %s" % (namingSystemName, exportName))
            ...
            # Start setting mappings:

            if len(resSetResonances) == 1 and residue and allAtoms == residue.atoms:
                # Residue-level constraint. Set it.

                resonanceToAtoms[resonance] = ll = []
                ll.append(ResonanceToAtoms(resonance, residue.chain, residue.seqId, None, atomSetName=None))

            elif residue and chemAtomSet and len(atomSets) == 2 and len(resSetResonances) <= 2:
                # Case where 1/2 resonances, 2 atomSets, one chemAtomSet
                # Typically prochiral pair

                # set up parameters

                # get chemAtomSet atomSysName and update atomSysNameMap
                if chemAtomSet in atomSysNameMap:
                    # NB Do not use .get. Might be in map with value None
                    atomSysName = atomSysNameMap[chemAtomSet]
                else:
                    atomSysName = findChemAtomOrSetSysName(
                        namingSystem, chemAtomSet, exportName, findIupac=not individualAtomsIfNoSet
                    )
                    # findIupac=not compressIfHasSet)
                    atomSysNameMap[chemAtomSet] = atomSysName
                ...
                if atomSysName:
                    ...
                    chemAtomSetName = None
                    print(
                        "  Warning: no %s sysNames found for %s (%s). ChemAtomSet not used."
                        % (namingSystemName, chemAtomSet.name, exportName)
                    )
                else:
                    chemAtomSetName = chemAtomSet.name
                    print(
                        "  Warning 1: no %s sysNames found for %s (%s)... using CCPN name."
                        % (namingSystemName, chemAtomSet.name, exportName)
                    )

                # Make list of chemAtomSet contents (chemAtoms or chemAtomSets)
                chemAtomOrSetList = chemAtomSet.sortedChemAtomSets() or chemAtomSet.sortedChemAtoms()

                # Get their atom names in preferred order
                preferredAtomNames = []
                for reson in resSetResonances:
                    ss = preferredAtomNameDict.get(reson)
                    if ss is not None:
                        # Already mapped as covalently bound prochiral resonance
                        preferredAtomNames.append(ss)

                    else:
                        # set preference order by standard heuristics
                        ii = getAmbigNamingPriority(reson, chemAtomSet)
                        preferredAtomNames.append(chemAtomOrSetList[ii].name)

                # Reverse chemAtomOrSetList if necessary, to make it fit preferred order
                if chemAtomOrSetList[-1].name == preferredAtomNames[0]:
                    chemAtomOrSetList.reverse()

                # Now loop over chemAtoms or chemAtomSets and do the work
                assignResonances = list(resSetResonances)  # copy, must be modified
                for chemAtomOrSet in chemAtomOrSetList:
                    # select resonance for ChemAtomOrSet
                    chemAtomOrSetName = chemAtomOrSet.name
                    if chemAtomOrSetName in preferredAtomNames:
                        # if name in list take out corresponding resonance for processing
                        useIndex = preferredAtomNames.index(chemAtomOrSetName)

                    else:
                        # take first resonance that does *not* fit a prefered name position
                        validIndices = range(len(preferredAtomNames))
                        for caos in chemAtomOrSetList:
                            ss = caos.name
                            if ss in preferredAtomNames:
                                removeIndex = preferredAtomNames.index(ss)
                                validIndices.pop(validIndices.index(removeIndex))
                        ...
                        useIndex = validIndices[0]
                    #
                    reson = assignResonances.pop(useIndex)
                    del preferredAtomNames[useIndex]

                    # Add to preferredAtomNameDict based on covalently bound resonances
                    # Only relevant for 'deep' prochiral case (e.g. Valine methyls)...
                    for covalentResonance in reson.covalentlyBound:
                        # NB from July 2012, FixedResonances have covalentlyBound too
                        ...
                        if covalentResSet:
                            ...
                            for covalentAtomSet in covalentResSet.atomSets:
                                ...
                                if refChemAtom.chemAtomSet:
                                    for refChemBond in refChemAtom.chemBonds:
                                        atSet = set(refChemBond.chemAtoms)
                                        atSet.remove(refChemAtom)
                                        otherRefChemAtom = atSet.pop()

                                        if hasattr(otherRefChemAtom, "chemAtomSet"):
                                            # the 'hasattr' required to weed out LinkAtoms

                                            if chemAtomOrSet is otherRefChemAtom.chemAtomSet:
                                                covalentPreferredAtomName = refChemAtom.name

                                            elif chemAtomOrSet is otherRefChemAtom:
                                                covalentPreferredAtomName = refChemAtom.chemAtomSet.name
                    ...
                    # Set up to get the chemAtom(Set)SysNames
                    tempChemAtomOrSetList = [chemAtomOrSet]
                    # Decide whether to split this list into individual component Atom(Set)s
                    if chemAtomOrSet.className == "ChemAtomSet":
                        splitChemAtomSet = False
                        if individualAtoms:
                            # switch says always split
                            splitChemAtomSet = "deep"
                        elif individualAtomsIfNoSet:
                            if not findChemAtomOrSetSysName(namingSystem, chemAtomOrSet):
                                # Split, as naming system has no name for AtomSet
                                splitChemAtomSet = "shallow"
                        ...
                        if splitChemAtomSet:
                            ...
                            if splitChemAtomSet == "deep" and tempChemAtomOrSetList[0].className == "ChemAtomSet":
                                # split all the way down to individual atoms
                                ii = 0
                                while ii < len(tempChemAtomOrSetList):
                                    caos = tempChemAtomOrSetList[ii]
                                    if caos.className == "ChemAtomSet":
                                        ...
                                        # we have a ChemAtom here. We could probably break,
                                        # but just to be safewe continue till the end
                                        ii += 1

                    # get sysName and set mapping
                    for tempChemAtomOrSet in tempChemAtomOrSetList:
                        atomSysName = findChemAtomOrSetSysName(
                            namingSystem, tempChemAtomOrSet, exportName, findIupac=True
                        )
                        if atomSysName:
                            refSysName = atomSysName.sysName
                            ...
                            refSysName = tempChemAtomOrSet.name
                            print(
                                "  Warning 2: no %s sysNames found for %s (%s)... using CCPN name."
                                % (namingSystemName, tempChemAtomOrSet.name, exportName)
                            )

                        # Finally set mapping
                        r2a = ResonanceToAtoms(
                            reson,
                            residue.chain,
                            residue.seqId,
                            refSysName,
                        ...
                        ll = resonanceToAtoms.get(reson)
                        if ll:
                            ll.append(r2a)
                        else:
                            resonanceToAtoms[reson] = [r2a]
                        # Add otherResonances
                        for otherResonance in resSetResonances:
                            if otherResonance != reson:
                                r2a.addOtherGroupResonance(otherResonance)

                    if not assignResonances:
                        break

            elif residue and len(atomSets) == 1 and len(resSetResonances) == 1:
                # NBNB Added and not tested in situ - RHF 12/11/13
                ...
                exportName = getResName(chain.code, residue.seqCode, atomSet.name or atomSet.findFirstAtom().name)
                ...
                if len(atomSet.atoms) == 1:
                    ...
                    if chemAtomSet:
                        if compressResonances and individualAtomsIfNoSet:
                            chemAtomSetSysName = findChemAtomOrSetSysName(namingSystem, chemAtomSet)
                            if chemAtomSetSysName:
                                chemAtomSetName = chemAtomSetSysName.sysName
                            else:
                                chemAtomSet = None
                            ...
                            chemAtomSetSysName = findChemAtomOrSetSysName(
                                namingSystem, chemAtomSet, exportName, findIupac=True
                            )
                            if chemAtomSetSysName:
                                chemAtomSetName = chemAtomSetSysName.sysName
                            else:
                ...
                elif chemAtomSet and not individualAtoms:
                    # assert chemAtomSet and len(atomSet.atoms) > 1

                    atomSysName = findChemAtomOrSetSysName(namingSystem, chemAtomSet)
                    if atomSysName:
                        refSysName = atomSysName.sysName
                    else:
                        refSysName = chemAtom.name
                    ...
                    for chemAtom in chemAtomSet.chemAtoms:
                        exportName = getResName(chain.code, residue.seqCode, chemAtom.name)
                        atomSysName = findChemAtomOrSetSysName(namingSystem, chemAtom, exportName, findIupac=True)

                        if atomSysName:
                            refSysName = atomSysName.sysName
                        else:
                            print(
                                "  Warning: no %s sysName for %s... using CCPN name." % (namingSystemName, exportName)
                            )
                            refSysName = chemAtom.name
                        ...
                        ll = resonanceToAtoms.get(resonance)
                        if ll:
                            ll.append(r2a)
                        else:
                            resonanceToAtoms[resonance] = [r2a]
                ...
                # Loop over atomSets and do the work
                for atomSet in atomSets:
                    # set up
                    sysNamesLink = []
                    chemAtomSet = None
                    ...
                    #
                    # Check whether sys names for individual atom(s) or for chemAtomSet.
                    #

                    if len(atomSet.atoms) > 1 and not individualAtoms:
                        chemAtomSet = atomSet.findFirstAtom().chemAtom.chemAtomSet
                        chemAtomSetName = chemAtomSet.name
                        if chemAtomSet.chemAtomSet:
                            # If there is a deep chemAtomSet, try to use this.

                            if namingSystem and checkChemAtomsForDeepSet(chemAtomSet.chemAtomSet, atomSets):
                                #
                                # This code might cause some problems for HD*|HE* for aromatics...
                                # Works fine if namingSystem DIANA - will use QR
                                # Never use this name as reference - better to split up!
                                ...
                                if chemAtomSetSysName:
                                    chemAtomSet = None
                                    chemAtomSetName = chemAtomSetSysName.sysName
                                    sysNamesLink = [(chemAtomSetName, chemAtomSet)]
                        ...
                        if not sysNamesLink:
                            ...
                            if atomSysName:
                                ...
                                if chemAtomSet.chemAtomSet and not individualAtomsIfNoSet:
                                    print(
                                        (
                                            "  Warning 3: no %s sysNames for %s ... using CCPN name ."
                    ...
                    if not sysNamesLink:
                        ...
                        for chemAtom in chemAtoms:
                            ...
                            if atomSysName:
                                ...
                                print(
                                    (
                                        "  Warning 4: no %s sysName for %s... using CCPN name."
                                        % (namingSystemName, exportName),
                                        chemAtomSetName,
                                    )
                                )
                        ...
                        if not chemAtomSetName and namingSystem and not individualAtoms and chemAtom.chemAtomSet:
                            chemAtomSetSysName = findChemAtomOrSetSysName(namingSystem, chemAtom.chemAtomSet)
                            if chemAtomSetSysName:
                                chemAtomSetName = chemAtomSetSysName.sysName
                ...
                for reson in resSetResonances:
                    for otherResonance in resSetResonances:
                        if otherResonance != reson:
                            resonanceToAtoms[reson][-1].addOtherLinkedResonance(otherResonance)

        else:
            # If there is no resonanceSet for the resonance, then ignore it
            print("  Warning: No assignment for resonance %s (%s) - ignored" % (resonance.serial, resonance.name))
    ...
    # printList = []
    # tag = 'resonanceSerial'
    # for res in sorted(resonanceToAtoms, key=operator.attrgetter(tag)):
    #  ll = [(rta.chain and rta.chain.code, rta.seqId, rta.atomName, rta.atomSetName)
    #        for rta in resonanceToAtoms[res]]
    #  ll2 = [ll, getattr(res,tag), res.name, len(ll)]
    #  printList.append(ll2)

    # printList.sort()

    # for ll in printList:
    #  print   '+++', ll[1], ll[2], ll[3], ['%s.%s.%s.%s' % tt for tt in ll[0]]
