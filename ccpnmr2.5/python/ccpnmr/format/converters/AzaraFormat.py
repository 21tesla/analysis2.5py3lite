...
from memops.universal.Util import (
    getPowerTwoExp,
    getUpperPowerTwo,
    returnInt,  # , isBigEndian
)
...
IOkeywords["writeProcScript"]["parFile"] = (
    None,
    False,
    "The file the processing script will use for writing spectrum parameters to.",
)
    ...
    def createProcScriptFile(self):
        ...
        if not self.outputFile:
            ...
            self.messageReporter.showError(
                "Error", "No output file given for %s processing script" % (self.format), parent=self.guiParent
            )

        self.procScriptFile = self.processingScriptIO.AzaraProcessingScriptFile(
            self.fileName, self.inputFile, self.outputFile, self.parFile, self.numDim
        )
    ...
    def setProcParsSpecific(self):
        ...
        if self.expOrigin == "varian":
            if self.numDim == 3:
                ...
                self.messageReporter.showError(
                    "Error", "only up to 3D experiment provided for azara Varian processing", parent=self.guiParent
                )
                return False
    ...
    def setProcScriptSpecific(self):
        ...
        for dataDimRef in self.dataDimRefs:
            ...
            values = [
                ...
                "fft",
                "",
                "phase %.1f %.1f" % (freqDataDim.phase0, freqDataDim.phase1),
                "",
            ]
            ...
            for i in range(0, len(values)):
                value = values[i]
                self.procScriptFile.dataValues[expDimIndex].append(
                    self.processingScriptIO.AzaraProcessingScript(self.procScriptFile, i, value)
                )
            ...
            ratioPowerTwo = (freqDataDim.numPoints * freqComplexFactor) / getUpperPowerTwo(
                fidDataDim.numPointsValid * fidComplexFactor
            )
        ...
        if not self.rawData.isNormalStorage:
            ...
            if sensEnhanced:
                if method == "echoAntiEcho":
                    ...
                    self.procScriptFile.setDataValue(
                        expDimIndex, "Rescale", "exchange                 ! Echo-antiecho type data"
                    )
                    self.procScriptFile.setDataValue(
                        acquisitionDimIndex,
                        "Rescale",
                        "scale2 1 %-d 0 1         ! Echo-antiecho type data" % acquisitionDim.numPointsValid,
                    )
                    ...
                    self.procScriptFile.setDataValue(
                        expDimIndex,
                        "Rescale",
                        "scale2 1 %-d 0 1         ! Rance-Kay type data" % fidDataDim.numPointsValid,
                    )
                    ...
                    for tempDataDimRef in self.dataDimRefs:
                        ...
                        if tempExpDimIndex != expDimIndex and tempExpDimIndex != acquisitionDimIndex:
                            ...
                            self.procScriptFile.setDataValue(
                                tempExpDimIndex,
                                "Rescale",
                                "scale2 1 %-d -1 0         ! Rance-Kay type data" % tempFidDim.numPointsValid,
                            )
            ...
            self.procScriptFile.setDataValue(
                acquisitionDimIndex,
                "Rescale",
                "scale2 1 %-d 0 1         ! %s type data" % (acquisitionDim.numPointsValid, mode),
            )
        ...
        if dqdData:
            self.procScriptFile.setDataValue(
                acquisitionDimIndex, "BrukerAvance", "avance " + dqdData["DSPFVS"] + " " + dqdData["DECIM"]
            )
