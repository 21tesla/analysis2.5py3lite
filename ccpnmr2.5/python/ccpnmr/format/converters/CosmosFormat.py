...
IOkeywords["writeProject"]["projectType"] = (
    "COD",
    True,
    "The type of project file. Use COO for coordinates/shifts, COD for constraints/shifts.",
)
...
class CosmosFormat(DataFormat):
    ...
    def createFullProject(self, fileName, models=None, shiftList=None, constraintList=None, projectType="COO"):
        ...
        if self.projectType == "COO":
            self.writeCoordinates(
                fileName, structures=models, noWrite=True, resetIOkeywords=False, useCcpnChainInfo=True
            )
            self.file.coordinateFile = self.coordinateFile

        if self.projectType == "COD" and constraintList:
            self.writeDistanceConstraints(
                fileName,
                constraintList=constraintList,
                noWrite=True,
                resetIOkeywords=False,
                resetMapping=False,
                individualAtoms=True,
                compressResonances=False,
                useCcpnChainInfo=True,
                retainOriginalCompression=False,
            )
    ...
    def setRawCoordinate(self):
        ...
        modelCoordinate = self.coordinatesIO.CosmosCoordinate(
        ...
        if self.chain not in self.atomToCoordinateSerial:
            self.atomToCoordinateSerial[self.chain] = {}
        if self.residue not in self.atomToCoordinateSerial[self.chain]:
            self.atomToCoordinateSerial[self.chain][self.residue] = {}
        if self.atom not in self.atomToCoordinateSerial[self.chain][self.residue]:
    ...
    def setChemShiftFileValue(self):
        ...
        if self.atomName in self.atomNamesDict.keys():
            ...
            if (
                hasattr(self, "atomToCoordinateSerial")
                and self.chain in self.atomToCoordinateSerial
                and self.residue in self.atomToCoordinateSerial[self.chain]
                and atom in self.atomToCoordinateSerial[self.chain][self.residue]
            ):
            ...
            self.measurementFileValues.append(self.rawMeasurementClass(measurement.value, atomSerial=coordAtomSerial))

            self.measurementFileValues[-1].addAssignment(
                self.atomName, residueLabel.upper(), resonanceToAtom.chain.code, resonanceToAtom.seqId
            )
            ...
            if self.projectType == "COD":
                for resonance in (
                    ...
                    + resonanceToAtom.otherLinkedResonances
                    + resonanceToAtom.otherGroupResonances
                ):
                    for otherResonanceToAtom in self.atomMeasurements.keys():
                        ...
                        if resonance == otherResonanceToAtom.resonance:
                            ...
                            self.measurementFileValues[-1].addAssignment(
                                otherAtomName,
                                self.getResidueLabelThreeLetter(otherResonanceToAtom).upper(),
                                otherResonanceToAtom.chain.code,
    ...
    def setRawDistanceConstraintItemMembers(self):
        ...
        for i in range(2):
            ...
            self.rawConstraintItem.members.append(
                self.rawConstraintItemMemberClass(chainCode, residue.seqId, atomName, residue.ccpCode.upper())
            )
