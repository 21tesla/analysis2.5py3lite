...
IOkeywords["readDihedralConstraints"]["validCount"] = (
    None,
    False,
    "The constraint is only considered valid if\n its TALOS count is higher than this number.",
)
IOkeywords["readDihedralConstraints"]["validClasses"] = (
    None,
    False,
    "The constraint is only considered valid if\n its TALOS class is one of the ones selected.",
)
IOkeywords["readDihedralConstraints"]["multiplyDeviation"] = (
    2.0,
    False,
    "The standard angle deviation reported by TALOS will be multiplied\n by this factor to get the dihedral angle constraint limits.",
)
...
class TalosFormat(DataFormat):
    ...
    def setSequenceFileElements(self):
        ...
        self.sequence.elements.append(
            self.sequenceIO.TalosSequenceElement(self.seqCode, self.getResidueCode1Letter(self.residue))
        )
