...
from ccpnmr.format.general.Constants import height_kw, tagSep, volume_kw
...
IOkeywords["readPeaks"]["usePointsInfo"] = (
    False,
    False,
    "If selected, information about the width and peak box will be directly imported as points. Only do this if your spectrum in the data model is exactly the same as the original NMRPIPE one!",
)
IOkeywords["writePeaks"]["useAliasing"] = (
    True,
    False,
    "If deselected, will export the peak chemical shifts at their real unaliased positions (i.e. outside the spectrum limits)",
)
...
class NmrDrawFormat(DataFormat):
        ...
        for self.rawPeak in self.peakFile.peaks[1:]:
            ...
            for rawPeakDimIndex in range(peakNumDims):
                if self.rawPeak.points[rawPeakDimIndex] > highValues[rawPeakDimIndex][0]:
                    highValues[rawPeakDimIndex] = (
                        self.rawPeak.points[rawPeakDimIndex],
                        self.rawPeak.ppm[rawPeakDimIndex],
                    )
                elif self.rawPeak.points[rawPeakDimIndex] < lowValues[rawPeakDimIndex][0]:
                    lowValues[rawPeakDimIndex] = (
                        self.rawPeak.points[rawPeakDimIndex],
                        self.rawPeak.ppm[rawPeakDimIndex],
                    )
    ...
    def setPeakExtras(self):
        ...
        clustIdAppData = Implementation.AppDataInt(
            application=self.format, keyword="clustId", value=self.rawPeak.clustId
        )
        self.peak.addApplicationData(clustIdAppData)

        numPeaksClustAppData = Implementation.AppDataInt(
            application=self.format, keyword="numPeaksClust", value=self.rawPeak.numPeaksClust
        )
    ...
    def setPeakDim(self):
        ...
        if self.usePointsInfo:
            ...
            self.peakDim.valueError = (
                self.pointToPpm[self.rawPeakDimIndex] * self.rawPeak.pointsDev[self.rawPeakDimIndex]
            )
    ...
    def setPeakFilePeakExtras(self):
        ...
        clustIdAppData = self.peak.findFirstApplicationData(application=self.format, keyword="clustId")
        if clustIdAppData:
            self.clustId = clustIdAppData.value

        numPeaksClustAppData = self.peak.findFirstApplicationData(application=self.format, keyword="numPeaksClust")
        if numPeaksClustAppData:
            self.numPeaksClust = numPeaksClustAppData.value
