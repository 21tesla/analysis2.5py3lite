...
from ccpnmr.format.converters.DataFormat import DataFormat, IOkeywords, cleanAtomNameLists
...
IOkeywords["readProject"]["peakFile"] = (
    None,
    False,
    "The peak file in export or storage format associated with this project.",
)
...
class AnsigFormat(DataFormat):
    ...
    def getFullProject(self, fileName, peakFile=None, peakKeyWdList=None, sequenceKeyWds=None):
        ...
        if not peakFile and not self.minimalPrompts:
            getPeakFile = self.messageReporter.showYesNo(
                "Read a peak file", "Do you want to read in an associated peak file (in export or storage format)?"
            )
            ...
            if getPeakFile:
                interaction = self.multiDialog.FileName(
                    ...
                    component="peaks",
                    format=self.format,
                    selectionText="Select peak file",
                    title="Select peak file",
                )
        ...
        if self.file.spectraFile:
            ...
            for spectrumInfo in spectraList:
                ...
                for dim in range(0, len(spectrumInfo["dims"])):
                    ...
                    if "aliased" in spectrumInfo["dimInfo"][dimName]:
                        nucleus = spectrumInfo["dimInfo"][dimName]["nucleus"]
                        if nucleus in self.file.spectraFile.ppmRange:
                ...
                dataSource = azaraFormat.readProcPars(
                    fileName=parFile, expName=specName, aliasing=aliasing, minimalPrompts=self.minimalPrompts
                )
    ...
    def createSequence(self):
        ...
        self.sequenceFile.sequences.append(
            self.sequenceIO.AnsigSequence(molName=self.chain.molecule.name, chainCode=self.exportChainCode)
        )
    ...
    def setSequenceFileElements(self):
        ...
        self.sequence.elements.append(
            self.sequenceIO.AnsigSequenceElement(self.seqCode, self.residue.molResidue.chemComp.ccpCode)
        )
    ...
    def setPeakIntensity(self):
        ...
        if self.rawPeak.volume != 0:
            peakInt = Nmr.PeakIntensity(
                self.peak, value=self.rawPeak.volume, method=self.methods[self.format]["Volume"]
            )
    ...
    def getPeakResNames(self):
        ...
        if self.rawPeak.assignResNum[self.rawPeakDimIndex] != "":
            ...
            self.resNames = [
                getResName(" ", seqCode, self.rawPeak.assignAtom[self.rawPeakDimIndex], seqInsertCode=seqInsertCode)
            ]
    ...
    def setPeakFileInfo(self):
        ...
        if self.peakList.name and not self.peakFile.specNames.count(self.peakList.name):
            ...
            specName = origSpecName = "%s_%d.%d" % (
                self.peakList.dataSource.experiment.name[:8],
                self.peakList.dataSource.serial,
                self.peakList.serial,
            )
            startId = 1
            ...
            specName = self.dataEntry.askString(
                "Enter experiment name",
                "Ansig peak list export: Please enter a name for the peaklist for experiment '%s':" % expName,
                expName,
                self.guiParent,
            )
        ...
        while self.peakFile.specNames.count(specName):
            specName = self.dataEntry.askString(
                "Enter experiment name",
                "Ansig peak list export: name '%s' exists already. Please enter another name:" % specName,
                specName,
                self.guiParent,
            )
            specName = specName[:12]
        ...
        if self.writeAssignments and peakDimContribs:
            ...
            for peakDimContrib in peakDimContribs:
                ...
                if resonance.resonanceSet:
                    code3Letter = (
                        resonance.resonanceSet.findFirstAtomSet().findFirstAtom().residue.molResidue.chemComp.ccpCode
                    )
            ...
            if atomIds:
                ...
                if len(atomIds) > 1:
                    print(
                        "   Warning: Multiple assignment possibilities for peak %d - only first used."
                        % self.peak.serial
                    )
