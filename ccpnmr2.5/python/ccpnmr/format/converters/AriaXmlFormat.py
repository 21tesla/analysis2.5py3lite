...
from ccpnmr.format.general.Constants import ccpNmr_kw, defaultMolCode, isAriaInput_kw
...
class AriaXmlFormat:
    ...
    def tagAriaInput(self, object):
        ...
        if not appData:
            object.addApplicationData(
                Implementation.AppDataBoolean(application=ccpNmr_kw, keyword=isAriaInput_kw, value=True)
            )
    ...
    def ccpNmrConversion(self):
        ...
        ccpNmrConv.setupProject(self.xmlFile.conversion.project.filename, self.xmlFile.conversion.project.name)
        ...
        for molecule in molecules:
            ...
            ccpNmrConv.readSequence(
                molecule.sequence_filename, molecule.name, molecule.type, segID, molecule.first_residue_number
            )
        ...
        for spectrum in spectra:
            ...
            peakLists = ccpNmrConv.readPeakList(
                spectrum.cross_peaks.filename, refExperiment, spectrum.name, peakListDimOrder
            )
    ...
    def checkMandatory(self):
        ...
        subBlocks = {
            "project": ["name", "filename", "naming_system"],
            "molecular_system": [
                "name",
