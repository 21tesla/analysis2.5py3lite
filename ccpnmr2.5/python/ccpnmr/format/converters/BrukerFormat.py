...
IOkeywords["readAcqPars"]["switchRefDimInfo"] = (
    True,
    False,
    "Switch acqu2s/3s info when reading\n(so it ends up as information for a different dimension).",
)
        ...
        if self.acqParsData.acqMode:
            ...
            for par in self.acqParsData.acqModeValues:
                setUniqueAppData(
                    ...
                    self.format,
                    "acqMode" + tagSep + par,
                    str(self.acqParsData.acqModeValues[par]),
                )
        ...
        if self.numDim == 3:
            ...
            if nucleusDim3:
                ...
                if self.matchNucleus(expDim2, nucleusDim3) and self.matchNucleus(expDim3, nucleusDim2):
                    print(
                        "  Warning: switched Bruker data acqu2s/3s info for experiment '%s'!" % (self.experiment.name)
                    )
                    self.switchRefDims()
                    ...
                    print(
                        "  Warning: %s experiment '%s' might have dimension order problems... isotopes do not fit acqu<n>s files"
                        % (self.formatLabel, self.experiment.name)
                    )

                    if self.switchRefDimInfo:
                        self.switchRefDims()
                        print(
                            "  Warning: default switch for Bruker data acqu2s/3s info for experiment '%s'!"
