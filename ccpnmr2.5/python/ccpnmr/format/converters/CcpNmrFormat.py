...
from ccpnmr.format.general.Constants import defaultMolCode, height_kw, volume_kw
...
class CcpNmrFormat(DataFormat):
    def setFormat(self):
        ...
        self.assignPatt = re.compile(r"([A-Za-z]*)(\d+)([A-Z][a-z]*)([^a-z]+)")
    ...
    def setPeakIntensity(self):
        ...
        if self.rawPeak.intensity != 0:
            peakInt = self.peak.newPeakIntensity(
                value=self.rawPeak.intensity, method=self.methods[self.format]["Intensity"]
            )
