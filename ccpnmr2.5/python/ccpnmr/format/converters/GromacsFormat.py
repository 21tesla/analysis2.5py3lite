...
from ccpnmr.format.general.Constants import atomSerial_kw, distanceConstraintDefaultLowerLimit
...
IOkeywords["writeDistanceConstraints"]["atomSerialFormat"] = (
    "gromos",
    False,
    "Format from where the atom serials for constraint output come from.",
)
IOkeywords["writeDistanceConstraints"]["upperLimitStartLinear"] = (
    0.1,
    False,
    "Add this distance (in nm!!) to upperlimit to define point where penalty function becomes linear.",
)
        ...
        for i in range(2):
            ...
            if resonance in self.resonanceToAtomSerial.keys():
                ...
                for resonanceToAtom in resonanceToAtoms:
                    ...
                    if atomSerial == None:
                        print(
                            f"  Error: no atom serial for atom '{atom.residue.chain.code}.{atom.residue.seqCode}.{atom.name}, aborting this constraint.'"
                        )
                        atomSerials = []
                        break
