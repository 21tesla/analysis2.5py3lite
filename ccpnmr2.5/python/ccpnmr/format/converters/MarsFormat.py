...
IOkeywords["readAssignment"]["reliabilityCutoff"] = (
    "H",
    True,
    "The cutoff for including the assignment (H/M/L, with L being the lowest reliability).",
)
IOkeywords["writeMeasurements"]["ignoreFixedConnection"] = (
    False,
    False,
    "Write out a fixed connections file for the spin systems.",
)
...
IOkeywords["writeProject"]["useFixedConnection"] = (
    True,
    False,
    "Write out a fixed connections file for the spin systems.",
)
IOkeywords["writeProject"]["useFixedAssignments"] = (
    True,
    False,
    "Write out a fixed assignments file for the spin systems.",
)
...
class MarsFormat(DataFormat):
  ...
  """
    ...
    def createFullProject(self, fileName):
        ...
        self.writeSequence(
            os.path.join(self.inputDirectory, sequenceFileName), chains=[self.chain], minimalPrompts=self.minimalPrompts
        )
        ...
        self.writeShifts(
            os.path.join(self.inputDirectory, chemShiftFile),
        ...
        if self.useFixedConnection:
            ...
            for spinSystem in self.spinSystemList:
                ...
                if spinSystem not in self.spinSystemToPrevId or not self.spinSystemToPrevId[spinSystem]:
                    continue

                refResonanceToSpinSystem = self.spinSystemToResonanceDict[spinSystem][0]

                serialKey = None
                if 1 in refResonanceToSpinSystem.linkedSpinSystems:
                    serialKey = (spinSystem.serial, refResonanceToSpinSystem.linkedSpinSystems[1].serial)
                elif -1 in refResonanceToSpinSystem.linkedSpinSystems:
                ...
                if spinSystem.residue:
                    ...
                    if spinSystem.residueProbs:
                        ...
                        for residueTypeProb in spinSystem.residueTypeProbs:
                            ...
                            if code1Letter not in resCodeList:
    ...
    def setSequenceFileElements(self):
        ...
        self.sequence.elements.append(
            self.sequenceIO.MarsSequenceElement(self.seqCode, self.getResidueCode1Letter(self.residue))
        )
    ...
    def setSpecificSpinSystemInfo(self):
        ...
        for resonanceToSpinSystem in self.atomMeasurements.keys():
            ...
            if spinSystem not in self.spinSystemToPrevId:
                self.spinSystemToPrevId[spinSystem] = None

            if resonanceToSpinSystem.linkedSpinSystems:
                if -1 in resonanceToSpinSystem.linkedSpinSystems:
                ...
                elif 1 in resonanceToSpinSystem.linkedSpinSystems:
                ...
                if (
                    prevResonanceToSpinSystem
                    and resonanceToSpinSystem
                    and (
                        self.ignoreFixedConnection
                        or (
 1 not in resonanceToSpinSystem.linkedSpinSystems
                            and -1 not in prevResonanceToSpinSystem.linkedSpinSystems
                        )
                    )
                ):
    ...
    def setMeasurementFileGenericValue(self):
        ...
        if self.atomNames in self.atomNamesDict:
            ...
            if resonanceToSpinSystem in self.atomMeasurements:
