...
from ccp.general.Util import findAllSysNamesByChemAtomOrSet, setUniqueAppData
from ccp.lib.nmr.Nmr.DataSource import getXEasyDimCodes
from ccpnmr.format.converters.DataFormat import DataFormat, IOkeywords
from ccpnmr.format.general.Constants import height_kw, peakNum_kw, volume_kw
...
IOkeywords["readShifts"]["readAllSerials"] = (None, False, "Read all atom serials, even if no shifts available.")
...
class XEasyFormat(DataFormat):
    ...
    def getMeasurementsSetFormatSpecificReadKeywds(self):
        ...
        if self.measurementApiCode == "Shift" and self.readAllSerials:
            readKeywds = {"readAll": True}
    ...
    def setSequenceFileElements(self):
        ...
        self.sequence.elements.append(
            self.sequenceIO.XEasySequenceElement(self.seqCode, self.residue.molResidue.chemComp.ccpCode)
        )

    def forceSpecificNamingSystemName(self):

        if (hasattr(self, "cyana21Naming") and self.cyana21Naming) or (
            hasattr(self, "cyanaFormat") and self.cyanaFormat
        ):
    ...
    def createShift(self, resonance, chemShift):
        ...
        resonance.addApplicationData(
            Implementation.AppDataInt(
                application=self.format, keyword=self.atomSerialKeyword, value=chemShift.atomSerial
            )
        )
        ...
        if chemShift.value:
            ...
            shift = self.measurementList.newShift(
                value=chemShift.value, error=chemShift.valueError, resonance=resonance
            )
    ...
    def setPeakIntensity(self):
        ...
        if self.intMethod != "Not integrated" or self.rawPeak.volume != None:
            if self.forceHeight or self.intMethod == "Not integrated":
                # PeakIntensity attributes
                peakInt = self.peak.newPeakIntensity(
                    value=self.rawPeak.volume, method=self.methods["unknown"]["Intensity"]
                )
                ...
                peakInt = self.peak.newPeakIntensity(
                    value=self.rawPeak.volume, method=self.methods[self.format][self.intMethod]
                )
    ...
    def createResonanceNamesDict(self, resonances):
        ...
        if self.resonanceNames == {}:
            self.messageReporter.showWarning(
                "Warning",
                "No atom serial numbers available for xeasy peak assignment linking - assignments cannot be transferred",
                self.guiParent,
            )
    ...
    def peakListAssignmentCheck(self, peakLists):
        ...
        if not self.minimalPrompts and percentageFound < 90:
            continueWriting = self.messageReporter.showYesNo(
                "Write shift list",
                "Only %.1f percent of the resonances has an atom serial - assignments cannot be transferred before a shift list is written. Do you want to continue?"
                % percentageFound,
            )
    ...
    def getAtomSerial(self, resToAtom):
        ...
        if resToAtom:
            ...
            if atomSerialDictKey in self.atomSerialDict:
    ...
    def peakAssignmentsCreate(self):
        ...
        if self.assignCombinations:
            ...
            for assignCombination in self.assignCombinations:
                combi = [
                    assignCombination[self.peakAssignmentsFile.assignmentsDim[0]],
                    assignCombination[self.peakAssignmentsFile.assignmentsDim[1]],
                ]
    ...
    def getResonanceAtomSerial(self, resonance, verbose=1):
        ...
        if not applData:
            if verbose:
                self.addWarning(
                    "No atomSerial for XEasy peak %d. Try writing XEasy shift file first." % self.peak.serial
                )
    ...
    def createPeakFilePeak(self):
        ...
        self.peakFile.peaks.append(
            self.peaksIO.XEasyPeak(
                self.peakNum,
                self.ppm,
                self.colour,
                self.userCode,
    ...
    def setPeakFileInfo(self):
        ...
        dimCodes = getXEasyDimCodes(self.peakList.dataSource)

        self.peakFile.dimCodes = dimCodes
        ...
        for peak in self.peakList.peaks:
            for intensity in peak.findAllPeakIntensities():
                if intensity.intensityType not in intensityTypes:
                    intensityTypes.append(intensity.intensityType)

        if self.integrationMethod not in intensityTypes and len(intensityTypes) == 1:
            print(
                "  Warning: resetting peak integration method to %s - no %s values available."
                % (intensityTypes[0], self.integrationMethod)
            )
    ...
    def orderByAtomNames(self, residueResonanceToAtomList):
        ...
        chemAtomSetSysNames = findAllSysNamesByChemAtomOrSet(
            chemCompVar.chemComp, chemCompVar.chemAtomSets, self.namingSystemName
        )
        ...
        for chemAtomSysName in chemAtomSysNames + chemAtomSetSysNames:
            chemAtomOrSet = chemCompVar.findFirstChemAtom(
                name=chemAtomSysName.atomName, subType=chemAtomSysName.atomSubType
            )

            if not chemAtomOrSet:
                chemAtomOrSet = chemCompVar.findFirstChemAtomSet(
                    name=chemAtomSysName.atomName, subType=chemAtomSysName.atomSubType
                )
            ...
            if not applData:
                if chemAtomSysName.sysName not in ["HN", "O", "H", "O'", "O''"]:
                    print(
                        "  Warning: no XEASY serial number for %s ('%s','%s'), sysName %s"
                        % (self.residue.ccpCode, chemCompVar.linking, chemCompVar.descriptor, chemAtomSysName.sysName)
                    )
                continue
    ...
    def setChemShiftFileValue(self):
        ...
        self.measurementFile.chemShifts.append(
            self.rawMeasurementClass(self.measurementFile, self.atomSerial, value, error, self.atomName, self.seqCode)
        )
