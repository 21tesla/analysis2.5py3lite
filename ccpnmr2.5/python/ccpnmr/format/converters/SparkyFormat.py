...
from ccpnmr.format.converters.DataFormat import DataFormat, IOkeywords, filteredNameInfo
from ccpnmr.format.general.Constants import defaultMolCode, height_kw, peakNum_kw, volume_kw
from ccpnmr.format.general.Util import getNmrIsotope, getResName, getResNameText
...
IOkeywords["writeProject"]["useOriginalResNames"] = (
    False,
    False,
    "If set to True (on), will use the original\n resonance names for export (where possible).",
)
IOkeywords["writeProject"]["useOriginalNumbers"] = (
    True,
    False,
    "If set to False (off), will not use the original\n peak numbers for export (if available).",
)
IOkeywords["writePeaks"]["volumeIntensityType"] = (
    [height_kw, volume_kw],
    False,
    "Values to output to Sparky peak list. Give as list with values 'volume' or 'intensity', or both.",
)
...
class SparkyFormat(DataFormat):
    ...
    def getFullProject(self, fileName, peakKeyWdList=None, sequenceKeyWds=None, shiftList=None):
        ...
        if not self.file.saveFiles and not self.file.sequenceFile and not self.file.chemShiftFiles:
            ...
            if fileName in self.file.peaksLocation:
                fileType = "saveFile"
                self.file.saveFiles.append(fileName)
                print(
                    "  Warning: file is of 'save' type - reading only single spectrum information, no sequences or chemical shifts."
                )
        ...
        for saveFile in self.file.saveFiles:
            if saveFile in self.file.peaksLocation:
        ...
        if not self.ignoreShifts:
            for listSerial in range(len(self.file.chemShiftFiles)):
                ...
                self.readShifts(
                    ...
                    measurementListName="Sparky %s project list %d" % (baseName, listSerial),
                )
    ...
    def createFullProject(self, fileName, peakKeyWdList=None):
        ...
        for csListNum in range(0, len(csLists)):
            ...
            self.writeShifts(
                "",
                measurementList=csLists[csListNum],
                chains=list(self.molSystem.sortedChains()),
                useOriginalResNames=self.useOriginalResNames,
                minimalPrompts=self.minimalPrompts,
                noWrite=True,
            )
        ...
        for experiment in experimentList:
            if experiment in expConditions:
                #
            ...
            for ds in experiment.sortedDataSources():
                ...
                for peakList in peakLists:
                    ...
                    self.writePeaks(
                        ...
                        useOriginalResNames=self.useOriginalResNames,
                        minimalPrompts=self.minimalPrompts,
                        useOriginalNumbers=self.useOriginalNumbers,
                        noWrite=True,
                        **keywds,
                    )
    ...
    def setSequenceFileElements(self):
        ...
        self.sequence.elements.append(
            self.sequenceIO.SparkySequenceElement(self.seqCode, code3Letter=self.residue.molResidue.chemComp.ccpCode)
        )
    ...
    def setPeakIntensity(self):
        ...
        if self.rawPeak.intensity != None and (self.rawPeak.intensity or not self.rawPeak.volume):
            peakInt = Nmr.PeakIntensity(
                self.peak, value=self.rawPeak.intensity, method=self.methods[self.format]["Intensity (data)"]
            )
            peakInt.intensityType = height_kw

        if self.rawPeak.intensityFit != None:
            peakInt = Nmr.PeakIntensity(
                self.peak, value=self.rawPeak.intensityFit, method=self.methods[self.format]["Intensity (fitted)"]
            )
            peakInt.intensityType = height_kw
        ...
        if self.rawPeak.volume != None and (self.rawPeak.volume or not self.rawPeak.intensity):
            ...
            peakInt = Nmr.PeakIntensity(
                self.peak, value=self.rawPeak.volume, method=self.methods[self.format][methodKey]
            )
    ...
    def getPeakResNames(self):
        ...
        if self.rawPeak.assign:
            ...
            if assign != "?":
                ...
                if assignString:
                    self.resLabelCodes = assignString.group(2).split(",")
                ...
                for addAssignString in addAssignStrings:
                    ...
                    resName = getResName(defaultMolCode, assignString.group(2), assignString.group(4))
    ...
    def setPeakFileInfo(self):
        ...
        for peak in self.peakList.peaks:
            self.peakIntDict[peak] = {
                "volume": None,
                "volumeMethod": "Unknown method",
                "intensity": 0.0,
            }  # , 'intensityFit': 0.0}
    ...
    def setPeakFilePeakDimInfo(self):
        ...
        if self.peakDim.decayRate:
            decayRate = (
                self.peakDim.decayRate * self.dataDimRef.valuePerPoint
            )  # TODO: this is NOT correct - shouldn't be stored in points in data model
        else:
            decayRate = None
        ...
        if self.writeAssignments and peakDimContribs:
            ...
            for peakDimContrib in peakDimContribs:
                ...
                if ll is None:
                    print(
                        "  Can't handle resonance %s for peak %d"
                        % (getResNameText(peakDimContrib.resonance), self.peak.serial)
                    )
                else:
            ...
            if resNames:
                ...
                if len(resNames) > 1:
                    print(
                        "  Warning: Multiple assignments currently not supported for %s. Only using first one."
                        % self.format
                    )
    ...
    def setChemShiftFileValue(self):
        ...
        if self.atomName in self.atomNamesDict:
            ...
            values = {
                "residue": [residueLabel + str(self.seqCode)],
                "atom": [self.atomName],
                "atomType": [atomType],
                "chemShift": [str(chemShift.value)],
            }

            self.measurementFileValues.append(self.rawMeasurementClass(self.measurementFile, values))
