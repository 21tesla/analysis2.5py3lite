...
from cing.Libs.html import (
    GOOGLE_ANALYTICS_TEMPLATE,
    GOOGLE_PLUS_ONE_TEMPLATE,
    copyCingHtmlJsAndCssToDirectory,
    getStandardCingRevisionHtml,
)
from cing.Libs.NTgenUtils import *  #@UnusedWildImport
from cing.Libs.NTutils import *  #@UnusedWildImport
from cing.NRG import ARCHIVE_NMR_REDO_ID, ARCHIVE_NRG_ID, ARCHIVE_RECOORD_ID, mapArchive2Base, mapArchive2Schema
from cing.NRG.nrgCingRdb import NrgCingRdb, getPdbIdList
from cing.NRG.PDBEntryLists import (
    getBmrbLinks,
    getBmrbNmrGridEntries,
    getBmrbNmrGridEntriesDOCRDone,
    getEntryListFromCsvFile,
    getPdbEntries,
    writeEntryListToFile,
)
from cing.NRG.runSqlForSchema import runSqlForSchema
from cing.NRG.settings import *  #@UnusedWildImport
from cing.NRG.WhyNot import (
    FAILED_TO_BE_CONVERTED_NRG,
    FAILED_TO_BE_VALIDATED_CING,
    ...
    PROJECT_ID_PDB,
    PROJECT_ID_WATTOS,
    PROJECT_ISSUE_URL_BMRB,
    PROJECT_ISSUE_URL_CCPN,
    PROJECT_ISSUE_URL_CING,
    PROJECT_ISSUE_URL_NRG,
    PROJECT_ISSUE_URL_PDB,
    PROJECT_ISSUE_URL_WATTOS,
    TO_BE_VALIDATED_BY_CING,
    WhyNot,
    WhyNotEntry,
)
...
from cing.Scripts.validateEntry import (
    ARCHIVE_TYPE_BY_CH23,
    ARCHIVE_TYPE_BY_CH23_BY_ENTRY,
    PROJECT_TYPE_CCPN,
    PROJECT_TYPE_CING,
)
from cing.Scripts.vCing.vCing import REFINE_ENTRY_STR, TEST_CING_STR, VALIDATE_ENTRY_NRG_STR, Vcing
...
#                nTdebug("Working on: " + entry_code)
                ...
                if entry_code not in self.entry_list_nmr:
...
#                 Can't remove prep because that gives us no chance to inspect the failure.
                if entry_code not in self.entry_list_prep_done:
...
#                                nTdebug('Ignoring temporarily update available for entry: %s' % entry_code)
                ...
                # end if
                if entry_code not in timeTakenDict:
...
#                # end if
            ...
            if entryCrashed:
                ...
                continue # don't mark it as stopped anymore.
            # end if
            if entry_code not in entry_dict_store_in_db:
...
#                nTdebug("%s removing CCPN directory %s in cwd: %s" % (entry_code, outputCcpnDir, os.getcwd() ))
        ...
        if convertStarCS:
            ...
            if entry_code not in self.matches_many2one:
    ...
    def prepare(self):
        ...
        for entry_code in self.entry_list_todo:
            ...
            if convertMrRestraints:
                convertMmCifCoor = 0
            # end if
            if entry_code in self.matches_many2one:
                convertStarCS = 1
            # end if
