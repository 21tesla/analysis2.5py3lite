...
categoryList = list(NmrExpPrototype.metaPackage.getElement("ExpCategory").enumeration)
constantTimeList = list(NmrExpPrototype.metaPackage.getElement("ConstantTimeType").enumeration)
expMeasurementTypeList = list(NmrExpPrototype.metaPackage.getElement("ExpMeasurementType").enumeration)
expTransferTypeList = list(NmrExpPrototype.metaPackage.getElement("ExpTransferType").enumeration)
namingSystemList = list(NmrExpPrototype.metaPackage.getElement("NamingSystem").enumeration)

isotopeList = ["1H", "13C", "15N", "19F", "31P", "111Cd", "113Cd", "2H", "14N", "17O", "23Na", "77Se", "79Br"]

atomSiteTypeList = ["H", "C", "N", "P", "CA", "CO", "CB", "Cali", "Caro", "Cmet", "H(2)", "H(2n)", "H(2n+1)"]
...
class NmrExpPrototypePopup(BasePopup):
    ...
    *A nomenclature and data model to describe NMR experiments.
    ...
    def body(self, guiFrame):
        ...
        self.selectedPrototypePulldown = PulldownList(guiFrame, tipText=tipText, callback=self.selectPulldownPrototype)
        ...
        self.categoryPulldown = PulldownList(self, texts=categoryList, callback=self.setCategory)
        self.synonymEntry = Entry(self, text="", returnCallback=self.setSynonym, width=12)
        self.keywordsEntry = MultiWidget(self, Entry, minRows=0, callback=self.setKeywords)
        self.detailsEntry = Entry(self, text="", returnCallback=self.setDetails, width=16)

        # frame1
        self.isotopePulldown = PulldownList(self, texts=isotopeList, callback=self.setIsotope)

        self.atomSiteNameEntry = Entry(self, text="", returnCallback=self.setAtomSiteName, width=12)
        self.minShiftEntry = FloatEntry(self, text="", returnCallback=self.setMinShift, width=10)
        self.maxShiftEntry = FloatEntry(self, text="", returnCallback=self.setMaxShift, width=10)
        self.minNumberEntry = IntEntry(self, text="", returnCallback=self.setMinNumber, width=3)
        self.maxNumberEntry = IntEntry(self, text="", returnCallback=self.setMaxNumber, width=3)
        self.numberStepEntry = IntEntry(self, text="", returnCallback=self.setNumberStep, width=3)
        self.expMeasurementTypePulldown = PulldownList(self, callback=self.setMeasurementType)

        self.atomSitesEntry = MultiWidget(self, CheckButton, minRows=1, callback=self.setAtomSites)
        self.atomSiteWeightsEntry = MultiWidget(self, FloatEntry, minRows=0, callback=self.setAtomSiteWeights)
        ...
        tipTexts = [
            ...
            "CCPN systematic name for the experiment prototype. Reflects the magnetisation transfer network",
        ...
        self.prototypeMatrix = ScrolledMatrix(
            frameA,
            multiSelect=False,
        ...
        tipTexts = [
            ...
            "Make a new experiment prototype that is a copy of the currently selected one",
            "Set selected experiment prototype to be no longer modifiable",
            "Delete the selected experiment prototype",
        ]

        commands = [
            self.newNmrExpPrototype,
            self.newCopyNmrExpPrototype,
            self.freezeNmrExpPrototype,
            self.deleteNmrExpPrototype,
        ]

        self.prototypeButtons = ButtonList(frameA, texts=texts, tipTexts=tipTexts, commands=commands)
        ...
        colHeadings = ["#", "Isotope", "Name", "Min\nShift", "Max\nShift", "Min\nNumber", "Max\nNumber", "Number\nStep"]
        editWidgets = [
            None,
            self.isotopePulldown,
            self.atomSiteNameEntry,
            self.minShiftEntry,
            self.maxShiftEntry,
            self.minNumberEntry,
            self.maxNumberEntry,
            self.numberStepEntry,
        ]
        editGetCallbacks = [
            None,
            self.getIsotope,
            self.getAtomSiteName,
            self.getMinShift,
            self.getMaxShift,
            self.getMinNumber,
            self.getMaxNumber,
        ...
        self.atomSiteMatrix = ScrolledMatrix(
            frame1,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        self.newAtomSiteButton = Button(
            frame1, borderwidth=1, text="Create New - Type :", tipText=tipText, command=self.newAtomSite
        )
        self.newAtomSiteButton.grid(row=1, column=0, sticky="ew")

        tipText = "Select atom site type to create"
        self.atomSiteTypePulldown = PulldownList(
            frame1, self.changeAtomSiteType, tipText=tipText, borderwidth=1, relief="raised"
        )

        self.atomSiteTypePulldown.grid(row=1, column=1, sticky="w")

        tipText = "Delete atom site"
        self.deleteAtomSiteButton = Button(frame1, bd=1, text="Delete", tipText=tipText, command=self.deleteAtomSite)
        ...
        tipTexts = [
            ...
            "Relative weights for multiple atom sites. Only for special types, like multiple quantum coherence.",
        ]

        editWidgets = [None, self.expMeasurementTypePulldown, self.atomSitesEntry, self.atomSiteWeightsEntry]

        editGetCallbacks = [None, self.getMeasurementType, self.getAtomSites, self.getAtomSiteWeights]
        editSetCallbacks = [None, self.setMeasurementType, None, None]
        self.expMeasurementMatrix = ScrolledMatrix(
            frame2,
            multiSelect=False,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
        ...
        tipTexts = [
            "Create new experimental measurement, using the currently selected atom site",
        ...
        self.refExperimentFrame = RefExperimentFrame(frameD, self.project, self.nmrExpPrototype, popup=self)
        ...
        self.expGraphFrame = ExpGraphFrame(frameC, self.project, self.nmrExpPrototype, popup=self)
    ...
    def newCopyNmrExpPrototype(self):
        ...
        obj = genUtil.copySubTree(
            ...
            self.project,
            topObjectParameters={"name": "copy_of_" + self.nmrExpPrototype.name, "isModifiable": True},
        )
        menuString = expPrototypeLabelFormat % (obj.serial, obj.name)
    ...
    def newAtomSite(self):
        ...
        dd = atomSiteTypeData.get(self.atomSiteType) or {"isotopeCode": "1H", "name": "H"}
    ...
    def getMeasurementType(self, expMeasurement):
        ...
        if expMeasurement:
            ...
            self.expMeasurementTypePulldown.setup(expMeasurementTypeList, expMeasurementTypeList, ii)
    ...
    def newExpMeasurement(self):
        atomSite = self.atomSiteMatrix.currentObject or self.nmrExpPrototype.findFirstAtomSite()
        obj = self.nmrExpPrototype.newExpMeasurement(measurementType="Shift", atomSites=(atomSite,))
    ...
    def update(self, updateMainTable=True):
        ...
        if updateMainTable:
            ...
            for ii, expPrototype in enumerate(objectList):
                ...
                name = re.sub(r"(.{20}.+)(\s+[|].+)", r"\1\n\2", name)
                synonym = re.sub(r"(.{20}.+)(\s+or.+)", r"\1\n\2", synonym)
        ...
        if expPrototype:
            ...
            for ii, expMeasurement in enumerate(objectList):
                ...
                ll[2] = ", ".join(["%s(%s)" % (x.serial, x.name) for x in expMeasurement.atomSites])
        ...
        self.expMeasurementMatrix.update(objectList=objectList, textMatrix=textMatrix)
...
class ExpGraphFrame(Frame):
    ...
    def __init__(self, parent, project, nmrExpPrototype, popup, *args, **kw):
        ...
        self.peakSignPulldown = PulldownList(self.popup, texts=self.__class__.peakSignNames, callback=self.setPeakSign)

        self.stepNumberEntry = IntEntry(self.popup, text="", returnCallback=self.setStepNumber, width=3)
        ...
        self.expGraphMatrix = ScrolledMatrix(
            frame1,
            editSetCallbacks=editSetCallbacks,
            editGetCallbacks=editGetCallbacks,
            editWidgets=editWidgets,
        ...
        editWidgets = [None, self.stepNumberEntry, self.expMeasurementPulldown]
        ...
        self.expStepMatrix = ScrolledMatrix(
            frame2,
            editSetCallbacks=editSetCallbacks,
            multiSelect=0,
            tipTexts=tipTexts,
        ...
        row += 1
        self.grid_rowconfigure(row, weight=1)
        tipText = "Table of magnetisation transfer connections between atom sites for selected graph"
        ...
        editWidgets = [None, self.expTransferTypePulldown, self.atomSite1Pulldown, self.atomSite2Pulldown, None]
        editGetCallbacks = [None, self.getTransferType, self.getAtomSite1, self.getAtomSite2, self.toggleTransferToSelf]
        editSetCallbacks = [None, self.setTransferType, self.setAtomSite1, self.setAtomSite2, None]
        tipTexts = [
        ...
        self.expTransferMatrix = ScrolledMatrix(
            frame3,
            editSetCallbacks=editSetCallbacks,
            multiSelect=0,
            editGetCallbacks=editGetCallbacks,
        ...
        self.expTransferButtons = ButtonList(frame3, texts=texts, commands=commands, tipTexts=tipTexts)
    ...
    def administerNotifiers(self, notifyFunc):
        ...
        notifyFunc(self.updateAfter, "ccp.nmr.NmrExpPrototype.ExpMeasurement", "delete")
        ...
        for func in ("__init__", "delete", ""):
            for className in (
                "ccp.nmr.NmrExpPrototype.ExpGraph",
                "ccp.nmr.NmrExpPrototype.ExpStep",
                "ccp.nmr.NmrExpPrototype.ExpTransfer",
            ):
                notifyFunc(self.updateAfter, className, func)
    ...
    def getExpMeasurement(self, expStep):
        ...
        entries = ["%s(%s)" % (x.serial, ",".join([y.name for y in x.atomSites])) for x in expMeasurements]
        if entries:
            try:
    ...
    def newExpStep(self):
        ...
        expMeasurement = NmrExpPrototypeUtil.nextFreeExpMeasurement(self.nmrExpPrototype, "expSteps")
    ...
    def newExpTransfer(self):
        ...
        if obj is not None and len(atomSites) > 1:
            obj.newExpTransfer(transferType=expTransferTypeList[0], atomSites=(atomSites[0], atomSites[-1]))
    ...
    def updateButtons(self):
        ...
        if self.nmrExpPrototype and hasattr(self.nmrExpPrototype, "isEditable"):
            ...
            for ii in (0, 1):
                for ll in (self.expGraphButtons.buttons, self.expStepButtons.buttons, self.expTransferButtons.buttons):
                    ll[ii].disable()
    ...
    def updateExpStep(self):
        ...
        for ii, expStep in enumerate(objectList):
            ...
            ll[2] = "%s %s(%s)" % (x.serial, x.measurementType, ",".join([y.name for y in x.atomSites]))
...
class RefExperimentFrame(Frame):
    ...
    def __init__(self, parent, project, nmrExpPrototype, popup, *args, **kw):
        ...
        self.namingSystemPulldown = PulldownList(
            frame0, texts=namingSystemList, tipText=tipText, callback=self.setNamingSystem
        )
        ...
        self.nameEntry = Entry(self.popup, text="", returnCallback=self.setName, width=12)

        self.synonymEntry = Entry(self.popup, text="", returnCallback=self.setSynonym, width=12)

        self.systematicNameEntry = Entry(self.popup, text="", returnCallback=self.setSystematicName, width=16)

        self.numDimsEntry = IntEntry(self.popup, text="", returnCallback=self.setNumDims, width=2)
        ...
        frame1.grid_rowconfigure(0, weight=1)
        frame1.grid_columnconfigure(0, weight=1)
        colHeadings = ["#", "Name", "numDims", "isReversed", "Synonym", "Systematic Name", "isEditable"]
        editWidgets = [None, self.nameEntry, self.numDimsEntry, None, self.synonymEntry, self.systematicNameEntry, None]
        editGetCallbacks = [
            None,
            self.getName,
            self.getNumDims,
            self.toggleIsReversed,
            self.getSynonym,
            self.getSystematicName,
            None,
        ]
        editSetCallbacks = [None, self.setName, self.setNumDims, None, self.setSynonym, self.setSystematicName, None]
        tipTexts = [
            "The serial number of the reference experiment",
            "CCPN systematic name for the reference experiment. Conforms to the Experiment Prototype name.",
            "Number of dimensions of reference experiment",
            "Is magnetisation transfer flow reversed relative to the experiment prototype specification?",
        ...
        self.refExperimentMatrix = ScrolledMatrix(
            frame1,
            editSetCallbacks=editSetCallbacks,
            multiSelect=0,
            editGetCallbacks=editGetCallbacks,
        ...
        texts = ["Create New", "Create Copy", "Freeze", "Delete"]
        commands = [
            self.newRefExperiment,
            self.newCopyRefExperiment,
            self.freezeRefExperiment,
            self.deleteRefExperiment,
        ]
        tipTexts = [
            "Make a new blank reference experiment . Information is added subsequently",
            "Make a new reference experiment that is a copy of the currently selected one",
            "Set selected reference experiment to be no longer modifiable",
            "Delete the selected reference experiment",
        ]
        self.refExperimentButtons = ButtonList(frame1, texts=texts, commands=commands, tipTexts=tipTexts)
        self.refExperimentButtons.grid(row=1, column=0, sticky="nsew")

        # RefExpDimRef table
        row += 1

        self.constantTimePulldown = PulldownList(self.popup, texts=constantTimeList, callback=self.setConstantTime)

        self.expMeasurementPulldown = PulldownList(self.popup, callback=self.setExpMeasurement)
        self.isotopeEntry = MultiWidget(self.popup, CheckButton, minRows=0, callback=self.setCoupledIsotopes)
        self.expStepEntry = MultiWidget(self.popup, PulldownMenu, minRows=0, callback=self.setExpSteps)
        self.scalingFactorsEntry = MultiWidget(self.popup, FloatEntry, minRows=0, callback=self.setValidScalingFactors)
        self.groupingIdEntry = IntEntry(self.popup, text="1", returnCallback=self.setGroupingId, width=3)
        ...
        editWidgets = [
            None,
            None,
            self.expMeasurementPulldown,
            self.scalingFactorsEntry,
            self.groupingIdEntry,
            self.isotopeEntry,
            self.constantTimePulldown,
            self.expStepEntry,
        ]
        editGetCallbacks = [
            None,
            None,
            self.getExpMeasurement,
            self.getValidScalingFactors,
            self.getGroupingId,
            self.getCoupledIsotopes,
            self.getConstantTime,
            self.getExpSteps,
        ]
        editSetCallbacks = [
            None,
            None,
            self.setExpMeasurement,
            None,
            self.setGroupingId,
            None,
            self.setConstantTime,
            None,
        ]
        tipTexts = [
        ...
        self.refExpDimRefMatrix = ScrolledMatrix(
            frame2,
            editSetCallbacks=editSetCallbacks,
            multiSelect=0,
            editGetCallbacks=editGetCallbacks,
        ...
        commands = [self.newRefExpDim, self.newRefExpDimRef, self.deleteRefExpDim, self.deleteRefExpDimRef]
        tipTexts = [
            "Add new reference experiment dimension",
            "Add new dimension referencing to the same dimension as the referencing currently selected",
        ...
        self.bottomButtons = ButtonList(self, texts=texts, commands=commands, tipTexts=tipTexts, expands=0)
    ...
    def setSystematicName(self, event):
        ...
        obj = self.refExperiment
        if obj:
            curSysNameObj = obj.findFirstSystematicName(namingSystem=self.namingSystem)
            if text and text.strip():
                if curSysNameObj is None:
                    obj.newSystematicName(namingSystem=self.namingSystem, name=text)
                else:
    ...
    def getSystematicName(self, refExperiment):
        ...
        if refExperiment:
            sysNameObj = refExperiment.findFirstSystematicName(namingSystem=self.namingSystem)
    ...
    def newCopyRefExperiment(self):
        ...
        obj = genUtil.copySubTree(
            current, self.nmrExpPrototype, topObjectParameters={"name": "copy_of_" + current.name}
        )
        obj.isEditable = True
        self.refExperimentMatrix.currentObject = self.refExperiment = obj
        self.updateAfter()
    ...
    def getExpSteps(self, refExpDimRef):
        ...
        if refExpDimRef:
            ...
            currentKeys = set((x.expGraph.serial, x.stepNumber) for x in refExpDimRef.expSteps)
            options = []
            values = []
    ...
    def newRefExpDim(self, expMeasurement=None):
        ...
        if expMeasurement is None:
            expMeasurement = NmrExpPrototypeUtil.nextFreeExpMeasurement(self.nmrExpPrototype, "refExpDimRefs")
    ...
    def updateRefExpDimRef(self):
        ...
        if obj:
            ...
            for ii, refExpDimRef in enumerate(objectList):
                ...
                ll[7] = ", ".join([str((x.expGraph.serial, x.stepNumber)) for x in refExpDimRef.expSteps])

        self.refExpDimRefMatrix.update(objectList=objectList, textMatrix=textMatrix)
        #
        self.waiting = False
    ...
    def update(self):
        """Fill table with values, initial or after change"""
        refExperiment = self.refExperiment
        if refExperiment is None or refExperiment.nmrExpPrototype is not self.nmrExpPrototype:
