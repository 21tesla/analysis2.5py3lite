...
class ViewRamachandranFrame(ScrolledCanvas):
    def __init__(
        ...
        **kw,
        ...
        self.parent = parent
        ScrolledCanvas.__init__(self, parent, resizeCallback=self.drawCanvas, width=width, height=height)
    ...
    def drawRamachandran(self, x0, y0, x1, y1):
        ...
        if not matrix:
            ...
            for x in rangeNbins:
                ...
                bx1 = bx0 + binWidth
                row = [
                    coordFunc(matrix[x][y], bx0, y0 + (y * binHeight), bx1, y0 + (y * binHeight) + binHeight)
                    for y in rangeNbins
                ]
    ...
    def updateObjects(self, phiPsiAccList=None, objectList=None, labels=None, colors=None):
        ...
        for i in range(N):
            ...
            if circle:
                ...
                circle = canvas.create_oval(phi - r, psi - r, phi + r, psi + r, fill=color, outline=outline)
    ...
    def recalculate(self, q):
        ...
        for object in newSelect:
            ...
            if (math.fabs(phi - phiMean) > phiThres) or (math.fabs(psi - psiMean) > psiThres):
