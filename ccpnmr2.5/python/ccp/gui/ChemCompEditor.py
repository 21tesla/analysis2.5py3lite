    ...
    try:
        ...
        if setValue and value != origValue:
            if metaAttrOrRole.changeability != ImpConstants.changeable:
                ...
                if checkValid:
                    makeChange = showYesNo(
                        "Confirm change", "This a non-changeable attribute. Really change?", parent=guiParent
                    )
...
class MultiWidget:
    ...
    def createPulldownMenu(self, widgetList, widgetDict=None):
        ...
        self.widget = PulldownMenu(
            self.window,
            entries=self.widgetList,
            callback=self.setFunc,
            width=25,
    ...
    def createScrolledList(self, widgetList, widgetDict):
        ...
        self.widgetList = widgetList
        self.widgetDict = widgetDict
        self.widget = ScrolledListbox(
            self.window, width=50, height=5, selectmode=Tkinter.MULTIPLE, initial_list=self.widgetList
        )
    ...
    def createValueList(self, widgetList):
        ...
        self.widgetDict = {}
        self.widget = ScrolledListbox(
            self.window, width=50, height=5, selectmode=Tkinter.MULTIPLE, initial_list=widgetList, addDeleteButtons=True
        )
...
class CcpnAttributeEditPopup(BasePopup):
    def setupMetaInfo(
        self, metaClass=None, keyNames=None, getRoles=False, ignoreNames=[], orderRoles=[], setDummy=False
    ):
        """
        metaclass:
        ...
        if getRoles:
            for roleName in self.metaClass.roleAllNames:
                ...
                if role.hierarchy not in ["child", "parent"] and role.otherClass.package.name in [
                    "ChemComp",
                    "RefStereochemistry",
                ]:
    ...
    def setValue(self, *extra):
        ...
        setCcpnObjectValue(
            ...
            metaAttrOrRole,
            value,
            self.objectValueEdit["Value"],
            self.updateTables,
            checkParent=False,
            checkValid=self.attributeTable.checkValid,
            attrIndex=attrIndex,
            attrName=attrName,
    ...
    def setupTable(self, master, parDict, headings):
        ...
        for heading in headings:
            ...
            #
                ...
                setFunc = lambda selInd=x, selVal=y, attrName=heading: self.setObjectValue(
                    selInd, selVal, attrName=attrName
                )
                getFunc = lambda object=x, attrName=heading: self.getObjectValue(object, attrName)
                self.objectValueEdit[heading] = PulldownMenu(
                    self, entries=parDict["sysNameList"], callback=setFunc, width=25, do_initial_callback=False
                )
                ...
                getFunc = lambda object=x, attrName=heading: self.getObjectValue(object, attrName)
                self.objectValueEdit[heading] = ScrolledListbox(
                    self, width=50, height=5, selectmode=Tkinter.MULTIPLE, addDeleteButtons=True
                )
    ...
    def setObjectValue(self, *extra, **kywds):
        ...
        if rowObject:
            ...
            setCcpnObjectValue(
                ...
                metaAttr,
                value,
                self.objectValueEdit[attrName],
                self.updateTables,
                checkValid=self.sysNameTable.checkValid,
            )
    ...
    def multiSelectPopup(self, ccpnSelectedObjects, ccpnObjects):
        ...
        if ccpnObjects:
            ...
            tempPopup = MultiSelectionListPopup(
                self, selectionList, selectionDict, selectedItems=selectedItems, text="Select chemCompVars", modal=True
            )
...
class CreateCcpnObject(CcpnAttributeEditPopup):
    def __init__(self, parent, ccpnParent, classType, chemComp=None, fromTable=None, ignoreNames=[], orderRoles=[]):
        ...
        BasePopup.__init__(
            self,
            parent=parent,
            title="Edit chemComp (%s, %s) window" % (self.chemComp.molType, self.chemComp.ccpCode),
            modal=True,
        )
    ...
    def body(self, master):
        ...
        self.setupMetaInfo(
            metaClass=self.classType._metaclass,
            keyNames=[],
            getRoles=True,
            setDummy=True,
            ignoreNames=self.ignoreNames,
            orderRoles=self.orderRoles,
        )
...
class ChemCompEditPopup(CcpnAttributeEditPopup):
    def __init__(self, parent, chemComp, *args, **kw):
        ...
        BasePopup.__init__(
            self, parent=parent, title="Edit chemComp (%s, %s) window" % (chemComp.molType, chemComp.ccpCode), **kw
        )
    ...
    def editChemAtoms(self):
        ...
        if "chemAtomEdit" in self.popups:
    ...
    def editChemAtomSets(self):
        ...
        if "chemAtomSetEdit" in self.popups:
    ...
    def editChemBonds(self):
        ...
        if "chemBondEdit" in self.popups:
    ...
    def editChemAngles(self):
        ...
        if "chemAngleEdit" in self.popups:
    ...
    def editChemTorsions(self):
        ...
        if "chemTorsionEdit" in self.popups:
    ...
    def editStereochem(self):
        ...
        if "stereochemEdit" in self.popups:
    ...
    def editChemCompVar(self):
        ...
        if "chemCompVarEdit" in self.popups:
...
class ChemAtomEditPopup(CcpnAttributeEditPopup):
    ...
    def body(self, master):
        ...
        self.chemAtomListWidget = PulldownMenu(
            frame, entries=self.chemAtomList[self.chemAtomType], callback=self.updateChemAtomList
        )
...
class ChemAtomSetEditPopup(CcpnAttributeEditPopup):
    ...
    def body(self, master):
        ...
        self.chemAtomSetListWidget = PulldownMenu(
            frame, entries=self.chemAtomSetList, callback=self.updateChemAtomSetList
        )
        ...
        self.setupMetaInfo(
            metaClass=self.ccpnObject._metaclass,
            getRoles=True,
            ignoreNames=self.ignoreNames,
            orderRoles=self.orderRoles,
        )
    ...
    def addChemAtomSet(self):
        ...
        CreateCcpnObject(
            self, self.chemComp, ChemComp.ChemAtomSet, ignoreNames=self.ignoreNames, orderRoles=self.orderRoles
        )
        self.setChemAtomSetList()
    ...
    def editChemAtomSetVars(self):
        ...
        if "chemAtomSetVarEdit" in self.popups and self.ccpnObject in self.popups["chemAtomSetVarEdit"]:
            self.popups["chemAtomSetVarEdit"][self.ccpnObject].open()

        else:
            if "chemAtomSetVarEdit" not in self.popups:
                self.popups["chemAtomSetVarEdit"] = {}
            self.popups["chemAtomSetVarEdit"][self.ccpnObject] = ChemAtomSetVarEditPopup(
                self, chemComp=self.parent.ccpnObject
            )
...
class GenericEditPopup(CcpnAttributeEditPopup):
    ...
    def addGeneric(self):
        ...
        CreateCcpnObject(
            self,
            self.chemComp,
            getattr(ChemComp, self.className),
            ignoreNames=self.ignoreNames,
            orderRoles=self.orderRoles,
        )
        self.setGenericList()
...
class ChemCompVarAddPopup(ChemCompVarEditPopup):
    ...
    def body(self, master):
        ...
        self.chemCompVarsWidget = ScrolledListbox(
            frame, width=50, height=5, selectmode=Tkinter.MULTIPLE, initial_list=self.genericList
        )
    ...
    def selectChemCompVars(self):
        ...
        tempPopup = MultiSelectionListPopup(
            self, selectionList, selectionDict, text="Select linking and descriptors", modal=True
        )
                    ...
                    if not linkings or ccv.linking in linkings:
                        ...
                        if not descriptors:
                            ...
                            for descKey, descAtom in descriptors:
                                if descKey in ccvDescriptorDict:
...
class ChemTorsionEditPopup(GenericEditPopup):
    ...
    def addSysName(self):
        ...
        CreateCcpnObject(
            self, self.ccpnObject, ChemComp.ChemTorsionSysName, chemComp=self.chemComp, fromTable=self.sysNameTable
        )
...
class StereochemistryEditPopup(GenericEditPopup):
    ...
    def getSpecificKey(self, stereochem):
        ...
        return "%d:%s" % (stereochem.serial, stereochem.value)
