...
from ccp.general.Io import getCcpCodeFromXmlFile, getChemComp, getDataPath
...
class ChemCompFrame(Frame):
    ...
    def __init__(
        ...
        path=None,
        molTypeEntries=None,
        chemCompEntries=None,
        selectedChemComps=None,
        selectLinking=None,
        *args,
        **kw,
    ):
        ...
        if molTypeEntries is None:
            self.mol_type_buttons = CheckButtons(self, entries=molTypeList, select_callback=self.updateTables)
        ...
        self.display_buttons = CheckButtons(
            self,
            entries=[self.showLocalText, self.showWebText],
        ...
        self.chem_comp_table = ScrolledMatrix(
            self,
        ...
        if chemCompEntries is None:
            self.chem_comp_buttons = CheckButtons(
                self, entries=chemCompList, selected=("ChemComp",), select_callback=self.updateChemCompVarTable
            )
    ...
    def toggleShow(self, chemCompInfo):
        ...
        if key in self.chem_comps_shown:
    ...
    def findChemComps(self):
        ...
        for molType in selected:
            if self.showLocalText in searchTypes:
                #
                # Find the files that are locally available
                #
                # Note Wim: taken out central storage, no point in adding that in (is in overview dictionary).
                # If people make their own chemComps they should add them to the data/ccp/molecule/chemComp directory.
                #

                ccpCodes = []

                localPath = self.project.findFirstRepository(name="userData").url.path

                for ccPath in (self.path, localPath):
                    chemCompFileSearchString = "%s+*.xml" % molType
                    chemCompFileSearchPath = os.path.join(ccPath, "ccp", "molecule", "ChemComp")

                    chemCompFileNameMatches = glob.glob(joinPath(chemCompFileSearchPath, chemCompFileSearchString))
                    ...
                    for chemCompFile in chemCompFileNameMatches:
                        if os.path.isfile(joinPath(chemCompFileSearchPath, chemCompFile)) and (
                            chemCompFile[-4:] == ".xml"
                        ):
                            ccpCodes.append(getCcpCodeFromXmlFile(joinPath(chemCompFileSearchPath, chemCompFile)))
                ...
                for ccpCode in ccpCodes:
                    ...
                    if molType != "other":
                        if ccpCode in chemCompStdDict[molType]:
                        ...
                        if ccpCode in chemCompOverview[molType]:
                            chemCompKey = (molType, ccpCode)
                            chemCompDict = chemCompOverview[molType]
                    ...
                    # Else try to get from local directory- TODO is this obsolete?!
                        ...
                        if not chemComp:
                            chemComp = getChemComp(self.project, molType, ccpCode, download=False)
            ...
            if self.showWebText in searchTypes:
                if molType == "other":
                    ...
                    overviewDict = chemCompOverview[molType]
                else:
    ...
    def updateChemCompTable(self):
        ...
        for molType, ccpCode in self.chemCompInfoList:
            n = n + 1
            isShown = (molType, ccpCode in self.chem_comps_shown)
            chemCompInfo = self.chemCompInfoDict[(molType, ccpCode)]
            chemCompInfos.append(chemCompInfo)
            text = [
                n,
                self.booleanString(isShown),
                molType,
                ccpCode,
                chemCompInfo.code1Letter,
                chemCompInfo.cifCode,
                chemCompInfo.name,
            ]
        ...
        try:
            ...
            for molType, ccpCode in self.chemCompInfoList:
                ...
                if isShown:
                    ...
                    if not chemComp:
                        chemComp = getChemComp(self.project, molType, ccpCode, showError=showError)
    ...
    def showAll(self):
        ...
        if self.chemCompDownload:
            if showYesNo(
                "Show all variants",
                "Are you sure you want to display all variants? This will also download the XML files for all chemComps that are not stored locally!",
            ):
                loadAll = True
        else:
            loadAll = True
