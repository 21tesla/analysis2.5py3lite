...
SOURCE_NAMES = ("ccpnImport", "pdb", "ideal", "euroCarbDb")
        ...
        items = [
            {
                ...
                "tipText": "Reset size and orientation to original values",
            },
            {"kind": "command", "label": "Spin", "command": self.spin, "tipText": "Slowly rotate the structure"},
            {
                "kind": "command",
                "label": "Print to File",
                "command": self.printCanvas,
                "tipText": "Save the graphical display as a PostScript file",
            },
        ]
    ...
    def displayStructure(self, model=0):
        ...
        if self.chemCompVar:
            ...
            for sourceName in SOURCE_NAMES:
                chemCompCoord = project.findFirstChemCompCoord(sourceName=sourceName, molType=molType, ccpCode=ccpCode)
            ...
            if chemCompCoord:
                ...
                chemCompVarCoord = chemCompCoord.findFirstChemCompVarCoord(linking=linking, descriptor=descriptor)
                if not chemCompVarCoord:
                    chemCompVarCoord = chemCompCoord  # just a default

            if not chemCompVarCoord:
                msg = (
                    "Chemical component %s available but no atom coordinates, so structure cannot be drawn"
                    % self.chemCompVar.chemComp.ccpCode
                )
            ...
            for atom1 in self.chemCompVar.chemAtoms:
                ...
                # Now allow this to see what is really in the chemCompCoord
