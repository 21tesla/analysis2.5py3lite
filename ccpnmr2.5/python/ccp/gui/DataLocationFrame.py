...
from memops.universal.Io import converseSplitPath, joinPath, splitPath
...
class DataLocationFrame(Frame):
    def __init__(self, parent, project, utilityButtons=False, *args, **kw):
        ...
        texts = ["Propagate\nAbsolute Path", "Shift Directory\nTo Absolute Path", "Shift Directory\nTo Relative Path"]
        tipTexts = [
        ...
        commands = [self.propagatePath, self.moveDirToAbsolute, self.moveDirToRelative]
        ...
        if utilityButtons:
            if isModal:
                ...
                tipTexts = ["Close the popup, satisfied that paths are OK"] + tipTexts
                commands = [
                    popup.close,
                ] + commands
            self.buttonList = UtilityButtonList(
                self, texts=texts, commands=commands, tipTexts=tipTexts, doClone=False, grid=(1, 0)
            )
            if isModal:
                self.buttonList.buttons[0].config(bg="#B0FFB0")
        else:
            self.buttonList = ButtonList(self, texts=texts, commands=commands, grid=(1, 0), tipTexts=tipTexts)
    ...
    def moveDirToAbsolute(self):
        ...
        for dataStore in dataStores:
            ...
            (head, tail) = converseSplitPath(relPath)

            if head:
                if head[0] in ("/", "."):
                    head = head[1:]
                relPathNew = tail
                absPathNew = os.path.join(absPath, head)
                ...
                if relPathNew != relPath:
                    ...
                    ###  dataUrlStore.path = converseSplitPath(dataUrlStore.path)[1]
    ...
    def update(self):
        ...
        for i, dataStore in enumerate(dataStores):
            if isinstance(dataStore, NumericMatrix):
                tt = [
                    "%s:%s" % (dataSource.experiment.name, dataSource.name) for dataSource in dataStore.nmrDataSources
                ]
            elif isinstance(dataStore, MimeTypeDataStore):
                tt = [
                    "%s:%s:%s"
                    % (
                        dataSourceImage.dataSource.experiment.name,
                        dataSourceImage.dataSource.name,
                        dataSourceImage.serial,
                    )
                    for dataSourceImage in dataStore.nmrDataSourceImages
                ]
            else:
                tt = []
        ...
        self.dataStoreTable.update(textMatrix=textMatrix, objectList=dataStores, colorMatrix=colorMatrix)
