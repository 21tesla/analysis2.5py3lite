...
class MMCIFSequenceFile(MMCIFFile):
    ...
    def read(self, mmCif=None, ignoreResNames=None, version=None, verbose=False):
            ...
            for chainCode in chainCodes:
                        ...
                        for covBondInfo in bondInfo["covale"]:
                            ...
                            if isBonded:
                                (
                                    ...
                                    otherAuthSeqId,
                                    otherAtomName,
                                    otherPdbInsertionCode,
                                ) = covBondInfo[otherIndex]
                ...
                if currentChainInfo["currentSequence"]:
                    ...
                    for i in range(len(seqElInfo)):
                        ...
                        self.setSequenceElement(
                            currentSequence, pdbChainCode, pdbSeqCode, pdbSeqCode, cifResLabel, None, pdbInsertionCode
                        )  # lastSeqCode + i
                ...
                #
                    ...
                    for connectedChainCode in connectedChains:
                        if connectedChainCode not in currentChainInfo["connectedChains"]:
        ...
        for otherChainInfo in otherChains:
            ...
            if not otherChainInfo["currentSequence"]:
                ...
                currentSequence = self.addSequence(
                    otherChainInfo["moleculeName"],
                    chainCodes[0],
                    otherChainInfo["moleculeType"],
                    alternateChainCodes=chainCodes[1:],
                )
        ...
        for mmCifBondType in mmCifBondTypes:
            ...
            for covBondInfo in bondInfo[mmCifBondType]:
                seqEls = []
                for k in range(2):
                    (cifChainCode, cifResLabel, cifSeqId, authChainCode, authSeqId, atomName, pdbInsertionCode) = (
                        covBondInfo[k]
                    )

                    seqKey = (authChainCode, authSeqId, cifResLabel)

                    if seqKey in self.seqElDict:
                        seqEl = (self.seqElDict[seqKey], atomName)
                    else:
                        print(seqKey, self.seqElDict.keys())
                        print(
                            "  Error: could not find sequence element for '%s.%s.%s' to set bond..."
                            % (cifChainCode, cifSeqId, atomName)
                        )
                        seqEl = None
    ...
    def addSequence(self, moleculeName, chainCode, moleculeType, alternateChainCodes=None):
        ...
        if chainCode in self.chainCodeToSequence.keys():
            print(
                "  Warning: trying alternate chain code for molecule %s - '%s' already exists..."
                % (moleculeName, chainCode)
            )

            if alternateChainCodes:
                for tmpChainCode in alternateChainCodes:
                    if tmpChainCode not in self.chainCodeToSequence.keys():
    ...
    def setSequenceElement(
        self, currentSequence, pdbChainCode, pdbSeqCode, seqCode, origResLabel, molType, insertionCode
    ):
        ...
        if not molType:
            ...
            if resLabel in self.chemCompInfo:
