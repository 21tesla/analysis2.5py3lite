        ...
        for constraint in self.constraints:
            ...
            for i in range(len(constraint.items)):
                ...
                for member in item.members:
                    ...
                    fout.write(
                        "   %-2s %sresid %-4d %s%s"
                        % ("(" * braces, segIdString, member.seqCode, atomNameString, ")" * braces)
                    )
                ...
                if i == 0:
                    ...
                    if hasattr(constraint, "peakNum"):
                        ...
                        peakString = " peak %5d weight  %8.5E volume  %8.5E" % (
                            constraint.peakNum,
                            weight,
                            constraint.peakVol,
                        )
                    ...
                    elif constraint.weight != None and constraint.weight != 1.0:
                        peakString = " weight  %8.5E" % constraint.weight

                    fout.write(
                        "  %9.3f %9.3f %9.3f%s"
                        % (constraint.targetDist, constraint.minusDist, constraint.plusDist, peakString)
                    )

                    fout.write(self.newline)

                if i != len(constraint.items) - 1:
                    if i != 0 and oneLinePerItem:
                        fout.write(self.newline)
                    ...
                    if len(constraint.items) > 1 and oneLinePerItem:
                        fout.write(self.newline)
