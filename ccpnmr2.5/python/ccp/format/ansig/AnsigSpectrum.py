...
from ccp.format.ansig.AnsigParse import ANSIG_FLOAT, ANSIG_INT, AnsigParseException, parseAnsigFile
from ccp.format.azara.processingParsIO import AzaraProcessingParsFile

ndimRe = re.compile(r"(\d+)D$")
dimRe = re.compile(r"F(\d+)$")
...
class AnsigSpectrum:
    ...
    def readParFile(self, parFile):
        ...
        if aPars["ndim"] != self.ndim:
            raise OSError(
                'spectrum "%s": ansig spd file has ndim=%d but par file "%s" has ndim=%d'
                % (self.name, self.ndim, parFile, aPars["ndim"])
            )
        ...
        # matrix_file attributes
            ...
            if values[0] != ndim:
                raise AnsigParseException(
                    'spectrum "%s": ansig spd file has ndim=%d but also have set ndim=%d'
                    % (spectrum.name, ndim, values[0])
                )
