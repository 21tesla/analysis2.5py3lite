...
def parseAnsigFile(fileName, callback, keywordDict, directoryDict, canDefineDirectory=False):
    ...
    try:
        while contentIndex < ncontent:
            ...
            if canDefineDirectory:
                ...
                if obj:
                    ...
                    dirKey = obj.group(1)
                    if len(values) != 1:
                        raise AnsigParseException(
                            'dir_symbol "%s" has values = %s, must be of length 1' % (key, values)
                        )
            ...
            if blockStart:
                ...
                while len(values) < nreqdValues:
                    ...
                    if contentIndex >= ncontent:
                        raise AnsigParseException(
                            'key "%s" only has %d arguments, requires %d' % (key, len(values), nreqdValues)
                        )
                ...
                if type(nreqdValues) == type(()) or nreqdValues == -1:
                    ...
                    if nreqdValues != -1:
                        if len(values) not in nreqdValues:
                            raise AnsigParseException(
                                'key "%s" has %d arguments, requires one of %s' % (key, len(values), nreqdValues)
                            )
                else:
                    while len(values) < nreqdValues:
                        contentIndex += 1
        ...
        raise OSError('In file "%s", line number %d: %s:\n  %s' % (fileName, lineNumber, str(e), line))
