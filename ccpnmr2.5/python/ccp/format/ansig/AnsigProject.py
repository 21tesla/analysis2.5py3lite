...
from ccp.format.ansig.AnsigParse import AnsigParseException, parseAnsigFile
...
def readProjectFile(projectFile, verbose=False):
    ...
    directoryDict = {}
    try:
        parseAnsigFile(projectFile, callback, keywordDict, directoryDict, canDefineDirectory=True)
        if not project.foundControl:
            raise OSError('no "control" statement found in file "%s"' % projectFile)
    finally:
