...
def parseProgramName(fname, parseInfo, specialCases=()):
    ...
    if not result["names"]:
        ...
        unused, endnames, suffix = reverseParseEndNames(parseInfo["endnames"], fname)
        result["unused"] = unused
        result["endnames"] = endnames
        result["suffix"] = suffix
...
def convertParseInfo(parseInfo):
    ...
    useTags = ["skipNamesEnding", "skipNamesStarting", "prefixes", "midnames", "endnames"]
    ...
    result["startnames"] = [dd["namesStartWith"][0] for dd in parseInfo["expNameMaps"]]
    #
    return result
