...
from memops.universal.Util import getUpperPowerTwo, returnFloat, returnFloats, returnInt
    ...
    def getSpecRefs(self, pars, nullValue, specTypeRef=None):
        ...
        for refs in self.refList:
            ...
            if noValue == 0:
                ...
                if nucleus in specRefs > 0:
                    print(
                        "  ERROR: %s has two transmitters channels with nucleus %s. Second one ignored."
                        % (self.fileDir, nucleus)
                    )
                    continue
                ...
                if int(baseFreq / 10) != int(specFreq / 10):
                    print(
                        "  Error: Exp %s bad referencing: baseFreq %.3f, specFreq %.3f. Reset specFreq to baseFreq"
                        % (self.fileDir, baseFreq, specFreq)
                    )
                    specFreq = baseFreq
