                ...
                if cols[0] == self.tolerancesTag:
                    ...
                    while not self.patt["onlyDollar"].search(line):
                        ...
                        if phaseSearchObj:
                            ...
                            peakFile.read()
                            peakFile.setProjectSpectrumInfo(
                                specName, rootSpec, intra, sequential, throughSpace, phase, dimInfo
                            )
    ...
    def write(self, verbose=0):
        ...
        for peakFile in self.peakFiles:
            ...
            for atomInfo, lowPpm, highPpm, correction, tolerance, foldStatus in peakFile.dimInfo:
                ...
                fout.write(
                    "{%-10s %6.2f %6.2f  %s   %.2f  %s }"
                    % (atomString, lowPpm, highPpm, correction, tolerance, foldStatus)
                    + self.newline
                )
