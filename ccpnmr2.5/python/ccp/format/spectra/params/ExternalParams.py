...
# per-dimension parameters handled outside normal mechanisms
extraDimParams = ["displayNames", "scalingFactors"]
...
class ExternalParams:
    ...
    def __init__(self, externalParams=None, extraData=None):

        if extraData:
            self.extraData = extraData.copy()
        else:
            self.extraData = {}
    ...
    def setAttrs(self, aPars):
        ...
        for key in topParamDefs.keys():
            if key in aPars:
                setattr(self, key, aPars[key])
        for key in dimParamDefs.keys():
            if key in aPars:
                ll = aPars[key]
                getattr(self, key)[: len(ll)] = ll
    ...
    def fixNullDims(self, ignoreDim=None):

        ndim = self.ndim
        for i in range(ndim - 1, -1, -1):
            if i is ignoreDim:
                continue
            if self.nuc[i] is None or self.npts[i] <= 1:
                if self.npts[i] <= 1:
                    del self.npts[i]
                    del self.block[i]
                    del self.refppm[i]
    ...
    def createDataSource(self, experiment, name, dataUrl=None):
        ...
        for n in range(self.ndim):
            ...
            if self.dimType[n] != freqDimType:
                if self.dimType[n] == fidDimType:
                    raise ApiError("dimension %s is FID - not yet supported." % (n + 1))
                else:
                    self.nuc[n] = None
            ...
            if self.dimType[n] == freqDimType:
                if expDim.expDimRefs:
                    if len(expDim.expDimRefs) == 1:
                        ...
                        if self.nuc[n]:
                            expDimRef = None
                            expDimRefs = [x for x in expDim.sortedExpDimRefs() if x.isotopeCodes == (self.nuc[n],)]
                            ...
                            if expDimRef is None:
                                if expDimRefs:
                                    ...
                                    print(
                                        (
                                            "Warning: in dim %d no ExpDimRef fits sf %s" % (n + 1, self.nuc[n]),
                                            expDimRef.sf,
                                            self.sf[n],
                                        )
                                    )
        ...
        if self.dataFile:
            ...
            newDataUrl, filePath = getDataStoringFromFilepath(
                memopsRoot, self.dataFile, preferDataUrls=preferDataUrls, keepDirectories=self.keepDirectories
            )

            if dataUrl is not None and newDataUrl is not dataUrl:
                raise ApiError("DataUrl with path %s not valid for file %s" % (dataUrl.url.path, self.dataFile))
            ...
            isComplex = [self.dimParams[ii].get("isComplex") for ii in range(self.ndim)]
            ...
            if self.format == "Factorised":
                for tag in ("numShapes", "isReconstructable", "isResolved", "numRecords"):
                ...
                dataStore = dataLocationStore.newBlockedBinaryMatrix(blockSizes=blockSizes, **params)
        ...
        if (
            self.nAxes > ndims
            and self.dimType[-1] == freqDimType
        ...
        hasNoAcqDim = expDims[0].experiment.findFirstExpDim(isAcquisition=True) is None
                ...
                if not lenExpDimRefs or (n >= self.ndim + lenExpDimRefs - 1):
                    ...
                    if self.nuc[n]:
                        xdr = expDim.newExpDimRef(
                            sf=self.sf[n], unit="ppm", baseFrequency=self.baseFreq[n], isotopeCodes=[self.nuc[n]]
                        )
                    else:
                        xdr = expDim.newExpDimRef(
                            sf=self.sf[n],
        ...
        dataSource = experiment.newDataSource(
            name=name, numDim=self.ndim, dataStore=dataStore, dataType="processed", scale=self.scale
        )
        scalingFactors = None
        ...
        for n in range(self.ndim):
            ...
            if self.dimType[n] == freqDimType:
                ...
                expDimRef = expDim.findFirstExpDimRef()
                ddr = freqDataDim.newDataDimRef(refPoint=self.refpt[n], refValue=self.refppm[n], expDimRef=expDimRef)
                ...
                if scalingFactors is not None:
                    ...
                    previousDimScaling = freqDataDim.findFirstDimensionScaling(expDimRef=expDimRef)
                    if previousDimScaling is None:
                        freqDataDim.newDimensionScaling(expDimRef=expDimRef, scalingFactors=(scalingFactors[n],))
                    else:
                ...
                dataSource.newSampledDataDim(
                    dim=n + 1, numPoints=npts, isComplex=False, pointValues=self.pointValues[n], expDim=expDim
                )
            else:
        ...
        for n in range(self.ndim, ndims):
            ...
            ddr = freqDataDim.newDataDimRef(
                refPoint=self.refpt[n],
