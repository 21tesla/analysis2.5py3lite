...
class NmrViewParams(ExternalParams):
    ...
    byteOrderFlags = {("34", "18", "AB", "CD"): "big", ("CD", "AB", "18", "34"): "little"}
    ...
    def fullSanityCheck(self):
        ...
        if fileSize < expectedSize:
            raise ApiError(
                "nmrview file %s\n is not the correct size (%d bytes; expected %d bytes)"
                % (self.dataFile, fileSize, expectedSize)
            )
    ...
    def parseFile(self):
        ...
        if len(s) < self.head:
            raise ApiError(
                "nmrview file %s\n is shorter than the expected length (%d bytes) of its header"
                % (self.dataFile, self.head)
            )
    ...
    def checkMagic(self, bytes):
        ...
        if inMagicBytes != self.magicBytes:
            ...
            raise ApiError(
                "nmrview file %s\n does not start with the expected magic bytes (%s; in any order) got %s instead"
                % (self.dataFile, magicBytesString, inMagicBytesString)
            )
    ...
    def guessConsistentNuclei(self):
        ...
        try:
            ...
            else:  # len(sfs) <= 1: # just assume all 1H
