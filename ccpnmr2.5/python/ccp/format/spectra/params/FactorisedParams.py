...
class FactorisedParams(ExternalParams):
    ...
    def parseFile(self):
        ...
        if topNode.tag == "Decomposition":
            ...
            if decompositions:
                if len(decompositions) > 1:
                    print(
                        "WARNING, found %s Decompositions in file %s\n Using first one"
        ...
        self.topParams["isReconstructable"] = decomposition.get("reconstructable") in ("True", "true")
        self.topParams["isResolved"] = decomposition.get("resolved") in ("True", "true")
        ...
        for elem in decomposition.findall("Component"):
            ...
            index = elem.get("c")
            if index is None:
                raise ApiError("Incorrect component, lacks 'c' attribute: <Component %s >" % elem.attrib)

            ampl = elem.get("ampl")
            if ampl is None:
                raise ApiError("Incorrect component, lacks 'ampl' attribute: <Component %s >" % elem.attrib)
            else:
    ...
    def parsePeaks(self, dataSource, software=None):
        ...
        if nshapes != dataSource.numDim:
            ...
            heightMethod = getMethod(software, task="find peak height", procedure="Factorised dataSource import")
                    ...
                    for ii, ee2 in enumerate(elem):
                        ...
                        for index in range(nPeaks):
                            ...
                            for ii in range(nshapes):
                                ...
                                ll = [(x.expDimRef.serial, x) for x in peakDim.dataDim.dataDimRefs]
                                if ll:
                                    ll.sort()
                            ...
                            peak.newPeakIntensity(intensityType="height", value=height, method=heightMethod)
            if compFound != ncomp:
