    ...
    def read(self, verbose=0, cyanaFormat=False, cyanaTextAssignments=False):
        ...
        if not n:
            if line[0] == "#":
                ...
                n = len(dimCodes)
                print(
                    "  Warning: irregular format, extracting dimensions from %s string on first first line." % dimCodes
                )
        ...
        while line:
            if not self.patt["hash"].search(line) and not self.patt["emptyline"].search(line):
                ...
                if (
                    len(peak_el) > n
                    and (peak_el[0] != "0" or peak_el[1] != "0.000")
                    and (not peaknum or peaknum != peak_el[0])
                ):
                    ...
                    if None not in peak.ppm + peak.assign + [peak.num, peak.volume, peak.volumeError, peak.colour]:
