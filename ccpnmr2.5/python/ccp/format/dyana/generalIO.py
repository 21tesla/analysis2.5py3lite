...
from ccp.format.general.Constants import defaultLowerDist, defaultMolCode
...
class DyanaGenericFile(FormatFile):
    def setGeneric(self):
        ...
        self.linkerResidueCodes = ["PL", "LL", "LL2", "LL5", "LP"]
        self.chainCodesString = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
class DyanaGenericDistanceConstraintFile(DyanaGenericFile):
    ...
    def readGenericFile(self, fileName, upper=1):
        ...
        while line:
            if self.patt["hash"].search(line) or self.patt["emptyline"].search(
                line
            ):  # TODO: is this correct for DYANA comments?
                line = fin.readline()
                continue
            ...
            if len(cols) == 2:
                ...
                if seqCode1.isdigit() and seqCode2.isdigit():
    ...
    def setPreviousDistanceConstraint(self, itemList, upper, commentList):
        ...
        if upper:
            ...
            if commentList and commentList[0]:
                for setFuncName, commentString in [
                    ("setPeakNum", "#peak"),
                    ("setPeakNum", "#PEAK"),
                    ("setQualityFactor", "#QF"),
                ]:
            ...
            for itemIndex in range(0, len(itemList)):
                ...
                for itemMember in item:
                    constraint.items[-1].members.append(
                        DyanaConstraintMember(self.chainCode, itemMember[0], itemMember[1], itemMember[2])
                    )
