...
from ccp.format.dyana.generalIO import DyanaConstraintItem, DyanaConstraintMember, DyanaGenericFile
            ...
            if line.count("Orientation") and line.count("Rhombicity"):
                ...
                while 1:
                    ...
                    if self.patt["hash"].search(coordFrameInfoLine):
                        ...
                        self.orientations[returnInt(orientation)] = (
                            returnFloat(magnitude),
                            returnFloat(rhombicity),
                            returnInt(frameResidueNumber),
                        )
    ...
    def write(self, singleAtom=False, verbose=False):
        ...
        if not singleAtom:
            ...
            for orientationKey in orientationKeys:
                ...
                fout.write(
                    "       %d       %8.3f    %8.3f          %4d"
                    % (orientationKey, magnitude, rhombicity, frameResidueNumber)
                )
        ...
        for constraint in self.constraints:
            if singleAtom:
                ...
                if not resLabel:
                    ...
                    if protonName:
                        fout.write("%4d %-4s %-5s %-7.2f" % (seqCode, resLabel, protonName, constraint.value))
