...
from ccp.format.general.Constants import bioPolymerCodes
...
class Mol2SequenceFile(Mol2GenericFile):
    ...
    def read(self, mol2File=None, verbose=0):
        ...
        if "molComment" in self.infoDict["MOLECULE"]:
        ...
        if totalAtoms != self.infoDict["MOLECULE"]["numAtoms"]:
            print(
                "  Warning: possible atom numbers mismatch. %d reported, %d counted."
                % (self.infoDict["MOLECULE"]["numAtoms"], totalAtoms)
            )
        ...
        for substInfo in self.infoDict["SUBSTRUCTURE"]:
            if "chain" in substInfo:
                chainId = substInfo["chain"]
            else:
                chainId = self.defaultMolCode

            if chainId not in chainIds.keys():
                chainIds[chainId] = []
        ...
        for chainId in chainIdList:
            ...
            for i in range(totalBonds):
                ...
                if substs[0] != substs[1]:
                    substs[0].setBond(
                        bondInfo["bondType"],
                        allSubstAtoms[substs[0]][atomId1]["atomName"],
                        substs[1],
                        allSubstAtoms[substs[1]][atomId2]["atomName"],
                    )
                    linkPair = [substs[0].seqCode, substs[1].seqCode]
                    linkPair.sort()
                    if linkPair not in circularInfo:
