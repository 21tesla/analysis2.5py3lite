...
class Mol2ChemCompFile(Mol2GenericFile):
    ...
    def read(self, mol2File=None, verbose=0):
        ...
        if "SUBSTRUCTURE" in self.infoDict:
        ...
        for substInfo in substInfoList:
            ...
            if not substAtoms:
                for attrType in ("chain", "chainSubType"):
                    ...
                    if verbose:
                        print(
                            "  Warning: no atoms found for substructure name '%s' - trying %s name '%s'"
                            % (substName, attrType, attrName)
                        )
            ...
            if onlyNameMatch:
                print(
                    "  Warning: %s substructure %s appears with different atom names and/or bond types."
                    % (self.format, substName)
                )
            ...
            if not chemComp:
                ...
                for atomInfo in substAtoms:
                    self.chemComps[-1].atoms.append(
                        Mol2Atom(self.chemComps[-1], atomInfo["atomId"], atomInfo["atomName"], atomInfo)
                    )

                for bondInfo in substBonds:
                    if "bondStatus" in bondInfo:
                    ...
                    self.chemComps[-1].bonds.append(
                        Mol2Bond(
                            self.chemComps[-1], bondInfo["atom1Id"], bondInfo["atom2Id"], bondInfo["bondType"], bondInfo
                        )
                    )
    ...
    def write(self, verbose=0):
        ...
        if chemComp.overallCharge != None:
            fout.write("#\n")
            fout.write("# Overall charge: %d\n" % chemComp.overallCharge)
            fout.write("#\n")
        ...
        for atom in chemComp.atoms:
            fout.write(
                "%7d %-8s %9.4f %9.4f %9.4f %-4s %6d %-7s %9.4f %s\n"
                % (
                    atom.serial,
                    atom.name,
                    atom.x,
        ...
        for i in range(0, len(chemComp.bonds)):
            ...
            fout.write("%5d %5d %5d %s\n" % ((i + 1), bond.atomSerial1, bond.atomSerial2, bond.bondType))


class Mol2ChemComp:
    def __init__(self, parent, name, atomNames, bondTypes, substInfo, molType="SMALL", overallCharge=None):
        ...
        self.overallCharge = overallCharge
