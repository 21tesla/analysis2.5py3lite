...
from ccp.format.sparky.chemShiftsIO import SparkyChemShift, SparkyChemShiftFile
from ccp.format.sparky.generalIO import SparkyGenericFile

#####################
# Class definitions #
#####################
from ccp.format.sparky.peaksIO import SparkyPeak, SparkyPeakFile
from ccp.format.sparky.sequenceIO import SparkySequence, SparkySequenceElement, SparkySequenceFile
...
class SparkyProjectFile(SparkyGenericFile):
    ...
    def initialize(self):
        ...
        self.refData = [
            ...
            ["user", []],
            ["spectrum", [["attached data", []], ["view", [["params", []]]], ["ornament", []]]],
        ]
                    ...
                    if bracketName not in locationSubs:
                        ...
                        if location == ["spectrum"]:
                            ...
                            self.chemShiftLocation[self.molNames[-1]][
                                self.moleculeConditions[self.molNames[-1]][-1]
                            ] = chemShiftFile
                ...
                if location == ["savefiles"]:
                    ...
                    if cols[0] == "sequence":
                        ...
                        for i in range(0, len(code1LetterSequence)):
                            ...
                            self.sequenceLocation[self.molNames[-1]].elements.append(
                                SparkySequenceElement(i + 1, code1Letter=code1Letter)
                            )
    ...
    def write(self, verbose=0):
        ...
        for molName in self.molNames:
            ...
            if molName in self.sequenceLocation:
                if self.sequenceLocation[molName].elements:
                    ...
                    fout.write(
                        "first_residue_number %d" % self.sequenceLocation[molName].elements[0].seqCode + self.newline
                    )
            ...
            for conditionName in self.moleculeConditions[molName]:
                ...
                if molName in self.chemShiftLocation:
                    if conditionName in self.chemShiftLocation[molName]:
                        ...
                        for chemShift in chemShiftFile.chemShifts:
                            ...
                            resonanceCode = self.getExportAssignCode(
                                chemShift.resLabel, chemShift.seqCode, chemShift.atomName
                            )
        ...
        for saveFile in self.saveFiles:
            ...
            for i in range(0, len(self.peaksLocation[saveFile])):
                ...
                if peakFile in self.peaksCondition:
