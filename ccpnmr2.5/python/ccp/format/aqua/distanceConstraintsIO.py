...
from ccp.format.aqua.generalIO import AquaConstraintItem, AquaConstraintMember, AquaGenericFile
    ...
    def read(self, verbose=0):
        ...
        while line:
            if self.patt["hash"].search(line) or self.patt["emptyline"].search(
                line
            ):  # TODO: is this correct for AQUA comments?
                line = fin.readline()
                continue
        ...
        if numConstraints != listedNumConstraints:
            print(
                "  Error: number of constraints read (%d) does not match number of constraints listed (%d)"
                % (numConstraints, listedNumConstraints)
            )
