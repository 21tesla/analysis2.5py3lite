...
from ccp.format.nmrStar.util import getNmrStarValue, returnStarFloat, returnStarInt, returnStarString
...
class NmrStarStructMeth:
    ...
    def setData(self, methTableTags, i):
        ...
        assignList = [["methodLabel", "Refine_method", None], ["methodDetails", "Refine_details", None]]
...
class NmrStarFile(NmrStarGenericFile):
    ...
    def setSfDict(self, component):
        ...
        if self.version == "2.1.1":
            ...
            if component == sfName or component == "all":
                ...
                self.sfDict[sfName].tables = [
                    ...
                    ["_Keyword", [["_Keyword", lambda x=value: returnStarString(x)]]]
                ]
            ...
            if component == sfName or component == "all":
                ...
                self.sfDict[sfName].tables = [
                    [
                        ...
                        [["_Mol_system_component_name", lambda x=value: x], ["_Mol_label", lambda x=value: x]],
                    ],
                    ["_Biological_function", [["_Biological_function", lambda x=value: x]]],
                ]
            ...
            if component == sfName or component == "all":
                ...
                # Warning: key here is tagName by which table is identified!
                self.sfDict[sfName].tables = [
                    ["_Sample_label", [["_Sample_label", lambda x=value: x]]],
                    [
                        ...
                        [
                            ...
                            [
                                ...
                                lambda x=value: returnStarInt(x),
            ...
            if component == sfName or component == "all":
                ...
                self.sfDict[sfName].tables = [
                    ["_Sample_label", [["_Sample_label", lambda x=value: x]]],
                    ["_Software_label", [["_Software_label", lambda x=value: x]]],
                    [
                        "_Residue_seq_code",
                        [
                            ["_Residue_seq_code", lambda x=value: returnStarInt(x)],
            ...
            if component == sfName or component == "all":
                ...
                self.sfDict[sfName].tags = [["_Saveframe_category", lambda x=value: x]]
            ...
            if component == sfName or component == "all":
                ...
                self.sfDict[sfName].tags = [
                    ...
                    [
                        "_Sample_conditions",
                        lambda x=value: x,
                    ],  # Warning; this is not correct but often occurs, so hacking it in...
        ...
        #############################################
            ...
            try:
                from pdbe.nmrStar.IO.nmrStarDict import sfDict as refSfDict
            except:
                raise self.FormatFileError("No NMR-STAR dictionary available for version 3.1!")
                    ...
                    if "tableNames" in refSfDict[sfName]:
                        for tableName in refSfDict[sfName]["tableNames"]:
                            ...
                            for tagName in refSfDict[sfName]["tables"][tableName]["tagNames"]:
                                self.sfDict[sfName].tables[-1][-1].append(
                                    [tagName, refSfDict[sfName]["tables"][tableName]["tags"][tagName][1]]
                                )
...
class GenericConstraint:
    def setErrors(self, errorTags, tableTagNames, i):
        ...
        for tableTagName in tableTagNames.keys():
            if tableTagName == "position" or tableTagNames[tableTagName] not in errorTags:
                continue
