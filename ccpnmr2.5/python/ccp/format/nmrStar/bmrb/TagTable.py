...
class TagTable(Lister):
    ...
    floatPatt = re.compile(r"^(\-?\d+)\.?(\d*)$")
    integerPatt = re.compile(r"^\d+$")

    colspacer = 3 * " "

    def __init__(self, free=None, title="", tagnames=None, tagvalues=None, verbosity=2):
        self.free = free
        self.title = title
    ...
    def star_text(self, flavor="NMR-STAR"):
        ...
        count = -1
        count_hash = (
            100000  # Show progress hashes while composing text for each count_hash number of values approximately
        )
    ...
    def check_integrity(self, check_type=0):
        ...
        for tag_id in range(values_length):
            if len(self.tagvalues[tag_id]) != column_length_first:
                print(
                    "ERROR: length column[%s](%s) is not the same as"
                    % (self.tagnames[tag_id], len(self.tagvalues[tag_id]))
                )
        ...
        if self.verbosity >= 9:
            print(
                "Checked integrity of TagTable (%2s names %4s values each): OK [%s]"
    ...
    def parse(self, text="", pos=0):
        ...
        match_tags_loop_2 = pattern_tags_loop_2.findall(text, pos, match_tags_loop.end())
        ...
        # Tag values
        if self._tagtable_loop_values_parse(text, pos, pos_end):  ## will set title too
...
if __name__ == "__main__":
    ...
    text = """_A a b c"""
    free = None
    tt = TagTable(free=free, tagnames=[], tagvalues=[], verbosity=STAR.verbosity)
