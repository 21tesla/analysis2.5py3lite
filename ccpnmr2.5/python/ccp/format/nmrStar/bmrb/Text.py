...
pattern_tagtable_loop = re.compile(
    r"""
...
pattern_semicolon_block = re.compile(
    r"""
...
pattern_tag_name = re.compile(
    r"""(_\S+) \s+
     """,
    re.DOTALL | re.MULTILINE | re.VERBOSE,
)
pattern_tags_loop = re.compile(
    r"""(?: (_\S+) \s* )+
     """,
    re.MULTILINE | re.VERBOSE,
)
pattern_tags_loop_2 = re.compile(
    r"""    (_\S+) \s*
     """,
    re.MULTILINE | re.VERBOSE,
)
...
pattern_comment_begin = re.compile(
    r"""^\s*\#.*\n           # A string starting a line with a sharp
                                   """,
    re.MULTILINE | re.VERBOSE,
)

pattern_nmrView_compress_empty = re.compile(
    r""" \{(\s+)\}
                                             """,
    re.MULTILINE | re.VERBOSE,
)
pattern_nmrView_compress_questionmark = re.compile(
    r""" \{(\s+\?)\}
                                                    """,
    re.MULTILINE | re.VERBOSE,
)
...
def pattern_unquoted_find(text, pattern, pos=0):
    while 1:
        ...
        if line[0] == ";":
            if verbosity > 1:
                print("WARNING: (1) found pattern: [%s] preceded by: [%s]" % (pattern.pattern, line))
            pos = pos + 1
            continue
...
def quotes_strip(text):
    ...
    if len(text) <= 1:
        return text
    for quote_symbol in ["'", '"']:
        if text[0] == quote_symbol and text[-1] == quote_symbol:
            return text[1:-1]
    return text
...
def comments_strip(text):
    ...
    text, _count = pattern_comment_middle.subn(r"\g<1>", text)
    #    if verbosity >= 9:
