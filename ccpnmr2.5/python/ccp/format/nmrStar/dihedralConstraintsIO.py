...
from ccp.format.general.Constants import bioPolymerCodes, defaultMolCode
            ...
            for i in range(0, len(constraintTableTags[tableTagNames["constraintID"]])):
                ...
                if dihedralConstraint.name and not constraintTableTags[tableTagNames["atomPresent"]][i]:
                    ...
                    for position in tableTagNames["position"]:
                        ...
                        dihedralConstraint.nodes[-1].members[-1].addStarInfo(
                            constraintTableTags, i, position, tableTagNames
                        )
...
class NmrStarDihedralConstraint(GenericConstraint):
    ...
    def setAtomMembers(self, constraintTableTags, i, tableTagNames):
        ...
        (seqCode, seqInsertCode) = getSeqAndInsertCode(
            constraintTableTags[tableTagNames["seqCode"] % tableTagNames["position"][0]][i]
        )
        ...
        if not refAngle:
            ...
            # Assuming DNA
                ...
                print(
                    "    Unrecognized dihedral angle '%s' for residue label '%s' - ignoring..." % (self.name, resLabel)
                )
                return
