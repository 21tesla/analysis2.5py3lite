    ...
    def parseSaveFrame(self):
        ...
        if tableName in self.saveFrame.tables:
            ...
            for i in range(len(constraintTableTags[tableTagNames["constraintID"]])):
                ...
                if isGeneralType:
                    self.constraints[-1].nodes.append(
                        NmrStarDistanceConstraintItem(Id=constraintTableTags[tableTagNames["memberID"]][i])
                    )
...
class NmrStarDistanceConstraintMember:
    ...
    def setData(self, nodeMembersTableTags, i, tableTagNames):
        ...
        if "chemCompID" in tableTagNames:
