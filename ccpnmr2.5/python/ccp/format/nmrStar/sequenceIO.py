                            ...
                            if "_Atom_site" in coordSf.tables:
                                ...
                                for i in range(len(coordTableTags["Model_ID"])):
                                    ...
                                    if chainId and seqId:
                                        ...
                                        if residueKey not in self.origSeqCodeBySeqId:
                                        ...
                                        if residueKey not in self.origSeqInsertCodeBySeqId and "PDB_ins_code" in coordTableTags:
                                            seqInsertCode = coordTableTags["PDB_ins_code"][i]
                                            if seqInsertCode:
                                                self.origSeqInsertCodeBySeqId[residueKey] = seqInsertCode

                                        if atomName:
                                            if residueKey not in self.atomNamesBySeqId:
                                                self.atomNamesBySeqId[residueKey] = []

                                            if atomName not in self.atomNamesBySeqId[residueKey]:
                    ...
                    for entitySaveFrame in self.parent.sfs["entity"]:
                        ...
                        if entityFound:
                            ...
                            self.sequences.append(
                                NmrStarSequence(
                                    entitySaveFrame.title,
                                    self,
                                    entitySaveFrame,
                                    Id=entityAssemblyId,
                                    entityId=entityId,
                                    originalChainCode=originalChainCode,
                                    details=details,
                                    role=role,
                                    physState=physState,
                                    confIsomer=confIsomer,
                                    chemExchState=chemExchState,
                                    magnEquivCode=magnEquivCode,
                                )
                            )
                            break
                                ...
                                if sequence.Id == tableTags["Entity_assembly_ID_%d" % j][i]:
                                    for sequenceElement in sequence.elements:
                                        if sequenceElement.seqCode == tableTags["Comp_index_ID_%d" % j][i]:
                                            sequenceElementData.append(
                                                (sequenceElement, tableTags["Atom_ID_%d" % j][i])
                                            )

                                    break

                        if len(sequenceElementData) == 2:
                            sequenceElementData[0][0].setBond(
                                bondType,
                                sequenceElementData[0][1],
                                sequenceElementData[1][0],
                                sequenceElementData[1][1],
                            )
                            sequenceElementData[1][0].setBond(
                                bondType,
                                sequenceElementData[1][1],
                                sequenceElementData[0][0],
                                sequenceElementData[0][1],
                            )
                        else:
...
class NmrStarSequence:
    def __init__(
        ...
        if self.version == "2.1.1":
            ...
            if tableName in self.saveFrame.tables and authorSeqCodeTag in self.saveFrame.tables[tableName].tags:
                validCode = 1
                    ...
                    if self.parent.origSeqCodeBySeqId:
                        ...
                        if residueKey in self.parent.origSeqCodeBySeqId:
                            ...
                            for i in range(1, 99999):
                                ...
                                if prevResidueKey not in self.parent.origSeqCodeBySeqId:
...
class NmrStarSequenceElement:
    ...
    def __init__(self, sequenceTableTags, i, tableTagNames):
        ...
        for attrName in tableTagNames.keys():
            if (
                tableTagNames[attrName] in sequenceTableTags
                and len(sequenceTableTags[tableTagNames[attrName]]) > i
                and sequenceTableTags[tableTagNames[attrName]][i] is not None
            ):
    ...
    def setBond(self, bondType, atomName, bondedSeqEl, bondedAtomName):
        ...
        if atomName not in self.bonds[bondType]:
            self.bonds[bondType][atomName] = []

        if (bondedSeqEl, bondedAtomName) not in self.bonds[bondType][atomName]:
            self.bonds[bondType][atomName].append((bondedSeqEl, bondedAtomName))
            print(
                "  Found %s bond from %s.%s - %s.%s"
                % (bondType, self.seqCode, atomName, bondedSeqEl.seqCode, bondedAtomName)
            )
