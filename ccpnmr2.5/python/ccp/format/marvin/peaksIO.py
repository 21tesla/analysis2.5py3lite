            ...
            if "peakAssign" in peak:
                for assignment in peak["peakAssign"]:
                    ...
                    if 1:
                        ...
                        if values[2] != None:
                            assign = [
                                self.getAssignString(values[5], "LIGHT:" + values[13] + ":" + values[7]),
...
class Marvin_reader:
    ...
    def dump(self, handler=None, sort=True, ostream=sys.stdout):
        ...
        if handler == None:
            print("! dumping marvin peak records", file=ostream)
            print("!---------------------------", file=ostream)
            print("", file=ostream)
            print("!%d marvin peak records" % len(self.records), file=ostream)
            print("", file=ostream)
            for record in self.records:
                print(record, file=ostream)
                print("", file=ostream)
        else:
            for record in self.records:
...
def Dump_assignment(Assignment_handler):
    ...
    def handle_assignment(record):
        ...
        print(
            from_assignment.shift_id,
            from_assignment.residue_id,
            from_assignment.residue_type,
            from_assignment.atom_name,
            file=self.ostream,
        )
        print(
            to_assignment.shift_id,
            to_assignment.residue_id,
            to_assignment.residue_type,
            to_assignment.atom_name,
            file=self.ostream,
        )
