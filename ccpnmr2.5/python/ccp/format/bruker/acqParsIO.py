...
from ccp.format.bruker.generalIO import BrukerGenericFile, BrukerPar, BrukerParFile
from ccp.format.general.acqParsDefs import FidDimReference, FreqDimReference, GenericAcqParData, SpectrometerReference
...
class BrukerAcqParData(BrukerGenericFile, GenericAcqParData):
    ...
    def get(self, verbose=0):
        ...
        for file in files:
            if file in self.parFiles:
                ...
                self.parFiles["pulseprogram"]["echoAntiEcho"] = (
                    0  # Should be 1 or more if echo-AntiEcho, especially igrad indicative!!
                )
                ...
                while line:
                    ...
                    if (
                        not self.parFiles["pulseprogram"]["name"]
                        and pulseLine
                        and self.parFiles["pulseprogram"]["comment"]
                    ):
                        ...
                        if stdIncrCount == self.numDim - 1:
                            for incr in increments[:]:
                                if incr not in self.stdIncrements:
                                    ...
                                    print(
                                        "  Warning: removed increment or decrement %s in favour of standard ones."
                                        % incr
                                    )
    ...
    def processv21(self):
        ...
        for ii in range(self.numDim):
            ...
            if "FnMODE" in acqusPars and ii != 0:
            ...
            if numPointsValid > npts:
                print("  WARNING: %s dim %s has greater number of valid points than actual ones" % (self.fileDir, ii))
            ...
            self.fidDimRefs[ii] = FidDimReference(npts, numPointsValid, timePerPoint, nucleus)
            ...
            (nPoints, nPointsOrig, valuePerPoint) = self.getFreqDimInfo(timePerPoint, numPointsValid)
            self.freqDimRefs[ii] = FreqDimReference(nPoints, nPointsOrig, timePerPoint, nucleus)
    ...
    def process(self):
        ...
        if "TITLE" in self.parFiles["acqus"].headerDict:
            ...
            if "TopSpin" in title:  # from Bruker 3.5 or so
                ...
                patt = re.compile(r"TopSpin\s*(\d+\.*\d*)")
            else:
    ...
    def processpre21(self):
        ...
        pulseProgram = None
        if "pulseprogram" in self.parFiles:
        ...
        if self.numDim == 3:
            ...
            if "NUC1" not in self.parFiles["acqu3s"].parameters:
                #
        ...
        if pulseProgram and self.numDim <= len(pulseProgram["increment"]):
            print(
                "  Warning: %s number of incremented delays found does not match number of dimensions!" % (self.fileDir)
            )
        ...
        for i in range(0, self.numDim):
            ...
            self.fidDimRefs[i] = FidDimReference(numPoints, numPointsValid, valuePerPoint, nucleus)

            #
            # And for the Freq
            #
            (numPoints, numPointsOrig, valuePerPoint) = self.getFreqDimInfo(valuePerPoint, numPointsValid)
            self.freqDimRefs[i] = FreqDimReference(numPoints, numPointsOrig, valuePerPoint, nucleus)
