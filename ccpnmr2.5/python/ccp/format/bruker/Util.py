...
from ccp.format.bruker.Constants import pulProgFile, stdExpNameDict
...
def parsePulProgName(name, fileLocation=None):
    ...
    if fileLocation is None:
        if pulProgNameMap is None:
            fileLocation = uniIo.joinPath(uniIo.getTopDirectory(), "data", "ccp", "bruker", "BrukerPulseProgMap.xml")
...
def testNameParsing(dirPath):
    ...
    # set of imput info
    stdInfoFile = uniIo.joinPath(uniIo.getTopDirectory(), "data", "ccp", "bruker", "BrukerPulseProgMap.xml")
    ...
    for fname in fileNames:
        ...
        if dd.get("status") == "skip" or not os.path.isfile(uniIo.joinPath(dirPath, fname)):
            skipped.append(dd)
...
def parseMusicExp(text, useInfo):
    ...
    unused, endnames, suffix = ccpGenUtil.parseEndNames(useInfo["endnames"], "_".join(ll[2:]))
        ...
        if getAminoAcids:
            ...
            for code1Letter in code1LetterToCcpCodeDict["protein"].keys():
                if aaNameComp.count(code1Letter.upper()):
                    print(
                        "    Bruker info: targeted at %s amino acid." % code1LetterToCcpCodeDict["protein"][code1Letter]
                    )
            ...
            if brukerExpName[strInd] != "*" and brukerExpName[strInd - 1] != "*":
                for brukerCode in self.brukerCodes:
                    if brukerExpName[strInd - len(brukerCode) + 1 : strInd + 1].count(brukerCode):
                        ...
                        brukerExpName = brukerExpName[:rIndex] + brukerExpName[rIndex:].replace(
                            brukerCode, "*" * len(brukerCode)
                        )
