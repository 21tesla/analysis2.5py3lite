...
from ccp.format.general.acqParsDefs import FidDimReference, FreqDimReference, GenericAcqParData
from ccp.format.varian.generalIO import VarianGenericFile
from memops.universal.Io import joinPath, splitPath
from memops.universal.Util import joinUnquote, returnFloat, returnFloats, returnInt, unquote
    ...
    def read(self, verbose=0, expname="unknown"):
        ...
        while line:
            ...
            if not (self.patt["emptyline"].search(line) or self.patt["hash"].search(line)):
                ...
                if len(cols) != 11:
                    print(
                        "  ERROR: %s Varian procpar has wrong number of fields on following line:" % expname
                        + self.newline
                        + line
                    )
                elif cols[0] in self.parameters:
                    print("Reoccuring parameter name: %s" % cols[0]) + self.newline
                else:
                    parname = cols[0]
                    self.parameters[parname] = VarianAcqPar(
                        parname, cols[1], cols[2], cols[3], cols[4], cols[5], cols[6], cols[7], cols[8], cols[9]
                    )
                    ...
                    if self.parameters[parname].basictype == 1:
                        ...
                        print(
                            "  ERROR: %s Varian procpar has undefined basic data type. Check parameter %s."
                            % (expname, parname)
                        )
...
class VarianAcqPar:
    ...
    def addvalues(self, expname, value_len, values):
        ...
        if value_len != len(values):
            print(
                "  ERROR: %s Varian procpar: given value_len does not match number of values (%s)"
                % (expname, self.name)
            )
    ...
    def addenum(self, expname, enum_len, enums):
        ...
        if enum_len > 0:
            if enum_len != len(enums):
                print(
                    "  ERROR: %s Varian procpar: given enum_len does not match number of enums (%s)"
                    % (expname, self.name)
                )
