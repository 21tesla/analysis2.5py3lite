...
def parseProcparFile(procparFile):
    ...
    try:
        ...
        while line1:
            ...
            if basictype == 0:
                raise OSError(
                    "line number %d: basictype = %d, do not know how to deal with case 0:\n  %s" % (n, basictype, line1)
                )
            ...
            if basictype == 1:  # (real)
                if len(fields) != (1 + value_len):
                    raise OSError(
                        "line number %d: need %d fields, have %d:\n  %s" % (n, 1 + value_len, len(fields), line2)
                    )
                if value_len == 1:
...
def readDataBlockHeader(dataFileParams, block=0):
    ...
    if len(header) != VNMR_BLOCK_HEADER_SIZE:
        raise OSError(
            "header for block %d only read as size %d, expecting %d" % (block, len(header), VNMR_BLOCK_HEADER_SIZE)
        )

    if header1 and len(header1) != VNMR_BLOCK_HEADER_SIZE:
        raise OSError(
            "header1 for block %d only read as size %d, expecting %d" % (block, len(header1), VNMR_BLOCK_HEADER_SIZE)
        )
