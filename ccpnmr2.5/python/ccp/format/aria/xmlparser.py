...
NOT_CHAR_DATA = re.compile(r"[^<&]*\]\]>[^<&]*")
LETTER = re.compile("[a-zA-Z]")
DIGIT = re.compile("[0-9]")
SPACE = re.compile(r"\s+")
OPTIONAL_SPACE = re.compile(r"\s*")
ATTRIBUTE_VALUE = re.compile("(\"[^\"]*\"|'[^']*')")
COMMENT = re.compile("<!--([^-]|-[^-])*-->")
ENCODING_NAME = re.compile(r"[A-Za-z]([A-Za-z0-9\._]|\-)*")
SYSTEM_LITERAL = re.compile("(\"[^\"]*\")|('[^']*')")
VERSION_NUMBER = re.compile(r"([a-zA-Z0-9_.:]|\-)+")
ILLEGALCHARS = re.compile("[^\t\r\n -\176\240-\377]")
EQUALS = re.compile(OPTIONAL_SPACE.pattern + "=" + OPTIONAL_SPACE.pattern)

VERSION_INFO = re.compile(SPACE.pattern + "version" + EQUALS.pattern + quote(VERSION_NUMBER))

ENCODING_DECLARATION = re.compile(SPACE.pattern + "encoding" + EQUALS.pattern + quote(ENCODING_NAME))

STANDALONE_DECLARATION = re.compile(SPACE.pattern + "standalone" + EQUALS.pattern + quote("(yes|no)"))

NAMECHAR = re.compile(LETTER.pattern + "|" + DIGIT.pattern + r"|\.|\-|_|:")

NAME = re.compile("(" + LETTER.pattern + "|_|:)" + "(" + NAMECHAR.pattern + ")*")

ATTRIBUTE = re.compile(NAME.pattern + EQUALS.pattern + ATTRIBUTE_VALUE.pattern)

START_TAG = re.compile(
    "<" + NAME.pattern + "(" + SPACE.pattern + ATTRIBUTE.pattern + ")*" + OPTIONAL_SPACE.pattern + ">"
)
...
DOCTYPE_DECLARATION = re.compile(
    "<!DOCTYPE"
    + SPACE.pattern
    + NAME.pattern
    + "("
...
MISCELLANEOUS = re.compile("(" + COMMENT.pattern + "|" + SPACE.pattern + ")")

PROLOG = re.compile(
    "("
    + XML_DECLARATION.pattern
    + ")?"
    + "("
...
class SelfmadeXMLParser:
    ...
    def setup(self):
        ...
        self.junk = re.compile("%s(%s)%s" % (opt_space, COMMENT.pattern, opt_space))

        name = "(?P<name>%s)" % NAME.pattern
        attr = "(?P<attr>(%s%s)*)" % (SPACE.pattern, ATTRIBUTE.pattern)

        self.start = re.compile(opt_space + "<" + name + attr + opt_space + "(?P<end>/?)>" + opt_space)
        self.end = re.compile("%s</%s%s>%s" % (opt_space, name, opt_space, opt_space))

        self.attr = re.compile(
            SPACE.pattern + "(?P<attr>%s)" % NAME.pattern + EQUALS.pattern + "(?P<value>%s)" % ATTRIBUTE_VALUE.pattern
        )
