...
def getChemComp(
    project, molType, ccpCode, download=True, showError=None, partialLoad=False, chemCompArchiveDir=None, copyFile=True
):
    ...
    if chemComp is None:
        ...
        if chemComp is None:
            ...
            ccLocator = project.findFirstPackageLocator(targetName=packageName) or project.findFirstPackageLocator(
                targetName="any"
            )
            repository = ccLocator.findFirstRepository()

            fileFound = getChemCompXmlFile(
                repository, chemCompPath, molType, ccpCode, showError=showError, copyFile=copyFile
            )
            if not fileFound and download:
                ...
                #                                    showError=showError)
                fileFound = downloadChemCompInfoFromCcpForge(repository, molType, ccpCode, showError=showError)
            if fileFound:
...
def getChemCompXmlFile(repository, chemCompPath, molType, ccpCode, showError=None, copyFile=True):
    """
    ...
    return getChemCompOrCoordXmlFile(
        repository, fileSearchString, fileSearchPath, className, identifier, showError, copyFile
    )


def getChemCompCoordXmlFile(repository, chemCompCoordPath, sourceName, molType, ccpCode, showError=None, copyFile=True):
    """
...
def getChemCompCoord(
    ...
    if chemCompCoord is None:
        ...
        chemCompCoordFileSearchString = "%s+%s+%s+*.xml" % (
            getCcpFileString(sourceName),
            molType,
        ...
        if chemCompCoord is None:
            ...
            fileFound = getChemCompCoordXmlFile(
                repository, chemCompCoordPath, sourceName, molType, ccpCode, showError=showError, copyFile=copyFile
            )
            if not fileFound and download:
                # Replaced by direct download from CcpForge (Wim 15/06/2010)
                # fileFound = downloadChemCompCoordXmlFile(repository, sourceName, molType, ccpCode,
                #                                    showError=showError)
                fileFound = downloadChemCompInfoFromCcpForge(
                    repository, molType, ccpCode, sourceName=sourceName, showError=showError
                )
...
def getCcpForgeSubPath(molType, ccpCode, sourceName=None):
    ...
    if not sourceName:
        ...
        fileSearchString = r"%s\+%s\+" % (molType, getCcpFileString(ccpCode))

    else:
        fileType = "ChemCompCoord"
        subPath = getChemCompCoordArchiveXmlFilePath("", sourceName, molType, ccpCode)
        fileSearchString = r"%s\+%s\+%s\+" % (getCcpFileString(sourceName), molType, getCcpFileString(ccpCode))
            ...
            if urlLocation:
                ...
                try:
                    ...
                    if data and (
                        ...
                        showError("File not found", "File not found on server: %s" % xmlFile)
                        ...
                        try:
                            ...
                            print(
                                "Downloaded %s %s%s, %s from server %s, written to file %s!"
                    ...
                    showError(
                        "Cannot read file",
                        "Cannot read %s %s%s, %s: %s" % (fileType, sourceText, molType, ccpCode, str(e)),
                    )

            else:
                showError(
                    "Cannot find file", "Cannot find %s XML file %s%s, %s." % (fileType, sourceText, molType, ccpCode)
                )

        except OSError as e:
            showError(
                "Cannot read directory",
                "Cannot read directory information for %s%s, %s: %s" % (sourceText, molType, ccpCode, str(e)),
            )

    except OSError as e:
        showError(
            "No connection",
            "Cannot connect to download server %s, or file does not exist...: %s " % (ccpForgeDownloadUrl, str(e)),
        )
    #
    return result
...
def getStdChemComps(project, molTypes=None):
    ...
    for molType in molTypes:
        if molType in standardResidueCcpCodes:
...
def setDataSourceDataStore(dataSource, dataUrlPath, localPath, dataLocationStore=None, dataUrl=None):
    ...
    blockedBinaryMatrix = dataLocationStore.newBlockedBinaryMatrix(path=localPath, dataUrl=dataUrl)
...
def setDataStoreFileName(dataStore, fileName):
    ...
    (dataUrl, filePath) = getDataStoringFromFilepath(
        dataStore.root,
...
def getDataStoringFromFilepath(
    memopsRoot, fullFilePath, preferDataUrls=None, dataLocationStore=None, keepDirectories=1
):

    # make absolute,, normalised path
    fullFilePath = uniIo.normalisePath(fullFilePath, makeAbsolute=True)

    dataUrl, filePath = findDataStoringFromFilepath(
        memopsRoot, fullFilePath, preferDataUrls, dataLocationStore, keepDirectories
    )

    if dataUrl is None:
        urlPath = uniIo.normalisePath(fullFilePath[: -len(filePath)])
        dataLocationStore = memopsRoot.currentDataLocationStore
        dataUrl = dataLocationStore.newDataUrl(url=Implementation.Url(path=urlPath))
        dataUrl.name = "auto-%s" % dataUrl.serial
    #
    return (dataUrl, filePath)


def findDataStoringFromFilepath(project, fullFilePath, preferDataUrls=None, dataLocationStore=None, keepDirectories=1):
    ...
    if not os.path.isabs(fullFilePath):
        raise ApiError("findDataStoringFromFilepath called with non-absolute file name %s" % fullFilePath)
    ...
    if dataUrls:
        ...
        if ss == "/":
            ...
            uniUtil.isWindowsOS() and ss.endswith("/") and ss[-2] == ":"
        ):  # deals with case on Windows when one has drive + '/'
            ss = ss[:-1]
        else:
            ss = uniIo.joinPath(
                dataUrl.url.path, ""
            )  # removes file separator from end, but not if just '/', hence special cases above
...
if __name__ == "__main__":
    ...
    try:
        request = urllib.request.urlopen(
            "https://raw.githubusercontent.com/VuisterLab/CcpNmr-ChemComps/master/data/pdbe/chemComp/archive/ChemComp/other/T/other%2BTho%2Bmsd_ccpnRef_2007-12-11-10-19-31_00014.xml",
            context=context,
            timeout=3.0,
        )
