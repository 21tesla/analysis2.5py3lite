...
class SelectObjectPopup(BasePopup):
    def __init__(self, parent, objects, key=None, labels=None, objectName=None, refClassName=None, **kw):
        ...
        BasePopup.__init__(self, parent, title="Object Selection", borderwidth=5, **kw)

    def resetObjectType(self, objects, key=None, labels=None, objectName=None, refClassName=None, **kw):
        if refClassName:
        ...
        self.labels["fullKey"] = [str(getattr(obj, "getFullKey")()) for obj in self.objects]
    ...
    def getAttributeHeadings(self, refClass):
        ...
        if self.key is None:
            ...
            try:
                ...
                raise ApiError("key %s not in attributes or labels: %s" % (self.key, attrNames))
    ...
    def update(self, objects, key=None, labels=None, objectName=None, refClassName=None, remove=None, reset=True):
        ...
        textMatrix = self.getTextMatrix(self.objects)
        self.scrolledMatrix.update(objectList=self.objects, headingList=self.headingList, textMatrix=textMatrix)
    ...
    def updateAfter(self, object):
        ...
        self.after_idle(lambda s=self: s.update(s.objects, remove=object, reset=False))

    def selectCell(self, object, row=None, col=None):
    ...
    def doUnregisters(self):
        ...
        try:
            Implementation.unregisterNotify(self.updateAfter, self.refClassName, "delete")
        except AttributeError:
            # refClassName may be an abstract class,
            # in which case registering will fail
            return
