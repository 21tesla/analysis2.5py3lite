...
userResidueCodesDict = {
    "DNA": {"A": "Ade", "T": "Thy", "G": "Gua", "C": "Cyt", "U": "Ura"},
...
class ArgumentServer:
    ...
    def getFile(self, file_types=None, dismiss_text="Cancel", selected_file_must_exist=False):
    ...
    def getDirectory(self, dismiss_text="Cancel", selected_file_must_exist=False):
        ...
        if self.inGui:
            module = importModule("memops.gui.FileSelectPopup")
            popup = module.FileSelectPopup(
                self.parent, dismiss_text=dismiss_text, selected_file_must_exist=selected_file_must_exist
            )
    ...
    def getProgressBar(self, text="", progress=0, total=100, width=200, height=15, transient=True):

        if self.inGui:
            from memops.gui.ProgressBar import ProgressBar

            return ProgressBar(
                self.parent, text=text, progress=progress, total=total, width=width, height=height, transient=transient
            )

    def chooseObject(self, objects, key=None, labels=None, objectName=None, refClassName=None):
        ...
        if len(objects) == 0:
            ...
            return self.chooseObjectDialog(
                objects, key=key, labels=labels, objectName=objectName, refClassName=refClassName
            )

    def objectSelectionPopup(self, objects, key=None, labels=None, objectName=None, refClassName=None):

        self.parent.argumentVar = Tkinter.IntVar()
        self.parent.argumentObject = None
        if self.popup:
            self.popup.update(objects, key=key, labels=labels, objectName=objectName, refClassName=refClassName)
            self.popup.open()
        else:
            self.popup = self.dataModelPopup.SelectObjectPopup(
                self.parent, objects, key=key, labels=labels, objectName=objectName, refClassName=refClassName
            )
    ...
    def getSpectrum(self, spectra=None):
        ...
        return self.chooseObject(spectra, key="Name", labels=labels, objectName="Spectrum")
    ...
    def getProject(self):
        ...
        if hasattr(parent, "project"):
            ...
            self.messageReporter.showWarning("Warning", "No project defined - code error")
            return
        #
        return project
    ...
    def getConstraintSet(self):
        ...
        constraintSet = self.chooseObject(nmrProject.sortedNmrConstraintStores(), objectName="Constraint Set")
    ...
    def getConstraintList(self, constraintSet=None):
        ...
        if constraintSet:
            constraintList = self.chooseObject(
                constraintSet.sortedConstraintLists(), refClassName="ccp.nmr.NmrConstraint.AbstractConstraintList"
            )
    ...
    def getShiftList(self):
        ...
        return self.getMeasurementList("ShiftList")

    def getMeasurementList(self, className=None):

        project = self.getProject()
        mLists = []
        for measurementList in project.currentNmrProject.sortedMeasurementLists():
            if (not className) or (measurementList.className == className):
                mLists.append(measurementList)

        return self.chooseObject(mLists)
    ...
    def getSpinSystem(self):
        ...
        return self.chooseObject(nmrProject.sortedResonanceGroups(), objectName="SpinSystem", labels=labels)
        # put residue
    ...
    def pickObjectByAttribute(self, objects, key=None, labels=None, objectName=None, refClassName=None):
        """9/1/09 Rasmus Fogh
        ...
        if key is None:
            ...
            try:
                ...
                raise ApiError("key %s not in attributes or labels: %s" % (key, attrNames))
            keyNames = [key]
            attrNames.insert(0, key)
            attrNames.append("fullKey")

        # add 'fullKey to labels
        labels["fullKey"] = [str(getattr(obj, "getFullKey")()) for obj in objects]
        ...
        for keyName in keyNames:
            ...
            if xx:
                ...
                try:
                    ...
                    self.messageReporter.showError("Error", "Object %s lacks key attribute %s" % (obj, keyName))
            ll.append(val)
            ...
            for attrName in attrNames:
                ...
                if xx:
                    ...
                    try:
                        ...
                        self.messageReporter.showError("Error", "Object %s lacks attribute %s" % (obj, attrName))
                ll.append(val)
