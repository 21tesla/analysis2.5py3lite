        ...
        if not fromInstanceAttrValue:
            ...
            if metaAttr.hicard == 1:
                ...
                if toInstanceAttrValue != None:
                    if verbose:
                        print(
                            "    Warning: can't relink attribute/role %s value %s: already set"
                            % (attrName, fromInstanceAttrValue)
                        )
                    ...
                    if verbose:
                        print(
                            "    Copied attribute %s (now %s, was %s)."
                            % (attrName, fromInstanceAttrValue, toInstanceAttrValue)
                        )

            # Is a list - just add the value (if not already present!)
            else:
                for fromInstanceAttrValueElement in fromInstanceAttrValue:
                    if fromInstanceAttrValueElement not in toInstanceAttrValue:
...
def copyResonanceInfo(
    fromRes, toRes, nmrShiftSelect=nmrShiftSelect, deleteOldInfo=False, toResName=None, copyTrack=None, verbose=True
):
            ...
            for linkedClass in linkedClassList:
                ...
                #
                    ...
                    #
                        ...
                        try:
                            newLinkedClass = linkedClass.__class__(
                                linkedClassParent, resonances=resonanceList, **parameters
                            )
                        except:
                            if verbose:
                        ...
                        if linkedClassName == "DihedralConstraint":
                            ...
                            for dihedralConstraintItem in linkedClass.items:
                                ...
                                newDihedralConstraintItem = dihedralConstraintItem.__class__(
                                    newLinkedClass, **parameters
                                )
                            ...
                            if copyTrack:
                                ...
                                newLinkedClass.addApplicationData(
                                    Implementation.AppDataInt(application="ccp", keyword="copied", value=resonanceIndex)
                                )
                        ...
                        for applData in linkedClass.applicationData:
                            ...
                            try:
                                ...
                                if verbose:
                                    ...
                                    print(
                                        "        %s" % traceback.format_exception_only(sys.exc_type, sys.exc_value)[-1]
                                    )
...
def createNewNmrConstraintStoreFromCopy(
    project,
    nmrProject,
    ncs,
    structureEnsemble,
...
def resetNmrConstraintStoreFile(newNmrConstraintStore, newConstraintsFile):
    ...
    guidPatt = re.compile(r'guid\="([^"]+)"')
    userPatt = re.compile(r'createdBy\="([^"]+)"')
    serialPatt = re.compile(r'(serial="\d+")')
