    ...
    def parseSaveFrame(self):
        ...
        while True:
            ...
            if tok == STARLexer.ERROR:
                ...
                STARLexer.DVNDOUBLE,
                STARLexer.DVNNON,
                STARLexer.DVNSEMICOLON,
                STARLexer.DVNFRAMECODE,
            ):
                if not needvalue:
                ...
                if tok == STARLexer.DVNSEMICOLON:
                    ...
                    if not n:
                        n = STARLexer._re_saveend.search(val)
                    if not n:
                        n = STARLexer._re_loop.search(val)
                    if not n:
                        n = STARLexer._re_stop.search(val)
                    if not n:
                        n = STARLexer._re_tag.search(val)
                    if n:
    ...
    def parseLoop(self):
        ...
        while True:
            ...
            # normal exit point
                ...
                if len(tags) < 1:
                    if self._eh.error(
                        self._lex.getLine(), "Loop with no tags: expected tag, found %s" % (self._lex.getText())
                    ):
                        return True
                if parsingtags:
                    parsingtags = False
