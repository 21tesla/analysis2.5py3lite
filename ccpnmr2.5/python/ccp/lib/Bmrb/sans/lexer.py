...
class STARLexer:
    ...
    def pushBack(self, text):
        if self._verbose:
            print("** push back |" + text + "|, buffer=|" + self._buffer + "|")
    ...
    def yylex(self):
        ...
        if self._verbose:
            print("1) buffer: |" + self._buffer + "| len:", len(self._buffer))
        ...
        if self._verbose:
            print("2) buffer: |" + self._buffer + "| len:", len(self._buffer))
        ...
        while len(self._buffer) > 0:
            if self._yystate == self.YYINITIAL:
                ...
                if m:
                    if self._verbose:
                        print("found opening single quote in |%s|, %d" % (self._buffer, self._lineno))
                    self._buffer = self._buffer[1:]
                    ...
                    self._buffer = self._buffer[n.end() :]
                    if self._verbose:
                        print("text is |%s|, buffer is |%s|, %d" % (self._text, self._buffer, self._lineno))
                    self._buffer = self._buffer.strip()
                    if len(self._text) < 1:
                        self._text = "Value |%s| is an empty string" % (self._text,)
                        return self.ERROR
                ...
                while True:
                    ...
                    if m:
                        ...
                        if self._verbose:
                            print("in YYSEMI, buffer:", self._buffer, "return to YYINITIAL")
