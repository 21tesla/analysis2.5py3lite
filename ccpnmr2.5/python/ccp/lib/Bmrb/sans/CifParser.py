...
from handlers import ContentHandler, ErrorHandler
    ...
    def parseDataBlock(self):
        ...
        while True:
            ...
            if tok == STARLexer.ERROR:
                ...
                (tok == STARLexer.DVNSINGLE)
                or (tok == STARLexer.DVNDOUBLE)
                or (tok == STARLexer.DVNNON)
                or (tok == STARLexer.DVNSEMICOLON)
                ...
                if (tok == STARLexer.SAVESTART) or (tok == STARLexer.SAVEEND):
                    self._eh.fatalError(
                        self._lex.getLine(), "Saveframes are not allowed in mmCIF: %s" % (self._lex.getText())
                    )
                else:
    ...
    def parseLoop(self):
        ...
        while True:
            ...
            # -- return to data block level
                ...
                if parsingtags:
                    if self._eh.error(
                        self._lex.getLine(), 'Tag or value expected, found "data_"' + self._lex.getText()
                    ):
                        return True
                # loop checks
                if len(tags) < 1:
                ...
                if len(tags) < 1:
                    if self._eh.error(
                        self._lex.getLine(), "Loop with no tags: expected tag, found" + self._lex.getText()
                    ):
                        return True
                if parsingtags:
                    parsingtags = False
