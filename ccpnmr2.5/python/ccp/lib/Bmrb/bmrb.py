...
from sans import ContentHandler, ErrorHandler, SansParser, STARLexer
...
def __cleanValue__(value):
    ...
    if (
        ...
        or (
            len(value) > 4
            and (value[:5] == "data_" or value[:5] == "save_" or value[:5] == "loop_" or value[:5] == "stop_")
        )
    ):
        if '"' in value:
            return "'" + value + "'"
...
class __entryParser__(ContentHandler, ErrorHandler):
    ...
    def comment(self, line, text):
        if verbose:
            print("Comment '%s' on line: %d" % (text, line))
        pass

    def startData(self, line, name):
        if verbose:
            print("Data '%s' started on line: %d" % (name, line))
        self.ent.bmrb_id = name

    def endData(self, line, name):
        if verbose:
            print("Data '%s' ended on line: %d" % (name, line))
        pass

    def startSaveFrame(self, line, name):
        if verbose:
            print("Saveframe '%s' started on line: %d" % (name, line))
    ...
    def endSaveFrame(self, line, name):
        if verbose:
            print("Saveframe '%s' ended on line: %d" % (name, line))
        self.curframe = None

    def startLoop(self, line):
        if verbose:
            print("Loop started on line: %d" % (line))
        self.curloop = loop.fromScratch()
        self.curframe.addLoop(self.curloop)

    def endLoop(self, line):
        if verbose:
            print("Loop ended on line: %d" % (line))
        self.curloop = None

    def data(self, tag, tagline, val, valline, delim, inloop):

        if verbose:
            print("Tag / value: %s : %s ( %d : %d ) d %s" % (tag, val, tagline, valline, delim))
...
class schema:
    ...
    def __init__(self, schema_file=None):
        ...
        if schema_file is None:
            schema_file = (
                "https://svn.bmrb.wisc.edu/svn/nmr-star-dictionary/bmrb_star_v3_files/adit_input/xlschem_ann.csv"
            )
...
class entry:
    ...
    def __init__(self, **kargs):
        ...
        if len(kargs) == 0:
            raise ValueError(
                "You must provide either a BMRB ID, a file name, an entry number, or a string to initialize. Use the class methods."
            )
        elif len(kargs) > 1:
            raise ValueError(
                "You cannot provide multiple optional arguments. Use the class methods instead of initializing directly."
            )
        ...
        if "the_string" in kargs:
            ...
            try:
                star_buffer = urllib.request.urlopen(
                    "http://rest.bmrb.wisc.edu/bmrb/NMR-STAR3/" + str(kargs["entry_num"])
                )
            except urllib.request.HTTPError:
                raise OSError("Entry " + str(kargs["entry_num"]) + " does not exist in the public database.")
        else:
            ...
            try:
                ...
                if key in self.frameDict():
                    ...
                    raise KeyError(
                        "Saveframe with name '"
                        + str(key)
                        + "' does not exist and therefore cannot be written to. Use the addSaveframe method to add new saveframes."
                    )
        else:
    ...
    def compare(self, other):
        ...
        try:
            if str(self.bmrb_id) != str(other.bmrb_id):
                diffs.append(
                    "BMRB ID does not match between entries: '"
                    + str(self.bmrb_id)
                    + "' vs '"
                    + str(other.bmrb_id)
                    + "'"
            ...
            if len(self.frame_list) != len(other.frame_list):
                diffs.append(
                    "The number of saveframes in the entries are not equal: "
                    + str(len(self.frame_list))
                    + " vs "
                    + str(len(other.frame_list))
                )
...
class saveframe:
            ...
            try:
                ...
                if key in self.loopDict():
                    ...
                    raise KeyError(
                        "Loop with category '"
                        + str(key)
                        + "' does not exist and therefore cannot be written to. Use addLoop instead."
                    )
        else:
    ...
    def addLoop(self, loop):
        ...
        if loop.category in self.loopDict():
            if loop.category is None:
                ...
                raise ValueError(
                    "You cannot have two loops with the same category in one saveframe. Category: '"
                    + str(loop.category)
                    + "'."
                )
        ...
        if "." in name:
            if name[0] != ".":
                ...
                if self.tag_prefix == None:
                    ...
                    raise ValueError(
                        "One saveframe cannot have tags with different prefixes (or tags that don't match the set prefix)! "
                        + str(self.tag_prefix)
                        + " vs "
                        + str(prefix)
                    )
    ...
    def compare(self, other):
        ...
        try:
            ...
            if str(self.tag_prefix) != str(other.tag_prefix):
                diffs.append(
                    "\tTag prefix does not match: " + str(self.tag_prefix) + "' vs '" + str(other.tag_prefix) + "'"
                )
            if len(self.tags) != len(other.tags):
                diffs.append(
                    "\tNumber of tags does not match: " + str(len(self.tags)) + "' vs '" + str(len(other.tags)) + "'"
                )
            for tag in self.tags:
                if self.tags[tag] != other.tags.get(tag, None):
                    diffs.append(
                        "\tMismatched tag values for tag '"
                        + str(tag)
                        + "': '"
                        + str(self.tags[tag]).replace("\n", "\\n")
                        + "' vs '"
                        + str(other.tags.get(tag, "[not present]")).replace("\n", "\\n")
                        + "'"
                    )
            if len(self.loops) != len(other.loops):
                diffs.append(
                    "\tNumber of children loops does not match: "
                    + str(len(self.loops))
                    + " vs "
                    + str(len(other.loops))
                )
            for loop in self.loopDict():
                if other.loopDict().get(loop, None) is None:
                    diffs.append(
                        "\tNo loop with category '" + str(self.loopDict()[loop].category) + "' in other entry."
                    )
                else:
    ...
    def validate(self, validation_schema=None):
        ...
        for pos, tag in enumerate(self.tags):
            errors.extend(
                my_schema.valType(
                    self.tag_prefix + "." + tag, self.tags[tag], category=self.getTag("Sf_category"), linenum=pos
                )
            )
...
class loop:
    ...
    def __str__(self):
        ...
        if len(self.columns) == 0:
            if len(self.data) == 0:
                ...
                raise ValueError(
                    "Impossible to print data if there are no associated tags. Loop: " + str(self.category)
                )
        ...
        if len(self.data) > 0:
            for row in self.data:
                if len(self.columns) != len(row):
                    raise ValueError(
                        "The number of column tags must match width of the data. Loop: " + str(self.category)
                    )
        ...
        if len(self.data) != 0:
            ...
            title_widths = [
                max(map(lambda x: len(str(x)) + 3, col))
                for col in [[row[x] for row in self.data] for x in range(0, len(self.data[0]))]
            ]
        ...
        if "." in name:
            if name[0] != ".":
                ...
                if self.category == None:
                    ...
                    raise ValueError(
                        "One loop cannot have columns with different categories (or columns that don't match the set prefix)!"
                    )
                name = name[name.index(".") + 1 :]
            else:
                name = name[1:]
    ...
    def addData(self, the_list):
        ...
        if len(the_list) != len(self.columns):
            raise ValueError(
                "The list must have the same number of elements as the number of columns! Insert column names first."
            )
        # Add the user data
    ...
    def addDataByColumn(self, column_id, value):
        ...
        if column_id not in self.columns:
            raise ValueError(
                "The column tag ("
                + column_id
                + ") to which you are attempting to add data does not yet exist. Create the columns before adding data."
            )
    ...
    def compare(self, other):
        ...
        try:
            if str(self.category) != str(other.category):
                diffs.append(
                    "\t\tCategory of loops does not match: '"
                    + str(self.category)
                    + "' vs '"
                    + str(other.category)
                    + "'"
    ...
    def deleteDataByTagValue(self, tag, value, index_tag=None):
        ...
        if category != self.category:
            raise ValueError(
                "Category provided in your column '"
                + category
                + "' does not match this loop's category '"
                + str(self.category)
                + "'."
            )
    ...
    def getDataByTag(self, tags=None):
        ...
        for pos, item in enumerate(map(lambda x: str(x), tags)):
            if "." in item and __formatCategory__(item) != self.category:
                raise ValueError(
                    "Cannot fetch data with column ("
                    + str(item)
                    + ") because the category does not match the category of this loop ("
                    + self.category
                    + ")."
                )
    ...
    def renumberRows(self, index_tag, start_value=1, maintain_ordering=False):
        ...
        if renumber_column >= len(self.columns) or renumber_column < 0:
            raise ValueError(
                "The renumbering column ID you provided '%s' is too large or too small! Value column ids are 0-%d."
                % (tag, len(self.columns) - 1)
            )
    ...
    def sortRows(self, tags, cmp=None):
        ...
        for cur_tag in processing_list:
            ...
            if renumber_column >= len(self.columns) or renumber_column < 0:
                raise ValueError(
                    "The sorting column ID you provided '%s' is too large or too small! Value column ids are 0-%d."
                    % (tag, len(self.columns) - 1)
                )
        ...
        for rownum, row in enumerate(self.data):
            ...
            if len(row) != len(self.columns):
                errors.append(
                    "Loop '"
                    + str(self.category)
                    + "' data width does not match it's column tag width on row "
                    + str(rownum)
                    + "."
                )
            for pos, datum in enumerate(row):
                errors.extend(
                    my_schema.valType(
                        self.category + "." + self.columns[pos],
                        datum,
                        category=category,
                        linenum=str(rownum) + " column " + str(pos),
                    )
                )
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser(
        usage="usage: %prog",
        version="%prog",
        description="STAR handling python module. Usually you'll want to import this. When ran directly a self test is performed.",
    )
    ...
    myrange = (15000, 15200)
    if len(sys.argv) == 3:
        myrange = (int(sys.argv[1]), int(sys.argv[2]))
    if len(sys.argv) == 2:
        myrange = (int(sys.argv[1]), (int(sys.argv[1]) + 1))
    ...
    for x in range(*myrange):
        ...
        if use_stardiff:
            ...
            if "NO DIFFERENCES REPORTED" not in results:
