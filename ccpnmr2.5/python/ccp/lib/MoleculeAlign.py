...
from ccp.lib.MoleculeQuery import DNA_MOLTYPE, DNARNA_MOLTYPE, PROTEIN_MOLTYPE, RNA_MOLTYPE
...
NAMAT = {
    "A": {"A": 2, "C": -5, "G": -5, "T": -5, "U": -5, "R": 1, "Y": -5, "N": 0, "X": 0},
...
def findMatchingChains(molSystem, ccpCodes, excludeChains=None, molTypes=None, doWarning=True):
    ...
    if scoreList:
        ...
        if (bestScore / float(len(bestChain.residues)) > 2.0) or ((max(stretch) == lenSeq) and (lenSeq > 20)):
...
def _dynamicAlign(seq1, seq2, matrix, inspen=2, extpen=1):
    ...
    seq1 = re.sub(r"-|\s", "", seq1)
    seq2 = re.sub(r"-|\s", "", seq2)

    seq1 = re.sub(r"\*", "X", seq1)
    seq2 = re.sub(r"\*", "X", seq2)
    ...
    for x in range(1, nX):
        ...
        for y in range(1, nY):
            ...
            paths = [M[x - 1][y - 1] + score, M[x - 1][y] - p1, M[x][y - 1] - p2]
        ...
        if not (excludeChains and chain in excludeChains):
            ...
            if len(seqCodeIndex) == len(msResidues):
                ...
                for resId, chemComp in inputSeq:
                    ...
                    return (chain, dict((seqCode, msResidues[indx].seqId) for seqCode, indx in seqCodeIndex.items()))
    #
    return None, {}
...
def matchByOffset(molSystem, residToChemComp, excludeChains=None):
    ...
    for chain in molSystem.sortedChains():
        if not (excludeChains and chain in excludeChains):
            ccSequence = dict((x.seqId, x.molResidue.chemComp) for x in chain.sortedResidues())
...
def matchSequences(molSystem, resIdToChemComp, chains=None):
    ...
    # get the mapping
    chains, mappings = findMatchingChains(molSystem, ccpCodes, excludeChains=chains)
    if mappings:
        chain = chains[0]
        mapping = mappings[0]
