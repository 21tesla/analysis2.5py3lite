...
from ccp.lib.MoleculeModify import addMolResidues, makeChain, makeMolecule, nextChainCode
from memops.universal.Util import returnFloat, returnInt

try:
    from memops.gui.MessageReporter import (
        showOkCancel,
        showWarning,  # , showError
        showYesNo,
    )
except:
    from memops.universal.MessageReporter import (
        showOkCancel,
        showWarning,  # , showError
        showYesNo,
    )
    ...
    if not pdbPaths:
        ...
        for pdbPath in pdbPaths:
            ...
            if dd:
                ...
                nSkipped += 1
                print("Error coordinate read failed. Skipping %s" % pdbPath)
        ...
        if data:
            ...
            return makeStructureEnsemble(data, molSystem, modelNames=modelNames, doWarnings=doWarnings)

        else:
            raise Exception("ERROR no PDB data read, %s files skipped" % nSkipped)


def makeStructureEnsemble(
    strucDict, molSystem, modelNames=None, doWarnings=False, checkTruncation=True, fixAtomNames=True
):
    ...
    for iChCode, chCode in enumerate(chCodes):
        ...
        seqIds = seqIdsGood
        msChains, mappings = findMatchingChains(
            molSystem, ccpCodes, excludeChains=usedChains, doWarning=doWarnings, molTypes=molTypes
        )
        ...
        for j, seqId in enumerate(seqIds):
            ...
            coordResidue = coordChain.newResidue(
                seqCode=msResidue.seqCode, seqInsertCode=msResidue.seqInsertCode, seqId=msResidue.seqId
            )
            ...
            for atomKey, coordDict in resDict[seqId].items():
                ...
                if key in msAtomDict:
                    ...
                    systemAtom = DataConvertLib.findMatchingMolSystemAtom(
                        atomName, msResidue, namingSystem, systemAtoms, fixAtomNames=fixAtomNames
                    )
                    msAtomDict[key] = systemAtom

                if (systemAtom is None) or atomCheckDict.get((systemAtom, altLoc)):
                    # print '### failing', atomName
                    failedAtoms.append("%s %d %s %4s" % (chCode, seqId, msResidue.ccpCode, atomName))
                    continue
                ...
                tt = (coordDict.get("recordId"), (chCode, seqId, atomKey), coordResidue, systemAtom.name, coordDict)
            ...
            for seqId, atomDict in sorted(resDict.items()):
                ...
                for atomKey, coordDict in atomDict.items():
                    ...
                    if tt is None:
                        ...
                        tt = (
                            coordDict.get("recordId"),
                            (chCode, seqId, atomKey),
                            coordResidue,
                            systemAtom.name,
                            coordDict,
                        )
    ...
    if failedAtoms:
        ...
        if ll:
            print(("## WARNING %s failed atoms. Unique, non-pesudo atoms are:" % len(failedAtoms), ", ".join(ll)))
...
def makePdbFromStructure(fileName, structure, model=None, useOxt=False):
  ...
  """
    ...
    for model in models:
        remark = "REMARK 210 CCPN ccp.molecule.MolStructure.Model %s" % model.getExpandedKey()
