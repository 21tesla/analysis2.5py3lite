...
from ccp.general.Geometry import calcTorsionAngleDegrees, calcTorsionAngleRadians
...
def makeEmptyEnsembleCopy(ensemble):
    ...
    dd = {}
    for tag in ("atomNamingSystem", "resNamingSystem", "softwareName", "details", "molSystem"):
...
def copyModelToEnsemble(ensemble, model, identicalEnsembles=False):
    ...
    if identicalEnsembles:
        ...
        if model.structureEnsemble.nAtoms != ensemble.nAtoms:
            showError("Ensemble Creation Failed", "nAtoms differs for 'identical' ensembles")
        ...
        for ii, atom in enumerate(model.structureEnsemble.orderedAtoms):
            ...
            if newChain is not None:
                ...
                if newRes is not None:
                    newAtom = newRes.findFirstAtom(name=atom.name, altLocationCode=atom.altLocationCode)

                # NB we do not compare residue types
                #  - if the model contains non-matching atoms the error will come there

            if newAtom is None:
                showError("Ensemble Creation Failed", "No match found for atom %s" % atom)
                return
...
def alignStructures(structures):
    ...
    from ccp.util.Validation import storeModelValidations, storeResidueValidations
    ...
    for structure in structures:
        ...
        validStore = getEnsembleValidationStore(structure, ANALYSIS_RMSD_CONTEXT, RMSD_KEYWORDS)
        storeModelValidations(validStore, ANALYSIS_RMSD_CONTEXT, RMSD_KEYWORDS[1], models, scores)
        j = k
...
def compareEnsembles(structure1, structure2, compareBackboneOnly=False):
    ...
    allCoordLists = _getMatchedStructureCoordinates(structure1, structure2, atomNameDict)
...
def getAtomSetCoords(atomSet, structure, model=None):
    ...
    key = "%s:%s:%d:%d" % (
        atomSet,  # Could be real AtomSet or a ConstraintSet one!
