    ...
    if len(sequence) > 1 and molType in ("protein", "DNA", "RNA"):
        ...
        for i in range(len(sequence)):
            ...
            if chemComp:
                ...
                molResidue = molecule.newMolResidue(
                    seqCode=i + startNum, chemComp=chemComp, linking=linking, descriptor=descriptor
                )
...
def makeLinearSequence(molecule, sequence, seqCodeStart=1, isCyclic=False):
    ...
    try:
        ...
        if isCyclic:
            ...
            molResData, otherLinkCodes = _getLinearChemCompData(project, molType, ccpCode, "start")

            molResidue = molecule.newMolResidue(seqCode=seqCode, serial=serial, **molResData)
            molResidues.append(molResidue)
        ...
        for seqTuple in doSequence:
            ...
            if seqTuple in chemCompData:
                ...
                molResData, otherLinkCodes = _getLinearChemCompData(project, molType, ccpCode, "middle")
        ...
        if not isCyclic:
            ...
            molResData, otherLinkCodes = _getLinearChemCompData(project, molType, ccpCode, "end")
        ...
        for second in range(1, len(sequence)):
            ...
            molResLinks.append(molecule.newMolResLink(molResLinkEnds=[nextLinkEnd, prevLinkEnd]))
...
def _getLinearChemCompData(project, molType, ccpCode, linking):
    ...
    chemCompVar = chemComp.findFirstChemCompVar(linking=linking, isDefaultVar=True) or chemComp.findFirstChemCompVar(
        linking=linking
    )
    ...
    if isOther and (chemCompVar is None):
        ...
        if linkEnd:
            ...
            chemCompVar = chemComp.findFirstChemCompVar(isDefaultVar=True) or chemComp.findFirstChemCompVar()

    if chemCompVar is None:
        raise ApiError("No ChemCompVar found for %s:%s linking %s" % (molType, ccpCode, linking))

    molResData = {"chemComp": chemComp, "linking": chemCompVar.linking, "descriptor": chemCompVar.descriptor}
...
def setMolResidueCcpCode(molResidue, ccpCode):
    ...
    chemCompVar = chemComp.findFirstChemCompVar(descriptor=molResidue.descriptor, linking=molResidue.linking)
    if not chemCompVar:
