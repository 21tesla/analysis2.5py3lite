...
# =========================================================================================
__copyright__ = (
    "Copyright (C) CCPN project (www.ccpn.ac.uk) 2014 - $Date: 2014-11-26 17:28:10 +0000 (Wed, 26 Nov 2014) $"
)
...
def _connectedShiftMeasurements(expMeasurement):
    ...
    if len(atomSites) != 1:
        print(
            "WARNING%s Shift must have single AtomSite, has: %s"
    ...
    result = [
        x
        for x in nmrExpPrototype.expMeasurements
        if x.measurementType.lower() == "shift" and x.findFirstAtomSite() in sites
    ]
    #
    return result
...
def _orderedMeasurements(nmrExpPrototype, forReversed=False):
    ...
    ll = [
        tt[1].expMeasurement for tt in sorted((x.stepNumber, x) for x in nmrExpPrototype.findFirstExpGraph().expSteps)
    ]
    if not forReversed:
        ll.reverse()
