...
def getBestChemComp(project, resName, atomNames, molType=None, download=True):
    ...
    chemComp = project.findFirstChemComp(ccpCode=resName.upper(), molType=molType) or project.findFirstChemComp(
        ccpCode=resName, molType=molType
    )
...
def findMatchingMolSystemAtom(atomName, residue, namingSystem, excludeAtoms, fixAtomNames=False):
    """
    ...
    moreNamingSystems = [x for x in priorityOrderedNamingSystems(chemComp) if x not in usedNamingSystems]
                ...
                for tag in tags:
                    ...
                    if prevId is None:
                        ...
                        if len(prevId[1]) == 1 and len(ccId[1]) > 1:
                            ...
                            raise Exception(
                                "Residue name conflict for %s between ChemComps %s and %s" % (tag, ccId, prevId)
                            )
        ...
        for ccpCode, tt in sorted(data.items()):
            ...
            for tag in tags:
                ...
                if prevId is None:
                    ...
                    raise Exception(
                        "Residue name conflict for %s between Entries %s and %s" % (tag, ccId, prevId), tags
                    )
