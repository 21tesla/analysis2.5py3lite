...
def createCoordinatesForResidue(coordDict, residue, sourceName="euroCarbDb", chemCompCoordArchiveDir=None):
    ...
    chemCompCoord = getChemCompCoord(
        residue.root,
        sourceName=sourceName,
        ccpCode=residue.ccpCode,
        molType=residue.molType,
        chemCompCoordArchiveDir=chemCompCoordArchiveDir,
    )
    ...
    if residue not in coordDict:
        coordDict[residue] = {}

        for chemAtomCoord in chemCompVarCoord.chemAtomCoords:
            coordDict[residue][(chemAtomCoord.chemAtom.className, chemAtomCoord.name, chemAtomCoord.subType)] = (
                chemAtomCoord.x,
                chemAtomCoord.y,
                chemAtomCoord.z,
            )
    ...
    for mrle in molResidue.molResLinkEnds:
        ...
        otherChemCompCoord = getChemCompCoord(
            otherResidue.root,
            sourceName=sourceName,
            ccpCode=otherResidue.ccpCode,
            molType=otherResidue.molType,
            chemCompCoordArchiveDir=chemCompCoordArchiveDir,
        )

        otherChemCompVar = otherMolResidue.chemCompVar
        otherChemCompVarCoord = otherChemCompCoord.findFirstChemCompVarCoord(
            linking=otherChemCompVar.linking, descriptor=otherChemCompVar.descriptor
        )
        ...
        otherChemAtomCoord = otherChemCompVarCoord.findFirstChemAtomCoord(
            name=otherChemAtom.name, subType=otherChemAtom.subType
        )
        otherLinkAtomCoord = otherChemCompVarCoord.findFirstChemAtomCoord(
            name=otherLinkAtom.name, subType=otherLinkAtom.subType
        )

        newVectorCoordAtoms = [otherLinkAtomCoord, otherChemAtomCoord]
        newVectors = [
            (otherLinkAtomCoord.x, otherLinkAtomCoord.y, otherLinkAtomCoord.z),
            (otherChemAtomCoord.x, otherChemAtomCoord.y, otherChemAtomCoord.z),
        ]

        for chemAtomCoord in otherChemCompVarCoord.chemAtomCoords:
            if chemAtomCoord not in newVectorCoordAtoms:
        ...
        for i in range(1, len(newVectorCoordAtoms)):
            ...
            coordDict[otherResidue][(chemAtomCoord.chemAtom.className, chemAtomCoord.name, chemAtomCoord.subType)] = (
                coords
            )
    ...
    for addedResidue in addedResidues:
        createCoordinatesForResidue(
            coordDict, addedResidue, sourceName=sourceName, chemCompCoordArchiveDir=chemCompCoordArchiveDir
        )
...
def createMolStructureFromChemCompCoords(chain, coordSourceName="euroCarbDb", chemCompCoordArchiveDir=None):
    ...
    createCoordinatesForResidue(
        coordDict, refResidue, sourceName=coordSourceName, chemCompCoordArchiveDir=chemCompCoordArchiveDir
    )
    ...
    for residue in coordDict.keys():
        coordResidue = coordChain.newResidue(
            seqCode=residue.seqCode, seqId=residue.seqId, seqInsertCode=residue.seqInsertCode
        )
