...
__copyright__ = (
    "Copyright (C) CCPN project (www.ccpn.ac.uk) 2014 - $Date: 2014-10-23 14:41:29 +0100 (Thu, 23 Oct 2014) $"
)
__credits__ = "Wayne Boucher, Rasmus H Fogh, Simon Skinner, Geerten Vuister"
__license__ = (
    "CCPN license. See www.ccpn.ac.uk/industry/licencefeesor ccpncore.memops.Credits.CcpnLicense for license text"
)
__reference__ = (
    "For publications, please use reference from www.ccpn.ac.uk/industry/licencefees"
    ...
    for resonanceGroup in nmrProject.sortedResonanceGroups():
        ...
        if residue:
            ...
            if chainMap:
                ...
                residue = chain.findFirstResidue(seqCode=residue.seqCode, seqInsertCode=residue.seqInsertCode)
        ...
        if resonanceGroup not in result:
            if not _findSpinSystemStretch(resonanceGroup, delta=+1):
                ...
                if not stretch:
                    ...
                    # Now set for rgprev
                    chemComp = rgprev.root.findFirstChemComp(molType=rgprev.molType, ccpCode=rgprev.ccpCode)
                    if chemComp:
                        residueType = chemComp.code3Letter
                    else:
                        residueType = None
                    ...
                    if rgnext is not None:
                        # Now set for rgnext
                        chemComp = rgnext.root.findFirstChemComp(molType=rgnext.molType, ccpCode=rgnext.ccpCode)
                        if chemComp:
                            residueType = chemComp.code3Letter
                        else:
                            residueType = None
    ...
    for resonanceGroup in nmrProject.sortedResonanceGroups():
        if resonanceGroup not in result:
            ...
            if (
                ...
                and rg0 is not None
                and not (assignment[1][-2] in "+-" and assignment[1][-1].isdigit())
            ):
                ...
                chemComp = resonanceGroup.root.findFirstChemComp(
                    molType=resonanceGroup.molType, ccpCode=resonanceGroup.ccpCode
                )
                if chemComp:
                    residueType = chemComp.code3Letter
                else:
            ...
            if chemAtomSet and len(atomSets) == 2 and len(resonances) <= 2:
                ...
                if topObject.className == "NmrProject":
                    ...
                    if len(resonances) == 2:
                        ...
                        for ii in range(2):
                            if resonance.name in (atomSetNames[ii], newNames[ii]) or resonance.name.upper() in (
                                atomSetNames[ii],
                                newNames[ii],
                            ):
                ...
                if len(atomSets) == 1:
                    ...
                    elif all((x.elementSymbol == nuc) for x in allAtoms) and len(
                        [x for x in residueChemAtoms if x.elementSymbol == nuc]
                    ) == len(allAtoms):
                ...
                for resonance in resonances:
                    ...
                    assignmentMap[resonance] = (chainCode, sequenceCode, residueType, resonanceName.replace("*", "#"))
            ...
            if assignNames[0][lenPrefix].isdigit() and assignNames[1][lenPrefix].isdigit():
                if assignNames[0][lenPrefix + 1 :] == assignNames[1][lenPrefix + 1 :]:
                    ...
                    if assignNames[0][lenPrefix] == char:
                        ...
                        newChar = "Y"
                    elif "a" in resonanceName and "b" not in resonanceName:
                        newChar = "X"
                    elif "b" in resonanceName and "a" not in resonanceName:
                        newChar = "Y"
...
def _mapUnAssignedResonances(nmrProject, assignmentMap, molSystem, chainMap=None):
    ...
    resonanceGroupMap = mapResonanceGroups(nmrProject, molSystem=molSystem, defaultChainCode=defaultChainCode)
...
def _findSpinSystemStretch(spinSystem, direction=1):
    ...
    while True:
        ll = []
        for link in stretch[-1].findAllResonanceGroupProbs(
            linkType="sequential", sequenceOffset=direction, isSelected=True
        ):
            ll.append(link.possibility)

        for link in stretch[-1].findAllFromResonanceGroups(
            linkType="sequential", sequenceOffset=-direction, isSelected=True
        ):
...
def _findIdentityResonanceGroup(resonanceGroup):
    ...
    ll = resonanceGroup.findAllResonanceGroupProbs(linkType="identity", isSelected=True)
    identicals = list(x.possibility for x in ll)
    ll2 = resonanceGroup.findAllFromResonanceGroups(linkType="identity", isSelected=True)
                        ...
                        for resonance2 in resonanceSet2.resonances:
                            if resonance2 is resonance:  # should not happen
                                if resonance not in funnyResonances:
                                    resonance.root._logger.warning(
                                        "in getBoundResonances():"
                                        "resonance %d tried to be linked to itself" % resonance.serial
                                    )
