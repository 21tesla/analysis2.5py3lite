...
def getShapeFile(spectrum):
    ...
    try:
        ...
        if ncomp != len(componentList):
            print("Warning: in %s, ncomp = %d, but found %d components" % (fileName, ncomp, componentList))
        ...
        for comp, (c, ampl, shapeData) in enumerate(componentList):
            ...
            for a, size, offset, values in shapeData:
                ...
                if shapeDim is None or (aa == shapeDim - 1):
                    ...
                    if len(values) != size:
                        raise OSError(
                            "in %s: comp = %d, a = %d, size = %d but found %d values"
                            % (fileName, c, a, size, len(values))
                        )
                    if shapeDim is None:
                        adim = 0
                    else:
...
def create1dShape(freqDataDim, applicationName):
    ...
    newDataStore = dataLocationStore.newShapeMatrix(
        dataUrl=dataStore.dataUrl,
        path=dataStore.path,
        numPoints=(freqDataDim.numPoints,),
        isBigEndian=dataStore.isBigEndian,
        numberType=dataStore.numberType,
        isComplex=(freqDataDim.isComplex,),
    ...
    newSpectrum = experiment.newDataSource(name=name, numDim=1, dataStore=newDataStore, dataType=spectrum.dataType)

    appData = AppDataInt(application=applicationName, keyword="shapeDim", value=dim)
    newSpectrum.addApplicationData(appData)

    newFreqDataDim = newSpectrum.newFreqDataDim(
        dim=dim,
        numPoints=freqDataDim.numPoints,
        isComplex=freqDataDim.isComplex,
        numPointsOrig=freqDataDim.numPointsOrig,
        valuePerPoint=freqDataDim.valuePerPoint,
        expDim=freqDataDim.expDim,
    )

    for dataDimRef in freqDataDim.dataDimRefs:
        newFreqDataDim.newDataDimRef(
            refPoint=dataDimRef.refPoint, refValue=dataDimRef.refValue, expDimRef=dataDimRef.expDimRef
        )
