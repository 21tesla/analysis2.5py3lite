...
PARAM_ATTR_DICT = {float: "floatValue", int: "intValue", str: "textValue", bool: "booleanValue"}
...
def setRunParameter(run, code, value, data=False):
    ...
    if data or data is None:
        runParameter = run.findFirstRunParameter(code=code, data=data) or run.newRunParameter(code=code, data=data)

    else:
        runParameter = run.findFirstRunParameter(code=code) or run.newRunParameter(code=code)
...
def getDataObjText(datum, missingText=DATA_MISSING):
    ...
    funcs = {
        "ConstraintStoreData": getConstraintStoreDataInfo,
