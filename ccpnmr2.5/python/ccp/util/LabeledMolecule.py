...
def setResidueScheme(molLabel, molResidue, scheme):
    ...
    resLabel.newResLabelFraction(schemeName=scheme.name, weight=1.0, isotopomerSerial=0)
...
def getExperimentAtomFractions(experiment, atom):
    ...
    for mixture in labelledMixtures:
        if mixture.labeledMolecule.molecule is molecule:
            ...
            atomName = atom.name
            fractionDict = singleAtomFractions(mixture, resId, atom.name)
            break
...
def getSchemeAtomFractions(scheme, atom):
    ...
    chemCompLabel = scheme.findFirstChemCompLabel(ccpCode=ccpCode, molType=molType)

    if chemCompLabel:
        isotopomers = chemCompLabel.isotopomers
        fractionDict = getIsotopomerSingleAtomFractions(isotopomers, atomName, subType)
    else:
...
def getExperimentAtomPairFractions(experiment, atom1, atom2):
    ...
    if molecule1 is molecule2:
        for mixture in labelledMixtures:
            if mixture.labeledMolecule.molecule is molecule1:
                fractionDict = atomPairFractions(mixture, resIds, atomNames)
                break

    else:
        fractionDict1 = getExperimentAtomFractions(experiment, atom1)
        fractionDict2 = getExperimentAtomFractions(experiment, atom2)
...
def getSchemeAtomPairFractions(scheme, atom1, atom2):
    ...
    if residue1 is residue2:
        ...
        chemCompLabel = scheme.findFirstChemCompLabel(ccpCode=ccpCode1, molType=molType1)
        ...
        chemCompLabel1 = scheme.findFirstChemCompLabel(ccpCode=ccpCode1, molType=molType1)
        chemCompLabel2 = scheme.findFirstChemCompLabel(ccpCode=ccpCode2, molType=molType2)

        if chemCompLabel1 and chemCompLabel2:
            isotopomers1 = chemCompLabel1.isotopomers
            fractionDict1 = getIsotopomerSingleAtomFractions(isotopomers1, atomName1, subType1)
            isotopomers2 = chemCompLabel2.isotopomers
            fractionDict2 = getIsotopomerSingleAtomFractions(isotopomers2, atomName2, subType2)
...
def atomPairFrac(labeledMixture, resIds, atNames, isotopeCodes):
    ...
    return atomPairFractions(labeledMixture, resIds, atNames).get(tuple(isotopeCodes))
...
def singleAtomFractions(labeledMixture, resId, atName):
    ...
    # set up
    molResidue = labeledMixture.labeledMolecule.molecule.findFirstMolResidue(serial=resId)
...
def atomPairFractions(
    ...
    if len(atNames) != 2:
        raise ("Error: length of atNames %s must be 2" % atNames)

    # calculate starting parameters
    elementNames = []
    subTypes = []
    for ii in (0, 1):
        molResidue = labeledMixture.labeledMolecule.molecule.findFirstMolResidue(serial=resIds[ii])
        if molResidue is None:
            return None
    ...
    for mlf in molLabelFractions:
        ...
        if resIds[0] == resIds[1]:
            ...
            for ii in (0, 1):
                if oneResLabel.findAllAtomLabels(atomName=atNames[ii]) or oneResLabel.findAllAtomLabels(
                    elementName=elementNames[ii]
                ):
        ...
        if uncorrelatedAtoms:
            ...
            for ii in (0, 1):
                ...
                dds.append(molLabelFractionsDict(resLabel, atNames[ii], subTypes[ii], elementNames[ii]))
...
def molLabelFractionsDict(resLabel, atName, subType, elementName):
    ...
    if atomLabels:
        ...
        for atomLabel in atomLabels:
            ...
            result[isotopeCode] = result.get(isotopeCode, 0.0) + atomLabel.weight / atWeightSum
...
def getIsotopomerAtomPairFractions(isotopomers, atomNames, subTypes):
    ...
    for isotopomer in isotopomers:
        for i in (0, 1):
            atLabels[i] = isotopomer.findAllAtomLabels(name=atomNames[i], subType=subTypes[i])
