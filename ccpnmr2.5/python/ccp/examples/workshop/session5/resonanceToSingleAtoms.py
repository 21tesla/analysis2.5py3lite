...
def printResonanceToAtoms(resonance):
    ...
    if resSet:
        ...
        for atomSet in resSet.atomSets:
            ...
            print(
                "        AtomSet '%s' with atoms %s (chain '%s', residue %d)"
                % (atomSet.name, str([atom.name for atom in atomSet.sortedAtoms()]), residue.chain.code, residue.seqId)
            )
