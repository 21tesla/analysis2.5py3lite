...
if __name__ == "__main__":
    ...
    for dc in dcl.sortedConstraints():
        ...
        print(
            "%-4d   %-10s %-10s %-10s"
            % (dc.serial, infoDict["lowerLimit"], infoDict["targetValue"], infoDict["upperLimit"])
        )
    ...
    for i in range(5):
        ...
        dihedralConstraintItem = dihedralConstraint.newDihedralConstraintItem(
            lowerLimit=100.0, upperLimit=140.0, targetValue=130.0
        )
        dihedralConstraintItem = dihedralConstraint.newDihedralConstraintItem(
            lowerLimit=300.0, upperLimit=20.0, targetValue=0.0
        )

    for dhdlConst in dhcl.sortedConstraints():
        print(
            "Constraint [%d]: resonances [%s] - [%s] - [%s] - [%s]"
            % (
                dhdlConst.serial,
                dhdlConst.resonances[0].serial,
                dhdlConst.resonances[1].serial,
                dhdlConst.resonances[2].serial,
                dhdlConst.resonances[3].serial,
            )
        )
        ...
        for dhdlConstItem in dhdlConst.sortedItems():
            print(
                "Item [%d]: [%.2f] - [%.2f]; [%.2f]"
                % (dhdlConstItem.serial, dhdlConstItem.lowerLimit, dhdlConstItem.upperLimit, dhdlConstItem.targetValue)
            )
