...
if __name__ == "__main__":
    ...
    cnsObj.readCoordinates([cnsFile], strucGen=strucGen, minimalPrompts=1, linkAtoms=0)
    ...
    classif = project.CurrentClassification = project.newClassification(namingSystem="local")
    ...
    sampStore = project.currentSampleStore = project.newSampleStore(name=project.name)

    # And make the new sample from the sampleStore with the sampleCategory.
    keywds = {"name": "mySample", "sampleCategories": [sampCat], "ph": 5.0}
    sample = sampStore.newSample(**keywds)

    # Reference store for making molecule components.
    refStore = project.currentRefSampleComponentStore = project.newRefSampleComponentStore(name=project.name)
    ...
    keywds2 = {"name": molecule.name, "molecule": molecule, "molType": "protein"}
    molComp = refStore.newMolComponent(**keywds2)

    # Concentration data for a sampleComponent for the protein molecule.
    keywds3 = {"concentration": 0.001, "concentrationError": 0.0002, "concentrationUnit": "M"}
    sampComp = sample.newSampleComponent(refComponent=molComp, **keywds3)

    labeledMol = project.newLabeledMolecule(name=molComp.name)

    molLabel = labeledMol.newMolLabel()

    labeledMix = labeledMol.newLabeledMixture()
    molComp.setLabeledMixture(labeledMix)

    molLabelFrac = labeledMix.newMolLabelFraction(molLabel=molLabel)

    for molRes in molecule.sortedMolResidues():
        resLabel = molLabel.newResLabel(resId=molRes.serial)

        # 99% Uniform 13C Labelling.

        weight = 0.99

        keywds4a = {"elementName": "C", "massNumber": 13, "weight": weight}

        uniformAtomLabel1 = resLabel.newUniformAtomLabel(**keywds4a)

        keywds4b = {"elementName": "C", "massNumber": 12, "weight": 1.00 - weight}

        if weight != 1.00:
            uniformAtomLabel2 = resLabel.newUniformAtomLabel(**keywds4b)

        # 98% Uniform 15N Labelling.

        weight = 0.98

        keywds5a = {"elementName": "N", "massNumber": 15, "weight": weight}

        uniformAtomLabel1 = resLabel.newUniformAtomLabel(**keywds5a)

        keywds5b = {"elementName": "N", "massNumber": 14, "weight": 1.00 - weight}

        if weight != 1.00:
            uniformAtomLabel2 = resLabel.newUniformAtomLabel(**keywds5b)

    # Old way of labelling the protein molecule.
    ...
    saltComp = refStore.newMolComponent(name="NaCl", details="NaCl")

    # New sampleComponent object connected to the substance object.
    keywds6 = {"concentration": 0.1, "concentrationUnit": "M"}
    ...
    keywds7 = {"value": 5.0, "condition": "pH"}
    sampCondPh = sampCondSet.newSampleCondition(**keywds7)

    keywds8 = {"value": 298, "condition": "Temperature", "unit": "K"}
    ...
    affStore = project.currentAffilitationStore = project.newAffiliationStore(name=project.name)

    manufacturer = affStore.newOrganisation(name="Varian")

    # instrumentStore for the NMR machine.
    instrStore = project.currentInstrumentStore = project.newInstrumentStore(name=project.name)
    ...
    methStore = project.currentMethodStore = project.newMethodStore(name=project.name)

    keywds10 = {"name": "assignment", "task": "NMR peak assignment"}
    method1 = methStore.newMethod(**keywds10)

    keywds11 = {"name": "peak picking", "task": "NMR peak picking"}
    method2 = methStore.newMethod(**keywds11)

    keywds12 = {"name": "Analysis", "vendorName": "ccpNmr", "version": "1.0", "methods": [method1, method2]}
    software = methStore.newSoftware(**keywds12)

    # Make a new BMRB Entry.
    entryStore = project.currentNmrEntryStore = project.newNmrEntryStore(name=project.name)
